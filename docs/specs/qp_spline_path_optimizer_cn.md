...
$$
\begin{aligned}
minimize  & \frac{1}{2}   x^T  H  x  + f^T  x \\
...
\begin{vmatrix} a_{i0} \\ a_{i1} \\ a_{i2} \\ a_{i3} \\ a_{i4} \\ a_{i5} \end{vmatrix}
$$
请注意我们最后得到一个6介的矩阵来表示5介样条插值的开销。应用同样的推理方法可以得到2介，3介样条插值的开销。

## 2  约束条件  

### 2.1  初始点约束

假设第一个点为 ($s_0$, $l_0$), ($s_0$, $l'_0$) and ($s_0$, $l''_0$)，其中$l_0$ , $l'_0$ and $l''_0$表示横向的偏移，并且规划路径的起始点的第一，第二个点的开销可以从$f_i(s)$, $f'_i(s)$, $f_i(s)''$计算得到。
...
和起始点相同，终点$(s_e, l_e)$ 也应当按照起始点的计算方法生成约束条件。将起始点和终点组合在一起，得出约束等式为：
