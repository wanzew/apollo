...
// The params is suitable after stereo camera calibration and left/right image
// remap.

struct StereoCalibrationParameters {
  double focus;  //(pixel) the camere focus in pixel. The focus value is all the
                 // same include left and right camera.

  double cx;  //(pixel) optical axis X center point. The optical axis center
              // point(X, Y) is all the same include left and right camera
