...
union UniqueKey {
  ...
  UniqueKey(uint32_t aLongkey) { longKey = aLongkey; }
  ...
  uint16_t lowWord() { return doubleKey.lowWord; }

  uint16_t highWord() { return doubleKey.highWord; }
...
struct BlockFileResp {
  uint32_t           received;
  uint16_t           continued;
  inline const char* fileName() { return (const char*)this + sizeof(BlockFileResp); }
};
