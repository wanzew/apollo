...
class Quaternion : public QuadWord {
 public:
  ...
  Quaternion& operator*=(const Quaternion& q) {
    setValue(m_floats[3] * q.x() + m_floats[0] * q.m_floats[3] + m_floats[1] * q.z() -
                 m_floats[2] * q.y(),
             m_floats[3] * q.y() + m_floats[1] * q.m_floats[3] + m_floats[2] * q.x() -
                 m_floats[0] * q.z(),
             m_floats[3] * q.z() + m_floats[2] * q.m_floats[3] + m_floats[0] * q.y() -
                 m_floats[1] * q.x(),
             m_floats[3] * q.m_floats[3] - m_floats[0] * q.x() - m_floats[1] * q.y() -
                 m_floats[2] * q.z());
    return *this;
  }
   ...
   * @param t The ratio between this and q to interpolate.  If t = 0 the result
   * is this, if t=1 the result is q. Slerp interpolates assuming constant
   * velocity.  */
  Quaternion slerp(const Quaternion& q, const tf2Scalar& t) const {
    ...
    if (theta != tf2Scalar(0.0)) {
      ...
      if (dot(q) < 0)  // Take care of long angle case see
                       // http://en.wikipedia.org/wiki/Slerp
        return Quaternion(
            (m_floats[0] * s0 + -q.x() * s1) * d, (m_floats[1] * s0 + -q.y() * s1) * d,
  ...
  static const Quaternion& getIdentity() {
    static const Quaternion identityQuat(tf2Scalar(0.), tf2Scalar(0.), tf2Scalar(0.),
                                         tf2Scalar(1.));
    return identityQuat;
  }
