...
// disable but used in MSCV Stl warning. 			#pragma warning(disable:4996) //Turn off
// warnings about deprecated C routines 			#pragma warning(disable:4786) // Disable
// the "debug name too long" warning
...
#define tf2RecipSqrt(x)                                                                            \
  ((tf2Scalar)(tf2Scalar(1.0) / tf2Sqrt(tf2Scalar(x)))) /* reciprocal square root */
...
tf2Select(unsigned condition, unsigned valueIfConditionNonZero, unsigned valueIfConditionZero) {
...
tf2Select(unsigned condition, int valueIfConditionNonZero, int valueIfConditionZero) {
  unsigned testNz  = (unsigned)(((int)condition | -(int)condition) >> 31);
  unsigned testEqz = ~testNz;
  return static_cast<int>((valueIfConditionNonZero & testNz) | (valueIfConditionZero & testEqz));
}
TF2SIMD_FORCE_INLINE float
tf2Select(unsigned condition, float valueIfConditionNonZero, float valueIfConditionZero) {
#ifdef TF2_HAVE_NATIVE_FSEL
  return (float)tf2Fsel((tf2Scalar)condition - tf2Scalar(1.0f), valueIfConditionNonZero,
                        valueIfConditionZero);
#else
