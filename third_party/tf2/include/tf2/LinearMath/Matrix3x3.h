...
class Matrix3x3 {
  ...
  */
  ...
  void setValue(const tf2Scalar& xx,
                const tf2Scalar& xy,
                const tf2Scalar& xz,
                const tf2Scalar& yx,
                const tf2Scalar& yy,
  ...
  void setRotation(const Quaternion& q) {
    ...
    setValue(tf2Scalar(1.0) - (yy + zz), xy - wz, xz + wy, xy + wz, tf2Scalar(1.0) - (xx + zz),
             yz - wx, xz - wy, yz + wx, tf2Scalar(1.0) - (xx + yy));
  }
  ...
  void setEulerYPR(tf2Scalar eulerZ, tf2Scalar eulerY, tf2Scalar eulerX) {
    ...
    setValue(cj * ch, sj * sc - cs, sj * cc + ss, cj * sh, sj * ss + cc, sj * cs - sc, -sj, cj * si,
             cj * ci);
  }
  ...
  void setIdentity() {
    setValue(tf2Scalar(1.0), tf2Scalar(0.0), tf2Scalar(0.0), tf2Scalar(0.0), tf2Scalar(1.0),
             tf2Scalar(0.0), tf2Scalar(0.0), tf2Scalar(0.0), tf2Scalar(1.0));
  }
   ...
   * @param pitch Pitch around Y axis
   * @param roll around X axis */
  void getEulerYPR(tf2Scalar&   yaw,
                   tf2Scalar&   pitch,
                   tf2Scalar&   roll,
                   unsigned int solution_number = 1) const {
    struct Euler {
    ...
    if (tf2Fabs(m_el[2].x()) >= 1) {
      ...
      euler_out.roll =
          tf2Atan2(m_el[2].y() / tf2Cos(euler_out.pitch), m_el[2].z() / tf2Cos(euler_out.pitch));
      euler_out2.roll =
          tf2Atan2(m_el[2].y() / tf2Cos(euler_out2.pitch), m_el[2].z() / tf2Cos(euler_out2.pitch));

      euler_out.yaw =
          tf2Atan2(m_el[1].x() / tf2Cos(euler_out.pitch), m_el[0].x() / tf2Cos(euler_out.pitch));
      euler_out2.yaw =
          tf2Atan2(m_el[1].x() / tf2Cos(euler_out2.pitch), m_el[0].x() / tf2Cos(euler_out2.pitch));
    }
   ...
   * less than the threshold multiplied by the sum of the absolute values of the
   * diagonal, or when maxSteps have been executed.
   *
  ...
  void diagonalize(Matrix3x3& rot, tf2Scalar threshold, int maxSteps) {
    ...
    for (int step = maxSteps; step > 0; step--) {
      ...
      if (max <= t) {
        if (max <= TF2SIMD_EPSILON * t) { return; }
        step = 1;
      }
...
TF2SIMD_FORCE_INLINE Matrix3x3 Matrix3x3::absolute() const {
  return Matrix3x3(tf2Fabs(m_el[0].x()), tf2Fabs(m_el[0].y()), tf2Fabs(m_el[0].z()),
...
TF2SIMD_FORCE_INLINE Matrix3x3 Matrix3x3::transpose() const {
  return Matrix3x3(m_el[0].x(), m_el[1].x(), m_el[2].x(), m_el[0].y(), m_el[1].y(), m_el[2].y(),
                   m_el[0].z(), m_el[1].z(), m_el[2].z());
}

TF2SIMD_FORCE_INLINE Matrix3x3 Matrix3x3::adjoint() const {
  return Matrix3x3(cofac(1, 1, 2, 2), cofac(0, 2, 2, 1), cofac(0, 1, 1, 2), cofac(1, 2, 2, 0),
                   cofac(0, 0, 2, 2), cofac(0, 2, 1, 0), cofac(1, 0, 2, 1), cofac(0, 1, 2, 0),
                   cofac(0, 0, 1, 1));
}
...
TF2SIMD_FORCE_INLINE Matrix3x3 Matrix3x3::inverse() const {
  ...
  return Matrix3x3(co.x() * s, cofac(0, 2, 2, 1) * s, cofac(0, 1, 1, 2) * s, co.y() * s,
                   cofac(0, 0, 2, 2) * s, cofac(0, 2, 1, 0) * s, co.z() * s, cofac(0, 1, 2, 0) * s,
                   cofac(0, 0, 1, 1) * s);
}
...
TF2SIMD_FORCE_INLINE Matrix3x3 Matrix3x3::timesTranspose(const Matrix3x3& m) const {
  return Matrix3x3(m_el[0].dot(m[0]), m_el[0].dot(m[1]), m_el[0].dot(m[2]), m_el[1].dot(m[0]),
                   m_el[1].dot(m[1]), m_el[1].dot(m[2]), m_el[2].dot(m[0]), m_el[2].dot(m[1]),
                   m_el[2].dot(m[2]));
}
...
TF2SIMD_FORCE_INLINE Matrix3x3 operator*(const Matrix3x3& m1, const Matrix3x3& m2) {
  return Matrix3x3(m2.tdotx(m1[0]), m2.tdoty(m1[0]), m2.tdotz(m1[0]), m2.tdotx(m1[1]),
