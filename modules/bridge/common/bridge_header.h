...
class BridgeHeader {
 ...
 public:
  ...
  hsize    GetHeaderSize() const {
    return static_cast<hsize>(header_body_size_ + HEADER_FLAG_SIZE + sizeof(hsize) + 2);
  }
  ...
  void SetHeaderVer(uint32_t header_ver) {
    ...
    header_body_size_ += static_cast<hsize>(Item_Header_Size + 1 + sizeof(uint32_t));
  }
  void SetMsgName(const std::string& msg_name) {
    msg_name_ = msg_name;
    header_body_size_ += static_cast<hsize>(Item_Header_Size + 1 + msg_name.length() + 1);
  }
  ...
  void SetFrameSize(bsize frame_size) {
    ...
    header_body_size_ += static_cast<hsize>(Item_Header_Size + 1 + sizeof(bsize));
  }
  ...
  void SetTimeStamp(double time_stamp) {
    ...
    header_body_size_ += static_cast<hsize>(Item_Header_Size + 1 + sizeof(double));
  }
