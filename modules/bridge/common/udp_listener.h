...
class UDPListener {
 public:
  ...
  ~UDPListener() {
    if (listener_sock_ != -1) { close(listener_sock_); }
  }
...
bool UDPListener<T>::Initialize(T* receiver, func msg_handle, uint16_t port) {
  ...
  if (listener_sock_ == -1) { return false; }
  ...
  if (bind(listener_sock_, (struct sockaddr*)&serv_addr, sizeof(struct sockaddr)) == -1) {
  ...
  while (true) {
    ...
    for (int i = 0; i < nfds; ++i) {
      if (events[i].data.fd == listener_sock_) {
        ...
        par->listener_ = this;
        if (pthread_create(&thread, &attr, &UDPListener<T>::pthread_handle_message,
...
bool UDPListener<T>::setnonblocking(int sockfd) {
  if (fcntl(sockfd, F_SETFL, fcntl(sockfd, F_GETFD, 0) | O_NONBLOCK) == -1) { return false; }
  return true;
}
...
void* UDPListener<T>::pthread_handle_message(void* param) {
  ...
  if (par) { delete par; }
  par = nullptr;
  if (!listener) { pthread_exit(nullptr); }
...
void UDPListener<T>::MessageHandle(int fd) {
  if (!receiver_ || !msg_handle_) { return; }
