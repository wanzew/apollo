...
bool Pt3dToCamera2d(const Eigen::Vector3d&   pt3d,
  ...
  if (local_pt[2] > 0) {
    *pt2d = (camera_model->Project(Eigen::Vector3f(static_cast<float>(local_pt[0]),
...
bool IsObjectEightVerticesAllBehindCamera(const std::shared_ptr<const base::Object>& obj,
                                          const Eigen::Matrix4d&   world2camera_pose,
                                          base::BaseCameraModelPtr camera_model) {
  ...
  for (const auto& vertice : vertices) {
    if ((Pt3dToCamera2d(vertice, world2camera_pose, camera_model, &pt2d))) { return false; }
  }
  return true;
}
...
float ObjectInCameraView(SensorObjectConstPtr     sensor_object,
  ...
  Eigen::Matrix4d world2sensor_pose = camera_sensor2world_pose.matrix().inverse();
  if (!world2sensor_pose.allFinite()) { return in_view_ratio; }
  double width  = static_cast<double>(camera_model->get_width());
  double height = static_cast<double>(camera_model->get_height());

  double          time_diff = camera_ts - sensor_object->GetBaseObject()->latest_tracked_time;
  Eigen::Vector3f offset = sensor_object->GetBaseObject()->velocity * static_cast<float>(time_diff);
  if (!motion_compensation) { offset.setZero(); }
  // 2.compute distance
  const auto&     cloud  = sensor_object->GetBaseObject()->lidar_supplement.cloud_world;
  Eigen::Vector3d center = sensor_object->GetBaseObject()->center;
  Eigen::Vector2d center2d;
  if (!Pt3dToCamera2d(center, world2sensor_pose, camera_model, &center2d)) { return in_view_ratio; }
  ...
  if (cloud.size() > 0) {
    ...
    for (int i = 0; i < point_num; ++i) {
      ...
      Eigen::Vector3d pt3d(pt.x + offset[0], pt.y + offset[1], pt.z + offset[2]);
      Eigen::Vector2d pt2d;
      bool            flag = Pt3dToCamera2d(pt3d, world2sensor_pose, camera_model, &pt2d);
      if (flag && IsPtInFrustum(pt2d, width, height)) { ++in_view_point_num; }
    }
    in_view_ratio = static_cast<float>(in_view_point_num / point_num);
    if (all_in) { in_view_ratio = (in_view_point_num == point_num) ? 1.0 : 0.0; }
    ...
    for (const auto& box3d_v : box3d_vs) {
      Eigen::Vector2d pt2d;
      bool            flag =
          Pt3dToCamera2d(box3d_v + offset.cast<double>(), world2sensor_pose, camera_model, &pt2d);
      if (!flag) { return in_view_ratio; }
    ...
    Eigen::Vector2d bound_top_left     = top_left.cwiseMax(Eigen::Vector2d(0.0, 0.0));
    Eigen::Vector2d bound_bottom_right = bottom_right.cwiseMin(Eigen::Vector2d(width, height));
    Eigen::Vector2d bound_box_size     = bound_bottom_right - bound_top_left;
    if ((bound_box_size.array() > 0.0).all()) {
      in_view_ratio = static_cast<float>(bound_box_size.prod() / box_size.prod());
    } else {
      in_view_ratio = 0.0;
    }
    if (all_in) { in_view_ratio = std::abs(1.0 - in_view_ratio) <= 1e-6 ? 1.0 : 0.0; }
  ...
  auto         sigmoid_like_fun = [max_dist, dist_slope](double obj_dist) {
    ...
    return 0.5 - 0.5 * x * dist_slope / std::sqrt(1 + x * x * dist_slope * dist_slope);
  };
