...
bool InformationFilter::Init(const Eigen::VectorXd& global_states,
  ...
  if (global_uncertainty.rows() != global_uncertainty.cols()) { return false; }

  states_num_ = static_cast<int>(global_uncertainty.rows());

  if (states_num_ <= 0) { return false; }

  if (states_num_ != global_states.rows()) { return false; }
...
bool InformationFilter::SetLastObservation(const Eigen::VectorXd& last_observation,
  ...
  if (last_observation.rows() != states_num_) { return false; }
  if (last_observation_uncertainty.rows() != states_num_) { return false; }
  if (last_observation_uncertainty.cols() != states_num_) { return false; }
  if (last_to_cur_transform_matrix.rows() != states_num_) { return false; }
  if (last_to_cur_transform_matrix.cols() != states_num_) { return false; }
  if (last_to_cur_env_uncertainty.rows() != states_num_) { return false; }
  if (last_to_cur_env_uncertainty.cols() != states_num_) { return false; }
...
bool InformationFilter::Predict(const Eigen::MatrixXd& transform_matrix,
  ...
  if (!init_) { return false; }
  if (transform_matrix.rows() != states_num_) { return false; }
  if (transform_matrix.cols() != states_num_) { return false; }
  if (env_uncertainty.rows() != states_num_) { return false; }
  if (env_uncertainty.cols() != states_num_) { return false; }
...
bool InformationFilter::Correct(const Eigen::VectorXd& cur_observation,
                                const Eigen::MatrixXd& cur_observation_uncertainty) {
  if (!init_) { return false; }
  if (cur_observation.rows() != states_num_) { return false; }
  if (cur_observation_uncertainty.rows() != states_num_) { return false; }
  if (cur_observation_uncertainty.cols() != states_num_) { return false; }
  ...
  if (last_observation_init_) {
    ...
    global_uncertainty_ =  // update information matrix
        ...
        (c_matrix_.transpose() * cur_observation_uncertainty_ * c_matrix_ -
         c_matrix_.transpose() * last_observation_uncertainty_.inverse() * c_matrix_);
    tmp_states_ +=  // update information vector
        (c_matrix_.transpose() * cur_observation_uncertainty_ * cur_observation_ -
         c_matrix_.transpose() * last_observation_uncertainty_.inverse() * last_observation_);
  } else {
    global_uncertainty_ += c_matrix_.transpose() * cur_observation_uncertainty_ * c_matrix_;
    tmp_states_ += c_matrix_.transpose() * cur_observation_uncertainty_ * cur_observation_;
  }
...
bool InformationFilter::SetControlMatrix(const Eigen::MatrixXd& control_matrix) {
  if (!init_) { return false; }
