...
TEST(CameraUtilTest, test_is_pt_in_frustum) {
  ...
  for (size_t i = 0; i < 3; i++) {
    for (size_t j = 0; j < 3; j++) {
      EXPECT_EQ(IsPtInFrustum<Eigen::Vector2f>(Eigen::Vector2f(x[i], y[j]), width, height),
                result[i] & result[j]);
    }
  }
}
...
TEST(CameraUtilTest, test_object_in_camera_view_and_is_behind_camera) {
  ...
  SensorObjectPtr lidar_sensor_object(new SensorObject(base_lidar_object, lidar_sensor_frame));
  ...
  bool flag1 = IsObjectEightVerticesAllBehindCamera(base_lidar_object, pose.matrix(), camera_model);
  EXPECT_FALSE(flag1);

  float view_ratio1 =
      ObjectInCameraView(lidar_sensor_object, camera_model, pose, 0.0, 100, false, false);
  float view_ratio2 =
      ObjectInCameraView(lidar_sensor_object, camera_model, pose, 0.0, 100, true, false);
  float view_ratio3 =
      ObjectInCameraView(lidar_sensor_object, camera_model, pose, 0.0, 100, false, true);
  float view_ratio4 =
      ObjectInCameraView(lidar_sensor_object, camera_model, pose, 0.0, 100, true, true);
  ...
  bool flag2 = IsObjectEightVerticesAllBehindCamera(base_lidar_object, pose.matrix(), camera_model);
  EXPECT_TRUE(flag2);
}
