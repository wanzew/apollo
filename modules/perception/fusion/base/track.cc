...
SensorObjectConstPtr Track::GetSensorObject(const std::string& sensor_id) const {
  ...
  if (lidar_it != lidar_objects_.end()) { return lidar_it->second; }

  auto radar_it = radar_objects_.find(sensor_id);
  if (radar_it != radar_objects_.end()) { return radar_it->second; }

  auto camera_it = camera_objects_.find(sensor_id);
  if (camera_it != camera_objects_.end()) { return camera_it->second; }
...
SensorObjectConstPtr Track::GetLatestSensorObject(const SensorId2ObjectMap& objects) const {
  ...
  for (auto it = objects.begin(); it != objects.end(); ++it) {
    if (obj == nullptr || obj->GetTimestamp() < it->second->GetTimestamp()) { obj = it->second; }
  }
  return obj;
}
...
void Track::UpdateWithSensorObject(const SensorObjectPtr& obj) {
  ...
  if (is_background_) { return UpdateWithSensorObjectForBackground(obj); }
...
void Track::UpdateWithoutSensorObject(const std::string& sensor_id, double measurement_timestamp) {
  ...
  is_alive_ = (!lidar_objects_.empty()) || (!radar_objects_.empty()) || (!camera_objects_.empty());
}
...
void Track::UpdateSupplementState(const SensorObjectPtr& src_object) {
  ...
  if (lidar_objects_.empty()) { dst_obj->lidar_supplement.Reset(); }
  if (radar_objects_.empty()) { dst_obj->radar_supplement.Reset(); }
  if (camera_objects_.empty()) { dst_obj->camera_supplement.Reset(); }
}
...
bool Track::IsLidarVisible() const {
  for (auto it = lidar_objects_.begin(); it != lidar_objects_.end(); ++it) {
    if (it->second->GetInvisiblePeriod() < 1.0e-6) { return true; }
  }
  return false;
}
...
void Track::UpdateWithSensorObjectForBackground(const SensorObjectPtr& obj) {
  std::shared_ptr<base::Object>       fused_base_object       = fused_object_->GetBaseObject();
  std::shared_ptr<const base::Object> measurement_base_object = obj->GetBaseObject();
