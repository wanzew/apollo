...
double TrackObjectDistance::s_lidar2lidar_association_center_dist_threshold_     = 10.0;
double TrackObjectDistance::s_lidar2radar_association_center_dist_threshold_     = 10.0;
double TrackObjectDistance::s_radar2radar_association_center_dist_threshold_     = 10.0;
size_t TrackObjectDistance::s_lidar2camera_projection_downsample_target_pts_num_ = 100;
size_t TrackObjectDistance::s_lidar2camera_projection_vertices_check_pts_num_    = 20;
...
bool TrackObjectDistance::QueryLidar2WorldPose(const SensorObjectConstPtr& lidar,
  ...
  if (!lidar->GetRelatedFramePose(&velo2world_pose)) { return false; }
  (*pose) = velo2world_pose.matrix();
  return true;
}

ProjectionCacheObject*
TrackObjectDistance::BuildProjectionCacheObject(const SensorObjectConstPtr&     lidar,
                                                const SensorObjectConstPtr&     camera,
  ...
  if (!QueryWorld2CameraPose(camera, &world2camera_pose)) { return nullptr; }
  Eigen::Matrix4d lidar2world_pose;
  if (!QueryLidar2WorldPose(lidar, &lidar2world_pose)) { return nullptr; }
...
ProjectionCacheObject*
TrackObjectDistance::QueryProjectionCacheObject(const SensorObjectConstPtr&     lidar,
                                                const SensorObjectConstPtr&     camera,
                                                const base::BaseCameraModelPtr& camera_model,
...
void TrackObjectDistance::QueryProjectedVeloCtOnCamera(const SensorObjectConstPtr& velodyne64,
  ...
  Eigen::Vector3d        offset  = velodyne64->GetBaseObject()->velocity.cast<double>() * time_diff;
  const Eigen::Vector3d& velo_ct = velodyne64->GetBaseObject()->center;
  Eigen::Vector4d        projected_ct_4d = static_cast<Eigen::Matrix<double, 4, 1, 0, 4, 1>>(
      lidar2camera_pose *
      Eigen::Vector4d(velo_ct[0] + offset[0], velo_ct[1] + offset[1], velo_ct[2] + offset[2], 1.0));
  *projected_ct = projected_ct_4d.head(3);
}

bool TrackObjectDistance::QueryPolygonDCenter(const base::ObjectConstPtr& object,
                                              const Eigen::Vector3d&      ref_pos,
                                              const int                   range,
                                              Eigen::Vector3d*            polygon_ct) {
  if (object == nullptr) { return false; }
  const base::PolygonDType& polygon = object->polygon;
  if (!ComputePolygonCenter(polygon, ref_pos, range, polygon_ct)) { return false; }
  return true;
}
...
bool TrackObjectDistance::IsTrackIdConsistent(const SensorObjectConstPtr& object1,
  ...
  if (object1->GetBaseObject()->track_id == object2->GetBaseObject()->track_id) { return true; }
  return false;
}
...
bool TrackObjectDistance::LidarCameraCenterDistanceExceedDynamicThreshold(
  ...
  if (center_distance > dynamic_threshold) { return true; }
  return false;
}
...
float TrackObjectDistance::Compute(const TrackPtr&                   fused_track,
  ...
  if (IsLidar(sensor_object)) {
    ...
    if (camera_object != nullptr) {
      bool is_lidar_track_id_consistent = IsTrackIdConsistent(lidar_object, sensor_object);
      distance =
          ComputeLidarCamera(sensor_object, camera_object, true, is_lidar_track_id_consistent);
      min_distance = std::min(distance, min_distance);
    }
    ...
    if (lidar_object != nullptr) {
      bool is_camera_track_id_consistent = IsTrackIdConsistent(camera_object, sensor_object);
      distance =
          ComputeLidarCamera(lidar_object, sensor_object, false, is_camera_track_id_consistent);
      min_distance = std::min(distance, min_distance);
    }
...
float TrackObjectDistance::ComputeRadarRadar(const SensorObjectPtr& fused_object,
                                             const SensorObjectPtr& sensor_object,
                                             const Eigen::Vector3d& ref_pos,
                                             int                    range) {
  double center_distance =
...
float TrackObjectDistance::ComputeLidarCamera(const SensorObjectConstPtr& lidar,
  ...
  if (!is_track_id_consistent) {
    if (LidarCameraCenterDistanceExceedDynamicThreshold(lidar, camera)) { return distance_thresh_; }
  }
  ...
  const base::PointFCloud& cloud          = lidar->GetBaseObject()->lidar_supplement.cloud;
  const base::BBox2DF&     camera_bbox    = camera->GetBaseObject()->camera_supplement.box;
  ...
  if (cloud.size() > 0) {
    ...
    double similarity = ComputePtsBoxSimilarity(&projection_cache_, cache_object, camera_bbox);
    distance          = distance_thresh_ * ((1.0f - static_cast<float>(similarity)) /
    ...
    QueryProjectedVeloCtOnCamera(lidar, camera, lidar2camera_pose, &projected_velo_ct);
    if (projected_velo_ct[2] > 0) {
      Eigen::Vector2f project_pt2f = camera_model->Project(Eigen::Vector3f(
          static_cast<float>(projected_velo_ct[0]), static_cast<float>(projected_velo_ct[1]),
      ...
      distance = static_cast<float>(ct_diff.norm()) * vc_diff2distance_scale_factor_;
    } else {
      distance = std::numeric_limits<float>::max();
    }
  }
...
float TrackObjectDistance::ComputeRadarCamera(const SensorObjectConstPtr& radar,
  ...
  Eigen::Matrix4d world2camera_pose;
  if (!QueryWorld2CameraPose(camera, &world2camera_pose)) { return distance; }
...
double TrackObjectDistance::ComputeLidarCameraSimilarity(const SensorObjectConstPtr& lidar,
  ...
  Eigen::Matrix4d world2camera_pose;
  if (!QueryWorld2CameraPose(camera, &world2camera_pose)) { return similarity; }
  Eigen::Matrix4d lidar2world_pose;
  if (!QueryLidar2WorldPose(lidar, &lidar2world_pose)) { return similarity; }
  ...
  if (cloud.size() > 0) {
    ProjectionCacheObject* cache_object =
        QueryProjectionCacheObject(lidar, camera, camera_model, measurement_is_lidar);
    if (cache_object == nullptr) { return similarity; }
...
bool TrackObjectDistance::ComputePolygonCenter(const base::PolygonDType& polygon,
