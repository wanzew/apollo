...
bool ProbabilisticFusion::Init(const FusionInitOptions& init_options) {
  ...
  if (!GetFusionInitOptions("ProbabilisticFusion", &options)) { return false; }
...
bool ProbabilisticFusion::Fuse(const FusionOptions&          options,
  ...
  // 1. save frame data
  {
    ...
    if (!params_.use_lidar && sensor_data_manager->IsLidar(sensor_frame)) { return true; }
    if (!params_.use_radar && sensor_data_manager->IsRadar(sensor_frame)) { return true; }
    if (!params_.use_camera && sensor_data_manager->IsCamera(sensor_frame)) { return true; }
    ...
    if (!is_publish_sensor) { return true; }
  }
  ...
  for (size_t i = 0; i < unassigned_obj_inds.size(); ++i) {
    ...
    std::for_each(params_.prohibition_sensors.begin(), params_.prohibition_sensors.end(),
                  [&](std::string sensor_name) {
                    if (sensor_name == frame->GetSensorId()) prohibition_sensor_flag = true;
                  });
    ...
    ADEBUG << "object id: " << frame->GetForegroundObjects()[obj_ind]->GetBaseObject()->track_id
...
void ProbabilisticFusion::RemoveLostTrack() {
  ...
  AINFO << "Remove " << foreground_tracks.size() - foreground_track_count << " foreground tracks. "
        << foreground_track_count << " tracks left.";
  ...
  AINFO << "Remove " << background_tracks.size() - background_track_count << " background tracks";
...
void ProbabilisticFusion::CollectFusedObjects(double                        timestamp,
  ...
  for (size_t i = 0; i < background_tracks.size(); ++i) {
    if (gate_keeper_->AbleToPublish(background_tracks[i])) {
      this->CollectObjectsByTrack(timestamp, background_tracks[i], fused_objects);
      ++bg_obj_num;
    }
  }
...
void ProbabilisticFusion::CollectObjectsByTrack(double                        timestamp,
  ...
  *obj                                          = *(fused_object->GetBaseObject());
  ...
  int num_measurements = static_cast<int>(lidar_measurements.size() + camera_measurements.size() +
...
void ProbabilisticFusion::CollectSensorMeasurementFromObject(
  ...
  if (IsCamera(object)) { measurement->box = object->GetBaseObject()->camera_supplement.box; }
}
