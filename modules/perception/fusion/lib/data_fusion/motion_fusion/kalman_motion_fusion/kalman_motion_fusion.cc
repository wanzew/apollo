...
bool KalmanMotionFusion::Init() {
  if (track_ref_ == nullptr) { return false; }
...
bool KalmanMotionFusion::InitFilter(const SensorObjectConstPtr& sensor_object) {
  ...
  if (!kalman_filter_.Init(global_states, global_uncertainty)) { return false; }
...
void KalmanMotionFusion::GetStates(Eigen::Vector3d* anchor_point, Eigen::Vector3d* velocity) {
...
void KalmanMotionFusion::UpdateWithMeasurement(const SensorObjectConstPtr& measurement,
                                               double                      target_timestamp) {
...
void KalmanMotionFusion::MotionFusionWithoutMeasurement(const double time_diff) {
...
void KalmanMotionFusion::MotionFusionWithMeasurement(const SensorObjectConstPtr& measurement,
                                                     double                      time_diff) {
  ...
  ADEBUG << "fusion_original_measurement@(" << std::setprecision(10) << observation(0) << ","
         << observation(1) << "," << observation(2) << "," << observation(3) << ")";
  ADEBUG << "fusion_original_measurement_covariance@(" << r_matrix(0, 0) << "," << r_matrix(0, 1)
         << "," << r_matrix(1, 0) << "," << r_matrix(1, 1) << "," << r_matrix(2, 2) << ","
         << r_matrix(2, 3) << "," << r_matrix(3, 2) << "," << r_matrix(3, 3) << ")";
  ...
  ADEBUG << "fusion_pseudo_measurement@(" << std::setprecision(10) << observation(0) << ","
         << observation(1) << "," << observation(2) << "," << observation(3) << ")";
  ADEBUG << "fusion_pseudo_measurement_covariance@(" << r_matrix(0, 0) << "," << r_matrix(0, 1)
         << "," << r_matrix(1, 0) << "," << r_matrix(1, 1) << "," << r_matrix(2, 2) << ","
         << r_matrix(2, 3) << "," << r_matrix(3, 2) << "," << r_matrix(3, 3) << ")";
  ...
  ADEBUG << "fusion_filter_belief@(" << std::setprecision(10) << kalman_filter_.GetStates()(0)
         << "," << kalman_filter_.GetStates()(1) << "," << kalman_filter_.GetStates()(2) << ","
         << kalman_filter_.GetStates()(3) << ")";
  ADEBUG << "fusion_filter_belief_covariance@(" << kalman_filter_.GetUncertainty()(0, 0) << ","
         << kalman_filter_.GetUncertainty()(0, 1) << "," << kalman_filter_.GetUncertainty()(1, 0)
         << "," << kalman_filter_.GetUncertainty()(1, 1) << ","
         << kalman_filter_.GetUncertainty()(2, 2) << "," << kalman_filter_.GetUncertainty()(2, 3)
...
Eigen::VectorXd KalmanMotionFusion::ComputeAccelerationMeasurement(
    const base::SensorType& sensor_type, const Eigen::Vector3d& velocity, const double& timestamp) {
...
Eigen::Vector4d KalmanMotionFusion::ComputePseudoMeasurement(const Eigen::Vector4d&  measurement,
  ...
  common::SensorManager* sensor_manager = common::SensorManager::Instance();
  if (sensor_manager->IsLidar(sensor_type)) { return ComputePseudoLidarMeasurement(measurement); }
  if (sensor_manager->IsRadar(sensor_type)) { return ComputePseudoRadarMeasurement(measurement); }
  if (sensor_manager->IsCamera(sensor_type)) { return ComputePseudoCameraMeasurement(measurement); }
...
Eigen::Vector4d
KalmanMotionFusion::ComputePseudoLidarMeasurement(const Eigen::Vector4d& measurement) {
  ...
  if (lidar_velocity_norm < kDoubleEpsilon) { return pseudo_measurement; }
  ...
  for (size_t count = 1; count < history_sensor_type_.size(); ++count) {
    ...
    if (common::SensorManager::Instance()->IsRadar(history_type)) {
      ...
      if (fabs(velocity_angle_change) > velocity_angle_change_thresh_) { continue; }
      ...
      if (fabs(acceleration_angle_change) > acceleration_angle_change_thresh_) { continue; }
...
Eigen::Vector4d
KalmanMotionFusion::ComputePseudoCameraMeasurement(const Eigen::Vector4d& measurement) {
  // Initialize status variables
  int             trace_count                       = 0;
  ...
  if (camera_velocity_norm < kDoubleEpsilon) { return pseudo_measurement; }
  // Trace back radar velocity history, try to find good radar measurement
...
Eigen::Vector4d
KalmanMotionFusion::ComputePseudoRadarMeasurement(const Eigen::Vector4d& measurement) {
  // Initialize status variables
  int             lidar_trace_count  = 0;
  int             camera_trace_count = 0;
  Eigen::Vector4d pseudo_measurement = measurement;
  int lidar_camera_history_length    = GetSensorHistoryLength(base::SensorType::VELODYNE_64) +
  ...
  for (size_t count = 1; count < history_sensor_type_.size(); ++count) {
    ...
    if (sensor_manager->IsLidar(history_type)) {
      ...
      if (fabs(velocity_angle_change) > M_PI / 20) { continue; }
      // Abandon lidar measurement, if its acceleration angle change is too
      // big
      if (fabs(acceleration_angle_change) > M_PI / 3) { continue; }
      // Compute normalized velocity angle change
      ...
      if (fabs(velocity_angle_change) > M_PI / 10) { continue; }
...
void KalmanMotionFusion::UpdateSensorHistory(const base::SensorType& sensor_type,
                                             const Eigen::Vector3d&  velocity,
                                             const double&           timestamp) {
  int lidar_history_length = GetSensorHistoryLength(base::SensorType::VELODYNE_64);
  int radar_history_length = GetSensorHistoryLength(base::SensorType::LONG_RANGE_RADAR);
...
int KalmanMotionFusion::GetSensorHistoryLength(const base::SensorType& sensor_type) {
  int sensor_history_length = 0;
  for (size_t i = 0; i < history_sensor_type_.size(); ++i) {
    if (history_sensor_type_[i] == sensor_type) { ++sensor_history_length; }
  }
  return sensor_history_length;
}

int KalmanMotionFusion::GetSensorHistoryIndex(const base::SensorType& sensor_type,
  ...
  for (size_t i = 1; i <= history_sensor_type_.size(); ++i) {
    ...
    if (history_sensor_type_[history_index] == sensor_type) { ++history_count; }
    if (history_count == trace_length) { return history_index; }
  }
