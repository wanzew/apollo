...
TEST(LanelineCalibratorTest, laneline_calibrator_test) {
  ...
  EXPECT_FALSE(
      adu::perception::obstacle::load_ref_camera_k_mat(".", k_mat, &image_width, &image_height));
  EXPECT_TRUE(adu::perception::obstacle::load_ref_camera_k_mat(intrinsics_file, k_mat, &image_width,
                                                               &image_height));
  AINFO << "----intrinsics:\n"
  ...
  EXPECT_TRUE(adu::perception::obstacle::load_filename(lane_path, "", &lane_list));
  EXPECT_TRUE(adu::perception::obstacle::load_filename(lane_path, ".txt", &lane_list));
  int nr_frames = lane_list.size();
  image_list.resize(0);
  for (int i = 0; i < nr_frames; ++i) {
    int         slash_pos = lane_list[i].find_last_of("/");
    int         dot_pos   = lane_list[i].find_last_of(".");
    std::string token     = lane_list[i].substr(slash_pos + 1, dot_pos - slash_pos - 1);
  ...
  EXPECT_FALSE(camera::LoadCamera2WorldTfs("abc", &frame_list, &time_stamps, &camera2world));
  EXPECT_TRUE(camera::LoadCamera2WorldTfs(tf_file, &frame_list, &time_stamps, &camera2world));
  ...
  calibrator.calibrator_.GetIntersectionFromTwoLineSegments(line_seg_l, line_seg_r, &v_point);

  LaneLine line;
  line.lane_point.resize(0);
  float line_seg[4] = {0};
  calibrator.calibrator_.SelectTwoPointsFromLineForVanishingPoint(line, line_seg);
  int num_points = 20;
  line.lane_point.resize(0);
  ...
  for (int i = 0; i < nr_frames; ++i) {
    ...
    camera::EgoLane ego_lane;
    if (!LoadLaneDet(lane_list[i], &ego_lane)) { continue; }
    ...
    if (FLAGS_vis) {
      ...
      // draw lane pts
      camera::draw_lane_pts(ego_lane.left_line.lane_point, kColorLaneLine_left, &test_image);
      camera::draw_lane_pts(ego_lane.right_line.lane_point, kColorLaneLine_right, &test_image);
    }
    ...
    calibrator_options.camera2world_pose.reset(new Eigen::Affine3d(frame.camera2world_pose));
    ...
    for (int j = 0; j < num_left_points; ++j) {
      frame.lane_objects[0].curve_image_point_set[j].x = ego_lane.left_line.lane_point[j](0);
      frame.lane_objects[0].curve_image_point_set[j].y = ego_lane.left_line.lane_point[j](1);
    }
    calibrator_options.lane_objects =
        std::make_shared<std::vector<base::LaneLine>>(frame.lane_objects);
    calibrator_options.camera2world_pose.reset(new Eigen::Affine3d(frame.camera2world_pose));
    ...
    for (int j = 0; j < num_right_points; ++j) {
      frame.lane_objects[1].curve_image_point_set[j].x = ego_lane.right_line.lane_point[j](0);
      frame.lane_objects[1].curve_image_point_set[j].y = ego_lane.right_line.lane_point[j](1);
    }
    ...
    if (update) { pitch_calib = pitch_angle; }
    ...
    const std::string timediff_yawrate_velocity_text =
        absl::StrCat("time_diff_: ", std::to_string(calibrator.GetTimeDiff()).substr(0, 4),
    ...
    if (FLAGS_vis) {
      adu::perception::obstacle::write_text_on_image(
          &test_image, 100.0f, 100.0f, timediff_yawrate_velocity_text.c_str(), font, kColorText);
    ...
    if (update) {
      ...
      if (FLAGS_vis) {
        std::string pitch_calib_text = "pitch: " + std::to_string(pitch_calib).substr(0, 8);
        adu::perception::obstacle::write_text_on_image(&test_image_half_result, 50.0f, 100.0f,
                                                       pitch_calib_text.c_str(), font, kColorText);
        EXPECT_TRUE(camera::draw_vanishing_row_on_image(kColorVaniLine_res, vanishing_row_res / 2,
                                                        &test_image_half_result));
        EXPECT_FALSE(
            camera::draw_vanishing_row_on_image(kColorVaniLine_res, -1, &test_image_half_result));
        EXPECT_FALSE(camera::draw_vanishing_row_on_image(
            kColorVaniLine_res, test_image_half_result.cols, &test_image_half_result));
      } else {
        cv::Mat image_fake(10, 10, CV_8UC3, cv::Scalar::all(100));
        EXPECT_TRUE(camera::draw_vanishing_row_on_image(kColorVaniLine_res, image_fake.cols / 2,
                                                        &image_fake));
        EXPECT_FALSE(camera::draw_vanishing_row_on_image(kColorVaniLine_res, -1, &image_fake));
        EXPECT_FALSE(
            camera::draw_vanishing_row_on_image(kColorVaniLine_res, image_fake.cols, &image_fake));
      }
    }
