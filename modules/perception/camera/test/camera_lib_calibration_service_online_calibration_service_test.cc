...
TEST(OnlineCalibrationServiceTest, online_calibration_service_test) {
  ...
  name_intrinsic_map.insert(std::pair<std::string, Eigen::Matrix3f>("onsemi_obstacle", intrinsic));
  ...
  BaseCalibrationService* calibration_service =
      BaseCalibrationServiceRegisterer::GetInstanceByName("OnlineCalibrationService");
  ...
  if (online_calib_service != nullptr) {
    ...
    online_calib_service->SetCameraHeightAndPitch(name_camera_ground_height_map,
                                                  name_camera_pitch_angle_diff_map, 0.f);
    ...
    EXPECT_TRUE(online_calib_service->QueryPoint3dOnGroundPlane(x, y, &point3d));
