...
static bool load_camera_k_mat_and_ground_pitch_height(const std::string& fname,
                                                      float              k_mat[9],
                                                      int*               image_width,
                                                      int*               image_height,
                                                      float*             camera_pitch,
                                                      float*             camera_height) {
  if (image_width == nullptr || image_height == nullptr || camera_pitch == nullptr ||
      camera_height == nullptr) {
...
static bool load_label_from_file(const std::string& filename, std::vector<ObjectInfo>* labels) {
  labels->clear();
  ...
  ObjectInfo label_in;
  while (fin >> label_in.type >> label_in.truncated >> label_in.occluded >> label_in.alpha >>
         label_in.bbox[0] >> label_in.bbox[1] >> label_in.bbox[2] >> label_in.bbox[3] >>
         label_in.size_hwl[0] >> label_in.size_hwl[1] >> label_in.size_hwl[2] >>
         label_in.location[0] >> label_in.location[1] >> label_in.location[2] >>
         label_in.rotation_y >> label_in.score) {
...
TEST(LocationRefinedObstaclePostProcessorTestOnlineCalibration,
  ...
  std::string fname_k_mat = "/apollo/modules/perception/testdata/"
                            "camera/lib/obstacle/postprocessor/location_refiner/"
                            "params/k_mat.txt";
  std::string fname_detection = "/apollo/modules/perception/testdata/"
                                "camera/lib/obstacle/postprocessor/location_refiner/"
                                "data/detection.txt";
  float                   k_mat[9]      = {0};
  ...
  load_camera_k_mat_and_ground_pitch_height(fname_k_mat, k_mat, &width, &height, &camera_pitch,
                                            &camera_height);
  ...
  // postprocessor
  BaseObstaclePostprocessor* postprocessor = BaseObstaclePostprocessorRegisterer::GetInstanceByName(
      "LocationRefinerObstaclePostprocessor");
  EXPECT_FALSE(postprocessor->Init(postprocessor_init_options));
  postprocessor_init_options.root_dir = "/apollo/modules/perception/testdata/"
                                        "camera/lib/obstacle/postprocessor/location_refiner/";
  ...
  for (auto& obj : detections) {
    if (std::find(candidate_type_list.begin(), candidate_type_list.end(), obj.type) ==
        candidate_type_list.end()) {
      continue;
    ...
    float theta_ray = static_cast<float>(atan2(point_in_camera.x(), point_in_camera.z()));
  ...
  intrinsic << k_mat[0], k_mat[1], k_mat[2], k_mat[3], k_mat[4], k_mat[5], k_mat[6], k_mat[7],
      k_mat[8];
  std::map<std::string, Eigen::Matrix3f> name_intrinsic_map;
  name_intrinsic_map.insert(std::pair<std::string, Eigen::Matrix3f>("front_12mm", intrinsic));
  CalibrationServiceInitOptions calibration_service_init_options;
  calibration_service_init_options.calibrator_working_sensor_name = "front_12mm";
  ...
  online_calib_service.SetCameraHeightAndPitch(name_camera_ground_height_map,
                                               name_camera_pitch_angle_diff_map, camera_pitch);
  frame.calibration_service = dynamic_cast<BaseCalibrationService*>(&online_calib_service);
  EXPECT_TRUE(postprocessor->Process(postprocessor_options, &frame));
