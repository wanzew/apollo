...
TEST(TwodThreedUtilTest, GetJaccardIndexTest) {
  ...
  float bbox_ref[4] = {200, 200, 400, 400};
  {
    ...
    EXPECT_NEAR(GetJaccardIndex(bbox_ref, bbox[0], bbox[1], bbox[2], bbox[3]), 0.f, eps);
  }
  {
  ...
  }
  {
    ...
    EXPECT_NEAR(GetJaccardIndex(bbox_ref, bbox[0], bbox[1], bbox[2], bbox[3]), 0.25f, eps);
  }
}
...
TEST(TwodThreedUtilTest, GetScoreViaRotDimensionCenterTest) {
  ...
  float score = GetScoreViaRotDimensionCenter(k_mat, image_width, image_height, bbox_ref, rot, hwl8,
                                              object_center1, check_truncation);
  EXPECT_NEAR(score, 0.f, 1e-5);

  check_truncation = true;
  score = GetScoreViaRotDimensionCenter(k_mat, image_width, image_height, bbox_ref, rot, hwl8,
                                        object_center1, check_truncation);
  EXPECT_NEAR(score, 0.f, 1e-5);

  score = GetScoreViaRotDimensionCenter(k_mat, image_width, image_height, bbox_ref, rot, hwl8,
                                        object_center1, check_truncation, bbox_res, bbox_near);
  EXPECT_NEAR(score, 0.f, 1e-5);
}
...
TEST(TwodThreedUtilTest, GetDxDzForCenterFromGroundLineSegTest) {
  {
    int   width    = 1920;
    int   height   = 1080;
    float k_mat[9] = {1975.43f, 0.0f, 958.069f, 0.0f, 1979.28f, 460.072f, 0.0f, 0.0f, 1.0f};
    LineSegment2D<float> ls(0.0f, 0.0f, 0.1f, 0.0f);
    float                plane[4]       = {0};
    float                pts_c[12]      = {0};
    float                ratio_x_over_z = 1.0f;
    float                dx_dz[2]       = {0};
    GetDxDzForCenterFromGroundLineSeg(ls, plane, pts_c, k_mat, width, height, ratio_x_over_z, dx_dz,
                                      true);
  ...
  }
  {
    int   width    = 1193;
    int   height   = 712;
    float k_mat[9] = {1447.44f, 0.0f, 495.503f, 0.0f, 1446.67f, 381.524f, 0.0f, 0.0f, 1.0f};
    LineSegment2D<float> ls(165.481f, 482.044f, 580.937f, 482.044f);
    float                plane[4]  = {0.0f, 1.0f, 0.0f, -1.5f};
    float                pts_c[12] = {7.03179f, 0.281616f, 13.5518f, 5.49636f, 0.281616f, 13.5518f,
                       5.49636f, 0.281616f, 17.5286f, 7.03179f, 0.281616f, 17.5286f};
  ...
  }
  {
    ...
    float                plane[4]  = {0.0f, 1.0f, 0.0f, -1.5f};
    float                pts_c[12] = {5.74431f, 0.84582f, 11.623f,  6.78823f, 0.84582f, 10.1878f,
                       2.88452f, 0.84582f, 7.34827f, 1.8406f,  0.84582f, 8.78343f};
  ...
  }
  {
    ...
    float pts_c[12]      = {2.49833f, 0.0501389f, 21.9282f, 3.86523f,  0.0501389f, 21.4518f,
                       2.33395f, 0.0501389f, 17.058f,  0.967046f, 0.0501389f, 17.5344f};
