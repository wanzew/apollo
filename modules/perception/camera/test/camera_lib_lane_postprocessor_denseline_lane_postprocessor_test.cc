...
TEST(DenselineLanePostprocessor, camera_lane_postprocessor_point_test) {
  ...
  for (int i = 1; i < 5; i++) {
    ...
    std::vector<LanePointInfo> laneline_infer_set = lane_postprocessor->GetAllInferLinePointSet();
    ...
    lane_postprocessor->GetLaneCCs(&lane_map, &lane_map_width, &lane_map_height, &ccs, &select_ccs);
    // delete detector;
  }
}
