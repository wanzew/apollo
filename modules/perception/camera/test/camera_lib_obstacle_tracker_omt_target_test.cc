...
TEST(TargetTest, target_test) {
  ...
  std::string   omt_config =
      cyber::common::GetAbsolutePath("/apollo/modules/perception/testdata/"
  ...
  ASSERT_EQ(target1.get_object(-1)->object->sub_type, base::ObjectSubType::TRAFFICCONE);
...
TEST(TargetTest, clapping_velocity_test) {
  ...
  auto read_pos_and_theta_vec = [](const std::string& fname) -> std::vector<std::vector<double>> {
  ...
  // case 1
  {
    ...
    for (size_t i = 0; i < pos_and_theta_vec.size(); ++i) {
      ...
      object->center << pos_and_theta_vec[i][0], pos_and_theta_vec[i][1], pos_and_theta_vec[i][2];

      frame.detected_objects.push_back(object);
      frame.frame_id      = static_cast<int>(i);
      frame.timestamp     = ts;
      frame.data_provider = &provider;
      if (i > 0) { target.Predict(&frame); }

      TrackObjectPtr track_obj(new TrackObject);
      track_obj->indicator = PatchIndicator(static_cast<int>(i), 0, sensor_name);
      track_obj->object    = object;
  ...
  // case 5 -- pedestrian type
  {
    ...
    for (size_t i = 0; i < pos_and_theta_vec.size(); ++i) {
      ...
      target.lost_age = 0;
      target.UpdateType(&frame);
      ASSERT_EQ(target.get_object(-1)->object->sub_type, base::ObjectSubType::PEDESTRIAN);
      target.Update2D(&frame);
      // unchanged template
      target.Update3D(&frame);
      ASSERT_EQ(target.Size(), i + 1);
