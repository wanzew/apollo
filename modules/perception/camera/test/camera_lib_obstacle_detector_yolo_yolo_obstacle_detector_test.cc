...
TEST(YoloCameraDetectorTest, demo_test) {
  ...
  ACHECK(frame.data_provider->FillImageData(cv_img.rows, cv_img.cols, image.gpu_data(), "bgr8"));
  ...
  for (auto obj : frame.detected_objects) {
    ...
    fprintf(stderr,
            ...
            obj->camera_supplement.box.xmax, obj->camera_supplement.box.ymax, obj->size[2],
            obj->size[1], obj->size[0], obj->type_probs[static_cast<int>(obj->type)], 0,
            obj->camera_supplement.visible_ratios[0], obj->camera_supplement.visible_ratios[1],
            obj->camera_supplement.visible_ratios[2], obj->camera_supplement.visible_ratios[3],
            obj->camera_supplement.cut_off_ratios[0], obj->camera_supplement.cut_off_ratios[1]);
...
TEST(YoloCameraDetectorTest, inference_init_test) {
  ...
  init_options.root_dir = "/apollo/modules/perception/testdata/"
                          "camera/lib/obstacle/detector/yolo/data/";
  init_options.conf_file      = "config.pt";
  std::string     config_path = GetAbsolutePath(init_options.root_dir, init_options.conf_file);
