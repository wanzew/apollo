...
TEST(darkSCNNLanePostprocessor, camera_lane_postprocessor_point_test) {
  ...
  postprocessor_init_options.detect_config_root = "/apollo/modules/perception/testdata/"
                                                  "camera/lib/lane/postprocessor/darkSCNN/data/";
  ...
  calibration_service_init_options.calibrator_working_sensor_name = "onsemi_obstacle";
  ...
  LoadExtrinsics("/apollo/modules/perception/testdata/"
                 "camera/lib/lane/postprocessor/darkSCNN/params/"
  ...
  EXPECT_TRUE(visualize_.Init_all_info_single_camera(
      camera_names, visual_camera, intrinsic_map, extrinsic_map, ex_lidar2imu, pitch_adj, yaw_adj,
      roll_adj, calibration_service_init_options.image_height,
  ...
  for (int i = 1; i < 5; i++) {
    std::string impath = cv::format("/apollo/modules/perception/testdata/"
    ...
    if (enable_visualization_) {
      ...
      visualize_.ShowResult_all_info_single_camera(img, frame, motion_buffer, world2camera);
    }
