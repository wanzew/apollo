...
TEST(CommonFunctions, poly_fit_error_test) {
  ...
  }
  {
    ...
    for (int i = 0; i < n; i++) {
      Eigen::Matrix<float, 2, 1> pos;
      pos << static_cast<float>(i), static_cast<float>(pow(i, 3) + pow(i, 2) + i);
      pos_vec.push_back(pos);
    }
...
TEST(CommonFunctions, FindKSmallValueTest) {
  ...
  for (int i = 0; i < nsize; i++) {
    ...
    float value      = 0.32f * static_cast<float>(rand_value) + static_cast<float>(rand_value);
    value_vec.push_back(value);
    snprintf(tmp, sizeof(tmp), "[%d %f] ", i, value);
    msg += tmp;
  }
...
TEST(CommonFunctions, FindKLargeValueTest) {
  ...
  for (int i = 0; i < nsize; i++) {
    int   rand_value = rand_r(&seed) % 10;
    float value      = 0.2f * static_cast<float>(rand_value) + static_cast<float>(rand_value);
    value_vec.push_back(value);
    snprintf(tmp, sizeof(tmp), "[%d %f] ", i, value);
    msg += tmp;
  }
