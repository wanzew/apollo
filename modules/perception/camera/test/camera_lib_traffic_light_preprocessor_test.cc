...
class TLPreprocessorTest : public ::testing::Test {
 protected:
  ...
  void PrepareTestDataLongFocus(CarPose* pose, std::vector<base::PointXYZID>* boundary) {
    pose->pose_ << 0.970765, -0.000792166, 0.240032, 408528, -0.239319, 0.0738927, 0.968126,
        4.31098e+06, -0.0185036, -0.997266, 0.0715428, -2.67629, 0, 0, 0, 1;
  ...
  void PrepareTestDataShortFocus(CarPose* pose, std::vector<base::PointXYZID>* boundary) {
    pose->pose_ << 0.969254, -0.0103669, 0.245844, 408563, -0.245933, -0.00847109, 0.96925,
        4.3111e+06, -0.00796562, -0.999911, -0.0107602, -2.93461, 0, 0, 0, 1;
...
TEST_F(TLPreprocessorTest, test_project_lights) {
  ...
  // empty signals
  {
    ...
    ASSERT_TRUE(preprocessor_->ProjectLights(pose, camera_name, &lights, &lights_on_image,
  ...
  // invalid camera id
  {
    ...
    ASSERT_FALSE(preprocessor_->ProjectLights(pose, camera_name, &lights, &lights_on_image,
                                              &lights_outside_image));
  }
  ...
  // long focus project outside image
  {
    ...
    preprocessor_->SetCameraWorkingFlag(camera_name, true);
    PrepareTestDataLongFocus(&pose, &(lights[0]->region.points));
    pose.c2w_poses_[camera_name](0, 3) = pose.c2w_poses_[camera_name](0, 3) + 100000;
    ASSERT_TRUE(preprocessor_->ProjectLights(pose, camera_name, &lights, &lights_on_image,
                                             &lights_outside_image));
...
TEST_F(TLPreprocessorTest, test_select_camera) {
...

  {
    ...
    // no lights info
    preprocessor_->SelectCamera(&lights_on_image_array, &lights_outside_image_array, option,
                                &selection);
...

  {
    ...
    std::string camera_name = "onsemi_traffic";
    ASSERT_TRUE(preprocessor_->SetCameraWorkingFlag(camera_name, true));
    ASSERT_TRUE(preprocessor_->ProjectLights(pose, camera_name, &lights, &lights_on_image_array[0],
  ...
  // test project on image border region
  {
    ...
    ASSERT_TRUE(preprocessor_->ProjectLights(pose, long_cam_id, &lights, &lights_on_image_array[0],
                                             &lights_outside_image_array[0]));
...
TEST_F(TLPreprocessorTest, UpdateLightsProjectionTest) {
  ...
  // no lights
  {
    ...
    option.image_borders_size = &image_border_sizes;
    ASSERT_TRUE(preprocessor_->UpdateLightsProjection(pose, option, "onsemi_traffic", &lights));
  }
  // invalid camera name
  {
    TLPreprocessorOption option;
    ...
    option.image_borders_size = &image_border_sizes;
    ASSERT_FALSE(preprocessor_->UpdateLightsProjection(pose, option, "invalid", &lights));
  }
  // project out of valid region
  {
    ...
    option.image_borders_size            = &image_border_sizes;
    ASSERT_FALSE(preprocessor_->UpdateLightsProjection(pose, option, "onsemi_traffic", &lights));
