...
int read_detections(const std::string& path,
                    const int&         feature_dim,
                    const std::string& camera_name,
                    CameraFrame*       frame) {
  const TemplateMap& kMinTemplateHWL = ObjectTemplateManager::Instance()->MinTemplateHWL();
  ...
  for (int i = 0; i < det_count; ++i) {
    ...
    float* data = frame->track_feature_blob->mutable_cpu_data() + i * feature_size;
...
int write_track_imgs(const std::string&                  out_path,
                     const cv::Mat&                      frame,
                     const std::vector<base::ObjectPtr>& tracked_objects) {
  cv::Mat image_mat_src = frame.clone();
  for (auto& visual_object : tracked_objects) {
    cv::Rect rect(
        visual_object->camera_supplement.box.xmin, visual_object->camera_supplement.box.ymin,
        visual_object->camera_supplement.box.xmax - visual_object->camera_supplement.box.xmin,
        visual_object->camera_supplement.box.ymax - visual_object->camera_supplement.box.ymin);
...
bool LoadExtrinsics(const std::string& yaml_file, Eigen::Matrix4d* camera_extrinsic) {
...
bool GetProjectMatrix(const std::string&                            camera_name,
  ...
  *project_matrix = intrinsic_map.at(base_camera_name).cast<double>() *
  ...
  if (pitch_diff != nullptr) {
    Eigen::Vector3d euler = (extrinsic_map.at(base_camera_name).block<3, 3>(0, 0).inverse() *
                             extrinsic_map.at(camera_name).block<3, 3>(0, 0))
                                .eulerAngles(0, 1, 2);
    *pitch_diff = euler(0);
...
bool LoadCameraIntrinsics(const std::string& yaml_file, Eigen::Matrix3f* camera_intrinsic) {
  if (!apollo::cyber::common::PathExists(yaml_file)) {
    AINFO << yaml_file << " not exist!";
    return false;
  }
...
TEST(FusionObstacleTrackerTest, FusionObstacleTracker_test) {
  ...
  for (int i = 0; i < camera_names.size(); i++) {
    ...
    ASSERT_TRUE(LoadCameraIntrinsics(
        FLAGS_data_root + "/params/" + camera_names[i] + "_intrinsics.yaml", &intrinsic));
    intrinsic_map[camera_names[i]] = intrinsic;
    Eigen::Matrix4d extrinsic;
    ASSERT_TRUE(LoadExtrinsics(FLAGS_data_root + "/params/" + camera_names[i] + "_extrinsics.yaml",
                               &extrinsic));
  ...
  std::vector<std::vector<base::CameraObjectSupplement>> tracked_results(det_gts.size());
  ...
  while (fin >> line) {
    ...
    std::string image_path =
        FLAGS_image_root + "/" + camera_name + "/" + image_name + FLAGS_image_ext;
    ...
    ASSERT_TRUE(
        GetProjectMatrix(camera_name, extrinsic_map, intrinsic_map, &project_matrix, &pitch_diff));
    ...
    AINFO << "Frame " << frame_num << " tracked object size: " << frame.tracked_objects.size();
  ...
  for (frame_num = dynamic_cast<OMTObstacleTracker*>(camera_tracker.get())
    ...
    for (int i = 0; i < det_gts[frame_num].size(); ++i) {
      if (ids[i] < -50) {
        ids[i] =
            det_gts[frame_num][i].local_track_id - tracked_results[frame_num][i].local_track_id;
      } else {
