...
void GetGroundPlanePitchHeight(const float&              baseline,
  ...
  if (ground4[3] > 0.0f) { common::IScale4(ground4.data(), -1.0f); }
...
GroundPlaneTracker::GroundPlaneTracker(int track_length) {
  if (track_length <= 0) { AERROR << "track_length, " << track_length << ", should be positive"; }
  ...
  for (int i = track_length - 1; i >= 0; --i) {
    const_weight_temporal_.at(i) = common::IPow(common::ISqrt(2.0f), track_length - 1 - i);
  }

  // normalize
  float accm_sum = common::ISum(const_weight_temporal_.data(), track_length);
  common::IScale(const_weight_temporal_.data(), track_length, common::IRec(accm_sum));

  head_ = track_length;
}

void GroundPlaneTracker::Push(const std::vector<float>& ph, const float& inlier_ratio) {
  CHECK_EQ(ph.size(), 2U);
  int i      = 0;
...
void GroundPlaneTracker::Restart() {
  unsigned int track_length = static_cast<unsigned int>(pitch_height_inlier_tracks_.size() / 3);
...
bool CameraGroundPlaneDetector::DetetGround(
    float pitch, float camera_height, float* vd, int count_vd, const std::vector<float>& plane) {
  ...
  if (!plane.empty()) {  // assigned from outside
    ...
    AINFO << "set ground plane from outside: " << plane[0] << ", " << plane[1] << ", " << plane[2]
          << ", " << plane[3];
    ...
    if (CameraGroundPlaneDetector::DetectGroundFromSamples(vd, count_vd, &inlier_ratio)) {
      ADEBUG << "l: " << l_[0] << ", " << l_[1] << ", " << l_[2];
      ground3.assign(l_, l_ + 3);
      GetGroundPlanePitchHeight(baseline_, k_mat, ground3, &ph[0], &ph[1]);
      ADEBUG << "ph: " << ph[0] << ", " << ph[1];
      success = fabs(ph[0]) < params_.max_tilt_angle && ph[1] < params_.max_camera_ground_height;
      if (success) {
    ...
    if (ground_plane_tracker_->GetCurTrackLength() > 0) {
      ...
      GetGround3FromPitchHeight(k_mat, baseline_, pitch, camera_height, &ground3);
      FillGroundModel(ground3);
    }
...
bool CameraGroundPlaneDetector::DetectGroundFromSamples(float* vd,
  ...
  *inlier_ratio = 0.0f;
  ...
  if (!common::RobustBinaryFitRansac<float, 1, 1, 2, 2, GroundHypoGenFunc<float>,
                                     GroundFittingCostFunc<float>, nullptr>(
          vs, ds, count_vd, p, &nr_inliers, inliers, kThresInlier, false, true, 0.99f,
          kMinInlierRatio)) {
    memset(l_, 0, sizeof(float) * 3);
    return false;
  } else {
    *inlier_ratio = static_cast<float>(nr_inliers) * common::IRec(static_cast<float>(count_vd));
  }
