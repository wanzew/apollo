...
bool Equal(double x, double target, double eps) { return std::abs(x - target) < eps; }
bool Equal(float x, float target, float eps) { return std::abs(x - target) < eps; }
  ...
  for (int dst_y = 0; dst_y < height; dst_y++) {
    for (int dst_x = 0; dst_x < width; dst_x++) {
      ...
      for (int c = 0; c < channel; c++) {
        ...
        src_reg   = src[idx11 + c];
        out       = out + (static_cast<float>(x2) - src_x) * (static_cast<float>(y2) - src_y) *
                        static_cast<float>(src_reg);
        int idx12 = (y1_read * stepwidth + x2_read) * channel;
        src_reg   = src[idx12 + c];
        out       = out + static_cast<float>(src_reg) * (src_x - static_cast<float>(x1)) *
                        (static_cast<float>(y2) - src_y);

        int idx21 = (y2_read * stepwidth + x1_read) * channel;
        src_reg   = src[idx21 + c];
        out       = out + static_cast<float>(src_reg) * (static_cast<float>(x2) - src_x) *
...
std::string GetCyberWorkRoot() {
  ...
  if (work_root.empty()) { work_root = cyber::common::GetEnv("CYBER_PATH"); }
  return work_root;
}
...
void FillObjectPolygonFromBBox3D(base::Object* object_ptr) {
  ...
  object_ptr->polygon[0].x = x1 * cos_theta + y1 * sin_theta + object_ptr->center[0];
  object_ptr->polygon[0].y = y1 * cos_theta - x1 * sin_theta + object_ptr->center[1];
  object_ptr->polygon[0].z = 0.0;

  object_ptr->polygon[1].x = x1 * cos_theta + y2 * sin_theta + object_ptr->center[0];
  object_ptr->polygon[1].y = y2 * cos_theta - x1 * sin_theta + object_ptr->center[1];
  object_ptr->polygon[1].z = 0.0;

  object_ptr->polygon[2].x = x2 * cos_theta + y2 * sin_theta + object_ptr->center[0];
  object_ptr->polygon[2].y = y2 * cos_theta - x2 * sin_theta + object_ptr->center[1];
  object_ptr->polygon[2].z = 0.0;

  object_ptr->polygon[3].x = x2 * cos_theta + y1 * sin_theta + object_ptr->center[0];
  object_ptr->polygon[3].y = y1 * cos_theta - x2 * sin_theta + object_ptr->center[1];
