...
bool IsCoveredHorizon(const base::Rect<T>& rect1, const base::Rect<T>& rect2, float thresh) {
  base::RectF inter = rect1 & rect2;
  if (inter.Area() > 0) { return inter.width / rect1.width > thresh; }
  return false;
}
template <typename T>
bool IsCoveredVertical(const base::Rect<T>& rect1, const base::Rect<T>& rect2, float thresh) {
  base::RectF inter = rect1 & rect2;
  if (inter.Area() > 0) { return inter.height / rect1.height > thresh; }
  return false;
}
...
bool OutOfValidRegion(const base::BBox2D<T> box,
  ...
  if (box.xmin < border_size || box.ymin < border_size) { return true; }
  if (box.xmax + border_size > width || box.ymax + border_size > height) { return true; }
  return false;
}
template <typename T>
...
void RefineBox(const base::Rect<T>& box_in, const T width, const T height, base::Rect<T>* box_out) {
  ...
  *box_out = box_in;
  ...
  box_out->width = (box_out->x + box_out->width <= width) ? box_out->width : width - box_out->x;
  box_out->height =
      (box_out->y + box_out->height <= height) ? box_out->height : height - box_out->y;
  if (box_out->width < 0) { box_out->width = 0; }
  if (box_out->height < 0) { box_out->height = 0; }
}
...
void CalculateMeanAndVariance(const std::vector<T>& data, T* mean, T* variance) {
  ...
  *mean = sum / data.size();
  ...
  T sum_of_diff_sqrs =
      std::inner_product(diff.begin(), diff.end(), diff.begin(), static_cast<T>(0));
