...
bool UndistortionHandler::Init(const std::string& sensor_name, int device) {
  ...
  if (!set_device(device)) { return false; }
  ...
  InitUndistortRectifyMap(
      distort_model->get_intrinsic_params(), distort_model->get_distort_params(), I,
      distort_model->get_intrinsic_params(), width_, height_, &d_mapx_, &d_mapy_);

  inited_ = true;
  return true;
}

bool UndistortionHandler::Handle(const base::Image8U& src_img, base::Image8U* dst_img) {
  if (!inited_) { return false; }

  if (!set_device(device_)) { return false; }
  ...
  switch (src_img.channels()) {
    case 1:
      status = nppiRemap_8u_C1R(src_img.gpu_data(), image_size, src_img.width_step(), remap_roi,
                                d_mapx_.gpu_data(), d_map_step, d_mapy_.gpu_data(), d_map_step,
                                dst_img->mutable_gpu_data(), dst_img->width_step(), image_size,
                                remap_mode);
      break;
    case 3:
      status = nppiRemap_8u_C3R(src_img.gpu_data(), image_size, src_img.width_step(), remap_roi,
                                d_mapx_.gpu_data(), d_map_step, d_mapy_.gpu_data(), d_map_step,
                                dst_img->mutable_gpu_data(), dst_img->width_step(), image_size,
                                remap_mode);
      break;
    default: AERROR << "Invalid number of channels: " << src_img.channels(); return false;
  }
...
void UndistortionHandler::InitUndistortRectifyMap(const Eigen::Matrix3f&           camera_model,
  ...
  for (int v = 0; v < height_; ++v) {
    ...
    for (int u = 0; u < width_; ++u) {
      ...
      xy1 << (static_cast<float>(u) - ncx) / nfx, (static_cast<float>(v) - ncy) / nfy, 1;
      auto   XYW      = Rinv * xy1;
      ...
      double nnx      = nx * scale + 2 * p1 * nx * ny + p2 * (r_square + 2 * nx * nx);
      double nny      = ny * scale + p1 * (r_square + 2 * ny * ny) + 2 * p2 * nx * ny;
