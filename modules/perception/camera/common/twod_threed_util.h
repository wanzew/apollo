...
template <typename T>
T GetJaccardIndex(
    const T* bbox_ref, const T& x_min, const T& y_min, const T& x_max, const T& y_max) {
  ...
  if (overlap_y <= (T)0 || overlap_x <= (T)0) { return (T)0; }
  T inter_area = overlap_x * overlap_y;
  T union_area = (y_max - y_min) * (x_max - x_min) +
                 (bbox_ref[3] - bbox_ref[1]) * (bbox_ref[2] - bbox_ref[0]) - inter_area;
...
bool Occlude(const T* bbox1, const T& h1, const T* bbox2, const T& h2) {
  ...
  if (overlap_y <= (T)0 || overlap_x <= (T)0) { return false; }
  ...
  bool    occ = occ_ratio > thres_occ_ratio && h1 * common::IRec(bh1) < h2 * common::IRec(bh2);
  return occ;
}
...
template <typename T>
T GetScoreViaRotDimensionCenter(const T*    k_mat,
                                int         width,
                                int         height,
                                const T*    bbox,
                                const T*    rot,
...
template <typename T>
void UpdateOffsetZ(
    T x_start, T z_start, T x_end, T z_end, const std::pair<T, T>& range, T* z_offset) {
  ...
  if (z_nearest < *z_offset) { *z_offset = z_nearest; }
}
...
void GetDxDzForCenterFromGroundLineSeg(const LineSegment2D<T>& ls,
  ...
  bool is_front_l =
      common::IBackprojectPlaneIntersectionCanonical(ls.pt_start, k_mat, plane, pt_of_line_seg_l);
  bool is_front_r =
      common::IBackprojectPlaneIntersectionCanonical(ls.pt_end, k_mat, plane, pt_of_line_seg_r);
  if (!is_front_l || !is_front_r) { return; }
  ADEBUG << "l&r on-ground points: " << pt_of_line_seg_l[0] << ", " << pt_of_line_seg_l[1] << ", "
         << pt_of_line_seg_l[2] << " | " << pt_of_line_seg_r[0] << ", " << pt_of_line_seg_r[1]
         << ", " << pt_of_line_seg_r[2];
  ...
  T v[3]   = {pt_of_line_seg_r[0] - pt_of_line_seg_l[0], pt_of_line_seg_r[1] - pt_of_line_seg_l[1],
  ...
  if (z_offset < Z_RESOLUTION && z_offset > -Z_RESOLUTION) { return; }

  // judge & convert back to camera coordinates
  if (z_offset > static_cast<T>(0.0f) && check_lowerbound) { return; }
  ...
  T dz_local = -z_offset;
  T dx_local = -l[1] * dz_local * common::IRec(l[0]); /*enforce to be along the ray*/
  ...
  if (z_offset > static_cast<T>(0.0f) && std::abs(dx_dz[1]) > dz_max) { dx_dz[0] = dx_dz[1] = 0; }
}
