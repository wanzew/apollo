...
class DenselineLanePostprocessor : public BaseLanePostprocessor {
 public:
  ...
  void GetLaneCCs(std::vector<unsigned char>*      lane_map,
                  int*                             lane_map_width,
                  int*                             lane_map_height,
 ...
 private:
  ...
  void
  CalLaneMap(const float* output_data, int width, int height, std::vector<unsigned char>* lane_map);
  ...
  bool ClassifyLaneCCsPosTypeInImage(const std::vector<ConnectedComponent>& select_lane_ccs,
                                     std::vector<LaneType>*                 ccs_pos_type);
  // @brief: compare CC's size
  static bool CompareCCSize(const ConnectedComponent& cc1, const ConnectedComponent& cc2) {
  ...
  void
  InferPointSetFromLaneCenter(const std::vector<ConnectedComponent>&   lane_ccs,
  ...
  void InferPointSetFromOneCC(const ConnectedComponent&                lane_cc,
                              int                                      left_index,
                              int                                      right_index,
                              std::vector<std::vector<LanePointInfo>>* lane_map_group_point_set);
  // @brief: find the point with the maximum score
  bool MaxScorePoint(const float*   score_pointer,
  ...
  void
  Convert2OriginalCoord(const std::vector<std::vector<LanePointInfo>>& lane_map_group_point_set,
