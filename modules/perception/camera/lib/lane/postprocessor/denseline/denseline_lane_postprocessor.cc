...
bool DenselineLanePostprocessor::Init(const LanePostprocessorInitOptions& options) {
  ...
  const std::string& postprocessor_config = GetAbsolutePath(root_dir, conf_file);
  AINFO << "postprocessor_config: " << postprocessor_config;
  if (!cyber::common::GetProtoFromFile(postprocessor_config, &lane_postprocessor_param_)) {
    AERROR << "Read config detect_param failed: " << postprocessor_config;
    return false;
  }
  std::string param_str;
  google::protobuf::TextFormat::PrintToString(lane_postprocessor_param_, &param_str);
  AINFO << "lane_postprocessor param: " << param_str;
  //
  omit_bottom_line_num_          = lane_postprocessor_param_.omit_bottom_line_num();
  laneline_map_score_thresh_     = lane_postprocessor_param_.laneline_map_score_thresh();
  laneline_point_score_thresh_   = lane_postprocessor_param_.laneline_point_score_thresh();
  laneline_point_min_num_thresh_ = lane_postprocessor_param_.laneline_point_min_num_thresh();
  cc_valid_pixels_ratio_         = lane_postprocessor_param_.cc_valid_pixels_ratio();
  laneline_reject_dist_thresh_   = lane_postprocessor_param_.laneline_reject_dist_thresh();
...
bool DenselineLanePostprocessor::Process2D(const LanePostprocessorOptions& options,
                                           CameraFrame*                    frame) {
  ...
  std::vector<base::LaneLinePositionType> line_pos_type_vec(image_group_point_set_.size());
  std::vector<bool>                       line_flag(image_group_point_set_.size(), false);
  ClassifyLanelinePosTypeInImage(image_group_point_set_, &line_pos_type_vec, &line_flag);
  base::LaneLineType line_type = base::LaneLineType::WHITE_SOLID;
  for (int line_index = 0; line_index < static_cast<int>(image_group_point_set_.size());
       line_index++) {
    if (!line_flag[line_index]) {
      ADEBUG << "line_pos_type is invalid";
      continue;
    }
    AddImageLaneline(image_group_point_set_[line_index], line_type, line_pos_type_vec[line_index],
                     line_index, &(frame->lane_objects));
  }
...
bool DenselineLanePostprocessor::Process3D(const LanePostprocessorOptions& options,
                                           CameraFrame*                    frame) {
...
void DenselineLanePostprocessor::ConvertImagePoint2Camera(CameraFrame* frame) {
  ...
  float camera_ground_height = frame->calibration_service->QueryCameraToGroundHeight();
  ...
  for (int line_index = 0; line_index < laneline_num; line_index++) {
    std::vector<base::Point2DF>& image_point_set  = lane_objects[line_index].curve_image_point_set;
    std::vector<base::Point3DF>& camera_point_set = lane_objects[line_index].curve_camera_point_set;
    for (int i = 0; i < static_cast<int>(image_point_set.size()); i++) {
      ...
      ImagePoint2Camera(image_point, pitch_angle, camera_ground_height, intrinsic_params_inverse,
                        &camera_point3d);
...
std::string DenselineLanePostprocessor::Name() const { return "DenselineLanePostprocessor"; }

void DenselineLanePostprocessor::CalLaneMap(const float*                output_data,
                                            int                         width,
                                            int                         height,
  ...
  for (int y = 0; y < height - omit_bottom_line_num_; y++) {
    ...
    for (int x = 0; x < width; ++x) {
      ...
      //  omit it
      if (max_channel_idx == 0 || max_score < laneline_map_score_thresh_) { continue; }
...
void DenselineLanePostprocessor::InferPointSetFromLaneCenter(
  ...
  for (int i = 0; i < static_cast<int>(lane_ccs.size()); i++) {
    ...
    InferPointSetFromOneCC(lane_ccs[i], left_index, right_index, lane_map_group_point_set);
  }
}

// @brief infer the lane line points from one CC
void DenselineLanePostprocessor::InferPointSetFromOneCC(
    const ConnectedComponent&                lane_cc,
    int                                      left_index,
    int                                      right_index,
    std::vector<std::vector<LanePointInfo>>* lane_map_group_point_set) {
  ...
  memset(lane_pos_blob_.mutable_cpu_data(), 0, sizeof(float) * lane_pos_blob_.count());
  memset(lane_hist_blob_.mutable_cpu_data(), 0, sizeof(int) * lane_hist_blob_.count());
  ...
  for (int j = 0; j < static_cast<int>(pixels.size()); j++) {
    ...
    float x_left_map =
        static_cast<float>(pixel_x) - dist_left * static_cast<float>(lane_map_width_);
    ...
    float x_right_map =
        static_cast<float>(pixel_x) + dist_right * static_cast<float>(lane_map_width_);
  ...
  for (int j = ymin; j <= ymax; j++) {
    ...
    if (left_index != -1) {
      ...
      bool          left_flag =
          MaxScorePoint(score_left_pointer, x_left_pointer, x_left_count_pointer, j, &left_point);
      if (left_flag) { (*lane_map_group_point_set)[left_index].push_back(left_point); }
    }
    ...
    if (right_index != -1) {
      ...
      bool right_flag = MaxScorePoint(score_right_pointer, x_right_pointer, x_right_count_pointer,
                                      j, &right_point);
      if (right_flag) { (*lane_map_group_point_set)[right_index].push_back(right_point); }
    }
  }
}
...
bool DenselineLanePostprocessor::MaxScorePoint(const float*   score_pointer,
  ...
  if (max_score <= laneline_point_score_thresh_) { return false; }
  (*point_info).x     = x_pointer[max_x] / static_cast<float>(x_count_pointer[max_x]);
...
bool DenselineLanePostprocessor::SelectLanecenterCCs(
  ...
  int valid_pixels_num =
      static_cast<int>(cc_valid_pixels_ratio_ * static_cast<float>(lane_map_height_));
...
bool DenselineLanePostprocessor::LocateLanelinePointSet(const CameraFrame* frame) {
  ...
  if (channels < net_model_channel_num_) {
    AERROR << "channel (" << channels << ") is less than net channel (" << net_model_channel_num_
           << ")";
    return false;
  }
  ...
  ADEBUG << "input_size: [" << input_image_width_ << "," << input_image_height_ << "] "
         << "output_shape: channels=" << channels << " height=" << lane_map_height_
  ...
  lane_ccs_.clear();
  bool flag = FindCC(lane_map_, lane_map_width_, lane_map_height_, roi, &lane_ccs_);
  if (!flag) { return false; }
  ...
  std::vector<LaneType> ccs_pos_type(select_lane_ccs_.size(), LaneType::UNKNOWN_LANE);
  ...
  InferPointSetFromLaneCenter(select_lane_ccs_, ccs_pos_type, &lane_map_group_point_set);
...
bool DenselineLanePostprocessor::ClassifyLaneCCsPosTypeInImage(
    const std::vector<ConnectedComponent>& select_lane_ccs, std::vector<LaneType>* ccs_pos_type) {
  //
  ...
  for (int i = 0; i < ccs_num; i++) {
    ...
    float dist            = static_cast<float>(fabs(cc_bottom_center_x[i] - lane_map_center_x));
    if (dist < min_dist) {
      min_dist  = dist;
      min_index = i;
    }
...
void DenselineLanePostprocessor::ClassifyLanelinePosTypeInImage(
  ...
  for (int i = 0; i < set_size; i++) {
    ...
    if (point_num <= laneline_point_min_num_thresh_) { continue; }
    ...
    if (fabs(dif_y) < 1e-3) {
      ...
      float fb                 = fx1 - fk * fy1;
      latitude_intersection[i] = fk * static_cast<float>(input_image_height_ - 1) + fb;
    }
    if (latitude_intersection[i] <= lane_center_pos && latitude_intersection[i] > ego_left_value) {
      ego_left_value = latitude_intersection[i];
      ego_left_index = i;
    }
    if (latitude_intersection[i] > lane_center_pos && latitude_intersection[i] < ego_right_value) {
  ...
  LocateNeighborLaneLine(latitude_intersection, ego_left_index, true, &adj_left_index);
  if (adj_left_index != -1) {
    (*laneline_type)[adj_left_index] = base::LaneLinePositionType::ADJACENT_LEFT;
    (*line_flag)[adj_left_index]     = true;
  }
  // locate adjacent right laneline
  int adj_right_index = -1;
  LocateNeighborLaneLine(latitude_intersection, ego_right_index, false, &adj_right_index);
  if (adj_right_index != -1) {
    (*laneline_type)[adj_right_index] = base::LaneLinePositionType::ADJACENT_RIGHT;
...
bool DenselineLanePostprocessor::LocateNeighborLaneLine(
  ...
  if (line_index < 0 || line_index >= set_size) { return false; }
...
void DenselineLanePostprocessor::Convert2OriginalCoord(
  ...
  float x_ratio = static_cast<float>(input_crop_width_) * lane_map_width_inverse_;
  float y_ratio = static_cast<float>(input_crop_height_) * lane_map_height_inverse_;
  for (int i = 0; i < static_cast<int>(lane_map_group_point_set.size()); i++) {
    for (int j = 0; j < static_cast<int>(lane_map_group_point_set[i].size()); j++) {
      LanePointInfo lane_map_info = lane_map_group_point_set[i][j];
      LanePointInfo original_info;
      original_info.x     = lane_map_info.x * x_ratio + static_cast<float>(input_offset_x_);
      original_info.y     = lane_map_info.y * y_ratio + static_cast<float>(input_offset_y_);
...
void DenselineLanePostprocessor::AddImageLaneline(const std::vector<LanePointInfo>& image_point_set,
  ...
  if (image_point_set_size <= laneline_point_min_num_thresh_) { return; }
  ...
  if (count > 0) { avg_dist = sum_dist / static_cast<float>(count); }
...
void DenselineLanePostprocessor::PolyFitCameraLaneline(CameraFrame* frame) {
  ...
  for (int line_index = 0; line_index < laneline_num; line_index++) {
    ...
    bool is_x_axis = true;
    bool fit_flag  = PolyFit(camera_pos_vec, max_poly_order, &camera_coeff, is_x_axis);
    if (!fit_flag) { continue; }
...
std::vector<std::vector<LanePointInfo>> DenselineLanePostprocessor::GetLanelinePointSet() {
  return image_group_point_set_;
}

std::vector<LanePointInfo> DenselineLanePostprocessor::GetAllInferLinePointSet() {
  float x_ratio = static_cast<float>(input_crop_width_) * lane_map_width_inverse_;
  float y_ratio = static_cast<float>(input_crop_height_) * lane_map_height_inverse_;
  ...
  for (int i = 0; i < lane_map_height_; i++) {
    for (int j = 0; j < lane_map_width_; j++) {
      ...
      float score     = lane_output_[lane_map_dim * 2 + pixel_pos];
      if (score < laneline_point_score_thresh_) { continue; }
      float dist_left   = lane_output_[pixel_pos];
      float dist_right  = lane_output_[lane_map_dim + pixel_pos];
      float map_x_left  = static_cast<float>(j) - dist_left * lane_map_width_float;
      float map_x_right = static_cast<float>(j) + dist_right * lane_map_width_float;
      float org_img_y   = static_cast<float>(i) * y_ratio + static_cast<float>(input_offset_y_);
      if (map_x_left > 0) {
        LanePointInfo left_point;
        float         org_img_x_left = map_x_left * x_ratio + static_cast<float>(input_offset_x_);
      ...
      if (map_x_right < lane_map_width_float) {
        LanePointInfo right_point;
        float         org_img_x_right = map_x_right * x_ratio + static_cast<float>(input_offset_x_);
