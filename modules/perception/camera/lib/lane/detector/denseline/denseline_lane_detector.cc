...
bool DenselineLaneDetector::Init(const LaneDetectorInitOptions& options) {
  ...
  std::copy(net_param.internal_blob_int8().begin(), net_param.internal_blob_int8().end(),
  ...
  for (auto& output_blob_name : net_outputs_) {
    ...
    AINFO << output_blob_name << " : " << output_blob->channels() << " " << output_blob->height()
          << " " << output_blob->width();
  }
...
bool DenselineLaneDetector::Detect(const LaneDetectorOptions& options, CameraFrame* frame) {
  if (frame == nullptr) {
  ...
  AINFO << "input_blob: " << blob_channel << " " << blob_height << " " << blob_width << std::endl;
...
std::string DenselineLaneDetector::Name() const { return "DenselineLaneDetector"; }
