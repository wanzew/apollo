...
bool DarkSCNNLaneDetector::Init(const LaneDetectorInitOptions& options) {
  ...
  CHECK_LE(crop_height_, input_height_) << "crop height larger than input height";
  ...
  cnnadapter_lane_.reset(inference::CreateInferenceByName(model_type, proto_file, weight_file,
  ...
  std::map<std::string, std::vector<int>> input_reshape{{net_inputs_[0], shape}};
  AINFO << "input_reshape: " << input_reshape[net_inputs_[0]][0] << ", "
        << input_reshape[net_inputs_[0]][1] << ", " << input_reshape[net_inputs_[0]][2] << ", "
  ...
  for (auto& input_blob_name : net_inputs_) {
    ...
    AINFO << input_blob_name << ": " << input_blob->channels() << " " << input_blob->height() << " "
          << input_blob->width();
  }

  auto output_blob = cnnadapter_lane_->get_blob(net_outputs_[0]);
  AINFO << net_outputs_[0] << " : " << output_blob->channels() << " " << output_blob->height()
        << " " << output_blob->width();
...
bool DarkSCNNLaneDetector::Detect(const LaneDetectorOptions& options, CameraFrame* frame) {
  if (frame == nullptr) {
  ...
  if (input_width_ != data_provider->src_width()) {
    AERROR << "Input size is not correct: " << input_width_ << " vs " << data_provider->src_width();
    return false;
  }
  ...
  if (!data_provider->GetImage(data_provider_image_option_, &image_src_)) { return false; }
  ...
  ADEBUG << "input_blob: " << blob_channel << " " << blob_height << " " << blob_width << std::endl;
  ...
  inference::ResizeGPU(image_src_, input_blob, static_cast<int>(crop_width_), 0,
  ...
  for (int i = 0; i < num_lanes_; ++i) {
    ...
    memcpy(tmp.data, seg_blob->cpu_data() + lane_output_width_ * lane_output_height_ * i,
  ...
  memcpy(lane_blob_->mutable_cpu_data(), reinterpret_cast<float*>(mask_color.data),
         lane_output_width_ * lane_output_height_ * sizeof(float));
  ...
  if (net_outputs_.size() > 1) {
    ...
    v_point[0] = v_point[0] * vpt_std_[0] + vpt_mean_[0] + (static_cast<float>(blob_width) / 2);
    v_point[1] = v_point[1] * vpt_std_[1] + vpt_mean_[1] + (static_cast<float>(blob_height) / 2);
    // compute coordinate in original image
    v_point[0] = v_point[0] / static_cast<float>(blob_width) * static_cast<float>(crop_width_) +
                 static_cast<float>(input_offset_x_);
    v_point[1] = v_point[1] / static_cast<float>(blob_height) * static_cast<float>(crop_height_) +
                 static_cast<float>(input_offset_y_);

    ADEBUG << "vanishing point: " << v_point[0] << " " << v_point[1];
    if (v_point[0] > 0 && v_point[0] < static_cast<float>(input_width_) && v_point[1] > 0 &&
        v_point[0] < static_cast<float>(input_height_)) {
...
std::string DarkSCNNLaneDetector::Name() const { return "DarkSCNNLaneDetector"; }
