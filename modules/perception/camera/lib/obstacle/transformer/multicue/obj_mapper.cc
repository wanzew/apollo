...
bool ObjMapper::SolveCenterFromNearestVerticalEdge(
  ...
  if (common::IRound(bbox[3]) >= height_ - 1) { height_bbox /= params_.occ_ratio; }
  ...
  float       radius_bbox = common::ISqrt(common::ISqr(hwl[2] / 2) + common::ISqr(hwl[1] / 2));
...
bool ObjMapper::Solve3dBboxGivenOneFullBboxDimensionOrientation(const float* bbox,
                                                                const float* hwl,
                                                                float*       ry,
                                                                float*       center) {
  const float PI               = common::Constant<float>::PI();
  const float PI_HALF          = PI / 2;
  const float small_angle_diff = common::IDegreeToRadians(params_.angle_resolution_degree);
  float       center_2d[2]     = {0};
  bool        success = SolveCenterFromNearestVerticalEdge(bbox, hwl, *ry, center, center_2d);
  ...
  bool  truncated  = bbox[0] <= min_x || bbox[2] >= max_x || bbox[1] <= min_y || bbox[3] >= max_y;
  float dist_rough = sqrtf(common::ISqr(center[0]) + common::ISqr(center[2]));
  bool  ry_pred_is_not_reliable =
      dist_rough > params_.dist_far && bbox[3] - bbox[1] < params_.small_bbox_height;
  ...
  if (!truncated) {
    ...
    success = SolveCenterFromNearestVerticalEdge(bbox, hwl, *ry, center, center_2d);
    PostRefineZ(bbox, hwl, center_2d, *ry, center);
  } else {
    FillRyScoreSingleBin(*ry);
  }
  return success && GetProjectionScore(*ry, bbox, hwl, center, true) > params_.iou_suc;
}
...
bool ObjMapper::Solve3dBbox(const ObjMapperOptions& options,
  ...
  *ry                     = options.ry;
  ...
  if (options.is_veh && check_dimension) {
    ...
    float min_tmplt_vol    = tmplt_with_min_vol[0] * tmplt_with_min_vol[1] * tmplt_with_min_vol[2];
    ...
    if (hwl[0] < params_.abnormal_h_veh ||
      ...
      int   tmplt_index  = -1;
      float score = object_template_manager_->VehObjHwlBySearchTemplates(hwl_tmplt, &tmplt_index);
      ...
      bool is_car_pred =
          type_min_vol_index == kLookUpTableMinVolumeIndex.at(TemplateIndex::CAR_MIN_VOLUME_INDEX);

      bool hwl_is_reliable = search_success && is_same_type;
      if (hwl_is_reliable) {
        memcpy(hwl, hwl_tmplt, sizeof(float) * 3);
      } else if (is_car_pred) {
        const float* tmplt_with_median_vol = tmplt_with_min_vol + kNrDimPerTmplt;
  ...
  float yaw_score_mean = common::IMean(ry_score_.data(), static_cast<int>(ry_score_.size()));
  float yaw_score_sdv =
      common::ISdv(ry_score_.data(), yaw_score_mean, static_cast<int>(ry_score_.size()));
...
void ObjMapper::PostRefineOrientation(const float* bbox,
                                      const float* hwl,
                                      const float* center,
                                      float*       ry) {
  ...
  if (score_best > params_.iou_high && score_cur < params_.iou_suc) { *ry = ry_best; }

  float bbox_res[4] = {0};
  float score_final = GetProjectionScore(*ry, bbox, hwl, center, true, bbox_res);
  if (score_final > params_.iou_high) {
    return;
  } else if (bbox[2] - bbox[0] < (bbox_res[2] - bbox_res[0]) * params_.factor_small) {
    *ry = *ry > 0 ? PI_HALF : -PI_HALF;
    FillRyScoreSingleBin(*ry);
  } else if ((bbox[2] - bbox[0]) * params_.factor_small > bbox_res[2] - bbox_res[0]) {
    *ry = 0.0f;
    FillRyScoreSingleBin(*ry);
  }
}

void ObjMapper::GetCenter(const float* bbox,
                          const float& z_ref,
                          const float& ry,
                          const float* hwl,
                          float*       center,
  ...
  while (!stop) {
    ...
    float cost_cur =
        common::ISqrt(common::ISqr(x_cur[0] - x_target[0]) + common::ISqr(x_cur[1] - x_target[1]));
    ...
    if (cost_cur >= cost_pre) {
      ...
      ++iter;
      stop = iter >= MAX_ITERATION || cost_delta < EPS_COST_DELTA || cost_pre < kMinCost;
    }
  }
}
