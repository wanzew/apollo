...
bool MultiCueObstacleTransformer::Init(const ObstacleTransformerInitOptions& options) {
...
void MultiCueObstacleTransformer::SetObjMapperOptions(base::ObjectPtr   obj,
                                                      ...
                                                      float*            theta_ray) {
  // prepare bbox2d
  float bbox2d[4] = {obj->camera_supplement.box.xmin, obj->camera_supplement.box.ymin,
  ...
  Eigen::Vector3f point_in_camera = static_cast<Eigen::Matrix<float, 3, 1, 0, 3, 1>>(
      camera_k_matrix.inverse() * image_point_low_center);
  *theta_ray = static_cast<float>(atan2(point_in_camera.x(), point_in_camera.z()));
  float               rotation_y = *theta_ray + static_cast<float>(obj->camera_supplement.alpha);
  ...
  obj_mapper_options->type_min_vol_index = MatchTemplates(sub_type, dimension_hwl);

  ADEBUG << "#2D-to-3D for one obj:";
  ADEBUG << "Obj pred ry:" << rotation_y;
  ADEBUG << "Obj pred type: " << static_cast<int>(sub_type);
  ADEBUG << "Bbox: " << bbox2d[0] << ", " << bbox2d[1] << ", " << bbox2d[2] << ", " << bbox2d[3];
}
...
int MultiCueObstacleTransformer::MatchTemplates(base::ObjectSubType sub_type,
  ...
  switch (sub_type) {
    ...
    case base::ObjectSubType::VAN:
      type_min_vol_index = kLookUpTableMinVolumeIndex.at(TemplateIndex::VAN_MIN_VOLUME_INDEX);
      break;
    case base::ObjectSubType::TRUCK:
      type_min_vol_index = kLookUpTableMinVolumeIndex.at(TemplateIndex::TRUCK_MIN_VOLUME_INDEX);
      break;
    case base::ObjectSubType::BUS:
      type_min_vol_index = kLookUpTableMinVolumeIndex.at(TemplateIndex::BUS_MIN_VOLUME_INDEX);
      break;
    default:
      if (min_dimension_val < multicue_param_.min_dimension_val()) {
        common::IScale3(dimension_hwl,
                        multicue_param_.min_dimension_val() * common::IRec(min_dimension_val));
      }
      break;
  }
...
void MultiCueObstacleTransformer::FillResults(float           object_center[3],
  ...
  ADEBUG << "Obj ori dimension: " << obj->size[2] << ", " << obj->size[1] << ", " << obj->size[0];
  ...
  Eigen::Vector3d dir =
      (camera2world_pose.matrix().block(0, 0, 3, 3) * Eigen::Vector3d(cos(theta), 0, -sin(theta)));
...
bool MultiCueObstacleTransformer::Transform(const ObstacleTransformerOptions& options,
                                            CameraFrame*                      frame) {
  ...
  for (auto& obj : frame->detected_objects) {
    ...
    SetObjMapperOptions(obj, camera_k_matrix, width_image, height_image, &obj_mapper_options,
                        &theta_ray);

    // process
    mapper_->Solve3dBbox(obj_mapper_options, object_center, dimension_hwl, &rotation_y);
...
std::string MultiCueObstacleTransformer::Name() const { return "MultiCueObstacleTransformer"; }
