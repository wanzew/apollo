...
bool SingleStageObstacleTransformer::Init(const ObstacleTransformerInitOptions& options) {
  std::string transformer_config =
      cyber::common::GetAbsolutePath(options.root_dir, options.conf_file);

  if (!cyber::common::GetProtoFromFile(transformer_config, &singlestage_param_)) {
...
int SingleStageObstacleTransformer::MatchTemplates(base::ObjectSubType sub_type,
                                                   float*              dimension_hwl) {
    ...
    default:
      if (min_dimension_val < singlestage_param_.min_dimension_val()) {
        common::IScale3(dimension_hwl,
                        singlestage_param_.min_dimension_val() * common::IRec(min_dimension_val));
      }
      break;
  }
...
void SingleStageObstacleTransformer::FillResults(float           object_center[3],
...
bool SingleStageObstacleTransformer::Transform(const ObstacleTransformerOptions& options,
                                               CameraFrame*                      frame) {
  ...
  for (auto& obj : frame->detected_objects) {
    ...
    float theta_ray =
        atan2(obj->camera_supplement.local_center[0], obj->camera_supplement.local_center[2]);
    ...
    CenterPointFromBbox(bbox, dimension_hwl, rotation_y, object_center, center2d, k_mat, height,
                        width);
    // fill back results
...
float SingleStageObstacleTransformer::CenterPointFromBbox(const float* bbox,
  ...
  if (fabsf(depth - center[2]) * common::IRec(center[2]) > 0.1) {
    ConstraintCenterPoint(bbox, center[2], ry, hwl, k_mat, center, center_2d, height, width);
  }
  return depth;
}
...
void SingleStageObstacleTransformer::ConstraintCenterPoint(const float* bbox,
  ...
  */
  ...
  while (!stop) {
    ...
    float cost_cur      = common::ISqrt(common::ISqr(center_cur[0] - center_2d_target[0]) +
    ...
    if (cost_cur >= cost_pre) {
      ...
      stop = iter >= MAX_ITERATION || cost_delta < EPS_COST_DELTA || cost_pre < K_MIN_COST;
    }
  }
}
