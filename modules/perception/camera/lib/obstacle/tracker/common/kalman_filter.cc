...
void KalmanFilterConstVelocity::Predict(float delta_t) {
  if (inited_) {
    ...
    variance_ = state_transition_matrix_ * variance_ * state_transition_matrix_.transpose() +
                process_noise_;
  }
}
...
void KalmanFilterConstVelocity::Correct(const Eigen::VectorXd& z) {
  if (inited_) {
    ...
    Eigen::Matrix2d cov =
        measure_matrix_ * variance_ * measure_matrix_.transpose() + measure_noise_;
    ...
    likelihood_   = std::exp(-0.5 * residual.transpose() * cov.inverse() * residual) /
...
struct {
  bool operator()(Eigen::VectorXd a, Eigen::VectorXd b) const { return a[0] > b[0]; }
} customLess;
...
void MaxNMeanFilter::AddMeasure(const Eigen::VectorXd& z) {
  ...
  if (measures_.size() > static_cast<unsigned int>(window_)) { measures_.resize(window_); }
}
