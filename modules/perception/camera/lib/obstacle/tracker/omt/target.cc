...
TrackObjectPtr         Target::get_object(int index) const {
  ...
  return tracked_objects[(index + tracked_objects.size()) % tracked_objects.size()];
}
...
void Target::RemoveOld(int frame_id) {
  size_t index = 0;
  while (index < tracked_objects.size() && tracked_objects[index]->indicator.frame_id < frame_id) {
    ++index;
  }
  tracked_objects.erase(tracked_objects.begin(), tracked_objects.begin() + index);
}
void Target::Init(const omt::TargetParam& param) {
  ...
  image_center.measure_noise_ *= target_param_.image_center().measure_variance();
  image_center.process_noise_ *= target_param_.image_center().process_variance();
  ...
  type_probs.assign(static_cast<int>(base::ObjectSubType::MAX_OBJECT_TYPE), 0.0f);
  ...
  world_center_const.covariance_ *= target_param_.world_center().init_variance();
...
void Target::Predict(CameraFrame* frame) {
  auto delta_t = static_cast<float>(frame->timestamp - latest_object->timestamp);
  if (delta_t < 0) { return; }
  ...
  world_center_const.process_noise_(1, 1) = world_center_const.process_noise_(0, 0);
...
void Target::Update3D(CameraFrame* frame) {
  ...
  if (!isLost()) {
    ...
    if (object->type == base::ObjectType::UNKNOWN_UNMOVABLE) {
      ...
      float obj_distance_to_main_car = obj_2_car_x * obj_2_car_x + obj_2_car_z * obj_2_car_z;
      float dis_err = target_param_.world_center().measure_variance() * obj_distance_to_main_car;
  ...
  if (object->type == base::ObjectType::UNKNOWN_UNMOVABLE) {
    ...
    object->center_uncertainty(0, 0) = static_cast<float>(world_center.variance_(0, 0));
    object->center_uncertainty(1, 1) = static_cast<float>(world_center.variance_(1, 1));
  ...
  if (object->type != base::ObjectType::UNKNOWN_UNMOVABLE && target_param_.clapping_velocity()) {
    ...
    if (avg_vel_norm > target_param_.stable_moving_speed()) {
      ...
      // consistent
      stable_moving =
          displacement_theta.get_variance()(0, 0) < target_param_.displacement_theta_var();
    }

    // check const position kalman residuals
    const auto& residual = world_center_const.residual_;
    if (!stable_moving && residual(0) * residual(0) < 4 * world_center_const.measure_noise_(0, 0) &&
        residual(1) * residual(1) < 4 * world_center_const.measure_noise_(1, 1)) {
      object->velocity(0) = 0;
      object->velocity(1) = 0;
    }
...
void Target::UpdateType(CameraFrame* frame) {
  ...
  if (!isLost()) {
    ...
    // hf = H * f/z = H * 50
    const TemplateMap& kMidTemplateHWL = object_template_manager_->MidTemplateHWL();
    float              alpha =
        gaussian(rect.height / (50 * (kMidTemplateHWL.at(object->sub_type).at(0) + 0.01f)), 1.0f,
                 target_param_.type_filter_var());
    ...
    ADEBUG << "Target " << id << " change type from " << static_cast<int>(object->sub_type)
           << " to " << static_cast<int>(type);
    object->sub_type = type;

    Eigen::Vector4d size_measurement;
    size_measurement << alpha, object->size(0), object->size(1), object->size(2);
    world_lwh.AddMeasure(size_measurement);
    world_lwh_for_unmovable.AddMeasure(size_measurement);
    // update 3d object size
    if (object->type == base::ObjectType::UNKNOWN_UNMOVABLE) {
      object->size = world_lwh_for_unmovable.get_state().block<3, 1>(1, 0).cast<float>();
    } else {
...
void Target::ClappingTrackVelocity(const base::ObjectPtr& obj) {
  ...
  if (obj->type == base::ObjectType::VEHICLE) {
    ...
    double          vel_heading_angle_change_1 = common::CalculateTheta2DXY(obj_dir, obj_vel);
    obj_dir *= -1;  // obj's heading may flip
    double vel_heading_angle_change_2 = common::CalculateTheta2DXY(obj_dir, obj_vel);
    double vel_heading_angle_change =
        std::min(std::fabs(vel_heading_angle_change_1), std::fabs(vel_heading_angle_change_2));
    ADEBUG << "obj_id: " << obj->track_id << " vel_heading_angle, 1: " << vel_heading_angle_change_1
           << " 2: " << vel_heading_angle_change_2 << " final: " << vel_heading_angle_change;
    if (vel_heading_angle_change > target_param_.abnormal_velocity_heading_angle_threshold()) {
...
bool Target::CheckStatic() {
  ...
  const int min_vel_size = std::min(static_cast<int>(history_world_states_.size()),
  ...
  tmp_obj_vel_avg           = std::accumulate(
      history_world_states_.begin() + history_world_states_.size() - min_vel_size,
      history_world_states_.end(), tmp_obj_vel_avg,
      [](const std::shared_ptr<base::Object> obj1, const std::shared_ptr<base::Object> obj2) {
  ...
  if (not_move) { return true; }

  // 3. Check velocity theta's variance
  std::vector<double> theta_vec(min_vel_size);
  std::transform(history_world_states_.begin() + history_world_states_.size() - min_vel_size,
...
bool Target::isTracked() const { return Size() >= target_param_.tracked_life(); }
