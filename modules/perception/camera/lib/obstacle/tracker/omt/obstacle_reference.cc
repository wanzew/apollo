...
void ObstacleReference::Init(const omt::ReferenceParam& ref_param, float width, float height) {
  ...
  for (int y = 0; y < ref_height_; ++y) {
    ...
    if (y < ref_height_ / 2 || y > ref_height_ - ref_param_.margin()) { continue; }
...
void ObstacleReference::UpdateReference(const CameraFrame*         frame,
  ...
  SyncGroundEstimator(sensor, frame->camera_k_matrix, static_cast<int>(img_width_),
  ...
  if (ref_map.empty()) { ref_map = init_ref_map_; }
  ...
  for (auto&& target : targets) {
    ...
    if (!Contain(object_template_manager_->TypeCanBeRef(), obj->sub_type)) { continue; }
    auto& box = obj->camera_supplement.box;

    if (box.ymax - box.ymin < ref_param_.min_allow_height()) { continue; }
    if (box.ymax < frame->camera_k_matrix(1, 2) + 1) { continue; }
    if (box.ymax >= img_height_ + 1) {
      AERROR << "box.ymax (" << box.ymax << ") is larger than img_height_ + 1 (" << img_height_ + 1
             << ")";
      return;
    }
    if (box.xmax >= img_width_ + 1) {
      AERROR << "box.xmax (" << box.xmax << ") is larger than img_width_ + 1 (" << img_width_ + 1
             << ")";
      return;
    }
...
void ObstacleReference::CorrectSize(CameraFrame* frame) {
  ...
  for (auto& obj : frame->detected_objects) {
    ...
    ADEBUG << "Det " << frame->frame_id << " (" << obj->id << ") "
           ...
           << "type: " << static_cast<int>(obj->sub_type) << " " << volume_object << " "
    ...
    if (Contain(object_template_manager_->TypeRefinedByTemplate(), obj->sub_type)) {
      ...
      if (volume_object < min_template_volume ||
        ...
        obj->size[2] = min_tmplt[0];
                 ...
                 obj->size[2] > (1 + ref_param_.height_diff_ratio()) * max_tmplt[0]) {
    ...
    if (Contain(object_template_manager_->TypeRefinedByRef(), obj->sub_type)) {
      ...
      double z_obj =
          frame->camera_k_matrix(1, 1) * obj->size[2] / (supplement.box.ymax - supplement.box.ymin);
      ...
      if (frame->calibration_service != nullptr) {
        ...
        bool   success = frame->calibration_service->QueryDepthOnGroundPlane(
            static_cast<int>(supplement.box.Center().x), static_cast<int>(supplement.box.ymax),
            &z_calib);
        if (success) {
