...
bool OMTObstacleTracker::Init(const ObstacleTrackerInitOptions& options) {
  ...
  std::string   type_change_cost = GetAbsolutePath(options.root_dir, omt_param_.type_change_cost());
...
bool OMTObstacleTracker::CombineDuplicateTargets() {
  ...
  for (size_t i = 0; i < targets_.size(); ++i) {
    if (targets_[i].Size() == 0) { continue; }
    for (size_t j = i + 1; j < targets_.size(); ++j) {
      if (targets_[j].Size() == 0) { continue; }
      int   count  = 0;
      float score  = 0.0f;
      int   index1 = 0;
      int   index2 = 0;
      while (index1 < targets_[i].Size() && index2 < targets_[j].Size()) {
        ...
        if (std::abs(p1->timestamp - p2->timestamp) < omt_param_.same_ts_eps()) {
          if (p1->indicator.sensor_name != p2->indicator.sensor_name) {
            ...
            score -= std::abs((rect1.width - rect2.width) * (rect1.height - rect2.height) /
      ...
      ADEBUG << "Overlap: (" << targets_[i].id << "," << targets_[j].id << ") score " << score
             << " count " << count;
      hypo.target = static_cast<int>(i);
      hypo.object = static_cast<int>(j);
      hypo.score  = (count > 0) ? score / static_cast<float>(count) : 0;
      if (hypo.score < omt_param_.target_combine_iou_threshold()) { continue; }
  ...
  for (auto& pair : score_list) {
    if (used_target[pair.target] || used_target[pair.object]) { continue; }
    ...
    std::sort(target_save.tracked_objects.begin(), target_save.tracked_objects.end(),
    ...
    target_del.Clear();
    AINFO << "Target " << target_del.id << " is merged into Target " << target_save.id
          << " with iou " << pair.score;
...
void OMTObstacleTracker::GenerateHypothesis(const TrackObjectPtrs& objects) {
  ...
  for (size_t i = 0; i < targets_.size(); ++i) {
    ...
    for (size_t j = 0; j < objects.size(); ++j) {
      ...
      ADEBUG << "Detection " << objects[j]->indicator.frame_id << "(" << j << ") sa:" << sa
             << " sm: " << sm << " ss: " << ss << " so: " << so << " score: " << hypo.score;

      // 95.44% area is range [mu - sigma*2, mu + sigma*2]
      // don't match if motion is beyond the range
      if (sm < 0.045 || hypo.score < omt_param_.target_thresh()) { continue; }
      score_list.push_back(hypo);
    }
  }
  ...
  std::vector<bool> used_target(targets_.size(), false);
  for (auto& pair : score_list) {
    if (used_target[pair.target] || used_[pair.object]) { continue; }
...
float OMTObstacleTracker::ScoreShape(const Target& target, TrackObjectPtr track_obj) {
  ...
  base::RectF     rect(track_obj->projected_box);
  float           s = static_cast<float>((shape[1] - rect.height) * (shape[0] - rect.width) /
                               (shape[1] * shape[0]));
  return -std::abs(s);
}
...
float OMTObstacleTracker::ScoreAppearance(const Target& target, TrackObjectPtr track_obj) {
  ...
  for (int i = target.Size() - 1; i >= 0; --i) {
    if (target[i]->indicator.sensor_name != sensor_name) { continue; }
...
int OMTObstacleTracker::CreateNewTarget(const TrackObjectPtrs& objects) {
  ...
  for (auto&& target : targets_) {
    if (!target.isTracked() || target.isLost()) { continue; }
  ...
  for (size_t i = 0; i < objects.size(); ++i) {
    if (!used_[i]) {
      ...
      if (OutOfValidRegion(rect, width_, height_, omt_param_.border())) {
        ...
        AINFO << "Rect x: " << rect.x << " Rect y: " << rect.y << " Rect height: " << rect.height
              << " Rect width: " << rect.width << " GT height_: " << height_
              << " GT width_: " << width_;
        continue;
      }
      for (auto&& target_rect : target_rects) {
        if (IsCovered(rect, target_rect, 0.4f) || IsCoveredHorizon(rect, target_rect, 0.5f)) {
          is_covered = true;
          break;
        }
      }
      ...
      if (min_tmplt.empty()  // unknown type
        ...
        AINFO << "Target " << target.id << " is created by " << objects[i]->indicator.frame_id
...
bool OMTObstacleTracker::Associate2D(const ObstacleTrackerOptions& options, CameraFrame* frame) {
  ...
  for (size_t i = 0; i < frame->detected_objects.size(); ++i) {
    ...
    track_ptr->indicator =
        PatchIndicator(frame->frame_id, static_cast<int>(i), frame->data_provider->sensor_name());
    track_ptr->object->camera_supplement.sensor_name = frame->data_provider->sensor_name();
    ProjectBox(frame->detected_objects[i]->camera_supplement.box, frame->project_matrix,
...
bool OMTObstacleTracker::Associate3D(const ObstacleTrackerOptions& options, CameraFrame* frame) {
  ...
  for (auto& target : targets_) {
    if (target.isLost() || target.Size() == 1) { continue; }
...
bool OMTObstacleTracker::Track(const ObstacleTrackerOptions& options, CameraFrame* frame) {
  return true;
}
