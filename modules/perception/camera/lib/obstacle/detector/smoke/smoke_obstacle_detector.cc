...
void SmokeObstacleDetector::LoadInputShape(const smoke::ModelParam& model_param) {
...
bool SmokeObstacleDetector::InitNet(const smoke::SmokeParam& smoke_param,
  ...
  shape_map.emplace((std::pair<std::string, std::vector<int>>(input_names[0], shape_input)));
  shape_map.emplace((std::pair<std::string, std::vector<int>>(input_names[1], shape_param)));
  shape_map.emplace((std::pair<std::string, std::vector<int>>(input_names[2], shape_ratio)));
  shape_map.emplace((std::pair<std::string, std::vector<int>>(output_names[0], shape_res)));
  shape_map.emplace((std::pair<std::string, std::vector<int>>(output_names[1], shape_feat)));
  if (!inference_->Init(shape_map)) { return false; }
  // inference_->Infer();
  return true;
}

void SmokeObstacleDetector::InitSmokeBlob(const smoke::NetworkParam& net_param) {
  ...
  smoke_blobs_.det1_loc_blob = inference_->get_blob(smoke_param_.net_param().det1_loc_blob());
}
...
bool SmokeObstacleDetector::Init(const ObstacleDetectorInitOptions& options) {
  ...
  if (!InitNet(smoke_param_, model_root)) { return false; }
...
bool SmokeObstacleDetector::InitFeatureExtractor(const std::string& root_dir) {
  ...
  if (!feature_extractor_->Init(feature_options)) { return false; }
  return true;
}

bool SmokeObstacleDetector::Detect(const ObstacleDetectorOptions& options, CameraFrame* frame) {
  ...
  ratio_data[0] =
      4.f * static_cast<float>(frame->data_provider->src_width()) / static_cast<float>(width_);
  ratio_data[1] =
      4.f * static_cast<float>(frame->data_provider->src_height()) / static_cast<float>(height_);
  ...
  get_smoke_objects_cpu(smoke_blobs_, types_, smoke_param_.model_param(), light_vis_conf_threshold_,
                        light_swt_conf_threshold_, overlapped_.get(), idx_sm_.get(),
                        &(frame->detected_objects), frame->data_provider->src_width(),
