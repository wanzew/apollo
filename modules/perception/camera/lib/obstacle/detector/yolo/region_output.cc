...
void get_max_score_index(const std::vector<float>&           scores,
  ...
  for (int i = 0; i < static_cast<int>(scores.size()); ++i) {
    if (scores[i] > threshold) { score_index_vec->push_back(std::make_pair(scores[i], i)); }
  }

  // Sort the score pair according to the scores in descending order
  std::stable_sort(score_index_vec->begin(), score_index_vec->end(), sort_score_pair_descend<int>);
...
void apply_softnms_fast(const std::vector<NormalizedBBox>& bboxes,
  ...
  while (!score_index_vec.empty()) {
    auto      best_it  = max_element(std::begin(score_index_vec), std::end(score_index_vec));
    ...
    for (std::vector<std::pair<float, int>>::iterator it = score_index_vec.begin();
      ...
      if (is_linear) {
        ...
        (*scores)[cur_idx] *= static_cast<float>(exp(-1.0 * pow(cur_overlap, 2) / sigma));
      }
      ++it;
    }
  }
...
void apply_boxvoting_fast(std::vector<NormalizedBBox>* bboxes,
  ...
  for (int i = 0; i < static_cast<int>(bboxes->size()); i++) {
    (*bboxes)[i].mask = false;
    if ((*scores)[i] > conf_threshold) { indices->push_back(i); }
  }
  for (int count = 0; count < static_cast<int>(indices->size()); count++) {
    ...
    for (int i = 1; i < static_cast<int>(indices->size()); i++) {
      ...
      if ((*bboxes)[idx].mask) { continue; }
      if ((*scores)[idx] > (*scores)[max_box_idx]) { max_box_idx = idx; }
    }
    ...
    for (int i = 0; i < static_cast<int>(indices->size()); i++) {
      ...
      if ((*bboxes)[sub_it].mask) { continue; }
      float cur_overlap = 0.;
      cur_overlap       = get_jaccard_overlap(best_bbox, (*bboxes)[sub_it]);
      if (sigma == 0) {
        (*bboxes)[sub_it].mask = true;
      } else {
        (*scores)[sub_it] *= static_cast<float>(exp(-1.0 * pow(cur_overlap, 2) / sigma));
      }
...
void apply_nms_fast(const std::vector<NormalizedBBox>& bboxes,
  ...
  while (!score_index_vec.empty()) {
    ...
    if (keep && eta < 1 && adaptive_threshold > 0.5) { adaptive_threshold *= eta; }
  }
}
...
void filter_bbox(const MinDims& min_dims, std::vector<base::ObjectPtr>* objects) {
  ...
  while (total_obj_idx < static_cast<int>(objects->size())) {
    ...
    if ((obj->camera_supplement.box.ymax - obj->camera_supplement.box.ymin) >=
        ...
        (min_dims.min_3d_length <= 0 || obj->size[0] >= min_dims.min_3d_length)) {
...
void recover_bbox(int roi_w, int roi_h, int offset_y, std::vector<base::ObjectPtr>* objects) {
  for (auto& obj : *objects) {
    ...
    base::RectF rect_det(static_cast<float>(x), static_cast<float>(y), static_cast<float>(w),
                         static_cast<float>(h));
    base::RectF rect_img(0, 0, static_cast<float>(roi_w), static_cast<float>(roi_h + offset_y));
...
int get_area_id(float visible_ratios[4]) {
  ...
  int max_face = 0;
  for (int i = 1; i < 4; i++) {
    if (visible_ratios[i] > visible_ratios[max_face]) { max_face = i; }
  }
...
void get_objects_cpu(const YoloBlobs&                        yolo_blobs,
  ...
  num_kept = get_objects_gpu(yolo_blobs, stream, types, nms, model_param, light_vis_conf_threshold,
                             light_swt_conf_threshold, overlapped, idx_sm, indices, conf_scores);
  objects->clear();
  ...
  for (auto it = indices.begin(); it != indices.end(); ++it) {
    ...
    for (size_t j = 0; j < indice.size(); ++j) {
      ...
      if (scores[idx] < model_param.confidence_threshold()) { continue; }
      ...
      if (maintainer.Add(idx, obj)) { objects->push_back(obj); }
    }
  }
}
