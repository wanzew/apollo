...
bool ObjPostProcessor::PostProcessObjWithGround(const ObjPostProcessorOptions& options,
                                                float                          center[3],
                                                float                          hwl[3],
                                                float*                         ry) {
  ...
  *ry = options.ry;
  ...
  // soft constraints
  bool adjust_soft = AdjustCenterWithGround(bbox, hwl, *ry, options.plane, center);
  if (center[2] > params_.dist_far) { return adjust_soft; }

  // hard constraints
  bool adjust_hard = PostRefineCenterWithGroundBoundary(
      bbox, hwl, *ry, options.plane, options.line_segs, center, options.check_lowerbound);

  return adjust_soft || adjust_hard;
}

bool ObjPostProcessor::PostProcessObjWithDispmap(const ObjPostProcessorOptions& options,
                                                 float                          center[3],
                                                 float                          hwl[3],
                                                 float*                         ry) {
  return true;
...
bool ObjPostProcessor::AdjustCenterWithGround(
  ...
  while (!stop) {
    ...
    x[0] *= common::IRec(x[2]);
    x[1] *= common::IRec(x[2]);
    bool in_front = common::IBackprojectPlaneIntersectionCanonical(x, k_mat_, plane, center_test);
    if (!in_front) {
    ...
    if (cost_cur >= cost_pre) {
      ...
      ++iter;
      stop = iter >= MAX_ITERATION || cost_delta < EPS_COST_DELTA || cost_pre < MIN_COST;
    }
...
bool ObjPostProcessor::PostRefineCenterWithGroundBoundary(
  ...
  bool truncated_on_bottom =
      bbox[3] >= static_cast<float>(height_) - (bbox[3] - bbox[1]) * params_.sampling_ratio_low;
  if (truncated_on_bottom) { return false; }
  ...
  for (int i = 0; i < nr_line_segs; ++i) {
    float dxdz[2] = {0};
    GetDxDzForCenterFromGroundLineSeg(line_seg_limits[i], plane, pts_c, k_mat_, width_, height_,
