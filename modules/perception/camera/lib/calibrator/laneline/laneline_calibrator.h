...
class LaneLineCalibrator : public BaseCalibrator {
 public:
  ...
  inline float GetVanishingRow() const { return calibrator_.get_vanishing_row(); }

 private:
  // @brief: load ego lane line from camera frame
  // @param [in]: lane_objects
  // @param [out]: ego_lane
  bool LoadEgoLaneline(const std::vector<base::LaneLine>& lane_objects, EgoLane* ego_lane);
