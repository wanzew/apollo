...
class DummyCalibrationService : public BaseCalibrationService {
 public:
  ...
  bool
  Init(const CalibrationServiceInitOptions& options = CalibrationServiceInitOptions()) override {
    return true;
  }
  bool BuildIndex() override { return true; }
  bool QueryDepthOnGroundPlane(int x, int y, double* depth) const override { return false; }
  bool QueryPoint3dOnGroundPlane(int x, int y, Eigen::Vector3d* point3d) const override {
    return false;
  }
  bool QueryGroundPlaneInCameraFrame(Eigen::Vector4d* plane_param) const override { return false; }
  ...
  void SetCameraHeightAndPitch(const std::map<std::string, float>& name_camera_ground_height_map,
