...
bool MotionService::Init() {
  ...
  std::string input_camera_channel_names_str = motion_service_param.input_camera_channel_names();
  ...
  std::function<void(const ImageMsgType&)> camera_callback =
      std::bind(&MotionService::OnReceiveImage, this, std::placeholders::_1, camera_name);
  auto camera_reader = node_->CreateReader(channel_name_img, camera_callback);

  // initialize localization listener
  const std::string& channel_name_local = motion_service_param.input_localization_channel_name();
  ...
  auto localization_reader = node_->CreateReader(channel_name_local, localization_callback);

  // initialize writer to output channel
  writer_ = node_->CreateWriter<Motion_Service>(motion_service_param.output_topic_channel_name());
  AINFO << "init MotionService success.";
  return true;
}

// On receiving image input, just need to record its timestamp
void MotionService::OnReceiveImage(const ImageMsgType& message, const std::string& camera_name) {
  std::lock_guard<std::mutex> lock(mutex_);
  const double                curr_timestamp = message->measurement_time() + timestamp_offset_;
  ADEBUG << "image received: camera_name: " << camera_name << " image ts: " << curr_timestamp;
...
void MotionService::OnLocalization(const LocalizationMsgType& message) {
  std::lock_guard<std::mutex> lock(mutex_);
  ADEBUG << "localization received: localization ts: " << message->measurement_time();
  ...
  vehicle_status.velocity   = static_cast<float>(sqrt(velx * velx + vely * vely + velz * velz));
  ...
  if (!start_flag_) {
    ...
    vehicle_status.roll_rate  = static_cast<float>(message->pose().angular_velocity_vrf().y());
    vehicle_status.pitch_rate = static_cast<float>(-message->pose().angular_velocity_vrf().x());
    vehicle_status.yaw_rate   = static_cast<float>(message->pose().angular_velocity_vrf().z());
...
void MotionService::PublishEvent(const double timestamp) {
  ...
  for (int k = static_cast<int>(motion_buffer.size()) - 1; k >= 0; k--) {
    apollo::perception::VehicleStatus* v_status_msg = motion_service_msg->add_vehicle_status();
...
// convert vehicle status buffer to output message
void MotionService::ConvertVehicleMotionToMsgOut(base::VehicleStatus                vs,
...
bool MotionService::GetMotionInformation(double timestamp, base::VehicleStatus* vs) {
