...
double
Select::Calc2dGaussianScore(base::Point2DI p1, base::Point2DI p2, float sigma1, float sigma2) {
  return std::exp(-0.5 * (static_cast<float>((p1.x - p2.x) * (p1.x - p2.x)) / (sigma1 * sigma1) +
                          (static_cast<float>((p1.y - p2.y) * (p1.y - p2.y)) / (sigma2 * sigma2))));
}

void Select::SelectTrafficLights(const std::vector<base::TrafficLightPtr>& refined_bboxes,
  ...
  for (size_t row = 0; row < hdmap_bboxes->size(); ++row) {
    ...
    for (size_t col = 0; col < refined_bboxes.size(); ++col) {
      ...
      double distance_score =
          Calc2dGaussianScore(center_hd, center_refine, gaussian_score, gaussian_score);
      ...
      (*munkres_.costs())(row, col) =
          static_cast<float>(detection_weight * detection_score + distance_weight * distance_score);
      ...
      if ((detection_roi & crop_roi) != detection_roi) {
        AINFO << "detection_roi outside crop_roi, set score to 0."
              ...
              << detection_roi.width << " " << detection_roi.height << " crop_roi: " << crop_roi.x
              << " " << crop_roi.y << " " << crop_roi.width << " " << crop_roi.height;
        (*munkres_.costs())(row, col) = 0.0;
      }
