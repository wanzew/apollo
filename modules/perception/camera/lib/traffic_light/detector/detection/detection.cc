...
bool TrafficLightDetection::Init(const camera::TrafficLightDetectorInitOptions& options) {
  ...
  std::string model_root = GetAbsolutePath(options.root_dir, detection_param_.model_name());
  AINFO << "model_root " << model_root;

  std::string proto_file = GetAbsolutePath(model_root, detection_param_.proto_file());
  AINFO << "proto_file " << proto_file;

  std::string weight_file = GetAbsolutePath(model_root, detection_param_.weight_file());
  ...
  AINFO << "net input blobs: "
        << std::accumulate(net_inputs_.begin(), net_inputs_.end(), std::string(""),
                           [](std::string& sum, const std::string& s) { return sum + "\n" + s; });
  AINFO << "net output blobs: "
        << std::accumulate(net_outputs_.begin(), net_outputs_.end(), std::string(""),
                           [](std::string& sum, const std::string& s) { return sum + "\n" + s; });

  const auto& model_type = detection_param_.model_type();
  AINFO << "model_type: " << model_type;

  rt_net_.reset(inference::CreateInferenceByName(model_type, proto_file, weight_file, net_outputs_,
  ...
  std::vector<int> shape_input = {max_batch_size_, resize_height, resize_width, 3};
  std::vector<int> shape_param = {max_batch_size_, 1, param_blob_length_, 1};

  std::map<std::string, std::vector<int>> input_reshape;
  input_reshape.insert((std::pair<std::string, std::vector<int>>(net_inputs_[0], shape_input)));
  input_reshape.insert((std::pair<std::string, std::vector<int>>(net_inputs_[1], shape_param)));
  ...
  switch (detection_param_.crop_method()) {
    ...
    case 0:
      ...
      break;
    case 1: crop_.reset(new CropBoxWholeImage()); break;
  }

  select_.Init(resize_width, resize_height);
  image_.reset(new base::Image8U(resize_height, resize_width, base::Color::BGR));
  return true;
}

// TODO(chenjiahao): temporarily do inference serially for multiple
//  traffic lights, because so far batch size can only be 1
bool TrafficLightDetection::Inference(std::vector<base::TrafficLightPtr>* lights,
  ...
  for (size_t i = 0; i < batch_num; ++i) {
    ...
    input_img_blob->Reshape(1, static_cast<int>(detection_param_.min_crop_size()),
    ...
    AINFO << "get crop box success " << cbox.x << " " << cbox.y << " " << cbox.width << " "
          << cbox.height;
    ...
    if (!OutOfValidRegion(cbox, img_width, img_height) && cbox.Area() > 0) {
      ...
      float resize_scale = static_cast<float>(detection_param_.min_crop_size()) /
                           static_cast<float>(std::min(cbox.width, cbox.height));
      resize_scale_list_.push_back(resize_scale);

      inference::ResizeGPU(*image_, input_img_blob, img_width, resize_index, mean_[0], mean_[1],
                           mean_[2], true, 1.0);
      resize_index++;
    }
    ...
    // dump the output
    SelectOutputBoxes(crop_box_list_, resize_scale_list_, resize_scale_list_, &detected_bboxes_);
  }
...
bool TrafficLightDetection::Detect(const TrafficLightDetectorOptions& options, CameraFrame* frame) {
...
bool TrafficLightDetection::SelectOutputBoxes(const std::vector<base::RectI>& crop_box_list,
  ...
  AINFO << "output blob size " << output_blob->shape(0) << " " << output_blob->shape(1) << " "
        << output_blob->shape(2) << " " << output_blob->shape(3);
  AINFO << "result box number: " << result_box_num << " each box length: " << each_box_length;
  ...
  for (int candidate_id = 0; candidate_id < result_box_num; candidate_id++) {
    ...
    if (static_cast<int>(tmp->region.detect_class_id) >= 0) {
      ...
      if (OutOfValidRegion(tmp->region.detection_roi, crop_box_list.at(img_id).width,
        ...
        AINFO << " max width " << crop_box_list.at(img_id).width << " max height "
              << crop_box_list.at(img_id).height << " at img_id " << img_id;
        continue;
      }
      ...
      AINFO << "detect roi x " << tmp->region.detection_roi.x << " " << tmp->region.detection_roi.y
            << " " << tmp->region.detection_roi.width << " " << tmp->region.detection_roi.height;

      lights->push_back(tmp);
    } else {
      AWARN << "Invalid class id: " << static_cast<int>(tmp->region.detect_class_id);
    }
  }
...
void TrafficLightDetection::ApplyNMS(std::vector<base::TrafficLightPtr>* lights,
  ...
  while (!score_index_vec.empty()) {
    ...
    if (keep) { kept_indices.push_back(idx); }
...
std::string TrafficLightDetection::Name() const { return "TrafficLightDetection"; }
