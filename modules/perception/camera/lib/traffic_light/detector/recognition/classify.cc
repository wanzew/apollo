...
void ClassifyBySimple::Init(const traffic_light::recognition::ClassifyParam& model_config,
                            const int                                        gpu_id,
  ...
  std::string model_root = GetAbsolutePath(work_root, model_config.model_name());

  std::string proto_file = GetAbsolutePath(work_root, model_config.proto_file());
  AINFO << "proto_file " << proto_file;

  std::string weight_file = GetAbsolutePath(work_root, model_config.weight_file());

  AINFO << "model_root" << model_root;

  rt_net_.reset(inference::CreateInferenceByName(model_config.model_type(), proto_file, weight_file,
  ...
  if (!rt_net_->Init(input_reshape)) { AWARN << "net init fail."; }
...
void ClassifyBySimple::Perform(const CameraFrame*                  frame,
  ...
  for (base::TrafficLightPtr light : *lights) {
    if (!light->region.is_detected) { continue; }
    ...
    inference::ResizeGPU(*image_, input_blob_recog, frame->data_provider->src_width(), 0, mean[0],
...
void ClassifyBySimple::Prob2Color(const float*          out_put_data,
                                  float                 threshold,
                                  base::TrafficLightPtr light) {
  int                        max_color_id = 0;
  std::vector<base::TLColor> status_map   = {base::TLColor::TL_BLACK, base::TLColor::TL_RED,
                                           base::TLColor::TL_YELLOW, base::TLColor::TL_GREEN};
  ...
  max_color_id =
      (*max_prob > threshold) ? static_cast<int>(std::distance(prob.begin(), max_prob)) : 0;
