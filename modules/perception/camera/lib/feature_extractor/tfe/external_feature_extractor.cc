...
bool ExternalFeatureExtractor::Init(const FeatureExtractorInitOptions& options) {
  ...
  std::string              proto_file  = GetAbsolutePath(options.root_dir, param_.proto_file());
  std::string              weight_file = GetAbsolutePath(options.root_dir, param_.weight_file());
  ...
  inference_.reset(inference::CreateInferenceByName(model_type, proto_file, weight_file,
                                                    output_names, input_names, options.root_dir));
  ...
  std::map<std::string, std::vector<int>> shape_map{{param_.input_blob(), shape}};
...
bool ExternalFeatureExtractor::InitFeatureExtractor(const std::string& root_dir) {
...
bool ExternalFeatureExtractor::Extract(const FeatureExtractorOptions& options, CameraFrame* frame) {
  ...
  auto offset_y_ = static_cast<int>(param_.offset_ratio() * static_cast<float>(raw_height) + 0.5f);
  image_options.crop_roi = base::RectI(0, offset_y_, raw_width, raw_height - offset_y_);
  ...
  feature_extractor_->set_roi(image_options.crop_roi.x, image_options.crop_roi.y,
                              image_options.crop_roi.width, image_options.crop_roi.height);
  feature_extractor_->Extract(feat_options, frame);
  AINFO << "Extract Done";
  return true;
}
std::string ExternalFeatureExtractor::Name() const { return "ExternalFeatureExtractor"; }
