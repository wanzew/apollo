...
int lane_postprocessor_eval() {
  ...
  if (!common::LoadBrownCameraIntrinsic("params/front_6mm_intrinsics.yaml", &model)) {
  ...
  for (int i = 0; i < static_cast<int>(imnames.size()); ++i) {
    ...
    if (FLAGS_debug_file.length() > 0) {
      if (FLAGS_file_title.find(FLAGS_debug_file) == std::string::npos) { continue; }
    }
    ...
    frame.data_provider->FillImageData(img.rows, img.cols,
    ...
    calibration_service_init_options.calibrator_working_sensor_name = "onsmi_obstacle";
    ...
    calibration_service.reset(
        BaseCalibrationServiceRegisterer::GetInstanceByName("OnlineCalibration"));
    ...
    calibration_service->SetCameraHeightAndPitch(name_camera_ground_height_map,
                                                 name_camera_pitch_angle_diff_map, pitch_angle);
    ...
    lane_postprocessor->GetLaneCCs(&lane_map, &lane_map_width, &lane_map_height, &lane_ccs,
                                   &select_lane_ccs);
    ...
    if (FLAGS_lane_line_debug) {
      save_img_path =
          absl::StrCat(FLAGS_save_dir, "/", FLAGS_file_title, "_0_", FLAGS_file_ext_name, ".jpg");
      ...
      save_img_path =
          absl::StrCat(FLAGS_save_dir, "/", FLAGS_file_title, "_1_", FLAGS_file_ext_name, ".jpg");
      show_detect_point_set(img, detect_laneline_point_set, save_img_path);
      AINFO << "detect_laneline_point_set num: " << detect_laneline_point_set.size();
    }
    // Draw the lane map, draw the connected_components
    if (FLAGS_lane_cc_debug) {
      save_img_path =
          absl::StrCat(FLAGS_save_dir, "/", FLAGS_file_title, "_2_", FLAGS_file_ext_name, ".jpg");
      show_lane_ccs(lane_map, lane_map_width, lane_map_height, lane_ccs, select_lane_ccs,
                    save_img_path);
    }
    if (FLAGS_lane_line_debug) {
      save_img_path =
          absl::StrCat(FLAGS_save_dir, "/", FLAGS_file_title, "_5_", FLAGS_file_ext_name, ".jpg");
      show_lane_lines(img, frame.lane_objects, save_img_path);
    }
    if (FLAGS_lane_result_output) {
      std::string save_path = absl::StrCat(FLAGS_save_dir, "/", FLAGS_file_title, ".txt");
