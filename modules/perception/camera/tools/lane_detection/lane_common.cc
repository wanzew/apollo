...
DEFINE_string(camera_intrinsics_yaml, "params/front_6mm_intrinsics.yaml", "camera intrinsics_yaml");
...
void show_detect_point_set(const cv::Mat&                                 image,
  ...
  for (size_t line_idx = 0; line_idx < detect_laneline_point_set.size(); ++line_idx) {
    if (line_idx == 0) {
...
void show_lane_lines(const cv::Mat&                     image,
  ...
  for (size_t i = 0; i < lane_marks.size(); ++i) {
    ...
    for (int j = static_cast<int>(y0); j <= static_cast<int>(y1); ++j) {
      int x = static_cast<int>(fa * pow(j, 3) + fb * pow(j, 2) + fc * static_cast<float>(j) + fd);
    ...
    for (float j = camera_xs; j <= camera_xe; j += 0.1f) {
      float y      = static_cast<float>(pow(j, 3) * camera_fa + pow(j, 2) * camera_fb +
      ...
      if (draw_x < 0 || draw_x >= ipm_width) { continue; }
      if (draw_y < 0 || draw_y >= ipm_height) { continue; }
      cv::circle(draw_ipm, cv::Point(draw_x, draw_y), 2, color);
    }
  }
  cv::circle(draw_ipm, cv::Point(ipm_width / 2, ipm_height - 2), 5, cv::Scalar(0, 0, 255), 5);
  cv::line(draw_ipm, cv::Point(ipm_width / 2, 0), cv::Point(ipm_width / 2, ipm_height),
           cv::Scalar(255, 255, 255));
  for (int i = 10; i < static_cast<int>(range_x); i += 10) {
    int x = ipm_width / 2;
    int y = ipm_height - static_cast<int>(static_cast<float>(i) * pixels_per_meter);
    cv::line(draw_ipm, cv::Point(0, y), cv::Point(ipm_width, y), cv::Scalar(255, 255, 255));
    std::string label = cv::format("%d", i);
    cv::putText(draw_ipm, label, cv::Point(x, y), CV_FONT_HERSHEY_COMPLEX_SMALL, 1.0,
                cv::Scalar(255, 255, 255));
  }
...
void show_lane_ccs(const std::vector<unsigned char>&      lane_map,
                   ...
                   const std::string&                     save_path) {
  cv::Mat lane_map_draw = cv::Mat::zeros(lane_map_height, lane_map_width, CV_8UC1);
  for (int y = 0; y < lane_map_height; ++y) {
    for (int x = 0; x < lane_map_width; x++) {
      lane_map_draw.at<unsigned char>(y, x) =
          static_cast<unsigned char>(255 - lane_map[y * lane_map_width + x] * 255);
    }
  }
  cv::Mat lane_draw;
  ...
  for (size_t i = 0; i < lane_ccs.size(); ++i) {
    ...
    for (size_t j = 0; j < select_lane_ccs.size(); ++j) {
      ...
      if (bbox1.xmin == bbox2.xmin && bbox1.xmax == bbox2.xmax && bbox1.ymin == bbox2.ymin &&
...
void output_laneline_to_json(const std::vector<base::LaneLine>& lane_objects,
  ...
  for (int j = 0; j < lane_line_size; ++j) {
    const base::LaneLineCubicCurve&    curve_camera     = lane_objects[j].curve_camera_coord;
    const base::LaneLineCubicCurve&    curve_img        = lane_objects[j].curve_image_coord;
    const std::vector<base::Point3DF>& camera_point_set = lane_objects[j].curve_camera_point_set;
    const std::vector<base::Point2DF>& image_point_set  = lane_objects[j].curve_image_point_set;
    fprintf(file_save, "{\n");
    fprintf(file_save, "\"type\":%d,\n", lane_objects[j].type);
    fprintf(file_save, "\"pos_type\":%d,\n", lane_objects[j].pos_type);
    //  camera curve
    fprintf(file_save, "\"camera_curve\":\n");
    fprintf(file_save, "{\"a\":%.10f,\"b\":%.10f,\"c\":%.10f,\"d\":%.10f,", curve_camera.a,
    ...
    for (size_t k = 0; k < camera_point_set.size(); ++k) {
      if (k < camera_point_set.size() - 1) {
        fprintf(file_save, "{\"x\":%.4f,\"y\":%.4f,\"z\":%.4f},", camera_point_set[k].x,
                camera_point_set[k].y, camera_point_set[k].z);
      } else {
        fprintf(file_save, "{\"x\":%.4f,\"y\":%.4f,\"z\":%.4f}", camera_point_set[k].x,
                camera_point_set[k].y, camera_point_set[k].z);
      }
    ...
    fprintf(file_save, "{\"a\":%.10f,\"b\":%.10f,\"c\":%.10f,\"d\":%.10f,", curve_img.a,
            curve_img.b, curve_img.c, curve_img.d);
    ...
    for (size_t k = 0; k < image_point_set.size(); ++k) {
      if (k < image_point_set.size() - 1) {
        fprintf(file_save, "{\"x\":%.4f,\"y\":%.4f},", image_point_set[k].x, image_point_set[k].y);
      } else {
        fprintf(file_save, "{\"x\":%.4f,\"y\":%.4f}", image_point_set[k].x, image_point_set[k].y);
      }
    }
...
void output_laneline_to_txt(const std::vector<base::LaneLine>& lane_objects,
  ...
  for (int j = 0; j < lane_line_size; ++j) {
    ...
    fprintf(file_save, "pos_type=%d\n", lane_objects[j].pos_type);
    fprintf(file_save, "camera_curve:\n");
    fprintf(file_save, "a=%f b=%f c=%f d=%f x0=%f x1=%f\n", curve_camera.a, curve_camera.b,
            curve_camera.c, curve_camera.d, curve_camera.x_start, curve_camera.x_end);
    fprintf(file_save, "image_curve:\n");
    fprintf(file_save, "a=%f b=%f c=%f d=%f x0=%f x1=%f\n", curve_img.a, curve_img.b, curve_img.c,
            curve_img.d, curve_img.x_start, curve_img.x_end);
    fprintf(file_save, "confidence=%f\n", lane_objects[j].confidence);
    std::vector<base::Point2DF> curve_image_point_set  = lane_objects[j].curve_image_point_set;
    std::vector<base::Point3DF> curve_camera_point_set = lane_objects[j].curve_camera_point_set;
    fprintf(file_save, "curve_image_point_set:\n");
    for (size_t k = 0; k < curve_image_point_set.size(); ++k) {
      fprintf(file_save, "[%f %f] ", curve_image_point_set[k].x, curve_image_point_set[k].y);
    }
    fprintf(file_save, "\n");
    fprintf(file_save, "curve_camera_point_set:\n");
    for (size_t k = 0; k < curve_camera_point_set.size(); ++k) {
      fprintf(file_save, "[%f %f %f] ", curve_camera_point_set[k].x, curve_camera_point_set[k].y,
              curve_camera_point_set[k].z);
    }
    fprintf(file_save, "\n");
  }
  fclose(file_save);
...
void show_detect_point_set(const cv::Mat&                     image,
  ...
  for (size_t i = 0; i < img_laneline_point_set.size(); ++i) {
    ...
    cv::circle(draw_mat, cv::Point(static_cast<int>(point.x), static_cast<int>(point.y)), draw_size,
               color, 4);
  }
  cv::imwrite(save_path, draw_mat);
}

void show_neighbor_point_set(const cv::Mat&                     image,
                             const std::vector<base::Point2DF>& img_laneline_point_set,
                             const std::vector<int>&            neighbor_point_info,
                             const std::string&                 save_path) {
  cv::Scalar color     = cv::Scalar(0, 255, 0);
  int        draw_size = 2;
  ...
  for (size_t i = 0; i < img_laneline_point_set.size(); ++i) {
    ...
    if (neighbor_point_index != -1) {
      ...
      const base::Point2DF& neighbor_point = img_laneline_point_set[neighbor_point_index];
      cv::line(draw_mat, cv::Point(static_cast<int>(point.x), static_cast<int>(point.y)),
               cv::Point(static_cast<int>(neighbor_point.x), static_cast<int>(neighbor_point.y)),
               color, 2);
    }
    std::string label = cv::format("%d", pass_point_num);
    cv::putText(draw_mat, label, cv::Point(static_cast<int>(point.x), static_cast<int>(point.y)),
...
void show_detect_point_set(const cv::Mat&                     image,
                           const std::vector<base::Point2DF>& img_laneline_point_set,
                           const std::vector<float>&          point_score_vec,
                           const std::string&                 save_path) {
  cv::Scalar color     = cv::Scalar(0, 255, 0);
  int        draw_size = 2;
