...
bool TransformServer::Init(const std::vector<std::string>& camera_names,
  ...
  // 1. Init lidar height
  try {
    YAML::Node      lidar_height = YAML::LoadFile(params_dir + "/" + "velodyne128_height.yaml");
    Eigen::Affine3d trans;
    trans.linear() = Eigen::Matrix3d::Identity();
    AINFO << trans.translation() << " " << lidar_height["vehicle"]["parameters"]["height"];
    trans.translation() << 0.0, 0.0, lidar_height["vehicle"]["parameters"]["height"].as<double>();
  ...
  extrinsic_filelist.push_back(params_dir + "/velodyne128_novatel_extrinsics.yaml");
  for (const auto& camera_name : camera_names) {
    extrinsic_filelist.push_back(params_dir + "/" + camera_name + "_extrinsics.yaml");
  }
  ...
  for (const auto& yaml_file : extrinsic_filelist) {
    try {
      ...
      if (!AddTransform(child_frame_id, frame_id, trans)) {
        AINFO << "failed to add transform from " << child_frame_id << " to " << frame_id
              << std::endl;
      }
    } catch (YAML::InvalidNode& in) {
...
bool TransformServer::LoadFromFile(const std::string& tf_input, float frequency) {
  if (frequency <= 0) {
  ...
  AINFO << "Load tf successfully. count: " << tf_.size() << " error limit:" << error_limit_;
  return true;
}
...
bool TransformServer::AddTransform(const std::string&     child_frame_id,
  ...
  for (auto iter = begin; iter != end; ++iter) {
    if (iter->second.frame_id == frame_id) { return false; }
  }
...
bool TransformServer::QueryTransform(const std::string& child_frame_id,
  ...
  *transform = Eigen::Affine3d::Identity();
  ...
  if (child_frame_id == frame_id) { return true; }
...
bool TransformServer::FindTransform(const std::string&           child_frame_id,
  ...
  for (auto iter = begin; iter != end; ++iter) {
    ...
    if ((*visited)[edge.frame_id]) { continue; }

    ADEBUG << "from " << edge.child_frame_id << " to " << edge.frame_id << std::endl;
...
void TransformServer::print() {
  for (auto item : edges_) {
    ...
    AINFO << "edge: " << std::endl;
    AINFO << "from " << item.second.child_frame_id << " to " << item.second.frame_id << std::endl;
    Eigen::Affine3d    trans = item.second.transform;
    Eigen::Quaterniond quat(trans.linear());
    AINFO << "rot: " << quat.x() << " " << quat.y() << " " << quat.z() << " " << quat.w()
          << std::endl;
    AINFO << "trans: " << trans.translation()[0] << " " << trans.translation()[1] << " "
