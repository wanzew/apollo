...
DEFINE_string(params_dir, "/apollo/modules/perception/data/params", "params directory");
...
DEFINE_string(undistortion_save_dir,
              "./undistortion_result",
              "Directory to save undistored images.");
DEFINE_string(save_dir, "./result", "Directory to save result images with detections.");
...
void save_image(const std::string& path, base::Image8U& image) {  // NOLINT
  ...
  cv::Mat cv_img(image.rows(), image.cols(), cv_type, image.mutable_cpu_data(), image.width_step());
...
int work() {
  ...
  // Init camera list
  const std::vector<std::string> camera_names = absl::StrSplit(FLAGS_sensor_name, ',');
  ...
  for (size_t i = 0; i < camera_names.size(); ++i) {
    ...
    name_provider_map.insert(
        std::pair<std::string, DataProvider*>(camera_names[i], &data_providers[i]));
  ...
  for (const auto& camera_name : camera_names) {
    ...
    AINFO << "#intrinsics of " << camera_name << ": " << intrinsic_map[camera_name];
  }
  ...
  // Init transform
  if (FLAGS_tf_file != "") { ACHECK(transform_server.LoadFromFile(FLAGS_tf_file)); }
  ...
  for (size_t i = 0; i < camera_names.size(); ++i) {
    ...
    if (!transform_server.QueryTransform(camera_names[i], "ground", &c2g)) {
      AINFO << "Failed to query transform from " << camera_names[i] << " to ground";
      return -1;
    }
    ...
    if (FLAGS_base_camera_name == camera_names[i]) {
      ...
      if (!transform_server.QueryTransform(camera_names[i], FLAGS_base_camera_name, &trans)) {
        AINFO << "Failed to query transform";
        return -1;
      }
      Eigen::Vector3d euler = trans.linear().eulerAngles(0, 1, 2);
      pitch_diff            = static_cast<float>(euler(0));
      if (pitch_diff > 10.0 * M_PI / 180.0) { pitch_diff = 0; }
      project_matrix = intrinsic_map.at(FLAGS_base_camera_name).cast<double>() * trans.linear() *
  ...
  perception.SetCameraHeightAndPitch(name_camera_ground_height_map,
                                     name_camera_pitch_angle_diff_map, kDefaultPitchAngle);
  ...
  while (fin >> line) {
    ...
    if (temp_strs.size() != 2) { AERROR << "invaid format in " << FLAGS_test_list; }
    ...
    if (frame.timestamp < 1e-3) { frame.timestamp = 1.0 / FLAGS_camera_fps * frame_id; }
    AINFO << "Timestamp: " << std::fixed << std::setprecision(10) << frame.timestamp;

    if (FLAGS_base_camera_name == camera_name) {
      frame.project_matrix = Eigen::Matrix3d::Identity();
    } else {
      Eigen::Affine3d trans;
      if (!transform_server.QueryTransform(camera_name, FLAGS_base_camera_name, &trans)) {
        AINFO << "Failed to query transform from " << camera_name << " to "
              << FLAGS_base_camera_name;
        return -1;
      }
      frame.project_matrix = intrinsic_map.at(FLAGS_base_camera_name).cast<double>() *
                             trans.linear() *
    ...
    Eigen::Affine3d pose;
    if (!transform_server.QueryPos(frame.timestamp, &pose)) { pose.setIdentity(); }
    ...
    frame.data_provider->FillImageData(image.rows, image.cols, (const uint8_t*)(image.data),
                                       "bgr8");
    ...
    if (FLAGS_do_undistortion && (FLAGS_undistortion_save_dir != "") &&
      ...
      AINFO << "Undistorted image saved to : " << save_dir + "/" + image_name + FLAGS_image_ext;
    }
    ...
    if (!cyber::common::PathExists(save_dir)) {
      ...
      for (auto obj : frame.detected_objects) {
        ...
        myfile << base::kSubType2NameMap.at(obj->sub_type) << " " << static_cast<int>(box.xmin)
               << " " << static_cast<int>(box.ymin) << " " << static_cast<int>(box.xmax) << " "
               << static_cast<int>(box.ymax) << " "
               << obj->sub_type_probs[static_cast<int>(obj->sub_type)] << "\n";
        cv::rectangle(image, cv::Point(static_cast<int>(box.xmin), static_cast<int>(box.ymin)),
      ...
      AINFO << "Result saved to : " << save_dir + "/" + image_name + FLAGS_image_ext;
    }
  }
  return 0;
}
