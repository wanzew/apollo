...
bool Cipv::Init(const Eigen::Matrix3d& homography_im2car, const CipvInitOptions& options) {
  b_image_based_cipv_ = options.image_based_cipv;
  debug_level_        = options.debug_level;
  // 0: no debug message
  // 1: minimal output
  ...
  margin_vehicle_to_lane_ = (average_lane_width_in_meter_ - max_vehicle_width_in_meter_) * 0.5f;
  ...
  half_virtual_egolane_width_in_meter_   = single_virtual_egolane_width_in_meter_ * 0.5f;
...
bool Cipv::Process(CameraFrame*                 frame,
  ...
  DetermineCipv(frame->lane_objects, options, world2camera, &frame->tracked_objects);
...
bool Cipv::DistanceFromPointToLineSegment(const Point2Df& point,
  ...
  apollo::common::math::LineSegment2d line_seg({line_seg_start_point(0), line_seg_start_point(1)},
...
bool Cipv::GetEgoLane(const std::vector<base::LaneLine>& lane_objects,
  ...
  for (size_t i = 0; i < lane_objects.size(); ++i) {
    ...
    const size_t curve_image_point_size         = lane_object.curve_image_point_set.size();
    const size_t curve_car_coord_point_set_size = lane_object.curve_car_coord_point_set.size();
    if (lane_object.pos_type == base::LaneLinePositionType::EGO_LEFT) {
      if (debug_level_ >= 2) {
        ...
        AINFO << "[GetEgoLane]LEFT_ground_lane_objects[" << i
              << "].curve_car_coord_point_set_size: " << curve_car_coord_point_set_size;
      }
...
bool Cipv::MakeVirtualLane(const LaneLineSimple& ref_lane_line,
  ...
  for (uint32_t i = 0; i < ref_lane_line.line_point.size(); ++i) {
    virtual_lane_line->line_point.emplace_back(ref_lane_line.line_point[i](0),
                                               ref_lane_line.line_point[i](1) + offset_distance);
  }
  return true;
}

float Cipv::VehicleDynamics(const uint32_t tick,
                            const float    yaw_rate,
...
float Cipv::VehicleDynamics(const uint32_t tick,
  ...
  *center_y               = displacement * static_cast<float>(sin(theta));
  if (theta < 0) { offset = -half_vehicle_width; }
  *left_x  = (displacement + offset) * static_cast<float>(cos(theta)) - offset;
  *left_y  = (displacement + offset) * static_cast<float>(sin(theta)) + half_vehicle_width;
  *right_x = (displacement - offset) * static_cast<float>(cos(theta)) + offset;
  *right_y = (displacement - offset) * static_cast<float>(sin(theta)) - half_vehicle_width;
...
bool Cipv::MakeVirtualEgoLaneFromYawRate(const float     yaw_rate,
  ...
  for (uint32_t i = 1; i < kMaxNumVirtualLanePoint; ++i) {
    VehicleDynamics(i, yaw_rate, velocity, kAverageFrameRate, offset_distance, &center_x, &center_y,
...
bool Cipv::ElongateEgoLane(const std::vector<base::LaneLine>& lane_objects,
  ...
  if (b_left_valid && b_right_valid) {
    ...
    if (debug_level_ >= 2) { AINFO << "Both lanes are fine"; }
    ...
    if (debug_level_ >= 2) { AINFO << "Made left lane with offset: " << offset_distance; }
    ...
    if (debug_level_ >= 2) { AINFO << "Made right lane with offset: " << offset_distance; }
  }
  return true;
}

// Create virtual lane line
bool Cipv::CreateVirtualEgoLane(const float yaw_rate,
                                const float velocity,
                                EgoLane*    egolane_ground) {
  float offset_distance = half_vehicle_width_in_meter_;
  // Generate new egolane using yaw-rate velocity
  MakeVirtualEgoLaneFromYawRate(yaw_rate, velocity, offset_distance, &egolane_ground->left_line,
                                &egolane_ground->right_line);
  if (debug_level_ >= 2) {
    AINFO << "Made both lane_objects with size of " << egolane_ground->left_line.line_point.size();
  }
...
bool Cipv::FindClosestObjectImage(const std::shared_ptr<base::Object>& object,
  ...
  float center_x = (object->camera_supplement.box.xmin + object->camera_supplement.box.xmin) * 0.5f;
  float center_y = object->camera_supplement.box.ymax;

  if (debug_level_ >= 3) {
    AINFO << "object->camera_supplement.box = base::RectF(" << object->camera_supplement.box.xmin
          << ", " << object->camera_supplement.box.ymin << ", "
          << object->camera_supplement.box.xmax - object->camera_supplement.box.xmin << ", "
          << object->camera_supplement.box.ymax - object->camera_supplement.box.ymin << ");";
  }
  ...
  *distance = static_cast<float>(sqrt(center_x * center_x + center_y * center_y));
  if (debug_level_ >= 2) {
...
bool Cipv::FindClosestObjectGround(const std::shared_ptr<base::Object>& object,
                                   ...
                                   LineSegment2Df*                      closted_object_edge,
                                   float*                               distance) {
  if (debug_level_ >= 2) { AINFO << "object->track_id = " << object->track_id; }
  float size_x = object->size(0);
  float size_y = object->size(1);
  float size_z = object->size(2);
  if (size_x < 1.0e-2 && size_y < 1.0e-2 && size_z < 1.0e-2) {
  ...
  if (debug_level_ >= 3) {
    ...
    AINFO << "object->camera_supplement.alpha = " << object->camera_supplement.alpha << ";";
    AINFO << "theta = " << theta << ";";
  ...
  if ((p[left_index](0) < 0 && p[right_index](0) < 0) || (p[closest_index](0) < 0)) {
    return false;
  }

  *distance = static_cast<float>(
      sqrt(p[closest_index](0) * p[closest_index](0) + p[closest_index](1) * p[closest_index](1)));
  if (debug_level_ >= 2) {
    AINFO << "start(" << closted_object_edge->start_point(0) << ", "
          << closted_object_edge->start_point(1) << ")->";
    AINFO << "end(" << closted_object_edge->end_point(0) << ", "
          << closted_object_edge->end_point(1) << ")";
    AINFO << "closest distance to p[" << closest_index << "](" << p[closest_index](0) << ", "
...
bool Cipv::AreDistancesSane(const float distance_start_point_to_right_lane,
  ...
  if (distance_start_point_to_right_lane > kMaxDistObjectToLaneInMeter) {
    if (debug_level_ >= 1) {
      AINFO << "distance from start to right lane(" << distance_start_point_to_right_lane
            << " m) is too long";
    }
    return false;
  }
  if (distance_start_point_to_left_lane > kMaxDistObjectToLaneInMeter) {
    if (debug_level_ >= 1) {
      AINFO << "distance from start to left lane(" << distance_start_point_to_left_lane
            << " m) is too long";
    }
    return false;
  }
  if (distance_end_point_to_right_lane > kMaxDistObjectToLaneInMeter) {
    if (debug_level_ >= 1) {
      AINFO << "distance from end to right lane(" << distance_end_point_to_right_lane
            << " m) is too long";
    }
    return false;
  }
  if (distance_end_point_to_left_lane > kMaxDistObjectToLaneInMeter) {
    if (debug_level_ >= 1) {
      AINFO << "distance from end to left lane(" << distance_end_point_to_left_lane
            << " m) is too long";
    }
    return false;
  }
  distance = static_cast<float>(
      fabs(distance_start_point_to_right_lane - distance_end_point_to_right_lane));
  if (distance > kMaxVehicleWidthInMeter) {
    if (debug_level_ >= 1) { AINFO << "width of vehicle (" << distance << " m) is too long"; }
    return false;
  }

  distance =
      static_cast<float>(fabs(distance_end_point_to_left_lane - distance_start_point_to_left_lane));
  if (distance > kMaxVehicleWidthInMeter) {
    if (debug_level_ >= 1) { AINFO << "width of vehicle (" << distance << " m) is too long"; }
    return false;
  }
...
bool Cipv::IsPointLeftOfLine(const Point2Df& point,
                             ...
                             const Point2Df& line_seg_end_point) {
  float cross_product =
      ((line_seg_end_point(0) - line_seg_start_point(0)) * (point(1) - line_seg_start_point(1))) -
      ((line_seg_end_point(1) - line_seg_start_point(1)) * (point(0) - line_seg_start_point(0)));

  if (cross_product > 0.0f) {
    if (debug_level_ >= 2) {
      AINFO << "point (" << point(0) << ", " << point(1) << ") is left of line_segment ("
  ...
  if (debug_level_ >= 2) {
    AINFO << "point (" << point(0) << ", " << point(1) << ") is right of line_segment ("
          << line_seg_start_point(0) << ", " << line_seg_start_point(1) << ")->("
          << line_seg_end_point(0) << ", " << line_seg_end_point(1)
          << "), cross_product: " << cross_product;
  }
...
bool Cipv::IsObjectInTheLaneImage(const std::shared_ptr<base::Object>& object,
  ...
  float b_valid_object =
      FindClosestObjectImage(object, egolane_image, &closted_object_edge, &distance);
  if (!b_valid_object) {
    if (debug_level_ >= 1) { AINFO << "The closest edge of an object is not available"; }
    return false;
  }
  *object_distance = distance;
  ...
  if (egolane_image.left_line.line_point.size() <= 1) {
    if (debug_level_ >= 1) { AINFO << "No left lane"; }
    return false;
  }
  ...
  for (size_t i = 0; i + 1 < egolane_image.left_line.line_point.size(); ++i) {
    ...
    distance = kMaxFloat;
    if (DistanceFromPointToLineSegment(closted_object_edge.end_point,
  ...
  if (closest_index >= 0) {
    ...
    if (IsPointLeftOfLine(closted_object_edge.end_point,
  ...
  if (egolane_image.right_line.line_point.size() <= 1) {
    if (debug_level_ >= 1) { AINFO << "No right lane"; }
    return false;
  }
  closest_index     = -1;
  shortest_distance = kMaxFloat;
  for (size_t i = 0; i + 1 < egolane_image.right_line.line_point.size(); ++i) {
    ...
    distance = kMaxFloat;
    if (DistanceFromPointToLineSegment(closted_object_edge.start_point,
  ...
  if (closest_index >= 0) {
    ...
    // Check if the end point is on the right of the line segment
    if (!IsPointLeftOfLine(closted_object_edge.start_point,
...
bool Cipv::IsObjectInTheLaneGround(const std::shared_ptr<base::Object>& object,
  ...
  float          max_dist_object_to_lane_in_meter =
      b_virtual ? max_dist_object_to_virtual_lane_in_meter_ : max_dist_object_to_lane_in_meter_;
  int closest_index = -1;
  // Find closest edge of a given object bounding box
  float b_valid_object = FindClosestObjectGround(object, egolane_ground, world2camera,
                                                 &closted_object_edge, &distance);
  if (!b_valid_object) {
    if (debug_level_ >= 1) { AINFO << "The closest edge of an object is not available"; }
    return false;
  }
...
bool Cipv::IsObjectInTheLane(const std::shared_ptr<base::Object>& object,
                             ...
                             const bool                           b_virtual,
                             float*                               distance) {
  if (b_image_based_cipv_) { return IsObjectInTheLaneImage(object, egolane_image, distance); }
  return IsObjectInTheLaneGround(object, egolane_ground, world2camera, b_virtual, distance);
}
...
bool Cipv::DetermineCipv(const std::vector<base::LaneLine>&          lane_objects,
  ...
  GetEgoLane(lane_objects, &egolane_image, &egolane_ground, &b_left_valid, &b_right_valid);
  ElongateEgoLane(lane_objects, b_left_valid, b_right_valid, options.yaw_rate, options.velocity,
                  &egolane_image, &egolane_ground);

  CreateVirtualEgoLane(options.yaw_rate, options.velocity, &virtual_egolane_ground);
  ...
  for (int32_t i = 0; i < static_cast<int32_t>(objects->size()); ++i) {
    ...
    if (IsObjectInTheLane((*objects)[i], egolane_image, egolane_ground, world2camera, false,
                          &distance) ||
        IsObjectInTheLane((*objects)[i], egolane_image, virtual_egolane_ground, world2camera, true,
                          &distance)) {
      ...
      if (debug_level_ >= 2) { AINFO << "current cipv_index: " << cipv_index; }
    }
    if ((*objects)[i]->track_id == old_cipv_track_id_) { old_cipv_index = cipv_index; }
  }
  if (debug_level_ >= 1) {
  ...
  if (cipv_index >= 0) {
    ...
    if (debug_level_ >= 1) { AINFO << "No cipv"; }
  }
...
bool Cipv::TranformPoint(const Eigen::VectorXf& in,
                         ...
                         Eigen::Vector3d*       out) {
  if (in.rows() != motion_matrix.cols()) { AERROR << "Matrix mismatch"; }
  Eigen::VectorXf trans_pt = motion_matrix * in;
  if (fabs(trans_pt(3)) < kFloatEpsilon) { return false; }
...
bool Cipv::CollectDrops(const base::MotionBufferPtr&                motion_buffer,
  ...
  if (debug_level_ >= 2) { AINFO << " motion_size: " << motion_size; }
  if (motion_size <= 0) {
  ...
  for (auto obj : *objects) {
    ...
    if (debug_level_ >= 2) { AINFO << "target ID: " << cur_id; }
    ...
    object_trackjectories_[cur_id].push_back(std::make_pair(center_x, center_y));
    ...
    Eigen::Matrix4f accum_motion_buffer = motion_buffer->at(motion_size - 1).motion;
    // Add drops
    for (std::size_t it = object_trackjectories_[cur_id].size() - 1, count = 0; it > 0; it--) {
      if (count >= kDropsHistorySize || count > motion_buffer->size()) { break; }
      if (static_cast<int>(it) + 1 > motion_size) { continue; }
      Eigen::VectorXf pt = Eigen::VectorXf::Zero((*motion_buffer)[0].motion.cols());
      ...
      if (debug_level_ >= 3) {
        ...
        AINFO << "target[" << obj->track_id << "][" << it << "]: (" << transformed_pt(0) << ", "
              << transformed_pt(1) << ")";
      }
  ...
  for (const auto& each_object : object_trackjectories_) {
    ...
    if (!b_found_id && object_trackjectories_[obj_id].size() > 0) {
      ...
      if (debug_level_ >= 2) {
        AINFO << "object_id_skip_count_[" << obj_id << " ]: " << object_id_skip_count_[obj_id];
      }
      if (object_id_skip_count_[obj_id] >= kMaxAllowedSkipObject) {
        if (debug_level_ >= 2) { AINFO << "Removed obsolete object " << obj_id; }
...
bool Cipv::image2ground(const float image_x,
                        const float image_y,
                        float*      ground_x,
                        float*      ground_y) {
  ...
  if (debug_level_ >= 1) { AINFO << "p_ground(2) too small :" << p_ground(2); }
  return false;
}

bool Cipv::ground2image(const float ground_x,
  ...
  if (debug_level_ >= 1) { AINFO << "p_image(2) too small :" << p_image(2); }
  return false;
}
