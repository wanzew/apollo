...
class Cipv : public BaseCipv {
 public:
  ...
  static float VehicleDynamics(const uint32_t tick,
 ...
 private:
  ...
  bool ElongateEgoLane(const std::vector<base::LaneLine>& lane_objects,
                       ...
                       EgoLane*                           egolane_image,
                       EgoLane*                           egolane_ground);
  bool CreateVirtualEgoLane(const float yaw_rate, const float velocity, EgoLane* egolane_ground);
  ...
  bool image2ground(const float image_x, const float image_y, float* ground_x, float* ground_y);
  bool ground2image(const float ground_x, const float ground_y, float* image_x, float* image_y);
  ...
  float margin_vehicle_to_lane_       = (kAverageLaneWidthInMeter - kMaxVehicleWidthInMeter) * 0.5f;
  float single_virtual_egolane_width_in_meter_    = kMaxVehicleWidthInMeter + kMarginVehicleToLane;
  float half_virtual_egolane_width_in_meter_      = single_virtual_egolane_width_in_meter_ * 0.5f;
  ...
  float max_dist_object_to_virtual_lane_in_meter_ = kMaxDistObjectToVirtualLaneInMeter;
