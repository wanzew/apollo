...
void LaneCameraPerception::InitLane(const std::string&          work_root,
                                    base::BaseCameraModelPtr&   model,
...
void LaneCameraPerception::InitCalibrationService(const std::string&             work_root,
...
void LaneCameraPerception::SetIm2CarHomography(Eigen::Matrix3d homography_im2car) {
  if (calibration_service_ == nullptr) {
    AERROR << "Calibraion service is not available";
    return;
  }
...
bool LaneCameraPerception::GetCalibrationService(BaseCalibrationService** calibration_service) {
  *calibration_service = calibration_service_.get();
  return true;
}

bool LaneCameraPerception::Perception(const CameraPerceptionOptions& options, CameraFrame* frame) {
  PERF_FUNCTION();
