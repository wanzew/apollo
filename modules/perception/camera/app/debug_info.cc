...
std::vector<std::string> type_string = {"Unknown", "Unknown", "Unknown", "Pedestrian",
                                        "Cyclist", "Car",     "Unknown"};
...
void WriteTracking(std::ofstream&                      fout,
  ...
  for (size_t i = 0; i < tracked_object.size(); ++i) {
    ...
    snprintf(output, sizeof(output),
             ...
             "%2.6f %2.6f %2.6f %.30s",
             frame_num, ptr->track_id, sub_type_string[static_cast<int>(ptr->sub_type)].c_str(),
             ptr->camera_supplement.alpha, ptr->camera_supplement.box.xmin,
             ptr->camera_supplement.box.ymin, ptr->camera_supplement.box.xmax,
             ptr->camera_supplement.box.ymax, ptr->size[2], ptr->size[1], ptr->size[0],
             ptr->center[0], ptr->center[1], ptr->center[2], ptr->theta,
             ptr->type_probs[static_cast<int>(ptr->type)], ptr->velocity[0], ptr->velocity[1],
             ptr->velocity[2], ptr->camera_supplement.sensor_name.c_str());
    fout << output << std::endl;
  }
}

int WriteDetections(const bool                          enabled,
  ...
  for (const auto& obj : objects) {
    switch (obj->type) {
      case base::ObjectType::VEHICLE: outf << "Car"; break;
      case base::ObjectType::BICYCLE: outf << "Cyclist"; break;
      case base::ObjectType::PEDESTRIAN: outf << "Pedestrian"; break;
      default: AERROR << "Unknown object type: " << static_cast<int>(obj->type); outf << "Unknown";
    }
...
int WriteDetections(const bool enabled, const std::string& out_path, CameraFrame* frame) {
  if (!enabled) { return -1; }
  ...
  int          feature_dim = frame->track_feature_blob->count() / frame->track_feature_blob->num();
...
int WriteLanelines(const bool                         enabled,
...
int WriteCalibrationOutput(bool enabled, const std::string& out_path, const CameraFrame* frame) {
  ...
  if (!frame->calibration_service->QueryCameraToGroundHeightAndPitchAngle(&camera_ground_height,
                                                                          &pitch_angle)) {
  ...
  if (camera_name == "front_12mm") {
    ...
    for (size_t i = 0; i < tracked_object.size(); ++i) {
      ...
      snprintf(output, sizeof(output),
               ...
               "%2.6f %2.6f %2.6f",
               frame_num, ptr->track_id, sub_type_string[static_cast<int>(ptr->sub_type)].c_str(),
               ptr->camera_supplement.alpha, ptr->camera_supplement.projected_box.xmin,
               ptr->camera_supplement.projected_box.ymin, ptr->camera_supplement.projected_box.xmax,
               ptr->camera_supplement.projected_box.ymax, ptr->size[2], ptr->size[1], ptr->size[0],
               ptr->center[0], ptr->center[1], ptr->center[2], ptr->theta,
               ptr->type_probs[static_cast<int>(ptr->type)], ptr->velocity[0], ptr->velocity[1],
               ptr->velocity[2]);
      fout << output << std::endl;
