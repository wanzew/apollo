...
class RTLogger : public nvinfer1::ILogger {
  void log(Severity severity, const char* msg) override {
    if (severity != Severity::kINFO) { AINFO << msg; }
  }
} rt_gLogger;
#define LOAD_DEBUG 0
...
void RTNet::ConstructMap(const LayerParameter& layer_param,
  ...
  for (int i = 0; i < layer_param.top_size(); i++) {
    ...
    if (it != tensor_modify_map->end()) {
      ...
      tensor_modify_map->insert(std::pair<std::string, std::string>(top_name, top_name));
    }
    tensor_map->insert(std::pair<std::string, nvinfer1::ITensor*>(top_name, layer->getOutput(i)));
...
void RTNet::addConvLayer(const LayerParameter&         layer_param,
  ...
  auto convLayer = net->addConvolution(*inTensor, nbOutputs, nvinfer1::DimsHW{kernelH, kernelW}, wt,
                                       bias_weight);
  ...
  if (convLayer) {
    ...
    int padW =
        p.has_pad_w() ? p.pad_w() : p.pad_size() > 1 ? p.pad(1) : p.pad_size() > 0 ? p.pad(0) : 0;
  ...
  for (int i = 0; i < 3; i++) {
    absl::StrAppend(&dim_string, " ", convLayer->getInput(0)->getDimensions().d[i]);
  }
...
void RTNet::addDeconvLayer(const LayerParameter&         layer_param,
  ...
  if ((*weight_map)[layer_param.name().c_str()].size() == 2) {
    deconvLayer = net->addDeconvolution(
        *inputs[0], conv.num_output(), nvinfer1::DimsHW{param.kernel_h, param.kernel_w},
        (*weight_map)[layer_param.name().c_str()][0], (*weight_map)[layer_param.name().c_str()][1]);
  } else {
    nvinfer1::Weights bias_weight{nvinfer1::DataType::kFLOAT, nullptr, 0};
    deconvLayer = net->addDeconvolution(*inputs[0], conv.num_output(),
...
void RTNet::addActiveLayer(const LayerParameter&         layer_param,
                           ...
                           TensorMap*                    tensor_map,
                           TensorModifyMap*              tensor_modify_map) {
  if (layer_param.type() == "ReLU" && layer_param.relu_param().negative_slope() > 0.0f) {
    std::shared_ptr<ReLUPlugin> relu_plugin;
    relu_plugin.reset(new ReLUPlugin(layer_param.relu_param(), inputs[0]->getDimensions()));
    nvinfer1::IPluginLayer* ReLU_Layer = net->addPlugin(inputs, nbInputs, *relu_plugin);
    ...
    if (pair != active_map.end()) { type = pair->second; }
...
void RTNet::addConcatLayer(const LayerParameter&         layer_param,
  ...
  nvinfer1::IConcatenationLayer* concatLayer = net->addConcatenation(inputs, nbInputs);
...
void RTNet::addPoolingLayer(const LayerParameter&         layer_param,
  ...
  nvinfer1::PoolingType pool_type = (pool.pool() == PoolingParameter_PoolMethod_MAX) ?
  ...
  if (pool.has_round_mode()) {
    switch (static_cast<int>(pool.round_mode())) {
      case 0: padding_mode = nvinfer1::PaddingMode::kCAFFE_ROUND_UP; break;
      case 1: padding_mode = nvinfer1::PaddingMode::kCAFFE_ROUND_DOWN; break;
      default: padding_mode = nvinfer1::PaddingMode::kCAFFE_ROUND_UP;
    }
  }
  ACHECK(modify_pool_param(&pool));
  nvinfer1::IPoolingLayer* poolLayer =
      net->addPooling(*inputs[0], pool_type,
                      {static_cast<int>(pool.kernel_h()), static_cast<int>(pool.kernel_w())});
  poolLayer->setStride(
      nvinfer1::DimsHW{static_cast<int>(pool.stride_h()), static_cast<int>(pool.stride_w())});
  poolLayer->setPadding(
      nvinfer1::DimsHW{static_cast<int>(pool.pad_h()), static_cast<int>(pool.pad_w())});
...
void RTNet::addSliceLayer(const LayerParameter&         layer_param,
  ...
  slice_plugin.reset(new SLICEPlugin(layer_param.slice_param(), inputs[0]->getDimensions()));
  nvinfer1::IPluginLayer* sliceLayer = net->addPlugin(inputs, nbInputs, *slice_plugin);
...
void RTNet::addInnerproductLayer(const LayerParameter&         layer_param,
  ...
  nvinfer1::IFullyConnectedLayer* fcLayer = net->addFullyConnected(
      *inputs[0], fc.num_output(), (*weight_map)[layer_param.name().c_str()][0], bias);
...
void RTNet::addScaleLayer(const LayerParameter&         layer_param,
                          ...
                          TensorMap*                    tensor_map,
                          TensorModifyMap*              tensor_modify_map) {
  std::vector<nvinfer1::Weights> lw(3, nvinfer1::Weights{nvinfer1::DataType::kFLOAT, nullptr, 0});
  if (layer_param.type() == "Power") {
    ...
    for (size_t i = 0; i < (*weight_map)[layer_param.name().c_str()].size(); i++) {
  ...
  nvinfer1::IScaleLayer* scaleLayer =
      net->addScale(*inputs[0], nvinfer1::ScaleMode::kCHANNEL, lw[1], lw[0], lw[2]);
...
void RTNet::addBatchnormLayer(const LayerParameter&         layer_param,
  ...
  nvinfer1::IScaleLayer* scaleLayer = net->addScale(
      *inputs[0], nvinfer1::ScaleMode::kCHANNEL, (*weight_map)[layer_param.name().c_str()][0],
...
void RTNet::addSoftmaxLayer(const LayerParameter&         layer_param,
  ...
  if (layer_param.has_softmax_param()) {
    ...
    softmax_plugin.reset(
        new SoftmaxPlugin(layer_param.softmax_param(), inputs[0]->getDimensions()));
    softmax_plugins_.push_back(softmax_plugin);
    nvinfer1::IPluginLayer* softmaxLayer = net->addPlugin(inputs, nbInputs, *softmax_plugin);
...
void RTNet::addEltwiseLayer(const LayerParameter&         layer_param,
  ...
  if (pair != eltwise_map.end()) { op = pair->second; }
  nvinfer1::IElementWiseLayer* eltwiseLayer = net->addElementWise(*inputs[0], *inputs[1], op);
...
void RTNet::addArgmaxLayer(const LayerParameter&         layer_param,
  ...
  argmax_plugin.reset(new ArgMax1Plugin(layer_param.argmax_param(), inputs[0]->getDimensions()));
  argmax_plugins_.push_back(argmax_plugin);
  nvinfer1::IPluginLayer* argmaxLayer = net->addPlugin(inputs, nbInputs, *argmax_plugin);
...
void RTNet::addPaddingLayer(const LayerParameter&         layer_param,
  ...
  nvinfer1::IPaddingLayer* padding_layer = net->addPadding(*inputs[0], pre_dims, post_dims);
...
void RTNet::addDFMBPSROIAlignLayer(const LayerParameter&         layer_param,
  ...
  dfmb_psroi_align_plugin.reset(
      new DFMBPSROIAlignPlugin(layer_param.dfmb_psroi_pooling_param(), input_dims, nbInputs));
  ...
  ConstructMap(layer_param, dfmb_psroi_align_layer, tensor_map, tensor_modify_map);
}
...
void RTNet::addRCNNProposalLayer(const LayerParameter&         layer_param,
  ...
  rcnn_proposal_plugin.reset(new RCNNProposalPlugin(
      layer_param.bbox_reg_param(), layer_param.detection_output_ssd_param(), input_dims));
...
void RTNet::addRPNProposalSSDLayer(const LayerParameter&         layer_param,
  ...
  ConstructMap(layer_param, rpn_proposal_ssd_layer, tensor_map, tensor_modify_map);
}
...
void RTNet::addLayer(const LayerParameter&         layer_param,
  ...
  if (layer_param.type() == "Convolution") {
    addConvLayer(layer_param, inputs, weight_map, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "Deconvolution") {
    addDeconvLayer(layer_param, inputs, weight_map, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "Sigmoid" || layer_param.type() == "TanH" ||
             layer_param.type() == "ReLU") {
    addActiveLayer(layer_param, inputs, nbInputs, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "Concat") {
    addConcatLayer(layer_param, inputs, nbInputs, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "Slice") {
    addSliceLayer(layer_param, inputs, nbInputs, net, tensor_map, tensor_modify_map);
    ...
    addPermuteLayer(layer_param, inputs, nbInputs, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "InnerProduct") {
    addInnerproductLayer(layer_param, inputs, weight_map, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "BatchNorm") {
    addBatchnormLayer(layer_param, inputs, weight_map, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "Scale") {
    addScaleLayer(layer_param, inputs, weight_map, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "Softmax") {
    addSoftmaxLayer(layer_param, inputs, nbInputs, net, tensor_map, tensor_modify_map);
    ...
    addArgmaxLayer(layer_param, inputs, nbInputs, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "Dropout") {
    AINFO << "skip dropout";
  } else if (layer_param.type() == "Power") {
    addScaleLayer(layer_param, inputs, weight_map, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "DFMBPSROIAlign") {
    addDFMBPSROIAlignLayer(layer_param, inputs, nbInputs, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "RCNNProposal") {
    addRCNNProposalLayer(layer_param, inputs, nbInputs, net, tensor_map, tensor_modify_map);
  } else if (layer_param.type() == "RPNProposalSSD") {
    addRPNProposalSSDLayer(layer_param, inputs, nbInputs, net, tensor_map, tensor_modify_map);
  } else {
...
bool RTNet::loadWeights(const std::string& model_file, WeightMap* weight_map) {
  ...
  for (int i = 0; i < net.layer_size(); i++) {
    ...
    for (int j = 0; j < net.layer(i).blobs_size(); j++) {
      ...
      if (net.layer(i).type() == "BatchNorm") { mergeBN(j, net.mutable_layer(i)); }
...
void RTNet::mergeBN(int index, LayerParameter* layer_param) {
  ...
  if (index == 0) {
    ...
    for (int k = 0; k < size; k++) {
      blob->set_data(k, 1.0f / static_cast<float>(sqrt(blob->data(k) * scale_factor + epsilon)));
    }
  }
}
...
RTNet::RTNet(const std::string&              net_file,
...
RTNet::RTNet(const std::string&              net_file,
  ...
  calibrator_ = new nvinfer1::Int8EntropyCalibrator(stream, 0, true, model_root_);
}
...
bool RTNet::shape(const std::string& name, std::vector<int>* res) {
  ...
  if (bindingIndex > static_cast<int>(buffers_.size())) { return false; }
  nvinfer1::DimsCHW dims =
      static_cast<nvinfer1::DimsCHW&&>(engine->getBindingDimensions(bindingIndex));
  res->resize(4);
...
bool RTNet::checkInt8(const std::string& gpu_name, nvinfer1::IInt8Calibrator* calibrator) {
...
bool RTNet::addInput(const TensorDimsMap&                           tensor_dims_map,
  ...
  for (auto dims_pair : tensor_dims_map) {
    ...
    auto data =
        network_->addInput(dims_pair.first.c_str(), nvinfer1::DataType::kFLOAT, dims_pair.second);
    CHECK_NOTNULL(data);
...
void RTNet::parse_with_api(const std::map<std::string, std::vector<int>>& shapes) {
  CHECK_GT(net_param_->layer_size(), 0);
  ...
  for (auto layer_param : order) {
    ...
    addLayer(layer_param, inputs.data(), layer_param.bottom_size(), &weight_map_, network_,
             &tensor_map, &tensor_modify_map_);
  }
...
RTNet::~RTNet() {
  if (is_own_calibrator_ && calibrator_ != nullptr) { delete calibrator_; }
  if (gpu_id_ >= 0) {
...
void RTNet::Infer() {
  ...
  for (auto name : output_names_) {
    auto blob = get_blob(name);
    if (blob != nullptr) { blob->mutable_gpu_data(); }
  }
}
std::shared_ptr<apollo::perception::base::Blob<float>> RTNet::get_blob(const std::string& name) {
