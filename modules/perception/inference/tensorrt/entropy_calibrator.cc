...
Int8EntropyCalibrator::Int8EntropyCalibrator(
    const apollo::perception::inference::BatchStream& stream,
    int                                               first_batch,
    bool                                              read_cache,
    std::string                                       network)
    : stream_(stream)
...
const void* Int8EntropyCalibrator::readCalibrationCache(size_t& length) {
  ...
  std::ifstream input(apollo::perception::inference::locateFile(network_, "CalibrationTable"),
                      std::ios::binary);
  input >> std::noskipws;
...
void Int8EntropyCalibrator::writeCalibrationCache(const void* cache, size_t length) {
  std::ofstream output(apollo::perception::inference::locateFile(network_, "CalibrationTable"),
                       std::ios::binary);
