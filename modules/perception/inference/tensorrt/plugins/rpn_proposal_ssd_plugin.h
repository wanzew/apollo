...
class RPNProposalSSDPlugin : public nvinfer1::IPlugin {
 public:
  RPNProposalSSDPlugin(const BBoxRegParameter&            bbox_reg_param,
    ...
    GenAnchorParameter gen_anchor_param = detection_output_ssd_param.gen_anchor_param();
    num_anchor_per_point_ =
        std::min(gen_anchor_param.anchor_width().size(), gen_anchor_param.anchor_height().size());
    ...
    min_size_mode_ = static_cast<int>(detection_output_ssd_param.min_size_mode());
