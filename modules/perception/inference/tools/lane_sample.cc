...
DEFINE_string(names_file, "./lane_parser/blob_names.txt", "path of output blob");
...
int main(int argc, char** argv) {
  ...
  std::vector<std::string> outputs{"loc_pred", "obj_pred", "cls_pred", "ori_pred",
                                   "dim_pred", "lof_pred", "lor_pred", "conv4_3"};
  ...
  if (FLAGS_int8) {
    ...
    rt_net = new apollo::perception::inference::RTNet(proto_file, weight_file, outputs, inputs,
                                                      model_root);
  } else {
    rt_net = new apollo::perception::inference::RTNet(proto_file, weight_file, outputs, inputs);
  }
  ...
  apollo::perception::inference::load_data<std::string>(FLAGS_test_list, &image_lists);
  std::vector<float> output_data_vec;

  const int count = width * height;
  for (auto& image_file : image_lists) {
    cv::Mat img = cv::imread(FLAGS_image_root + image_file + FLAGS_image_ext, CV_8UC1);
    ADEBUG << img.channels();
    ...
    for (auto& output_name : outputs) {
      ...
      std::vector<float> tmp_vec(blob->cpu_data(), blob->cpu_data() + blob->count());
      ...
      output_data_vec.insert(output_data_vec.end(), tmp_vec.begin(), tmp_vec.end());
    }
  }
