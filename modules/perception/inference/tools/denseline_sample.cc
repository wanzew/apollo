...
DEFINE_string(names_file, "./denseline_parser/blob_names.txt", "path of output blob");
...
int main(int argc, char** argv) {
  ...
  for (auto image_file : image_lists) {
    ...
    cudaMemcpy(input_blob->mutable_gpu_data(), input.data(), count * sizeof(float),
    ...
    for (auto output_name : outputs) {
      ...
      for (int i = 0; i < blob->count(); ++i) {
        sum += blob->cpu_data()[i];
        if (i < 100) { AINFO << blob->cpu_data()[i]; }
      }
