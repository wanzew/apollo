...
TorchNet::TorchNet(const std::string&              net_file,
...
void TorchNet::Infer() {
  ...
  torch::Tensor tensor_image =
      torch::from_blob(blob->data()->mutable_gpu_data(),
                       {blob->shape(1), blob->shape(2), blob->shape(3)}, torch::kFloat32);
  if (device_id_ >= 0) { tensor_image = tensor_image.to(device); }
