...
std::shared_ptr<Blob<float>> TorchDet::get_blob(const std::string& name) {
...
void TorchDet::Infer() {
  ...
  torch::Tensor tensor_param =
      torch::from_blob(input_param->data()->mutable_cpu_data(),
                       {input_param->shape(0), input_param->shape(2)}, torch::kFloat32);
  ...
  auto outputs = net_.forward({std::make_tuple(tensor_image, tensor_param)}).toTuple()->elements();
  ...
  torch::Tensor img_id  = torch::zeros({scores.sizes()}, torch::kFloat).to(device);
  torch::Tensor one_hot = torch::zeros({scores.size(0), 2}, torch::kFloat).to(device);

  torch::Tensor result = torch::cat({img_id, bbox, labels, scores, one_hot}, 1);
  blobs_[output_names_[0]]->Reshape(
      {static_cast<int>(result.size(0)), static_cast<int>(result.size(1)), 1, 1});
  if (result.size(0) == 0) { result = torch::zeros({1, 9}, torch::kFloat).to(device); }
