...
TEST(loadTest, test) { ACHECK(!apollo::perception::inference::load_binary_data("unknow.txt")); }
...
TEST(UtilTest, GemmTest) {
  ...
  inference::GPUGemmFloat(CblasNoTrans, CblasTrans, 2, 2, 2, 1, a.gpu_data(), b.gpu_data(), 0,
                          c.mutable_gpu_data());
  ...
  inference::GPUGemmFloat(CblasNoTrans, CblasNoTrans, 2, 2, 2, 1, a.gpu_data(), b.gpu_data(), 0,
                          c.mutable_gpu_data());
  out = c.cpu_data();
  EXPECT_EQ(out[0], 10);
  EXPECT_EQ(out[1], 13);
  ...
  inference::GPUGemmFloat(CblasTrans, CblasNoTrans, 2, 2, 2, 1, a.gpu_data(), b.gpu_data(), 0,
                          c.mutable_gpu_data());
  out = c.cpu_data();
  EXPECT_EQ(out[0], 14);
  EXPECT_EQ(out[1], 18);
...
TEST(UtilTest, test_resize_gpu) {
  ...
  nppiCopy_8u_C3R(src_image.gpu_data(), src_image.width_step(), src_blob->mutable_gpu_data(),
                  src_blob->count(2) * sizeof(uint8_t), roi);

  {
    std::vector<int> shape = {1, img.rows * 0.5, img.cols * 0.5, img.channels()};
...

  {
    ...
    EXPECT_TRUE(inference::ResizeGPU(src_image, dst_blob, width, 0, mean_bgr[0], mean_bgr[1],
                                     mean_bgr[2], true, 1.0));
    EXPECT_EQ(dst_blob->shape(0), 1);
    EXPECT_EQ(dst_blob->shape(1), src_image.rows() * 0.5);
    EXPECT_EQ(dst_blob->shape(2), src_image.cols() * 0.5);
  ...
  {
    std::vector<int> shape = {1, img.channels(), img.rows * 0.5, img.cols * 0.5};
    std::shared_ptr<apollo::perception::base::Blob<float>> dst_blob(
        new apollo::perception::base::Blob<float>(shape));
    int mean_bgr[3] = {128, 128, 128};
    EXPECT_TRUE(inference::ResizeGPU(src_image, dst_blob, width, 0, mean_bgr[0], mean_bgr[1],
                                     mean_bgr[2], false, 1.0));
    EXPECT_EQ(dst_blob->shape(0), 1);
...

  {
    ...
    std::shared_ptr<apollo::perception::base::Blob<float>> dst_blob(
        new apollo::perception::base::Blob<float>(shape));
    int mean_bgr[3] = {128, 128, 128};
    EXPECT_FALSE(inference::ResizeGPU(src_image, dst_blob, width, 0, mean_bgr[0], mean_bgr[1],
                                      mean_bgr[2], true, 1.0));
  }

  {
    ...
    std::shared_ptr<apollo::perception::base::Blob<float>> dst_blob(
        new apollo::perception::base::Blob<float>(shape));
    int mean_bgr[3] = {128, 128, 128};
    EXPECT_TRUE(inference::ResizeGPU(*src_blob, dst_blob, width, 0, mean_bgr[0], mean_bgr[1],
                                     mean_bgr[2], true, 1.0));
...

  {
    ...
    std::shared_ptr<apollo::perception::base::Blob<float>> dst_blob(
        new apollo::perception::base::Blob<float>(shape));
    int mean_bgr[3] = {128, 128, 128};
    EXPECT_FALSE(inference::ResizeGPU(*src_blob, dst_blob, width, 0, mean_bgr[0], mean_bgr[1],
                                      mean_bgr[2], true, 1.0));
  }
}
}  // namespace inference
