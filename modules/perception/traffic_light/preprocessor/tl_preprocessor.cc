...
bool TLPreprocessor::Init() {
  ...
  if (!GetProtoFromFile(FLAGS_traffic_light_preprocessor_config, &config_)) {
    AERROR << "Cannot get config proto from file: " << FLAGS_traffic_light_preprocessor_config;
    return false;
  }
...
bool TLPreprocessor::CacheLightsProjections(const CarPose&             pose,
  ...
  AINFO << "TLPreprocessor has " << cached_lights_.size() << " lights projections cached.";

  // pop front if cached array'size > FLAGS_max_cached_image_lights_array_size
  while (cached_lights_.size() > static_cast<size_t>(config_.max_cached_lights_size())) {
  ...
  if (signals.size() > 0) {
    ...
    for (int cam_id = 0; cam_id < kCountCameraId; ++cam_id) {
      if (!ProjectLights(pose, signals, static_cast<CameraId>(cam_id),
                         lights_on_image[cam_id].get(), lights_outside_image[cam_id].get())) {
...
bool TLPreprocessor::SyncImage(ImageSharedPtr  image,
  ...
  for (; cached_lights_ptr != cached_lights_.rend(); ++cached_lights_ptr) {
    ...
    if (fabs(light_ts - image_ts) < config_.sync_interval_seconds()) {
      ...
      auto proj_cam_id_str = (kCameraIdToStr.find(proj_cam_id) != kCameraIdToStr.end() ?
      ...
      if (image_ts < last_output_ts_) {
        AWARN << "TLPreprocessor reject the image pub ts:" << GLOG_TIMESTAMP(image_ts)
              << " which is earlier than last output ts:" << GLOG_TIMESTAMP(last_output_ts_)
  ...
  if (sync_ok && cached_lights_ptr != cached_lights_.rend()) {
    *image_lights                       = *cached_lights_ptr;
    (*image_lights)->diff_image_pose_ts = image_ts - (*cached_lights_ptr)->timestamp;
    ...
    *should_pub         = true;
    ...
    if (fabs(image_ts - last_no_signals_ts_) < config_.no_signals_interval_seconds()) {
      ...
      AWARN << "TLPreprocessor " << cached_array_str
            << " sync failed, image ts: " << GLOG_TIMESTAMP(image_ts) << ", which is earlier than "
            << cached_array_str << ".front() ts: " << GLOG_TIMESTAMP(pose_ts)
            << ", diff between image and pose ts: " << GLOG_TIMESTAMP(image_ts - pose_ts)
            << "; system ts: " << GLOG_TIMESTAMP(system_ts)
            << ", diff between image and system ts: " << GLOG_TIMESTAMP(image_ts - system_ts)
            << ", camera_id: " << kCameraIdToStr.at(camera_id);
    } else if (image_ts > cached_lights_.back()->timestamp) {
      double pose_ts   = cached_lights_.back()->timestamp;
      double system_ts = TimeUtil::GetCurrentTime();
      AWARN << "TLPreprocessor " << cached_array_str
            << " sync failed, image ts: " << GLOG_TIMESTAMP(image_ts) << ", which is older than "
            << cached_array_str << ".back() ts: " << GLOG_TIMESTAMP(pose_ts)
            << ", diff between image and pose ts: " << GLOG_TIMESTAMP(image_ts - pose_ts)
            << "; system ts: " << GLOG_TIMESTAMP(system_ts)
            << ", diff between image and system ts: " << GLOG_TIMESTAMP(image_ts - system_ts)
            << ", camera_id: " << kCameraIdToStr.at(camera_id);
      ...
      AWARN << "TLPreprocessor " << cached_array_str
            ...
            << ".front() ts: " << GLOG_TIMESTAMP(cached_lights_.front()->timestamp) << ", "
            << cached_array_str
            << ".back() ts: " << GLOG_TIMESTAMP(cached_lights_.back()->timestamp)
            << ", camera_id: " << kCameraIdToStr.at(camera_id);
    }
  }
...
void TLPreprocessor::set_last_pub_camera_id(CameraId camera_id) { last_pub_camera_id_ = camera_id; }

CameraId TLPreprocessor::last_pub_camera_id() const { return last_pub_camera_id_; }

int TLPreprocessor::max_cached_lights_size() const { return config_.max_cached_lights_size(); }
...
void TLPreprocessor::SelectImage(const CarPose&     pose,
  ...
  *selection = static_cast<CameraId>(kShortFocusIdx);
  ...
  for (int cam_id = 0; cam_id < kCountCameraId; ++cam_id) {
    if (!lights_outside_image_array[cam_id]->empty()) { continue; }
    bool ok = true;
    // find the short focus camera without range check
    if (cam_id != kShortFocusIdx) {
      for (LightPtr light : *(lights_on_image_array[cam_id])) {
        if (IsOnBorder(cv::Size(config_.projection_image_cols(), config_.projection_image_rows()),
...
bool TLPreprocessor::IsOnBorder(const cv::Size  size,
  ...
  if (roi.x < border_size || roi.y < border_size) { return true; }
