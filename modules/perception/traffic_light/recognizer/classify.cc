...
void ClassifyBySimple::Perform(const cv::Mat& ros_image, std::vector<LightPtr>* lights) {
  ...
  caffe::Blob<float>* output_blob_recog =
      classify_net_ptr_->top_vecs()[classify_net_ptr_->top_vecs().size() - 1][0];
  cv::Mat img = ros_image(crop_box_);
  for (LightPtr light : *lights) {
    if (!light->region.is_detected || !BoxIsValid(light->region.rectified_roi, ros_image.size())) {
      continue;
    }
...
void ClassifyBySimple::ProbToColor(const float* out_put_data, float threshold, LightPtr light) {
  int                      max_color_id = 0;
