...
bool TLPreprocessorSubnode::InitInternal() {
  ...
  AdapterManager::AddImageLongCallback(&TLPreprocessorSubnode::SubLongFocusCamera, this);
  CHECK(AdapterManager::GetImageShort())
      << "TLPreprocessorSubnode init failed.ImageShort is not initialized.";
  AdapterManager::AddImageShortCallback(&TLPreprocessorSubnode::SubShortFocusCamera, this);
  return true;
}
...
bool TLPreprocessorSubnode::AddDataAndPublishEvent(const std::shared_ptr<ImageLights>& data,
  ...
  if (!SubnodeHelper::ProduceSharedDataKey(timestamp, device_str, &key)) {
    AERROR << "TLPreprocessorSubnode gen share data key failed. ts:" << GLOG_TIMESTAMP(timestamp);
    return false;
  }
...
void TLPreprocessorSubnode::SubLongFocusCamera(const sensor_msgs::Image& msg) {
  ...
  SubCameraImage(AdapterManager::GetImageLong()->GetLatestObservedPtr(), LONG_FOCUS);
  PERF_FUNCTION("SubLongFocusCamera");
}

void TLPreprocessorSubnode::SubShortFocusCamera(const sensor_msgs::Image& msg) {
  AdapterManager::Observe();
  SubCameraImage(AdapterManager::GetImageShort()->GetLatestObservedPtr(), SHORT_FOCUS);
  PERF_FUNCTION("SubShortFocusCamera");
}

void TLPreprocessorSubnode::SubCameraImage(boost::shared_ptr<const sensor_msgs::Image> msg,
                                           CameraId                                    camera_id) {
  ...
  if (FLAGS_output_raw_img) {
    ...
    image->GenerateMat();
    char filename[100];
    snprintf(filename, sizeof(filename), "%s/%lf.jpg", image->camera_id_str().c_str(), timestamp);
  ...
  if (last_proc_image_ts_ > 0.0 &&
    ...
    AINFO << "skip current image, img_ts: " << GLOG_TIMESTAMP(timestamp)
          << " ,because proc_interval_seconds_: " << GLOG_TIMESTAMP(proc_interval_seconds_);
    return;
  }
  ...
  if (fabs(image_lights->diff_image_sys_ts) > image_sys_ts_diff_threshold) {
    ...
    debug_string += ("diff_image_sys_ts:" + std::to_string(image_lights->diff_image_sys_ts));
    debug_string += (",camera_id:" + kCameraIdToStr.at(camera_id));
    debug_string += (",camera_ts:" + std::to_string(timestamp));

    AWARN << "image_ts - system_ts(in seconds): " << std::to_string(image_lights->diff_image_sys_ts)
          << ". Check if image timestamp drifts."
          << ", camera_id: " + kCameraIdToStr.at(camera_id) << ", debug_string: " << debug_string;
  }

  if (!should_pub) {
    AINFO << "TLPreprocessorSubnode not publish image, ts:" << GLOG_TIMESTAMP(image->ts())
          << ", camera_id: " << kCameraIdToStr.at(camera_id);
    return;
  }
  ...
  if (!VerifyLightsProjection(image_lights)) {
    AINFO << "verify_lights_projection on image failed, ts:" << GLOG_TIMESTAMP(image->ts())
          << ", camera_id: " << kCameraIdToStr.at(camera_id);
    return;
  }
  ...
  if (AddDataAndPublishEvent(image_lights, camera_id, image->ts())) {
    ...
    AINFO << "TLPreprocessorSubnode::sub_camera_image msg_time: " << GLOG_TIMESTAMP(image->ts())
          << " sync_image_latency: " << sync_image_latency * 1000 << " ms."
          << " sub_camera_image_latency: "
          << (TimeUtil::GetCurrentTime() - sub_camera_image_start_ts) * 1000 << " ms."
          << " camera_id: " << kCameraIdToStr.at(camera_id);
    AINFO << " number of lights: " << image_lights->lights->size();
  }
}

bool TLPreprocessorSubnode::GetSignals(double ts, CarPose* pose, std::vector<Signal>* signals) {
  // get pose
  if (!GetCarPose(ts, pose)) {
    AERROR << "camera_selection failed to get car pose, ts:" << GLOG_TIMESTAMP(ts);
    return false;
  }
...
bool TLPreprocessorSubnode::VerifyLightsProjection(ImageLightsPtr image_lights) {
  std::vector<Signal> signals;
  CarPose             pose;
  if (!GetSignals(image_lights->timestamp, &pose, &signals)) { return false; }
...
void TLPreprocessorSubnode::CameraSelection(double ts) {
  ...
  if (last_query_tf_ts_ > 0.0 &&
      current_ts - last_query_tf_ts_ <
          config_.tl_preprocessor_subnode_config().query_tf_inverval_seconds()) {
  ...
  CarPose             pose;
  std::vector<Signal> signals;
  if (!GetSignals(ts, &pose, &signals)) { return; }
