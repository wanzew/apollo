...
void SubDebugCallback(const TrafficLightDetection& tl_result) {
  ...
  if (!found_image) { return; }

  if (signals_num > 0 && tl_debug_msg.has_cropbox()) {
    // crop roi
    auto     crop_box = tl_debug_msg.cropbox();
    cv::Rect cv_crop_rect(crop_box.x(), crop_box.y(), crop_box.width(), crop_box.height());
    ...
    for (int box_idx = signals_num * 2; box_idx < box_size; ++box_idx) {
      ...
      cv::Rect cv_debug_roi_box(debug_roi_box.x(), debug_roi_box.y(), debug_roi_box.width(),
    ...
    for (int box_idx = signals_num; box_idx < signals_num * 2; ++box_idx) {
      ...
      cv::Rect cv_projection_box(projection_box.x(), projection_box.y(), projection_box.width(),
    ...
    for (int box_idx = 0; box_idx < signals_num; ++box_idx) {
      ...
      cv::Rect   cv_rectified_box(rectified_box.x(), rectified_box.y(), rectified_box.width(),
                                rectified_box.height());
      cv::Scalar color;

      using apollo::perception::TrafficLight;
      switch (rectified_box.color()) {
        case TrafficLight::RED: color = kColorTable["red_light_box"]; break;
        case TrafficLight::GREEN: color = kColorTable["green_light_box"]; break;
        case TrafficLight::BLACK: color = kColorTable["black_light_box"]; break;
        case TrafficLight::YELLOW: color = kColorTable["yellow_light_box"]; break;
        default: color = kColorTable["unknown_light_box"]; break;
      }
  ...
  if (!tl_debug_msg.valid_pos()) {
    ...
    pos_y += 50;
    std::string diff_img_pose_ts_str = "ts diff: " + std::to_string(tl_debug_msg.ts_diff_pos());
    cv::putText(img, diff_img_pose_ts_str, cv::Point(30, pos_y), cv::FONT_HERSHEY_PLAIN, 3.0,
                CV_RGB(255, 0, 0), 2);

    pos_y += 50;
    std::string diff_img_sys_ts_str = "ts diff sys: " + std::to_string(tl_debug_msg.ts_diff_sys());
    cv::putText(img, diff_img_sys_ts_str, cv::Point(30, pos_y), cv::FONT_HERSHEY_PLAIN, 3.0,
                CV_RGB(255, 0, 0), 2);
  }
  pos_y += 50;
  {
    std::string signal_txt = "camera id: " + apollo::perception::traffic_light::kCameraIdToStr.at(
  ...
  if (tl_debug_msg.project_error() > 100) {
    std::string   img_border_txt = "Offset size: " + std::to_string(tl_debug_msg.project_error());
...
void SubLongFocusCallback(sensor_msgs::ImagePtr msg) { SubImage(CameraId::LONG_FOCUS, msg); }

void SubShortFocusCallback(sensor_msgs::ImagePtr msg) { SubImage(CameraId::SHORT_FOCUS, msg); }
