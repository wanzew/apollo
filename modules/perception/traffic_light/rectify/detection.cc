...
void Detection::Perform(const cv::Mat& ros_image, std::vector<LightPtr>* lights) {
  ...
  cv::resize(crop_image, fw_image,
             cv::Size(crop_image.cols * crop_col_shrink_, crop_image.rows * crop_row_shrink_));
  ...
  int forward_time_for_this_sample = refine_input_layer_->GetForwardTimesForCurSample();
  ...
  SelectOutputBboxes(crop_image.size(), VERTICAL_CLASS, inflate_col, inflate_row, lights);
  SelectOutputBboxes(crop_image.size(), QUADRATE_CLASS, inflate_col, inflate_row, lights);
...
void Detection::Init(const int          resize_len,
  ...
  refine_input_layer_ =
      static_cast<caffe::PyramidImageOnlineDataLayer<float>*>(refine_net_ptr_->layers()[0].get());
  refine_output_layer_ = static_cast<caffe::ROIOutputSSDLayer<float>*>(
      refine_net_ptr_->layers()[refine_net_ptr_->layers().size() - 1].get());
  AINFO << refine_input_layer_->resize_scale << " " << refine_input_layer_->type();

  resize_len_ = resize_len;
}

Detection::Detection(int                min_crop_size,
                     const std::string& refine_net,
                     const std::string& refine_model) {
  Init(min_crop_size, refine_net, refine_model);
}

bool Detection::SelectOutputBboxes(const cv::Size&        img_size,
                                   int                    class_id,
                                   float                  inflate_col,
                                   float                  inflate_row,
  ...
  vector<caffe::BBox<float>>& result_bbox = refine_output_layer_->GetFilteredBBox(class_id);
  for (size_t candidate_id = 0; candidate_id < result_bbox.size(); candidate_id++) {
    LightPtr tmp(new Light);
    tmp->region.rectified_roi.x     = static_cast<int>(result_bbox[candidate_id].x1 * inflate_col);
    tmp->region.rectified_roi.y     = static_cast<int>(result_bbox[candidate_id].y1 * inflate_row);
    tmp->region.rectified_roi.width = static_cast<int>(
        (result_bbox[candidate_id].x2 - result_bbox[candidate_id].x1 + 1) * inflate_col);
    tmp->region.rectified_roi.height = static_cast<int>(
        (result_bbox[candidate_id].y2 - result_bbox[candidate_id].y1 + 1) * inflate_row);
