...
void GaussianSelect::Select(const cv::Mat&               ros_image,
  ...
  auto max_area_refined_bbox = std::max_element(
      refined_bboxes.begin(), refined_bboxes.end(), [](const LightPtr lhs, const LightPtr rhs) {
        return lhs->region.rectified_roi.area() < rhs->region.rectified_roi.area();
      });

  //  cv::Mat_<int> cost_matrix(hdmap_bboxes.size(), refined_bboxes.size());
  std::vector<std::vector<double>> score_matrix(hdmap_bboxes.size(),
                                                std::vector<double>(refined_bboxes.size(), 0));
  for (size_t row = 0; row < hdmap_bboxes.size(); ++row) {
    ...
    for (size_t col = 0; col < refined_bboxes.size(); ++col) {
      ...
      double distance_score =
          static_cast<double>(Get2dGaussianScore(center_hd, center_refine, 100, 100));
      // width score:
      //   larger width diff => 0
      ...
      // score * weight h
      score_matrix[row][col] = (0.05 + 0.4 * refined_bboxes[col]->region.detect_score +
  ...
  for (size_t i = 0; i < hd_index.size(); ++i) {
    if (hd_index[i] < 0 || static_cast<size_t>(hd_index[i]) >= hdmap_bboxes.size() ||
        refined_index[i] < 0 || static_cast<size_t>(refined_index[i]) >= refined_bboxes.size() ||
  ...
  for (size_t i = 0; i < hdmap_bboxes.size(); ++i) {
    if (!hdmap_bboxes[i]->region.is_selected) { selected_bboxes->push_back(hdmap_bboxes[i]); }
  }
}
