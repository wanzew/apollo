...
void CropBox::GetCropBox(const cv::Size&              size,
  ...
  for (int i = 0; i < lights_num; ++i) {
    ...
    if (!BoxIsValid(light.region.projection_roi, size)) { continue; }
    initialized = true;
    if (xl > light.region.projection_roi.x) { xl = light.region.projection_roi.x; }
    if (yt > light.region.projection_roi.y) { yt = light.region.projection_roi.y; }
...
CropBox::CropBox(float crop_scale, float min_crop_size) { Init(crop_scale, min_crop_size); }
