...
bool Velodyne64::Process() {
  const auto& point_cloud_msg = AdapterManager::GetPointCloud()->GetLatestObserved();
  ADEBUG << "PointCloud: " << point_cloud_msg.header;
  ProcessPointCloudData(point_cloud_msg);
  return true;
}

bool Velodyne64::ProcessPointCloudData(const sensor_msgs::PointCloud2& message) {
  PointCloudPtr cld(new PointCloud);
  TransPointCloudMsgToPCL(message, &cld);
  AINFO << "PointCloud size = " << cld->points.size();

  std::shared_ptr<Matrix4d> velodyne_to_novatel_trans = std::make_shared<Matrix4d>();
  if (!GetTrans(FLAGS_novatel_frame_name, FLAGS_velodyne64_frame_name, message.header.stamp.toSec(),
                velodyne_to_novatel_trans.get())) {
    AERROR << "Fail to transform velodyne64 to novatel at time: " << message.header.stamp.toSec();
    return false;
  }
...
void Velodyne64::TransPointCloudMsgToPCL(const sensor_msgs::PointCloud2& cloud_msg,
                                         PointCloudPtr*                  cloud_pcl) {
...
bool Velodyne64::GetTrans(const std::string& to_frame,
  ...
  std::string  err_msg;
  if (!tf2_buffer.canTransform(to_frame, from_frame, query_stamp, ros::Duration(kTf2BuffSize),
                               &err_msg)) {
    AERROR << "Cannot transform frame from " << from_frame << " to frame " << to_frame
           << " , err: " << err_msg << ". Frames: " << tf2_buffer.allFramesAsString();
    return false;
  }

  geometry_msgs::TransformStamped transform_stamped;
  try {
    transform_stamped = tf2_buffer.lookupTransform(to_frame, from_frame, query_stamp);
...
bool Velodyne64::TransformPointCloudToWorld(std::shared_ptr<Matrix4d> velodyne_trans,
                                            PointCloudPtr*            cld) {
