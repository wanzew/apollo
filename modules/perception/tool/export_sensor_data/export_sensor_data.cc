...
DEFINE_string(lidar_path, "modules/perception/tool/export_sensor_data/lidar/", "lidar path");
DEFINE_string(radar_path, "modules/perception/tool/export_sensor_data/radar/", "radar path");
...
Status ExportSensorData::Init() {
  ...
  AdapterManager::AddLocalizationCallback(&ExportSensorData::OnLocalization, this);
  ...
  if (!LoadExtrinsic(short_camera_extrinsic_path, &short_camera_extrinsic)) {
    AERROR << "Failed to load extrinsic from file " << short_camera_extrinsic_path;
    return Status(ErrorCode::PERCEPTION_ERROR_TF, "Failed to load extrinsic");
  }
  AINFO << "get short camera  extrinsic succ. pose: \n" << short_camera_extrinsic.matrix();
  /// get radar-velodyne extrinsic
  radar2velodyne_extrinsic_ = short_camera_extrinsic.matrix() * radar_extrinsic.matrix();

  return Status::OK();
}

void ExportSensorData::WriteRadar(const std::string& file_pre, const ContiRadar& radar_obs) {
...
void ExportSensorData::WritePose(const std::string&     file_pre,
  ...
  std::fstream fout(filename.c_str(), std::ios::out | std::ios::binary);
  if (!fout.is_open()) { AINFO << "Failed to write pose."; }
  Eigen::Matrix3f    mat3f = pose.block<3, 3>(0, 0).cast<float>();
  Eigen::Quaternionf quaternion(mat3f);
  fout << std::setprecision(16) << seq_num << " " << timestamp << " " << pose(0, 3) << " "
       << pose(1, 3) << " " << pose(2, 3) << " " << quaternion.x() << " " << quaternion.y() << " "
...
void ExportSensorData::WriteVelocityInfo(const std::string&     file_pre,
  ...
  std::fstream fout(filename.c_str(), std::ios::out | std::ios::binary);
  if (!fout.is_open()) { AINFO << "Failed to write velocity."; }
  fout << std::setprecision(16) << seq_num << " " << timestamp << " " << velocity(0) << " "
...
void ExportSensorData::TransPointCloudToPCL(const sensor_msgs::PointCloud2& in_msg,
...
void ExportSensorData::OnPointCloud(const sensor_msgs::PointCloud2& message) {
  ...
  if (!GetVelodyneTrans(kTimeStamp, velodyne_trans.get())) {
    AERROR << "failed to get velodyne trans at timestamp: " << GLOG_TIMESTAMP(kTimeStamp);
    return;
  }
...
void ExportSensorData::OnRadar(const ContiRadar& radar_obs) {
  ...
  AINFO << "FRAME_STATISTICS:Radar:Start:msg_time[" << GLOG_TIMESTAMP(timestamp) << "]:cur_time["
  ...
  std::shared_ptr<Eigen::Matrix4d> velo2world_pose = std::make_shared<Eigen::Matrix4d>();
  if (!GetVelodyneTrans(timestamp, velo2world_pose.get())) {
    AERROR << "Failed to get radar trans at timestamp: " << GLOG_TIMESTAMP(timestamp);
    return;
  }
  std::shared_ptr<Eigen::Matrix4d> radar2world_pose = std::make_shared<Eigen::Matrix4d>();
  *radar2world_pose                                 = *velo2world_pose * radar2velodyne_extrinsic_;
  ...
  if (!GetCarLinearSpeed(timestamp, &car_linear_speed)) {
    AERROR << "Failed to call get_car_linear_speed. [timestamp: " << GLOG_TIMESTAMP(timestamp);
    return;
  }
...
bool ExportSensorData::GetCarLinearSpeed(double timestamp, Eigen::Vector3f* car_linear_speed) {
  ...
  const auto& velocity   = localization_buffer_[idx].second.pose().linear_velocity();
