...
int main(int argc, char** argv) {
  ...
  boost_desc.add_options()("help", "produce help message")(
      ...
      "result", boost::program_options::value<std::string>(), "provide the result folder or list")(
      "groundtruth", boost::program_options::value<std::string>(),
      "provide the groundtruth folder or list")("is_folder", boost::program_options::value<bool>(),
                                                "provide folder or list")(
      "loading_thread_num", boost::program_options::value<unsigned int>()->default_value(4),
      "thread number for loading data")(
      "evaluation_thread_num", boost::program_options::value<unsigned int>()->default_value(4),
      "thread number for evaluation")(
      "parallel_processing_num", boost::program_options::value<unsigned int>()->default_value(4),
      "parallel processing number for each time")(
      "reserve", boost::program_options::value<std::string>()->default_value(""));
  ...
  if (boost_args.count("help") || !boost_args.count("cloud") || !boost_args.count("result") ||
      !boost_args.count("groundtruth") || !boost_args.count("is_folder")) {
  ...
  unsigned int      loading_thread_num    = boost_args["loading_thread_num"].as<unsigned int>();
  unsigned int      evaluation_thread_num = boost_args["evaluation_thread_num"].as<unsigned int>();
  unsigned int parallel_processing_num = boost_args["parallel_processing_num"].as<unsigned int>();
  const std::string reserve            = boost_args["reserve"].as<std::string>();

  apollo::perception::benchmark::DetectionEvaluation evaluator;
  evaluator.init(cloud, result, groundtruth, is_folder, loading_thread_num, evaluation_thread_num,
                 parallel_processing_num, reserve);
