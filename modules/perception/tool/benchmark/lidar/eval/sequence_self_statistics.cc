...
void SequenceSelfStatistics<KeyType>::reset() {
  _type_change_counts.resize(MetaStatistics::get_type_dim(),
                             std::vector<unsigned int>(MetaStatistics::get_type_dim(), 0));
}

template <typename KeyType>
bool SequenceSelfStatistics<KeyType>::add_objects(const std::vector<ObjectPtr>& objects,
                                                  KeyType                       key) {
  if (!_sequences.add_data(objects, key)) { return false; }
...
template <typename KeyType>
void SequenceSelfStatistics<KeyType>::add_statistics(SequenceType<KeyType>* sequence) {
...
void SequenceSelfStatistics<KeyType>::get_classification_type_change_rates(
  ...
  rate_per_class->resize(MetaStatistics::get_type_dim(),
  ...
  for (unsigned int i = 0; i < MetaStatistics::get_type_dim(); ++i) {
    unsigned int sum =
        std::accumulate(_type_change_counts[i].begin(), _type_change_counts[i].end(), 0);
    total_sum += sum;
    change_sum += sum - _type_change_counts[i][i];
    for (unsigned int j = 0; j < MetaStatistics::get_type_dim(); ++j) {
      rate_per_class->at(i)[j] = static_cast<double>(_type_change_counts[i][j]) / sum;
    }
  }
