...
double cal_point_based_jaccard_index(unsigned int n1, unsigned int n2, unsigned int overlap) {
  if (n1 + n2 <= overlap) { return 0; }
...
void FrameStatistics::set_roi_is_main_lanes(bool value) { _s_roi_is_main_lanes = value; }

double FrameStatistics::jaccard_index_percentile() const {
  if (_jaccard_indices.empty()) { return 0.0; }
  std::size_t id = static_cast<std::size_t>(static_cast<double>((_jaccard_indices.size() - 1)) *
...
bool FrameStatistics::find_association() {
  ...
  std::vector<std::map<unsigned int, unsigned int>> intersections(gt_objects_num);
  ...
  for (unsigned int i = 0; i < gt_objects.size(); ++i) {
    ...
    for (auto iter = intersections[i].begin(); iter != intersections[i].end(); ++iter) {
      ...
      match.jaccard_index     = cal_point_based_jaccard_index(
          static_cast<unsigned int>(objects[match.second]->indices->indices.size()),
          static_cast<unsigned int>(gt_objects[match.first]->indices->indices.size()),
    ...
    if (best_jaccard_index > _s_jaccard_index_threshold) {
      _strict_match_indices.push_back(static_cast<unsigned int>(_matches.size() - 1));
  ...
  for (unsigned int i = 0; i < objects.size(); ++i) {
    if (!is_obj_matched[i]) { _isolated_object_indices_2017.push_back(i); }
  }
  ...
  for (auto& i : _strict_match_indices) {
    ...
    _orientation_similarity[m.second].cal_orientation_similarity(objects[m.second],
                                                                 gt_objects[m.first]);
  }
  ...
  std::size_t isolated_det_object_num =
      (valid_jaccard_indices_num >= objects_num) ? 0 : objects_num - valid_jaccard_indices_num;
  std::size_t isolated_gt_object_num = gt_objects_num - valid_jaccard_indices_num;
  _jaccard_indices.resize(isolated_gt_object_num + isolated_det_object_num, 0.0);
  ...
  for (auto& i : _isolated_gt_object_indices_2017) {
    if (grt_match_ids[i].empty()) { continue; }
...
bool FrameStatistics::cal_meta_statistics() {
  _meta_stat.reset();
  std::vector<unsigned int> gt_object_range_indices(_gt_object_position.size(), 0);
  ...
  for (std::size_t i = 0; i < _object_position.size(); ++i) {
    ...
    // orientation
    _meta_stat._total_yaw_angle_diff[object_range_indices[i]] += _orientation_similarity[i].delta;
  }
  ...
  for (auto& i : _strict_match_indices) {
    auto& m = _matches[i];
    if (object_range_indices[m.second] == (MetaStatistics::get_range_dim() - 1)) { continue; }
    unsigned int conf_id    = MetaStatistics::get_confidence_index(m.confidence);
    unsigned int gt_type_id = MetaStatistics::get_type_index(gt_objects[m.first]->type);
  ...
  for (unsigned int t = 0; t < MetaStatistics::get_type_dim(); ++t) {
    for (int i = static_cast<int>(_meta_stat._cumulated_match_num_per_conf[t].size() - 2); i >= 0;
         --i) {
  ...
  for (int i =
           static_cast<int>(_meta_stat._cumulated_orientation_similarity_sum_per_conf.size() - 2);
       i >= 0; --i) {
  ...
  for (auto& i : _strict_match_indices) {
    auto&         m           = _matches[i];
    unsigned int  gt_type_id  = MetaStatistics::get_type_index(gt_objects[m.first]->type);
    unsigned int  obj_type_id = MetaStatistics::get_type_index(objects[m.second]->type);
  ...
  for (auto& i : _isolated_object_indices_2017) {
    ...
    if (object_range_indices[i] == (MetaStatistics::get_range_dim() - 1)) { continue; }
    unsigned int conf_id =
        MetaStatistics::get_confidence_index(static_cast<double>(objects[i]->confidence));
    ++_meta_stat._cumulated_det_alone_per_conf[obj_type_id][conf_id];
  }
  for (unsigned int t = 0; t < MetaStatistics::get_type_dim(); ++t) {
    for (int i = static_cast<int>(_meta_stat._cumulated_det_alone_per_conf[t].size() - 2); i >= 0;
         --i) {
  ...
  // filling _gt_alone
  for (auto& i : _isolated_gt_object_indices_2017) {
    unsigned int  gt_type_id = MetaStatistics::get_type_index(gt_objects[i]->type);
  ...
  for (auto& i : _underseg_gt_object_indices_2017) {
    if (gt_object_range_indices[i] == (MetaStatistics::get_range_dim() - 1)) { continue; }
    unsigned int gt_type_id = MetaStatistics::get_type_index(gt_objects[i]->type);
