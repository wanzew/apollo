...
void Visibility::set_car_pos(const Eigen::Vector3d& car_pos) { car_pos_ = car_pos; }
...
void Visibility::fill_objects(std::vector<ObjectPtr>* objs, float thresh) {
  ...
  auto point_in_rectangle = [this](const VisPoint& point) -> bool {
    ...
    return x < half_length_ && x > -half_length_ && y < half_width_ && y > -half_width_;
  };
  ...
  for (size_t i = 0; i < objs->size(); ++i) {
    ...
    float            theta = static_cast<float>(atan2(obj->direction(1), obj->direction(0)));
    ...
    Eigen::Vector2f origin_in_obj_cord = (trans_mat.inverse() * Eigen::Vector3f(0, 0, 1)).head(2);
    ...
    VisPoint              p0((trans_mat * Eigen::Vector3f(half_length, half_width, 1)).head(2));
    VisPoint              p1((trans_mat * Eigen::Vector3f(half_length, -half_width, 1)).head(2));
    VisPoint              p2((trans_mat * Eigen::Vector3f(-half_length, -half_width, 1)).head(2));
    VisPoint              p3((trans_mat * Eigen::Vector3f(-half_length, half_width, 1)).head(2));
    ...
    for (size_t j = 1; j < point_pos.size(); ++j) {
      add_segment(points_vec[point_pos[j - 1]], points_vec[point_pos[j]], static_cast<int>(i));
    }

    full_visual_angle_.push_back(
        calculate_visual_angle(points_vec[*point_pos.begin()], points_vec[*point_pos.rbegin()]));
  }
...
float Visibility::calculate(std::vector<ObjectPtr>* objs, float thresh) {
  ...
  for (auto it = points_.begin(); it != points_.end(); ++it) {
    ...
    if (!init) {
      ...
      if (candidate_segment_.empty()) { continue; }
      pending_p   = current_p;
...
void Visibility::add_segment(const VisPoint& a, const VisPoint& b, int idx) {
  if (compute_orientation(VisPoint(0, 0), a, b) == Orientation::collinear) { return; }

  if (approx_equal(a.x(), 0.0) || approx_equal(b.x(), 0.0)) {
    VisPoint ta = a, tb = b;
    if (!approx_equal(ta.x(), 0.0)) { std::swap(ta, tb); }
  ...
  VisPoint intersection;
  bool     axis_y_intersect = intersects(VisPoint(0, 1), Segment(a, b), &intersection);
  if (axis_y_intersect) {
    Segment seg1 = a.x() < 0 ? Segment(a, intersection, idx) : Segment(intersection, a, idx);
    Segment seg2 = b.x() < 0 ? Segment(b, intersection, idx) : Segment(intersection, b, idx);
...
void Visibility::query_segments(const VisPoint& p) {
  if (p == latest_query_point_) { return; }
  latest_query_segments_cache_.clear();
...
void Visibility::update_candidate_segment(const VisPoint& p, UpdateOperation op) {
  query_segments(p);

  auto func   = (op == UpdateOperation::add ?
                   [](const Segment& seg, std::set<Segment>& segments) { segments.insert(seg); } :
                   [](const Segment& seg, std::set<Segment>& segments) { segments.erase(seg); });
  auto update = [this, func](const Segment& seg) { return func(seg, candidate_segment_); };

  for (const auto& seg : latest_query_segments_cache_) {
    VisPoint p_pending = (op == UpdateOperation::add ? seg.start : seg.end);
    if (p_pending == p) { update(seg); }
  }
}

float Visibility::calculate_area(const VisPoint& a, const VisPoint& b) {
  float dot  = a.dot(b);
  float area = static_cast<float>(sqrt(a.length_squared() * b.length_squared() - dot * dot) / 2);
...
float Visibility::calculate_visual_angle(const VisPoint& a, const VisPoint& b) {
  float cos_theta = static_cast<float>(a.dot(b) / sqrt(a.length_squared() * b.length_squared()));
  float theta     = static_cast<float>(approx_equal(cos_theta, 1.0) ? 0.0 : acos(cos_theta));
