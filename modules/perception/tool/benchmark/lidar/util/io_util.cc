...
static const std::map<std::string, std::function<bool(const std::string&, PointCloudPtr)>>
    s_load_method = {
...
bool load_pcl_pcds(const std::string& filename,
  ...
  if (iter == s_load_method.end()) { return false; }
...
bool load_frame_objects(const std::string&           filename,
  ...
  if (!fin.is_open()) {
    std::cerr << "frame objects file " << filename << " is not exist!" << std::endl;
    return false;
  }
  int frame_id      = -1;
  int object_number = -1;
  ...
  for (int i = 0; i < object_number; ++i) {
    ...
    fin >> sensor_type >> object_id >> track_id >> is_background >> confidence >> type >>
        bbox_center_x >> bbox_center_y >> bbox_center_z >> bbox_length >> bbox_width >>
        bbox_height >> yaw >> roll >> pitch >> truncated >> occluded >> velocity_x >> velocity_y >>
        velocity_z;
    ...
    if (indice_number >= 0) {
      ...
      for (int j = 0; j < polygon_points_number; ++j) {
        fin >> obj->polygon.points[j].x >> obj->polygon.points[j].y >> obj->polygon.points[j].z;
      }
      indice_number = 0;
    }

    if (point_number == 0 && indice_number == 0) {
      std::cerr << "illegal object with empty point cloud: " << filename << " object " << i
                << std::endl;
    } else if (point_number != 0 && indice_number != 0 && point_number != indice_number) {
      std::cerr << "illegal object with different points & indices" << filename << " object " << i
                << std::endl;
      fin.close();
      return false;
    }
    ...
    if (black_list.empty() || black_list.find(type) == black_list.end()) {
      ...
      std::cerr << "Ignore object " << i << " since it is in black list, type = " << type
                << std::endl;
    }
  }
  ...
  if (left_boundary != nullptr && right_boundary != nullptr && road_polygon != nullptr) {
...
bool load_sensor2world_pose(const std::string& filename, Eigen::Matrix4d* pose_out_pt) {
  ...
  double quat[4];
  double matrix3x3[9];
  sscanf(buffer, "%d %lf %lf %lf %lf %lf %lf %lf %lf", &id, &(time_samp), &(pose_out(0, 3)),
         &(pose_out(1, 3)), &(pose_out(2, 3)), &(quat[0]), &(quat[1]), &(quat[2]), &(quat[3]));
...
bool save_frame_objects(const std::string&            filename,
  ...
  for (auto& object : objects) {
    fout << translate_sensor_type_to_string(object->sensor_type) << " " << object->id << " "
         << object->track_id << " " << object->is_background << " " << std::setprecision(8)
         << object->confidence << " " << translate_type_to_string(object->type) << " "
         << object->reserve << " " << object->center(0) << " " << object->center(1) << " "
         << object->center(2) << " " << object->length << " " << object->width << " "
         << object->height << " " << object->yaw << " " << object->roll << " " << object->pitch
         << " " << object->truncated << " " << object->occluded << " " << object->velocity(0) << " "
         << object->velocity(1) << " " << object->velocity(2) << " ";
    fout << object->cloud->size() << " ";
    for (auto& point : object->cloud->points) {
      fout << std::setprecision(8) << point.x << " " << point.y << " " << point.z << " "
           << point.intensity << " ";
    }
