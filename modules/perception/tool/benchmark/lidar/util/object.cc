...
std::string Object::to_string() const {
  ...
  oss << "Object[id: " << id << ", track_id: " << track_id << ", cloud_size: " << cloud->size()
      << ", indices_size: " << indices->indices.size() << ", direction: " << direction.transpose()
      << ", yaw: " << yaw << ", center: " << center.transpose()
      << " velocity: " << velocity.transpose() << ", width: " << width << ", length: " << length
      << ", height: " << height << ", confidence: " << confidence << ", type: " << type
...
std::string get_object_name(ObjectType obj_type) {
  std::string obj_name;
  switch (obj_type) {
    case UNKNOWN: obj_name = "unknown"; break;
    case UNKNOWN_MOVABLE: obj_name = "unknown_movable"; break;
    case UNKNOWN_UNMOVABLE: obj_name = "unknown_unmovable"; break;
    case PEDESTRIAN: obj_name = "pedestrian"; break;
    case BICYCLE: obj_name = "bicycle"; break;
    case VEHICLE: obj_name = "vehicle"; break;
    default: obj_name = "error"; break;
  }
  return obj_name;
}
...
std::string get_sensor_name(SensorType sensor_type) {
  ...
  switch (sensor_type) {
    case VELODYNE_64: sensor_name = "velodyne_64"; break;
    case VELODYNE_16: sensor_name = "velodyne_16"; break;
    case RADAR: sensor_name = "radar"; break;
    case CAMERA: sensor_name = "camera"; break;
    case UNKNOWN_SENSOR_TYPE: sensor_name = "unknown_sensor_type"; break;
  }
  return sensor_name;
}
...
std::string SensorObjects::to_string() const {
  std::ostringstream oss;
  oss << "SensorObjects[sensor_type: " << get_sensor_name(type) << ", name: " << name
      << ", timestamp:" << FORMAT_TIMESTAMP(timestamp) << ", sensor2world_pose:\n";
...
ObjectType translate_string_to_type(const std::string& str) {
  if (str == "bigMot" || str == "smallMot" || str == "vehicle" || str == "midMot" || str == "5") {
    return VEHICLE;
  } else if (str == "pedestrian" || str == "3") {
    return PEDESTRIAN;
  } else if (str == "nonMot" || str == "cyclist" || str == "motorcyclist" || str == "bicyclist" ||
             str == "4") {
    return BICYCLE;
  } else {
    return UNKNOWN;
...
std::string translate_type_to_string(ObjectType type) {
  switch (type) {
    ...
    case UNKNOWN_UNMOVABLE: return "others";
...
std::string translate_sensor_type_to_string(const SensorType& type) {
  switch (type) {
    case VELODYNE_64: return "velodyne_64";
    case VELODYNE_16: return "velodyne_16";
