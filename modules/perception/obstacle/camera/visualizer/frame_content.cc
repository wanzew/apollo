...
void FrameContent::set_lane_content(double timestamp, const LaneObjects& objects) {
  auto key = DoubleToMapKey(timestamp);
...
void FrameContent::set_motion_content(double timestamp, const MotionBuffer& motion_buffer) {
  motion_caches_[DoubleToMapKey(timestamp)].motion_frame_content_ = motion_buffer;
...
void FrameContent::update_timestamp(double ref) {
  ...
  if (image_caches_.size() > 1) {
    ...
    if (it != image_caches_.end()) {
      ...
      image_caches_.erase(image_caches_.begin(), std::prev(image_caches_.end()));
    }
  }
  ...
  if (camera_caches_.size() > 1) {
    ...
    if (it != camera_caches_.end()) {
      ...
      camera_caches_.erase(camera_caches_.begin(), std::prev(camera_caches_.end()));
    }
  }
  ...
  common::util::erase_map_where(radar_caches_,
  ...
  common::util::erase_map_where(fusion_caches_,
  ...
  common::util::erase_map_where(
      gt_caches_, [this, best_ts](std::map<int64_t, GroundTruthContent>::value_type& p) {
        return this->MapKeyToDouble(p.first) < best_ts;
      });
  ...
  common::util::erase_map_where(motion_caches_,
...
Eigen::Matrix4d FrameContent::get_camera_to_world_pose() {
  ...
  if (it == camera_caches_.end()) { return Eigen::Matrix4d::Identity(); }
...
std::vector<std::shared_ptr<Object>> FrameContent::get_camera_objects(double* ts) {
  if (!camera_caches_.empty()) {
    auto it = camera_caches_.begin();
    if (ts != nullptr) { *ts = it->second.timestamp_; }
...
MotionBuffer FrameContent::get_motion_buffer() {
  ...
  if (it == motion_caches_.end()) {
    ...
    AINFO << "no motion available: " << DoubleToMapKey(current_motion_timestamp_);
...
std::vector<std::shared_ptr<Object>> FrameContent::get_radar_objects(double* ts) {
  auto it = radar_caches_.find(DoubleToMapKey(current_radar_timestamp_));
  if (it == radar_caches_.end()) { return std::vector<std::shared_ptr<Object>>(); }
  RadarContent content = it->second;
  if (ts != nullptr) { *ts = content.timestamp_; }
...
std::vector<std::shared_ptr<Object>> FrameContent::get_fused_objects(double* ts) {
  auto it = fusion_caches_.find(DoubleToMapKey(current_fusion_timestamp_));
  if (it == fusion_caches_.end()) { return std::vector<std::shared_ptr<Object>>(); }
...
std::vector<std::shared_ptr<Object>> FrameContent::get_gt_objects() {
  ...
  if (it == gt_caches_.end()) { return std::vector<std::shared_ptr<Object>>(); }
  const auto& content = it->second;
...
LaneObjects FrameContent::get_lane_objects() {
  ...
  if (it == lane_caches_.end()) { return LaneObjects(); }
  const auto& content = it->second;
  return content.lane_objects_;
}

void FrameContent::offset_object(std::shared_ptr<Object> object, const Eigen::Vector3d& offset) {
