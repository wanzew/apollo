...
std::vector<std::vector<int>> GLFWFusionViewer::s_color_table = {
    std::vector<int>{0, 0, 128},   std::vector<int>{0, 0, 255},   std::vector<int>{0, 128, 255},
    std::vector<int>{0, 128, 128}, std::vector<int>{0, 128, 0},   std::vector<int>{0, 255, 0},
    std::vector<int>{0, 255, 128}, std::vector<int>{0, 255, 255}, std::vector<int>{255, 0, 0},
    std::vector<int>{255, 0, 128}, std::vector<int>{255, 0, 255}, std::vector<int>{255, 255, 0},
...
bool GLFWFusionViewer::initialize() {
  ...
  if (show_lane_) {
    ...
    AINFO << "onboard lane post-processor: " << FLAGS_onboard_lane_post_processor;
...
bool GLFWFusionViewer::window_init() {
  ...
  win_height_ = (image_height_ * 2 > win_height_) ? image_height_ * 2 : win_height_;
  window_     = glfwCreateWindow(win_width_, win_height_, "gl_camera_visualizer", nullptr, nullptr);
  if (window_ == nullptr) {
...
void GLFWFusionViewer::pre_draw() {
  ...
  //  pers_camera_->load_projection_matrix();
  {
    ...
    glLoadIdentity();
    GLdouble tmp_projection_matrix[16] = {1.37509, 0, 0,       0,  0, 2.41421, 0,        0,
                                          0,       0, -1.0095, -1, 0, 0,       -17.4028, 0};
...
void GLFWFusionViewer::draw_fusion_association(FrameContent* content) {
  ...
  std::vector<std::shared_ptr<Object>> cam_objects    = content->get_camera_objects();
  std::vector<std::shared_ptr<Object>> fusion_objects = content->get_fused_objects();
  ...
  for (size_t i = 0; i < fusion_objects.size(); i++) {
    ...
    if (obj->camera_supplement == nullptr) { continue; }
...
vec3 GLFWFusionViewer::get_velocity_src_position(const std::shared_ptr<Object>& object) {
  vec3 velocity_src;
  vec3 center;
  vec3 direction;
  vec3 size;
...
void GLFWFusionViewer::render() {
  ...
  glViewport(scene_width_, 0, image_width_, image_height_ * 2);
  {
    ...
    if (show_lane_) {
      lane_objects_ = std::make_shared<LaneObjects>(frame_content_->get_lane_objects());
      if (draw_lane_objects_) {
        const auto motion_buffer = frame_content_->get_motion_buffer();
        int        n             = motion_buffer.size();
        if (n > 0) { motion_matrix_ = motion_buffer[n - 1].motion; }
  ...
  if (show_lane_) { draw_lane_objects_image(&image_mat2); }
  ...
  if (capture_video_ || capture_screen_) {
    ...
    if (capture_screen_) { capture_screen_ = false; }
  }
...
void GLFWFusionViewer::framebuffer_size_callback(GLFWwindow* window, int width, int height) {
...
void GLFWFusionViewer::window_size_callback(GLFWwindow* window, int width, int height) {
  void* user_data = glfwGetWindowUserPointer(window);
  if (user_data == nullptr) { return; }
...
void GLFWFusionViewer::mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
}
...
void GLFWFusionViewer::mouse_scroll_callback(GLFWwindow* window, double xoffset, double yoffset) {
  void* user_data = glfwGetWindowUserPointer(window);
  if (user_data == nullptr) { return; }
...
void GLFWFusionViewer::resize_window(int width, int height) {
  if (width == win_width_ && height == win_height_) { return; }
...
void GLFWFusionViewer::keyboard(int key) {
  switch (key) {
    ...
    case 65:  // 'A'
      ...
      break;
    // for camera visualization
    case GLFW_KEY_I: show_type_id_label_ = !show_type_id_label_; break;
    case GLFW_KEY_Q:  // Q
    ...
    case GLFW_KEY_D:  // D
      ...
      break;
    case GLFW_KEY_O: show_camera_bdv_ = !show_camera_bdv_; break;
    case GLFW_KEY_T: show_trajectory_ = !show_trajectory_; break;
    case GLFW_KEY_2:  // 2
...
void GLFWFusionViewer::capture_screen(const std::string& file_name) {
  ...
  glReadPixels(0, 0, win_width_, win_height_, GL_BGRA, GL_UNSIGNED_BYTE, rgba_buffer_);
...
GLuint GLFWFusionViewer::image_to_gl_texture(const cv::Mat& mat,
  ...
  if (mag_filter == GL_LINEAR_MIPMAP_LINEAR || mag_filter == GL_LINEAR_MIPMAP_NEAREST ||
      mag_filter == GL_NEAREST_MIPMAP_LINEAR || mag_filter == GL_NEAREST_MIPMAP_NEAREST) {
...
void GLFWFusionViewer::draw_camera_frame(FrameContent* content,
  ...
  GLuint image_tex = image_to_gl_texture(*image_mat, GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_CLAMP);
  ...
  Eigen::Matrix4d camera_to_world_pose_static = content->get_camera_to_world_pose_static();
  ...
  if (show_3d_class) {
    ...
    if (show_fusion_) {
      ...
      draw_camera_box3d(camera_objects, fused_objects, v2c, offset_x, offset_y, image_width,
                        image_height);
    } else {
      std::vector<std::shared_ptr<Object>> camera_objects;
      camera_objects = content->get_camera_objects();
      draw_camera_box(camera_objects, v2c, v2c_static, offset_x, offset_y, image_width,
                      image_height);
    }
  } else {
    // show 2d bbox
    draw_camera_box2d(camera_objects, v2c, offset_x, offset_y, image_width, image_height);
  }
  if (show_radar_pc_) {
    std::vector<std::shared_ptr<Object>> objects = content->get_radar_objects();
    draw_objects2d(objects, v2c, "radar", offset_x, offset_y, image_width, image_height);
  }
}
...
void GLFWFusionViewer::draw_lane_objects_ground() {
  ...
  for (size_t k = 0; k < lane_objects_->size(); ++k) {
    ...
    if (show_trajectory_) {
      ...
      // add new point
      for (auto p = lane_objects_->at(k).pos.begin(); p != lane_objects_->at(k).pos.end(); ++p) {
        auto point_poly = *p;
    ...
    // }
    for (auto p = lane_objects_->at(k).pos.begin(); p != lane_objects_->at(k).pos.end(); ++p) {
      drawHollowCircle(static_cast<GLfloat>(p->x()), static_cast<GLfloat>(p->y()),
                       static_cast<GLfloat>(0.4));
    }
  ...
  glLineWidth(3);
  glBegin(GL_LINE_STRIP);
  glVertex2f(static_cast<GLfloat>(left_1.x()), static_cast<GLfloat>(left_1.y()));
  glVertex2f(static_cast<GLfloat>(right_2.x()), static_cast<GLfloat>(right_2.y()));
  glVertex2f(static_cast<GLfloat>(right_4.x()), static_cast<GLfloat>(right_4.y()));
  glVertex2f(static_cast<GLfloat>(left_3.x()), static_cast<GLfloat>(left_3.y()));
  glVertex2f(static_cast<GLfloat>(left_1.x()), static_cast<GLfloat>(left_1.y()));
  glEnd();
  glFlush();
...
bool GLFWFusionViewer::draw_lane_objects_image(cv::Mat* image_mat) {
  ...
  CameraFrameSupplementPtr camera_frame_supplement = frame_content_->get_camera_frame_supplement();
      ...
      if (x >= 0 && x < lane_map.cols && y >= 0 && y < lane_map.rows &&
        ...
        for (uint16_t c = 0; c < 3; c++) {
          image_mat->at<cv::Vec3b>(h, w)[c] =
              static_cast<unsigned char>(image_mat->at<cv::Vec3b>(h, w)[c] * alpha_blending +
  ...
  for (size_t k = 0; k < lane_objects_->size(); ++k) {
    ...
    if (lane_objects_->at(k).is_compensated) { continue; }
    ...
    switch (lane_objects_->at(k).spatial) {
      ...
      default: {
        AERROR << "unknown lane spatial label: " << static_cast<int>(lane_objects_->at(k).spatial);
      }
    }

    // Besides the fitted polynomial curves we draw lane markers as well
    for (auto p = lane_objects_->at(k).image_pos.begin(); p != lane_objects_->at(k).image_pos.end();
         ++p) {
      cv::circle(*image_mat, cv::Point(static_cast<int>(p->x()), static_cast<int>(p->y())), 4,
                 lane_object_color, -1);
    }

    // draw polynomial curve
    float img_y_start = static_cast<float>(lane_objects_->at(k).img_curve.x_start);
  ...
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  glOrtho(scene_width_, scene_width_ + image_width_, image_height_, 0.0, 0.0, 100.0);
...
void GLFWFusionViewer::draw_vp_ground(const Eigen::Matrix4d& v2c,
                                      bool                   stat,
                                      int                    offset_x,
                                      int                    offset_y,
                                      int                    image_width,
  ...
  draw_line2d(tmp1, tmp2, 2, color_v[0], color_v[1], color_v[2], offset_x, offset_y, image_width,
              image_height);
  ...
  for (double y = -10.0; y <= 10.0; y += 2.0) {
    ...
    for (double x = 0.0; x < 100.0; x += 5.0) {
      ...
      if (x > 5.0) {
        draw_line2d(prev_pt2d, pt2d, 2, color_g[0], color_g[1], color_g[2], offset_x, offset_y,
...
bool GLFWFusionViewer::project_point_undistort(Eigen::Matrix4d  v2c,
                                               Eigen::Vector3d  pc,
                                               Eigen::Vector2d* p2d) {
  Eigen::Vector3d pc3d = (v2c * Eigen::Vector4d(pc[0], pc[1], pc[2], 1)).head(3);
  if (pc3d[2] < 0) { return false; }
...
void GLFWFusionViewer::get_8points(float                         width,
                                   float                         height,
                                   float                         length,
...
bool GLFWFusionViewer::get_boundingbox(Eigen::Vector3d               center,
  ...
  for (int i = 0; i < 8; i++) {
    camera_points[i] = camera_intrinsic_.block(0, 0, 3, 3) * (center + r * obj_points[i]);
...
void GLFWFusionViewer::draw_camera_box2d(const std::vector<std::shared_ptr<Object>>& objects,
                                         Eigen::Matrix4d                             v2c,
                                         int                                         offset_x,
                                         int                                         offset_y,
                                         int                                         image_width,
  ...
  for (auto obj : objects) {
    ...
    ADEBUG << "camera obj " << obj->track_id << " center: " << center2d[0] << " " << center2d[1];
    ...
    get_boundingbox(tc, v2c, width, height, length, obj->direction, theta, &points);
    ...
    if (show_camera_box2d_) {
      if (obj->camera_supplement != nullptr) {
        ...
        if (use_class_color_) { get_class_color(static_cast<unsigned>(obj->type), rgb); }
        ...
        if (obj->b_cipv) {
          ADEBUG << "draw_camera_box2d This is CIPV, obj->track_id: " << obj->track_id;
          box2d_color[0] = 255;
          box2d_color[1] = 0;
          box2d_color[2] = 0;
        }
        ...
        draw_rect2d(upper_left_pt, lower_right_pt, 2, box2d_color[0], box2d_color[1],
                    box2d_color[2], offset_x, offset_y, image_width, image_height);
        // Draw texts using OpenGL
        // distance
        if (show_type_id_label_) {
          std::string c =
              std::to_string(static_cast<int>((sqrt(tc.x() * tc.x() + tc.z() * tc.z()))));
          ...
          switch (obj->type) {
            case ObjectType::PEDESTRIAN: obj_type_str = "PED"; break;
            case ObjectType::BICYCLE: obj_type_str = "CYC"; break;
            case ObjectType::VEHICLE: obj_type_str = "CAR"; break;
            default: break;
          }
...
void GLFWFusionViewer::draw_camera_box3d(const std::vector<std::shared_ptr<Object>>& camera_objects,
  ...
  for (auto fused_obj : fused_objects) {
    if (fused_obj->camera_supplement == nullptr) { continue; }
    ...
    if (it != cam_track_id_2_ind.end()) {
      ...
      Eigen::Vector2d center2d;
      get_project_point(v2c, center, &center2d);
      ADEBUG << "draw_camera_box3d camera obj " << obj->track_id << " center: " << center2d[0]
             << " " << center2d[1];
      ...
      std::vector<Eigen::Vector2d> points;
      points.resize(8);
      Eigen::Vector3d tc = (v2c * Eigen::Vector4d(center[0], center[1], center[2], 1)).head(3);
      ...
      if (show_camera_box3d_) {
        draw_8pts_box(points, Eigen::Vector3f(box3d_color[0], box3d_color[1], box3d_color[2]),
                      offset_x, offset_y, image_width, image_height);
      }
    }
  }
...
void GLFWFusionViewer::draw_objects(double                                      timestamp,
                                    const std::vector<std::shared_ptr<Object>>& objects,
                                    const Eigen::Matrix4d&                      c2w,
                                    bool                                        draw_cube,
                                    bool                                        draw_velocity,
                                    const Eigen::Vector3f&                      color,
                                    bool                                        use_class_color,
                                    bool                                        use_track_color) {
  if (show_associate_color_) { use_class_color = false; }
  ...
  if (draw_cube) {
    ...
    for (i = 0; i < static_cast<int>(objects.size()); i++) {
      ...
      if (use_track_color) {
        auto tmp_color = s_color_table[objects[i]->track_id % s_color_table.size()];
        rgb[0]         = tmp_color[0];
        rgb[1]         = tmp_color[1];
        rgb[2]         = tmp_color[2];
      ...
      if (show_verbose_) {
        ...
        glRasterPos2i(tc[0] + offset, tc[1]);
        raster_text_->print_string(std::string("t:") + std::to_string(timestamp));
        offset += 2;
      }
  ...
  if (draw_velocity) {
    ...
    for (i = 0; i < static_cast<int>(objects.size()); i++) {
      ...
      if (use_class_color) { get_class_color(static_cast<unsigned>(objects[i]->type), rgb); }
      glColor3f((GLfloat)rgb[0], (GLfloat)rgb[1], (GLfloat)rgb[2]);
      glBegin(GL_LINES);
...
void GLFWFusionViewer::drawHollowCircle(GLfloat x, GLfloat y, GLfloat radius, GLfloat z) {
...
void GLFWFusionViewer::draw_3d_classifications(FrameContent* content, bool show_fusion) {
  Eigen::Matrix4d c2v = content->get_camera_to_world_pose();
  double          ts  = 0.0f;
  if (show_camera_bdv_) {
    std::vector<std::shared_ptr<Object>> objs = content->get_camera_objects(&ts);
    draw_objects(ts, objs, c2v, true, true, Eigen::Vector3f(1, 1, 0), use_class_color_);
  }
  ...
  if (show_fusion_) {
    ...
    std::vector<std::shared_ptr<Object>> objects       = content->get_fused_objects(&ts);
    ADEBUG << "fused object size in glfw viewer is " << objects.size();
    for (auto obj : objects) {
      ADEBUG << "object in fuse: " << obj->ToString();
    }
    std::vector<std::shared_ptr<Object>> objects_cam = content->get_camera_objects();
    ADEBUG << " camera object size is " << objects_cam.size();
    for (auto obj : objects_cam) {
      ADEBUG << "object in cam: " << obj->ToString();
    }
    draw_objects(ts, objects, c2v, draw_cube, draw_velocity, fused_color, false, false);

    if (FLAGS_show_fusion_association) { draw_fusion_association(content); }
  }

  if (show_trajectory_ && content->get_motion_buffer().size() > 0) { draw_trajectories(content); }

  if (show_radar_pc_) {
    Eigen::Vector3f                      radar_color(1, 1, 1);
    bool                                 draw_cube     = true;
    bool                                 draw_velocity = true;
    std::vector<std::shared_ptr<Object>> objects       = content->get_radar_objects(&ts);
    draw_objects(ts, objects, c2v, draw_cube, draw_velocity, radar_color, false, false);
  }
}

void GLFWFusionViewer::draw_camera_box(const std::vector<std::shared_ptr<Object>>& objects,
                                       Eigen::Matrix4d                             v2c,
                                       ...
                                       int                                         image_height) {
  if (show_vp_grid_) {
    draw_vp_ground(v2c_static, true, offset_x, offset_y, image_width, image_height);
  ...
  for (auto obj : objects) {
    ...
    Eigen::Vector2d        center2d;
    get_project_point(v2c, center, &center2d);
    ADEBUG << "draw_camera_box camera obj " << obj->track_id << " center: " << center[0] << " "
           << center[1];
    ...
    auto box3d_color = s_color_table[0];
    if (obj->b_cipv) {
      ADEBUG << "draw_camera_box This is CIPV, obj->track_id: " << obj->track_id;
      box3d_color[0] = 255;
      box3d_color[1] = 0;
      box3d_color[2] = 0;
...
void GLFWFusionViewer::draw_objects2d(const std::vector<std::shared_ptr<Object>>& objects,
                                      Eigen::Matrix4d                             v2c,
                                      std::string                                 name,
                                      int                                         offset_x,
                                      int                                         offset_y,
  ...
  if (name == "radar") {
    ...
    for (auto obj : objects) {
      ...
      Eigen::Vector2d center2d;
      get_project_point(v2c, center, &center2d);
      if ((center2d[0] > image_width) || (center2d[1] > image_height) || (center2d[0] < 0) ||
          (center2d[1] < 0)) {
        continue;
      }
      ...
      if (obj->b_cipv) {
        ADEBUG << "radar draw_objects2d This is CIPV, obj->track_id: " << obj->track_id;
        glColor3ub(255, 0, 0);
      } else {
        glColor3ub(0, 0, 0);
      }
...
void GLFWFusionViewer::draw_8pts_box(const std::vector<Eigen::Vector2d>& points,
  ...
  draw_line2d(p1, p2, 2, color_bottom[0], color_bottom[1], color_bottom[2], offset_x, offset_y,
              image_width, image_height);
  draw_line2d(p2, p3, 2, color_bottom[0], color_bottom[1], color_bottom[2], offset_x, offset_y,
              image_width, image_height);
  draw_line2d(p3, p4, 2, color_bottom[0], color_bottom[1], color_bottom[2], offset_x, offset_y,
              image_width, image_height);
  draw_line2d(p4, p1, 2, color_bottom[0], color_bottom[1], color_bottom[2], offset_x, offset_y,
              image_width, image_height);

  draw_line2d(p5, p6, 2, color_top[0], color_top[1], color_top[2], offset_x, offset_y, image_width,
              image_height);
  draw_line2d(p6, p7, 2, color_top[0], color_top[1], color_top[2], offset_x, offset_y, image_width,
              image_height);
  draw_line2d(p7, p8, 2, color_top[0], color_top[1], color_top[2], offset_x, offset_y, image_width,
              image_height);
  draw_line2d(p8, p5, 2, color_top[0], color_top[1], color_top[2], offset_x, offset_y, image_width,
              image_height);

  draw_line2d(p5, p1, 2, color_side[0], color_side[1], color_side[2], offset_x, offset_y,
              image_width, image_height);
  draw_line2d(p3, p7, 2, color_side[0], color_side[1], color_side[2], offset_x, offset_y,
              image_width, image_height);
  draw_line2d(p4, p8, 2, color_side[0], color_side[1], color_side[2], offset_x, offset_y,
              image_width, image_height);
  draw_line2d(p6, p2, 2, color_side[0], color_side[1], color_side[2], offset_x, offset_y,
              image_width, image_height);
}
