...
bool FlatCameraTransformer::Transform(std::vector<std::shared_ptr<VisualObject>>* objects) {
  ...
  for (auto obj_ptr : *objects) {
    ...
    if (HaveHighConfidence(obj_ptr)) {
      Eigen::Vector3f center_ego = pos_ground + Eigen::Vector3f(0.0f, 0.0f, obj_ptr->height / 2.0f);
...
bool FlatCameraTransformer::SetExtrinsics(const Eigen::Matrix<double, 4, 4>& extrinsics) {
  camera2car_             = extrinsics.cast<float>();
  camera2car_adj_         = camera2car_;
  camera2car_flat_offset_ = Eigen::Matrix<float, 3, 1>(camera2car_(0, 3), camera2car_(1, 3), 0.0f);
  return true;
}

bool FlatCameraTransformer::GetAdjustedExtrinsics(Eigen::Matrix<double, 4, 4>* extrinsics) {
...
std::string FlatCameraTransformer::Name() const { return "FlatCameraTransformer"; }

Eigen::Matrix<float, 3, 1>
FlatCameraTransformer::MakeUnit(const Eigen::Matrix<float, 3, 1>& v) const {
...
bool FlatCameraTransformer::HaveHighConfidence(std::shared_ptr<VisualObject> obj_ptr) {
  ...
  double azimuth = std::atan2(obj_ptr->center[1], obj_ptr->center[0]) * 180.0 / M_PI;
...
void FlatCameraTransformer::GetDynamicExtrinsics(
  ...
  for (float p = -3.0; p < 3.0; p += 0.2f) {
    ...
    for (auto center_p : centers) {
      ...
      diff += std::sqrt(diff_3dim.x() * diff_3dim.x() + diff_3dim.y() * diff_3dim.y() +
