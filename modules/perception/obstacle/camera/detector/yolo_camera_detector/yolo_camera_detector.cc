...
bool YoloCameraDetector::Init(const CameraDetectorInitOptions& options) {
  ...
  const string  yolo_config =
      apollo::common::util::GetAbsolutePath(yolo_root, FLAGS_yolo_config_filename);

  const string lane_config = apollo::common::util::GetAbsolutePath(yolo_root, "lane.pt");
  ...
  if (!init_cnn(yolo_root) || !init_cnn_lane(yolo_root)) { return false; }
  load_nms_params();
...
void YoloCameraDetector::init_anchor(const string& yolo_root) {
  ...
  string model_root = apollo::common::util::GetAbsolutePath(yolo_root, model_param.model_name());

  // loading anchors
  string anchors_file =
      apollo::common::util::GetAbsolutePath(model_root, model_param.anchors_file());
  ...
  // loading types
  string types_file = apollo::common::util::GetAbsolutePath(model_root, model_param.types_file());
  yolo::load_types(types_file, &types_);

  res_box_tensor_.reset(new caffe::SyncedMemory(obj_size_ * s_box_block_size * sizeof(float)));
  res_box_tensor_->cpu_data();
  res_box_tensor_->gpu_data();

  res_cls_tensor_.reset(new caffe::SyncedMemory(types_.size() * obj_size_ * sizeof(float)));
...
void YoloCameraDetector::load_intrinsic(const CameraDetectorInitOptions& options) {
  const auto& model_param = yolo_param_.model_param();
  ...
  height_ =
      static_cast<int>(width_ * roi_ratio + aligned_pixel / 2) / aligned_pixel * aligned_pixel;
  ADEBUG << "image_height=" << image_height_ << ", "
         << "image_width=" << image_width_ << ", "
         << "roi_ratio=" << roi_ratio;
  ADEBUG << "offset_y=" << offset_y_ << ", height=" << height_ << ", width=" << width_;
  ...
  int channel = 3;
  image_data_.reset(new caffe::SyncedMemory(roi_w * roi_h * channel * sizeof(unsigned char)));
}

void YoloCameraDetector::load_intrinsic_lane(const CameraDetectorInitOptions& options) {
  ...
  int channel = 3;
  image_data_lane_.reset(new caffe::SyncedMemory(roi_w * roi_h * channel * sizeof(unsigned char)));
}
...
bool YoloCameraDetector::init_cnn(const string& yolo_root) {
  ...
  const auto& model_param = yolo_param_.model_param();
  string model_root  = apollo::common::util::GetAbsolutePath(yolo_root, model_param.model_name());
  string proto_file  = apollo::common::util::GetAbsolutePath(model_root, model_param.proto_file());
  string weight_file = apollo::common::util::GetAbsolutePath(model_root, model_param.weight_file());
  string feature_file =
      apollo::common::util::GetAbsolutePath(model_root, model_param.feature_file());
  ...
  switch (model_type) {
    case obstacle::yolo::ModelType::Caffe: cnnadapter_.reset(new CNNCaffe); break;
    default: AERROR << "unknown model type."; return false;
  }
  ...
  if (feat_param.has_remap_model()) {
    ...
    track_model        = apollo::common::util::GetAbsolutePath(model_root, track_model);
  ...
  for (int i = 0; i < feat_param.extractor_size(); ++i) {
    ...
    switch (extractor_param.feat_type()) {
      case ExtractorParam::Reorg: extractors_[i].reset(new ReorgFeatureExtractor()); break;
...
bool YoloCameraDetector::init_cnn_lane(const string& yolo_root) {
  ...
  ADEBUG << "model_type=" << model_type;
  switch (model_type) {
    case obstacle::yolo::ModelType::Caffe: cnnadapter_lane_.reset(new CNNCaffe); break;
    default: AERROR << "unknown model type."; return false;
  }
...
bool YoloCameraDetector::Multitask(const cv::Mat&                         frame,
  ...
  auto seg_blob = cnnadapter_->get_blob_by_name(yolo_param_.net_param().seg_blob());
  if (seg_blob == nullptr) {
  ...
  *mask = cv::Mat(lane_output_height_, lane_output_width_, CV_32FC1);
  memcpy(mask->data, seg_blob->cpu_data() + lane_output_width_ * lane_output_height_,
...
bool YoloCameraDetector::Lanetask(const cv::Mat& frame, cv::Mat* mask) {
  ...
  auto input_blob = cnnadapter_lane_->get_blob_by_name(lane_param_.net_param().input_blob());
  ...
  auto seg_blob = cnnadapter_lane_->get_blob_by_name(lane_param_.net_param().seg_blob());
  ...
  *mask = cv::Mat(lane_output_height_lane_, lane_output_width_lane_, CV_32FC1);
  ...
  memcpy(tmp.data, seg_blob->cpu_data() + lane_output_width_lane_ * lane_output_height_lane_,
...
bool YoloCameraDetector::Detect(const cv::Mat&                         frame,
  ...
  int      roi_h = frame.rows - offset_y_;
  cv::Rect roi(0, offset_y_, roi_w, roi_h);
  auto     input_blob = cnnadapter_->get_blob_by_name(yolo_param_.net_param().input_blob());
  ...
  for (int i = 0; i < static_cast<int>(temp_objects.size()); ++i) {
    ...
    ADEBUG << "type prob size for object" << i << " is " << sizeof(obj->type_probs) << " (" << obj
           << ")";
    ADEBUG << "prob: " << obj->type_probs[static_cast<int>(obj->type)];
    ADEBUG << "object feature size for object" << i << " is " << obj->object_feature.size();
    ADEBUG << "internal type probs size for object" << i << " is "
           << sizeof(obj->internal_type_probs);
  }

  auto ori_blob = cnnadapter_->get_blob_by_name(yolo_param_.net_param().ori_blob());
  auto dim_blob = cnnadapter_->get_blob_by_name(yolo_param_.net_param().dim_blob());
...
bool YoloCameraDetector::get_objects_cpu(std::vector<std::shared_ptr<VisualObject>>* objects) {
  int  num_classes = types_.size();
  auto loc_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().loc_blob());
  auto obj_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().obj_blob());
  auto cls_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().cls_blob());
  auto dim_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().dim_blob());
  auto ori_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().ori_blob());
  auto lof_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().lof_blob());
  auto lor_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().lor_blob());
  ...
  if (res_box_tensor_ == nullptr || res_cls_tensor_ == nullptr || overlapped_ == nullptr) {
    return false;
  }

  for (int i = 0; i < obj_size_; ++i) {
    get_object_helper(i, loc_data, obj_data, cls_data, ori_data, dim_data, lof_data, lor_data,
                      width, height, num_classes, with_ori, with_dim, with_lof, with_lor);
  }

  const float* cpu_cls_data = static_cast<const float*>(res_cls_tensor_->cpu_data());
  ...
  for (int k = 0; k < num_classes; k++) {
    apply_nms_gpu(static_cast<const float*>(res_box_tensor_->gpu_data()),
                  cpu_cls_data + k * obj_size_, obj_size_, confidence_threshold_, top_k_,
                  nms_.threshold, &(indices[static_cast<int>(types_[k])]), overlapped_, idx_sm_);
    num_kept += indices[static_cast<int>(types_[k])].size();
    vector<float> conf_score(cpu_cls_data + k * obj_size_, cpu_cls_data + (k + 1) * obj_size_);
  ...
  const float* cpu_box_data = static_cast<const float*>(res_box_tensor_->cpu_data());
  ...
  for (auto it = indices.begin(); it != indices.end(); ++it) {
    ...
    for (int j = 0; j < static_cast<int>(indice.size()); ++j) {
      ...
      if (scores[idx] < confidence_threshold_) { continue; }
...
bool YoloCameraDetector::get_objects_gpu(std::vector<std::shared_ptr<VisualObject>>* objects) {
  auto         loc_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().loc_blob());
  auto         obj_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().obj_blob());
  auto         cls_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().cls_blob());
  auto         dim_blob    = cnnadapter_->get_blob_by_name(yolo_param_.net_param().dim_blob());
  ...
  GetObjectsGPU(obj_size_, (const float*)loc_blob->gpu_data(), (const float*)obj_blob->gpu_data(),
                (const float*)cls_blob->gpu_data(), ori_data, dim_data, lof_data, lor_data,
                anchor_data, obj_width, obj_height, num_anchors_, num_classes,
                confidence_threshold_, with_ori, with_dim, with_lof, with_lor,
                static_cast<float*>(res_box_tensor_->mutable_gpu_data()),
                static_cast<float*>(res_cls_tensor_->mutable_gpu_data()), s_box_block_size);
  const float* cpu_cls_data = static_cast<const float*>(res_cls_tensor_->cpu_data());
...
void YoloCameraDetector::get_object_helper(const int    i,
  ...
  float*       res_box_data = static_cast<float*>(res_box_tensor_->mutable_cpu_data());
  float*       res_cls_data = static_cast<float*>(res_cls_tensor_->mutable_cpu_data());
  ...
  float hw = std::exp(loc_data[offset_loc + 2]) * anchor_data[2 * c] / width * 0.5;
  float hh = std::exp(loc_data[offset_loc + 3]) * anchor_data[2 * c + 1] / height * 0.5;
  ...
  if (with_ori) {
    ...
    res_box_data[i * box_block + 4] = std::atan2(ori_data[offset_ori + 1], ori_data[offset_ori]);
  }
