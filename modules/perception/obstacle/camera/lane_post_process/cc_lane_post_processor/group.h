...
struct Group {
  ...
  inline int ComputeOrientation(const std::vector<Marker>& markers, const GroupParam& param);

  inline ScalarType ComputeDistance(const Group& tar_group, const AssociationParam& param) const;
...
inline int Group::ComputeOrientation(const std::vector<Marker>& markers,
  ...
  for (; start_marker_count < static_cast<int>(start_marker_idx.size()); ++start_marker_count) {
    if (start_marker_idx[start_marker_count] == -1) { break; }
  }
  ...
  switch (shape_type) {
    case MarkerShapeType::POINT: {
      ...
      if (start_marker_count > 1) {
        start_ind =
            std::min(group_param.orientation_estimation_skip_marker_num, start_marker_count - 2);
      }
      int end_ind = start_marker_count - 1;
      ADEBUG << "start markers: start_ind=" << start_ind << ", end_ind=" << end_ind;
    ...
    case MarkerShapeType::LINE_SEGMENT: {
      int start_ind =
          std::min(group_param.orientation_estimation_skip_marker_num, start_marker_count - 1);
      int end_ind = start_marker_count - 1;
      ADEBUG << "start markers: start_ind=" << start_ind << ", end_ind=" << end_ind;
  ...
  for (; end_marker_count < static_cast<int>(end_marker_idx.size()); ++end_marker_count) {
    if (end_marker_idx[end_marker_count] == -1) { break; }
  }
  ...
  switch (shape_type) {
    case MarkerShapeType::POINT: {
      ...
      if (end_marker_count > 1) {
        end_ind =
            std::min(group_param.orientation_estimation_skip_marker_num, end_marker_count - 2);
      }
      ADEBUG << "end markers: start_ind=" << start_ind << ", end_ind=" << end_ind;

      end_orie =
          markers.at(end_marker_idx.at(end_ind)).pos - markers.at(end_marker_idx.at(start_ind)).pos;
      break;
    }
    ...
    case MarkerShapeType::LINE_SEGMENT: {
      int start_ind = end_marker_count - 1;
      int end_ind =
          std::min(group_param.orientation_estimation_skip_marker_num, end_marker_count - 1);
      ADEBUG << "end markers: start_ind=" << start_ind << ", end_ind=" << end_ind;
...
inline ScalarType Group::ComputeDistance(const Group&            tar_group,
  ...
  ADEBUG << "target point = (" << tar_group.start_pos(0) << ", " << tar_group.start_pos(1) << ")";
  Vector2D   displacement = tar_group.start_pos - this->end_pos;
  ScalarType norm_v       = this->end_orie.norm();
  if (norm_v < kEpsilon) {
    ADEBUG << "norm of orientation vector for reference group is too small: " << norm_v;
  }
  ...
  if (norm_v >= kEpsilon) { projection_dist /= norm_v; }
  ADEBUG << "(1) projection_dist = " << std::to_string(projection_dist) << " ["
         << std::to_string(param.min_distance) << ", " << std::to_string(param.max_distance) << "]";
  if (projection_dist < param.min_distance || projection_dist > param.max_distance) { return -1; }
  ...
  ScalarType departure_dist = std::abs(
      this->end_orie.y() * tar_group.start_pos.x() - this->end_orie.x() * tar_group.start_pos.y() +
      this->end_orie.x() * this->end_pos.y() - this->end_orie.y() * this->end_pos.x());
  if (norm_v >= kEpsilon) { departure_dist /= norm_v; }
  ADEBUG << "(2) departure_dist = " << std::to_string(departure_dist) << " ("
         << std::to_string(param.max_departure_distance) << ")";
  if (departure_dist > param.max_departure_distance) { return -2; }
  ...
  ADEBUG << "(3) deviation_angle_dist = " << std::to_string(deviation_angle_dist / M_PI * 180.0);
  if (deviation_angle_dist > param.max_deviation_angle) { return -3; }
  ...
  if (tar_group_start_len > param.min_orientation_estimation_size) {
    ...
    if (orie_dist > param.max_relative_orie) { return -4; }
  }

  ScalarType r = std::max(std::abs(param.min_distance), std::abs(param.max_distance));
  if (r > kEpsilon) { projection_dist = std::abs(projection_dist) / r; }
  if (param.max_departure_distance > kEpsilon) { departure_dist /= param.max_departure_distance; }
  if (param.max_deviation_angle > kEpsilon) { deviation_angle_dist /= param.max_deviation_angle; }
  if (param.max_relative_orie > kEpsilon) { orie_dist /= param.max_relative_orie; }

  ScalarType dist =
      param.distance_weight * projection_dist + param.departure_distance_weight * departure_dist +
      param.deviation_angle_weight * deviation_angle_dist + param.relative_orie_weight * orie_dist;
