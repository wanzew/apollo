...
class ConnectedComponent {
  ...
  */
  ...
  std::shared_ptr<const std::vector<cv::Point2i>> GetPixels() const { return pixels_; }
  ...
  std::shared_ptr<const std::vector<int>> bbox_pixel_idx() const { return bbox_.bbox_pixel_idx; }

  int GetBboxPixelCount() const { return static_cast<int>(bbox_.bbox_pixel_idx->size()); }

  // vertices
  void                                       FindVertices();
  std::shared_ptr<const std::vector<Vertex>> GetVertices() const { return vertices_; }
  ...
  bool IsValidEdgeVertices(int i, int j) {
    return i >= 0 && i < this->GetVertexCount() && j >= 0 && j < this->GetVertexCount() && i != j;
  }
  ...
  std::shared_ptr<const Edge> GetClockWiseEdge() const { return clockwise_edge_; }
  std::shared_ptr<const Edge> GetAntiClockWiseEdge() const { return anticlockwise_edge_; }
  std::shared_ptr<const Edge> GetInnerEdge() const { return inner_edge_; }

  void                               SplitContour(int split_len);
  std::shared_ptr<std::vector<Edge>> GetClockWiseEdges() const { return clockwise_edges_; }
  std::shared_ptr<std::vector<Edge>> GetAntiClockWiseEdges() const { return anticlockwise_edges_; }
  std::shared_ptr<std::vector<Edge>> GetInnerEdges() const { return inner_edges_; }
 ...
 private:
  ...
  void
       SplitContourVertical(int start_vertex_id, int end_vertex_id, int len_split, bool is_clockwise);
  void SplitContourVertical(int len_split, bool is_clockwise, int start_pos, int end_pos);
  void
       SplitContourHorizontal(int start_vertex_id, int end_vertex_id, int len_split, bool is_clockwise);
