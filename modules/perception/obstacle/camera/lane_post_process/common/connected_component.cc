...
void ConnectedComponent::AddPixel(int x, int y) {
  if (pixel_count_ == 0) {
    ...
    if (x < bbox_.x_min) { bbox_.x_min = x; }
    if (x > bbox_.x_max) { bbox_.x_max = x; }
    if (y < bbox_.y_min) { bbox_.y_min = y; }
    if (y > bbox_.y_max) { bbox_.y_max = y; }
  }
...
ConnectedComponent::BoundingBoxSplitType ConnectedComponent::DetermineSplit(ScalarType split_siz) {
  ...
  if (diag_len >= split_siz) {
    bbox_.split = (height < 5) ? BoundingBoxSplitType::HORIZONTAL : BoundingBoxSplitType::VERTICAL;
  } else {
...
void ConnectedComponent::FindContourForSplit() {
  if (bbox_.split == BoundingBoxSplitType::NONE) { return; }

  // initialize contours
  if (bbox_.split == BoundingBoxSplitType::VERTICAL) {
    bbox_.left_contour.reset(new std::vector<int>(bbox_.y_max - bbox_.y_min + 1, bbox_.x_max));
    bbox_.right_contour.reset(new std::vector<int>(bbox_.y_max - bbox_.y_min + 1, bbox_.x_min));
  } else if (bbox_.split == BoundingBoxSplitType::HORIZONTAL) {
    bbox_.up_contour.reset(new std::vector<int>(bbox_.x_max - bbox_.x_min + 1, bbox_.y_max));
    bbox_.down_contour.reset(new std::vector<int>(bbox_.x_max - bbox_.x_min + 1, bbox_.y_min));
  }
  ...
  for (int i = 0; i < pixel_count_; ++i) {
    ...
    if (bbox_.split == BoundingBoxSplitType::VERTICAL) {
      ...
      bbox_.left_contour->at(y)  = min(bbox_.left_contour->at(y), pixels_->at(i).x);
      bbox_.right_contour->at(y) = max(bbox_.right_contour->at(y), pixels_->at(i).x);
    } else if (bbox_.split == BoundingBoxSplitType::HORIZONTAL) {
      int x                     = pixels_->at(i).x - bbox_.x_min;
      bbox_.up_contour->at(x)   = min(bbox_.up_contour->at(x), pixels_->at(i).y);
      bbox_.down_contour->at(x) = max(bbox_.down_contour->at(x), pixels_->at(i).y);
    }
  }
}
...
void ConnectedComponent::FindVertices() {
  ...
  for (auto it = bbox_.bbox_pixel_idx->begin(); it != bbox_.bbox_pixel_idx->end(); ++it) {
    const cv::Point2i* p = &(pixels_->at(*it));
    if (p->x == x_min()) { left_boundary.insert(p->y); }
    if (p->y == y_min()) { up_boundary.insert(p->x); }
    if (p->x == x_max()) { right_boundary.insert(p->y); }
    if (p->y == y_max()) { down_boundary.insert(p->x); }
  }
...
void ConnectedComponent::FindEdges() {
  ...
  for (int i = 0; i < GetVertexCount(); ++i) {
    ...
    new_vec = p - q;
    cross   = clockwise_edge_->vec(0) * new_vec(1) - new_vec(0) * clockwise_edge_->vec(1);
    if (cross > kEpsCross) {
      if (clockwise_edge_->vec(0) >= 0) {
        // from left to right
        if (p(0) == static_cast<ScalarType>(x_max()) || p(1) == static_cast<ScalarType>(y_min())) {
        ...
        // from right to left
        if (p(0) == static_cast<ScalarType>(x_min()) || p(1) == static_cast<ScalarType>(y_min())) {
          clockwise_edge_->end_vertex_id = i;
        } else {
          clockwise_edge_->start_vertex_id = i;
        }
      }
      const Vertex& new_start_vertex = vertices_->at(clockwise_edge_->start_vertex_id);
      const Vertex& new_end_vertex   = vertices_->at(clockwise_edge_->end_vertex_id);
    ...
    new_vec = p - q;
    cross   = anticlockwise_edge_->vec(0) * new_vec(1) - new_vec(0) * anticlockwise_edge_->vec(1);
    if (cross < -kEpsCross) {
      ...
      const Vertex& new_start_vertex = vertices_->at(anticlockwise_edge_->start_vertex_id);
      const Vertex& new_end_vertex   = vertices_->at(anticlockwise_edge_->end_vertex_id);
  ...
  clockwise_edge_->orie = atan2(clockwise_edge_->vec(1), clockwise_edge_->vec(0));

  anticlockwise_edge_->len  = anticlockwise_edge_->vec.norm();
  anticlockwise_edge_->orie = atan2(anticlockwise_edge_->vec(1), anticlockwise_edge_->vec(0));
  ...
  anticlockwise_edges_->back().start_vertex_id = anticlockwise_edge_->start_vertex_id;
  anticlockwise_edges_->back().end_vertex_id   = anticlockwise_edge_->end_vertex_id;
...
void ConnectedComponent::SplitContourVertical(int  start_vertex_id,
                                              ...
                                              int  len_split,
                                              bool is_clockwise) {
  int start_pos = static_cast<int>(vertices_->at(start_vertex_id)(1));  // y_start
  ...
  for (int k = 0; k < static_cast<int>(lens.size()) - 1; ++k) {
    ...
    (is_clockwise ? clockwise_edges_ : anticlockwise_edges_)
        ->push_back(MakeEdge(start_vertex_id, static_cast<int>(vertices_->size()) - 1));
    start_pos = end_pos - 1;
...
void ConnectedComponent::SplitContourVertical(int  len_split,
                                              bool is_clockwise,
                                              int  start_pos,
                                              int  end_pos) {
  int         height = start_pos - end_pos + 1;
...
void ConnectedComponent::SplitContourHorizontal(int  start_vertex_id,
  ...
  if (start_pos <= end_pos) {
    ...
    for (int k = 0; k < static_cast<int>(lens.size()) - 1; ++k) {
      end_pos = start_pos + lens[k] - 1;
      int y   = (is_clockwise ? bbox_.down_contour : bbox_.up_contour)->at(end_pos - this->x_min());
      vertices_->push_back(Vertex(end_pos, y));
      (is_clockwise ? clockwise_edges_ : anticlockwise_edges_)
          ->push_back(MakeEdge(start_vertex_id, static_cast<int>(vertices_->size()) - 1));
      start_pos = end_pos + 1;
      y = (is_clockwise ? bbox_.down_contour : bbox_.up_contour)->at(start_pos - this->x_min());
    ...
    for (int k = 0; k < static_cast<int>(lens.size()) - 1; ++k) {
      end_pos = start_pos - lens[k] + 1;
      int y   = (is_clockwise ? bbox_.up_contour : bbox_.down_contour)->at(end_pos - this->x_min());
      vertices_->push_back(Vertex(end_pos, y));
      (is_clockwise ? clockwise_edges_ : anticlockwise_edges_)
          ->push_back(MakeEdge(start_vertex_id, static_cast<int>(vertices_->size()) - 1));
      start_pos = end_pos - 1;
      y = (is_clockwise ? bbox_.up_contour : bbox_.down_contour)->at(start_pos - this->x_min());
      vertices_->push_back(Vertex(start_pos, y));
      start_vertex_id = static_cast<int>(vertices_->size()) - 1;
    }
    (is_clockwise ? clockwise_edges_ : anticlockwise_edges_)
...
void ConnectedComponent::SplitContourHorizontal(int  len_split,
  ...
  if (start_pos <= end_pos) {
    // direction from left to right
    int y = (is_clockwise ? bbox_.down_contour : bbox_.up_contour)->at(start_pos - this->x_min());
    vertices_->push_back(Vertex(start_pos, y));
    int start_vertex_id = static_cast<int>(vertices_->size()) - 1;

    y = (is_clockwise ? bbox_.down_contour : bbox_.up_contour)->at(end_pos - this->x_min());
    vertices_->push_back(Vertex(end_pos, y));
    int end_vertex_id = static_cast<int>(vertices_->size()) - 1;
    ...
    // direction from right to left
    int y = (is_clockwise ? bbox_.up_contour : bbox_.down_contour)->at(start_pos - this->x_min());
    vertices_->push_back(Vertex(start_pos, y));
    int start_vertex_id = static_cast<int>(vertices_->size()) - 1;

    y = (is_clockwise ? bbox_.up_contour : bbox_.down_contour)->at(end_pos - this->x_min());
    vertices_->push_back(Vertex(end_pos, y));
    int end_vertex_id = static_cast<int>(vertices_->size()) - 1;
...
void ConnectedComponent::SplitContour(int split_len) {
  ...
  if (bbox_.split == BoundingBoxSplitType::VERTICAL) {
    ...
    std::cerr << "unknown bounding box split type: " << bbox_.split << std::endl;
  }
}
...
vector<int> ConnectedComponent::GetSplitRanges(int siz, int len_split) {
  if (siz <= 0) { std::cerr << "siz should be a positive number: " << siz << std::endl; }
  if (len_split <= 0) {
    std::cerr << "len_split should be a positive number: " << len_split << std::endl;
  }
...
ConnectedComponentGenerator::ConnectedComponentGenerator(int image_width, int image_height)
  ...
  total_pix_ = static_cast<size_t>(image_width_) * static_cast<size_t>(image_height_);
...
ConnectedComponentGenerator::ConnectedComponentGenerator(int      image_width,
  ...
  if (roi_x_min_ < 0) { std::cerr << "x_min is less than zero: " << roi_x_min_ << std::endl; }
  if (roi_y_min_ < 0) { std::cerr << "y_min is less than zero: " << roi_y_min_ << std::endl; }
  if (roi_x_max_ >= image_width_) {
    std::cerr << "x_max is larger than image width: " << roi_x_max_ << "|" << image_width_
              << std::endl;
  }
  if (roi_y_max_ >= image_height_) {
    std::cerr << "y_max is larger than image height: " << roi_y_max_ << "|" << image_height_
              << std::endl;
  }
...
bool ConnectedComponentGenerator::FindConnectedComponents(
  ...
  if (cc == NULL) {
    std::cerr << "the pointer of output connected components is null." << std::endl;
    return false;
  }
    ...
    for (x = roi_x_min_; x <= roi_x_max_; x++, cur_idx++) {
      ...
      if (cur_p[x] > 0) {
        if (left_val == 0 && up_val == 0) {
          ...
          // neighbors
          frame_label_[cur_idx] = (frame_label_[left_idx] > frame_label_[up_idx]) ?
                                      frame_label_[up_idx] :
    ...
    for (x = roi_x_min_; x <= roi_x_max_; x++, cur_idx++) {
      ...
      if (curt_label >= 0) {
        if (curt_label >= static_cast<int>(labels_.Num())) {
          std::cerr << "curt_label should be smaller than labels.num(): " << curt_label << " vs. "
                    << labels_.Num() << std::endl;
          return false;
        }
        curt_label = labels_.Find(curt_label);
        if (curt_label >= static_cast<int>(root_map_.size())) {
          std::cerr << "curt_label should be smaller than root_map.size() " << curt_label << " vs. "
                    << root_map_.size() << std::endl;
          return false;
        }
