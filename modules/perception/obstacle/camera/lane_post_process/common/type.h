...
constexpr ScalarType INVERSE_AVEAGE_LANE_WIDTH_METER = 1.0 / AVEAGE_LANE_WIDTH_METER;
...
struct Marker {
  ...
  static bool comp(const Marker& a, const Marker& b) {
    ...
    return (a.space_type == SpaceType::IMAGE) ? a.pos(1) > b.pos(1) : a.pos(1) < b.pos(1);
  }
};
...
struct LaneInstance {
  ...
  static bool CompareSiz(const LaneInstance& a, const LaneInstance& b) { return a.siz > b.siz; }
...
struct LaneObject {
  ...
  void ToLaneMarkerProto(LaneMarker* lane_marker) const {
    ...
    lane_marker->set_view_range(std::max(longitude_end, static_cast<ScalarType>(0)));
  ...
  std::string GetSpatialLabel() const {
    switch (spatial) {
      ...
      default: AERROR << "unknown lane spatial label."; return "unknown spatial label";
    }
  }
};
...
void LaneObjectsToLaneMarkerProto(const LaneObjects& lane_objects, LaneMarkers* lane_markers);
}  // namespace perception
}  // namespace apollo
