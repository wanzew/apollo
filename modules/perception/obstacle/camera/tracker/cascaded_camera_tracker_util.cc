...
void GetDetectedFromVO(const cv::Size&                                   sz,
  ...
  for (auto obj_ptr : objects) {
    ...
    float x1 =
        std::min(static_cast<float>(obj_ptr->upper_left[0]), static_cast<float>(sz.width - 1));
    x1 = std::max(x1, 0.0f);

    float y1 =
        std::min(static_cast<float>(obj_ptr->upper_left[1]), static_cast<float>(sz.height - 1));
    y1 = std::max(y1, 0.0f);

    float x2 =
        std::min(static_cast<float>(obj_ptr->lower_right[0]), static_cast<float>(sz.width - 1));
    x2 = std::max(x2, 0.0f);

    float y2 =
        std::min(static_cast<float>(obj_ptr->lower_right[1]), static_cast<float>(sz.height - 1));
    y2 = std::max(y2, 0.0f);
...
void MergeAffinityMatrix(const std::vector<std::vector<float>>& to_merge,
  ...
  for (size_t i = 0; i < affinity_matrix->size(); ++i) {
    for (size_t j = 0; j < (*affinity_matrix)[0].size(); ++j) {
      ...
      if ((*affinity_matrix)[i][j] < 9.0f) { (*affinity_matrix)[i][j] *= to_merge[i][j]; }
    }
  }
    ...
    for (size_t j = 0; j < (*affinity_matrix)[0].size(); ++j) {
      if ((*affinity_matrix)[i][j] > high_threshold) {
        ...
        for (size_t k = 0; k < affinity_matrix->size(); ++k) {
          if (k == i) continue;
          if ((*affinity_matrix)[k][j] > other_threshold) other_high_conf = true;
        }
        // Same row
        for (size_t k = 0; k < (*affinity_matrix)[0].size(); ++k) {
          if (k == j) continue;
          if ((*affinity_matrix)[i][k] > other_threshold) other_high_conf = true;
        }
...
void MatrixMatching(const std::vector<std::vector<float>>& affinity_matrix,
  ...
  std::vector<std::vector<double>> affinity(affinity_matrix.size(),
  ...
  if (!affinity_matrix.empty()) {
    ...
    for (size_t i = 0; i < affinity_matrix.size(); ++i) {
      ...
      AINFO << i << "th T, track_id:" << tracked[i].track_id_
            << " predetect_id_:" << tracked[i].detect_id_ << "\t" << to_print_affinity;
    }
  } else {
