...
bool Cipv::DistanceFromPointToLineSegment(const Point2Df& point,
  ...
  common::math::LineSegment2d line_seg({line_seg_start_point[0], line_seg_start_point[1]},
...
bool Cipv::GetEgoLane(const LaneObjectsPtr lane_objects,
  ...
  for (size_t i = 0; i < lane_objects->size(); ++i) {
    if ((*lane_objects)[i].spatial == L_0) {
      ...
      if ((*lane_objects)[i].image_pos.size() < MIN_LANE_LINE_LENGTH_FOR_CIPV_DETERMINATION) {
        *b_left_valid = false;
      } else {
        *b_left_valid = true;
        ...
        for (size_t j = 0; j < (*lane_objects)[i].image_pos.size(); ++j) {
          ...
          Eigen::Vector2f ground_point((*lane_objects)[i].pos[j][0], (*lane_objects)[i].pos[j][1]);
...
bool Cipv::MakeVirtualLane(const LaneLine& ref_lane_line,
  ...
  if (b_image_based_cipv_ == false) {
    for (uint32_t i = 0; i < ref_lane_line.line_point.size(); ++i) {
      Eigen::Vector2f virtual_line_point(ref_lane_line.line_point[i][0],
...
bool Cipv::ElongateEgoLane(const LaneObjectsPtr lane_objects,
  ...
  if (b_left_valid && b_right_valid) {
    ...
    offset_distance =
        -(fabs(egolane_ground->right_line.line_point[0][1]) + EGO_CAR_HALF_VIRTUAL_LANE);
    ...
    offset_distance =
        (fabs(egolane_ground->left_line.line_point[0][1]) + EGO_CAR_HALF_VIRTUAL_LANE);
...
bool Cipv::FindClosestEdgeOfObjectImage(const std::shared_ptr<Object>& object,
  ...
  // angle = atan2(y, x)
  if (fabs(atan2(object->direction[1], object->direction[0])) <= FOURTY_FIVE_DEGREE) {
    // get back of the vehicle
    closted_object_edge->start_point[0] = x2 * cos_theta + y1 * sin_theta + center_x;
    closted_object_edge->start_point[1] = y1 * cos_theta - x2 * sin_theta + center_y;

    closted_object_edge->end_point[0] = x2 * cos_theta + y2 * sin_theta + center_x;
    closted_object_edge->end_point[1] = y2 * cos_theta - x2 * sin_theta + center_y;

    // If a vehicle faces side way, extract the side edge of a vehicle
  } else if (atan2(object->direction[1], object->direction[0]) > FOURTY_FIVE_DEGREE) {
    // get left side of the vehicle
    closted_object_edge->start_point[0] = x2 * cos_theta + y1 * sin_theta + center_x;
    closted_object_edge->start_point[1] = y1 * cos_theta - x2 * sin_theta + center_y;

    closted_object_edge->end_point[0] = x2 * cos_theta + y1 * sin_theta + center_x;
    closted_object_edge->end_point[1] = y1 * cos_theta - x2 * sin_theta + center_y;
  } else if (atan2(object->direction[1], object->direction[0]) < -FOURTY_FIVE_DEGREE) {
    // get right side of the vehicle

    closted_object_edge->start_point[0] = x1 * cos_theta + y2 * sin_theta + center_x;
    closted_object_edge->start_point[1] = y2 * cos_theta - x1 * sin_theta + center_y;

    closted_object_edge->end_point[0] = x2 * cos_theta + y2 * sin_theta + center_x;
    closted_object_edge->end_point[1] = y2 * cos_theta - x2 * sin_theta + center_y;
  } else {
    // don't get front of vehicle
    closted_object_edge->start_point[0] = x1 * cos_theta + y1 * sin_theta + center_x;
    closted_object_edge->start_point[1] = y1 * cos_theta - x1 * sin_theta + center_y;

    closted_object_edge->end_point[0] = x1 * cos_theta + y2 * sin_theta + center_x;
    closted_object_edge->end_point[1] = y2 * cos_theta - x1 * sin_theta + center_y;
  }
...
bool Cipv::FindClosestEdgeOfObjectGround(const std::shared_ptr<Object>& object,
  ...
  float  y2          = 0 - y1;
  double len         = sqrt(direction_x * direction_x + direction_y * direction_y);
  if (len < B_FLT_EPSILON) { return false; }
  float cos_theta = direction_x / len;
  float sin_theta = -direction_y / len;
...
bool Cipv::AreDistancesSane(const float distance_start_point_to_right_lane,
  ...
  if (distance_start_point_to_right_lane > MAX_DIST_OBJECT_TO_LANE_METER) {
    if (debug_level_ >= 1) {
      AINFO << "distance from start to right lane(" << distance << " m) is too long";
    }
    return false;
  }
  if (distance_start_point_to_left_lane > MAX_DIST_OBJECT_TO_LANE_METER) {
    if (debug_level_ >= 1) {
      AINFO << "distance from start to left lane(" << distance << " m) is too long";
    }
    return false;
  }
  if (distance_end_point_to_right_lane > MAX_DIST_OBJECT_TO_LANE_METER) {
    if (debug_level_ >= 1) {
      AINFO << "distance from end to right lane(" << distance << " m) is too long";
    }
    return false;
  }
  if (distance_end_point_to_left_lane > MAX_DIST_OBJECT_TO_LANE_METER) {
    if (debug_level_ >= 1) {
      AINFO << "distance from end to left lane(" << distance << " m) is too long";
    }
    return false;
  }
  distance = fabs(distance_start_point_to_right_lane - distance_end_point_to_right_lane);
  if (distance > MAX_VEHICLE_WIDTH_METER) {
    if (debug_level_ >= 1) { AINFO << "width of vehicle (" << distance << " m) is too long"; }
    return false;
  }

  distance = fabs(distance_end_point_to_left_lane - distance_start_point_to_left_lane);
  if (distance > MAX_VEHICLE_WIDTH_METER) {
    if (debug_level_ >= 1) { AINFO << "width of vehicle (" << distance << " m) is too long"; }
    return false;
  }
...
bool Cipv::IsObjectInTheLaneGround(const std::shared_ptr<Object>& object,
  ...
  float b_valid_object =
      FindClosestEdgeOfObjectGround(object, egolane_ground, &closted_object_edge);
  if (!b_valid_object) {
    if (debug_level_ >= 1) { ADEBUG << "The closest edge of an object is not available"; }
    return false;
  }
  ...
  if (egolane_ground.left_line.line_point.size() <= 1) {
    if (debug_level_ >= 1) { ADEBUG << "No left lane"; }
    return false;
  }
  ...
  for (size_t i = 0; i + 1 < egolane_ground.left_line.line_point.size(); ++i) {
    ...
    if (DistanceFromPointToLineSegment(
            closted_object_edge.end_point, egolane_ground.left_line.line_point[i],
  ...
  if (egolane_ground.right_line.line_point.size() <= 1) {
    if (debug_level_ >= 1) { ADEBUG << "No right lane"; }
    return false;
  }
  closest_index     = -1;
  shortest_distance = MAX_FLOAT;
  for (size_t i = 0; i + 1 < egolane_ground.right_line.line_point.size(); ++i) {
    ...
    distance = MAX_FLOAT;
    if (DistanceFromPointToLineSegment(
            closted_object_edge.start_point, egolane_ground.right_line.line_point[i],
  ...
  if (AreDistancesSane(
          shortest_distance_start_point_to_right_lane, shortest_distance_start_point_to_left_lane,
          shortest_distance_end_point_to_right_lane, shortest_distance_end_point_to_left_lane)) {
...
bool Cipv::DetermineCipv(const LaneObjectsPtr                  lane_objects,
  ...
  ElongateEgoLane(lane_objects, b_left_valid, b_right_valid, yaw_rate, velocity, &egolane_image,
                  &egolane_ground);

  for (int32_t i = 0; i < static_cast<int32_t>(objects->size()); ++i) {
    if (debug_level_ >= 2) { AINFO << "objects[i]->track_id: " << (*objects)[i]->track_id; }
    if (IsObjectInTheLane((*objects)[i], egolane_image, egolane_ground) == true) {
      if (cipv_index < 0 || (*objects)[i]->center[0] < (*objects)[cipv_index]->center[0]) {
...
bool Cipv::CollectDrops(const MotionBuffer&                   motion_buffer,
  ...
  for (auto obj : *objects) {
    ...
    object_trackjectories_[cur_id].push_back(std::make_pair(obj->center[0], obj->center[1]));
    ...
    obj->drops.clear();
    // Add drops
    for (std::size_t it = object_trackjectories_[cur_id].size() - 1, count = 0; it > 0;
         it--, count++) {
      if (count >= DROPS_HISTORY_SIZE || count > motion_buffer.size()) { break; }
      ...
      TranformPoint(pt, motion_buffer[motion_size - count - 1].motion, &transformed_pt);
  ...
  for (const auto& each_object : object_trackjectories_) {
    ...
    if (b_found_id == false && object_trackjectories_[obj_id].size() > 0) {
      ...
      if (object_id_skip_count_[obj_id] >= MAX_ALLOWED_SKIP_OBJECT) {
        if (debug_level_ >= 2) { AINFO << "Removed obsolte object " << obj_id; }
