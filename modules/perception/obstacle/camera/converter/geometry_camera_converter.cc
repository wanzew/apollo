...
bool GeometryCameraConverter::Init() {
  ...
  if (!LoadCameraIntrinsics(config_.camera_intrinsic_file())) {
    AERROR << "Failed to get camera intrinsics: " << config_.camera_intrinsic_file();
    return false;
  }
...
bool GeometryCameraConverter::Convert(std::vector<std::shared_ptr<VisualObject>>* objects) {
  ...
  for (auto& obj : *objects) {
    ...
    if (obj->trunc_height < 0.25f) {
      ...
      ConvertSingle(obj->height, obj->width, obj->length, deg_alpha, upper_left, lower_right, false,
                    &distance, &mass_center_pixel);
    } else if (obj->trunc_width < 0.25f && obj->trunc_height > 0.25f) {
      // 2D height truncation and no width truncation
      ConvertSingle(obj->height, obj->width, obj->length, deg_alpha, upper_left, lower_right, true,
                    &distance, &mass_center_pixel);
    } else {
    ...
    float scale =
        obj->distance / sqrt(camera_ray.x() * camera_ray.x() + camera_ray.y() * camera_ray.y() +
...
std::string GeometryCameraConverter::Name() const { return "GeometryCameraConverter"; }

bool GeometryCameraConverter::LoadCameraIntrinsics(const std::string& file_path) {
...
bool GeometryCameraConverter::ConvertSingle(const float            h,
                                            const float            w,
                                            const float            l,
                                            const float            alpha_deg,
                                            const Eigen::Vector2f& upper_left,
                                            const Eigen::Vector2f& lower_right,
                                            bool                   use_width,
                                            float*                 distance,
  ...
  float relative_x = (center_pixel.x() - min_pixel_x) / (max_pixel_x - min_pixel_x);
  float relative_y = (center_pixel.y() - min_pixel_y) / (max_pixel_y - min_pixel_y);
  ...
  Eigen::Matrix<float, 3, 1> mass_center_v = camera_model_.unproject(*mass_center_pixel);
  mass_center_v                            = MakeUnit(mass_center_v);

  // Distance search
  *distance = SearchDistance(pixel_length, use_width, mass_center_v, 0.1f, 150.0f);
  for (size_t i = 0; i < 1; ++i) {
    // Mass center search
    SearchCenterDirection(box_center_pixel, *distance, &mass_center_v, mass_center_pixel);
    // Distance search
    *distance = SearchDistance(pixel_length, use_width, mass_center_v, 0.9f * (*distance),
                               1.1f * (*distance));
  }
...
void GeometryCameraConverter::Rotate(const float                   alpha_deg,
...
float GeometryCameraConverter::SearchDistance(const int                         pixel_length,
                                              const bool&                       use_width,
  ...
  while (close_d <= far_d && depth < kMaxDistanceSearchDepth_) {
    ...
    if (std::abs(next_d - curr_d) < 0.1f) { break; }
...
void GeometryCameraConverter::SearchCenterDirection(
    const Eigen::Matrix<float, 2, 1>& box_center_pixel,
    const float                       curr_d,
  ...
  while (depth < kMaxCenterDirectionSearchDepth_) {
    ...
    float min_pixel_y = std::numeric_limits<float>::max();
    for (size_t i = 0; i < corners_.size(); ++i) {
      Eigen::Vector2f point_2d = camera_model_.project(corners_[i] + new_center_v);
      min_pixel_x              = std::min(min_pixel_x, point_2d.x());
      max_pixel_x              = std::max(max_pixel_x, point_2d.x());
      min_pixel_y              = std::min(min_pixel_y, point_2d.y());
      max_pixel_y              = std::max(max_pixel_y, point_2d.y());
...
GeometryCameraConverter::MakeUnit(const Eigen::Matrix<float, 3, 1>& v) const {
  Eigen::Matrix<float, 3, 1> unit_v = v;
  float                      to_unit_scale =
      std::sqrt(unit_v.x() * unit_v.x() + unit_v.y() * unit_v.y() + unit_v.z() * unit_v.z());
  unit_v /= to_unit_scale;
  return unit_v;
}

void GeometryCameraConverter::CheckSizeSanity(std::shared_ptr<VisualObject> obj) const {
...
void GeometryCameraConverter::CheckTruncation(
    std::shared_ptr<VisualObject> obj, Eigen::Matrix<float, 2, 1>* trunc_center_pixel) const {
  ...
  if (obj->upper_left.y() < 30.0f || height - 30.0f < obj->lower_right.y()) {
    ...
    trunc_center_pixel->x() = (obj->upper_left.x() + obj->lower_right.x()) / 2.0f;
  }
...
void GeometryCameraConverter::SetBoxProjection(std::shared_ptr<VisualObject> obj) const {
  obj->pts8.resize(16);
