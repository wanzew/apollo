...
void PbfKalmanMotionFusion::Initialize(const std::shared_ptr<PbfSensorObject> new_object) {
  ...
  r_matrix_.topLeftCorner(2, 2) =
      FLAGS_r_matrix_amplifier * new_object->object->position_uncertainty.topLeftCorner(2, 2);
  r_matrix_.block<2, 2>(2, 2) =
      FLAGS_r_matrix_amplifier * new_object->object->velocity_uncertainty.topLeftCorner(2, 2);

  p_matrix_.setIdentity();
  p_matrix_.topLeftCorner(2, 2) =
      FLAGS_p_matrix_amplifier * new_object->object->position_uncertainty.topLeftCorner(2, 2);
  p_matrix_.block<2, 2>(2, 2) =
      FLAGS_p_matrix_amplifier * new_object->object->velocity_uncertainty.topLeftCorner(2, 2);
...
void PbfKalmanMotionFusion::UpdateWithObject(const std::shared_ptr<PbfSensorObject> new_object,
                                             const double                           time_diff) {
  ACHECK(new_object != nullptr && new_object->object != nullptr)
  ...
  Eigen::Vector4d predict_measurement(priori_state_(0), priori_state_(1), priori_state_(2),
                                      priori_state_(3));
  ...
  if (belief_velocity_.head(2).norm() < 0.05) { belief_velocity_ = Eigen::Vector3d(0, 0, 0); }
}
...
int PbfKalmanMotionFusion::GetRadarHistoryLength() {
  ...
  for (size_t i = 0; i < history_velocity_is_radar_.size(); ++i) {
    if (history_velocity_is_radar_[i]) { history_length++; }
  }
...
int PbfKalmanMotionFusion::GetLidarHistoryIndex(const int history_seq) {
  ...
  for (size_t i = 1; i <= history_velocity_is_radar_.size(); ++i) {
    ...
    if (!history_velocity_is_radar_[history_index]) { history_count++; }
    if (history_count == history_seq) { break; }
  }
