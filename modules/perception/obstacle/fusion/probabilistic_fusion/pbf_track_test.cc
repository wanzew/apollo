...
TEST(PbfTrackTest, test_pbf_update_measurements_life) {
  ...
  track.UpdateMeasurementsLifeWithMeasurement(&(track.lidar_objects_), "velodyne_64_1", 0.25, 0.2);
  CHECK_EQ(track.lidar_objects_.size(), 1);
  ...
  track.UpdateMeasurementsLifeWithoutMeasurement((&track.lidar_objects_), "velodyne_64", 0.35, 0.2,
                                                 &invisible_state);
  CHECK_EQ(track.lidar_objects_.size(), 1);
