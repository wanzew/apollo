...
bool PbfHmTrackObjectMatcher::Match(
  ...
  ComputeAssociationMat(fusion_tracks, sensor_objects, *unassigned_fusion_tracks,
                        *unassigned_sensor_objects, *(options.ref_point), &association_mat);
  ...
  if (unassigned_fusion_tracks->empty() || unassigned_sensor_objects->empty()) { return true; }

  bool state =
      HmAssign(association_mat, assignments, unassigned_fusion_tracks, unassigned_sensor_objects);
  ...
  for (const auto& track_measurement_pair : *assignments) {
    ...
    ADEBUG << "track_ind is matched to measurement_ind for sensor " << sensor_objects[0]->sensor_id
    ...
    if (track_ind_loc >= 0 && measurement_ind_loc >= 0) {
      track2measurements_dist->at(track_ind) = association_mat[track_ind_loc][measurement_ind_loc];
  ...
  for (size_t i = 0; i < unassigned_fusion_tracks->size(); ++i) {
    ...
    for (size_t j = 1; j < association_mat[track_ind_loc].size(); ++j) {
      if (track2measurements_dist->at(track_ind) > association_mat[track_ind_loc][j]) {
        track2measurements_dist->at(track_ind) = association_mat[track_ind_loc][j];
      }
    }
  }
...
std::string PbfHmTrackObjectMatcher::name() const { return "PbfHmTrackObjectMatcher"; }
...
void PbfHmTrackObjectMatcher::ComputeAssociationMat(
  ...
  for (size_t i = 0; i < unassigned_fusion_tracks.size(); ++i) {
    ...
    for (size_t j = 0; j < unassigned_sensor_objects.size(); ++j) {
      ...
      const std::shared_ptr<PbfSensorObject>& sensor_object = sensor_objects[sensor_idx];
...
bool PbfHmTrackObjectMatcher::HmAssign(const std::vector<std::vector<double>>& association_mat,
  ...
  for (size_t i = 0; i < fusion_components.size(); ++i) {
    if (fusion_components[i].empty() || sensor_components[i].empty()) {
      continue;
    } else if (fusion_components[i].size() == 1 && sensor_components[i].size() == 1) {
    ...
    for (size_t j = 0; j < fusion_components[i].size(); ++j) {
      ...
      for (size_t k = 0; k < sensor_components[i].size(); ++k) {
        if (j == 0) { sensor_l2g[k] = sensor_components[i][k]; }
        loc_mat[j][k] = association_mat[fusion_components[i][j]][sensor_components[i][k]];
      }
    }
  ...
  for (size_t i = 0; i < unassigned_fusion_tracks->size(); ++i) {
    if ((*unassigned_fusion_tracks)[i] >= 0) {
      (*unassigned_fusion_tracks)[unassigned_fusion_num++] = (*unassigned_fusion_tracks)[i];
    }
  }
  ...
  for (size_t i = 0; i < unassigned_sensor_objects->size(); ++i) {
    if ((*unassigned_sensor_objects)[i] >= 0) {
      (*unassigned_sensor_objects)[unassigned_sensor_num++] = (*unassigned_sensor_objects)[i];
    }
  }
...
void PbfHmTrackObjectMatcher::ComputeConnectedComponents(
  ...
  int no_obj   = 0;
  if (no_track != 0) { no_obj = association_mat[0].size(); }
