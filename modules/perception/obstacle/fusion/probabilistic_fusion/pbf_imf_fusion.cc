...
void PbfIMFFusion::Initialize(const std::shared_ptr<PbfSensorObject> new_object) {
  ...
  ADEBUG << "PBFIMF pbf imf filter initial state is " << posteriori_state_(0) << " "
         << posteriori_state_(1) << " " << posteriori_state_(2) << " " << posteriori_state_(3)
         << " for object info: " << new_object->object->ToString();
  std::cerr << "next track\n" << std::endl;
  std::cerr << "PBFIMF pbf imf initial uncertainty set " << new_object->object->state_uncertainty
            << std::endl;
...
void PbfIMFFusion::UpdateWithObject(const std::shared_ptr<PbfSensorObject> new_object,
                                    const double                           time_diff) {
  if (new_object == nullptr) {
  ...
  ADEBUG << "\nPBFIMF: previous state: " << belief_anchor_point_(0) << " "
         << belief_anchor_point_(1) << " " << belief_velocity_(0) << " " << belief_velocity_(1);
  ADEBUG << "PBFIMF: previous timestamp: " << std::fixed << std::setprecision(15)
         << last_fuse_timestamp;
  ADEBUG << "PBFIMF: new object information: " << new_object->object->ToString();
  ADEBUG << "PBFIMF: new object timestamp: " << std::fixed << std::setprecision(15)
  ...
  omega_matrix_               = omega_matrix_.inverse().eval();
  ADEBUG << "PBFIMF:predicted state " << priori_state_(0) << " " << priori_state_(1) << " "
  ...
  if (new_object->sensor_type == SensorType::CAMERA) {
    ...
    sensor_transition_matrix = &(CameraFrameSupplement::state_vars.trans_matrix);
  ...
  if (sensor_object != nullptr) {
    ...
    if (!ObtainSensorPrediction(sensor_object->object, timestamp_sensor_prev,
                                *sensor_processor_noise, *sensor_transition_matrix,
                                &state_sensor_prev, &cov_sensor_prev)) {
    ...
    if (!ObtainSensorPrediction(new_object->object, timestamp_sensor, *sensor_processor_noise,
    ...
    ADEBUG << "PBFIMF: state sensor " << state_sensor(0) << " " << state_sensor(1);
    ADEBUG << "PBFIMF: state sensor prev " << state_sensor_prev(0) << " " << state_sensor(1);

    std::cerr << "PBFIMF: cov sensor inverse " << cov_sensor_inverse << std::endl;
    std::cerr << "PBFIMF: cov sensor prev inverse " << cov_sensor_prev_inverse << std::endl;
    ...
    std::cerr << "PBFIMF: cov sensor delta " << (cov_sensor_inverse - cov_sensor_prev_inverse)
              << std::endl;
    std::cerr << "PBFIMF: time diff is\n " << time_diff << std::endl;
    omega_matrix_ = omega_matrix_ + (cov_sensor_inverse - cov_sensor_prev_inverse);
    std::cerr << "PBFIMF: omega matrix after update " << omega_matrix_ << std::endl;
    std::cerr << "PBFIMF: initial updated covariance matrix " << omega_matrix_.inverse()
              << std::endl;
    ...
    xi_ = xi_ + (cov_sensor_inverse * state_sensor - cov_sensor_prev_inverse * state_sensor_prev);
    ...
    AWARN << "Sensor data deprecation in Fusion, should not see this many times";
  ...
  if (std::abs(posteriori_state_(2)) > 5 || std::abs(posteriori_state_(3)) > 5) {
  ...
  ADEBUG << "PBFIMF: new state is " << belief_anchor_point_(0) << " " << belief_anchor_point_(1)
         << " " << belief_velocity_(0) << " " << belief_velocity_(1) << "\n";
  CacheSensorObjects(new_object);
}
...
bool PbfIMFFusion::ObtainSensorPrediction(std::shared_ptr<Object> object,
  ...
  (*cov_pre) = trans_matrix_time * cov * trans_matrix_time.transpose() + process_noise;
...
void PbfIMFFusion::GetState(Eigen::Vector3d* anchor_point, Eigen::Vector3d* velocity) {
...
void PbfIMFFusion::GetUncertainty(Eigen::Matrix3d* position_uncertainty,
  ...
  *velocity_uncertainty << 0.01, 0.0, 0.0, 0.0, 0.01, 0.0, 0.0, 0.0, 0.01;
  (*position_uncertainty).topLeftCorner(2, 2) = omega_matrix_.inverse().topLeftCorner(2, 2);
  (*velocity_uncertainty).topLeftCorner(2, 2) = omega_matrix_.inverse().block<2, 2>(2, 2);
}

void PbfIMFFusion::CacheSensorObjects(const std::shared_ptr<PbfSensorObject> new_object) {
...
std::shared_ptr<PbfSensorObject> PbfIMFFusion::GetSensorLatestCache(const SensorType type) {
  auto it = cached_sensor_objects_.find(type);
  if (it != cached_sensor_objects_.end()) {
    const auto& objects = it->second;
    if (!objects.empty()) { return objects.back(); }
  }
  return nullptr;
}

void PbfIMFFusion::SetState(const Eigen::Vector3d& anchor_point, const Eigen::Vector3d& velocity) {
  belief_anchor_point_ = anchor_point;
  belief_velocity_     = velocity;
}
}  // namespace perception
