...
void PbfTrack::UpdateWithSensorObject(std::shared_ptr<PbfSensorObject> obj, double match_dist) {
...
void PbfTrack::UpdateWithoutSensorObject(const SensorType&  sensor_type,
  ...
  UpdateMeasurementsLifeWithoutMeasurement(&lidar_objects_, sensor_id, timestamp,
                                           s_max_lidar_invisible_period_, &invisible_in_lidar_);
  UpdateMeasurementsLifeWithoutMeasurement(&radar_objects_, sensor_id, timestamp,
                                           s_max_radar_invisible_period_, &invisible_in_radar_);
  UpdateMeasurementsLifeWithoutMeasurement(&camera_objects_, sensor_id, timestamp,
                                           s_max_camera_invisible_period_, &invisible_in_camera_);

  is_dead_ = (lidar_objects_.empty() && radar_objects_.empty() && camera_objects_.empty());

  if (!is_dead_) {
    double time_diff = timestamp - fused_timestamp_;
    motion_fusion_->UpdateWithoutObject(time_diff);
    if (FLAGS_use_navigation_mode) { PerformMotionCompensation(fused_object_, timestamp); }
...
std::shared_ptr<PbfSensorObject> PbfTrack::GetFusedObject() { return fused_object_; }

double PbfTrack::GetFusedTimestamp() const { return fused_timestamp_; }

std::shared_ptr<PbfSensorObject> PbfTrack::GetLidarObject(const std::string& sensor_id) {
  std::shared_ptr<PbfSensorObject> obj = nullptr;
  auto                             it  = lidar_objects_.find(sensor_id);
  if (it != lidar_objects_.end()) { obj = it->second; }
  return obj;
}

std::shared_ptr<PbfSensorObject> PbfTrack::GetRadarObject(const std::string& sensor_id) {
  std::shared_ptr<PbfSensorObject> obj = nullptr;
  auto                             it  = radar_objects_.find(sensor_id);
  if (it != radar_objects_.end()) { obj = it->second; }
  return obj;
}

std::shared_ptr<PbfSensorObject> PbfTrack::GetCameraObject(const std::string& sensor_id) {
  std::shared_ptr<PbfSensorObject> obj = nullptr;
  auto                             it  = camera_objects_.find(sensor_id);
  if (it != camera_objects_.end()) { obj = it->second; }
  return obj;
}
...
void PbfTrack::PerformMotionFusionAsync(std::shared_ptr<PbfSensorObject> obj) {
  ...
  AINFO << "obj timestamp " << std::fixed << std::setprecision(15) << obj->timestamp;
  AINFO << "current fuse ts is " << std::fixed << std::setprecision(15) << current_time;

  // for low cost, we only consider radar and camera fusion for now
  if (is_camera(sensor_type) || is_radar(sensor_type)) {
    if (is_camera(sensor_type)) { AINFO << "camera sensor in async fusion"; }
    if (is_radar(sensor_type)) { AINFO << "radar sensor in async fusion"; }
    ...
    ADEBUG << "fused object in pbftrack is " << fused_object_->object->ToString();
  ...
  if (FLAGS_use_navigation_mode) {
    if (is_camera(sensor_type) || is_radar(sensor_type)) {
      if (motion_fusion_->Initialized()) {
        ...
        if (is_camera(sensor_type)) { motion_fusion_->Initialize(obj); }
      }
    }
  } else {
    if (is_lidar(sensor_type)) {
      fused_object_->clone(*obj);
      if (motion_fusion_->Initialized() && (lidar_object != nullptr || radar_object != nullptr)) {
        motion_fusion_->UpdateWithObject(obj, time_diff);
        Eigen::Vector3d anchor_point;
        Eigen::Vector3d velocity;
        motion_fusion_->GetState(&anchor_point, &velocity);
...
void PbfTrack::PerformMotionCompensation(std::shared_ptr<PbfSensorObject> obj, double timestamp) {
...
bool PbfTrack::AbleToPublish() {
  if (FLAGS_use_navigation_mode && !camera_objects_.empty()) { return true; }
  ...
  if (s_publish_if_has_lidar_ && !invisible_in_lidar_ && !lidar_objects_.empty()) { return true; }
...
std::shared_ptr<PbfSensorObject> PbfTrack::GetSensorObject(const SensorType&  sensor_type,
                                                           const std::string& sensor_id) {
  if (is_lidar(sensor_type)) {
