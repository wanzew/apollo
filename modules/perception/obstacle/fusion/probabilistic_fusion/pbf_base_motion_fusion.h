...
class PbfBaseMotionFusion {
 public:
  ...
  // @return nothing
  virtual void Initialize(const Eigen::Vector3d& anchor_point, const Eigen::Vector3d& velocity) = 0;

  // @brief initialize the state of the filter
  // @params[IN] new_object: initial object for filtering
  // @return nothing
  virtual void Initialize(const std::shared_ptr<PbfSensorObject> new_object) = 0;
  ...
  // @return nothing
  virtual void
  Predict(Eigen::Vector3d* anchor_point, Eigen::Vector3d* velocity, const double time_diff) = 0;
  ...
  // @return nothing
  virtual void SetState(const Eigen::Vector3d& anchor_point, const Eigen::Vector3d& velocity) = 0;
