...
TEST(ContiRadarUtilTest, conti_radar_util_test) {
  ...
  state = ContiRadarUtil::IsFp(contiobs, conti_params, delay_frames, tracking_times);
  EXPECT_EQ(state, true);
  contiobs.set_obstacle_class(static_cast<int>(ContiObjectType::CONTI_PEDESTRIAN));
  state = ContiRadarUtil::IsFp(contiobs, conti_params, delay_frames, tracking_times);
  EXPECT_EQ(state, true);
  contiobs.set_obstacle_class(static_cast<int>(ContiObjectType::CONTI_MOTOCYCLE));
  state = ContiRadarUtil::IsFp(contiobs, conti_params, delay_frames, tracking_times);
  EXPECT_EQ(state, true);
