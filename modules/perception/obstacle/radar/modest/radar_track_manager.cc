...
void RadarTrackManager::AssignTrackObsIdMatch(const SensorObjects&              radar_obs,
                                              std::vector<std::pair<int, int>>* assignment,
                                              std::vector<int>*                 unassigned_track,
                                              std::vector<int>*                 unassigned_obs) {
  ...
  for (size_t i = 0; i < obs_tracks_.size(); i++) {
    ...
    for (size_t j = 0; j < radar_obs.objects.size(); j++) {
      double distance =
          DistanceBetweenObs(*obs, timestamp_track, *(radar_obs.objects[j]), timestamp_obs);
      if (obs->track_id == radar_obs.objects[j]->track_id && distance < RADAR_TRACK_THRES) {
  ...
  for (size_t i = 0; i < track_used.size(); i++) {
    if (track_used[i] == false) { unassigned_track->at(unassigned_track_num++) = i; }
  }
  ...
  for (size_t i = 0; i < obs_used.size(); i++) {
    if (obs_used[i] == false) { unassigned_obs->at(unassigned_obs_num++) = i; }
  }
  unassigned_obs->resize(unassigned_obs_num);
}

void RadarTrackManager::UpdateAssignedTrack(const SensorObjects&                    radar_obs,
                                            const std::vector<std::pair<int, int>>& assignment) {
  for (size_t i = 0; i < assignment.size(); i++) {
    obs_tracks_[assignment[i].first].UpdataObsRadar(radar_obs.objects[assignment[i].second],
                                                    radar_obs.timestamp);
  }
}

void RadarTrackManager::UpdateUnassignedTrack(const double            timestamp,
...
void RadarTrackManager::DeleteLostTrack() {
  int track_num = 0;
  for (size_t i = 0; i < obs_tracks_.size(); i++) {
    if (obs_tracks_[i].GetObsRadar() != nullptr) { obs_tracks_[track_num++] = obs_tracks_[i]; }
  }
...
void RadarTrackManager::CreateNewTrack(const SensorObjects&    radar_obs,
  ...
  for (size_t i = 0; i < unassigned_obs.size(); i++) {
    obs_tracks_.push_back(RadarTrack(*(radar_obs.objects[unassigned_obs[i]]), radar_obs.timestamp));
  }
}
