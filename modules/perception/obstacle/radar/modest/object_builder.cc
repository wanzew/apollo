...
bool ObjectBuilder::Build(const ContiRadar&      raw_obstacles,
  ...
  for (int i = 0; i < raw_obstacles.contiobs_size(); ++i) {
    ...
    if (current_con_ids[obstacle_id] <= delay_frames_) { object_ptr->is_background = true; }
    int tracking_times = current_con_ids[obstacle_id];
    if (use_fp_filter_ && ContiRadarUtil::IsFp(raw_obstacles.contiobs(i), conti_params_,
    ...
    object_ptr->position_uncertainty = radar_pose.topLeftCorner(3, 3) * dist_rms *
                                       dist_rms.transpose() *
                                       radar_pose.topLeftCorner(3, 3).transpose();
    object_ptr->velocity_uncertainty = radar_pose.topLeftCorner(3, 3) * vel_rms *
                                       vel_rms.transpose() *
                                       radar_pose.topLeftCorner(3, 3).transpose();
    ...
    RadarUtil::MockRadarPolygon(point, object_ptr->length, object_ptr->width, theta,
                                &(object_ptr->polygon));
