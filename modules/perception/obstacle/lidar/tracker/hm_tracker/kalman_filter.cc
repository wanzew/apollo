...
Eigen::Matrix3d KalmanFilter::s_propagation_noise_           = 10 * Eigen::Matrix3d::Identity();
...
bool KalmanFilter::SetAssociationScoreMaximum(const double association_score_maximum) {
  if (association_score_maximum > 0) {
    s_association_score_maximum_ = association_score_maximum;
    AINFO << "association score maximum of KalmanFilter is " << s_association_score_maximum_;
    return true;
  }
...
bool KalmanFilter::SetBreakdownThresholdMaximum(const double breakdown_threshold_maximum) {
  if (breakdown_threshold_maximum > 0) {
    s_breakdown_threshold_maximum_ = breakdown_threshold_maximum;
    AINFO << "breakdown threshold maximum of KalmanFilter is " << s_breakdown_threshold_maximum_;
    return true;
  }
...
bool KalmanFilter::InitParams(const double measurement_noise,
  ...
  AINFO << "initial velocity noise of KalmanFilter is " << s_initial_velocity_noise_;
...
KalmanFilter::KalmanFilter() {
  ...
  s_measurement_cached_history_size_maximum_ = s_measurement_cached_history_size_minimum_;
  velocity_covariance_ = s_initial_velocity_noise_ * Eigen::Matrix3d::Identity();
  // states
  update_quality_           = 1.0;
...
void KalmanFilter::UpdateWithObject(const std::shared_ptr<TrackedObject>& new_object,
  ...
  Eigen::Vector3f measured_velocity = ComputeMeasuredVelocity(new_object, old_object, time_diff);

  // debug: compute measured acceleration
  Eigen::Vector3f measured_acceleration = ComputeMeasuredAcceleration(measured_velocity, time_diff);
...
void KalmanFilter::GetState(Eigen::Vector3f* anchor_point, Eigen::Vector3f* velocity) {
...
Eigen::VectorXf
KalmanFilter::ComputeMeasuredVelocity(const std::shared_ptr<TrackedObject>& new_object,
                                      const std::shared_ptr<TrackedObject>& old_object,
                                      const double                          time_diff) {
...
Eigen::VectorXf
KalmanFilter::ComputeMeasuredAnchorPointVelocity(const std::shared_ptr<TrackedObject>& new_object,
                                                 const std::shared_ptr<TrackedObject>& old_object,
                                                 const double                          time_diff) {
...
Eigen::VectorXf
KalmanFilter::ComputeMeasuredBboxCenterVelocity(const std::shared_ptr<TrackedObject>& new_object,
                                                const std::shared_ptr<TrackedObject>& old_object,
                                                const double                          time_diff) {
  ...
  ComputeBboxSizeCenter<pcl_util::Point>(new_object->object_ptr->cloud, old_dir, &new_size,
                                         &new_center);
  ...
  Eigen::Vector3f measured_bbox_center_velocity = measured_bbox_center_velocity_with_old_dir;
  Eigen::Vector3f project_dir = new_object->anchor_point - old_object->anchor_point;
...
Eigen::VectorXf
KalmanFilter::ComputeMeasuredBboxCornerVelocity(const std::shared_ptr<TrackedObject>& new_object,
                                                const std::shared_ptr<TrackedObject>& old_object,
                                                const double                          time_diff) {
  ...
  Eigen::Vector3d old_bbox_corner =
      old_center + old_dir * old_size(0) * 0.5 + ortho_old_dir * old_size(1) * 0.5;
  Eigen::Vector3d new_bbox_corner =
      new_center + old_dir * new_size(0) * 0.5 + ortho_old_dir * new_size(1) * 0.5;
  old_bbox_corner_list[0] = old_bbox_corner;
  new_bbox_corner_list[0] = new_bbox_corner;
  old_bbox_corner = old_center - old_dir * old_size(0) * 0.5 + ortho_old_dir * old_size(1) * 0.5;
  new_bbox_corner = new_center - old_dir * new_size(0) * 0.5 + ortho_old_dir * new_size(1) * 0.5;
  ...
  Eigen::Vector3f min_bbox_corner_velocity_on_project_dir = Eigen::Vector3f(100, 100, 0);
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    float bbox_corner_velocity_project_dir_inner_product =
        bbox_corner_velocity(0) * project_dir(0) + bbox_corner_velocity(1) * project_dir(1);
    ...
    float bbox_corner_velocity_norm_on_project_dir =
        bbox_corner_velocity.head(2).norm() * bbox_corner_velocity_project_dir_angle_cos;
    ...
    if (bbox_corner_velocity_on_project_dir_gain.norm() <
      ...
      min_bbox_corner_velocity_on_project_dir = bbox_corner_velocity_on_project_dir;
      min_bbox_corner_velocity_on_project_dir_gain_norm =
...
Eigen::Vector3f
KalmanFilter::SelectMeasuredVelocity(const std::vector<Eigen::Vector3f>& candidates) {
...
Eigen::Vector3f KalmanFilter::SelectMeasuredVelocityAccordingMotionConsistency(
  ...
  Eigen::Vector3f measured_velocity_gain = measured_velocity - belief_velocity_.cast<float>();
  for (size_t i = 1; i < candidates.size(); ++i) {
    Eigen::Vector3f candidate_velocity_gain = candidates[i] - belief_velocity_.cast<float>();
...
void KalmanFilter::UpdateVelocity(const Eigen::VectorXf& measured_anchor_point,
  ...
  Eigen::Vector3d measured_anchor_point_d = measured_anchor_point.cast<double>();
  ...
  velocity_covariance_ = (Eigen::Matrix3d::Identity() - mat_k * mat_c) * velocity_covariance_;
}

void KalmanFilter::ComputeUpdateQuality(const std::shared_ptr<TrackedObject>& new_object,
                                        const std::shared_ptr<TrackedObject>& old_object) {
  ...
  if (update_quality_according_association_score > update_quality_according_point_num_change) {
...
float KalmanFilter::ComputeUpdateQualityAccordingAssociationScore(
  ...
  if (s_association_score_maximum_ == 0) { return update_quality; }
...
float KalmanFilter::ComputeUpdateQualityAccordingPointNumChange(
  ...
  if (new_pt_num <= 0 || old_pt_num <= 0) { return 0; }
  float update_quality = 1 - std::abs(new_pt_num - old_pt_num) / std::max(new_pt_num, old_pt_num);
  return update_quality;
}
...
Eigen::Vector3f KalmanFilter::ComputeMeasuredAcceleration(const Eigen::Vector3f& measured_velocity,
                                                          const double           time_diff) {
  if (history_measured_velocity_.size() < 3) { return Eigen::Vector3f::Zero(); }
  ...
  for (size_t i = history_index + 1; i < history_measured_velocity_.size(); ++i) {
...
void KalmanFilter::UpdateAcceleration(const Eigen::VectorXf& measured_acceleration) {
  // Compute kalman gain
  Eigen::Matrix3d mat_c = Eigen::Matrix3d::Identity();
