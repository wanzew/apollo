...
bool ObjectTrack::SetTrackCachedHistorySizeMaximum(const int track_cached_history_size_maximum) {
...
bool ObjectTrack::SetSpeedNoiseMaximum(const double speed_noise_maximum) {
  if (speed_noise_maximum > 0) {
    ...
    AINFO << "speed noise maximum of object track is " << s_speed_noise_maximum_;
    return true;
  }
...
bool ObjectTrack::SetAccelerationNoiseMaximum(const double acceleration_noise_maximum) {
...
void ObjectTrack::UpdateWithObject(std::shared_ptr<TrackedObject>* new_object,
  ...
  if (history_size >= s_track_cached_history_size_maximum_) { history_objects_.pop_front(); }
...
void ObjectTrack::SmoothTrackVelocity(const std::shared_ptr<TrackedObject>& new_object,
                                      const double                          time_diff) {
  ...
  is_static_hypothesis_ = CheckTrackStaticHypothesis(new_object->object_ptr, time_diff);
...
bool ObjectTrack::CheckTrackStaticHypothesis(const std::shared_ptr<Object>& new_object,
                                             const double                   time_diff) {
  ...
  if (velocity_is_noise) { return true; }
  ...
  if (velocity_is_small && is_velocity_angle_change) { return true; }
  return false;
}
...
bool ObjectTrack::CheckTrackStaticHypothesisByVelocityAngleChange(
  ...
  Eigen::Vector3f previous_velocity     = history_objects_[history_objects_.size() - 1]->velocity;
  ...
  if (fabs(velocity_angle_change) > M_PI / 4.0) { return true; }
  return false;
}
...
bool ObjectTrackSet::SetTrackConsecutiveInvisibleMaximum(
  ...
  if (track_consecutive_invisible_maximum >= 0) {
    s_track_consecutive_invisible_maximum_ = track_consecutive_invisible_maximum;
...
bool ObjectTrackSet::SetTrackVisibleRatioMinimum(const float track_visible_ratio_minimum) {
...
int ObjectTrackSet::RemoveLostTracks() {
  ...
  for (size_t i = 0; i < tracks_.size(); ++i) {
    ...
    float track_visible_ratio = tracks_[i]->total_visible_count_ * 1.0f / tracks_[i]->age_;
    ...
    int track_consecutive_invisible_count = tracks_[i]->consecutive_invisible_count_;
    if (track_consecutive_invisible_count > s_track_consecutive_invisible_maximum_) continue;
    // C. update
    if (i == track_num) {
      track_num++;
    } else {
