...
bool TrackObjectDistance::SetLocationDistanceWeight(const float location_distance_weight) {
  if (location_distance_weight >= 0) {
    s_location_distance_weight_ = location_distance_weight;
    AINFO << "location distance weight of TrackObjectDistance is " << s_location_distance_weight_;
    return true;
  }
...
bool TrackObjectDistance::SetDirectionDistanceWeight(const float direction_distance_weight) {
  if (direction_distance_weight >= 0) {
    s_direction_distance_weight_ = direction_distance_weight;
    AINFO << "direction distance weight of TrackObjectDistance is " << s_direction_distance_weight_;
    return true;
  }
...
bool TrackObjectDistance::SetBboxSizeDistanceWeight(const float bbox_size_distance_weight) {
  if (bbox_size_distance_weight >= 0) {
    s_bbox_size_distance_weight_ = bbox_size_distance_weight;
    AINFO << "bbox size distance weight of TrackObjectDistance is " << s_bbox_size_distance_weight_;
    return true;
  }
...
bool TrackObjectDistance::SetPointNumDistanceWeight(const float point_num_distance_weight) {
  if (point_num_distance_weight >= 0) {
    s_point_num_distance_weight_ = point_num_distance_weight;
    AINFO << "point num distance weight of TrackObjectDistance is " << s_point_num_distance_weight_;
    return true;
  }
...
bool TrackObjectDistance::SetHistogramDistanceWeight(const float histogram_distance_weight) {
  if (histogram_distance_weight >= 0) {
    s_histogram_distance_weight_ = histogram_distance_weight;
    AINFO << "histogram distance weight of TrackObjectDistance is " << s_histogram_distance_weight_;
    return true;
  }
...
float TrackObjectDistance::ComputeDistance(ObjectTrackPtr                        track,
...
float TrackObjectDistance::ComputeLocationDistance(
  ...
  Eigen::Vector2f measurement_predict_diff = measured_anchor_point - predicted_anchor_point;
  ...
  if (track_speed > 2) {
    ...
    float motion_dir_distance = track_motion_dir(0) * measurement_predict_diff(0) +
...
float TrackObjectDistance::ComputeDirectionDistance(
  ...
  if (!anchor_point_shift.head(2).isZero() && !predicted_track_motion.head(2).isZero()) {
...
float TrackObjectDistance::ComputeBboxSizeDistance(
  ...
  if (bbox_dir_close) {
    float diff_1 =
        fabs(old_bbox_size(0) - new_bbox_size(0)) / std::max(old_bbox_size(0), new_bbox_size(0));
    float diff_2 =
        fabs(old_bbox_size(1) - new_bbox_size(1)) / std::max(old_bbox_size(1), new_bbox_size(1));
    size_distance = std::min(diff_1, diff_2);
  } else {
    float diff_1 =
        fabs(old_bbox_size(0) - new_bbox_size(1)) / std::max(old_bbox_size(0), new_bbox_size(1));
    float diff_2 =
        fabs(old_bbox_size(1) - new_bbox_size(0)) / std::max(old_bbox_size(1), new_bbox_size(0));
    size_distance = std::min(diff_1, diff_2);
  }
...
float TrackObjectDistance::ComputePointNumDistance(
  ...
  float point_num_distance = std::abs(old_point_number - new_point_number) * 1.0f /
...
float TrackObjectDistance::ComputeHistogramDistance(
  ...
  for (size_t i = 0; i < old_object_shape_features.size(); ++i) {
    histogram_distance += std::fabs(old_object_shape_features[i] - new_object_shape_features[i]);
  }
