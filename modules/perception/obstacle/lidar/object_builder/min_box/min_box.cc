...
double MinBoxObjectBuilder::ComputeAreaAlongOneEdge(std::shared_ptr<Object> obj,
...
void MinBoxObjectBuilder::ReconstructPolygon(const Eigen::Vector3d&  ref_ct,
  ...
  if (obj->polygon.points.size() <= 0) { return; }
  ...
  for (size_t i = min_point_index, count = 0; count < obj->polygon.points.size();
    ...
    if (j != min_point_index && j != max_point_index) {
      ...
      if (line[0] * ray[1] - ray[0] * line[1] < EPSILON) {
        ...
        total_len += dist;
        if (dist - max_dis > EPSILON) { max_dis = dist; }
      } else {
        // outline
        has_out = true;
      }
  ...
  for (size_t i = min_point_index; count < obj->polygon.points.size();
    ...
    p_j[2] = obj->polygon.points[j].z;
    double dist =
        sqrt((p_x[0] - p_j[0]) * (p_x[0] - p_j[0]) + (p_x[1] - p_j[1]) * (p_x[1] - p_j[1]));
...
void MinBoxObjectBuilder::ComputePolygon2dxy(std::shared_ptr<Object> obj) {
  ...
  for (idx = 2; idx < cloud->points.size(); ++idx) {
    ...
    if ((diff_x * tdiff_y - tdiff_x * diff_y) > min_eps) { break; }
  }
