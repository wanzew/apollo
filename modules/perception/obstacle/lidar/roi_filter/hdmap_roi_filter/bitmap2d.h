...
class Bitmap2D {
 public:
  ...
  Bitmap2D(const Eigen::Vector2d& min_p,
  ...
  void Set(const uint64_t x_id, const uint64_t min_y_id, const uint64_t max_y_id);
 ...
 private:
  ...
  inline void SetUint64RangeBits(const size_t head, const size_t tail, uint64_t* block);
