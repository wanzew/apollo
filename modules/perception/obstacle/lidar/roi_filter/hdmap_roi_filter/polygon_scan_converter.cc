...
void PolygonScanConverter::Init(const DirectionMajor major_dir,
  ...
  scans_size_ = static_cast<uint64_t>(std::abs(valid_x_range.second - valid_x_range.first) / step);
  step_       = step;
}
...
void PolygonScanConverter::BuildEdgeTable() {
  ...
  for (size_t i = 0; i < segments_.size(); ++i) {
    ...
    if (ConvertSegmentToEdge(i, &out_edge)) { edges.push_back(out_edge); }
  }
  ...
  for (size_t i = 0; i < edges.size(); ++i) {
    ...
    if (x_id >= 0) {
      ...
      if (active_edge.MoveUp(0.0 - active_edge.x)) { active_edge_table_.push_back(active_edge); }
    }
  }
}

bool PolygonScanConverter::ConvertSegmentToEdge(const size_t          seg_id,
                                                std::pair<int, Edge>* out_edge) {
