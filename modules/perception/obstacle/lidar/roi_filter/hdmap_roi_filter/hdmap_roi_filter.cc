...
bool HdmapROIFilter::Filter(pcl_util::PointCloudPtr cloud,
  ...
  if (roi_filter_options.hdmap == nullptr || roi_filter_options.velodyne_trans == nullptr) {
    return false;
  }
...
bool HdmapROIFilter::FilterWithPolygonMask(pcl_util::PointCloudPtr         cloud,
...
MajorDirection
HdmapROIFilter::GetMajorDirection(const std::vector<PolygonType>&             map_polygons,
...
bool HdmapROIFilter::Bitmap2dFilter(const pcl::PointCloud<pcl_util::Point>::ConstPtr in_cloud_ptr,
  ...
  for (size_t i = 0; i < in_cloud_ptr->size(); ++i) {
    ...
    if (bitmap.IsExist(p) && bitmap.Check(p)) { roi_indices_ptr->indices.push_back(i); }
  }
  return true;
}

void HdmapROIFilter::MergeRoadBoundariesToPolygons(const std::vector<RoadBoundary>& road_boundaries,
                                                   std::vector<PolygonDType>*       polygons) {
...
void HdmapROIFilter::MergeHdmapStructToPolygons(HdmapStructConstPtr        hdmap_struct_ptr,
                                                std::vector<PolygonDType>* polygons) {
...
bool HdmapROIFilter::Init() {
  if (!GetProtoFromFile(FLAGS_hdmap_roi_filter_config, &config_)) {
    AERROR << "Cannot get config proto from file: " << FLAGS_hdmap_roi_filter_config;
    return false;
  }
...
void HdmapROIFilter::TransformFrame(pcl_util::PointCloudConstPtr     cloud,
