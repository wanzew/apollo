...
void GetValidXRange(const typename PolygonScanConverter::Polygon& polygon,
  ...
  valid_x_range->first  = std::max(polygon_min_pt[major_dir], bitmap_min_pt[major_dir]);
  valid_x_range->second = std::min(polygon_max_pt[major_dir], bitmap_max_pt[major_dir]);

  // For numerical stability
  valid_x_range->first =
      (static_cast<int>((valid_x_range->first - bitmap_min_pt[major_dir]) / major_dir_grid_size) +
       0.5) *
          major_dir_grid_size +
...
void DrawPolygonInBitmap(const typename PolygonScanConverter::Polygon& polygon,
  ...
  for (size_t i = 0; i < scans_intervals.size(); x += major_dir_grid_size, ++i) {
    for (const auto& scan_interval : scans_intervals[i]) {
      ...
      valid_y_range.first =
          std::max(bitmap_min_pt[op_major_dir], scan_interval.first - extend_dist);

      valid_y_range.second =
          std::min(bitmap_max_pt[op_major_dir], scan_interval.second + extend_dist);
...
void DrawPolygonInBitmap(const std::vector<typename PolygonScanConverter::Polygon>& polygons,
                         const double                                               extend_dist,
                         Bitmap2D*                                                  bitmap) {
