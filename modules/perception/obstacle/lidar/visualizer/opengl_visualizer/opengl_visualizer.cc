...
void OpenglVisualizer::Render(const FrameContent& content) {
  opengl_vs_->SetCameraPara(
      Eigen::Vector3d(camera_center_world_.x, camera_center_world_.y, camera_center_world_.z),
...
void OpenglVisualizer::UpdateCameraSystem(FrameContent* content) {
  ...
  TransformPoint<pcl_util::Point>(camera_center_velodyne_, pose_v2w, &camera_center_world_);

  TransformPoint<pcl_util::Point>(view_point_velodyne_, pose_v2w, &view_point_world_);
