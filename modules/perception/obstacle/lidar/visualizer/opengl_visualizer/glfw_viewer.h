...
class GLFWViewer {
 public:
  ...
  void SetFrameContent(const FrameContent& frame_content) { frame_content_ = frame_content; }
  void Spin();
  void SpinOnce();
  void Close();
  ...
  void SetCameraPara(Eigen::Vector3d i_position,
 ...
 private:
  ...
  void DrawObstacles();
  void DrawObstacle(const std::shared_ptr<Object> obj,
                    bool                          show_cloud,
                    bool                          show_polygon,
                    bool                          show_velocity,
                    bool                          show_direction);
  void DrawOffsetVolumn(Eigen::Vector3d* polygon_points, double h, int polygon_size);
 ...
 private:
  ...
  enum class OBJ_Type { CIRCIE = 0, CUBE = 1, CLOUD = 2, POLYGON = 3, NUM_VAO_TYPE = 4 };

  enum class VBO_Type { VBO_VERTICES = 0, VBO_COLORS = 1, VBO_ELEMENTS = 2, NUM_VBO_TYPE = 3 };
  ...
  GLuint  cloud_VBO_buf_ids_[kCloud_VAO_Num_][static_cast<int>(VBO_Type::NUM_VBO_TYPE)];
  ...
  GLuint           circle_VBO_buf_ids_[kCircle_VAO_Num_][static_cast<int>(VBO_Type::NUM_VBO_TYPE)];
};
