...
void Camera::SetUpDirection(Eigen::Vector3d world_vec) {
  ...
  Eigen::Vector3d up_vec_proj_in_cam_frame(up_vec_cam_frame(0), up_vec_cam_frame(1), 0);
  ...
  align_axis.normalize();  // the axis for Eigen::AngleAxisd must be a unit vector
  double          align_ang     = acos(cam_y_axis.dot(up_vec_proj_in_cam_frame));
  Eigen::Matrix3d align_rot_mat = Eigen::AngleAxisd(align_ang, align_axis).toRotationMatrix();
...
Eigen::Matrix4d Camera::GetProjectionMat() {
  ...
  GLdouble aspect_ratio =
      static_cast<GLdouble>(screen_width_) / static_cast<GLdouble>(screen_height_);
...
Eigen::Vector3d Camera::PointOnScreen(Eigen::Vector3d point) {
  ...
  viewport[3] = -screen_height_;  // origin of opengl frame is at left-bottom corner
  ...
  GLdouble proj_mat[16] = {proj_mat_(0, 0), proj_mat_(1, 0), proj_mat_(2, 0), proj_mat_(3, 0),
  ...
  gluProject(point(0), point(1), point(2), view_mat, proj_mat, viewport, &x, &y, &z);
