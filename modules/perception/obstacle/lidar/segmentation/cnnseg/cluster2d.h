...
struct Obstacle {
  ...
  Obstacle()
      : score(0.0)
      , height(-5.0)
      , meta_type(MetaType::META_UNKNOWN) {
...
class Cluster2D {
 public:
  ...
  void Cluster(const caffe::Blob<float>&                         category_pt_blob,
    ...
    for (int row = 0; row < rows_; ++row) {
      for (int col = 0; col < cols_; ++col) {
        Node* node = &nodes[row][col];
        if (node->is_object && node->traversed == 0) { Traverse(node); }
      }
    }
    for (int row = 0; row < rows_; ++row) {
      for (int col = 0; col < cols_; ++col) {
        ...
        for (int row2 = row - 1; row2 <= row + 1; ++row2) {
          for (int col2 = col - 1; col2 <= col + 1; ++col2) {
            if ((row2 == row || col2 == col) && IsValidRowCol(row2, col2)) {
              ...
              if (node2->is_center) { DisjointSetUnion(node, node2); }
            }
          }
        }
      }
  ...
  void Classify(const caffe::Blob<float>& classify_pt_blob) {
    ...
    for (size_t obs_id = 0; obs_id < obstacles_.size(); obs_id++) {
      ...
      for (int k = 0; k < num_classes; k++) {
        ...
        if (obs->meta_type_probs[k] > obs->meta_type_probs[meta_type_id]) { meta_type_id = k; }
      }
  ...
  void GetObjects(const float                           confidence_thresh,
    ...
    for (size_t obstacle_id = 0; obstacle_id < obstacles_.size(); obstacle_id++) {
      Obstacle* obs = &obstacles_[obstacle_id];
      if (static_cast<int>(obs->cloud->size()) < min_pts_num) { continue; }
 ...
 private:
  ...
  ObjectType GetObjectType(const MetaType meta_type_id) {
    switch (meta_type_id) {
      case MetaType::META_UNKNOWN: return ObjectType::UNKNOWN;
      case MetaType::META_SMALLMOT: return ObjectType::VEHICLE;
      case MetaType::META_BIGMOT: return ObjectType::VEHICLE;
      case MetaType::META_NONMOT: return ObjectType::BICYCLE;
      case MetaType::META_PEDESTRIAN: return ObjectType::PEDESTRIAN;
      default: {
  ...
  std::vector<float> GetObjectTypeProbs(const std::vector<float>& meta_type_probs) {
    std::vector<float> object_type_probs(static_cast<int>(ObjectType::MAX_OBJECT_TYPE), 0.0);
