...
bool VisualizationSubnode::InitInternal() {
  ...
  if (camera_event_id_ != -1 || fusion_event_id_ != -1) {
    ...
    AINFO << "Init shared datas successfully, data: " << camera_object_data_->name();
    ...
    AINFO << "Init shared datas successfully, data: " << camera_shared_data_->name();
  }

  // init cipv object data
  if (cipv_event_id_ != -1) {
    cipv_object_data_ =
        dynamic_cast<CIPVObjectData*>(shared_data_manager_->GetSharedData("CIPVObjectData"));
    if (cipv_object_data_ == nullptr) {
      AERROR << "Failed to get CIPVObjectData.";
      return false;
    }
    AINFO << "Init shared datas successfully, data: " << cipv_object_data_->name();
  }
  ...
  if (radar_event_id_ != -1 || fusion_event_id_ != -1) {
    ...
    AINFO << "Init shared datas successfully, data: " << radar_object_data_->name();
  }
  ...
  frame_visualizer_.reset(BaseVisualizerRegisterer::GetInstanceByName(FLAGS_frame_visualizer));
  ...
  CalibrationConfigManager* config_manager = Singleton<CalibrationConfigManager>::get();
...
void VisualizationSubnode::SetCameraContent(const std::string& data_key,
  ...
  if (!camera_shared_data_->Get(data_key, &camera_item) || camera_item == nullptr) {
  ...
  content->set_camera_content(timestamp, objs->sensor2world_pose, objs->sensor2world_pose_static,
                              objs->objects, (*(objs->camera_frame_supplement)));
}
...
void VisualizationSubnode::SetFusionContent(const std::string& data_key,
  ...
  if (trigger_device_id == "camera") {
    SubnodeHelper::ProduceSharedDataKey(trigger_ts, trigger_device_id, &data_key_sensor);
...
void VisualizationSubnode::SetFrameContent(const Event&       event,
  ...
  if (event.event_id == camera_event_id_) {
    ...
    if (FLAGS_show_camera_objects || FLAGS_show_camera_objects2d || FLAGS_show_camera_parsing) {
      std::shared_ptr<CameraItem> camera_item;
      if (!camera_shared_data_->Get(data_key, &camera_item) || camera_item == nullptr) {
        AERROR << "Failed to get shared data: " << camera_shared_data_->name();
        return;
      ...
      LOG(INFO) << "number of objects in cipv is " << objs->objects.size() << timestamp
                << " with cipv index is " << objs->cipv_index;
      ...
      if (FLAGS_show_camera_parsing) {
        ...
        content->set_camera_content(timestamp, objs->sensor2world_pose, objs->objects);
      }
    }
  ...
  if (event.event_id == vis_driven_event_id_) { content->update_timestamp(timestamp); }
}
...
apollo::common::Status VisualizationSubnode::ProcEvents() {
  for (auto event_meta : sub_meta_events_) {
    ...
    for (size_t j = 0; j < events.size(); j++) {
      ...
      AINFO << "event: " << events[j].event_id << " device_id:" << device_id << " timestamp: ";
