...
bool UltrasonicObstacleSubnode::InitInternal() {
  ...
  AdapterManager::AddChassisCallback(&UltrasonicObstacleSubnode::OnUltrasonic, this);
...
void UltrasonicObstacleSubnode::OnUltrasonic(const apollo::canbus::Chassis& message) {
  ++seq_num_;
  PERF_BLOCK_START();
...
bool UltrasonicObstacleSubnode::PublishDataAndEvent(const double                        timestamp,
...
void UltrasonicObstacleSubnode::BuildSingleObject(const apollo::canbus::Sonar& sonar,
  ...
  double sonar_relative_heading = apollo::common::math::QuaternionToHeading(
      sonar.rotation().qw(), sonar.rotation().qx(), sonar.rotation().qy(), sonar.rotation().qz());
  ...
  Eigen::Vector3d direction(std::cos(sonar_heading), std::sin(sonar_heading), 0.0);
...
void UltrasonicObstacleSubnode::BuildAllObjects(const apollo::canbus::Surround& surround,
