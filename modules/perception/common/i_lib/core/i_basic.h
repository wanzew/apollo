...
inline float IDiv(float a, int b) { return ((b != 0) ? (a / static_cast<float>(b)) : 1.0f); }
inline float IDiv(float a, unsigned int b) {
  float result = 1.0f;
  if (b != 0) { result = a / static_cast<float>(b); }
  return result;
}
inline double IDiv(int a, int b) { return ((b != 0) ? (static_cast<double>(a) / b) : 1.0); }
inline double IDiv(unsigned int a, unsigned int b) {
  double result = 1.0;
  if (b != 0) { result = static_cast<double>(a) / b; }
  return result;
}
...
inline double ISqrt(int a) { return (a > 0) ? (sqrt(static_cast<double>(a))) : 0.0; }
inline double ISqrt(unsigned int a) { return (a > 0) ? (sqrt(static_cast<double>(a))) : 0.0; }
...
//  * return double
inline float  ICbrt(float a) { return (a >= 0.f) ? powf(a, 1.f / 3) : -powf(-a, 1.f / 3); }
inline double ICbrt(int a) {
  return (a >= 0) ? pow(static_cast<double>(a), 1.0 / 3) : -pow(-static_cast<double>(a), 1.0 / 3);
}
inline double ICbrt(unsigned int a) { return pow(static_cast<double>(a), 1.0 / 3); }
inline double ICbrt(double a) { return (a >= 0.0) ? pow(a, 1.0 / 3) : -pow(-a, 1.0 / 3); }
...
inline int          ISqr(unsigned char a) { return (static_cast<int>(a) * static_cast<int>(a)); }
...
inline double ILog(int x) { return ((x > 0) ? log(static_cast<double>(x)) : 0.0); }
inline double ILog(unsigned int x) { return ((x > 0) ? log(static_cast<double>(x)) : 0.0); }
...
inline double IPow(int a, int b) { return pow(static_cast<double>(a), static_cast<double>(b)); }
...
inline float  IRadiansToDegree(float r) { return (r * Constant<float>::RADIAN_TO_DEGREE()); }
inline double IRadiansToDegree(double r) { return (r * Constant<double>::RADIAN_TO_DEGREE()); }
inline float  IDegreeToRadians(float d) { return (d * Constant<float>::DEGREE_TO_RADIAN()); }
inline double IDegreeToRadians(double d) { return (d * Constant<double>::DEGREE_TO_RADIAN()); }
...
inline unsigned int IHammingLut(unsigned int a, unsigned int b) {
  ...
  distance = kIUByteBitCountLut[p[0]] + kIUByteBitCountLut[p[1]] + kIUByteBitCountLut[p[2]] +
             kIUByteBitCountLut[p[3]];
  return distance;  // Return the number of different bits
}
...
inline void IGaussian2D(float* kernel, int n, const float sigma) {
  ...
  const float nf  = IDiv(0.5f, (sigma * sigma));  // normalization factor = 1/(2sigma*sigma)
...
inline void IGaussian2D(double* kernel, int n, const double sigma) {
  ...
  const double nf  = IDiv(0.5, (sigma * sigma));  // normalization factor = 1/(2sigma*sigma)
...
template <typename T>
inline bool
IWithin2D(const T p[2], const T x_upper_left, const T y_upper_left, const T width, const T height) {
  return (p[0] >= x_upper_left && p[1] >= y_upper_left && p[0] < width + x_upper_left &&
          p[1] < height + y_upper_left);
}
...
//  * and height of the bounding box
template <typename T>
inline bool IWithin2D(const T x,
                      const T y,
