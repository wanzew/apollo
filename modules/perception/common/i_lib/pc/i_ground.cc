...
bool PlaneFitGroundDetectorParam::Validate() const {
  if (nr_grids_coarse < 2 || nr_grids_fine < 2 || nr_grids_coarse > nr_grids_fine ||
      nr_points_max == 0 || nr_samples_min_threshold == 0 || nr_samples_max_threshold == 0 ||
      nr_inliers_min_threshold == 0 || nr_ransac_iter_threshold == 0 || roi_region_rad_x <= 0.f ||
      roi_region_rad_y <= 0.f || roi_region_rad_z <= 0.f ||
...
int PlaneFitPointCandIndices::Prune(unsigned int min_nr_samples, unsigned int max_nr_samples) {
  assert(min_nr_samples < max_nr_samples);
  unsigned int size = static_cast<unsigned int>(indices.size());
  unsigned int half = 0;
  if (size > max_nr_samples) {
    IRandomizedShuffle1(indices.data(), static_cast<int>(indices.size()), &random_seed);
...
PlaneFitGroundDetector::PlaneFitGroundDetector(const PlaneFitGroundDetectorParam& param)
...
bool PlaneFitGroundDetector::Init() {
  ...
  if (!param_.Validate()) { return false; }
  // fine grid:
  vg_fine_ = new VoxelGridXY<float>();
  if (vg_fine_ == nullptr) { return false; }
  if (!vg_fine_->Alloc(param_.nr_grids_fine, param_.nr_grids_fine, -param_.roi_region_rad_x,
                       param_.roi_region_rad_x, -param_.roi_region_rad_y, param_.roi_region_rad_y,
                       -param_.roi_region_rad_z, param_.roi_region_rad_z)) {
    return false;
  }
  // coarse grid:
  vg_coarse_ = new VoxelGridXY<float>();
  if (vg_coarse_ == nullptr) { return false; }
  if (!vg_coarse_->Alloc(param_.nr_grids_coarse, param_.nr_grids_coarse, -param_.roi_region_rad_x,
                         param_.roi_region_rad_x, -param_.roi_region_rad_y, param_.roi_region_rad_y,
                         -param_.roi_region_rad_z, param_.roi_region_rad_z)) {
    return false;
  }
  ...
  // ground plane:
  ground_planes_ = IAlloc2<GroundPlaneLiDAR>(param_.nr_grids_coarse, param_.nr_grids_coarse);
  if (!ground_planes_) { return false; }
  ground_planes_sphe_ =
      IAlloc2<GroundPlaneSpherical>(param_.nr_grids_coarse, param_.nr_grids_coarse);
  if (!ground_planes_sphe_) { return false; }
  ground_z_ = IAlloc2<std::pair<float, bool>>(param_.nr_grids_coarse, param_.nr_grids_coarse);
  if (!ground_z_) { return false; }
  // sample candis:
  local_candis_ = IAlloc2<PlaneFitPointCandIndices>(param_.nr_grids_coarse, param_.nr_grids_coarse);
  if (!local_candis_) { return false; }
  // Reserve space to avoid runtime memory re-allocation
  for (r = 0; r < param_.nr_grids_coarse; ++r) {
    for (c = 0; c < param_.nr_grids_coarse; ++c) {
      if (r < (param_.nr_grids_coarse << 3) && c < (param_.nr_grids_coarse << 3)) {
        capacity = 16384;
      } else if (r < (param_.nr_grids_coarse << 2) && c < (param_.nr_grids_coarse << 2)) {
        capacity = 8192;
      } else if (r < (param_.nr_grids_coarse << 1) && c < (param_.nr_grids_coarse << 1)) {
        capacity = 4096;
      } else {
        capacity = 512;
      }
  ...
  pf_threeds_ = IAllocAligned<float>(param_.nr_samples_max_threshold * dim_point_, 4);
  if (!pf_threeds_) { return false; }
  ...
  memset(reinterpret_cast<void*>(labels_), 0, param_.nr_points_max * sizeof(char));
  // map of fine grid id to coarse id:
  map_fine_to_coarse_ = IAllocAligned<unsigned int>(param_.nr_grids_fine * param_.nr_grids_fine, 4);
  if (!map_fine_to_coarse_) { return false; }
  ...
  if (!sampled_z_values_) { return false; }
  memset(reinterpret_cast<void*>(sampled_z_values_), 0, param_.nr_z_comp_candis * sizeof(float));
  // ransac memory:
  sampled_indices_ = IAllocAligned<int>(param_.nr_z_comp_candis, 4);
  if (!sampled_indices_) { return false; }
  memset(reinterpret_cast<void*>(sampled_indices_), 0, param_.nr_z_comp_candis * sizeof(int));
  // ransac thresholds:
  pf_thresholds_ = IAlloc2<float>(param_.nr_grids_coarse, param_.nr_grids_coarse);
  if (!pf_thresholds_) { return false; }
  // compute thresholds
...
void PlaneFitGroundDetector::CleanUp() {
  if (vg_fine_) { delete vg_fine_; }
  if (vg_coarse_) { delete vg_coarse_; }
...
int PlaneFitGroundDetector::CompareZ(const float*              point_cloud,
  ...
  while (iter < indices.cend()) {
    ...
    if (IAbs(ptr[0]) < param_.roi_near_rad && IAbs(ptr[1]) < param_.roi_near_rad) {
      if (z < param_.sample_region_z_lower || z > param_.sample_region_z_upper) { continue; }
    }
    if (nr_compares > nr_z_comp_fail_threshold) {
      for (i = 0; i < nr_compares; ++i) {
        ...
        if (delta_z > param_.planefit_filter_threshold) {
          nr_contradi++;
          if (nr_contradi > nr_z_comp_fail_threshold) { break; }
        }
      }
    }
...
void PlaneFitGroundDetector::ComputeAdaptiveThreshold() {
  ...
  IMinMaxElements(pf_thresholds_[0], param_.nr_grids_coarse * param_.nr_grids_coarse, &min_dist,
                  &max_dist);
  if (max_dist - min_dist < Constant<float>::EPSILON()) {
    thre = (param_.planefit_dist_threshold_near + param_.planefit_dist_threshold_far) / 2;
    for (r = 0; r < param_.nr_grids_coarse; ++r) {
      for (c = 0; c < param_.nr_grids_coarse; ++c) {
    ...
    k = param_.planefit_dist_threshold_far - param_.planefit_dist_threshold_near;
    k = k / (max_dist - min_dist);
    b = param_.planefit_dist_threshold_far + param_.planefit_dist_threshold_near;
...
void PlaneFitGroundDetector::ComputeSignedGroundHeight(const float* point_cloud,
  ...
  ComputeSignedGroundHeightLine(point_cloud, ground_planes_[0], ground_planes_[0],
                                ground_planes_[1], height_above_ground, 0, nr_points,
                                nr_point_elements);
  for (r = 1; r < nm1; ++r) {
    ComputeSignedGroundHeightLine(point_cloud, ground_planes_[r - 1], ground_planes_[r],
                                  ground_planes_[r + 1], height_above_ground, r, nr_points,
                                  nr_point_elements);
  }
  ComputeSignedGroundHeightLine(point_cloud, ground_planes_[nm1 - 1], ground_planes_[nm1],
                                ground_planes_[nm1], height_above_ground, nm1, nr_points,
                                nr_point_elements);
}

void PlaneFitGroundDetector::ComputeSignedGroundHeightLine(const float*            point_cloud,
  ...
  while (iter < (*vg_coarse_)(r, 0).indices_.cend()) {
    ...
    dist[0] = plane[0] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[0], cptr) :
                                    std::numeric_limits<float>::max();
    min_abs_dist = IAbs(dist[0]);
    id           = 0;
    // for candidates we take min dist:
    if (label) {
      dist[1] = plane[1] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[1], cptr) :
                                      std::numeric_limits<float>::max();
      dist[2] = plane[2] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[2], cptr) :
                                      std::numeric_limits<float>::max();
      dist[3] = plane[3] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[3], cptr) :
                                      std::numeric_limits<float>::max();
      for (i = 1; i < 4; ++i) {
  ...
  for (c = 1; c < nm1; ++c) {
    ...
    while (iter < (*vg_coarse_)(r, c).indices_.cend()) {
      ...
      if (label) {
        ...
        dist[3] = plane[3] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[3], cptr) :
                                        std::numeric_limits<float>::max();
        dist[4] = plane[4] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[4], cptr) :
                                        std::numeric_limits<float>::max();
        for (i = 1; i < 5; ++i) {
          if (min_abs_dist > IAbs(dist[i])) {
            min_abs_dist = IAbs(dist[i]);
...
int PlaneFitGroundDetector::FilterGrid(const Voxel<float>&       vx,
  ...
  nr_candis = CompareZ(point_cloud, vx.indices_, sampled_z_values_, candi, nr_points,
                       nr_point_element, nr_samples);
  return nr_candis;
}
...
int PlaneFitGroundDetector::FilterLine(unsigned int r) {
  ...
  for (c = 0; c < param_.nr_grids_fine; c++) {
    ...
    nr_candis += FilterGrid((*vg_fine_)(r, c), point_cloud, &local_candis_[0][parent], nr_points,
                            nr_point_element);
  }
  return nr_candis;
}
...
int PlaneFitGroundDetector::Filter() {
  ...
  unsigned int i         = 0;
  unsigned int r         = 0;
  memset(reinterpret_cast<void*>(labels_), 0, vg_fine_->NrPoints() * sizeof(char));
...
int PlaneFitGroundDetector::FitGrid(const float*              point_cloud,
  ...
  if (candi->Size() < param_.nr_inliers_min_threshold) { return 0; }
  ...
  int nr_inliers_termi =
      IRound(static_cast<float>(nr_samples) * param_.termi_inlier_percen_threshold);
  ...
  for (i = 0; i < param_.nr_ransac_iter_threshold; ++i) {
    ...
    if (plane.GetDegreeNormalToZ() > param_.planefit_orien_threshold) { continue; }
    ...
    fit_cost = nr_inliers > 0 ? (fit_cost / static_cast<float>(nr_inliers)) : dist_thre;
    ...
    if (nr_inliers >= nr_inliers_best) {
      ...
      if (nr_inliers_best > nr_inliers_termi) { break; }
    }
  }
  // check if meet the inlier number requirement
  if (!groundplane->IsValid()) { return 0; }
...
int PlaneFitGroundDetector::FitLine(unsigned int r) {
  ...
  GroundPlaneLiDAR gp;
  for (c = 0; c < param_.nr_grids_coarse; c++) {
    if (FitGrid(vg_coarse_->const_data(), &local_candis_[r][c], &gp, vg_coarse_->NrPoints(),
                vg_coarse_->NrPointElement(),
                pf_thresholds_[r][c]) >= static_cast<int>(param_.nr_inliers_min_threshold)) {
...
int PlaneFitGroundDetector::FilterCandidates(int                               r,
                                             int                               c,
                                             const float*                      point_cloud,
                                             PlaneFitPointCandIndices*         candi,
...
inline float calculate_two_angles(const GroundPlaneLiDAR& p1, const GroundPlaneLiDAR& p2) {
...
int PlaneFitGroundDetector::FitGridWithNeighbors(int               r,
                                                 int               c,
                                                 const float*      point_cloud,
                                                 GroundPlaneLiDAR* groundplane,
                                                 unsigned int      nr_points,
  ...
  for (int i = 0; i < param_.nr_ransac_iter_threshold; ++i) {
    ...
    // check if the plane hypothesis has valid geometry
    if (hypothesis[i].GetDegreeNormalToZ() > param_.planefit_orien_threshold) { continue; }
    // iterate samples and check if the point to plane distance is below
    // threshold
    psrc       = pf_threeds_;
    nr_inliers = 0;
    for (int j = 0; j < nr_samples; ++j) {
      ...
      if (ptp_dist < dist_thre) { nr_inliers++; }
      psrc += dim_point_;
    }
    // Assign number of supports
    hypothesis[i].SetNrSupport(nr_inliers);

    if (nr_inliers > nr_inliers_termi) { break; }
  }
  ...
  for (int i = 0; i < kNr_iter; ++i) {
    if (!(hypothesis[i].IsValid())) { continue; }
...
float PlaneFitGroundDetector::CalculateAngleDist(
    const GroundPlaneLiDAR& plane, const std::vector<std::pair<int, int>>& neighbors) {
...
int PlaneFitGroundDetector::FitInOrder() {
  ...
  for (i = 0; i < vg_coarse_->NrVoxel(); ++i) {
    ...
    if (FitGridWithNeighbors(r, c, vg_coarse_->const_data(), &gp, vg_coarse_->NrPoints(),
...
void PlaneFitGroundDetector::GetNeighbors(
    int r, int c, int rows, int cols, std::vector<std::pair<int, int>>* neighbors) {
  int left  = IMax(0, c - 1);
...
int PlaneFitGroundDetector::SmoothLine(unsigned int up, unsigned int r, unsigned int dn) {
  int                  nr_grids = 0;
  unsigned int         c        = 0;
  unsigned int         nm1      = param_.nr_grids_coarse - 1;
  ...
  if (/*!(*_vg_coarse)(r, 0).empty()*/ true) {
    if (!ground_planes_sphe_[r][0].IsValid()) {
      nr_grids += CompleteGrid(ground_planes_sphe_[r][0], ground_planes_sphe_[r][1],
                               ground_planes_sphe_[up][0], ground_planes_sphe_[dn][0], &plane);
    } else {
      nr_grids += SmoothGrid(ground_planes_sphe_[r][0], ground_planes_sphe_[r][0],
  ...
  for (c = 1; c < nm1; ++c) {
    if (/*!(*_vg_coarse)(r, c).empty()*/ true) {
      if (!ground_planes_sphe_[r][c].IsValid()) {
        nr_grids += CompleteGrid(ground_planes_sphe_[r][c - 1], ground_planes_sphe_[r][c + 1],
                                 ground_planes_sphe_[up][c], ground_planes_sphe_[dn][c], &plane);
      } else {
        nr_grids += SmoothGrid(ground_planes_sphe_[r][c], ground_planes_sphe_[r][c - 1],
  ...
  if (/*!(*_vg_coarse)(r, nm1).empty()*/ true) {
    if (!ground_planes_sphe_[r][nm1].IsValid()) {
      nr_grids += CompleteGrid(ground_planes_sphe_[r][nm1 - 1], ground_planes_sphe_[r][nm1],
                               ground_planes_sphe_[up][nm1], ground_planes_sphe_[dn][nm1], &plane);
    } else {
      nr_grids += SmoothGrid(ground_planes_sphe_[r][nm1], ground_planes_sphe_[r][nm1 - 1],
...
int PlaneFitGroundDetector::CompleteGrid(const GroundPlaneSpherical& lt,
  ...
  if (!support_sum) { return 0; }
  weights[0] = static_cast<float>(supports[0]) / static_cast<float>(support_sum);
  weights[1] = static_cast<float>(supports[1]) / static_cast<float>(support_sum);
  weights[2] = static_cast<float>(supports[2]) / static_cast<float>(support_sum);
  weights[3] = static_cast<float>(supports[3]) / static_cast<float>(support_sum);
  // weighted average:
  gp->theta =
      weights[0] * lt.theta + weights[1] * rt.theta + weights[2] * up.theta + weights[3] * dn.theta;
  gp->phi = weights[0] * lt.phi + weights[1] * rt.phi + weights[2] * up.phi + weights[3] * dn.phi;
  gp->d   = weights[0] * lt.d + weights[1] * rt.d + weights[2] * up.d + weights[3] * dn.d;
...
int PlaneFitGroundDetector::SmoothGrid(const GroundPlaneSpherical& g,
  ...
  // 1 4 1
  //   1
  if (lt.GetStatus()) { supports[0] = lt.GetNrSupport(); }
  if (rt.GetStatus()) { supports[1] = rt.GetNrSupport(); }
  if (up.GetStatus()) { supports[2] = up.GetNrSupport(); }
  if (dn.GetStatus()) { supports[3] = dn.GetNrSupport(); }
  ...
  weights[2] = static_cast<float>(supports[2]) / static_cast<float>(support_sum);
  weights[3] = static_cast<float>(supports[3]) / static_cast<float>(support_sum);
  weights[4] = static_cast<float>(supports[4]) / static_cast<float>(support_sum);
  // weighted average:
  gp->theta = weights[0] * lt.theta + weights[1] * rt.theta + weights[2] * up.theta +
...
bool PlaneFitGroundDetector::Detect(const float* point_cloud,
  ...
  if (!vg_fine_->SetS(point_cloud, nr_points, nr_point_elements)) { return false; }
  // setup the coarse voxel grid
  if (!vg_coarse_->SetS(point_cloud, nr_points, nr_point_elements)) { return false; }
  // int nr_candis = 0;
  // int nr_valid_grid = 0;
  unsigned int r = 0;
  unsigned int c = 0;
  ...
  for (r = 0; r < param_.nr_grids_coarse; ++r) {
    for (c = 0; c < param_.nr_grids_coarse; ++c) {
      if ((*vg_coarse_)(r, c).Empty()) { ground_planes_[r][c].ForceInvalid(); }
    }
  }
...
const VoxelGridXY<float>* PlaneFitGroundDetector::GetGrid() const { return vg_coarse_; }

const GroundPlaneLiDAR* PlaneFitGroundDetector::GetGroundPlane(int r, int c) const {
...
unsigned int PlaneFitGroundDetector::GetGridDimX() const { return vg_coarse_->NrVoxelX(); }

unsigned int PlaneFitGroundDetector::GetGridDimY() const { return vg_coarse_->NrVoxelY(); }

float PlaneFitGroundDetector::GetUnknownHeight() { return std::numeric_limits<float>::max(); }

PlaneFitPointCandIndices** PlaneFitGroundDetector::GetCandis() const { return local_candis_; }
