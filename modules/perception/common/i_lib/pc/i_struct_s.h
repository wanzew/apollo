...
class PtCluster {
 public:
  ...
  unsigned int ClustersizeInByte() const { return (d * sizeof(T) * nr_points_); }
...
template <typename T>
inline int IAssignPointToVoxel(const T* data,
                               T        bound_x_min,
                               T        bound_x_max,
                               T        bound_y_min,
                               T        bound_y_max,
  ...
  if (x < bound_x_min || x > bound_x_max || y < bound_y_min || y > bound_y_max || z < bound_z_min ||
      z > bound_z_max) {
    return (-1);
  }

  // compute the x, y voxel indices
  k = IMin(nr_voxel_x - 1, static_cast<int>((x - bound_x_min) * voxel_width_x_rec));
  j = IMin(nr_voxel_y - 1, static_cast<int>((y - bound_y_min) * voxel_width_y_rec));
  i = (nr_voxel_x * j) + k;
  return (i);
}
...
class VoxelGridXY {
 ...
 public:
  VoxelGridXY();
  VoxelGridXY(unsigned int nr_voxel_x,
              unsigned int nr_voxel_y,
              T            spatial_bound_x_min,
              T            spatial_bound_x_max,
              T            spatial_bound_y_min,
              T            spatial_bound_y_max,
              T            spatial_bound_z_min,
              T            spatial_bound_z_max);
  ...
  VoxelGridXY& operator=(const VoxelGridXY<T>& vg) {
    if (this != &vg) {
      ...
      vg.GetGridDimension(&this->dim_x_[0], &this->dim_x_[1], &this->dim_y_[0], &this->dim_y_[1],
                          &this->dim_z_[0], &this->dim_z_[1]);
  ...
  bool Alloc(unsigned int nr_voxel_x,
             unsigned int nr_voxel_y,
             T            spatial_bound_x_min,
             T            spatial_bound_x_max,
             T            spatial_bound_y_min,
  ...
  bool GetGridDimension(
      T* dim_min_x, T* dim_max_x, T* dim_min_y, T* dim_max_y, T* dim_min_z, T* dim_max_z) const {
    if (!initialized_) {
      dim_min_x = dim_max_x = dim_min_y = dim_max_y = dim_min_z = dim_max_z = static_cast<T>(0);
      return (false);
    }
    *dim_min_x = dim_x_[0];
    *dim_max_x = dim_x_[1];
  ...
  void
  SetGridDimension(T dim_min_x, T dim_max_x, T dim_min_y, T dim_max_y, T dim_min_z, T dim_max_z) {
    dim_x_[0] = dim_min_x;
    dim_x_[1] = dim_max_x;
    dim_y_[0] = dim_min_y;
    dim_y_[1] = dim_max_y;
  ...
  void SetNrPointElement(unsigned int nr_point_element) { nr_point_element_ = nr_point_element; }
...
template <typename T>
VoxelGridXY<T>::VoxelGridXY(unsigned int nr_voxel_x,
                            unsigned int nr_voxel_y,
                            T            spatial_bound_x_min,
                            T            spatial_bound_x_max,
                            T            spatial_bound_y_min,
                            ...
                            T            spatial_bound_z_min,
                            T            spatial_bound_z_max) {
  Alloc(nr_voxel_x, nr_voxel_y, spatial_bound_x_min, spatial_bound_x_max, spatial_bound_y_min,
...
bool VoxelGridXY<T>::AllocAlignedMemory() {
  if (!mem_aligned16_f32_) { mem_aligned16_f32_ = IAllocAligned<float>(4, 4); }
  if (!mem_aligned16_i32_) { mem_aligned16_i32_ = IAllocAligned<int>(4, 4); }
  return (mem_aligned16_f32_ != nullptr && mem_aligned16_i32_ != nullptr &&
          IVerifyAlignment(mem_aligned16_f32_, 4) && IVerifyAlignment(mem_aligned16_i32_, 4));
}
...
int VoxelGridXY<T>::WhichVoxel(T x, T y, T z) const {
  ...
  if (x < dim_x_[0] || x > dim_x_[1] || y < dim_y_[0] || y > dim_y_[1] || z < dim_z_[0] ||
      z > dim_z_[1]) {
    return (-1);  // points that are outside the defined BBOX are ignored
  }

  k = static_cast<int>(IMax(
      (unsigned int)0, IMin(nr_voxel_x_ - 1, (unsigned int)((x - dim_x_[0]) / voxel_dim_[0]))));
  j = static_cast<int>(IMax(
      (unsigned int)0, IMin(nr_voxel_y_ - 1, (unsigned int)((y - dim_y_[0]) / voxel_dim_[1]))));
...
template <typename T>
bool VoxelGridXY<T>::Alloc(unsigned int nr_voxel_x,
                           unsigned int nr_voxel_y,
                           T            spatial_bound_x_min,
                           T            spatial_bound_x_max,
                           T            spatial_bound_y_min,
...
template <typename T>
bool VoxelGridXY<T>::Set(const T* data, unsigned int nr_points, unsigned int nr_point_element) {
  if (!data || !nr_points || !nr_point_element || !nr_voxel_x_ || !nr_voxel_y_ ||
      nr_voxel_z_ != 1) {
    initialized_ = false;
    return initialized_;
  }
  ...
  for (; n < static_cast<int>(nr_points_); n++, nd += nr_point_element_) {
    id = IAssignPointToVoxel(data_ + nd, bound_x_min, bound_x_max, bound_y_min, bound_y_max,
                             bound_z_min, bound_z_max, voxel_width_x_rec, voxel_width_y_rec,
                             static_cast<int>(nr_voxel_x_), static_cast<int>(nr_voxel_y_));

    if (id >= 0) { voxels_[id].indices_.push_back(n); }
  }
...
template <typename T>
bool VoxelGridXY<T>::SetS(const float* data,
                          unsigned int nr_points,
                          unsigned int nr_point_element) {
  if (!data || !nr_points || !nr_point_element || !nr_voxel_x_ || !nr_voxel_y_ ||
      nr_voxel_z_ != 1) {
  ...
  __m128i iv_nr_voxel_x =
      _mm_setr_epi32(static_cast<int>(nr_voxel_x_), 0, static_cast<int>(nr_voxel_x_), 0);
  ...
  for (i = 0; i < nr_loops; ++i, n += 4) {
    ...
    if (mem_aligned16_f32_[0] != 0) { voxels_[mem_aligned16_i32_[0]].indices_.push_back(n); }
    if (mem_aligned16_f32_[1] != 0) { voxels_[mem_aligned16_i32_[1]].indices_.push_back(n + 1); }
    if (mem_aligned16_f32_[2] != 0) { voxels_[mem_aligned16_i32_[2]].indices_.push_back(n + 2); }
    if (mem_aligned16_f32_[3] != 0) { voxels_[mem_aligned16_i32_[3]].indices_.push_back(n + 3); }
    cptr_x += d4;
    cptr_y += d4;
    cptr_z += d4;
  }
  ...
  for (i = 0; i < remainder; i++, n++) {
    id = IAssignPointToVoxel(cptr_remainder, dim_x_[0], dim_x_[1], dim_y_[0], dim_y_[1], dim_z_[0],
                             dim_z_[1], voxel_width_x_rec, voxel_width_y_rec,
                             static_cast<int>(nr_voxel_x_), static_cast<int>(nr_voxel_y_));
...
template <typename T>
bool VoxelGridXY<T>::Set(const T*     data,
                         unsigned int nr_points,
                         unsigned int nr_point_element,
                         unsigned int nr_voxel_x,
                         unsigned int nr_voxel_y,
  ...
  if (force_bound) {
    ...
    IGetPointcloudsDimWBound(data_, nr_points_, 0, nr_point_element_, &dim_x_[0], &dim_x_[1],
  ...
  for (; n < nr_points_; n++, nd += nr_point_element_) {
    ...
    if (x < dim_x_min || x > dim_x_max || y < dim_y_min || y > dim_y_max || z < dim_z_min ||
        z > dim_z_max) {
      // points that are outside the defined BBOX are ignored
      continue;
    }
    ...
    k = IMax((unsigned int)0,
             IMin(nr_voxel_xm1, (unsigned int)((x - dim_x_min) * voxel_width_x_rec)));
    j = IMax((unsigned int)0,
             IMin(nr_voxel_ym1, (unsigned int)((y - dim_y_min) * voxel_width_y_rec)));
    i = nr_voxel_x * j + k;
...
template <typename T>
bool IDownsampleVoxelGridXY(const VoxelGridXY<T>& src,
                            VoxelGridXY<T>*       dst,
  ...
  src.GetVoxelDimension(&voxel_width_x_src, &voxel_width_y_src, &voxel_width_z_src);

  src.GetGridDimension(&dim_min_x_src, &dim_max_x_src, &dim_min_y_src, &dim_max_y_src,
                       &dim_min_z_src, &dim_max_z_src);
  ...
  dim_x_dst[1] = (0 == nr_res_voxel_x_dst) ? dim_max_x_src :
                                             (voxel_width_x_dst * nr_voxel_x_dst + dim_x_dst[0]);
  dim_y_dst[1] = (0 == nr_res_voxel_y_dst) ? dim_max_y_src :
  ...
  dst->SetVoxelDimension(voxel_width_x_dst, voxel_width_y_dst, voxel_width_z_dst);
  dst->SetGridDimension(dim_x_dst[0], dim_x_dst[1], dim_y_dst[0], dim_y_dst[1], dim_z_dst[0],
                        dim_z_dst[1]);
  ...
  for (r = 0; r < nr_voxel_y_dst; r++) {
    ...
    for (c = 0; c < nr_voxel_x_dst; c++) {
      cs = (c * sf_x);
      voxels[n].init(vxs[c], vy, vz, voxel_width_x_dst, voxel_width_y_dst, voxel_width_z_dst,
                     static_cast<int>(c), static_cast<int>(r), static_cast<int>(0));
...
class VoxelGridXYPyramid {
 public:
  VoxelGridXYPyramid();
  VoxelGridXYPyramid(unsigned int nr_scale,
    ...
    Alloc(nr_scale, nr_voxel_x_base, nr_voxel_y_base, dsf_x, dsf_y, spatial_bound_x_min,
          spatial_bound_x_max, spatial_bound_y_min, spatial_bound_y_max, spatial_bound_z_min,
          spatial_bound_z_max);
  }
  ...
  bool Alloc(unsigned int nr_scale,
             unsigned int nr_voxel_x_base,
             unsigned int nr_voxel_y_base,
             unsigned int dsf_x,
             unsigned int dsf_y,
...
template <typename DATA_TYPE>
bool VoxelGridXYPyramid<DATA_TYPE>::Alloc(unsigned int nr_scale,
                                          unsigned int nr_voxel_x_base,
                                          unsigned int nr_voxel_y_base,
                                          unsigned int dsf_x,
                                          unsigned int dsf_y,
                                          ...
                                          DATA_TYPE    spatial_bound_z_min,
                                          DATA_TYPE    spatial_bound_z_max) {
  if (!nr_scale || !nr_voxel_x_base || !nr_voxel_y_base) { return false; }
  ...
  for (scale = 0; scale < nr_scale; ++scale) {
    if (!vgrids_[scale].alloc(nr_voxel_x, nr_voxel_y, spatial_bound_x_min, spatial_bound_x_max,
  ...
  if (scale == 0) {
    ...
    if (scale != nr_scale) { vgrids_.resize(scale); }
  }
...
bool VoxelGridXYPyramid<DATA_TYPE>::Set(const DATA_TYPE* pc,
                                        unsigned int     nr_points,
                                        unsigned int     nr_point_element) {
  if (!pc || !nr_points || !nr_point_element || vgrids_.empty()) { return false; }
  ...
  for (scale = 0; scale < nr_scale; ++scale) {
    if (!vgrids_[scale].set(pc, nr_points, nr_point_element)) { break; }
  }
...
bool VoxelGridXYPyramid<DATA_TYPE>::SetS(const float* pc,
  ...
  for (scale = 0; scale < nr_scale; ++scale) {
    if (!vgrids_[scale].set_s(pc, nr_points, nr_point_element)) { break; }
  }
...
bool VoxelGridXYPyramid<DATA_TYPE>::Set(const DATA_TYPE* pc,
                                        ...
                                        DATA_TYPE        spatial_bound_z_min,
                                        DATA_TYPE        spatial_bound_z_max) {
  if (!pc || !nr_scale || !nr_points || !nr_point_element || !nr_voxel_x_base || !nr_voxel_y_base) {
    return false;
  }
  ...
  if (!vgrids_[0].set(pc_, nr_points, nr_point_element, nr_voxel_x_base, nr_voxel_y_base,
  ...
  for (s = 1; s < nr_scale; ++s) {
    if (!IDownsampleVoxelGridXY(vgrids_[s - 1], &vgrids_[s], dsf_x_, dsf_y_)) { break; }
  }
...
bool VoxelGridXYPyramid<DATA_TYPE>::Initialized() const {
  unsigned int i;
  if (vgrids_.empty()) { return false; }
  for (i = 0; i < vgrids_.size(); ++i) {
    if (!vgrids_[i].initialized()) { break; }
  }
