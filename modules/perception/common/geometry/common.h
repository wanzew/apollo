...
bool IsPointXYInPolygon2DXY(const PointT& point, const base::PointCloud<PointT>& polygon) {
  ...
  for (size_t i = 0; i < nr_poly_points; ++i) {
    ...
    if (x1 <= point.x && point.x <= x2 && distance < std::numeric_limits<Type>::epsilon()) {
      return true;
    }
...
bool IsPointInBBox(const Eigen::Matrix<typename PointT::Type, 3, 1>& gnd_c,
  ...
  T                      x    = diff.dot(dir_x);
  if (fabs(x) > size[0] * 0.5) { return false; }
  T y = diff.dot(dir_y);
  if (fabs(y) > size[1] * 0.5) { return false; }
  T z = diff.dot(dir_z);
  if (fabs(z) > size[2] * 0.5) { return false; }
  return true;
}
...
void CalculateMostConsistentBBoxDir2DXY(const Eigen::Matrix<Type, 3, 1>& prev_dir,
  ...
  if (fabs(dot_val_00) >= fabs(dot_val_01)) {
    if (dot_val_00 < 0) { (*curr_dir) = -(*curr_dir); }
  } else {
    if (dot_val_01 < 0) {
      (*curr_dir) = Eigen::Matrix<Type, 3, 1>((*curr_dir)(1), -(*curr_dir)(0), 0);
    } else {
      (*curr_dir) = Eigen::Matrix<Type, 3, 1>(-(*curr_dir)(1), (*curr_dir)(0), 0);
    }
  }
...
template <typename PointT>
bool CalculateDistAndDirToSegs(const Eigen::Matrix<typename PointT::Type, 3, 1>& pt,
...
template <typename PointT>
void CalculateDistAndDirToBoundary(const Eigen::Matrix<typename PointT::Type, 3, 1>& pt,
  ...
  CalculateDistAndDirToSegs(pt, right_boundary, &dist_to_right, &direction_right);
...
void CalculateDistAndDirToBoundary(
  ...
  for (size_t i = 0; i < left_boundary.size(); i++) {
    ...
    if (CalculateDistAndDirToSegs(pt, left_boundary[i], &dist_temp, &dir_temp)) {
  ...
  for (size_t i = 0; i < right_boundary.size(); i++) {
    Type                      dist_temp = std::numeric_limits<Type>::max();
    Eigen::Matrix<Type, 3, 1> dir_temp;
    if (CalculateDistAndDirToSegs(pt, right_boundary[i], &dist_temp, &dir_temp)) {
