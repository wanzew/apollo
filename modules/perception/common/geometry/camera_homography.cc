...
bool PointCamera1ToCamera2(const Eigen::Vector2d& point,
  ...
  Eigen::Vector3d camera2_3d = static_cast<Eigen::Matrix<double, 3, 1, 0, 3, 1>>(
...
bool IsCamerasFieldOverlap(const base::PinholeCameraModel& from_camera,
  ...
  points << 0.0, 0.0, static_cast<double>(f_width), 0.0, static_cast<double>(f_width),
      static_cast<double>(f_height), 0.0, static_cast<double>(f_height);
  Eigen::Vector2d pt_min(std::numeric_limits<double>::max(), std::numeric_limits<double>::max());
  Eigen::Vector2d pt_max(-std::numeric_limits<double>::max(), -std::numeric_limits<double>::max());
  Eigen::Matrix3d from_camera_intrinsic_inverse =
      from_camera.get_intrinsic_params().cast<double>().inverse();
  for (int i = 0; i < 4; ++i) {
    Eigen::Vector2d point_out;
    bool status = PointCamera1ToCamera2(points.block<1, 2>(i, 0), from_camera_intrinsic_inverse,
                                        to_camera.get_intrinsic_params().cast<double>(),
                                        f2t_rotation, &point_out);
    if (status) {
  ...
  if ((up_left->array() < low_right->array()).all()) { return true; }
  return false;
}
