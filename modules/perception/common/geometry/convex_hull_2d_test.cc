...
TEST(ConvexHull2DTest, convex_hull_2d) {
  ...
  flag = convex_hull_2d.GetConvexHullWithoutGround(pointcloud_in, distance_above_ground_thres,
                                                   &pointcloud_out);
  EXPECT_TRUE(flag);
  EXPECT_EQ(pointcloud_out.size(), 4);
  pointcloud_out.clear();
  ...
  flag                              = convex_hull_2d.GetConvexHullWithoutGroundAndHead(
      pointcloud_in, distance_above_ground_thres, distance_beneath_head_thres, &pointcloud_out);
  EXPECT_TRUE(flag);
  EXPECT_EQ(pointcloud_out.size(), 4);
...
TEST(ConvexHull2DTest1, convex_hull_2d1) {
  ConvexHull2D<PointFCloud, PointFCloud> convex_hull_2d;
  PointFCloudPtr pointcloud_in_ptr = PointFCloudPtr(new PointFCloud(128, 128, PointF()));
  ...
  flag = convex_hull_2d.GetConvexHullWithoutGround(*pointcloud_in_ptr, distance_above_ground_thres,
                                                   &pointcloud_out);
  EXPECT_TRUE(flag);
  EXPECT_EQ(pointcloud_out.size(), 4);
  ...
  pointcloud_out.clear();
  flag = convex_hull_2d.GetConvexHullWithoutGroundAndHead(
      *pointcloud_in_ptr, distance_above_ground_thres, distance_beneath_head_thres,
      &pointcloud_out);
  EXPECT_TRUE(flag);
  EXPECT_EQ(pointcloud_out.size(), 4);
}
