...
bool IsPtInRoi(const HdmapStructConstPtr roi, const PointD pt) {
  for (std::size_t j = 0; j < roi->road_polygons.size(); j++) {
    if (IsPointXYInPolygon2DXY(pt, roi->road_polygons[j])) { return true; }
  }
  for (std::size_t j = 0; j < roi->junction_polygons.size(); j++) {
    if (IsPointXYInPolygon2DXY(pt, roi->junction_polygons[j])) { return true; }
  }
  return false;
}
...
bool IsObjectBboxInRoi(const HdmapStructConstPtr roi, const ObjectConstPtr obj) {
  ...
  bbox_dir(2)              = 0.0;
  if (bbox_dir.norm() < std::numeric_limits<float>::epsilon()) { return IsPtInRoi(roi, ct); }
  bbox_dir.normalize();
  Eigen::Vector3d bbox_ortho_dir = Eigen::Vector3d(-bbox_dir(1), bbox_dir(0), 0.0f);
  double          bbox_length    = obj->size[0];
  double          bbox_width     = obj->size[1];
  Eigen::Vector3d bbox_corners[4];
  bbox_corners[0] = bbox_center + bbox_dir * bbox_length / 2 + bbox_ortho_dir * bbox_width / 2;
  bbox_corners[1] = bbox_center - bbox_dir * bbox_length / 2 + bbox_ortho_dir * bbox_width / 2;
  bbox_corners[2] = bbox_center + bbox_dir * bbox_length / 2 - bbox_ortho_dir * bbox_width / 2;
  bbox_corners[3] = bbox_center - bbox_dir * bbox_length / 2 - bbox_ortho_dir * bbox_width / 2;
  for (int i = 0; i < 4; ++i) {
    PointD corner;
    corner.x = bbox_corners[i][0];
    corner.y = bbox_corners[i][1];
    corner.z = bbox_corners[i][2];
    if (IsPtInRoi(roi, corner)) { return true; }
  }
  return false;
}
...
bool ObjectInRoiCheck(const HdmapStructConstPtr     roi,
  ...
  if (roi == nullptr || (roi->road_polygons.empty() && roi->junction_polygons.empty())) {
  ...
  for (std::size_t i = 0; i < objects.size(); i++) {
    if (IsObjectInRoi(roi, objects[i])) { valid_objects->push_back(objects[i]); }
  }
