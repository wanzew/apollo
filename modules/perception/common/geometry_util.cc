...
void TransformPointCloud(pcl_util::PointCloudPtr cloud,
  ...
  if (trans_cloud->size() != indices.size()) { trans_cloud->resize(indices.size()); }
...
void TransformPointCloud(pcl_util::PointCloudPtr  cloud,
  ...
  if (trans_cloud->size() != cloud->size()) { trans_cloud->resize(cloud->size()); }
...
void TransAffineToMatrix4(const Eigen::Vector3d& translation,
  ...
  (*trans_matrix) << 1 - 2 * (qy * qy + qz * qz), 2 * (qx * qy - qz * qw), 2 * (qx * qz + qy * qw),
      t_x, 2 * (qx * qy + qz * qw), 1 - 2 * (qx * qx + qz * qz), 2 * (qy * qz - qx * qw), t_y,
      2 * (qx * qz - qy * qw), 2 * (qy * qz + qx * qw), 1 - 2 * (qx * qx + qy * qy), t_z, 0, 0, 0,
      1;
}
...
void ComputeMostConsistentBboxDirection(const Eigen::Vector3f& previous_dir,
  ...
  if (fabs(dot_val_00) >= fabs(dot_val_01)) {
    if (dot_val_00 < 0) { (*current_dir) = -(*current_dir); }
  } else {
    if (dot_val_01 < 0) {
