...
void DummyDetector::ContiObs2Frame(const drivers::ContiRadar& corrected_obstacles,
  ...
  for (const auto& radar_obs : corrected_obstacles.contiobs()) {
    ...
    radar_object->center_uncertainty   = (dist_rms * dist_rms.transpose()).cast<float>();
    radar_object->velocity_uncertainty = (vel_rms * vel_rms.transpose()).cast<float>();
    ...
    radar_object->theta          = static_cast<float>(std::atan2(direction(1), direction(0)));
...
bool DummyRoiFilter::RoiFilter(const RoiFilterOptions& options, base::FramePtr radar_frame) {
  return true;
}
