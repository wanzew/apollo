...
void AdaptiveKalmanFilter::Init(const base::Object& object) {
  ...
  r_matrix_.topLeftCorner(2, 2) = object.center_uncertainty.topLeftCorner(2, 2).cast<double>();
  r_matrix_.block<2, 2>(2, 2)   = object.velocity_uncertainty.topLeftCorner(2, 2).cast<double>();
  p_matrix_.setIdentity();
  p_matrix_.topLeftCorner(2, 2) = object.center_uncertainty.topLeftCorner(2, 2).cast<double>();
  p_matrix_.block<2, 2>(2, 2)   = object.velocity_uncertainty.topLeftCorner(2, 2).cast<double>();
...
Eigen::VectorXd AdaptiveKalmanFilter::UpdateWithObject(const base::Object& new_object,
                                                       double              time_diff) {
  ...
  r_matrix_.setIdentity();
  r_matrix_.topLeftCorner(2, 2) = new_object.center_uncertainty.topLeftCorner(2, 2).cast<double>();
  r_matrix_.block<2, 2>(2, 2) = new_object.velocity_uncertainty.topLeftCorner(2, 2).cast<double>();
...
void AdaptiveKalmanFilter::GetState(Eigen::Vector3d* anchor_point, Eigen::Vector3d* velocity) {
