...
ContiArsTracker::~ContiArsTracker() {
  if (matcher_ != nullptr) { delete matcher_; }
  if (track_manager_ != nullptr) { delete track_manager_; }
}
...
bool ContiArsTracker::Init() {
  ...
  if (!model_config->get_value("tracked_times_threshold", &tracked_times_threshold)) {
  ...
  ACHECK(track_manager_ != nullptr) << "Failed to get RadarTrackManager instance.";
  return state;
}
...
void ContiArsTracker::TrackObjects(const base::Frame& radar_frame) {
  ...
  matcher_->Match(radar_tracks, radar_frame, matcher_options, &assignments, &unassigned_tracks,
                  &unassigned_objects);
...
void ContiArsTracker::UpdateAssignedTracks(const base::Frame&           radar_frame,
                                           std::vector<TrackObjectPair> assignments) {
  auto& radar_tracks = track_manager_->mutable_tracks();
  for (size_t i = 0; i < assignments.size(); ++i) {
    radar_tracks[assignments[i].first]->UpdataObsRadar(radar_frame.objects[assignments[i].second],
                                                       radar_frame.timestamp);
  }
}

void ContiArsTracker::UpdateUnassignedTracks(const base::Frame&         radar_frame,
  ...
  for (size_t i = 0; i < unassigned_tracks.size(); ++i) {
    if (radar_tracks[unassigned_tracks[i]]->GetObs() != nullptr) {
      ...
      if (time_diff > s_tracking_time_win_) { radar_tracks[unassigned_tracks[i]]->SetDead(); }
    } else {
...
void ContiArsTracker::CreateNewTracks(const base::Frame&         radar_frame,
                                      const std::vector<size_t>& unassigned_objects) {
  for (size_t i = 0; i < unassigned_objects.size(); ++i) {
    RadarTrackPtr radar_track;
    radar_track.reset(
        new RadarTrack(radar_frame.objects[unassigned_objects[i]], radar_frame.timestamp));
