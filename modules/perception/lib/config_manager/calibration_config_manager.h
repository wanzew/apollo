...
struct CameraCoeffient {
 ...
 private:
  ...
  bool init_camera_intrinsic_matrix_and_distort_params(const std::string& camera_intrinsic_file);
...
class CalibrationConfigManager {
 public:
  ...
  inline CameraCalibrationPtr get_camera_calibration() { return camera_calibration_; }
...
class CameraCalibration {
 public:
  CameraCalibration();
  ~CameraCalibration();
  bool init(const std::string& intrinsic_path, const std::string& extrinsic_path);

  void calculate_homographic();

  inline const Eigen::Matrix<double, 3, 4>& get_camera_intrinsic() { return camera_intrinsic_; }

  inline const Eigen::Matrix<double, 4, 4>& get_camera_extrinsics() { return *_camera2car_pose; }

  inline const Eigen::Matrix<double, 4, 4>& get_car2camera_extrinsic() { return *_car2camera_pose; }

  void SetCar2CameraExtrinsicsAdj(Eigen::Matrix<double, 4, 4> matrix, bool adjusted) {
  ...
  inline CameraUndistortionPtr get_camera_undistort_handler() { return undistort_handler_; }
  ...
  Eigen::Matrix<double, 3, 3> get_car2camera_homography_mat() { return homography_mat_inverse_; }

  Eigen::Matrix<double, 3, 4> get_camera_projection_mat() { return camera_projection_mat_; }

  inline void get_image_height_width(int32_t* image_height, int32_t* image_width) {
 ...
 private:
  ...
  std::shared_ptr<Eigen::Matrix<double, 4, 4>> _camera2car_pose;  // camera to ego car pose
  std::shared_ptr<Eigen::Matrix<double, 4, 4>> _car2camera_pose;  // car to camera pose
  ...
  Eigen::Matrix<double, 3, 3> homography_mat_;          // homography mat from camera 2 car
  Eigen::Matrix<double, 3, 3> homography_mat_inverse_;  // homography mat from car 2 camera
