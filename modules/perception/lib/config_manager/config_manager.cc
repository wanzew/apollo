...
ConfigManager::ConfigManager() {
  ...
  if (work_root_.empty()) {
    ...
    if (work_root_.empty()) { work_root_ = cyber::common::GetEnv("CYBER_PATH"); }
  }
}
  ...
  for (const auto& model_config_file : model_config_files) {
    ...
    for (const std::string& model_config_path : file_list_proto.model_config_path()) {
      ...
      for (const ModelConfigProto& model_config_proto : multi_model_config_proto.model_configs()) {
        ...
        if (!model_config->Reset(model_config_proto)) { return false; }
...
bool ConfigManager::GetModelConfig(const std::string&  model_name,
  ...
  if (citer == model_config_map_.end()) { return false; }
