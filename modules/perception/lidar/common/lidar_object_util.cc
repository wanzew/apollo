...
void GetBoundingBox2d(const std::shared_ptr<Object>& object,
  ...
  box->at(0).x = half_length * direction(0) + half_width * odirection(0) + object->center(0);
  box->at(0).y = half_length * direction(1) + half_width * odirection(1) + object->center(1);
  box->at(1).x = -half_length * direction(0) + half_width * odirection(0) + object->center(0);
  box->at(1).y = -half_length * direction(1) + half_width * odirection(1) + object->center(1);
  box->at(2).x = -half_length * direction(0) - half_width * odirection(0) + object->center(0);
  box->at(2).y = -half_length * direction(1) - half_width * odirection(1) + object->center(1);
  box->at(3).x = half_length * direction(0) - half_width * odirection(0) + object->center(0);
  box->at(3).y = half_length * direction(1) - half_width * odirection(1) + object->center(1);
}
...
void ComputeObjectShapeFromPolygon(std::shared_ptr<Object> object, bool use_world_cloud) {
  ...
  object->size << static_cast<float>(max_polygon_xy(0) - min_polygon_xy(0)),
      static_cast<float>(max_polygon_xy(1) - min_polygon_xy(1)), static_cast<float>(max_z - min_z);
  ...
  polygon_xy = projected_polygon_xy(0) * direction + projected_polygon_xy(1) * odirection;
