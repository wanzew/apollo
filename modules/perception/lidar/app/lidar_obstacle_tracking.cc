...
bool LidarObstacleTracking::Init(const LidarObstacleTrackingInitOptions& options) {
  ...
  config_file = cyber::common::GetAbsolutePath(config_file, "lidar_obstacle_tracking.conf");
...
LidarProcessResult LidarObstacleTracking::Process(const LidarObstacleTrackingOptions& options,
                                                  LidarFrame*                         frame) {
  ...
  if (!multi_target_tracker_->Track(tracker_options, frame)) {
    return LidarProcessResult(LidarErrorCode::TrackerError, "Fail to track objects.");
  }
  ...
  if (!fusion_classifier_->Classify(fusion_classifier_options, frame)) {
    return LidarProcessResult(LidarErrorCode::ClassifierError, "Fail to fuse object types.");
  }
