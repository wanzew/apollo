...
bool FusedClassifier::Init(const ClassifierInitOptions& options) {
  ...
  one_shot_fuser_   = BaseOneShotTypeFusionRegisterer::GetInstanceByName(one_shot_fusion_method_);
  bool init_success = true;
  CHECK_NOTNULL(one_shot_fuser_);
  ACHECK(one_shot_fuser_->Init(init_option_));
  sequence_fuser_ = BaseSequenceTypeFusionRegisterer::GetInstanceByName(sequence_fusion_method_);
  CHECK_NOTNULL(sequence_fuser_);
  ACHECK(sequence_fuser_->Init(init_option_));
  return init_success;
}

bool FusedClassifier::Classify(const ClassifierOptions& options, LidarFrame* frame) {
  if (frame == nullptr) { return false; }
  std::vector<ObjectPtr>* objects =
      use_tracked_objects_ ? &(frame->tracked_objects) : &(frame->segmented_objects);
  if (enable_temporal_fusion_ && frame->timestamp > 0.0) {
    ...
    for (auto& object : *objects) {
      if (object->lidar_supplement.is_background) {
        ...
        object->type_probs[static_cast<int>(ObjectType::UNKNOWN_UNMOVABLE)] = 1.0;
        continue;
      }
