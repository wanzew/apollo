...
void FusedClassifierTest::GenerateSmoothProb(std::vector<float>* prob, std::size_t id, float seed) {
...
void FusedClassifierTest::BuildBackground(base::ObjectPtr* object, int track_id) {
  object->reset(new base::Object);
  (*object)->track_id                       = track_id;
  (*object)->lidar_supplement.is_background = 1;
}
void FusedClassifierTest::BuildOneMethod(base::ObjectPtr* object, int track_id) {
  object->reset(new base::Object);
  (*object)->track_id = track_id;
...
void FusedClassifierTest::BuildObjectsBadTimestamp() {
  ...
  for (size_t i = 0; i < kSequenceLength; ++i) {
    ...
    for (size_t j = 0; j < kObjectNum - 1; ++j) {
      ...
      ls.raw_probs.resize(
          2, std::vector<float>(static_cast<int>(base::ObjectType::MAX_OBJECT_TYPE), 0.f));
...
void FusedClassifierTest::BuildObjects() {
  ...
  BuildBackground(&frames_[0].segmented_objects[kObjectNum - 1], static_cast<int>(kObjectNum - 1));
  BuildOneMethod(&frames_[1].segmented_objects[kObjectNum - 1], static_cast<int>(kObjectNum));
  BuildNoMethod(&frames_[2].segmented_objects[kObjectNum - 1], static_cast<int>(kObjectNum - 1));
  for (size_t i = 3; i < kSequenceLength - 1; ++i) {
    BuildBackground(&frames_[i].segmented_objects[kObjectNum - 1],
                    static_cast<int>(kObjectNum - 1));
  }
  BuildOneMethod(&frames_[kSequenceLength - 1].segmented_objects[kObjectNum - 1],
                 static_cast<int>(kObjectNum + 1));
  timestamps_[kSequenceLength - 1] = static_cast<double>(kSequenceLength - 1) * 0.1 + 3.0;
  timestamps_[kSequenceLength - 1] = static_cast<double>(kSequenceLength - 1) * 0.1 + 11.0;
}
...
TEST_F(FusedClassifierTest, test_one_shot_fusion) {
  ...
  for (auto& i : instances) {
    ...
    for (size_t n = 0; n < kSequenceLength; ++n) {
      ...
      for (size_t j = 0; j < kObjectNum - 1; ++j) {
        EXPECT_EQ(static_cast<size_t>(frames_[n].segmented_objects[j]->type), IdMap(j));
      }
    }
  }
}
