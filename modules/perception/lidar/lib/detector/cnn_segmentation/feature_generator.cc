...
bool FeatureGenerator::Init(const FeatureParam& feature_param, base::Blob<float>* out_blob) {
...
#if USE_GPU == 1
  log_blob_.reset(new base::Blob<float>(1, 1, 1, static_cast<int>(log_table_.size())));
...
#if USE_GPU == 1
    cudaMemcpy(direction_data_, direction_data.data(), direction_data.size() * sizeof(float),
               cudaMemcpyHostToDevice);
    cudaMemcpy(distance_data_, distance_data.data(), distance_data.size() * sizeof(float),
               cudaMemcpyHostToDevice);
#else
    memcpy(direction_data_, direction_data.data(), direction_data.size() * sizeof(float));
    memcpy(distance_data_, distance_data.data(), distance_data.size() * sizeof(float));
#endif
  }
  return true;
}
...
void FeatureGenerator::GenerateCPU(const base::PointFCloudPtr& pc_ptr,
  ...
  for (size_t i = 0; i < pc_ptr->size(); ++i) {
    ...
    if (max_height_data_[idx] < pz) {
      ...
      if (use_intensity_feature_) { top_intensity_data_[idx] = pi; }
    }
    mean_height_data_[idx] += static_cast<float>(pz);
    if (use_intensity_feature_) { mean_intensity_data_[idx] += static_cast<float>(pi); }
    count_data_[idx] += 1.f;
  }
  ...
  for (int i = 0; i < map_size; ++i) {
    if (count_data_[i] <= std::numeric_limits<float>::epsilon()) {
      ...
      if (use_intensity_feature_) { mean_intensity_data_[i] /= count_data_[i]; }
