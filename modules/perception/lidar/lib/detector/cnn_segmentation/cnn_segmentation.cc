...
bool CNNSegmentation::Init(const LidarDetectorInitOptions& options) {
  ...
  inference_.reset(inference::CreateInferenceByName(cnnseg_param_.model_type(), proto_file,
                                                    weight_file, output_names, input_names));
  ...
  if (!feature_param.use_intensity_feature()) { input_shape[1] -= 2; }
  if (!feature_param.use_constant_feature()) { input_shape[1] -= 2; }
...
bool CNNSegmentation::InitClusterAndBackgroundSegmentation() {
  ...
  ground_detector_ =
      BaseGroundDetectorRegisterer::GetInstanceByName(cnnseg_param_.ground_detector());
  ...
  if (height_pt_blob_ != nullptr) { spp_data.height_pt_blob = height_pt_blob_.get(); }
  if (cnnseg_param_.do_classification()) { spp_data.classify_pt_blob = classify_pt_blob_.get(); }
  if (cnnseg_param_.do_heading()) { spp_data.heading_pt_blob = heading_pt_blob_.get(); }
  ...
  worker_.Bind([&]() {
    ...
    if (lidar_frame_ref_->cloud != original_cloud_) { lidar_frame_ref_->cloud = original_cloud_; }
...
void CNNSegmentation::MapPointToGrid(const std::shared_ptr<AttributePointCloud<PointF>>& pc_ptr) {
  ...
  for (size_t i = 0; i < pc_ptr->size(); ++i) {
    ...
    if (pos_y < 0 || pos_y >= height_ || pos_x < 0 || pos_x >= width_) { continue; }
    point2grid_[i] = pos_y * width_ + pos_x;
  }
}

bool CNNSegmentation::Detect(const LidarDetectorOptions& options, LidarFrame* frame) {
  // check input
  if (frame == nullptr) {
...
void CNNSegmentation::GetObjectsFromSppEngine(std::vector<std::shared_ptr<Object>>* objects) {
  ...
  memcpy(&original_world_cloud_->mutable_points_height()->at(0),
         &original_cloud_->points_height().at(0), sizeof(float) * original_cloud_->size());
  memcpy(&original_world_cloud_->mutable_points_label()->at(0),
         &original_cloud_->points_label().at(0), sizeof(uint8_t) * original_cloud_->size());
  if (cnnseg_param_.remove_ground_points()) {
    num_foreground = spp_engine_.RemoveGroundPointsInForegroundCluster(
        original_cloud_, lidar_frame_ref_->roi_indices, lidar_frame_ref_->non_ground_indices);
    if (num_foreground == 0) { ADEBUG << "No foreground segmentation output"; }
  }
