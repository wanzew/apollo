...
bool SppCCDetector::BuildNodes(int start_row_index, int end_row_index) {
  ...
  for (int row = start_row_index; row < end_row_index; ++row) {
    for (int col = 0; col < cols_; ++col) {
      ...
      int center_row =
          static_cast<int>(*offset_row_ptr++ * scale_ + static_cast<float>(row) + 0.5f);
      int center_col =
          static_cast<int>(*offset_col_ptr++ * scale_ + static_cast<float>(col) + 0.5f);
...
void SppCCDetector::TraverseNodes() {
  for (int row = 0; row < rows_; row++) {
    for (int col = 0; col < cols_; col++) {
      ...
      if (node.is_object() && node.get_traversed() == 0) { Traverse(&node); }
    }
  }
}
  ...
  for (int row = 0; row < rows_; ++row) {
    for (int col = 0; col < cols_; ++col) {
      ...
      if (col < cols_ - 1) {
        ...
        if (node_neighbor->is_center()) { DisjointSetUnion(node, node_neighbor); }
      }
      // down
      if (row < rows_ - 1) {
...
SppCCDetector::Node* SppCCDetector::DisjointSetFind(Node* x) {
  ...
  if (y == x || nodes_[0] + y->parent == y) { return y; }
