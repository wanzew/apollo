...
class TestClass {
 public:
  ...
  void PclXYZITToArray(const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_pcl_pc_ptr,
                       float*                                      out_points_array,
                       const float                                 normalizing_factor = 1.0);
  ...
  void PreprocessGPU(const float* in_points_array,
                     int          in_num_points,
                     int*         x_coors,
                     int*         y_coors,
                     float*       num_points_per_pillar,
                     float*       pillar_point_feature,
                     float*       pillar_coors,
                     int*         sparse_pillar_map,
                     int*         host_pillar_count);
  void GenerateAnchors(float* anchors_px,
                       float* anchors_py,
                       float* anchors_pz,
                       float* anchors_dx,
                       float* anchors_dy,
...
TestClass::TestClass()
  ...
  preprocess_points_ptr_.reset(new PreprocessPoints(
      max_num_pillars, max_num_points_per_pillar, num_point_feature, grid_x_size, grid_y_size,
      grid_z_size, pillar_x_size, pillar_y_size, pillar_z_size, min_x_range, min_y_range,
      min_z_range, num_inds_for_scan));
  preprocess_points_cuda_ptr_.reset(new PreprocessPointsCuda(
      num_threads, max_num_pillars, max_num_points_per_pillar, num_point_feature, num_inds_for_scan,
      grid_x_size, grid_y_size, grid_z_size, pillar_x_size, pillar_y_size, pillar_z_size,
...
TestClass::TestClass(const int   num_class,
                     const int   max_num_pillars,
                     const int   max_num_points_per_pillar,
                     const int   num_point_feature,
                     const int   grid_x_size,
...
void TestClass::Preprocess(const float* in_points_array,
                           int          in_num_points,
                           int*         x_coors,
                           int*         y_coors,
                           float*       num_points_per_pillar,
...
void TestClass::PreprocessGPU(const float* in_points_array,
                              int          in_num_points,
                              int*         x_coors,
                              int*         y_coors,
                              float*       num_points_per_pillar,
...
void TestClass::PclToArray(const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_pcl_pc_ptr,
                           float*                                      out_points_array,
...
void TestClass::PclXYZITToArray(const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_pcl_pc_ptr,
                                float*                                      out_points_array,
                                const float                                 normalizing_factor) {
  for (size_t i = 0; i < in_pcl_pc_ptr->size(); ++i) {
    pcl::PointXYZI point        = in_pcl_pc_ptr->at(i);
    ...
    out_points_array[i * 5 + 3] = static_cast<float>(point.intensity / normalizing_factor);
...
void TestClass::GenerateAnchors(float* anchors_px,
                                float* anchors_py,
                                float* anchors_pz,
                                float* anchors_dx,
                                float* anchors_dy,
...
void TestClass::ConvertAnchors2BoxAnchors(float* anchors_px,
                                          float* anchors_py,
                                          float* box_anchors_min_x,
                                          float* box_anchors_min_y,
                                          float* box_anchors_max_x,
                                          float* box_anchors_max_y) {
  return point_pillars_ptr_->ConvertAnchors2BoxAnchors(anchors_px, anchors_py, box_anchors_min_x,
...
void TestClass::DoInference(const float*        in_points_array,
                            ...
                            std::vector<float>* out_detections,
                            std::vector<int>*   out_labels) {
  return point_pillars_ptr_->DoInference(in_points_array, in_num_points, out_detections,
                                         out_labels);
}
/*
...
TEST(TestSuite, CheckDoInference) {
  ...
  apollo::perception::benchmark::PointCloudPtr org_cloud_ptr(
      new pcl::PointCloud<apollo::perception::benchmark::PointXYZIL>);
  std::string file_name = "/apollo/modules/perception/testdata/lidar/app/data/0001_00.pcd";
  ...
  std::vector<float> out_detections;
  std::vector<int>   out_labels;
  test_obj.DoInference(points_array, pcl_pc_ptr->size(), &out_detections, &out_labels);
  ...
  for (int j = 0; j < num_objects; ++j) {
    ...
    std::cout << "object id: " << j << ", x: " << x << ", y: " << y << ", z: " << z
              << ", dx: " << dx << ", dy: " << dy << ", dz: " << dz << ", yaw: " << yaw
