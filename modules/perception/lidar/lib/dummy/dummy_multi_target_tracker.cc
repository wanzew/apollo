...
bool DummyMultiTargetTracker::Init(const MultiTargetTrackerInitOptions& options) { return true; }

bool DummyMultiTargetTracker::Track(const MultiTargetTrackerOptions& options, LidarFrame* frame) {
  if (frame == nullptr) { return false; }
  // transform objects
  frame->tracked_objects.clear();
  base::ObjectPool::Instance().BatchGet(frame->segmented_objects.size(), &frame->tracked_objects);
  for (size_t i = 0; i < frame->segmented_objects.size(); i++) {
    *(frame->tracked_objects[i])        = *(frame->segmented_objects[i]);
    ...
    obj->lidar_supplement.cloud_world.resize(obj->lidar_supplement.cloud.size());
    ...
    common::TransformPointCloud(pose, (base::AttributePointCloud<base::PointD>*)&obj->polygon);
