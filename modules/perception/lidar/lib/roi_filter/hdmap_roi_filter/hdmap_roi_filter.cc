...
bool HdmapROIFilter::Init(const ROIFilterInitOptions& options) {
  ...
  AINFO << " HDMap Roi Filter Parameters: "
        << " range: " << range_ << " cell_size: " << cell_size_ << " extend_dist: " << extend_dist_
        << " no_edge_table: " << no_edge_table_ << " set_roi_service: " << set_roi_service_;
  return true;
}
...
bool HdmapROIFilter::Filter(const ROIFilterOptions& options, LidarFrame* frame) {
  ...
  TransformFrame(frame->cloud, frame->lidar2world_pose, polygons_world_, &polygons_local_,
                 &cloud_local);
  ...
  if (ret) {
    for (auto index : frame->roi_indices.indices) {
      frame->cloud->mutable_points_label()->at(index) = static_cast<uint8_t>(LidarPointLabel::ROI);
  ...
  if (set_roi_service_) {
    ...
    if (roi_service != nullptr) {
      ...
      if (!ret) {
        std::fill(roi_service_content_.bitmap_.begin(), roi_service_content_.bitmap_.end(), -1);
      }
...
bool HdmapROIFilter::FilterWithPolygonMask(const base::PointFCloudPtr&      cloud,
  ...
  if ((max_y - min_y) < (max_x - min_x)) { major_dir = DirectionMajor::YMAJOR; }
...
void HdmapROIFilter::TransformFrame(const base::PointFCloudPtr&       cloud,
...
bool HdmapROIFilter::Bitmap2dFilter(const base::PointFCloudPtr& in_cloud,
  ...
  for (size_t i = 0; i < in_cloud->size(); ++i) {
    ...
    if (!bitmap.IsExists(e_pt)) { continue; }
    if (bitmap.Check(e_pt)) { roi_indices->indices.push_back(static_cast<int>(i)); }
  }
  return true;
}
