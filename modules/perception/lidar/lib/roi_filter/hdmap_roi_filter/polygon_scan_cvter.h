 ...
 public:
  ...
  struct Edge {
    ...
    bool MoveUp(double delta_x) {
      if (delta_x < 0 || !std::isfinite(k)) { return false; }
      x += delta_x;
    ...
    friend std::ostream& operator<<(std::ostream& out, const Edge& edge) {
      out << boost::format("max_x: %13.10lf max_y: %13.10lf"
                           "min_x: %13.10lf min_y: %13.10lf"
                           "x: %13.10lf y: %13.10lf k %13.10lf") %
                 edge.max_x % edge.max_y % edge.min_x % edge.min_y % edge.x % edge.y % edge.k;
      return out;
    }
  ...
  static inline DirectionMajor OppositeDirection(const DirectionMajor dir_major) {
  ...
  void
       ScanCvt(const T& scan_loc, DirectionMajor dir_major, std::vector<IntervalOut>* scan_intervals);
...
template <typename T>
const double PolygonScanCvter<T>::s_epsilon_ = std::numeric_limits<float>::epsilon();
...
template <typename T>
void PolygonScanCvter<T>::ScanCvt(const T&                  scan_loc,
    ...
    if (std::isfinite(k)) {
      if ((x >= low_x && x < high_x) || (x <= low_x && x > high_x)) {
        if (x == low_x) {
          ...
          if (is_singular[i]) { nodes.push_back(low_y); }
        } else {
          nodes.push_back(low_y + (x - low_x) * k);
        }
      }
    } else {
      if (std::abs(x - low_x) < s_epsilon_ || std::abs(x - high_x) < s_epsilon_) {
        if (low_y < high_y) {
...
template <typename T>
void PolygonScanCvter<T>::ScansCvt(const IntervalIn&                      scans_interval,
                                   const DirectionMajor                   dir_major,
                                   const T&                               step,
                                   std::vector<std::vector<IntervalOut>>* scans_intervals) {
...
template <typename T>
void PolygonScanCvter<T>::ParsePolygon(const DirectionMajor dir_major, const bool disturb) {
  polygon_disturbed_ = polygon_;
  if (disturb) { DisturbPolygon(dir_major); }
  DirectionMajor op_dir_major  = OppositeDirection(dir_major);
  ...
  for (size_t i = 0; i < vertices_size; ++i) {
    const Point& pre_vertex = polygon_disturbed_[(i + vertices_size - 1) % vertices_size];
    ...
    std::abs(x_diff) < s_epsilon_ ? ks.push_back(s_inf_) : ks.push_back(y_diff / x_diff);
    ...
    if (std::abs(x_diff) < s_epsilon_ || std::abs(pre_x_diff) < s_epsilon_) {
      ...
      pre_x_diff* x_diff > 0 ? is_singular.push_back(true) : is_singular.push_back(false);
    }
  }
}
...
void PolygonScanCvter<T>::BuildEt() {
  ...
  for (size_t i = 0; i < segments.size(); ++i) {
    ...
    if (ConvertSegment(i, &(out_edge))) { edges.push_back(out_edge); }
  }
  ...
  for (size_t i = 0; i < edges.size(); ++i) {
    ...
    if (x_id >= static_cast<int>(scans_size_)) { continue; }
    // add x_id == 0 to act
    if (x_id >= 0) {
      ...
      if (active_edge.MoveUp(0.0 - active_edge.x)) { aet_.second.push_back(active_edge); }
    }
  }
}
...
void PolygonScanCvter<T>::UpdateAet(std::vector<IntervalOut>* scan_intervals) {
  ...
  CHECK_EQ(valid_edges_num & 1, static_cast<size_t>(0))
      << boost::format("valid edges num: %d x: %lf bottom_x: %lf \n vertices num: %d "
                       "\n") %
...
template <typename T>
bool PolygonScanCvter<T>::ConvertSegment(const size_t seg_id, std::pair<int, Edge>* out_edge) {
  const auto& segments = segments_[dir_major_];
  ...
  if (std::isfinite(edge.k)) {
    ...
    edge.y = min_y;
    // swap
    if (edge.y > edge.max_y) { std::swap(edge.y, edge.max_y); }
  }
  edge.min_y = edge.y;

  // save top edge
  if (static_cast<size_t>(x_id) >= scans_size_) {
    std::pair<double, double> seg(low_vertex[op_dir_major_], high_vertex[op_dir_major_]);
    top_segments_.push_back(seg);
  }
  // check edge valid, for length < step
  if (std::isfinite(edge.k) && edge.max_x < edge.x) { return false; }
  return true;
}
