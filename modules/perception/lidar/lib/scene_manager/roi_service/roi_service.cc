...
void ROIServiceContent::SetContent(const SceneServiceContent& content) {
  const ROIServiceContent* roi_content = dynamic_cast<const ROIServiceContent*>(&content);
  if (roi_content == nullptr) { return; }
  // copy content
...
inline bool ROIServiceContent::CheckBit(const size_t loc, const uint64_t block) const {
  return block & (static_cast<uint64_t>(1) << loc);
}

bool ROIServiceContent::Check(const Eigen::Vector3d& world_point) const {
  if (!service_ready_) { return false; }
  Eigen::Vector2d pt;
  pt(0) = world_point(0) - transform_(0) + range_;
  pt(1) = world_point(1) - transform_(1) + range_;
  if (pt(0) >= 0 && pt(0) < 2 * range_ && pt(1) >= 0 && pt(1) < 2 * range_) {
    const size_t ix    = static_cast<size_t>(pt(static_cast<size_t>(major_dir_)) / cell_size_);
    const size_t iy    = static_cast<size_t>(pt(static_cast<size_t>(major_dir_) ^ 1) / cell_size_);
