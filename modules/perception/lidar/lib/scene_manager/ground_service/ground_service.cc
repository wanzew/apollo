...
void GroundServiceContent::GetCopy(SceneServiceContent* content) const {
  GroundServiceContent* ground_content = dynamic_cast<GroundServiceContent*>(content);
  if (ground_content == nullptr) { return; }
...
void GroundServiceContent::SetContent(const SceneServiceContent& content) {
  const GroundServiceContent* ground_content = dynamic_cast<const GroundServiceContent*>(&content);
  if (ground_content == nullptr) { return; }
...
uint32_t inline GetIndex(uint32_t r, uint32_t c, uint32_t cols) { return (r * cols + c); }
...
bool GroundServiceContent::PointToGrid(const Eigen::Vector3d& world_point,
  ...
  if (x < bound_x_min_ || x > bound_x_max_ || y < bound_y_min_ || y > bound_y_max_) {
    (*grid_index) = 0;
    return false;
  }
  uint32_t c    = std::min(cols_ - 1, static_cast<uint32_t>((x - bound_x_min_) / resolution_x_));
  uint32_t r    = std::min(rows_ - 1, static_cast<uint32_t>((y - bound_y_min_) / resolution_y_));
  (*grid_index) = GetIndex(r, c, cols_);
  return true;
}

float GroundServiceContent::PointToPlaneDistance(const Eigen::Vector3d& world_point) const {
  uint32_t grid_index = 0;
  if (!PointToGrid(world_point, &grid_index)) { return std::numeric_limits<float>::max(); }
...
bool GroundServiceContent::Init(double roi_x, double roi_y, uint32_t rows, uint32_t cols) {
  bound_x_min_ = -roi_x;
  bound_y_min_ = -roi_y;
  bound_x_max_ = roi_x;
  bound_y_max_ = roi_y;
...
bool GroundService::Init(const SceneServiceInitOptions& options) {
  ...
  ground_content_ref_ = dynamic_cast<GroundServiceContent*>(self_content_.get());
...
float GroundService::QueryPointToGroundDistance(const Eigen::Vector3d& world_point) {
  std::lock_guard<std::mutex> lock(mutex_);
  float distance = QueryPointToGroundDistance(world_point, *ground_content_ref_);
  return distance;
}
