...
bool MlfMotionRefiner::Refine(const MlfTrackDataConstPtr& track_data, TrackedObjectPtr new_object) {
  ...
  bool need_keep_motion = filter_acceleration_gain > claping_acceleration_threshold_;
  ...
  bool is_static_hypothesis = CheckStaticHypothesisByState(latest_object, new_object);
...
bool MlfMotionRefiner::CheckStaticHypothesisByState(const TrackedObjectConstPtr& latest_object,
  ...
  if (velocity_noise_level_is_1 && latest_object->type != base::ObjectType::PEDESTRIAN) {
    velocity_noise_level_is_0 = true;
  }
  if (velocity_noise_level_is_0) { return true; }
  ...
  if (velocity_noise_level_is_2 && latest_object->type != base::ObjectType::PEDESTRIAN) {
    velocity_noise_level_is_1 = true;
  }
  double reasonable_angle_change_maximum_0 = M_PI / 6;
  bool   velocity_angle_change_level_is_0  = CheckStaticHypothesisByVelocityAngleChange(
      latest_object, new_object, reasonable_angle_change_maximum_0);
  if (velocity_noise_level_is_1 && velocity_angle_change_level_is_0) { return true; }
  // believe track is static if velocity noise level is
  // 2 && angle change level is 1
  // use loose threshold for object is not pedestrian
  if (velocity_noise_level_is_3 && latest_object->type != base::ObjectType::PEDESTRIAN) {
    velocity_noise_level_is_2 = true;
  }
  double reasonable_angle_change_maximum_1 = M_PI / 4;
  bool   velocity_angle_change_level_is_1  = CheckStaticHypothesisByVelocityAngleChange(
      latest_object, new_object, reasonable_angle_change_maximum_1);
  if (velocity_noise_level_is_2 && velocity_angle_change_level_is_1) { return true; }
  return false;
}
...
bool MlfMotionRefiner::CheckStaticHypothesisByVelocityAngleChange(
  ...
  if (current_velocity.norm() < kEpsilon) { return true; }
  // believe angle change is obvious if it is greater than threshold

  double velocity_angle_change = common::CalculateTheta2DXY(previous_velocity, current_velocity);
  if (fabs(velocity_angle_change) > reasonable_angle_change_maximum) { return true; }
  ...
  velocity_angle_change =
      std::min(fabs(velocity_heading_angle_change_1), fabs(velocity_heading_angle_change_2));
  if (fabs(velocity_angle_change) > reasonable_angle_change_maximum) { return true; }
  return false;
}
