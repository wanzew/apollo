...
void MlfMotionMeasurement::ComputeMotionMeasurment(const MlfTrackDataConstPtr& track_data,
                                                   TrackedObjectPtr            new_object) {
  // prefer to choose objects from the same sensor
  std::string           sensor_name   = new_object->sensor_info.name;
  TrackedObjectConstPtr latest_object = track_data->GetLatestSensorObject(sensor_name).second;
  if (latest_object == nullptr) { latest_object = track_data->GetLatestObject().second; }
  ...
  if (fabs(time_diff) < EPSILON_TIME) { time_diff = DEFAULT_FPS; }
...
void MlfMotionMeasurement::MeasurementSelection(const MlfTrackDataConstPtr&  track_data,
  ...
  for (int i = 0; i < 4; ++i) {
    corner_velocity_gain_norms[i] = static_cast<float>(
        (new_object->measured_corners_velocity[i] - latest_object->belief_velocity).norm());
  }
  std::vector<float>::iterator corener_min_gain = std::min_element(
      std::begin(corner_velocity_gain_norms), std::end(corner_velocity_gain_norms));
  ...
  velocity_gain_norms[1] = static_cast<float>(
      (new_object->measured_barycenter_velocity - latest_object->belief_velocity).norm());
  velocity_gain_norms[2] = static_cast<float>(
      (new_object->measured_center_velocity - latest_object->belief_velocity).norm());
  std::vector<float>::iterator min_gain =
      std::min_element(std::begin(velocity_gain_norms), std::end(velocity_gain_norms));
  int64_t min_gain_pos = min_gain - velocity_gain_norms.begin();
  if (min_gain_pos == 0) {
    new_object->selected_measured_velocity = new_object->measured_corners_velocity[corner_index];
  }
  if (min_gain_pos == 1) {
    new_object->selected_measured_velocity = new_object->measured_barycenter_velocity;
  }
  if (min_gain_pos == 2) {
    new_object->selected_measured_velocity = new_object->measured_center_velocity;
  }
}

void MlfMotionMeasurement::MeasurementQualityEstimation(const TrackedObjectConstPtr& latest_object,
                                                        TrackedObjectPtr             new_object) {
  // 1. point size diff (only for same sensor)
  int    pre_num = static_cast<int>(latest_object->object_ptr->lidar_supplement.cloud_world.size());
  int    cur_num = static_cast<int>(new_object->object_ptr->lidar_supplement.cloud_world.size());
  ...
  double quality_based_on_association_score = pow(1.0 - new_object->association_score, 2.0);
  new_object->update_quality =
      std::min(quality_based_on_association_score, quality_based_on_point_diff_ratio);
}
