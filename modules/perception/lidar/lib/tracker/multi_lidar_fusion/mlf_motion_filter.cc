...
void MlfMotionFilter::UpdateWithObject(const MlfFilterOptions&     options,
  ...
  if (!use_adaptive_) { new_object->update_quality = 1.0; }
  TrackedObjectConstPtr latest_object = track_data->GetLatestObject().second;
  // 2. kalman filter update
  KalmanFilterUpdateWithPartialObservation(track_data, latest_object, new_object);
...
void MlfMotionFilter::InitializeTrackState(TrackedObjectPtr new_object) {
  new_object->boostup_need_history_size      = static_cast<int>(boostup_history_size_minimum_);
  ...
  new_object->velocity_covariance = Eigen::Matrix3d::Identity() * init_velocity_variance_;
  new_object->belief_velocity_online_covariance = new_object->velocity_covariance;
...
void MlfMotionFilter::KalmanFilterUpdateWithPartialObservation(
  ...
  const Eigen::Matrix4d& last_state_covariance = latest_object->state_covariance;

  double time_diff =
      new_object->object_ptr->latest_tracked_time - latest_object->object_ptr->latest_tracked_time;
  ...
  if (new_object->type != base::ObjectType::PEDESTRIAN && range < trust_orientation_range_) {
  ...
  auto  measurement_covariance = new_object->measurement_covariance.block<2, 2>(0, 0);
  // 1. prediction stage
  Eigen::Matrix4d predict_covariance =
      Eigen::Matrix4d::Identity() * predict_variance_per_sqrsec_ * time_diff * time_diff;
  state = transition * last_state;
  state_covariance =
      transition * last_state_covariance * transition.transpose() + predict_covariance;
  ...
  if (new_object->type == base::ObjectType::PEDESTRIAN && range < trust_orientation_range_) {
    ...
    measurement_covariance =
        ...
        (measured_velocity_variance_ + fabs(measurement.dot(odirection)) * kVarianceAmplifier) *
  ...
  Eigen::Matrix<double, 4, 2> kalman_gain_matrix =
      static_cast<Eigen::Matrix<double, 4, 2, 0, 4, 2>>(
          ...
          (observation_transform * state_covariance * observation_transform.transpose() +
           measurement_covariance)
              .inverse());
  ...
  state_covariance =
      (Eigen::Matrix4d::Identity() - kalman_gain_matrix * observation_transform) * state_covariance;

  // 4. finally, state to belief and output to keep consistency
  new_object->belief_velocity_gain << state_gain.head<2>(), 0;
}

void MlfMotionFilter::StateGainAdjustment(const MlfTrackDataConstPtr&  track_data,
                                          const TrackedObjectConstPtr& latest_object,
                                          const TrackedObjectConstPtr& new_object,
                                          Eigen::Vector4d*             gain) {
  // 1 quality penalize and gain break down
  if (use_adaptive_) { *gain *= new_object->update_quality; }
...
void MlfMotionFilter::BeliefToOutput(TrackedObjectPtr object) {
  ...
  object->output_velocity_uncertainty = object->belief_velocity_online_covariance;
}
...
void MlfMotionFilter::OnlineCovarianceEstimation(const MlfTrackDataConstPtr& track_data,
                                                 TrackedObjectPtr            object) {
  object->belief_velocity_online_covariance = Eigen::Matrix3d::Zero();
  size_t evaluate_window                    = std::min(track_data->history_objects_.size(),
  ...
  if (evaluate_window == 0) {
    ...
    object->belief_velocity_online_covariance =
        Eigen::Matrix3d::Identity() * predict_variance_per_sqrsec_ * predict_variance_per_sqrsec_;
    return;
  }
  ...
  for (size_t i = 0; i < evaluate_window; ++i) {
    Eigen::Vector3d velocity_resisual =
        cur_obj_pair->second->selected_measured_velocity - object->output_velocity;
    if (velocity_resisual.head(2).norm() < std::numeric_limits<double>::epsilon()) {
      velocity_resisual = Eigen::Vector3d::Identity() * noise_maximum_ * noise_maximum_;
    }
    ++cur_obj_pair;
    object->belief_velocity_online_covariance.block<2, 2>(0, 0) +=
        velocity_resisual.head<2>() * velocity_resisual.head<2>().transpose();
  }
  object->belief_velocity_online_covariance /= static_cast<double>(evaluate_window);
  object->belief_velocity_online_covariance +=
      Eigen::Matrix3d::Identity() * noise_maximum_ * noise_maximum_;
  object->output_velocity_uncertainty = object->belief_velocity_online_covariance;
}

void MlfMotionFilter::ConvergenceEstimationAndBoostUp(const MlfTrackDataConstPtr&  track_data,
                                                      const TrackedObjectConstPtr& latest_object,
                                                      TrackedObjectPtr             new_object) {
  new_object->boostup_need_history_size = latest_object->boostup_need_history_size;
  ...
  if (window_size < static_cast<size_t>(new_object->boostup_need_history_size)) { return; }
...
void MlfMotionFilter::ComputeConvergenceConfidence(const MlfTrackDataConstPtr& track_data,
                                                   TrackedObjectPtr            new_object,
  ...
  Eigen::Vector3d velocity_residual =
      ...
      (velocity_source_is_belief ? new_object->belief_velocity : new_object->output_velocity);
  convergence_score_list[0] =
      base_convergence_noise / std::max(base_convergence_noise, velocity_residual.norm());

  // history objects score
  size_t visible_obj_idx = 1;
  for (auto cur_object_pair = history_objects.rbegin(); cur_object_pair != history_objects.rend();
       ++cur_object_pair) {
  ...
  if (!new_object->converged) {
    ...
    for (size_t i = 0; i < static_cast<size_t>(boostup_need_history_size); ++i) {
      if (min_convergence_confidence_score > convergence_score_list[i]) {
        min_convergence_confidence_score = convergence_score_list[i];
      }
    }
    new_object->convergence_confidence = static_cast<float>(min_convergence_confidence_score);
  } else {
    ...
    new_object->convergence_confidence = static_cast<float>(mean_convergence_confidence_score);
  }
}
...
void MlfMotionFilter::BoostupState(const MlfTrackDataConstPtr& track_data,
  ...
  int actual_boostup_history_size =
      static_cast<size_t>(boostup_need_history_size) > track_data->history_objects_.size() ?
  ...
  while (boostup_used_history_size < actual_boostup_history_size &&
    ...
    Eigen::Vector3d measured_velocity_on_project_dir =
        common::Calculate2DXYProjectVector<double>(measured_velocity, new_obj_belief_velocity);
    if (measured_velocity_on_project_dir.dot(new_obj_belief_velocity) <= 0) {
      measured_velocity_on_project_dir = Eigen::Vector3d::Zero();
    }
    double measured_velocity_norm_on_project_dir = measured_velocity_on_project_dir.norm();
    ...
    if (boostup_used_history_size > 1) { ++cur_object_pair; }
    if (cur_object_pair != history_objects.rend()) { cur_object = cur_object_pair->second; }
  }
...
void MlfMotionFilter::UpdateConverged(const MlfTrackDataConstPtr& track_data,
  ...
  if (object->convergence_confidence > converged_confidence_minimum_) {
    ...
    if (static_cast<size_t>(object->boostup_need_history_size) < boostup_history_size_maximum_) {
