...
bool MlfEngine::Track(const MultiTargetTrackerOptions& options, LidarFrame* frame) {
  ...
  SplitAndTransformToTrackedObjects(frame->segmented_objects, frame->sensor_info);
  ...
  bool is_main_sensor = (main_sensor_.find(frame->sensor_info.name) != main_sensor_.end());
  if (is_main_sensor) {
  ...
  if (is_main_sensor) { CollectTrackedResult(frame); }
  // 6. remove stale data
  ...
  AINFO << "MlfEngine publish objects: " << frame->tracked_objects.size()
        << " sensor_name: " << frame->sensor_info.name << " at timestamp: " << frame->timestamp;
  return true;
}

void MlfEngine::SplitAndTransformToTrackedObjects(const std::vector<base::ObjectPtr>& objects,
  ...
  for (size_t i = 0; i < objects.size(); ++i) {
    tracked_objects[i]->AttachObject(objects[i], sensor_to_local_pose_, global_to_local_offset_,
                                     sensor_info);
    if (!objects[i]->lidar_supplement.is_background && use_histogram_for_match_) {
  ...
  AINFO << "MlfEngine: " << sensor_info.name << " foreground: " << foreground_objects_.size()
        << " background: " << background_objects_.size();
}

void MlfEngine::TrackObjectMatchAndAssign(const MlfTrackObjectMatcherOptions&  match_options,
  ...
  matcher_->Match(match_options, objects, *tracks, &assignments, &unassigned_tracks,
                  &unassigned_objects);
  AINFO << "MlfEngine: " + name + " assignments " << assignments.size() << " unassigned_tracks "
        << unassigned_tracks.size() << " unassigned_objects " << unassigned_objects.size();
...
void MlfEngine::TrackStateFilter(const std::vector<MlfTrackDataPtr>& tracks,
  ...
  for (auto& track_data : tracks) {
    ...
    if (objects.empty()) { tracker_->UpdateTrackDataWithoutObject(frame_timestamp, track_data); }
  }
}

void convertPoseToLoc(const Eigen::Affine3d&              pose,
                      localization::LocalizationEstimate* localization) {
  ADEBUG << "translation x y z " << pose.translation()[0] << " " << pose.translation()[1] << " "
         << pose.translation()[2];
  localization->mutable_pose()->mutable_position()->set_x(pose.translation()[0]);
  localization->mutable_pose()->mutable_position()->set_y(pose.translation()[1]);
  localization->mutable_pose()->mutable_position()->set_z(pose.translation()[2]);
...
void MlfEngine::CollectTrackedResult(LidarFrame* frame) {
  ...
  size_t num_objects = foreground_track_data_.size() + background_track_data_.size();
  ...
  if (num_predict != 0) {
    AINFO << "MlfEngine, num_predict: " << num_predict << " num_objects: " << num_objects;
...
void MlfEngine::RemoveStaleTrackData(const std::string&            name,
                                     double                        timestamp,
                                     std::vector<MlfTrackDataPtr>* tracks) {
  size_t pos = 0;
  for (size_t i = 0; i < tracks->size(); ++i) {
    if (tracks->at(i)->latest_visible_time_ + reserved_invisible_time_ >= timestamp) {
      if (i != pos) { tracks->at(pos) = tracks->at(i); }
      ++pos;
    }
  }
  AINFO << "MlfEngine: " << name << " remove stale tracks, from " << tracks->size() << " to "
        << pos;
  tracks->resize(pos);
}
