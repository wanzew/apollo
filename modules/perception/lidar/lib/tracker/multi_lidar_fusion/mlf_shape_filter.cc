...
void MlfShapeFilter::UpdateWithObject(const MlfFilterOptions&     options,
  ...
  if (new_object->is_background) {
    ...
    hull_.GetConvexHullWithoutGroundAndHead(
        obj->lidar_supplement.cloud_world, static_cast<float>(bottom_points_ignore_threshold_),
  ...
  if (track_data->age_ > 0) {
    ...
    if (new_object->direction.dot(latest_object->direction) < 0) { new_object->direction *= -1; }
    static const double kMovingAverage = 0.6;
    new_object->direction              = latest_object->output_direction * (1 - kMovingAverage) +
