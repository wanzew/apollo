...
float LocationDistance(const TrackedObjectConstPtr& last_object,
  ...
  Eigen::Vector3f measured_anchor_point  = (new_object->anchor_point).cast<float>();
  ...
  float location_dist = static_cast<float>(
  ...
  if (speed > 2) {
    ...
    double dx     = ref_dir(0) * measure_predict_diff(0) + ref_dir(1) * measure_predict_diff(1);
    double dy     = ref_o_dir(0) * measure_predict_diff(0) + ref_o_dir(1) * measure_predict_diff(1);
...
float DirectionDistance(const TrackedObjectConstPtr& last_object,
  ...
  Eigen::Vector3f old_anchor_point       = (last_object->belief_anchor_point).cast<float>();
  ...
  if (!track_motion_dir.head(2).isZero() && !anchor_point_shift_dir.head(2).isZero()) {
    ...
    cos_theta = common::CalculateCosTheta2DXY<float>(track_motion_dir, anchor_point_shift_dir);
  }
...
float PointNumDistance(const TrackedObjectConstPtr& last_object,
  ...
  int old_point_number =
      static_cast<int>((last_object->object_ptr->lidar_supplement).cloud_world.size());
  int new_point_number =
      static_cast<int>((new_object->object_ptr->lidar_supplement).cloud_world.size());

  float point_num_dist = static_cast<float>(fabs(old_point_number - new_point_number)) * 1.0f /
...
float HistogramDistance(const TrackedObjectConstPtr& last_object,
  ...
  const std::vector<float>& old_object_shape_features = last_object->shape_features;
  const std::vector<float>& new_object_shape_features = new_object->shape_features;
...
float CentroidShiftDistance(const TrackedObjectConstPtr& last_object,
                            ...
                            const double                 time_diff) {
  float dist =
      static_cast<float>((last_object->barycenter.head(2) - new_object->barycenter.head(2)).norm());
  return dist;
}
...
float BboxIouDistance(const TrackedObjectConstPtr& last_object,
  ...
  base::PointDCloud& cloud               = (new_object->object_ptr->lidar_supplement).cloud_world;
  ...
  Eigen::Vector2d old_center_transformed_2d =
      static_cast<Eigen::Matrix<double, 2, 1, 0, 2, 1>>(trans_mat * old_center.head(2));
  Eigen::Vector2d new_center_transformed_2d =
      static_cast<Eigen::Matrix<double, 2, 1, 0, 2, 1>>(trans_mat * new_center.head(2));
  ...
  double iou = common::CalculateIou2DXY<double>(old_center, old_size_tmp, new_center, new_size_tmp);
  // Step 4: compute dist
