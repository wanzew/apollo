...
void MeasureBboxCenterVelocity(TrackedObjectPtr             new_object,
  ...
  common::CalculateBBoxSizeCenter2DXY(cloud, old_dir_tmp, &new_size_tmp, &new_center,
                                      minimum_edge_length);
  // Eigen::Vector3d old_dir = old_dir_tmp.cast<double>();
  // Eigen::Vector3d new_size = new_size_tmp.cast<double>();
  Eigen::Vector3d measured_bbox_center_velocity_with_old_dir = (new_center - old_center);
  measured_bbox_center_velocity_with_old_dir /= time_diff;
  measured_bbox_center_velocity_with_old_dir(2) = 0.0;
  Eigen::Vector3d measured_bbox_center_velocity = measured_bbox_center_velocity_with_old_dir;
  Eigen::Vector3d project_dir = new_object->anchor_point - old_object->belief_anchor_point;
...
void MeasureBboxCornerVelocity(TrackedObjectPtr             new_object,
  ...
  double length_change = fabs(direct_old_size(0) - direct_new_size(0)) / direct_old_size(0);
  double width_change  = fabs(direct_old_size(1) - direct_new_size(1)) / direct_old_size(1);
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    Eigen::Vector3d bbox_corner_velocity = ((new_bbox_corner - old_bbox_corner) / time_diff);
    Eigen::Vector3d bbox_corner_velocity_on_project_dir =
        common::Calculate2DXYProjectVector<double>(bbox_corner_velocity, project_dir);
    ...
    new_object->measured_corners_velocity[i] = bbox_corner_velocity_on_project_dir;
  }
}
