...
bool GetVelodyneTrans(const double query_time, Eigen::Matrix4d* trans) {
  ...
  if (!tf2_buffer.canTransform(FLAGS_localization_tf2_frame_id,
    ...
    AERROR << "Cannot transform frame: " << FLAGS_localization_tf2_frame_id << " to frame "
           << FLAGS_localization_tf2_child_frame_id << " , err: " << err_msg
           << ". Frames: " << tf2_buffer.allFramesAsString();
    return false;
  }
  try {
    transform_stamped = tf2_buffer.lookupTransform(
        FLAGS_localization_tf2_frame_id, FLAGS_localization_tf2_child_frame_id, query_stamp);
  } catch (tf2::TransformException& ex) {
    AERROR << "Exception: " << ex.what();
    return false;
  }
  ...
  ADEBUG << "get " << FLAGS_lidar_tf2_frame_id << " to " << FLAGS_localization_tf2_child_frame_id
         << " trans: " << *trans;
  return true;
}
...
bool GetRadarTrans(const double query_time, Eigen::Matrix4d* trans) {
  ...
  const double kTf2BuffSize = FLAGS_tf2_buff_in_ms / 1000.0;
  std::string  err_msg;
  if (!tf2_buffer.canTransform(FLAGS_radar_tf2_frame_id, FLAGS_radar_tf2_child_frame_id,
                               query_stamp, ros::Duration(kTf2BuffSize), &err_msg)) {
    AERROR << "Cannot transform frame: " << FLAGS_radar_tf2_frame_id << " to frame "
           << FLAGS_radar_tf2_child_frame_id << " , err: " << err_msg
           << ". Frames: " << tf2_buffer.allFramesAsString();
    return false;
  }
  ...
  geometry_msgs::TransformStamped transform_stamped;
  try {
    transform_stamped = tf2_buffer.lookupTransform(FLAGS_radar_tf2_frame_id,
  ...
  ADEBUG << "get " << FLAGS_radar_tf2_frame_id << " to " << FLAGS_radar_tf2_child_frame_id
         << " trans: " << radar2novatel_trans;
  ...
  *trans = novatel2world_trans * radar2novatel_trans;
  ADEBUG << "get " << FLAGS_radar_tf2_frame_id << " to " << FLAGS_localization_tf2_child_frame_id
         << " trans: " << *trans;
  return true;
}
