...
class SubnodeHelper {
 public:
  ...
  static bool ProduceSharedDataKey(double stamp, const std::string& device_id, std::string* key);

  // produce key for shared data which is always map
  // key = (long)(stamp * 100)*100 + device_id
  static bool ProduceSharedDataKey(double stamp, const std::string& device_id, int64_t* key);
