...
DEFINE_double(obs_max_local_pose_extrapolation_latency,
              0.15,
...
Eigen::Quaterniond
Slerp(const Eigen::Quaterniond& source, const double& t, const Eigen::Quaterniond& other) {
  const double one = 1.0 - std::numeric_limits<double>::epsilon();
  double       d   = source.x() * other.x() + source.y() * other.y() + source.z() * other.z() +
             source.w() * other.w();
  ...
  return Eigen::Quaterniond(
      scale0 * source.w() + scale1 * other.w(), scale0 * source.x() + scale1 * other.x(),
      scale0 * source.y() + scale1 * other.y(), scale0 * source.z() + scale1 * other.z());
}

bool TransformCache::QueryTransform(double            timestamp,
                                    StampedTransform* transform,
                                    double            max_duration) {
  if (transforms_.empty() || transform == nullptr) { return false; }

  double delt = timestamp - transforms_.back().timestamp;
  if (delt > max_duration) {
    AINFO << "ERROR: query timestamp is " << delt << "s later than cached timestamp";
    return false;
  } else if (delt < 0.0) {
  ...
  if (size == 1) {
    ...
    AINFO << "use transform at " << transforms_.back().timestamp << " for " << timestamp;
  } else {
    double ratio = (timestamp - transforms_[size - 2].timestamp) /
                   (transforms_[size - 1].timestamp - transforms_[size - 2].timestamp);

    transform->rotation =
        Slerp(transforms_[size - 2].rotation, ratio, transforms_[size - 1].rotation);

    transform->translation.x() = transforms_[size - 2].translation.x() * (1 - ratio) +
                                 transforms_[size - 1].translation.x() * ratio;
    transform->translation.y() = transforms_[size - 2].translation.y() * (1 - ratio) +
                                 transforms_[size - 1].translation.y() * ratio;
    transform->translation.z() = transforms_[size - 2].translation.z() * (1 - ratio) +
                                 transforms_[size - 1].translation.z() * ratio;

    AINFO << "estimate pose at " << timestamp << " from poses at "
          << transforms_[size - 2].timestamp << " and " << transforms_[size - 1].timestamp;
  }
  return true;
}

void TransformWrapper::Init(const std::string& sensor2novatel_tf2_child_frame_id) {
  ...
  novatel2world_tf2_child_frame_id_  = FLAGS_obs_novatel2world_tf2_child_frame_id;
  transform_cache_.SetCacheDuration(FLAGS_obs_transform_cache_size);
  inited_ = true;
}

void TransformWrapper::Init(const std::string& sensor2novatel_tf2_frame_id,
...
bool TransformWrapper::GetSensor2worldTrans(double           timestamp,
  ...
  if (sensor2novatel_extrinsics_ == nullptr) {
    ...
    if (!QueryTrans(timestamp, &trans_sensor2novatel, sensor2novatel_tf2_frame_id_,
  ...
  if (!QueryTrans(timestamp, &trans_novatel2world, novatel2world_tf2_frame_id_,
    ...
    if (FLAGS_obs_enable_local_pose_extrapolation) {
      if (!transform_cache_.QueryTransform(timestamp, &trans_novatel2world,
  ...
  novatel2world       = trans_novatel2world.translation * trans_novatel2world.rotation;
  *sensor2world_trans = novatel2world * (*sensor2novatel_extrinsics_);
  if (novatel2world_trans != nullptr) { *novatel2world_trans = novatel2world; }
...
bool TransformWrapper::GetTrans(double             timestamp,
  ...
  if (FLAGS_obs_enable_local_pose_extrapolation) { transform_cache_.AddTransform(transform); }
...
bool TransformWrapper::QueryTrans(double             timestamp,
  ...
  try {
    ...
    trans->translation = Eigen::Translation3d(stamped_transform.transform().translation().x(),
                                              stamped_transform.transform().translation().y(),
                                              stamped_transform.transform().translation().z());
    trans->rotation    = Eigen::Quaterniond(stamped_transform.transform().rotation().qw(),
...
bool TransformWrapper::GetExtrinsicsBySensorId(const std::string& from_sensor_id,
  ...
  if (status) { *trans = transform.translation * transform.rotation; }
  return status;
}
