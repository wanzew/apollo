...
bool MsgSerializer::SerializeMsg(double                              timestamp,
...
bool MsgSerializer::ConvertObjectToPb(const base::ObjectPtr& object_ptr,
  ...
  if (object_ptr == nullptr || pb_msg == nullptr) { return false; }
  ...
  for (size_t i = 0; i < 3; i++) {
    for (size_t j = 0; j < 3; j++) {
      ...
      pb_msg->add_acceleration_covariance(object_ptr->acceleration_uncertainty(i, j));
    }
  }
  ...
  if (object_ptr->lidar_supplement.height_above_ground != kFloatMax) {
    pb_msg->set_height_above_ground(object_ptr->lidar_supplement.height_above_ground);
  } else {
  ...
  if (FLAGS_obs_save_fusion_supplement && object_ptr->fusion_supplement.on_use) {
    for (const auto& measurement : object_ptr->fusion_supplement.measurements) {
      ...
      pb_measurement->set_type(static_cast<PerceptionObstacle::Type>(measurement.type));
  ...
  //  (*target_traj->mutable_trajectory_point()) =
  //  (src_traj.trajectory_point());
