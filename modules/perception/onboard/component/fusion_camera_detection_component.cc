...
static int GetGpuId(const camera::CameraPerceptionInitOptions& options) {
  ...
  std::string                  config_file = GetAbsolutePath(options.root_dir, options.conf_file);
...
static bool SetCameraHeight(const std::string& sensor_name,
  ...
  try {
    YAML::Node lidar_height = YAML::LoadFile(params_dir + "/" + lidar_sensor_name + "_height.yaml");
    base_h                  = lidar_height["vehicle"]["parameters"]["height"].as<float>();
    AINFO << base_h;
    YAML::Node camera_ex = YAML::LoadFile(params_dir + "/" + sensor_name + "_extrinsics.yaml");
...
static bool LoadExtrinsics(const std::string& yaml_file, Eigen::Matrix4d* camera_extrinsic) {
...
static bool GetProjectMatrix(const std::vector<std::string>&               camera_names,
  ...
  *project_matrix = intrinsic_map.at(camera_names[0]).cast<double>() *
  ...
  if (pitch_diff != nullptr) {
    Eigen::Vector3d euler = (extrinsic_map.at(camera_names[0]).block<3, 3>(0, 0).inverse() *
                             extrinsic_map.at(camera_names[1]).block<3, 3>(0, 0))
                                .eulerAngles(0, 1, 2);
    *pitch_diff = euler(0);
...
bool FusionCameraDetectionComponent::Init() {
  ...
  writer_             = node_->CreateWriter<PerceptionObstacles>(output_obstacles_channel_name_);
  sensorframe_writer_ = node_->CreateWriter<SensorFrameMessage>(prefused_channel_name_);
  camera_viz_writer_ =
      node_->CreateWriter<CameraPerceptionVizMessage>(camera_perception_viz_message_channel_name_);
  camera_debug_writer_ =
      node_->CreateWriter<apollo::perception::camera::CameraDebug>(camera_debug_channel_name_);
  ...
  LoadExtrinsics(FLAGS_obs_sensor_intrinsic_path + "/" + FLAGS_lidar_sensor_name +
  ...
  ACHECK(visualize_.Init_all_info_single_camera(
      camera_names_, visual_camera_, intrinsic_map_, extrinsic_map_, ex_lidar2imu, pitch_adj_degree,
      yaw_adj_degree, roll_adj_degree, image_height_, image_width_));
  ...
  if (enable_cipv_) {
    camera::BaseCipv* cipv = camera::BaseCipvRegisterer::GetInstanceByName(cipv_name_);
    CHECK_NOTNULL(cipv);
    cipv_.reset(cipv);
    ACHECK(cipv_->Init(homography_im2car_, cipv_init_options_)) << "camera cipv init error";
  }
...
void FusionCameraDetectionComponent::OnReceiveImage(
    const std::shared_ptr<apollo::drivers::Image>& message, const std::string& camera_name) {
  ...
  if (last_timestamp_ - msg_timestamp > ts_diff_) {
    AINFO << "Received an old message. Last ts is " << std::setprecision(19) << last_timestamp_
          << " current ts is " << msg_timestamp << " last - current is "
  ...
  // prefused msg
  std::shared_ptr<SensorFrameMessage> prefused_message(new (std::nothrow) SensorFrameMessage);

  if (InternalProc(message, camera_name, &error_code, prefused_message.get(), out_message.get()) !=
      cyber::SUCC) {
    AERROR << "InternalProc failed, error_code: " << error_code;
    if (MakeProtobufMsg(msg_timestamp, seq_num_, {}, {}, error_code, out_message.get()) !=
        cyber::SUCC) {
      AERROR << "MakeProtobufMsg failed";
      return;
    }
    if (output_final_obstacles_) { writer_->Write(out_message); }
    return;
  }

  bool send_sensorframe_ret = sensorframe_writer_->Write(prefused_message);
  AINFO << "send out prefused msg, ts: " << msg_timestamp << "ret: " << send_sensorframe_ret;
  // Send output msg
  if (output_final_obstacles_) { writer_->Write(out_message); }
  // for e2e lantency statistics
  {
    const double end_timestamp = Clock::NowInSeconds();
    const double end_latency   = (end_timestamp - message->measurement_time()) * 1e3;
...
int FusionCameraDetectionComponent::InitConfig() {
  ...
  apollo::perception::onboard::FusionCameraDetection fusion_camera_detection_param;
  ...
  boost::algorithm::split(input_camera_channel_names_, input_camera_channel_names_str,
                          boost::algorithm::is_any_of(","));
  if (input_camera_channel_names_.size() != camera_names_.size()) {
    AERROR << "wrong input_camera_channel_names_.size(): " << input_camera_channel_names_.size();
    return cyber::FAIL;
  }
  ...
  output_obstacles_channel_name_ = fusion_camera_detection_param.output_obstacles_channel_name();
  ...
  output_final_obstacles_ = fusion_camera_detection_param.output_final_obstacles();
  prefused_channel_name_  = fusion_camera_detection_param.prefused_channel_name();
  default_camera_pitch_ = static_cast<float>(fusion_camera_detection_param.default_camera_pitch());
  default_camera_height_ =
      static_cast<float>(fusion_camera_detection_param.default_camera_height());
  output_camera_debug_msg_   = fusion_camera_detection_param.output_camera_debug_msg();
  camera_debug_channel_name_ = fusion_camera_detection_param.camera_debug_channel_name();
  ts_diff_                   = fusion_camera_detection_param.ts_diff();
  write_visual_img_          = fusion_camera_detection_param.write_visual_img();

  cipv_init_options_.min_laneline_length_for_cipv =
      static_cast<float>(fusion_camera_detection_param.min_laneline_length_for_cipv());
  cipv_init_options_.average_lane_width_in_meter =
      static_cast<float>(fusion_camera_detection_param.average_lane_width_in_meter());
  cipv_init_options_.max_vehicle_width_in_meter =
      static_cast<float>(fusion_camera_detection_param.max_vehicle_width_in_meter());
  ...
  std::string config_info_str =
      str(boost::format(format_str.c_str()) % camera_names_[0] % camera_names_[1] %
          camera_perception_init_options_.root_dir % camera_perception_init_options_.conf_file %
          frame_capacity_ % image_channel_num_ % enable_undistortion_ % enable_visualization_ %
          output_obstacles_channel_name_ % camera_perception_viz_message_channel_name_ %
          visual_debug_folder_ % visual_camera_ % output_final_obstacles_ % prefused_channel_name_ %
          write_visual_img_);
...
int FusionCameraDetectionComponent::InitSensorInfo() {
  ...
  std::string sensor_info_str =
      str(boost::format(format_str.c_str()) % camera_names_[0] % camera_names_[1] %
          tf_camera_frame_id_map_[camera_names_[0]] % tf_camera_frame_id_map_[camera_names_[1]] %
          image_width_ % image_height_ % image_channel_num_);
...
int FusionCameraDetectionComponent::InitCameraFrames() {
  ...
  for (const auto& camera_name : camera_names_) {
    ...
    if (gpu_id == -1) { return cyber::FAIL; }
    data_provider_init_options.device_id = gpu_id;
    AINFO << "data_provider_init_options.device_id: " << data_provider_init_options.device_id
  ...
  for (const auto& camera_name : camera_names_) {
    ...
    AINFO << "#intrinsics of " << camera_name << ": " << intrinsic_map_[camera_name];
    Eigen::Matrix4d extrinsic;
    LoadExtrinsics(FLAGS_obs_sensor_intrinsic_path + "/" + camera_name + "_extrinsics.yaml",
                   &extrinsic);
    extrinsic_map_[camera_name] = extrinsic;
    AINFO << "#extrinsics of " << camera_name << ": " << extrinsic_map_[camera_name];
  }

  // Init camera height
  for (const auto& camera_name : camera_names_) {
    float height = 0.0f;
    SetCameraHeight(camera_name, FLAGS_obs_sensor_intrinsic_path, FLAGS_lidar_sensor_name,
...
int FusionCameraDetectionComponent::InitProjectMatrix() {
  if (!GetProjectMatrix(camera_names_, extrinsic_map_, intrinsic_map_, &project_matrix_,
                        &pitch_diff_)) {
  ...
  name_camera_pitch_angle_diff_map_[camera_names_[1]] = static_cast<float>(pitch_diff_);
...
int FusionCameraDetectionComponent::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    std::function<void(const ImageMsgType&)>        camera_callback = std::bind(
        &FusionCameraDetectionComponent::OnReceiveImage, this, std::placeholders::_1, camera_name);
...
int FusionCameraDetectionComponent::InitMotionService() {
  ...
  std::function<void(const MotionServiceMsgType&)> motion_service_callback =
      std::bind(&FusionCameraDetectionComponent::OnMotionService, this, std::placeholders::_1);
  auto motion_service_reader = node_->CreateReader(channel_name_local, motion_service_callback);
...
void FusionCameraDetectionComponent::OnMotionService(const MotionServiceMsgType& message) {
...
void FusionCameraDetectionComponent::SetCameraHeightAndPitch() {
  camera_obstacle_pipeline_->SetCameraHeightAndPitch(
      camera_height_map_, name_camera_pitch_angle_diff_map_, default_camera_pitch_);
}
...
int FusionCameraDetectionComponent::InternalProc(
  ...
  if (!camera2world_trans_wrapper_map_[camera_name]->GetSensor2worldTrans(msg_timestamp,
                                                                          &camera2world_trans)) {
    const std::string err_str = absl::StrCat(
        "failed to get camera to world pose, ts: ", msg_timestamp, " camera_name: ", camera_name);
    AERROR << err_str;
  ...
  camera_frame.data_provider->FillImageData(
      image_height_, image_width_, reinterpret_cast<const uint8_t*>(in_message->data().data()),
      in_message->encoding());
  ...
  AINFO << "##" << camera_name << ": pitch " << camera_frame.calibration_service->QueryPitchAngle()
        << " | camera_grond_height "
  ...
  if (output_final_obstacles_ &&
      MakeProtobufMsg(msg_timestamp, seq_num_, camera_frame.tracked_objects,
                      camera_frame.lane_objects, *error_code, out_message) != cyber::SUCC) {
  ...
  *error_code                                              = apollo::common::ErrorCode::OK;
  ...
  if (FLAGS_obs_enable_visualization) {
    ...
    prefused_message->frame_->camera_frame_supplement.image_blob.reset(new base::Blob<uint8_t>);
    camera_frame.data_provider->GetImageBlob(
        image_options, prefused_message->frame_->camera_frame_supplement.image_blob.get());
  }
  ...
  if (enable_visualization_) {
    ...
    std::shared_ptr<CameraPerceptionVizMessage> viz_msg(
        new (std::nothrow) CameraPerceptionVizMessage(
            ...
            camera_frame.tracked_objects, camera_frame.lane_objects, *error_code));
    ...
    cv::Mat       output_image(image_height_, image_width_, CV_8UC3, cv::Scalar(0, 0, 0));
    ...
    memcpy(output_image.data, out_image.cpu_data(), out_image.total() * sizeof(uint8_t));
    visualize_.ShowResult_all_info_single_camera(output_image, camera_frame, motion_buffer_,
                                                 world2camera);
  }
  ...
  if (output_camera_debug_msg_) {
    ...
    if (MakeCameraDebugMsg(msg_timestamp, camera_name, camera_frame, camera_debug_msg.get()) !=
        cyber::SUCC) {
...
int FusionCameraDetectionComponent::MakeProtobufMsg(
  ...
  apollo::perception::LaneMarkers* lane_markers   = obstacles->mutable_lane_marker();
  apollo::perception::LaneMarker*  lane_marker_l0 = lane_markers->mutable_left_lane_marker();
  apollo::perception::LaneMarker*  lane_marker_r0 = lane_markers->mutable_right_lane_marker();
  apollo::perception::LaneMarker*  lane_marker_l1 = lane_markers->add_next_left_lane_marker();
  apollo::perception::LaneMarker*  lane_marker_r1 = lane_markers->add_next_right_lane_marker();

  for (const auto& lane : lane_objects) {
    base::LaneLineCubicCurve curve_coord = lane.curve_car_coord;

    switch (lane.pos_type) {
      case base::LaneLinePositionType::EGO_LEFT: fill_lane_msg(curve_coord, lane_marker_l0); break;
      case base::LaneLinePositionType::EGO_RIGHT: fill_lane_msg(curve_coord, lane_marker_r0); break;
...
int FusionCameraDetectionComponent::ConvertObjectToPb(
    const base::ObjectPtr& object_ptr, apollo::perception::PerceptionObstacle* pb_msg) {
  if (!object_ptr || !pb_msg) { return cyber::FAIL; }
  ...
  pb_msg->set_sub_type(static_cast<PerceptionObstacle::SubType>(object_ptr->sub_type));
  ...
  if (object_ptr->type == base::ObjectType::VEHICLE) {
    apollo::perception::LightStatus* light_status = pb_msg->mutable_light_status();
...
int FusionCameraDetectionComponent::ConvertObjectToCameraObstacle(
  ...
  apollo::perception::PerceptionObstacle* obstacle = camera_obstacle->mutable_obstacle();
  ConvertObjectToPb(object_ptr, obstacle);

  camera_obstacle->set_type(
      static_cast<apollo::perception::camera::CameraObstacle::CameraType>(object_ptr->type));
  for (const auto& prob : object_ptr->type_probs) {
    camera_obstacle->add_type_probs(prob);
  }
  camera_obstacle->mutable_upper_left()->set_x(object_ptr->camera_supplement.box.xmin);
  camera_obstacle->mutable_upper_left()->set_y(object_ptr->camera_supplement.box.ymin);
  camera_obstacle->mutable_lower_right()->set_x(object_ptr->camera_supplement.box.xmax);
  camera_obstacle->mutable_lower_right()->set_y(object_ptr->camera_supplement.box.ymax);
...
int FusionCameraDetectionComponent::ConvertLaneToCameraLaneline(
    const base::LaneLine& lane_line, apollo::perception::camera::CameraLaneLine* camera_laneline) {
  ...
  apollo::perception::camera::LaneLinePositionType pos_type =
      static_cast<apollo::perception::camera::LaneLinePositionType>(lane_line.pos_type);
  ...
  apollo::perception::camera::LaneLineUseType use_type =
      static_cast<apollo::perception::camera::LaneLineUseType>(lane_line.use_type);
  ...
  camera_laneline->mutable_curve_camera_coord()->set_a(lane_line.curve_camera_coord.a);
  camera_laneline->mutable_curve_camera_coord()->set_b(lane_line.curve_camera_coord.b);
  camera_laneline->mutable_curve_camera_coord()->set_c(lane_line.curve_camera_coord.c);
  camera_laneline->mutable_curve_camera_coord()->set_d(lane_line.curve_camera_coord.d);
  ...
  camera_laneline->mutable_curve_image_coord()->set_a(lane_line.curve_image_coord.a);
  camera_laneline->mutable_curve_image_coord()->set_b(lane_line.curve_image_coord.b);
  camera_laneline->mutable_curve_image_coord()->set_c(lane_line.curve_image_coord.c);
  camera_laneline->mutable_curve_image_coord()->set_d(lane_line.curve_image_coord.d);
  ...
  for (size_t i = 0; i < lane_line.curve_image_point_set.size(); i++) {
    ...
    apollo::common::Point2D* lane_point_2d = camera_laneline->add_curve_image_point_set();
  ...
  for (size_t i = 0; i < lane_line.curve_camera_point_set.size(); i++) {
    ...
    apollo::common::Point3D* lane_point_3d = camera_laneline->add_curve_camera_point_set();
...
int FusionCameraDetectionComponent::MakeCameraDebugMsg(
  ...
  int input_camera_channel_names_idx  = static_cast<int>(itr - camera_names_.begin());
  int input_camera_channel_names_size = static_cast<int>(input_camera_channel_names_.size());
  if (input_camera_channel_names_idx < 0 ||
      input_camera_channel_names_idx >= input_camera_channel_names_size) {
    AERROR << "invalid input_camera_channel_names_idx: " << input_camera_channel_names_idx
           << " input_camera_channel_names_.size(): " << input_camera_channel_names_.size();
    return cyber::FAIL;
  }
  std::string source_channel_name = input_camera_channel_names_[input_camera_channel_names_idx];
