...
int CameraObstacleDetectionComponent::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    std::function<void(const ImageMsgType&)>        camera_callback =
        std::bind(&CameraObstacleDetectionComponent::OnReceiveImage, this, std::placeholders::_1,
                  camera_name);
...
int CameraObstacleDetectionComponent::InitMotionService() {
  ...
  std::function<void(const MotionServiceMsgType&)> motion_service_callback =
      std::bind(&CameraObstacleDetectionComponent::OnMotionService, this, std::placeholders::_1);
...
void CameraObstacleDetectionComponent::OnMotionService(const MotionServiceMsgType& message) {
