...
bool LaneDetectionComponent::Init() {
  ...
  if (!EXEC_ALL_FUNS(LaneDetectionComponent, this, LaneDetectionComponent::init_func_arry_)) {
    return false;
  }
...
void LaneDetectionComponent::OnMotionService(const MotionServiceMsgType& message) {
...
void LaneDetectionComponent::OnReceiveImage(const std::shared_ptr<apollo::drivers::Image>& message,
...
int LaneDetectionComponent::InitConfig() {
  ...
  std::string input_camera_channel_names_str = lane_detection_param.input_camera_channel_names();
  ...
  camera_perception_init_options_.root_dir = lane_detection_param.camera_lane_perception_conf_dir();
  ...
  default_camera_pitch_      = static_cast<float>(lane_detection_param.default_camera_pitch());
  default_camera_height_     = static_cast<float>(lane_detection_param.default_camera_height());
  ...
  std::string config_info_str =
      str(boost::format(format_str.c_str()) % camera_names_[0] % camera_names_[1] %
          ...
          visual_debug_folder_ % visual_camera_ % output_lanes_channel_name_ % write_visual_img_);
...
int LaneDetectionComponent::InitMotionService() {
  ...
  std::function<void(const MotionServiceMsgType&)> motion_service_callback =
      std::bind(&LaneDetectionComponent::OnMotionService, this, std::placeholders::_1);
...
int LaneDetectionComponent::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    std::function<void(const ImageMsgType&)>        camera_callback = std::bind(
        &LaneDetectionComponent::OnReceiveImage, this, std::placeholders::_1, camera_name);
...
int LaneDetectionComponent::InternalProc(
  ...
  camera_lane_pipeline_->GetCalibrationService(&camera_frame.calibration_service);

  if (!camera_lane_pipeline_->Perception(camera_perception_options_, &camera_frame)) {
    AERROR << "camera_lane_pipeline_->Perception() failed msg_timestamp: " << msg_timestamp;
  ...
  *error_code                   = apollo::common::ErrorCode::OK;
  ...
  if (enable_visualization_) {
    ...
    visualize_.ShowResult_all_info_single_camera(output_image, camera_frame, mot_buffer_,
                                                 world2camera);
  }
  ...
  std::shared_ptr<apollo::perception::PerceptionLanes> lanes_msg(
      new (std::nothrow) apollo::perception::PerceptionLanes);
  if (MakeProtobufMsg(msg_timestamp, camera_name, camera_frame, lanes_msg.get()) != cyber::SUCC) {
...
int LaneDetectionComponent::MakeProtobufMsg(double                               msg_timestamp,
                                            const std::string&                   camera_name,
  ...
  for (const auto& lane_obj : lane_objects) {
    apollo::perception::camera::CameraLaneLine* camera_laneline = lanes_msg->add_camera_laneline();
