...
DEFINE_string(resource_monitor_name, "ResourceMonitor", "Name of the resource monitor.");

DEFINE_double(resource_monitor_interval, 5, "Topic status checking interval (s).");
...
bool GetPIDByCmdLine(const std::string& process_dag_path, int* pid) {
  ...
  for (const auto& dir_name : dirs) {
    if (!std::all_of(dir_name.begin(), dir_name.end(), isdigit)) { continue; }
    *pid = std::stoi(dir_name);
    std::ifstream cmdline_file(absl::StrCat(system_proc_path, "/", dir_name, proc_cmdline_path));
    std::getline(cmdline_file, cmd_line);
    if (absl::StrContains(cmd_line, process_dag_path)) { return true; }
  }
  return false;
}
...
float GetMemoryUsage(const int pid, const std::string& process_name) {
  ...
  return static_cast<float>(std::stoll(stats[resident_idx]) * page_size_kb) / gb_2_kb;
}
...
float GetSystemMemoryUsage() {
  ...
  const int mem_total = 0, mem_free = 1, buffers = 3, cached = 4, swap_total = 14, swap_free = 15,
            slab        = 21;
...
float GetSystemCPUUsage() {
  ...
  for (int cur_stat = users; cur_stat <= total; ++cur_stat) {
    ...
    if (cur_stat <= system) { work_jiffies += cur_stat_value; }
  }
  ...
  if (tmp_prev_jiffies == 0) { return 0.f; }
  return 100.f *
         (static_cast<float>(work_jiffies - tmp_prev_work_jiffies) / (jiffies - tmp_prev_jiffies));
}
...
float GetSystemDiskload(const std::string& device_name) {
  ...
  if (tmp_prev_disk_stats == 0) { return 0.f; }
...
ResourceMonitor::ResourceMonitor()
    : RecurrentRunner(FLAGS_resource_monitor_name, FLAGS_resource_monitor_interval) {}
...
void ResourceMonitor::UpdateStatus(const apollo::dreamview::ResourceMonitorConfig& config,
...
void ResourceMonitor::CheckDiskSpace(const apollo::dreamview::ResourceMonitorConfig& config,
                                     ComponentStatus*                                status) {
  ...
  for (const auto& disk_space : config.disk_spaces()) {
    for (const auto& path : cyber::common::Glob(disk_space.path())) {
      ...
      if (available_gb < disk_space.insufficient_space_error()) {
        const std::string err = absl::StrCat(path, " has insufficient space: ", available_gb,
...
void ResourceMonitor::CheckCPUUsage(const apollo::dreamview::ResourceMonitorConfig& config,
                                    ComponentStatus*                                status) {
  for (const auto& cpu_usage : config.cpu_usages()) {
    ...
    if (cpu_usage_value > high_cpu_error) {
      const std::string err = absl::StrCat(
          process_dag_path, " has high cpu usage: ", cpu_usage_value, "% > ", high_cpu_error, "%");
      SummaryMonitor::EscalateStatus(ComponentStatus::ERROR, err, status);
    } else if (cpu_usage_value > high_cpu_warning) {
      const std::string warn =
          absl::StrCat(process_dag_path, " has high cpu usage: ", cpu_usage_value, "% > ",
                       high_cpu_warning, "%");
...
void ResourceMonitor::CheckMemoryUsage(const apollo::dreamview::ResourceMonitorConfig& config,
                                       ComponentStatus*                                status) {
  for (const auto& memory_usage : config.memory_usages()) {
    ...
    if (memory_usage_value > static_cast<float>(high_memory_error)) {
      const std::string err =
          absl::StrCat(process_dag_path, " has high memory usage: ", memory_usage_value, " > ",
                       high_memory_error);
      SummaryMonitor::EscalateStatus(ComponentStatus::ERROR, err, status);
...
void ResourceMonitor::CheckDiskLoads(const apollo::dreamview::ResourceMonitorConfig& config,
                                     ComponentStatus*                                status) {
  for (const auto& disk_load : config.disk_load_usages()) {
    ...
    if (disk_load_value > static_cast<float>(high_disk_load_error)) {
      const std::string err =
          absl::StrCat(disk_load.device_name(), " has high disk load: ", disk_load_value, " > ",
