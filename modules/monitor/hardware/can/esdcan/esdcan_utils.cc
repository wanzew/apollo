...
void esdcan_print_if_status(int id, const CAN_IF_STATUS& if_status) {
  printf("Net %3d: ID=%s\n"
         ...
         if_status.dll & 0xff, if_status.driver >> 12, (if_status.driver >> 8) & 0xf,
         if_status.driver & 0xff, if_status.firmware >> 12, (if_status.firmware >> 8) & 0xf,
         if_status.firmware & 0xff, if_status.hardware >> 12, (if_status.hardware >> 8) & 0xf,
         if_status.hardware & 0xff, (unsigned int)if_status.boardstatus);
}
...
void esdcan_print_ctrl_state(const NTCAN_CTRL_STATE& c_state) {
  printf("Err counter     : (Rx/Tx): %d/%d Status: %02x\n", c_state.rcv_err_counter,
         c_state.xmit_err_counter, c_state.status);
}
...
void esdcan_print_bitrate(const NTCAN_BITRATE& bitrate) {
  ...
  if (NTCAN_SUCCESS == bitrate.valid) {
    ...
    printf("Deviation from configured rate : %ld.%02ld%%\n",
           static_cast<int64_t>(bitrate.error / 100), static_cast<int64_t>(bitrate.error % 100));
