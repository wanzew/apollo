...
NTCAN_RESULT EsdCanDetails::esdcan_do_test(int id) {
  ...
  if (ret != NTCAN_SUCCESS) {
    AERROR << "NTCAN_IOCTL_GET_BUS_STATISTIC failed for device " << id << " with error: " << ret
           << " (" << esdcan_err_to_str(ret) << ")";
    goto err;
  }
  ...
  if (ret != NTCAN_SUCCESS) {
    AERROR << "NTCAN_IOCTL_GET_CTRL_STATUS failed for device " << id << "with error: " << ret
           << " (" << esdcan_err_to_str(ret) << ")";
    goto err;
  }
  ...
  if (ret != NTCAN_SUCCESS) {
    AERROR << "NTCAN_IOCTL_GET_BITRATE_ for device " << id << " with error: " << ret << " ("
           << esdcan_err_to_str(ret) << ")";
    goto err;
  }
...
void EsdCanDetails::print_summary(std::ostream& os) {
  if (result == NTCAN_SUCCESS) {
    os << "ESD-CAN test PASSED, CAN bus statistics:\n"
       ...
       << stats.rcv_count.std_rtr << ", Ext(Data/RTR): " << stats.rcv_count.ext_data << "/"
       << stats.rcv_count.ext_rtr << std::endl
       << "Xmit frames     : Std(Data/RTR): " << stats.xmit_count.std_data << "/"
       << stats.xmit_count.std_rtr << ", Ext(Data/RTR): " << stats.xmit_count.ext_data << "/"
       << stats.xmit_count.ext_rtr << std::endl
       << "Bytes           : (Rcv/Xmit): " << stats.rcv_byte_count << "/" << stats.xmit_byte_count
       << std::endl
       << "Overruns        : (Controller/FIFO): " << stats.ctrl_ovr << "/" << stats.fifo_ovr
       << std::endl
       << "Err frames      : " << stats.err_frames << std::endl
       << "Aborted frames  : " << stats.aborted_frames << std::endl
       << "Err counter     : (Rx/Tx): " << static_cast<int>(ctrl_state.rcv_err_counter) << "/"
       << static_cast<int>(ctrl_state.xmit_err_counter) << std::endl
       << "Status          : " << std::hex << static_cast<int>(ctrl_state.status) << std::endl
       << "Rcv bits        : " << std::dec << stats.bit_count << std::endl;
  } else {
    os << "ESD-CAN test FAILED with error " << result << ": " << esdcan_err_to_str(result)
       << std::endl;
  }
}
