...
DEFINE_string(esdcan_component_name, "ESD-CAN", "Name of the ESD CAN component in SystemStatus.");
...
#  ifdef NTCAN_IO_INCOMPLETE
    case NTCAN_IO_INCOMPLETE: return "NTCAN_IO_INCOMPLETE";
#  endif
#  ifdef NTCAN_IO_PENDING
    case NTCAN_IO_PENDING: return "NTCAN_IO_PENDING";
#  endif
...
#  ifdef NTCAN_ERROR_NO_BAUDRATE
    case NTCAN_ERROR_NO_BAUDRATE: return "NTCAN_ERROR_NO_BAUDRATE";
#  endif
#  ifdef NTCAN_ERROR_LOM
    case NTCAN_ERROR_LOM: return "NTCAN_ERROR_LOM";
#  endif
    default: break;
  }
...
NTCAN_RESULT EsdCanTest(const int can_id, NTCAN_HANDLE* handle) {
  ...
  if (ret != NTCAN_SUCCESS) {
    AERROR << "Cannot get status of ESD-CAN, ret=" << ret << " (" << StatusString(ret) << ")";
    return ret;
  }
  ...
  if (ret != NTCAN_SUCCESS) {
    AERROR << "NTCAN_IOCTL_GET_BUS_STATISTIC failed for device with error: " << ret << " ("
           << StatusString(ret) << ")";
    return ret;
  }
  ...
  if (ret != NTCAN_SUCCESS) {
    AERROR << "NTCAN_IOCTL_GET_CTRL_STATUS failed for device with error: " << ret << " ("
           << StatusString(ret) << ")";
    return ret;
  }
  ...
  if (ret != NTCAN_SUCCESS) {
    AERROR << "NTCAN_IOCTL_GET_BITRATE_ for device with error: " << ret << " (" << StatusString(ret)
           << ")";
    return ret;
  }
  return ret;
...
void EsdCanTest(const int can_id, ComponentStatus* status) {
  ...
  SummaryMonitor::EscalateStatus(ret == NTCAN_SUCCESS ? ComponentStatus::OK :
...
void EsdCanTest(const int can_id, ComponentStatus* status) {
  SummaryMonitor::EscalateStatus(ComponentStatus::ERROR,
                                 "USE_ESD_CAN is not defined during compiling", status);
}
#endif
...
EsdCanMonitor::EsdCanMonitor()
    : RecurrentRunner(FLAGS_esdcan_monitor_name, FLAGS_esdcan_monitor_interval) {}

void EsdCanMonitor::RunOnce(const double current_time) {
  Component* component = apollo::common::util::FindOrNull(
      *MonitorManager::Instance()->GetStatus()->mutable_components(), FLAGS_esdcan_component_name);
