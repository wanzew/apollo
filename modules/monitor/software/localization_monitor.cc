...
DEFINE_string(localization_monitor_name,
              "LocalizationMonitor",
              "Name of the localization monitor.");

DEFINE_double(localization_monitor_interval, 5, "Localization status checking interval (s).");

DEFINE_string(localization_component_name, "Localization", "Localization component name.");
...
LocalizationMonitor::LocalizationMonitor()
    : RecurrentRunner(FLAGS_localization_monitor_name, FLAGS_localization_monitor_interval) {}
...
void LocalizationMonitor::RunOnce(const double current_time) {
  ...
  if (status == nullptr) {
    SummaryMonitor::EscalateStatus(ComponentStatus::ERROR, "No LocalizationStatus received",
                                   component_status);
    return;
  }
  ...
  switch (status->fusion_status()) {
    ...
    case MeasureState::ERROR:
      SummaryMonitor::EscalateStatus(ComponentStatus::WARN,
                                     absl::StrCat("ERROR: ", status->state_message()),
                                     component_status);
      break;
    ...
    case MeasureState::FATAL_ERROR:
      SummaryMonitor::EscalateStatus(ComponentStatus::FATAL,
                                     ...
                                     component_status);
      break;
    default: AFATAL << "Unknown fusion_status: " << status->fusion_status(); break;
  }
}
