...
DEFINE_string(channel_monitor_name, "ChannelMonitor", "Name of the channel monitor.");

DEFINE_double(channel_monitor_interval, 5, "Channel monitor checking interval in seconds.");
...
using ReaderAndMessagePair =
    std::pair<std::shared_ptr<cyber::ReaderBase>, std::shared_ptr<google::protobuf::Message>>;
...
ReaderAndMessagePair GetReaderAndLatestMessage(const std::string& channel) {
  static const auto channel_function_map = std::unordered_map<
      std::string, std::function<ReaderAndMessagePair(const std::string& channel)>>{
      {FLAGS_control_command_topic, &CreateReaderAndLatestsMessage<control::ControlCommand>},
      ...
      {FLAGS_prediction_topic, &CreateReaderAndLatestsMessage<prediction::PredictionObstacles>},
      {FLAGS_planning_trajectory_topic, &CreateReaderAndLatestsMessage<planning::ADCTrajectory>},
      {FLAGS_conti_radar_topic, &CreateReaderAndLatestsMessage<drivers::ContiRadar>},
      {FLAGS_relative_map_topic, &CreateReaderAndLatestsMessage<relative_map::MapMsg>},
      {FLAGS_pointcloud_topic, &CreateReaderAndLatestsMessage<drivers::PointCloud>},
      {FLAGS_pointcloud_16_topic, &CreateReaderAndLatestsMessage<drivers::PointCloud>},
      {FLAGS_pointcloud_16_raw_topic, &CreateReaderAndLatestsMessage<drivers::PointCloud>},
      {FLAGS_pointcloud_128_topic, &CreateReaderAndLatestsMessage<drivers::PointCloud>},
      {FLAGS_pointcloud_16_front_up_topic, &CreateReaderAndLatestsMessage<drivers::PointCloud>},
      {FLAGS_chassis_detail_topic, &CreateReaderAndLatestsMessage<canbus::ChassisDetail>},
      {FLAGS_pointcloud_hesai_40p_topic, &CreateReaderAndLatestsMessage<drivers::PointCloud>}
      // Add more channels here if you want to monitor.
  };

  auto entry = channel_function_map.find(channel);
  if (entry != channel_function_map.end()) { return (entry->second)(channel); }
...
bool ValidateFields(const google::protobuf::Message& message,
  ...
  if (field_step >= fields.size()) { return true; }
...
ChannelMonitor::ChannelMonitor(const std::shared_ptr<LatencyMonitor>& latency_monitor)
    : RecurrentRunner(FLAGS_channel_monitor_name, FLAGS_channel_monitor_interval)
...
void ChannelMonitor::RunOnce(const double current_time) {
  ...
  for (const auto& iter : mode.monitored_components()) {
    ...
    if (config.has_channel()) {
      double     freq;
      const auto update_freq = latency_monitor_->GetFrequency(config.channel().name(), &freq);
      UpdateStatus(config.channel(), components->at(name).mutable_channel_status(), update_freq,
                   freq);
    }
  }
}

void ChannelMonitor::UpdateStatus(const apollo::dreamview::ChannelMonitorConfig& config,
  ...
  if (reader == nullptr) {
    SummaryMonitor::EscalateStatus(
        ...
        absl::StrCat(config.name(), " is not registered in ChannelMonitor."), status);
    return;
  }

  if (message == nullptr || message->ByteSize() == 0) {
    SummaryMonitor::EscalateStatus(
        ComponentStatus::FATAL, absl::StrCat("the message ", config.name(), " reseived is empty."),
        status);
    return;
  }
  ...
  if (message != nullptr) {
    for (const auto& field : config.mandatory_fields()) {
      if (!ValidateFields(*message, absl::StrSplit(field, field_sepr), 0)) {
        SummaryMonitor::EscalateStatus(
            ComponentStatus::ERROR, absl::StrCat(config.name(), " missing field ", field), status);
      }
    }
  }
