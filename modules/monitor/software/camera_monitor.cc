...
using ReaderAndMessagePair =
    std::pair<std::shared_ptr<cyber::ReaderBase>, std::shared_ptr<drivers::Image>>;

ReaderAndMessagePair CreateReaderAndLatestsMessage(const std::string& camera) {
  const auto reader = MonitorManager::Instance()->CreateReader<drivers::Image>(camera);
  reader->Observe();
...
static const auto camera_topic_set = std::set<std::string>{
    FLAGS_image_long_topic,       FLAGS_camera_image_long_topic,  FLAGS_camera_image_short_topic,
    FLAGS_camera_front_6mm_topic, FLAGS_camera_front_6mm_2_topic, FLAGS_camera_front_12mm_topic,
...
void CameraMonitor::UpdateStatus(ComponentStatus* status) {
  ...
  for (const auto& topic : camera_topic_set) {
    ...
    if (reader != nullptr && message != nullptr) {
      if (frame_id.empty()) {
        ...
        if (header.has_frame_id()) { frame_id = header.frame_id(); }
      } else {
  ...
  if (frame_id.empty()) {
    SummaryMonitor::EscalateStatus(ComponentStatus::ERROR, absl::StrCat("No camera is detected"),
                                   status);
  } else {
