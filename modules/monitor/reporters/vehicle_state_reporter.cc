...
DEFINE_string(vehicle_state_reporter_name, "VehicleStateReporter", "Vehicle state reporter name.");

DEFINE_double(vehicle_state_report_interval, 5, "Vehicle state reporting interval (s).");

namespace apollo {
namespace monitor {

VehicleStateReporter::VehicleStateReporter()
    : RecurrentRunner(FLAGS_vehicle_state_reporter_name, FLAGS_vehicle_state_report_interval) {}

void VehicleStateReporter::RunOnce(const double current_time) {
  VehicleStateReport report;
  *report.mutable_vehicle_info() = apollo::data::InfoCollector::GetVehicleInfo();
  *report.mutable_vehicle_state() =
      apollo::common::VehicleStateProvider::instance()->vehicle_state();

  const nlohmann::json json =
      apollo::common::util::JsonUtil::ProtoToTypedJson("VehicleStateReport", report);
