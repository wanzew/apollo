...
class RealtimeRecordProcessor : public RecordProcessor {
 ...
 private:
  ...
  void PublishStatus(const RecordingState state, const std::string& message) const;

  std::shared_ptr<cyber::record::Recorder>            recorder_               = nullptr;
  std::shared_ptr<cyber::Node>                        smart_recorder_node_    = nullptr;
  std::shared_ptr<cyber::Writer<SmartRecorderStatus>> recorder_status_writer_ = nullptr;
