...
std::string GetNextRecordFileName(const std::string& record_path) {
  ...
  if (record_path.empty()) { return kInitialSequence; }
  std::stringstream record_suffix;
  record_suffix.fill('0');
  record_suffix.width(kSuffixLen);
  record_suffix << std::stoi(record_path.substr(record_path.size() - kSuffixLen, kSuffixLen)) + 1;
  return record_suffix.str();
}

bool IsRecordValid(const std::string& record_path) {
  if (!PathExists(record_path)) { return false; }
...
RealtimeRecordProcessor::RealtimeRecordProcessor(const std::string& source_record_dir,
  ...
  default_output_filename_.erase(
      std::remove(default_output_filename_.begin(), default_output_filename_.end(), '-'),
      default_output_filename_.end());
  default_output_filename_ = GetFileName(absl::StrCat(default_output_filename_, ".record"), false);
}
...
bool RealtimeRecordProcessor::Init(const SmartRecordTrigger& trigger_conf) {
  ...
  if (!EnsureDirectory(source_record_dir_) || !EnsureDirectory(restored_output_dir_)) {
  ...
  recorder_status_writer_ =
      smart_recorder_node_->CreateWriter<SmartRecorderStatus>(FLAGS_recorder_status_topic);
  ...
  const std::set<std::string>& all_channels_set = ChannelPool::Instance()->GetAllChannels();
  std::copy(all_channels_set.begin(), all_channels_set.end(), std::back_inserter(all_channels));
  recorder_ =
      std::make_shared<Recorder>(absl::StrCat(source_record_dir_, "/", default_output_filename_),
                                 false, all_channels, black_channels, HeaderBuilder::GetHeader());
  // Init base
...
void RealtimeRecordProcessor::MonitorStatus() {
  ...
  while (++interval_counter * kMessageInterval < recorder_wait_time_) {
    MonitorManager::Instance()->LogBuffer().WARN("SmartRecorder is terminating...");
...
bool RealtimeRecordProcessor::GetNextValidRecord(std::string* record_path) const {
...
void RealtimeRecordProcessor::RestoreMessage(const uint64_t message_time) {
  ...
  const uint64_t        target_end = std::max(
      interval.end_time, message_time - static_cast<uint64_t>(max_backward_time_ * 1000000000UL));
  const bool small_channels_only = restore_reader_time_ >= interval.end_time;
  if (small_channels_only &&
      target_end <=
          restore_reader_time_ + static_cast<uint64_t>(min_restore_chunk_ * 1000000000UL)) {
    return;
  }
  do {
    ...
    AINFO << "target restoring " << restore_path_ << ": " << restore_reader_time_ << " - "
          << target_end;
    auto reader          = std::make_shared<RecordReader>(restore_path_);
    restore_reader_time_ = std::max(restore_reader_time_, reader->GetHeader().begin_time());
    ...
    if (target_end >= reader->GetHeader().end_time()) { GetNextValidRecord(&restore_path_); }
