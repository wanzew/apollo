...
class ManualTrafficLight final : public apollo::cyber::TimerComponent {
 public:
  bool Init() {
    ...
    traffic_light_detection_writer_ =
        node_->CreateWriter<TrafficLightDetection>(FLAGS_traffic_light_detection_topic);
  ...
  bool Proc() {
    ...
    if (IsDifferent(prev_traffic_lights_, signal_ids)) {
      prev_traffic_lights_ = std::unordered_set<std::string>(signal_ids.begin(), signal_ids.end());
      updated_             = true;
    }
    ...
    if (updated_) {
      ...
      std::cout << print_color << "Current Light: " << (is_green_ ? "GREEN" : "RED");
      if (signal_ids.empty()) {
        if (FLAGS_all_lights) {
        ...
        if (signal_ids.size() < 5) {
          ...
          std::cout << " IDs: " << absl::StrJoin(signal_ids.begin(), signal_ids.begin() + 4, " ")
                    << " ...";
        }
      }
 ...
 private:
  bool GetAllTrafficLights(std::vector<SignalInfoConstPtr>* traffic_lights) {
    ...
    if (map_proto.lane().empty() && map_traffic_lights.empty()) {
      ...
      } else if (!apollo::cyber::common::GetProtoFromFile(map_filename, &map_proto)) {
        return false;
      }
  ...
  bool GetTrafficLightsWithinDistance(std::vector<SignalInfoConstPtr>* traffic_lights) {
    ...
    int  ret      = hdmap->GetForwardNearestSignalsOnLane(position, FLAGS_traffic_light_distance,
                                                    traffic_lights);
    if (ret != 0) {
 ...
 private:
  ...
  std::shared_ptr<apollo::cyber::Writer<TrafficLightDetection>> traffic_light_detection_writer_ =
      nullptr;
  std::shared_ptr<apollo::cyber::Reader<LocalizationEstimate>> localization_reader_ = nullptr;
};
