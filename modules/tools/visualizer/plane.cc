...
bool Plane::FillVertexBuffer(GLfloat* pBuffer) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, texture_->width(), texture_->height(), 0,
               texture_->texture_format(), GL_UNSIGNED_BYTE, texture_->data());
...
void Plane::Draw(void) {
  if (texture_->data()) {
    if (texture_->isSizeChanged()) {
      ...
      glBindTexture(GL_TEXTURE_2D, texture_id_);
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, texture_->width(), texture_->height(),
                      texture_->texture_format(), GL_UNSIGNED_BYTE, texture_->data());
      glBindTexture(GL_TEXTURE_2D, 0);
      texture_->removeDirty();
    }
