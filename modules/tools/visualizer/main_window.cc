...
const char* aboutMessage = "Cyber_Visualizer\n"
                           "\n"
...
#define MEMBER_OFFSET(StructType, Member)                                                          \
  (size_t)(reinterpret_cast<char*>(&(reinterpret_cast<StructType*>(1)->Member)) - 1)
...
struct MainWindow::VideoImgProxy {
  ...
  union {
    ...
    CyberChannReader<apollo::drivers::CompressedImage>* compressed_image_reader_;
  };
...
MainWindow::MainWindow(QWidget* parent)
  ...
  connect(pointcloud_button_, SIGNAL(clicked(bool)), this, SLOT(PlayRenderableObject(bool)));
...
void MainWindow::calculateWH(void) {
  ...
  if (count > 0) {
    ...
    for (VideoImgProxy* p : video_image_viewer_list_) {
      ...
      ui_->videoImageGridLayout->addWidget(&p->video_image_viewer_, index / 3, index % 3, 1, 1);
      ++index;
    }
...
MainWindow::VideoImgProxy* MainWindow::AddVideoImgViewer() {
  ...
  if (ret) {
    ...
    ret->video_image_viewer_.setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);
...
void MainWindow::ActionAddGrid(void) {
  if (grid_shader_ == nullptr) {
    grid_shader_ = RenderableObject::CreateShaderProgram(tr(gridVertexPath), tr(gridFragPath));
    if (grid_shader_ != nullptr) { ui_->sceneWidget->AddNewShaderProg("grid", grid_shader_); }
  }
  ...
  if (grid_root_item_ == nullptr) {
    ...
    grid_ = new Grid(6);
    if (grid_ == nullptr) { goto _ret1; }

    if (grid_shader_ == nullptr) { goto _ret2; }

    enable_grid_checkBox_ = new QCheckBox(ui_->treeWidget);
    if (enable_grid_checkBox_ == nullptr) { goto _ret2; }

    grid_root_item_ = new QTreeWidgetItem(ui_->treeWidget);
    if (grid_root_item_ == nullptr) { goto _ret3; }

    colorChild = new QTreeWidgetItem(grid_root_item_);
    if (colorChild == nullptr) { goto _ret4; }
    grid_root_item_->addChild(colorChild);
    colorChild->setText(0, "Color");
    colorChild->setText(1, tr("%1;%2;%3").arg(grid_->red()).arg(grid_->green()).arg(grid_->blue()));

    spinbox = new QSpinBox(ui_->treeWidget);
    if (spinbox == nullptr) { goto _ret5; }
    ...
    connect(enable_grid_checkBox_, SIGNAL(clicked(bool)), this, SLOT(EnableGrid(bool)));
    connect(ui_->treeWidget, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this,
            SLOT(EditGridColor(QTreeWidgetItem*, int)));
    connect(spinbox, SIGNAL(valueChanged(int)), this, SLOT(ChangeGridCellCountBySize(int)));
    return;
  ...
  _ret2:
    delete grid_;
  _ret1:
    QMessageBox::warning(this, tr("Error"), tr("There is no enough memory for creating Grid!!!"),
                         QMessageBox::Ok);
    return;
  }
}
...
void MainWindow::EditGridColor(QTreeWidgetItem* item, int column) {
  if (column && item != nullptr && grid_root_item_ != nullptr &&
    ...
    if (color.isValid()) {
      QString str = tr("%1;%2;%3").arg(color.red()).arg(color.green()).arg(color.blue());
      item->setText(1, str);
...
void MainWindow::ActionOpenRadarChannel(void) {
  if (radar_points_shader_ == nullptr) {
    radar_points_shader_ =
        RenderableObject::CreateShaderProgram(tr(radarVertexPath), tr(radarFragPath));
    if (radar_points_shader_ != nullptr) {
      ui_->sceneWidget->AddNewShaderProg("radarpoints", radar_points_shader_);
    } else {
      QMessageBox::warning(this, tr("NO Shader"),
                           tr("There is no suitable shader for Radar Points!!!"), QMessageBox::Ok);
      return;
    }
  }
  ...
  if (closed_radarData_list_.empty()) {
    ...
    ui_->treeWidget->setItemWidget(&radarProxy->action_item_, 1, &radarProxy->action_item_button_);

    for (int i = 0; i < all_channel_root_->childCount(); ++i) {
      QTreeWidgetItem* child   = all_channel_root_->child(i);
      QString          channel = child->text(0);
      if (channel.contains("radar")) { radarProxy->channel_name_combobox_.addItem(channel); }
    }
  } else {
  ...
  connect(&radarProxy->channel_name_combobox_, SIGNAL(currentIndexChanged(int)), this,
          SLOT(ChangeRadarChannel()));
...
void MainWindow::openRadarChannel(bool b) {
  ...
  RadarData*   theVideoImg = StructPtrByMemberPtr(obj, RadarData, action_item_button_);
...
void MainWindow::DoOpenRadarChannel(bool b, RadarData* radarProxy) {
  if (b) {
    if (radarProxy->channel_name_combobox_.currentText().isEmpty()) {
      QMessageBox::warning(this, tr("Setup Channel Name"),
    ...
    if (!radarProxy->channel_reader_) {
      radarProxy->channel_reader_ = new CyberChannReader<apollo::drivers::RadarObstacles>();
      ...
      auto radarcallback =
          [this, radarProxy](const std::shared_ptr<apollo::drivers::RadarObstacles>& pdata) {
      ...
      if (!radarProxy->channel_reader_->InstallCallbackAndOpen(
              radarcallback, radarProxy->channel_name_combobox_.currentText().toStdString(),
              nodeName)) {
        QMessageBox::warning(this, tr("Setup Channel Callback"),
...
void MainWindow::RadarRenderCallback(
    const std::shared_ptr<const apollo::drivers::RadarObstacles>& rawData, RadarData* radar) {
  ...
  if (rawData != nullptr) {
    ...
    if (r) {
      if (!r->FillData(rawData) ||
          !ui_->sceneWidget->AddTempRenderableObj(radar->root_item_.text(0).toStdString(), r)) {
        delete r;
      }
    } else {
...
void MainWindow::ActionOpenPointCloud(void) {
  if (pointcloud_shader_ == nullptr) {
    pointcloud_shader_ = RenderableObject::CreateShaderProgram(tr(pcVertexPath), tr(pcFragPath));
    if (pointcloud_shader_ != nullptr) {
      ui_->sceneWidget->AddNewShaderProg("pointcloud", pointcloud_shader_);
    } else {
      QMessageBox::warning(this, tr("NO Shader"),
                           tr("There is no suitable shader for Pointcloud!!!"), QMessageBox::Ok);
      return;
    }
  }
  ...
  if (pointcloud_top_item_ == nullptr) {
    ...
    if (pointcloud_top_item_ == nullptr) { return; }
    ...
    if (item == nullptr) {
      QMessageBox::warning(this, tr("NO Enough Memory"),
                           tr("Cannot create tree item for channel name!!!"), QMessageBox::Ok);
      return;
    }
    ...
    if (item == nullptr) {
      QMessageBox::warning(this, tr("NO Enough Memory"),
                           tr("Cannot create tree item for channel Action!!!"), QMessageBox::Ok);
      return;
    }
...
void MainWindow::ActionOpenImages(void) {
  if (open_images_dialog_ == nullptr) {
    ...
    if (open_images_dialog_ == nullptr) {
      QMessageBox::warning(this, tr("No Enough Memory"), tr("There is no enough memory!!!"),
                           QMessageBox::Ok);
      return;
    } else {
      connect(open_images_dialog_, SIGNAL(accepted()), this, SLOT(AddVideoImages()));
    }
  }
...
void MainWindow::ActionOpenImage(void) {
  ...
  if (closed_video_image_viewer_list_.empty()) {
    ...
    for (int i = 0; i < all_channel_root_->childCount(); ++i) {
      QTreeWidgetItem* child   = all_channel_root_->child(i);
      QString          channel = child->text(0);
      if (channel.contains("camera")) { videoImgProxy->channel_name_combobox_.addItem(channel); }
    }
  } else {
  ...
  connect(&videoImgProxy->channel_name_combobox_, SIGNAL(currentIndexChanged(int)), this,
          SLOT(ChangeVideoImgChannel()));
  connect(&videoImgProxy->video_image_viewer_, SIGNAL(focusOnThis(FixedAspectRatioWidget*)), this,
...
void MainWindow::ActionDelVideoImage(void) {
  ...
  VideoImgProxy*   proxy   = StructPtrByMemberPtr(topItem, VideoImgProxy, root_item_);
...
void MainWindow::CloseVideoImgViewer(bool b) {
  if (!b) {
    ...
    DoDeleteVideoImg(StructPtrByMemberPtr(dock, VideoImgProxy, video_image_viewer_));
  }
}

void MainWindow::DoDeleteVideoImg(VideoImgProxy* proxy) {
  disconnect(&proxy->action_item_button_, SIGNAL(clicked(bool)), this, SLOT(PlayVideoImage(bool)));
  disconnect(&proxy->channel_name_combobox_, SIGNAL(currentIndexChanged(int)), this,
             SLOT(ChangeVideoImgChannel()));
  disconnect(&proxy->video_image_viewer_, SIGNAL(focusOnThis(FixedAspectRatioWidget*)), this,
             SLOT(SelectCurrentTreeItem(FixedAspectRatioWidget*)));
...
void MainWindow::UpdateActions(void) {
  ...
  ui_->actionDelImage->setEnabled(false);
  if (item) {
    if (!item->parent() && item != pointcloud_top_item_ && item != all_channel_root_ &&
        item != grid_root_item_) {
...
void MainWindow::PlayRenderableObject(bool b) {
  if (b) {
    ...
    if (!pointcloud_channel_Reader_) {
      pointcloud_channel_Reader_ = new CyberChannReader<apollo::drivers::PointCloud>();
      ...
      auto pointCallback = [this](const std::shared_ptr<apollo::drivers::PointCloud>& pdata) {
      ...
      if (!pointcloud_channel_Reader_->InstallCallbackAndOpen(
              pointCallback, pointcloud_comboBox_->currentText().toStdString(), nodeName)) {
        QMessageBox::warning(this, tr("Setup Channel Callback"),
                             tr("Channel Callback cannot be installed!!!\nPlease check it!"),
                             QMessageBox::Ok);
        delete pointcloud_channel_Reader_;
...
void MainWindow::ImageReaderCallback(const std::shared_ptr<const apollo::drivers::Image>& imgData,
  ...
  if (theVideoImgProxy->dynamic_texture_ != nullptr && imgData != nullptr) {
    if (theVideoImgProxy->dynamic_texture_->UpdateData(imgData)) {
      theVideoImgProxy->video_image_viewer_.SetupDynamicTexture(theVideoImgProxy->dynamic_texture_);
    } else {
      std::cerr << "--------Cannot update dynamic Texture Data--------" << std::endl;
    }
  } else {
    std::cerr << "----Dynamic Texture is nullptr or apollo.drivers.Image is nullptr" << std::endl;
  }
...
void MainWindow::ImageReaderCallback(
  ...
  if (theVideoImgProxy->dynamic_texture_ != nullptr && imgData != nullptr) {
    ...
    if (img.loadFromData(reinterpret_cast<const unsigned char*>(data.c_str()),
      ...
      std::cerr << "-----------Cannot load compressed image from data with QImage" << std::endl;
    }
  } else {
    std::cerr << "----Dynamic Texture is nullptr or apollo.drivers.Image is nullptr" << std::endl;
  }
...
void MainWindow::PlayVideoImage(bool b) {
  QPushButton*   obj         = static_cast<QPushButton*>(QObject::sender());
  VideoImgProxy* theVideoImg = StructPtrByMemberPtr(obj, VideoImgProxy, action_item_button_);
...
void MainWindow::DoPlayVideoImage(bool b, VideoImgProxy* theVideoImg) {
  if (b) {
    ...
    const std::string channelName = theVideoImg->channel_name_combobox_.currentText().toStdString();
    ...
    if (!theVideoImg->image_reader_) {
      if (theVideoImg->isCompressedImage_) {
        ...
        theVideoImg->image_reader_ = new CyberChannReader<apollo::drivers::Image>();
      }
      ...
      if (theVideoImg->isCompressedImage_) {
        auto videoCallback =
            [this, theVideoImg](const std::shared_ptr<apollo::drivers::CompressedImage>& pdata) {
              this->ImageReaderCallback(pdata, theVideoImg);
            };

        ret = theVideoImg->compressed_image_reader_->InstallCallbackAndOpen(videoCallback,
                                                                            channelName, nodeName);
      } else {
        auto videoCallback = [this,
                              theVideoImg](const std::shared_ptr<apollo::drivers::Image>& pdata) {
          this->ImageReaderCallback(pdata, theVideoImg);
        };
        ret = theVideoImg->image_reader_->InstallCallbackAndOpen(videoCallback, channelName,
                                                                 nodeName);
      }
      if (!ret) {
        QMessageBox::warning(this, tr("Setup Channel Callback"),
...
void MainWindow::ChangePointCloudChannel() {
  if (pointcloud_channel_Reader_ != nullptr) {
    ...
    std::string nodeName("Visualizer-");
    nodeName.append(pointcloud_top_item_->text(0).toStdString());
    pointcloud_channel_Reader_->OpenChannel(pointcloud_comboBox_->currentText().toStdString(),
                                            nodeName);
  }
}
...
void MainWindow::ChangeVideoImgChannel() {
  ...
  VideoImgProxy* theVideoImg = StructPtrByMemberPtr(obj, VideoImgProxy, channel_name_combobox_);

  if (theVideoImg->image_reader_ != nullptr) { theVideoImg->deleteReader(); }
}

void MainWindow::ChangeRadarChannel(void) {
  QComboBox* obj   = static_cast<QComboBox*>(QObject::sender());
  RadarData* radar = StructPtrByMemberPtr(obj, RadarData, channel_name_combobox_);

  if (radar->channel_reader_ != nullptr) {
    radar->channel_reader_->CloseChannel();
    std::string nodeName("Visualizer-");
    nodeName.append(radar->root_item_.text(0).toStdString());
    radar->channel_reader_->OpenChannel(obj->currentText().toStdString(), nodeName);
  }
}

void MainWindow::SelectCurrentTreeItem(FixedAspectRatioWidget* dock) {
  if (dock) {
    VideoImgProxy* theVideoImg = StructPtrByMemberPtr(dock, VideoImgProxy, video_image_viewer_);
...
void MainWindow::TopologyChanged(const apollo::cyber::proto::ChangeMsg& changeMsg) {
  if (apollo::cyber::proto::ChangeType::CHANGE_CHANNEL == changeMsg.change_type() &&
...
void MainWindow::AddNewWriter(const apollo::cyber::proto::RoleAttributes& role) {
  const std::string& channelName = role.channel_name();
  if (_channelName2TypeMap.find(channelName) != _channelName2TypeMap.end()) { return; }
  const std::string& msgTypeName    = role.message_type();
  _channelName2TypeMap[channelName] = msgTypeName;

  QTreeWidgetItem* child = new QTreeWidgetItem();
  if (child == nullptr) {
    QMessageBox::warning(this, tr("Error"), tr("No Enough for New Channel!!!"), QMessageBox::Ok);
    return;
  }
  ...
  if (b) { all_channel_root_->addChild(child); }
  ...
  if (msgType.contains("pointcloud", Qt::CaseInsensitive)) { pointcloud_comboBox_->addItem(str); }
