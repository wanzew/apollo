...
common::Status ParkingRoutingManager::Init(const ParkingRoutingTask& parking_routing_task) {
  // get the message form routing
  has_space_    = parking_routing_task.routing_request().has_parking_info();
  has_space_id_ = parking_routing_task.routing_request().parking_info().has_parking_space_id();
  id_           = parking_routing_task.routing_request().parking_info().parking_space_id();
  return common::Status::OK();
}

bool ParkingRoutingManager::SizeVerification(const ParkingRoutingTask& parking_routing_task) {
  auto plot_type = parking_routing_task.routing_request().parking_info().parking_space_type();
  ...
  auto     corner_point      = parking_routing_task.routing_request().parking_info().corner_point();
...
bool ParkingRoutingManager::RoadWidthVerification(const ParkingRoutingTask& parking_routing_task) {
  const auto&  vehicle_config = common::VehicleConfigHelper::Instance()->GetConfig();
