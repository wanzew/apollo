...
DeadEndRoutingManager::DeadEndRoutingManager()
    : monitor_logger_buffer_(apollo::common::monitor::MonitorMessageItem::TASK_MANAGER) {}

common::Status DeadEndRoutingManager::Init(const DeadEndRoutingTask& dead_end_routing_task) {
  cycle_               = 2;
...
bool DeadEndRoutingManager::JudgeCarInDeadEndJunction(const Vec2d&            car_position,
                                                      const common::PointENU& target_point) {
  size_t              waypoint_num   = routing_request_in_.waypoint().size();
  PointENU            dead_end_point = routing_request_in_.waypoint().at(waypoint_num - 1).pose();
  const hdmap::HDMap* base_map_ptr   = hdmap::HDMapUtil::BaseMapPtr();
  std::vector<JunctionInfoConstPtr> junctions;
  JunctionInfoConstPtr              junction;
  if (base_map_ptr->GetJunctions(dead_end_point, FLAGS_search_junction_threshold, &junctions) !=
      0) {
  ...
  for (size_t i = 0; i < junction_num; ++i) {
    if (junctions.at(i)->junction().type() == DEAD_END) {
      ...
      if (distance_to_vehicle < FLAGS_dead_end_destination_threshold) { return true; }
    } else {
      return false;
    }
  }
...
bool DeadEndRoutingManager::GetNewRouting(const localization::Pose& pose,
                                          routing::RoutingRequest*  routing_request) {
  Vec2d car_position;
  car_position.set_x(pose.position().x());
  car_position.set_y(pose.position().y());
  const common::PointENU& target_point = routing_request_out_.waypoint().at(0).pose();
  if (JudgeCarInDeadEndJunction(car_position, target_point)) {
    if (routing_out_flag_) {
      *routing_request = routing_request_out_;
      routing_request->mutable_dead_end_info()->set_dead_end_routing_type(routing::ROUTING_OUT);
      --cycle_;
    ...
    if (routing_in_flag_) {
      *routing_request = routing_request_in_;
      routing_request->mutable_dead_end_info()->set_dead_end_routing_type(routing::ROUTING_IN);
      routing_request->mutable_dead_end_info()->mutable_target_point()->CopyFrom(target_point);
