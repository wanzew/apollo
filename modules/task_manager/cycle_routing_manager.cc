...
common::Status CycleRoutingManager::Init(const CycleRoutingTask& cycle_routing_task) {
  ...
  AINFO << "New cycle routing task: cycle " << cycle_ << ", begin point " << begin_point_.pose().x()
        << " " << begin_point_.pose().y() << ", end point " << end_point_.pose().x() << " "
        << end_point_.pose().y();
...
bool CycleRoutingManager::GetNewRouting(const localization::Pose& pose,
  ...
  AINFO << "GetNewRouting: localization_pose: " << pose.position().x() << " " << pose.position().y()
        << ", begin point " << begin_point_.pose().x() << " " << begin_point_.pose().y()
        << ", end point " << end_point_.pose().x() << " " << end_point_.pose().y() << ", threshold "
        << FLAGS_threshold_for_destination_check << ", allowed_to_send_routing_request "
        << is_allowed_to_route_;
  ...
  if (is_allowed_to_route_) {
    if (CheckPointDistanceInThreshold(begin_point_.pose(), pose.position(),
      ...
      if (!map_service_->ConstructLaneWayPointWithHeading(pose.position().x(), pose.position().y(),
