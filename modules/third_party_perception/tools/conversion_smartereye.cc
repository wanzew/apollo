...
SmartereyeToPerceptionObstacles(const apollo::drivers::SmartereyeObstacles& smartereye_obstacles,
  ...
  obstacles.mutable_lane_marker()->mutable_left_lane_marker()->set_quality(
      smartereye_lanemark.lane_road_data().roadway().left_lane().quality() / 4.0);
  obstacles.mutable_lane_marker()->mutable_left_lane_marker()->set_model_degree(
      smartereye_lanemark.lane_road_data().roadway().left_lane().right_boundary().degree());

  // Convert everything to FLU
  obstacles.mutable_lane_marker()->mutable_left_lane_marker()->set_c0_position(
      smartereye_lanemark.lane_road_data().roadway().left_lane().right_boundary().c0_position());
  obstacles.mutable_lane_marker()->mutable_left_lane_marker()->set_c1_heading_angle(
      smartereye_lanemark.lane_road_data()
          .roadway()
          .left_lane()
          .right_boundary()
          .c1_heading_angle());
  obstacles.mutable_lane_marker()->mutable_left_lane_marker()->set_c2_curvature(
      smartereye_lanemark.lane_road_data().roadway().left_lane().right_boundary().c2_curvature());
  obstacles.mutable_lane_marker()->mutable_left_lane_marker()->set_c3_curvature_derivative(
      smartereye_lanemark.lane_road_data()
          .roadway()
          .left_lane()
          .right_boundary()
  ...
  obstacles.mutable_lane_marker()->mutable_right_lane_marker()->set_quality(
      smartereye_lanemark.lane_road_data().roadway().right_lane().quality() / 4.0);
  obstacles.mutable_lane_marker()->mutable_right_lane_marker()->set_model_degree(
      smartereye_lanemark.lane_road_data().roadway().right_lane().left_boundary().degree());
  obstacles.mutable_lane_marker()->mutable_right_lane_marker()->set_c0_position(
      smartereye_lanemark.lane_road_data().roadway().right_lane().left_boundary().c0_position());
  obstacles.mutable_lane_marker()->mutable_right_lane_marker()->set_c1_heading_angle(
      smartereye_lanemark.lane_road_data()
          .roadway()
          .right_lane()
          .left_boundary()
          .c1_heading_angle());
  obstacles.mutable_lane_marker()->mutable_right_lane_marker()->set_c2_curvature(
      smartereye_lanemark.lane_road_data().roadway().right_lane().left_boundary().c2_curvature());
  obstacles.mutable_lane_marker()->mutable_right_lane_marker()->set_c3_curvature_derivative(
      smartereye_lanemark.lane_road_data()
          .roadway()
          .right_lane()
          .left_boundary()
  ...
  for (int index = 0; index < smartereye_obstacles.num_obstacles() &&
    ...
    const auto& data_obstacle = smartereye_obstacles.output_obstacles().at(index);
    ...
    double sma_z = (data_obstacle.real3dupy() + data_obstacle.real3dlowy()) / 2.0;
    // relative speed
    ...
    if (sma_l > FLAGS_max_mobileye_obstacle_length) { sma_l = GetDefaultObjectLength(sma_type); }
    if (sma_w > FLAGS_max_mobileye_obstacle_width) { sma_w = GetDefaultObjectWidth(sma_type); }
    ...
    if (obstacles.lane_marker().left_lane_marker().quality() >=
      ...
      path_c3 = obstacles.lane_marker().left_lane_marker().c3_curvature_derivative();
    } else {
      ...
      path_c3 = obstacles.lane_marker().right_lane_marker().c3_curvature_derivative();
    }
    ...
    if (!FLAGS_use_navigation_mode) {
      ...
      double nearest_lane_heading = converted_vx > 0 ?
    ...
    // create polygon
    FillPerceptionPolygon(sma, sma->position().x(), sma->position().y(), sma->position().z(),
                          sma->length(), sma->width(), sma->height(), sma->theta());
