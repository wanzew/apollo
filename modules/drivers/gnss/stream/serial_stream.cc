...
SerialStream::SerialStream(const char* device_name, speed_t baud_rate, uint32_t timeout_usec)
    ...
    , errno_(0)
    , is_open_(false) {
  if (device_name_.empty()) { status_ = Stream::Status::ERROR; }
}
...
void SerialStream::open(void) {
  ...
  if (fd == -1) {
    switch (errno) {
      ...
      case EMFILE:
      default:
        AERROR << "Open device " << device_name_ << " failed, error: " << strerror(errno);
        return;
    }
  }
...
size_t SerialStream::read(uint8_t* buffer, size_t max_length) {
  ...
  while (max_length > 0) {
    ...
    if (bytes_current_read < 0) {
      switch (errno) {
        case EAGAIN:
        case EINVAL: bytes_current_read = 0; break;
...
size_t SerialStream::write(const uint8_t* data, size_t length) {
  ...
  while ((length > 0) && (delay_times < 5)) {
    ...
    if (nsent == 0) {
      if (!wait_writable(byte_time_us_)) { break; }
      ++delay_times;
      continue;
    }
...
bool SerialStream::wait_readable(uint32_t timeout_us) {
  ...
  if (!FD_ISSET(fd_, &readfds)) { return false; }
...
bool SerialStream::wait_writable(uint32_t timeout_us) {
  ...
  // This shouldn't happen, if r > 0 our fd has to be in the list!
  if (!FD_ISSET(fd_, &writefds)) { return false; }
  // Data available to write.
  return true;
}

Stream* Stream::create_serial(const char* device_name, uint32_t baud_rate, uint32_t timeout_usec) {
  speed_t baud = get_serial_baudrate(baud_rate);
  return baud == 0 ? nullptr : new SerialStream(device_name, baud, timeout_usec);
}
