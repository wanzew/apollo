...
Stream* Stream::create_udp(const char* address, uint16_t port, uint32_t timeout_usec) {
...
void UdpStream::open() {
  ...
  if (timeout_usec_ != 0) {
    ...
    if (flags == -1) {
      ::close(fd);
      AERROR << "fcntl get flag failed, errno: " << errno << ", " << strerror(errno);
      return;
    }

    if (fcntl(fd, F_SETFL, flags & ~O_NONBLOCK) == -1) {
      ::close(fd);
      AERROR << "fcntl set block failed, errno: " << errno << ", " << strerror(errno);
      return;
    }

    struct timeval block_to = {timeout_usec_ / 1000000, timeout_usec_ % 1000000};
    if (setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<char*>(&block_to),
                   sizeof(block_to)) < 0) {
      ::close(fd);
      AERROR << "setsockopt set rcv timeout failed, errno: " << errno << ", " << strerror(errno);
      return;
    }

    if (setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<char*>(&block_to),
                   sizeof(block_to)) < 0) {
      ::close(fd);
      AERROR << "setsockopt set snd timeout failed, errno: " << errno << ", " << strerror(errno);
      return;
    }
  } else {
    int flags = fcntl(fd, F_GETFL, 0);
    ...
    if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) == -1) {
      ::close(fd);
      AERROR << "fcntl set non block failed, errno: " << errno << ", " << strerror(errno);
      return;
    }
  }
...
size_t UdpStream::read(uint8_t* buffer, size_t max_length) {
  ...
  while ((ret = ::recvfrom(sockfd_, buffer, max_length, 0, (struct sockaddr*)&peer_sockaddr,
...
size_t UdpStream::write(const uint8_t* data, size_t length) {
  ...
  while (length > 0) {
    ssize_t nsent = ::sendto(sockfd_, data, length, 0, (struct sockaddr*)&peer_sockaddr,
