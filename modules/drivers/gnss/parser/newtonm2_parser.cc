...
Parser* Parser::CreateNewtonM2(const config::Config& config) { return new NewtonM2Parser(config); }
...
bool NewtonM2Parser::HandleCorrImuData(const novatel::CorrImuData* imu) {
  newtonm2::rfu_to_flu(
      imu->x_velocity_change * imu_measurement_hz_, imu->y_velocity_change * imu_measurement_hz_,
  ...
  double seconds = imu->gps_week * newtonm2::SECONDS_PER_WEEK + imu->gps_seconds;
...
bool NewtonM2Parser::HandleInsCov(const novatel::InsCov* cov) {
  for (int i = 0; i < 9; ++i) {
    ...
    ins_.set_euler_angles_covariance(newtonm2::INDEX[i],
...
bool NewtonM2Parser::HandleInsPva(const novatel::InsPva* pva) {
  ...
  ins_.mutable_euler_angles()->set_z(newtonm2::azimuth_deg_to_yaw_rad(pva->azimuth));
  ...
  double seconds = pva->gps_week * newtonm2::SECONDS_PER_WEEK + pva->gps_seconds;
  if (ins_.measurement_time() != seconds) {
    ins_.set_measurement_time(seconds);
    return false;
  }
...
bool NewtonM2Parser::HandleRawImuX(const novatel::RawImuX* imu) {
  ...
  switch (imu_frame_mapping_) {
    case 5:  // Default mapping.
      newtonm2::rfu_to_flu(
          imu->x_velocity_change * accel_scale_, -imu->y_velocity_change_neg * accel_scale_,
    ...
    case 6:
      newtonm2::rfu_to_flu(
          -imu->y_velocity_change_neg * accel_scale_, imu->x_velocity_change * accel_scale_,
          -imu->z_velocity_change * accel_scale_, imu_.mutable_linear_acceleration());
      newtonm2::rfu_to_flu(-imu->y_angle_change_neg * gyro_scale_,
                           imu->x_angle_change * gyro_scale_, -imu->z_angle_change * gyro_scale_,
...
bool NewtonM2Parser::HandleRawImu(const novatel::RawImu* imu) {
  ...
  switch (imu_frame_mapping_) {
    case 5:  // Default mapping.
      newtonm2::rfu_to_flu(
          imu->x_velocity_change * accel_scale, -imu->y_velocity_change_neg * accel_scale,
          imu->z_velocity_change * accel_scale, imu_.mutable_linear_acceleration());
      newtonm2::rfu_to_flu(imu->x_angle_change * gyro_scale, -imu->y_angle_change_neg * gyro_scale,
                           imu->z_angle_change * gyro_scale, imu_.mutable_angular_velocity());
      break;
    case 6:
      newtonm2::rfu_to_flu(
          -imu->y_velocity_change_neg * accel_scale, imu->x_velocity_change * accel_scale,
          -imu->z_velocity_change * accel_scale, imu_.mutable_linear_acceleration());
      newtonm2::rfu_to_flu(-imu->y_angle_change_neg * gyro_scale, imu->x_angle_change * gyro_scale,
                           -imu->z_angle_change * gyro_scale, imu_.mutable_angular_velocity());
      break;
    default: AERROR_EVERY(5) << "Unsupported IMU frame mapping: " << imu_frame_mapping_;
  }
...
bool NewtonM2Parser::DecodeGnssObservation(const uint8_t* obs_data, const uint8_t* obs_data_end) {
      ...
      case 1:  // observation data
        ...
        for (int i = 0; i < raw_.obs.n; ++i) {
          ...
          for (j = 0; j < NFREQ + NEXOBS; ++j) {
            if (newtonm2::is_zero(raw_.obs.data[i].L[j])) { break; }
