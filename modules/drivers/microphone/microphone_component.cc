...
bool MicrophoneComponent::Init() {
  ...
  if (!apollo::cyber::common::GetProtoFromFile(config_file_path_, microphone_config_ptr_.get())) {
    return false;
  }
  ...
  n_chunk_    = static_cast<int>(std::ceil(microphone_config_ptr_->sample_rate() *
  ...
  audio_data_ptr_->mutable_header()->set_frame_id(microphone_config_ptr_->frame_id());
  ...
  writer_ptr_   = node_->CreateWriter<AudioData>(microphone_config_ptr_->channel_name());
