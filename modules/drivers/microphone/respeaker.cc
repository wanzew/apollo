...
void Stream::init_stream(
    int rate, int channels, int chunk, int input_device_index, PaSampleFormat format) {
  ...
  err = Pa_OpenStream(&pastream_ptr_, input_parameters_ptr_, nullptr, rate, chunk,
...
void Stream::read_stream(int n_frames, char* buffer) const {
  err = Pa_ReadStream(pastream_ptr_, reinterpret_cast<void*>(buffer), n_frames);
  if (err != paNoError) {
...
void Respeaker::init(const std::shared_ptr<const MicrophoneConfig>& microphone_config) {
  ...
  stream_ptr_->init_stream(microphone_config->sample_rate(), microphone_config->channel_type_size(),
                           microphone_config->chunk(), device_index,
                           get_format_from_width(microphone_config->sample_width()));
}

const PaSampleFormat Respeaker::get_format_from_width(int width, bool is_unsigned) const {
  switch (width) {
    case 1: return is_unsigned ? paUInt8 : paInt8;
    case 2: return paInt16;
    case 3: return paInt24;
    case 4: return paFloat32;
    default: break;
  }
...
const PaDeviceInfo* Respeaker::get_device_info(const PaDeviceIndex index) const {
  const PaDeviceInfo* device_info = reinterpret_cast<const PaDeviceInfo*>(Pa_GetDeviceInfo(index));
  if (!device_info) { AERROR << "internal error: invalid device index" << index; }

  return device_info;
}

const PaDeviceIndex
Respeaker::host_api_device_index_to_device_index(const PaHostApiIndex host_api,
                                                 const int            host_api_device_index) const {
  // Get standard device index from host-API-specific device index
  PaDeviceIndex device_index = Pa_HostApiDeviceIndexToDeviceIndex(host_api, host_api_device_index);
  if (device_index < 0) { report_error(device_index, "Pa_HostApiDeviceIndexToDeviceIndex"); }
  return device_index;
}

const PaHostApiInfo* Respeaker::get_host_api_info(const PaHostApiIndex index) const {
  ...
  if (!pa_host_api_info) { AERROR << "internal error: invalid Host Api Index " << index; }
