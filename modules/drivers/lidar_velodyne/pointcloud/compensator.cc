...
bool Compensator::pointcloud_compensate(const sensor_msgs::PointCloud2ConstPtr msg,
...
inline void Compensator::get_timestamp_interval(const sensor_msgs::PointCloud2ConstPtr msg,
...
inline bool Compensator::check_message(const sensor_msgs::PointCloud2ConstPtr msg) {
...
bool Compensator::query_pose_affine_from_tf2(const double& timestamp, Eigen::Affine3d* pose) {
  ...
  if (!tf2_buffer.canTransform("world", conf_.child_frame_id(), query_time,
                               ros::Duration(conf_.tf_query_timeout()), &err_string)) {
    AWARN << "Can not find transform. " << std::fixed << timestamp << " Error info: " << err_string
          << " Please check location and tf, "
  ...
  try {
    stamped_transform = tf2_buffer.lookupTransform("world", conf_.child_frame_id(), query_time);
...
inline uint Compensator::get_field_size(const int datatype) {
  switch (datatype) {
    ...
    default: AERROR << "can not get field size by datatype:" << datatype; return 0;
  }
}
