...
Status Velodyne::Init() {
  ...
  for (auto i = 0; i < conf_size; i++) {
    ...
    if (FLAGS_pipeline_mode) { size += conf_unit_.conf(i).npackets(); }
...
void Velodyne::Packet(RawDataCache* output, const VelodyneConf& conf) {
  ...
  velodyne_msgs::VelodyneScanUnifiedPtr full_scan(new velodyne_msgs::VelodyneScanUnified());
  full_scan->packets.resize(conf.npackets());
  int scan_idx = 0;

  while (running_) {
    velodyne_msgs::VelodyneScanUnifiedPtr scan(new velodyne_msgs::VelodyneScanUnified());
    if (!dvr->poll(scan)) {
    ...
    if (FLAGS_pipeline_mode) {
      pub = full_scan;
      memcpy(&(full_scan->packets[scan_idx].data[0]), &(scan->packets[0].data[0]),
             FIRING_DATA_PACKET_SIZE);
      full_scan->packets[scan_idx].stamp = scan->packets[0].stamp;
      scan_idx++;

      if (scan_idx != conf.npackets()) { continue; }
...
void Velodyne::Convert(RawDataCache* input, PointCloudCache* output, const VelodyneConf& conf) {
  ...
  while (running_) {
    ...
    if (FLAGS_publish_raw_pointcloud) {
      MultipleVelodyneAdapter::PublishPointCloudRawByIndex(conf.index(), pointcloud);
    }
...
void Velodyne::PointCloudFusion(std::map<uint32_t, PointCloudCache*> fusion_cache) {
  ...
  while (running_) {
    ...
    int64_t                     t1 = GetTime();
    sensor_msgs::PointCloud2Ptr point_cloud_fusion(new sensor_msgs::PointCloud2());
    ...
    if (it != fusion_cache.end()) {
      if (size == 1) {
        ...
        for (; it != fusion_cache.end(); it++) {
          if (it->first == major_index) { continue; }
          slave_point_cloud_vec.push_back(it->second->take());
        }
        t1              = GetTime();
        bool fusion_ret = velodyne_fusion.fusion(major_point_cloud_ptr, slave_point_cloud_vec,
                                                 point_cloud_fusion);
        if (!fusion_ret) { AERROR << "fusion error!"; }
...
Status Velodyne::Start() {
  auto conf_size = conf_unit_.conf_size();
  for (auto i = 0; i < conf_size; i++) {
    std::shared_ptr<std::thread> packet_thread(new std::thread(
        std::bind(&Velodyne::Packet, this, packet_cache_vec_[i], conf_unit_.conf(i))));
    std::shared_ptr<std::thread> convert_therad(
        new std::thread(std::bind(&Velodyne::Convert, this, packet_cache_vec_[i],
                                  pointcloud_cache_vec_[i], conf_unit_.conf(i))));
    std::shared_ptr<std::thread> compensate_thread(new std::thread(
        std::bind(&Velodyne::Compensate, this, pointcloud_cache_vec_[i], conf_unit_.conf(i))));
  ...
  if (is_fusion_) {
    std::shared_ptr<std::thread> fusion_thread(
        new std::thread(std::bind(&Velodyne::PointCloudFusion, this, fusion_cache_)));
...
void Velodyne::Notice() {
  ...
  AINFO_IF(!FLAGS_publish_raw_pointcloud) << "publish_raw_pointcloud flag closed,"
                                          << "will not publish pointcloud2 raw data";
  AINFO_IF(!FLAGS_publish_compensator_pointcloud) << "publish_compensator_pointcloud flag closed,"
...
bool Velodyne::FusionCheckInit(const std::map<uint32_t, uint8_t>& velodyne_index_map) {
  if (is_fusion_) {
    uint32_t major_index           = conf_unit_.fusion_conf().major_index();
    ...
    for (; it != fusion_index_map_.end(); it++) {
      ...
      if (velodyne_index_map.count(index) == 0) { return false; }
      fusion_pointcloud_cache->init(conf_unit_.conf(index).cache_size(),
