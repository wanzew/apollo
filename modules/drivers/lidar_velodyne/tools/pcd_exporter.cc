...
PCDExporter::~PCDExporter() {
  ...
  if (pose_file_handle_ != NULL) { fclose(pose_file_handle_); }
}
...
bool PCDExporter::init(const VelodyneToolsConf& conf) {
  ...
  if (!boost::filesystem::exists(conf_.pcd_folder())) {
    AINFO << "The directory" << conf_.pcd_folder() << "is not exists, create now";
...
int PCDExporter::write_pcd_pose_file(const sensor_msgs::PointCloud2::ConstPtr& msg, int index) {
  double time = msg->header.stamp.toSec();

  Eigen::Matrix4d pose;
  if (!get_pose(msg->header.stamp, &pose)) { return -1; }
  ...
  if (conf_.skip_static_frames()) {
    ...
    if (!get_pose(query_time, &pose2)) { return -1; }
...
bool PCDExporter::get_pose(const ros::Time& time, Eigen::Matrix4d* pose) {
  ...
  try {
    ...
    stamped_transform = tf_buffer_ptr_->lookupTransform("world", conf_.child_frame_id(), time);
...
void PCDExporter::pcd_writer_callback(const sensor_msgs::PointCloud2& cloud) {
  ...
  *pt = cloud;
  ...
  for (auto iter = queue_.begin(); iter != queue_.end();) {
    ...
    std::string pcd_filename =
        conf_.pcd_folder() + "/" + boost::lexical_cast<std::string>(index) + ".pcd";
    ...
    if (ret == 0) {
      ...
      std::list<sensor_msgs::PointCloud2ConstPtr>::iterator iter_inside = queue_.begin();
