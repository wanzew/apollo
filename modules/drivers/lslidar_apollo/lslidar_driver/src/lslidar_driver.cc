...
LslidarDriver::LslidarDriver(ros::NodeHandle& n, ros::NodeHandle& pn)
    : nh(n)
    , pnh(pn)
    , socket_id(-1) {}

LslidarDriver::~LslidarDriver() { (void)close(socket_id); }
...
bool LslidarDriver::createRosIO() {
  ...
  packet_pub = nh.advertise<lslidar_msgs::LslidarPacket>("lslidar_packet", 100);
  return true;
}
...
int LslidarDriver::getPacket(lslidar_msgs::LslidarPacketPtr& packet) {
...
void LslidarDriver::getFPGA_GPSTimeStamp(lslidar_msgs::LslidarPacketPtr& packet) {
  ...
  if (head2[0] == 0xA5 && head2[1] == 0xFF) {
    ...
    packet_timestamp = (packet->data[1200] + packet->data[1201] * pow(2, 8) +
                        packet->data[1202] * pow(2, 16) + packet->data[1203] * pow(2, 24)) *
                       1e3;
    ...
    ROS_DEBUG("ROS TS: %f, GPS: y:%d m:%d d:%d h:%d m:%d s:%d; FPGA: us:%lu", timeStamp.toSec(),
              cur_time.tm_year, cur_time.tm_mon, cur_time.tm_mday, cur_time.tm_hour,
              cur_time.tm_min, cur_time.tm_sec, packet_timestamp);
  }
}
