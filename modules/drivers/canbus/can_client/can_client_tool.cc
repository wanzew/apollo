...
struct TestCanParam {
  ...
  void print() {
    AINFO << "conf: " << conf.ShortDebugString() << ", total send: " << send_cnt + send_err_cnt
          << "/" << FLAGS_agent_mutual_send_frames << ", send_ok: " << send_cnt
          << " , send_err_cnt: " << send_err_cnt << ", send_lost_cnt: " << send_lost_cnt
...
class CanAgent {
 public:
  explicit CanAgent(TestCanParam* param_ptr)
  ...
  void SendThreadFunc() {
    ...
    id              = start_id;
    int32_t send_id = id;
    AINFO << "port:" << param->conf.ShortDebugString() << ", start_id:" << start_id
          << ", end_id:" << end_id;
    ...
    while (true) {
      // param->print();
      if (count >= FLAGS_agent_mutual_send_frames) { break; }
      ...
      if (client->Send(frames, &frame_num) != ErrorCode::OK) {
        ...
        AINFO << "send_frames: " << frame_num << "send_frame#" << frames[0].CanFrameString()
    ...
    while (!other_agent()->is_sending_finish()) {
      ...
      for (int32_t i = 0; i < len; ++i) {
        ...
        AINFO << "recv_frame#" << buf[i].CanFrameString()
              << " conf:" << param->conf.ShortDebugString() << ",recv_cnt: " << param->recv_cnt;
      }
    }
    int64_t end      = AsInt64<micros>(Clock::Now());
  ...
  void WaitForFinish() {
    if (thread_send_ != nullptr && thread_send_->joinable()) {
      thread_send_->join();
      thread_send_.reset();
      AINFO << "Send thread stopped. conf:" << param_ptr_->conf.ShortDebugString();
    }
    if (thread_recv_ != nullptr && thread_recv_->joinable()) {
      thread_recv_->join();
      thread_recv_.reset();
      AINFO << "Recv thread stopped. conf:" << param_ptr_->conf.ShortDebugString();
    }
  }
...
int main(int32_t argc, char** argv) {
  ...
  if (!apollo::common::util::GetProtoFromFile(FLAGS_can_client_conf_file_a, &can_client_conf_a)) {
    AERROR << "Unable to load canbus conf file: " << FLAGS_can_client_conf_file_a;
    return 1;
  } else {
  ...
  if (!client_a || !client_a->Init(can_client_conf_a) || client_a->Start() != ErrorCode::OK) {
  ...
  if (!FLAGS_only_one_send) {
    if (!apollo::common::util::GetProtoFromFile(FLAGS_can_client_conf_file_b, &can_client_conf_b)) {
      AERROR << "Unable to load canbus conf file: " << FLAGS_can_client_conf_file_b;
      return 1;
    } else {
    ...
    if (!client_b || !client_b->Init(can_client_conf_b) || client_b->Start() != ErrorCode::OK) {
  ...
  if (!FLAGS_only_one_send) { agent_b.WaitForFinish(); }
  param_ptr_a->print();
  if (!FLAGS_only_one_send) { param_ptr_b->print(); }
