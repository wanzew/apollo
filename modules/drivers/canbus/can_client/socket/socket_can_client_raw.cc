...
ErrorCode SocketCanClientRaw::Start() {
  ...
  if (interface_ != CANCardParameter::VIRTUAL) {
    ...
    ret = setsockopt(dev_handler_, SOL_CAN_RAW, CAN_RAW_FILTER, &filter, sizeof(filter));
    if (ret < 0) {
  ...
  int enable = 1;
  ret        = ::setsockopt(dev_handler_, SOL_CAN_RAW, CAN_RAW_FD_FRAMES, &enable, sizeof(enable));
  if (ret < 0) {
  ...
  ret              = ::bind(dev_handler_, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr));
...
ErrorCode SocketCanClientRaw::Send(const std::vector<CanFrame>& frames, int32_t* const frame_num) {
  ...
  for (size_t i = 0; i < frames.size() && i < MAX_CAN_SEND_FRAME_LEN; ++i) {
    if (frames[i].len > CANBUS_MESSAGE_LENGTH || frames[i].len < 0) {
      AERROR << "frames[" << i << "].len = " << frames[i].len
             << ", which is not equal to can message data length (" << CANBUS_MESSAGE_LENGTH
             << ").";
      return ErrorCode::CAN_CLIENT_ERROR_SEND_FAILED;
    }
    ...
    // Synchronous transmission of CAN messages
    int ret = static_cast<int>(write(dev_handler_, &send_frames_[i], sizeof(send_frames_[i])));
    if (ret <= 0) {
...
ErrorCode SocketCanClientRaw::Receive(std::vector<CanFrame>* const frames,
  ...
  for (int32_t i = 0; i < *frame_num && i < MAX_CAN_RECV_FRAME_LEN; ++i) {
    ...
    if (recv_frames_[i].can_dlc > CANBUS_MESSAGE_LENGTH || recv_frames_[i].can_dlc < 0) {
      AERROR << "recv_frames_[" << i << "].can_dlc = " << recv_frames_[i].can_dlc
             << ", which is not equal to can message data length (" << CANBUS_MESSAGE_LENGTH
             << ").";
      return ErrorCode::CAN_CLIENT_ERROR_RECV_FAILED;
    }
...
std::string SocketCanClientRaw::GetErrorString(const int32_t /*status*/) { return ""; }
