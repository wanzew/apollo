...
CameraDriver::CameraDriver(const CameraH265Config* h265_cfg) { config_ = *h265_cfg; }

bool CameraDriver::Poll(std::shared_ptr<CompressedImage> h265) { return PollByFrame(h265); }
...
bool CameraDriver::PollByFrame(std::shared_ptr<CompressedImage> h265Pb) {
  ...
  AINFO << "Get frame from port: " << config_.udp_port() << ", size: " << h265Pb->data().size()
        << ", ts: camera/host " << camera_timestamp << "/" << current_time
        << ", diff: " << static_cast<double>(current_time - camera_timestamp) * 1e-6;
