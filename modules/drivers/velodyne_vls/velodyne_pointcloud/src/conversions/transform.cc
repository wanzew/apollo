...
Transform::Transform(ros::NodeHandle node, ros::NodeHandle private_nh)
  ...
  tf_filter_ = new tf::MessageFilter<velodyne_msgs::VelodyneScan>(velodyne_scan_, listener_,
                                                                  config_.frame_id, 10);
  tf_filter_->registerCallback(boost::bind(&Transform::processScan, this, _1));
}

void Transform::reconfigure_callback(velodyne_pointcloud::TransformNodeConfig& config,
                                     uint32_t                                  level) {
...
void Transform::processScan(const velodyne_msgs::VelodyneScan::ConstPtr& scanMsg) {
  ...
  for (size_t next = 0; next < scanMsg->packets.size(); ++next) {
    ...
    try {
      ROS_DEBUG_STREAM("transforming from " << inPc_.header.frame_id << " to " << config_.frame_id);
      VPointCloud inPc_V;
