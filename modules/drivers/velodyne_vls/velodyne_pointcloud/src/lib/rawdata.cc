...
void RawData::compute_xyzi(const uint8_t  chan_id,
  ...
  intensity +=
      focal_slope * (abs(focal_offset - 256 * (1 - static_cast<float>(azimuth_uint) / 65535) *
 ...
 *  @param pc shared pointer to point cloud (points are appended)
 */
void RawData::unpack_hdl64(const velodyne_msgs::VelodynePacket& pkt, VPointCloud& pc) {
      ...
      for (int seq_id = 0, k = 0; seq_id < VLP16_NUM_SEQS_PER_BLOCK; seq_id++) {
        for (int chan_id = 0; chan_id < VLP16_NUM_CHANS_PER_SEQ; chan_id++, k += CHANNEL_SIZE) {
          ...
          if (pointInRange(distance)) {
            ...
            azimuth_corrected_f =
                azimuth +
                (azimuth_diff * ((chan_id * CHANNEL_TDURATION) + (seq_id * SEQ_TDURATION)) /
                 VLP16_BLOCK_TDURATION);
            azimuth_corrected = ((uint16_t)round(azimuth_corrected_f)) % 36000;

            // apply calibration file and convert polar coordinates to Euclidean XYZ
            compute_xyzi(chan_id, azimuth_corrected, distance, intensity, x_coord, y_coord,
                         z_coord);
 ...
 *  @param pc shared pointer to point cloud (points are appended)
 */
void RawData::unpack_vlp32(const velodyne_msgs::VelodynePacket& pkt, VPointCloud& pc) {
  float    azimuth_diff, azimuth_corrected_f;
  float    last_azimuth_diff = 0;
  uint16_t azimuth, azimuth_next, azimuth_corrected;
  float    x_coord, y_coord, z_coord;
...
unsigned int swizzle_bytes_in_word(unsigned int a) {
  unsigned int temp;
  // std::cerr << "Intoswizzle (0x" << std::setw(8) << std::hex  << a << ")" << std::dec <<
  // std::endl;
  temp = 0;
  temp |= ((a & 0xff000000) >> 0);
  temp |= ((a & 0x00ff0000) >> 0);
 ...
 *  @param pc shared pointer to point cloud (points are appended)
 */
void RawData::unpack_hdl32(const velodyne_msgs::VelodynePacket& pkt, VPointCloud& pc) {
  float    azimuth_diff, azimuth_corrected_f;
  float    last_azimuth_diff = 0;
  uint16_t azimuth, azimuth_next, azimuth_corrected;
  float    x_coord, y_coord, z_coord;
    ...
    if ((config_.min_angle < config_.max_angle && azimuth >= config_.min_angle &&
      ...
      for (int j = 0, k = 0; j < NUM_CHANS_PER_BLOCK; j++, k += CHANNEL_SIZE) {
        ...
        if (pointInRange(distance)) {
          ...
          /** correct for the laser rotation as a function of timing during the firings **/
          azimuth_corrected_f =
              azimuth + (azimuth_diff * (chan_id * HDL32_CHANNEL_TDURATION) / HDL32_SEQ_TDURATION);
          azimuth_corrected = ((uint16_t)round(azimuth_corrected_f)) % 36000;
