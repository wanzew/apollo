...
double convertBinaryToDecimal(std::string binaryString) {
  ...
  for (int i = binaryString.length() - 1; i >= 0; i--) {
    if (binaryString[i] == '1') { value += pow(2, indexCounter); }
    indexCounter++;
  }
  return value;
}
...
VelodyneDriver::VelodyneDriver(ros::NodeHandle node, ros::NodeHandle private_nh) {
  ...
  {                                   // 1 packet holds 384 points
    ...
    packet_rate = 1507;  // 12 groups of 32 firings where a pair of 2 firings corresponds
                         // to 55.296us -> 1/(12*55.296us)
    model_full_name = "VLP-32C";
  } else if (config_.model == "VLS128") {
    packet_rate = 1507;  // 3 groups of 128 firings where a set of 8 firings corresponds to 55.296us
                         // -> 1/(12*55.296us)
