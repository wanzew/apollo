...
void cameraCallback(boost::shared_ptr<cv::Mat> matp, double timestamp, int picid, bool distortion) {
  printf("callback %d %d timestamp %lf \n", picid, distortion, timestamp);
}

int main(int argc, char** argv) {
  Pandora pandora(std::string("192.168.20.51"), 2368, 10110, lidarCallback, gpsCallback, 13500,
                  9870, cameraCallback, 1, 0, std::string("hesai40"));
  pandora.Start();
  while (true) {
    sleep(100);
  }
