...
int PandoraCamera::Start() {
  ...
  pandora_client_ =
      PandoraClientNew(ip_.c_str(), camera_port_, CameraClientCallback, static_cast<void*>(this));
  if (!pandora_client_) {
    continue_process_pic_ = false;
    return -1;
  }
  process_pic_thread_ = new boost::thread(boost::bind(&PandoraCamera::processPic, this));
...
void PandoraCamera::Stop() {
  continue_process_pic_ = false;
  if (pandora_client_) { PandoraClientDestroy(pandora_client_); }
  pandora_client_ = NULL;
...
void PandoraCamera::processPic() {
  while (continue_process_pic_) {
    struct timespec ts;
    if (clock_gettime(CLOCK_REALTIME, &ts) == -1) { printf("processPic, get time error\n"); }

    ts.tv_sec += 1;
    if (sem_timedwait(&pic_sem_, &ts) == -1) { continue; }
    ...
    switch (pic->header.pic_id) {
      case 0:
        ...
        if (need_remap_)
          remap(cvMatPic->clone(), *cvMatPic, mapx_[pic->header.pic_id], mapy_[pic->header.pic_id],
                CV_INTER_LINEAR);
        break;
      case 1:
      case 2:
      ...
      case 4:
        uint8_t* bmp;
        uint32_t bmpSize;
        decompressJpeg(static_cast<uint8_t*>(pic->yuv), pic->header.len, &bmp, &bmpSize);
    ...
    if (camera_callback_) {
      ...
      camera_callback_(cvMatPic, timestamp + 2 + tz_second_, pic->header.pic_id, need_remap_);
    }
    free(pic->yuv);
    pic->yuv = NULL;
    free(pic);
...
bool PandoraCamera::loadIntrinsics(const std::vector<cv::Mat>& cameras_k,
  ...
  for (int i = 0; i < CAMERA_NUM; i++) {
    ...
    initUndistortRectifyMap(cameras_k[i], cameras_d[i], R, cameras_k[i], HesaiLidarSDK_IMAGE_SIZE,
                            CV_32FC1, mapx, mapy);
...
void PandoraCamera::yuvToRgb(const int iY, const int iU, const int iV, int* iR, int* iG, int* iB) {
...
int PandoraCamera::decompressJpeg(uint8_t*       jpgBuffer,
