...
static const float pandar40p_elev_angle_map[] = {
    6.96,   5.976,  4.988,  3.996,  2.999,   2.001,   1.667,   1.333,  1.001,   0.667,
    0.333,  0,      -0.334, -0.667, -1.001,  -1.334,  -1.667,  -2.001, -2.331,  -2.667,
    -3,     -3.327, -3.663, -3.996, -4.321,  -4.657,  -4.986,  -5.311, -5.647,  -5.974,
    -6.957, -7.934, -8.908, -9.871, -10.826, -11.772, -12.705, -13.63, -14.543, -15.444};

// Line 40 Lidar azimuth Horizontal offset ,  Line 1 - Line 40
static const float pandar40p_horizatal_azimuth_offset_map[] = {
    0.005,  0.006,  0.006,  0.006,  -2.479, -2.479, 2.491,  -4.953, -2.479, 2.492,
    -4.953, -2.479, 2.492,  -4.953, 0.007,  2.491,  -4.953, 0.006,  4.961,  -2.479,
    0.006,  4.96,   -2.478, 0.006,  4.958,  -2.478, 2.488,  4.956,  -2.477, 2.487,
    2.485,  2.483,  0.004,  0.004,  0.003,  0.003,  -2.466, -2.463, -2.46,  -2.457};
...
Pandar40P_Internal::Pandar40P_Internal(
  ...
  for (int i = 0; i < LASER_COUNT; ++i) {
    ...
    horizatal_azimuth_offset_map_[i] = pandar40p_horizatal_azimuth_offset_map[i];
  }
...
int Pandar40P_Internal::LoadCorrectionFile(const std::string& correction_content) {  // NOLINT
  ...
  while (std::getline(ifs, line)) {
    ...
    if (lineId != lineCounter) { return -1; }
 ...
 */
void Pandar40P_Internal::ResetStartAngle(uint16_t start_angle) { start_angle_ = start_angle; }

int Pandar40P_Internal::Start() {
  Stop();
  enable_lidar_recv_thr_    = true;
  enable_lidar_process_thr_ = true;
  lidar_process_thr_ = new boost::thread(boost::bind(&Pandar40P_Internal::ProcessLiarPacket, this));
  lidar_recv_thr_    = new boost::thread(boost::bind(&Pandar40P_Internal::RecvTask, this));
}
...
void Pandar40P_Internal::ProcessLiarPacket() {
  ...
  while (enable_lidar_process_thr_) {
    if (clock_gettime(CLOCK_REALTIME, &ts) == -1) { std::cout << "get time error" << std::endl; }

    ts.tv_sec += 1;
    if (sem_timedwait(&lidar_sem_, &ts) == -1) { continue; }
...
void Pandar40P_Internal::ProcessGps(const PandarGPS& gpsMsg) {
  ...
  if (gps_callback_) { gps_callback_(static_cast<double>(mktime(&t) + tz_second_) + 1); }
}

int Pandar40P_Internal::ParseRawData(Pandar40PPacket* packet, const uint8_t* buf, const int len) {
  if (len != PACKET_SIZE) {
    std::cout << "packet size mismatch Pandar40P_Internal " << len << "," << PACKET_SIZE
              << std::endl;
    return -1;
  }
...
int Pandar40P_Internal::ParseGPS(PandarGPS* packet, const uint8_t* recvbuf, const int size) {
  if (size != GPS_PACKET_SIZE) { return -1; }
  int index    = 0;
...
void Pandar40P_Internal::CalcPointXYZIT(Pandar40PPacket*               pkt,
                                        int                            blockid,
                                        boost::shared_ptr<PPointCloud> cld) {
  Pandar40PBlock* block = &pkt->blocks[blockid];

  double unix_second = static_cast<double>(mktime(&pkt->t) + 1 + tz_second_);  // 1 second offset
  ...
  for (int i = 0; i < LASER_COUNT; ++i) {
    ...
    /* skip wrong points */
    if (unit.distance <= 0.5 || unit.distance > 200.0) { continue; }
    ...
    if (pkt->echo == 0x39) {
      ...
      point.timestamp =
          point.timestamp -
          (static_cast<double>(blockOffset_[blockid / 2] + laserOffset_[i / 2]) / 1000000.0f);
    } else {
      point.timestamp = point.timestamp -
                        (static_cast<double>(blockOffset_[blockid] + laserOffset_[i]) / 1000000.0f);
    }
