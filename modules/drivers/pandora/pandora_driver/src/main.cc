...
class PandoraHesaiClient {
 public:
  PandoraHesaiClient(ros::NodeHandle node, ros::NodeHandle nh) {
    ...
    bool ret = parseParameter(nh, &pandoraIP, &lidarRecvPort, &gpsRecvPort, &startAngle,
                              &pandoraCameraPort, &lidarTopic, cameraTopics, &enableCamera,
                              &timezone, &frameId);
    if (!ret) {
  ...
  bool parseParameter(ros::NodeHandle nh,
                      std::string*    pandoraIP,
                      int*            lidarRecvPort,
                      int*            gpsRecvPort,
                      int*            startAngle,
                      int*            pandoraCameraPort,
                      std::string*    lidarTopic,
                      std::string     cameraTopics[],
                      bool*           enableCamera,
                      int*            timezone,
                      std::string*    frameId) {
    if (nh.hasParam("pandora_ip")) { nh.getParam("pandora_ip", *pandoraIP); }
    if (nh.hasParam("lidar_recv_port")) { nh.getParam("lidar_recv_port", *lidarRecvPort); }
    if (nh.hasParam("gps_recv_port")) { nh.getParam("gps_recv_port", *gpsRecvPort); }
    if (nh.hasParam("start_angle")) { nh.getParam("start_angle", *startAngle); }
    if (nh.hasParam("pandora_camera_port")) {
      nh.getParam("pandora_camera_port", *pandoraCameraPort);
    }
    if (nh.hasParam("lidar_topic")) { nh.getParam("lidar_topic", *lidarTopic); }
    if (nh.hasParam("camera0_topic")) { nh.getParam("camera0_topic", cameraTopics[0]); }
    if (nh.hasParam("camera1_topic")) { nh.getParam("camera1_topic", cameraTopics[1]); }
    if (nh.hasParam("camera2_topic")) { nh.getParam("camera2_topic", cameraTopics[2]); }
    if (nh.hasParam("camera3_topic")) { nh.getParam("camera3_topic", cameraTopics[3]); }
    if (nh.hasParam("camera4_topic")) { nh.getParam("camera4_topic", cameraTopics[4]); }
    if (nh.hasParam("enable_camera")) { nh.getParam("enable_camera", *enableCamera); }
    if (nh.hasParam("timezone")) { nh.getParam("timezone", *timezone); }
    if (nh.hasParam("frame_id")) { nh.getParam("frame_id", *frameId); }

    std::cout << "Configs: pandoraIP: " << *pandoraIP << ", lidarRecvPort: " << *lidarRecvPort
              << ", gpsRecvPort: " << *gpsRecvPort << ", startAngle: " << *startAngle
              << ", pandoraCameraPort: " << *pandoraCameraPort << ", lidarTopic: " << *lidarTopic
              << ", enableCamera: " << *enableCamera << ", frameId: " << *frameId << std::endl;
    for (int i = 0; i < 5; i++) {
    ...
    return checkPort(*lidarRecvPort) && checkPort(*gpsRecvPort) && checkPort(*pandoraCameraPort) &&
  ...
  void
  cameraCallback(boost::shared_ptr<cv::Mat> matp, double timestamp, int pic_id, bool distortion) {
