...
Compensator::Compensator(ros::NodeHandle node, ros::NodeHandle private_nh)
  ...
  private_nh.param("topic_compensated_pointcloud", topic_compensated_pointcloud_,
  ...
  compensation_pub_ =
      node.advertise<sensor_msgs::PointCloud2>(topic_compensated_pointcloud_, queue_size_);
  pointcloud_sub_ =
      node.subscribe(topic_pointcloud_, queue_size_, &Compensator::pointcloud_callback,
...
void Compensator::pointcloud_callback(sensor_msgs::PointCloud2ConstPtr msg) {
  ...
  if (query_pose_affine_from_tf2(timestamp_min, &pose_min_time) &&
    ...
    *q_msg = *msg;
    motion_compensation<float>(q_msg, timestamp_min, timestamp_max, pose_min_time, pose_max_time);
    q_msg->header.stamp.fromSec(timestamp_max);
    compensation_pub_.publish(q_msg);
  }
}

inline void Compensator::get_timestamp_interval(sensor_msgs::PointCloud2ConstPtr msg,
  ...
  *timestamp_min = std::numeric_limits<double>::max();
  ...
  for (int i = 0; i < total; ++i) {
    ...
    memcpy(&timestamp, &msg->data[i * msg->point_step + timestamp_offset_], timestamp_data_size_);
...
inline bool Compensator::check_message(sensor_msgs::PointCloud2ConstPtr msg) {
  ...
  for (size_t i = 0; i < msg->fields.size(); ++i) {
    ...
    if (f.name == "x") {
      ...
      if (f.count != 1 || y_offset_ == -1) { return false; }
    } else if (f.name == "z") {
      z_offset_   = f.offset;
      z_data_type = f.datatype;
      if (f.count != 1 || z_offset_ == -1) { return false; }
    } else if (f.name == "timestamp") {
      timestamp_offset_    = f.offset;
      timestamp_data_size_ = f.count * get_field_size(f.datatype);
      if (timestamp_offset_ == -1 || timestamp_data_size_ == -1) { return false; }
    } else {
  ...
  if (!(x_data_type == y_data_type && y_data_type == z_data_type)) { return false; }
  return true;
}

bool Compensator::query_pose_affine_from_tf2(const double timestamp, Eigen::Affine3d* pose) {
  ros::Time   query_time(timestamp);
  std::string err_string;
  if (!tf2_buffer_.canTransform("world", child_frame_id_, query_time, ros::Duration(tf_timeout_),
                                &err_string)) {
    ROS_WARN_STREAM("Can not find transform. " << std::fixed << timestamp
  ...
  try {
    stamped_transform = tf2_buffer_.lookupTransform("world", child_frame_id_, query_time);
...
inline uint Compensator::get_field_size(const int datatype) {
  switch (datatype) {
    ...
    default: ROS_ERROR_STREAM("can not get field size by datatype:" << datatype); return 0;
  }
}
...
void Compensator::motion_compensation(sensor_msgs::PointCloud2::Ptr msg,
  ...
  for (int i = 0; i < total; ++i) {
    Scalar* x_scalar = reinterpret_cast<Scalar*>(&msg->data[i * msg->point_step + x_offset_]);
    if (std::isnan(*x_scalar)) {
      ROS_DEBUG_STREAM("nan point do not need motion compensation");
      continue;
    }
    Scalar* y_scalar = reinterpret_cast<Scalar*>(&msg->data[i * msg->point_step + y_offset_]);
    Scalar* z_scalar = reinterpret_cast<Scalar*>(&msg->data[i * msg->point_step + z_offset_]);
    Eigen::Vector3d p(*x_scalar, *y_scalar, *z_scalar);
