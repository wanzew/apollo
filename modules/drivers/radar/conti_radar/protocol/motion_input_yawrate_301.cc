...
void MotionInputYawRate301::UpdateData(uint8_t* data) {
  ...
  yaw_rate_low.set_value(static_cast<unsigned char>((yaw_rate_value & 0xFF00) >> 8), 0, 8);
  Byte yaw_rate_high(data + 1);
  yaw_rate_high.set_value(static_cast<unsigned char>(yaw_rate_value & 0x00FF), 0, 8);
}
...
void MotionInputYawRate301::SetYawRate(const float& yaw_rate) { yaw_rate_ = yaw_rate; }
