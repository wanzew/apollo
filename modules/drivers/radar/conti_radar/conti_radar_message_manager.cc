...
void ContiRadarMessageManager::set_can_client(std::shared_ptr<CanClient> can_client) {
  can_client_ = can_client;
}

ProtocolData<ContiRadar>*
ContiRadarMessageManager::GetMutableProtocolDataById(const uint32_t message_id) {
...
void ContiRadarMessageManager::Parse(const uint32_t message_id,
  ...
  if (message_id == RadarState201::ID) {
    ...
    if (sensor_data_.radar_state().send_quality() == radar_config_.radar_conf().send_quality() &&
      ...
      SenderMessage<ContiRadar> sender_message(RadarConfig200::ID, &radar_config_);
