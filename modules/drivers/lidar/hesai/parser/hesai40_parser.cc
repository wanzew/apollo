...
Hesai40Parser::Hesai40Parser(const std::shared_ptr<Node>& node, const Config& conf)
    : Parser(node, conf) {
...
void Hesai40Parser::ParseRawPacket(const uint8_t* buf, const int len, bool* is_end) {
  // PrintMem(buf, len);
  ...
  for (int i = 0; i < BLOCKS_PER_PACKET; ++i) {
    ...
    if (last_azimuth_ != packet.blocks[i].azimuth && azimuthGap < 600 /* 6 degree*/) {
      /* for all the blocks */
