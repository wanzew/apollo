...
bool Compensator::QueryPoseAffineFromTF2(const uint64_t&    timestamp,
                                         void*              pose,
  ...
  if (!tf2_buffer_ptr_->canTransform(config_.world_frame_id(), child_frame_id, query_time,
    ...
    AERROR << "Can not find transform. " << timestamp << " frame_id:" << child_frame_id
...
bool Compensator::MotionCompensation(const std::shared_ptr<const PointCloud>& msg,
  ...
  msg_compensated->mutable_header()->set_timestamp_sec(cyber::Time::Now().ToSecond());
  ...
  if (QueryPoseAffineFromTF2(timestamp_min, &pose_min_time, frame_id) &&
    ...
    AINFO << "compenstator tf msg diff:" << tf_time - new_time
          << ";meta:" << msg->header().lidar_timestamp();
    MotionCompensation(msg, msg_compensated, timestamp_min, timestamp_max, pose_min_time,
                       pose_max_time);
...
inline void Compensator::GetTimestampInterval(const std::shared_ptr<const PointCloud>& msg,
...
void Compensator::MotionCompensation(const std::shared_ptr<const PointCloud>& msg,
                                     std::shared_ptr<PointCloud>              msg_compensated,
