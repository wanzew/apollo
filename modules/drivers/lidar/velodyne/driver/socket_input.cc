...
int SocketInput::get_firing_data_packet(VelodynePacket* pkt) {
  ...
  while (true) {
    if (!input_available(POLL_TIMEOUT)) { return SOCKET_TIMEOUT; }
    // Receive packets that should now be available from the
    // socket using a blocking read.
    uint8_t bytes[FIRING_DATA_PACKET_SIZE];
    ssize_t nbytes = recvfrom(sockfd_, bytes, FIRING_DATA_PACKET_SIZE, 0, nullptr, nullptr);
...
int SocketInput::get_positioning_data_packet(NMEATimePtr nmea_time) {
  while (true) {
    ...
    ssize_t nbytes = recvfrom(sockfd_, bytes, POSITIONING_DATA_PACKET_SIZE, 0, nullptr, nullptr);
