...
void Velodyne128Parser::GeneratePointcloud(const std::shared_ptr<VelodyneScan>& scan_msg,
  ...
  if (size == 0) {
    ...
    const auto timestamp = out_msg->point(static_cast<int>(size) - 1).timestamp();
...
uint64_t Velodyne128Parser::GetTimestamp(double base_time, float time_offset, uint16_t block_id) {
  (void)block_id;
...
void Velodyne128Parser::Order(std::shared_ptr<PointCloud> cloud) { (void)cloud; }

void Velodyne128Parser::Unpack(const VelodynePacket& pkt, std::shared_ptr<PointCloud> pc) {
  ...
  for (int block = 0; block < BLOCKS_PER_PACKET; block++) {
    ...
    for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
      ...
      float real_distance = raw_distance.raw_distance * VSL128_DISTANCE_RESOLUTION;
      float distance      = real_distance + corrections.dist_correction;

      uint64_t timestamp = static_cast<uint64_t>(
          GetTimestamp(basetime, (*inner_time_)[block][j], static_cast<uint16_t>(block)));
       ...
       * firings **/
      azimuth_corrected_f =
          azimuth + (azimuth_diff * (firing_order * CHANNEL_TDURATION) / SEQ_TDURATION);
      azimuth_corrected = (static_cast<uint16_t>(round(azimuth_corrected_f))) % 36000;
      ...
      intensity = IntensityCompensate(corrections, raw_distance.raw_distance, intensity);
...
int Velodyne128Parser::IntensityCompensate(const LaserCorrection& corrections,
  ...
  intensity += static_cast<int>(
      focal_slope *
      static_cast<float>(
          std::abs(focal_offset - 256.0f * (1.0f - static_cast<float>(raw_distance) / 65535.0f) *
