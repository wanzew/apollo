...
void Velodyne64Parser::setup() {
  ...
  if (!config_.calibration_online() && config_.organized()) { InitOffsets(); }
}
...
void Velodyne64Parser::GeneratePointcloud(const std::shared_ptr<VelodyneScan>& scan_msg,
  ...
  if (config_.calibration_online() && !calibration_.initialized_) {
    ...
    if (config_.organized()) { InitOffsets(); }
  }
...
uint64_t Velodyne64Parser::GetTimestamp(double base_time, float time_offset, uint16_t block_id) {
  ...
  if (is_s2_) {
    ...
    timestamp = static_cast<double>(GetGpsStamp(t, &previous_packet_stamp, &gps_base_usec));
  } else {                                           // 64E_S3
    index                           = block_id & 3;  // % 4
    double&   previous_packet_stamp = previous_packet_stamp_[index];
    uint64_t& gps_base_usec         = gps_base_usec_[index];
...
int Velodyne64Parser::IntensityCompensate(const LaserCorrection& corrections,
  ...
  intensity += static_cast<int>(corrections.focal_slope *
...
void Velodyne64Parser::Unpack(const VelodynePacket& pkt, std::shared_ptr<PointCloud> pc) {
  ...
  for (int i = 0; i < BLOCKS_PER_PACKET; ++i) {  // 12
    ...
    for (int j = 0, k = 0; j < SCANS_PER_BLOCK; ++j, k += RAW_SCAN_SIZE) {  // 32, 3
      // One point
      uint8_t laser_number = static_cast<uint8_t>(j + bank_origin);  // hardware laser number
      ...
      // compute time
      uint64_t timestamp = GetTimestamp(basetime, (*inner_time_)[i][j], static_cast<uint16_t>(i));
      ...
      point->set_intensity(
          IntensityCompensate(corrections, raw_distance.raw_distance, raw->blocks[i].data[k + 2]));
