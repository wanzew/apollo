...
void Velodyne16Parser::GeneratePointcloud(const std::shared_ptr<VelodyneScan>& scan_msg,
...
uint64_t Velodyne16Parser::GetTimestamp(double base_time, float time_offset, uint16_t block_id) {
  double   t         = base_time - time_offset;
  uint64_t timestamp = Velodyne16Parser::GetGpsStamp(t, &previous_packet_stamp_, &gps_base_usec_);
  return timestamp;
}
 ...
 */
void Velodyne16Parser::Unpack(const VelodynePacket& pkt, std::shared_ptr<PointCloud> pc) {
