...
Velodyne32Parser::Velodyne32Parser(const Config& config)
    ...
    , previous_firing_stamp_(0)
    , gps_base_usec_(0) {
  inner_time_             = &velodyne::INNER_TIME_HDL32E;
  need_two_pt_correction_ = false;
  if (config_.model() == VLP32C) { inner_time_ = &velodyne::INNER_TIME_VLP32C; }
}

void Velodyne32Parser::GeneratePointcloud(const std::shared_ptr<VelodyneScan>& scan_msg,
...
uint64_t Velodyne32Parser::GetTimestamp(double base_time, float time_offset, uint16_t block_id) {
  double t = base_time - time_offset;
  if (config_.model() == VLP32C) { t = base_time + time_offset; }
  ...
  if (t < previous_firing_stamp_) {
    ...
    if (std::abs(previous_firing_stamp_ - t) > 3599000000) {
      ...
      AINFO << "Base time plus 3600s. Model: " << config_.model() << std::fixed << ". current:" << t
            << ", last time:" << previous_firing_stamp_;
    } else if (config_.model() != VLP32C || (previous_firing_stamp_ - t > 34.560f) ||
  ...
  } else if (previous_firing_stamp_ != 0 && t - previous_firing_stamp_ > 100000) {  // 100ms
...
void Velodyne32Parser::UnpackVLP32C(const VelodynePacket& pkt, std::shared_ptr<PointCloud> pc) {
  ...
  for (int i = 0; i < BLOCKS_PER_PACKET; i++) {  // 12
    ...
    if (i < (BLOCKS_PER_PACKET - 1)) {
      azimuth_diff = static_cast<float>(
          (36000 + raw->blocks[i + 1].rotation - raw->blocks[i].rotation) % 36000);
    ...
    for (int laser_id = 0, k = 0; laser_id < SCANS_PER_BLOCK;
      ...
      uint64_t timestamp =
          GetTimestamp(basetime, (*inner_time_)[i][laser_id], static_cast<uint16_t>(i));

      azimuth_corrected_f = azimuth + (azimuth_diff * (static_cast<float>(laser_id) / 2.0f) *
                                       CHANNEL_TDURATION / SEQ_TDURATION);
      azimuth_corrected   = static_cast<int>(round(fmod(azimuth_corrected_f, 36000.0)));

      float real_distance = raw_distance.raw_distance * VLP32_DISTANCE_RESOLUTION;
      ...
      ComputeCoords(real_distance, corrections, static_cast<uint16_t>(azimuth_corrected), point);
...
void Velodyne32Parser::Unpack(const VelodynePacket& pkt, std::shared_ptr<PointCloud> pc) {
  ...
  for (int i = 0; i < BLOCKS_PER_PACKET; i++) {  // 12
    for (int laser_id = 0, k = 0; laser_id < SCANS_PER_BLOCK;
      ...
      if (raw_distance.raw_distance == 0 || !is_scan_valid(rotation, distance)) {
      ...
      point->set_timestamp(timestamp);
      // Position Calculation, append this point to the cloud
      ComputeCoords(real_distance, corrections, static_cast<uint16_t>(rotation), point);
      point->set_intensity(raw->blocks[i].data[k + 2]);
...
void Velodyne32Parser::Order(std::shared_ptr<PointCloud> cloud) {
  if (config_.model() == VLP32C) { return; }
  int width = 32;
