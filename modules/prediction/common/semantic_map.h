...
class SemanticMap {
 ...
 private:
  cv::Point2i
  GetTransPoint(const double x, const double y, const double base_x, const double base_y) {
    return cv::Point2i(static_cast<int>((x - base_x) / 0.1),
                       static_cast<int>(2000 - (y - base_y) / 0.1));
  }

  void DrawBaseMap(const double x, const double y, const double base_x, const double base_y);
  ...
  void DrawADCTrajectory(const cv::Scalar& color,
                         const double      base_x,
                         const double      base_y,
                         cv::Mat*          img);

  cv::Mat CropArea(const cv::Mat& input_img, const cv::Point2i& center_point, const double heading);
