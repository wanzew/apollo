...
double ValidationChecker::ProbabilityByCentripedalAcceleration(const LaneSequence& lane_sequence,
                                                               const double        speed) {
  ...
  double mean_cost =
      centripetal_acc_cost_sqr_sum / (centripetal_acc_cost_sum + FLAGS_double_precision);
...
bool ValidationChecker::ValidCentripedalAcceleration(
  ...
  for (std::size_t i = 0; i + 1 < num_point; ++i) {
    ...
    double      theta_diff =
        std::abs(second_point.path_point().theta() - first_point.path_point().theta());
    double time_diff = std::abs(second_point.relative_time() - first_point.relative_time());
    if (time_diff < FLAGS_double_precision) { continue; }
...
bool ValidationChecker::ValidTrajectoryPoint(const TrajectoryPoint& trajectory_point) {
  return trajectory_point.has_path_point() && (!std::isnan(trajectory_point.path_point().x())) &&
         (!std::isnan(trajectory_point.path_point().y())) &&
         (!std::isnan(trajectory_point.path_point().theta())) &&
         (!std::isnan(trajectory_point.v())) && (!std::isnan(trajectory_point.a())) &&
