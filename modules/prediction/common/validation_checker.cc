...
double ValidationChecker::ProbabilityByCentripetalAcceleration(const LaneSequence& lane_sequence,
                                                               const double        speed) {
...
bool ValidationChecker::ValidCentripetalAcceleration(
  ...
  for (size_t i = 0; i + 1 < trajectory_points.size(); ++i) {
    ...
    double theta_diff =
        std::abs(common::math::NormalizeAngle(p1.path_point().theta() - p0.path_point().theta()));
    double v         = (p0.v() + p1.v()) * 0.5;
    double angular_a = v * theta_diff / time_diff;
    if (angular_a > FLAGS_centripedal_acc_threshold) { return false; }
  }
  return true;
}
