...
bool ValidFeatureHistory(const ObstacleHistory& obstacle_history,
  ...
  if (obstacle_history.feature_size() == 0) { return false; }
...
void SemanticMap::RunCurrFrame(
  ...
  if (obstacle_id_history_map.find(FLAGS_ego_vehicle_id) == obstacle_id_history_map.end()) {
    return;
  }
  ...
  if (FLAGS_enable_draw_adc_trajectory) {
    DrawADCTrajectory(cv::Scalar(0, 255, 255), curr_base_x_, curr_base_y_, &curr_img_);
  }

  // Draw all obstacles_history
  for (const auto obstacle_id_history_pair : obstacle_id_history_map) {
    DrawHistory(obstacle_id_history_pair.second, cv::Scalar(0, 255, 255), curr_base_x_,
                curr_base_y_, &curr_img_);
  }
...
void SemanticMap::DrawBaseMap(const double x,
                              const double y,
                              const double base_x,
                              const double base_y) {
...
void SemanticMap::DrawRoads(const common::PointENU& center_point,
  ...
  for (const auto& road : roads) {
    for (const auto& section : road->road().section()) {
        ...
        if (edge.type() == 2) {  // left edge
          for (const auto& segment : edge.curve().segment()) {
            for (const auto& point : segment.line_segment().point()) {
              polygon.push_back(std::move(GetTransPoint(point.x(), point.y(), base_x, base_y)));
            }
          }
      ...
      cv::fillPoly(base_img_, std::vector<std::vector<cv::Point>>({std::move(polygon)}), color);
    }
  }
}
...
void SemanticMap::DrawJunctions(const common::PointENU& center_point,
  ...
  apollo::hdmap::HDMapUtil::BaseMap().GetJunctions(center_point, 141.4, &junctions);
...
void SemanticMap::DrawCrosswalks(const common::PointENU& center_point,
  ...
  apollo::hdmap::HDMapUtil::BaseMap().GetCrosswalks(center_point, 141.4, &crosswalks);
    ...
    for (const auto& segment : lane->lane().central_curve().segment()) {
      for (int i = 0; i < segment.line_segment().point_size() - 1; ++i) {
        ...
        const auto& p1 = GetTransPoint(segment.line_segment().point(i + 1).x(),
                                       segment.line_segment().point(i + 1).y(), base_x, base_y);
        double      theta =
            atan2(segment.line_segment().point(i + 1).y() - segment.line_segment().point(i).y(),
                  segment.line_segment().point(i + 1).x() - segment.line_segment().point(i).x());
...
cv::Scalar SemanticMap::HSVtoRGB(double H, double S, double V) {
  ...
  switch (I) {
    case 0: return cv::Scalar(V, K, M) * 255.0;
    case 1: return cv::Scalar(N, V, M) * 255.0;
    case 2: return cv::Scalar(M, V, K) * 255.0;
    case 3: return cv::Scalar(M, N, V) * 255.0;
    case 4: return cv::Scalar(K, M, V) * 255.0;
    default: return cv::Scalar(V, M, N) * 255.0;
  }
}
...
void SemanticMap::DrawRect(const Feature&    feature,
  ...
  polygon.push_back(std::move(GetTransPoint(obs_x + (cos(theta) * obs_l - sin(theta) * obs_w) / 2,
  ...
  polygon.push_back(std::move(GetTransPoint(obs_x + (cos(theta) * obs_l - sin(theta) * -obs_w) / 2,
                                            obs_y + (sin(theta) * obs_l + cos(theta) * -obs_w) / 2,
                                            base_x, base_y)));
  cv::fillPoly(*img, std::vector<std::vector<cv::Point>>({std::move(polygon)}), color);
}
...
void SemanticMap::DrawPoly(const Feature&    feature,
  ...
  for (auto& polygon_point : feature.polygon_point()) {
    polygon.push_back(
        std::move(GetTransPoint(polygon_point.x(), polygon_point.y(), base_x, base_y)));
  }
  cv::fillPoly(*img, std::vector<std::vector<cv::Point>>({std::move(polygon)}), color);
}
...
cv::Mat SemanticMap::CropArea(const cv::Mat&     input_img,
  ...
  cv::Mat rotation_mat = cv::getRotationMatrix2D(center_point, 90.0 - heading * 180.0 / M_PI, 1.0);
  cv::Mat rotated_mat;
...
cv::Mat SemanticMap::CropByHistory(const ObstacleHistory& history,
  ...
  const cv::Point2i& center_point =
      GetTransPoint(curr_feature.position().x(), curr_feature.position().y(), base_x, base_y);
...
bool SemanticMap::GetMapById(const int obstacle_id, cv::Mat* feature_map) {
  if (obstacle_id_history_map_.find(obstacle_id) == obstacle_id_history_map_.end()) {
    return false;
  }
  const auto& obstacle_history = obstacle_id_history_map_[obstacle_id];

  if (!ValidFeatureHistory(obstacle_history, curr_base_x_, curr_base_y_)) { return false; }

  cv::Mat output_img = CropByHistory(obstacle_id_history_map_[obstacle_id], cv::Scalar(0, 0, 255),
