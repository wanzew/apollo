...
std::vector<double> Softmax(const std::vector<double>& value, bool use_exp) {
  ...
  for (std::size_t i = 0; i < value.size(); ++i) {
    ...
    if (use_exp) { exp_value = std::exp(value[i]); }
    sum += exp_value;
...
int SolveQuadraticEquation(const std::vector<double>& coefficients,
  ...
  if (coefficients.size() != 3) { return -1; }
  const double a = coefficients[0];
  const double b = coefficients[1];
  const double c = coefficients[2];
  if (std::fabs(a) <= std::numeric_limits<double>::epsilon()) { return -1; }
...
double EvaluateQuinticPolynomial(const std::array<double, 6>& coeffs,
  ...
  if (t >= end_t) {
    switch (order) {
      case 0: {
        double end_value =
            ((((coeffs[5] * end_t + coeffs[4]) * end_t + coeffs[3]) * end_t + coeffs[2]) * end_t +
             coeffs[1]) *
                end_t +
            coeffs[0];
  ...
  switch (order) {
    case 0: {
      return ((((coeffs[5] * t + coeffs[4]) * t + coeffs[3]) * t + coeffs[2]) * t + coeffs[1]) * t +
             coeffs[0];
    }
    case 1: {
      return (((5.0 * coeffs[5] * t + 4.0 * coeffs[4]) * t + 3.0 * coeffs[3]) * t +
              2.0 * coeffs[2]) *
                 t +
             coeffs[1];
    }
    case 2: {
      return (((20.0 * coeffs[5] * t + 12.0 * coeffs[4]) * t) + 6.0 * coeffs[3]) * t +
             2.0 * coeffs[2];
    }
    case 3: {
...
double EvaluateQuarticPolynomial(const std::array<double, 5>& coeffs,
  ...
  if (t >= end_t) {
    switch (order) {
      case 0: {
        double end_value =
            (((coeffs[4] * end_t + coeffs[3]) * end_t + coeffs[2]) * end_t + coeffs[1]) * end_t +
            coeffs[0];
...
double EvaluateCubicPolynomial(const std::array<double, 4>& coefs,
  ...
  if (t > end_t) {
    switch (order) {
      case 0: {
        double end_value = ((coefs[3] * end_t + coefs[2]) * end_t + coefs[1]) * end_t + coefs[0];
        return end_value + (t - end_t) * end_v;
      }
      case 1: {
        return end_v;
...
double GetSByConstantAcceleration(const double v0, const double acceleration, const double t) {
  if (acceleration > -FLAGS_double_precision) { return v0 * t + 0.5 * acceleration * t * t; }
...
void GenerateFreeMoveTrajectoryPoints(Eigen::Matrix<double, 6, 1>*       state,
  ...
  for (std::size_t i = 0; i < num; ++i) {
    ...
    if (speed > std::numeric_limits<double>::epsilon()) {
      ...
      if (points->size() > 0) { theta = points->back().path_point().theta(); }
    }
    ...
    trajectory_point.set_relative_time(start_time + static_cast<double>(i) * period);
...
double AdjustSpeedByCurvature(const double speed, const double curvature) {
  if (std::abs(curvature) < FLAGS_turning_curvature_lower_bound) { return speed; }
  ...
  return apollo::common::math::lerp(
      ...
      FLAGS_speed_at_upper_curvature, FLAGS_turning_curvature_upper_bound, curvature);
}
