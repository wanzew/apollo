...
TEST_F(JunctionAnalyzerTest, SingleLane) {
  ...
  const JunctionFeature& junction_feature = junction_analyzer.GetJunctionFeature("l61");
...
TEST_F(JunctionAnalyzerTest, MultiLane) {
  ...
  const JunctionFeature& merged_junction_feature =
      junction_analyzer.GetJunctionFeature(std::vector<std::string>{"l35", "l61", "l114", "l162"});
