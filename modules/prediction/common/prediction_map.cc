...
Eigen::Vector2d PredictionMap::PositionOnLane(std::shared_ptr<const LaneInfo> lane_info,
                                              const double                    s) {
  common::PointENU point = lane_info->GetSmoothPoint(s);
  return {point.x(), point.y()};
}

double PredictionMap::HeadingOnLane(std::shared_ptr<const LaneInfo> lane_info, const double s) {
...
double PredictionMap::LaneTotalWidth(std::shared_ptr<const hdmap::LaneInfo> lane_info,
                                     const double                           s) {
  double left  = 0.0;
  double right = 0.0;
...
void PredictionMap::OnLane(const std::vector<std::shared_ptr<const LaneInfo>>& prev_lanes,
  ...
  for (const auto& candidate_lane : candidate_lanes) {
    ...
    if (on_lane && !candidate_lane->IsOnLane(vec_point)) { continue; }
    ...
    double           distance              = 0.0;
    common::PointENU nearest_point         = candidate_lane->GetNearestPoint(vec_point, &distance);
...
bool PredictionMap::InJunction(const Eigen::Vector2d& point, const double radius) {
  ...
  for (const auto junction_info : junction_infos) {
    ...
    if (junction_polygon.IsPointIn(vec)) { return true; }
  }
  return false;
}
  ...
  if (lanes.size() == 0) {
    ...
    for (auto& lane_ptr : lanes) {
      ...
      for (auto& lane_id : lane_ptr->lane().left_neighbor_forward_lane_id()) {
        ...
        if (s >= 0.0 && std::fabs(l) > radius) { continue; }
        lane_ids.insert(id);
...
bool PredictionMap::IsLeftNeighborLane(std::shared_ptr<const LaneInfo> left_lane,
                                       std::shared_ptr<const LaneInfo> curr_lane) {
  if (curr_lane == nullptr) { return true; }
  if (left_lane == nullptr) { return false; }
  for (const auto& left_lane_id : curr_lane->lane().left_neighbor_forward_lane_id()) {
    if (left_lane->id().id() == left_lane_id.id()) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsLeftNeighborLane(std::shared_ptr<const LaneInfo> left_lane,
  ...
  for (auto& lane : lanes) {
    if (IsLeftNeighborLane(left_lane, lane)) { return true; }
  }
  return false;
}

bool PredictionMap::IsRightNeighborLane(std::shared_ptr<const LaneInfo> right_lane,
                                        std::shared_ptr<const LaneInfo> curr_lane) {
  if (curr_lane == nullptr) { return true; }
  if (right_lane == nullptr) { return false; }
  for (auto& right_lane_id : curr_lane->lane().right_neighbor_forward_lane_id()) {
    if (right_lane->id().id() == right_lane_id.id()) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsRightNeighborLane(std::shared_ptr<const LaneInfo> right_lane,
  ...
  if (lanes.size() == 0) { return true; }
  for (const auto& lane : lanes) {
    if (IsRightNeighborLane(right_lane, lane)) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsSuccessorLane(std::shared_ptr<const LaneInfo> succ_lane,
                                    std::shared_ptr<const LaneInfo> curr_lane) {
  if (curr_lane == nullptr) { return true; }
  if (succ_lane == nullptr) { return false; }
  for (auto& successor_lane_id : curr_lane->lane().successor_id()) {
    if (succ_lane->id().id() == successor_lane_id.id()) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsSuccessorLane(std::shared_ptr<const LaneInfo>                     succ_lane,
  ...
  if (lanes.size() == 0) { return true; }
  for (auto& lane : lanes) {
    if (IsSuccessorLane(succ_lane, lane)) { return true; }
  }
  return false;
}

bool PredictionMap::IsPredecessorLane(std::shared_ptr<const LaneInfo> pred_lane,
                                      std::shared_ptr<const LaneInfo> curr_lane) {
  if (curr_lane == nullptr) { return true; }
  if (pred_lane == nullptr) { return false; }
  for (auto& predecessor_lane_id : curr_lane->lane().predecessor_id()) {
    if (pred_lane->id().id() == predecessor_lane_id.id()) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsPredecessorLane(std::shared_ptr<const LaneInfo>                     pred_lane,
  ...
  if (lanes.size() == 0) { return true; }
  for (auto& lane : lanes) {
    if (IsPredecessorLane(pred_lane, lane)) { return true; }
  }
  return false;
}
