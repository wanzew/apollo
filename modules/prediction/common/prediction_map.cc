...
Eigen::Vector2d PredictionMap::PositionOnLane(const std::shared_ptr<const LaneInfo> lane_info,
                                              const double                          s) {
  common::PointENU point = lane_info->GetSmoothPoint(s);
  return {point.x(), point.y()};
}

double PredictionMap::HeadingOnLane(const std::shared_ptr<const LaneInfo> lane_info,
                                    const double                          s) {
  return lane_info->Heading(s);
}

double PredictionMap::CurvatureOnLane(const std::string& lane_id, const double s) {
...
double PredictionMap::LaneTotalWidth(const std::shared_ptr<const hdmap::LaneInfo> lane_info,
                                     const double                                 s) {
  double left  = 0.0;
  double right = 0.0;
...
std::shared_ptr<const LaneInfo> PredictionMap::LaneById(const std::string& str_id) {
  return HDMapUtil::BaseMap().GetLaneById(hdmap::MakeMapId(str_id));
}

std::shared_ptr<const JunctionInfo> PredictionMap::JunctionById(const std::string& str_id) {
  return HDMapUtil::BaseMap().GetJunctionById(hdmap::MakeMapId(str_id));
}

std::shared_ptr<const OverlapInfo> PredictionMap::OverlapById(const std::string& str_id) {
  return HDMapUtil::BaseMap().GetOverlapById(hdmap::MakeMapId(str_id));
}

bool PredictionMap::GetProjection(const Eigen::Vector2d&                pos,
...
bool PredictionMap::HasNearbyLane(const double x, const double y, const double radius) {
...
bool PredictionMap::ProjectionFromLane(std::shared_ptr<const LaneInfo> lane_info,
                                       const double                    s,
...
bool PredictionMap::IsVirtualLane(const std::string& lane_id) {
  ...
  bool left_virtual       = lane.has_left_boundary() && lane.left_boundary().has_virtual_() &&
                      lane.left_boundary().virtual_();
  bool right_virtual = lane.has_right_boundary() && lane.right_boundary().has_virtual_() &&
                       lane.right_boundary().virtual_();
  return left_virtual && right_virtual;
}

bool PredictionMap::OnVirtualLane(const Eigen::Vector2d& point, const double radius) {
  std::vector<std::shared_ptr<const LaneInfo>> lanes;
  ...
  for (const auto& lane : lanes) {
    if (IsVirtualLane(lane->id().id())) { return true; }
  }
  return false;
}

void PredictionMap::OnLane(const std::vector<std::shared_ptr<const LaneInfo>>& prev_lanes,
  ...
  if (HDMapUtil::BaseMap().GetLanesWithHeading(hdmap_point, radius, heading, max_lane_angle_diff,
                                               &candidate_lanes) != 0) {
    return;
  }
  ...
  for (const auto& candidate_lane : candidate_lanes) {
    if (candidate_lane == nullptr) { continue; }
    if (on_lane && !candidate_lane->IsOnLane({point.x(), point.y()})) { continue; }
    if (!FLAGS_use_navigation_mode && !IsIdenticalLane(candidate_lane, prev_lanes) &&
    ...
    double diff = std::fabs(common::math::AngleDiff(heading, nearest_point_heading));
    if (diff <= max_lane_angle_diff) { lane_pairs.emplace_back(candidate_lane, diff); }
  }
  if (lane_pairs.empty()) { return; }
  ...
  for (const auto& lane_pair : lane_pairs) {
    ...
    ++count;
    if (count >= max_num_lane) { break; }
  }
}

std::shared_ptr<const LaneInfo>
PredictionMap::GetMostLikelyCurrentLane(const common::PointENU& position,
                                        const double            radius,
                                        const double            heading,
                                        const double            angle_diff_threshold) {
  std::vector<std::shared_ptr<const LaneInfo>> candidate_lanes;
  if (HDMapUtil::BaseMap().GetLanesWithHeading(position, radius, heading, angle_diff_threshold,
                                               &candidate_lanes) != 0) {
    return nullptr;
  }
  ...
  for (auto candidate_lane : candidate_lanes) {
    if (!candidate_lane->IsOnLane({position.x(), position.y()})) { continue; }
    double           distance = 0.0;
    common::PointENU nearest_point =
        candidate_lane->GetNearestPoint({position.x(), position.y()}, &distance);
    double nearest_point_heading = PathHeading(candidate_lane, nearest_point);
    double angle_diff = std::fabs(common::math::AngleDiff(heading, nearest_point_heading));
...
bool PredictionMap::IsProjectionApproximateWithinLane(const Eigen::Vector2d& ego_position,
                                                      const std::string&     lane_id) {
  auto        ptr_lane    = LaneById(lane_id);
  const auto& lane_points = ptr_lane->points();
  if (lane_points.size() < 2) { return false; }
  ...
  if (approx_lane_length < 1.0e-3) { return false; }

  auto dist_vec = common::math::Vec2d(ego_position.x(), ego_position.y()) - start_point;

  auto projection_length = dist_vec.InnerProd(lane_vec) / approx_lane_length;

  if (projection_length < 0.0 || projection_length > approx_lane_length) { return false; }
  return true;
}

bool PredictionMap::NearJunction(const Eigen::Vector2d& point, const double radius) {
  common::PointENU hdmap_point;
  hdmap_point.set_x(point.x());
  hdmap_point.set_y(point.y());
...
bool PredictionMap::IsPointInJunction(const double                              x,
                                      const double                              y,
...
PredictionMap::GetJunctions(const Eigen::Vector2d& point, const double radius) {
  common::PointENU hdmap_point;
  hdmap_point.set_x(point.x());
  hdmap_point.set_y(point.y());
  std::vector<std::shared_ptr<const JunctionInfo>> junctions;
...
PredictionMap::GetPNCJunctions(const Eigen::Vector2d& point, const double radius) {
  common::PointENU hdmap_point;
  hdmap_point.set_x(point.x());
  hdmap_point.set_y(point.y());
...
bool PredictionMap::InJunction(const Eigen::Vector2d& point, const double radius) {
  ...
  if (junction_infos.empty()) { return false; }
  for (const auto junction_info : junction_infos) {
    if (junction_info == nullptr || !junction_info->junction().has_polygon()) { continue; }
    ...
    if (junction_polygon.IsPointIn({point.x(), point.y()})) { return true; }
  }
  return false;
}

bool PredictionMap::IsLaneInJunction(const std::shared_ptr<const LaneInfo> lane_info,
                                     const std::string&                    junction_id) {
  if (lane_info == nullptr) { return false; }

  // first, check whether the lane is virtual
  if (!PredictionMap::IsVirtualLane(lane_info->lane().id().id())) { return false; }

  // second, use junction from lane
  if (lane_info->lane().has_junction_id() && lane_info->lane().junction_id().id() == junction_id) {
    return true;
  }

  // third, use junction from road
  auto ptr_road_info = HDMapUtil::BaseMap().GetRoadById(lane_info->road_id());
  if (ptr_road_info->has_junction_id() && ptr_road_info->junction_id().id() == junction_id) {
    return true;
  }
...
bool PredictionMap::SmoothPointFromLane(const std::string& id,
                                        const double       s,
                                        const double       l,
                                        Eigen::Vector2d*   point,
                                        double*            heading) {
  if (point == nullptr || heading == nullptr) { return false; }
...
void PredictionMap::NearbyLanesByCurrentLanes(
  ...
  if (lanes.empty()) {
    ...
    OnLane(prev_lanes, point, heading, radius, false, max_num_lane, FLAGS_max_lane_angle_diff,
           nearby_lanes);
  } else {
    ...
    for (auto& lane_ptr : lanes) {
      ...
      for (auto& lane_id : lane_ptr->lane().left_neighbor_forward_lane_id()) {
        ...
        if (lane_ids.find(id) != lane_ids.end()) { continue; }
        std::shared_ptr<const LaneInfo> nearby_lane = LaneById(id);
        double                          s           = -1.0;
        double                          l           = 0.0;
        GetProjection(point, nearby_lane, &s, &l);
        if (s < 0.0 || s >= nearby_lane->total_length() || std::fabs(l) > radius) { continue; }
        lane_ids.insert(id);
...
std::shared_ptr<const LaneInfo>
PredictionMap::GetLeftNeighborLane(const std::shared_ptr<const LaneInfo>& ptr_ego_lane,
                                   const Eigen::Vector2d&                 ego_position,
                                   const double                           threshold) {
  std::vector<std::string> neighbor_ids;
  for (const auto& lane_id : ptr_ego_lane->lane().left_neighbor_forward_lane_id()) {
...
std::shared_ptr<const LaneInfo>
PredictionMap::GetRightNeighborLane(const std::shared_ptr<const LaneInfo>& ptr_ego_lane,
                                    const Eigen::Vector2d&                 ego_position,
                                    const double                           threshold) {
  std::vector<std::string> neighbor_ids;
  for (const auto& lane_id : ptr_ego_lane->lane().right_neighbor_forward_lane_id()) {
    neighbor_ids.push_back(lane_id.id());
  }
...
std::shared_ptr<const LaneInfo>
PredictionMap::GetNeighborLane(const std::shared_ptr<const LaneInfo>& ptr_ego_lane,
                               const Eigen::Vector2d&                 ego_position,
  ...
  if (s_diff_min > threshold) { return nullptr; }
  return ptr_lane_min;
}

std::vector<std::string> PredictionMap::NearbyLaneIds(const Eigen::Vector2d& point,
                                                      const double           radius) {
...
bool PredictionMap::IsLeftNeighborLane(std::shared_ptr<const LaneInfo> target_lane,
                                       std::shared_ptr<const LaneInfo> curr_lane) {
  if (curr_lane == nullptr) { return true; }
  if (target_lane == nullptr) { return false; }
  for (const auto& left_lane_id : curr_lane->lane().left_neighbor_forward_lane_id()) {
    if (target_lane->id().id() == left_lane_id.id()) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsLeftNeighborLane(std::shared_ptr<const LaneInfo> target_lane,
  ...
  for (auto& lane : lanes) {
    if (IsLeftNeighborLane(target_lane, lane)) { return true; }
  }
  return false;
}

bool PredictionMap::IsRightNeighborLane(std::shared_ptr<const LaneInfo> target_lane,
                                        std::shared_ptr<const LaneInfo> curr_lane) {
  if (curr_lane == nullptr) { return true; }
  if (target_lane == nullptr) { return false; }
  for (auto& right_lane_id : curr_lane->lane().right_neighbor_forward_lane_id()) {
    if (target_lane->id().id() == right_lane_id.id()) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsRightNeighborLane(std::shared_ptr<const LaneInfo> target_lane,
  ...
  if (lanes.empty()) { return true; }
  for (const auto& lane : lanes) {
    if (IsRightNeighborLane(target_lane, lane)) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsSuccessorLane(std::shared_ptr<const LaneInfo> target_lane,
  ...
  for (const auto& successor_lane_id : curr_lane->lane().successor_id()) {
    if (target_lane->id().id() == successor_lane_id.id()) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsSuccessorLane(std::shared_ptr<const LaneInfo>                     target_lane,
  ...
  if (lanes.empty()) { return true; }
  for (auto& lane : lanes) {
    if (IsSuccessorLane(target_lane, lane)) { return true; }
  }
  return false;
}

bool PredictionMap::IsPredecessorLane(std::shared_ptr<const LaneInfo> target_lane,
                                      std::shared_ptr<const LaneInfo> curr_lane) {
  if (curr_lane == nullptr) { return true; }
  if (target_lane == nullptr) { return false; }
  for (const auto& predecessor_lane_id : curr_lane->lane().predecessor_id()) {
    if (target_lane->id().id() == predecessor_lane_id.id()) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsPredecessorLane(std::shared_ptr<const LaneInfo> target_lane,
  ...
  if (lanes.empty()) { return true; }
  for (auto& lane : lanes) {
    if (IsPredecessorLane(target_lane, lane)) { return true; }
  }
  return false;
}
...
bool PredictionMap::IsIdenticalLane(std::shared_ptr<const LaneInfo> other_lane,
                                    std::shared_ptr<const LaneInfo> curr_lane) {
  if (curr_lane == nullptr || other_lane == nullptr) { return true; }
...
bool PredictionMap::IsIdenticalLane(std::shared_ptr<const LaneInfo>                     other_lane,
  ...
  if (lanes.empty()) { return true; }
  for (auto& lane : lanes) {
    if (IsIdenticalLane(other_lane, lane)) { return true; }
  }
  return false;
}
...
int PredictionMap::LaneTurnType(const std::string& lane_id) {
  ...
  if (lane != nullptr) { return static_cast<int>(lane->lane().turn()); }
  return 1;
}

std::vector<std::shared_ptr<const LaneInfo>>
PredictionMap::GetNearbyLanes(const common::PointENU& position, const double nearby_radius) {
...
std::shared_ptr<const LaneInfo> PredictionMap::LaneWithSmallestAverageCurvature(
  ...
  double smallest_curvature = AverageCurvature(selected_lane_info->id().id(), sample_size);
...
double PredictionMap::AverageCurvature(const std::string& lane_id, const size_t sample_size) {
  CHECK_GT(sample_size, 0U);
  std::shared_ptr<const hdmap::LaneInfo> lane_info_ptr = PredictionMap::LaneById(lane_id);
  if (lane_info_ptr == nullptr) { return 0.0; }
