...
CruiseScenarioFeatures::CruiseScenarioFeatures() { scenario_.set_type(Scenario::CRUISE); }
...
void CruiseScenarioFeatures::BuildCruiseScenarioFeatures(
  ...
  if (reverse_lane_ids.empty()) { ADEBUG << "No reverse lane considered"; }
  for (const std::string& reverse_lane_id : reverse_lane_ids) {
    lane_ids_of_interest_.insert(reverse_lane_id);
  }
  for (const std::string& reverse_lane_id : reverse_lane_ids) {
    std::shared_ptr<const LaneInfo> reverse_lane_info = PredictionMap::LaneById(reverse_lane_id);
...
void CruiseScenarioFeatures::SearchForwardAndInsert(const std::string& start_lane_id,
                                                    const double       start_lane_s,
                                                    const double       range) {
  ...
  while (!lane_queue.empty()) {
    ...
    if (accumulated_s > range) { continue; }
    for (const auto succ_lane_id : lane.successor_id()) {
      ConstLaneInfoPtr succ_lane_info   = PredictionMap::LaneById(succ_lane_id.id());
