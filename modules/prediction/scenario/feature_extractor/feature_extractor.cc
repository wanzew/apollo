...
EnvironmentFeatures
FeatureExtractor::ExtractEnvironmentFeatures(ContainerManager* container_manager) {
  ...
  if (ego_state_container == nullptr || ego_state_container->ToPerceptionObstacle() == nullptr) {
  ...
  if (std::isnan(ptr_ego_state->position().x()) || std::isnan(ptr_ego_state->position().y()) ||
      std::isnan(ptr_ego_state->theta()) || std::isnan(ptr_ego_state->velocity().x()) ||
      std::isnan(ptr_ego_state->velocity().y())) {
    AERROR << "nan found in ego state";
    return environment_features;
  }

  Vec2d ego_position(ptr_ego_state->position().x(), ptr_ego_state->position().y());

  Vec2d ego_velocity(ptr_ego_state->velocity().x(), ptr_ego_state->velocity().y());

  environment_features.set_ego_speed(ego_velocity.Length());
  environment_features.set_ego_heading(ptr_ego_state->theta());

  auto ptr_ego_lane = GetEgoLane(ptr_ego_state->position(), ptr_ego_state->theta());

  ExtractEgoLaneFeatures(&environment_features, ptr_ego_lane, ego_position);

  ExtractNeighborLaneFeatures(&environment_features, ptr_ego_lane, ego_position);
...
void FeatureExtractor::ExtractEgoLaneFeatures(EnvironmentFeatures*       ptr_environment_features,
  ...
  if (ptr_left_neighbor_lane == nullptr && ptr_ego_lane->lane().has_left_boundary() &&
    ...
    const auto& reverse_lanes = ptr_ego_lane->lane().left_neighbor_reverse_lane_id();
...
void FeatureExtractor::ExtractNeighborLaneFeatures(EnvironmentFeatures* ptr_environment_features,
                                                   const LaneInfoPtr&   ptr_ego_lane,
  ...
  auto ptr_left_neighbor_lane = PredictionMap::GetLeftNeighborLane(
      ptr_ego_lane, {ego_position.x(), ego_position.y()}, FLAGS_lane_distance_threshold);
  ...
  if (ptr_left_neighbor_lane != nullptr) {
    ...
    ptr_environment_features->SetLeftNeighborLane(ptr_left_neighbor_lane->id().id(),
  ...
  if (ptr_right_neighbor_lane != nullptr) {
    ...
    ptr_environment_features->SetRightNeighborLane(ptr_right_neighbor_lane->id().id(),
                                                   right_neighbor_lane_s);
  }
}

void FeatureExtractor::ExtractFrontJunctionFeatures(EnvironmentFeatures* ptr_environment_features,
  ...
  for (const auto& overlap_id : junction->junction().overlap_id()) {
    if (PredictionMap::OverlapById(overlap_id.id()) != nullptr) {
      for (const auto& object : PredictionMap::OverlapById(overlap_id.id())->overlap().object()) {
        if (object.has_signal_overlap_info() || object.has_stop_sign_overlap_info()) {
          need_consider = true;
        }
      }
    }
...
LaneInfoPtr FeatureExtractor::GetEgoLane(const common::Point3D& position, const double heading) {
  common::PointENU position_enu;
  position_enu.set_x(position.x());
  position_enu.set_y(position.y());
  position_enu.set_z(position.z());

  return PredictionMap::GetMostLikelyCurrentLane(position_enu, FLAGS_lane_distance_threshold,
                                                 heading, FLAGS_lane_angle_difference_threshold);
}
