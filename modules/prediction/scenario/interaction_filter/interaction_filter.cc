...
int NearestFrontObstacleIdOnLaneSequence(const LaneSequence& lane_sequence) {
  ...
  for (const auto& nearby_obs : lane_sequence.nearby_obstacle()) {
    if (nearby_obs.s() < 0.0 || nearby_obs.s() > FLAGS_interaction_search_distance_ahead) {
      continue;
    }
...
int NearestBackwardObstacleIdOnLaneSequence(const LaneSequence& lane_sequence) {
  ...
  double smallest_relative_s          = std::numeric_limits<double>::max();
  for (const auto& nearby_obs : lane_sequence.nearby_obstacle()) {
    if (nearby_obs.s() > 0.0 || nearby_obs.s() < -FLAGS_interaction_search_distance_backward) {
      continue;
    }
    if (-nearby_obs.s() < smallest_relative_s) {
      smallest_relative_s          = -nearby_obs.s();
...
InteractionFilter::InteractionFilter(const std::shared_ptr<ContainerManager>& container_manager)
...
void InteractionFilter::AssignInteractiveTagInJunction(const Obstacle&     ego_vehicle,
  ...
  for (const int obstacle_id : obstacle_ids) {
    ...
    if (obstacle_ptr->IsInJunction(junction_id)) {
      SetInteractiveIfCloseToEgo(ego_vehicle, FLAGS_interaction_distance_threshold, obstacle_ptr);
    }
  }
}
    ...
    if (lane_sequence.vehicle_on_lane()) {
      ...
      if (nearest_front_obstacle_id >= 0) {
        ...
        if (front_obstacle_ptr != nullptr) {
          SetInteractiveIfCloseToEgo(ego_vehicle, FLAGS_interaction_distance_threshold,
                                     front_obstacle_ptr);
        }
      }
...
void InteractionFilter::AssignInteractiveTagByEgoReferenceLine(
  ...
  for (const std::string& lane_id : lane_ids) {
    ...
    if (lane_id != ego_lane_id_) {
      AssignInteractiveByMerge(ego_vehicle, lane_info_ptr, &visited_lanes, obstacles_container);
    }
    AssignInteractiveByOverlap(ego_vehicle, lane_info_ptr, &visited_lanes, obstacles_container);
    if (accumulated_s > FLAGS_interaction_search_distance_ahead + ego_vehicle_s) { break; }
  }
}

void InteractionFilter::RankingInteractiveTagObstacles(const Obstacle&     ego_vehicle,
                                                       ObstaclesContainer* obstacles_container) {
  ...
  for (const int obstacle_id : obstacle_ids) {
    ...
    if (!obstacle_ptr->IsInteractiveObstacle()) { continue; }
  ...
  while (static_cast<int>(interactive_obstacle_queue.size()) > FLAGS_interactive_obs_max_nums) {
...
void InteractionFilter::AssignInteractiveByMerge(
    ...
    ObstaclesContainer*                    obstacles_container) {
  SetInteractiveBackward(FLAGS_interaction_search_distance_backward_for_merge, ego_vehicle,
...
void InteractionFilter::AssignInteractiveByOverlap(
  ...
  for (const auto overlap_ptr : cross_lanes) {
    ...
    for (const auto& object : overlap_ptr->overlap().object()) {
      ...
      SetInteractiveBackward(ahead_s + FLAGS_interaction_search_distance_backward_for_overlap,
                             ego_vehicle, overlap_lane_ptr, visited_lanes, obstacles_container);
    }
  }
}

void InteractionFilter::SetInteractiveBackward(const double                    max_distance,
                                               const Obstacle&                 ego_vehicle,
...
void InteractionFilter::SetInteractiveIfCloseToEgo(const Obstacle& ego_vehicle,
