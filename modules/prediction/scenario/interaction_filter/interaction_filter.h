...
class InteractionFilter {
 public:
  ...
  explicit InteractionFilter(const std::shared_ptr<ContainerManager>& container_manager);

  /**
   * @brief Assign interactive tag for vehicle type obstacles which are close to
   * ADC
   */
 ...
 private:
   ...
   */
  void AssignInteractiveTagCruiseKeepLane(const Obstacle&     ego_vehicle,
                                          ObstaclesContainer* obstacles_container);
   ...
   */
  void AssignInteractiveTagCruiseChangeLane(const Obstacle&     ego_vehicle,
                                            ObstaclesContainer* obstacles_container);
   ...
   */
  void AssignInteractiveTagByEgoReferenceLine(const Obstacle&     ego_vehicle,
                                              ObstaclesContainer* obstacles_container);

  void RankingInteractiveTagObstacles(const Obstacle&     ego_vehicle,
                                      ObstaclesContainer* obstacles_container);

  void AssignInteractiveByMerge(const Obstacle&                        ego_vehicle,
                                std::shared_ptr<const hdmap::LaneInfo> lane_info_ptr,
                                std::unordered_set<std::string>* const visited_lanes,
                                ObstaclesContainer*                    obstacles_container);

  void AssignInteractiveByOverlap(const Obstacle&                        ego_vehicle,
                                  std::shared_ptr<const hdmap::LaneInfo> lane_info_ptr,
                                  std::unordered_set<std::string>* const visited_lanes,
                                  ObstaclesContainer*                    obstacles_container);

  void SetInteractiveBackward(const double                           distance,
