...
bool IsLaneSequenceInReferenceLine(const LaneSequence&           lane_sequence,
  ...
  for (const auto& lane_segment : lane_sequence.lane_segment()) {
    ...
    if (ego_trajectory_container->IsLaneIdInTargetReferenceLine(lane_id)) { return true; }
  }
  return false;
}
...
int NearestFrontObstacleIdOnLaneSequence(const LaneSequence& lane_sequence) {
  ...
  for (const auto& nearby_obs : lane_sequence.nearby_obstacle()) {
    if (nearby_obs.s() < 0.0 || nearby_obs.s() > FLAGS_caution_search_distance_ahead) { continue; }
...
int NearestBackwardObstacleIdOnLaneSequence(const LaneSequence& lane_sequence) {
  ...
  double smallest_relative_s          = std::numeric_limits<double>::max();
  for (const auto& nearby_obs : lane_sequence.nearby_obstacle()) {
    if (nearby_obs.s() > 0.0 || nearby_obs.s() < -FLAGS_caution_search_distance_backward) {
      continue;
    }
    if (-nearby_obs.s() < smallest_relative_s) {
      smallest_relative_s          = -nearby_obs.s();
...
void ObstaclesPrioritizer::AssignIgnoreLevel() {
  auto obstacles_container =
      container_manager_->GetContainer<ObstaclesContainer>(AdapterConfig::PERCEPTION_OBSTACLES);
  if (obstacles_container == nullptr) {
    AERROR << "Obstacles container pointer is a null pointer.";
    return;
  }

  Obstacle* ego_obstacle_ptr = obstacles_container->GetObstacle(FLAGS_ego_vehicle_id);
  ...
  const auto& obstacle_ids = obstacles_container->curr_frame_movable_obstacle_ids();
  for (const int obstacle_id : obstacle_ids) {
    ...
    double pedestrian_like_nearby_lane_radius = FLAGS_pedestrian_nearby_lane_search_radius;
    bool   is_near_lane =
        PredictionMap::HasNearbyLane(obstacle_x, obstacle_y, pedestrian_like_nearby_lane_radius);
    ...
    bool need_consider =
        is_in_scan_area || is_on_lane || is_near_junction || is_pedestrian_like_in_front_near_lanes;

    if (!need_consider) {
      latest_feature_ptr->mutable_priority()->set_priority(ObstaclePriority::IGNORE);
    } else {
      latest_feature_ptr->mutable_priority()->set_priority(ObstaclePriority::NORMAL);
    }
  }
...
void ObstaclesPrioritizer::AssignCautionLevel() {
  ...
  Obstacle* ego_vehicle = obstacles_container->GetObstacle(FLAGS_ego_vehicle_id);
  ...
  auto storytelling_container =
      container_manager_->GetContainer<StoryTellingContainer>(AdapterConfig::STORYTELLING);
  if (storytelling_container->ADCDistanceToJunction() < FLAGS_junction_distance_threshold) {
  ...
  AssignCautionLevelPedestrianByEgoReferenceLine(*ego_vehicle, obstacles_container);
...
void ObstaclesPrioritizer::AssignCautionLevelInJunction(const Obstacle&     ego_vehicle,
  ...
  for (const int obstacle_id : obstacle_ids) {
    ...
    if (obstacle_ptr->IsInJunction(junction_id)) {
      SetCautionIfCloseToEgo(ego_vehicle, FLAGS_caution_distance_threshold, obstacle_ptr);
    }
  }
}
...
void ObstaclesPrioritizer::AssignCautionLevelCruiseKeepLane(
  ...
  for (const LaneSequence& lane_sequence : ego_latest_feature.lane().lane_graph().lane_sequence()) {
    int nearest_front_obstacle_id = NearestFrontObstacleIdOnLaneSequence(lane_sequence);
    if (nearest_front_obstacle_id < 0) { continue; }
    Obstacle* obstacle_ptr = obstacles_container->GetObstacle(nearest_front_obstacle_id);
    if (obstacle_ptr == nullptr) {
...
void ObstaclesPrioritizer::AssignCautionLevelCruiseChangeLane(
  ...
  ADCTrajectoryContainer* ego_trajectory_container =
      container_manager_->GetContainer<ADCTrajectoryContainer>(AdapterConfig::PLANNING_TRAJECTORY);
  const Feature& ego_latest_feature = ego_vehicle.latest_feature();
  for (const LaneSequence& lane_sequence : ego_latest_feature.lane().lane_graph().lane_sequence()) {
    if (lane_sequence.vehicle_on_lane()) {
      ...
      SetCautionIfCloseToEgo(ego_vehicle, FLAGS_caution_distance_threshold, obstacle_ptr);
    } else if (IsLaneSequenceInReferenceLine(lane_sequence, ego_trajectory_container)) {
      int nearest_front_obstacle_id    = NearestFrontObstacleIdOnLaneSequence(lane_sequence);
      int nearest_backward_obstacle_id = NearestBackwardObstacleIdOnLaneSequence(lane_sequence);
      if (nearest_front_obstacle_id >= 0) {
        Obstacle* front_obstacle_ptr = obstacles_container->GetObstacle(nearest_front_obstacle_id);
        if (front_obstacle_ptr != nullptr) {
          SetCautionIfCloseToEgo(ego_vehicle, FLAGS_caution_distance_threshold, front_obstacle_ptr);
        }
      }
...
void ObstaclesPrioritizer::AssignCautionLevelByEgoReferenceLine(
  ...
  const std::vector<std::string>& lane_ids = adc_trajectory_container->GetADCTargetLaneIDSequence();
  ...
  accumulated_s = 0.0;
  for (const std::string& lane_id : lane_ids) {
    if (lane_id == ego_lane_id_) { break; }
    ego_back_lane_id_set_.insert(lane_id);
  }

  std::unordered_set<std::string> visited_lanes(lane_ids.begin(), lane_ids.end());
  ...
  for (const std::string& lane_id : lane_ids) {
    ...
    if (lane_id != ego_lane_id_) {
      AssignCautionByMerge(ego_vehicle, lane_info_ptr, &visited_lanes, obstacles_container);
    }
    AssignCautionByOverlap(ego_vehicle, lane_info_ptr, &visited_lanes, obstacles_container);
    if (accumulated_s > FLAGS_caution_search_distance_ahead + ego_vehicle_s) { break; }
  }
}
...
void ObstaclesPrioritizer::AssignCautionLevelPedestrianByEgoReferenceLine(
  ...
  for (const int obstacle_id : obstacles_container->curr_frame_movable_obstacle_ids()) {
    Obstacle* obstacle_ptr = obstacles_container->GetObstacle(obstacle_id);
    if (obstacle_ptr == nullptr) {
      AERROR << "Null obstacle pointer found.";
      continue;
    ...
    Feature* latest_feature_ptr = obstacle_ptr->mutable_latest_feature();
    if (latest_feature_ptr->type() != PerceptionObstacle::PEDESTRIAN) { continue; }
    double start_x = latest_feature_ptr->position().x();
    double start_y = latest_feature_ptr->position().y();
    double end_x =
        start_x + FLAGS_caution_pedestrian_approach_time * latest_feature_ptr->raw_velocity().x();
    double end_y =
        start_y + FLAGS_caution_pedestrian_approach_time * latest_feature_ptr->raw_velocity().y();
    ...
    if (nearby_lane_ids.empty()) { continue; }
    for (const std::string& lane_id : nearby_lane_ids) {
      if (!adc_trajectory_container->IsLaneIdInTargetReferenceLine(lane_id)) { continue; }
      std::shared_ptr<const LaneInfo> lane_info_ptr = PredictionMap::LaneById(lane_id);
      if (lane_info_ptr == nullptr) {
        AERROR << "Null lane info pointer found.";
        continue;
      ...
      double end_s   = 0.0;
      double end_l   = 0.0;
      if (PredictionMap::GetProjection({start_x, start_y}, lane_info_ptr, &start_s, &start_l) &&
          PredictionMap::GetProjection({end_x, end_y}, lane_info_ptr, &end_s, &end_l)) {
...
void ObstaclesPrioritizer::AssignCautionLevelPedestrianInFront(
  ...
  for (const int obstacle_id : obstacle_ids) {
    Obstacle* obstacle_ptr = obstacles_container->GetObstacle(obstacle_id);
    if (!obstacle_ptr->IsPedestrian() || obstacle_ptr->history_size() == 0) { continue; }
    ...
    double         inner_prod  = std::cos(ego_heading) * diff_x + std::sin(ego_heading) * diff_y;
    if (inner_prod < 0.0) { continue; }
    SetCautionIfCloseToEgo(ego_vehicle, FLAGS_caution_distance_threshold, obstacle_ptr);
  }
}

void ObstaclesPrioritizer::RankingCautionLevelObstacles(const Obstacle&     ego_vehicle,
                                                        ObstaclesContainer* obstacles_container) {
  const Point3D& ego_position = ego_vehicle.latest_feature().position();
  const auto&    obstacle_ids = obstacles_container->curr_frame_movable_obstacle_ids();
  ...
  for (const int obstacle_id : obstacle_ids) {
    ...
    if (!obstacle_ptr->IsCaution()) { continue; }
  ...
  while (static_cast<int>(caution_obstacle_queue.size()) > FLAGS_caution_obs_max_nums) {
...
void ObstaclesPrioritizer::AssignCautionByMerge(
    ...
    ObstaclesContainer*                    obstacles_container) {
  SetCautionBackward(FLAGS_caution_search_distance_backward_for_merge, ego_vehicle, lane_info_ptr,
                     visited_lanes, obstacles_container);
}
...
void ObstaclesPrioritizer::AssignCautionByOverlap(
  ...
  const std::vector<std::shared_ptr<const OverlapInfo>> cross_lanes = lane_info_ptr->cross_lanes();
  for (const auto overlap_ptr : cross_lanes) {
    ...
    if (!consider_overlap) { continue; }

    for (const auto& object : overlap_ptr->overlap().object()) {
      const auto& object_id = object.id().id();
      if (object_id == lane_info_ptr->id().id()) { continue; }
      std::shared_ptr<const LaneInfo> overlap_lane_ptr = PredictionMap::LaneById(object_id);
      // ahead_s is the length in front of the overlap
      double ahead_s = overlap_lane_ptr->total_length() - object.lane_overlap_info().start_s();
      SetCautionBackward(ahead_s + FLAGS_caution_search_distance_backward_for_overlap, ego_vehicle,
                         overlap_lane_ptr, visited_lanes, obstacles_container);
    }
  }
}

void ObstaclesPrioritizer::SetCautionBackward(const double                    max_distance,
                                              const Obstacle&                 ego_vehicle,
                                              std::shared_ptr<const LaneInfo> start_lane_info_ptr,
                                              std::unordered_set<std::string>* const visited_lanes,
                                              ObstaclesContainer* obstacles_container) {
  std::string start_lane_id = start_lane_info_ptr->id().id();
  if (ego_back_lane_id_set_.find(start_lane_id) != ego_back_lane_id_set_.end()) { return; }
  ...
  lane_info_queue.emplace(start_lane_info_ptr, start_lane_info_ptr->total_length());
  while (!lane_info_queue.empty()) {
    ...
    if (visited_lanes->find(lane_id) == visited_lanes->end() &&
        lane_obstacles.find(lane_id) != lane_obstacles.end() && !lane_obstacles[lane_id].empty()) {
    ...
    if (cumu_distance > max_distance) { continue; }
    for (const auto& pre_lane_id : curr_lane->lane().predecessor_id()) {
      if (ego_back_lane_id_set_.find(pre_lane_id.id()) != ego_back_lane_id_set_.end()) { continue; }
      ConstLaneInfoPtr pre_lane_ptr = PredictionMap::LaneById(pre_lane_id.id());
      lane_info_queue.emplace(pre_lane_ptr, cumu_distance + pre_lane_ptr->total_length());
    }
  }
}
...
void ObstaclesPrioritizer::SetCautionIfCloseToEgo(const Obstacle& ego_vehicle,
  ...
  if (distance < distance_threshold) { obstacle_ptr->SetCaution(); }
}
