...
Trajectory Predictor::GenerateTrajectory(const std::vector<TrajectoryPoint>& points) {
...
void Predictor::SetEqualProbability(const double total_probability,
  ...
  for (int i = start_index; i < num; ++i) {
    obstacle_ptr->mutable_latest_feature()->mutable_predicted_trajectory(i)->set_probability(prob);
  }
}

void Predictor::Clear() {}

void Predictor::TrimTrajectories(const ADCTrajectoryContainer& adc_trajectory_container,
                                 Obstacle*                     obstacle) {
...
bool Predictor::TrimTrajectory(const ADCTrajectoryContainer& adc_trajectory_container,
                               Obstacle*                     obstacle,
                               Trajectory*                   trajectory) {
  ...
  if (num_of_point == 0) { return false; }
  const Feature& feature         = obstacle->latest_feature();
  double         vehicle_length  = feature.length();
  double         vehicle_heading = feature.velocity_heading();
  double forward_length = std::fmax(vehicle_length / 2.0 - FLAGS_distance_beyond_junction, 0.0);

  double front_x =
      trajectory->trajectory_point(0).path_point().x() + forward_length * std::cos(vehicle_heading);
  double front_y =
      trajectory->trajectory_point(0).path_point().y() + forward_length * std::sin(vehicle_heading);
  PathPoint front_point;
  front_point.set_x(front_x);
  front_point.set_y(front_y);
  bool front_in_junction = adc_trajectory_container.IsPointInJunction(front_point);

  const PathPoint& start_point       = trajectory->trajectory_point(0).path_point();
  bool             start_in_junction = adc_trajectory_container.IsPointInJunction(start_point);

  if (front_in_junction || start_in_junction) { return false; }

  int index = 0;
  while (index < num_of_point) {
    const PathPoint& point = trajectory->trajectory_point(index).path_point();
    if (adc_trajectory_container.IsPointInJunction(point)) { break; }
    ++index;
  }

  // if no intersect
  if (index == num_of_point) { return false; }
...
bool Predictor::SupposedToStop(const Feature& feature,
                               ...
                               double*        acceleration) {
  if (stop_distance < std::max(feature.length() * 0.5, 1.0)) { return false; }
  if (stop_distance > FLAGS_distance_to_slow_down_at_stop_sign) { return false; }
  double speed  = feature.speed();
  *acceleration = -speed * speed / (2.0 * stop_distance);
  return *acceleration <= -FLAGS_double_precision && *acceleration >= FLAGS_vehicle_min_linear_acc;
}

const ObstacleConf::PredictorType& Predictor::predictor_type() { return predictor_type_; }
