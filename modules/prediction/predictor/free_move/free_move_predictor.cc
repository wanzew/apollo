...
FreeMovePredictor::FreeMovePredictor() { predictor_type_ = ObstacleConf::FREE_MOVE_PREDICTOR; }

bool FreeMovePredictor::Predict(const ADCTrajectoryContainer* adc_trajectory_container,
                                Obstacle*                     obstacle,
  ...
  if (feature.predicted_trajectory().empty()) {
    ...
    DrawFreeMoveTrajectoryPoints(position, velocity, acc, theta, 0.0, prediction_total_time,
    ...
    for (int i = 0; i < feature.predicted_trajectory_size(); ++i) {
      Trajectory* trajectory = obstacle->mutable_latest_feature()->mutable_predicted_trajectory(i);
      ...
      std::vector<TrajectoryPoint> points;
      const TrajectoryPoint&       last_point = trajectory->trajectory_point(traj_size - 1);
      double                       theta      = last_point.path_point().theta();
      Eigen::Vector2d position(last_point.path_point().x(), last_point.path_point().y());
      Eigen::Vector2d velocity(last_point.v() * std::cos(theta), last_point.v() * std::sin(theta));
      Eigen::Vector2d acc(last_point.a() * std::cos(theta), last_point.a() * std::sin(theta));
      double          last_relative_time = last_point.relative_time();
      DrawFreeMoveTrajectoryPoints(position, velocity, acc, theta, last_relative_time,
...
void FreeMovePredictor::DrawFreeMoveTrajectoryPoints(const Eigen::Vector2d&        position,
  ...
  state(4, 0) =
      common::math::Clamp(acc(0), FLAGS_vehicle_min_linear_acc, FLAGS_vehicle_max_linear_acc);
  state(5, 0) =
      common::math::Clamp(acc(1), FLAGS_vehicle_min_linear_acc, FLAGS_vehicle_max_linear_acc);
