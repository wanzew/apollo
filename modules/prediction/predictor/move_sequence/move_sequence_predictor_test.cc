...
TEST_F(MoveSequencePredictorTest, Polynomial) {
  ...
  for (const auto& lane_sequence : lane_graph.lane_sequence()) {
    ...
    bool ret_lon = predictor.GetLongitudinalPolynomial(*obstacle_ptr, lane_sequence, lon_end_state,
                                                       &lon_coefficients);
    EXPECT_TRUE(ret_lon);
    std::array<double, 4> lat_coefficients;
    bool                  ret_lat =
        predictor.GetLateralPolynomial(*obstacle_ptr, lane_sequence, 3.0, &lat_coefficients);
    EXPECT_TRUE(ret_lat);
  }
}
...
TEST_F(MoveSequencePredictorTest, Utils) {
  ...
  const LaneGraph&      lane_graph = feature.lane().lane_graph();
  for (const auto& lane_sequence : lane_graph.lane_sequence()) {
    double speed = predictor.ComputeTimeToLatEndConditionByVelocity(*obstacle_ptr, lane_sequence);
    EXPECT_GT(speed, 0.0);
  }
