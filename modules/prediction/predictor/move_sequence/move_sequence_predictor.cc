...
bool MoveSequencePredictor::Predict(const ADCTrajectoryContainer* adc_trajectory_container,
                                    Obstacle*                     obstacle,
  ...
  Obstacle*         ego_vehicle_ptr = obstacles_container->GetObstacle(FLAGS_ego_vehicle_id);
  FilterLaneSequences(feature, lane_id, ego_vehicle_ptr, adc_trajectory_container,
                      &enable_lane_sequence);
  for (int i = 0; i < num_lane_sequence; ++i) {
    const LaneSequence& sequence = feature.lane().lane_graph().lane_sequence(i);
    if (sequence.lane_segment().empty() || sequence.lane_segment(0).lane_point().empty()) {
      ADEBUG << "Empty lane segments.";
      continue;
    }

    if (!enable_lane_sequence[i]) {
      ADEBUG << "Lane sequence [" << ToString(sequence) << "] with probability ["
    ...
    if (is_about_to_stop) {
      ...
      DrawMoveSequenceTrajectoryPoints(*obstacle, sequence, FLAGS_prediction_trajectory_time_length,
 ...
 */
bool MoveSequencePredictor::DrawMoveSequenceTrajectoryPoints(const Obstacle&     obstacle,
  ...
  std::pair<double, double> lon_end_vt   = ComputeLonEndState({s0, ds0, dds0}, lane_sequence);

  std::array<double, 4> lateral_coeffs;
  std::array<double, 5> longitudinal_coeffs;
  GetLateralPolynomial(obstacle, lane_sequence, time_to_lat_end_state, &lateral_coeffs);
  GetLongitudinalPolynomial(obstacle, lane_sequence, lon_end_vt, &longitudinal_coeffs);
  ...
  for (size_t i = 0; i < total_num; ++i) {
    ...
    lane_l = EvaluateCubicPolynomial(lateral_coeffs, relative_time, 0, time_to_lat_end_state, 0.0);
    double curr_s = EvaluateQuarticPolynomial(longitudinal_coeffs, relative_time, 0,
                                              lon_end_vt.second, lon_end_vt.first);
    double prev_s = (i > 0) ? EvaluateQuarticPolynomial(longitudinal_coeffs, relative_time - period,
                                                        0, lon_end_vt.second, lon_end_vt.first) :
                              0.0;
    lane_s += std::max(0.0, (curr_s - prev_s));
    if (curr_s + FLAGS_double_precision < prev_s) { lane_l = prev_lane_l; }
    ...
    prev_lane_l       = lane_l;
    double lane_speed = EvaluateQuarticPolynomial(longitudinal_coeffs, relative_time, 1,
                                                  lon_end_vt.second, lon_end_vt.first);
    double lane_acc   = EvaluateQuarticPolynomial(longitudinal_coeffs, relative_time, 2,
                                                lon_end_vt.second, lon_end_vt.first);
...
MoveSequencePredictor::ComputeLonEndState(const std::array<double, 3>& init_s,
                                          const LaneSequence&          lane_sequence) {
  ...
  for (int i = 0; i < lane_sequence.path_point_size(); ++i) {
    ...
    if (path_point.s() < init_s[0] + FLAGS_double_precision) { continue; }
  ...
  double v_end = apollo::prediction::predictor_util::AdjustSpeedByCurvature(init_s[1], max_kappa);
  ...
  if (t < FLAGS_double_precision) { return {v_init, FLAGS_prediction_trajectory_time_length}; }
...
double
MoveSequencePredictor::ComputeTimeToLatEndConditionByVelocity(const Obstacle&     obstacle,
                                                              const LaneSequence& lane_sequence) {
  // Sanity check.
  CHECK_GT(obstacle.history_size(), 0U);
  ...
  const Feature&   feature          = obstacle.latest_feature();
  const LanePoint& first_lane_point = lane_sequence.lane_segment(0).lane_point(0);
  ...
  if (std::fabs(v_l) < FLAGS_default_lateral_approach_speed || lane_l * v_l < 0.0) {
...
double MoveSequencePredictor::CostFunction(const double max_lat_acc,
  ...
  double cost_of_trajectory = max_lat_acc + FLAGS_cost_function_alpha * time_to_end_state;
  if (FLAGS_use_bell_curve_for_cost_function) {
    double bell_curve_weight =
        1.0 / std::sqrt(2.0 * M_PI * FLAGS_cost_function_sigma * FLAGS_cost_function_sigma) *
        std::exp(-(time_to_lane_edge - bell_curve_mu) * (time_to_lane_edge - bell_curve_mu) /
