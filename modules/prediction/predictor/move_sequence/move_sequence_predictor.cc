...
void MoveSequencePredictor::Predict(Obstacle* obstacle) {
  ...
  for (int i = 0; i < num_lane_sequence; ++i) {
    ...
    if (sequence.lane_segment_size() <= 0 || sequence.lane_segment(0).lane_point_size() <= 0) {
    ...
    DrawMoveSequenceTrajectoryPoints(*obstacle, sequence, FLAGS_prediction_duration,
  ...
  ADEBUG << "Obstacle [" << obstacle->id() << "] has total " << trajectories_.size()
         << " trajectories.";
}

void MoveSequencePredictor::DrawMoveSequenceTrajectoryPoints(const Obstacle&     obstacle,
  ...
  for (size_t i = 0; i < total_num; ++i) {
    ...
    double          theta = M_PI;
    lane_l =
        EvaluateQuinticPolynomial(lateral_coeffs, relative_time, 0, time_to_lat_end_state, 0.0);
...
bool MoveSequencePredictor::GetLongitudinalPolynomial(const Obstacle&            obstacle,
                                                      const LaneSequence&        lane_sequence,
                                                      std::pair<double, double>* lon_end_vt,
  ...
  std::pair<double, double> lon_end_state = ComputeLonEndState({s0, ds0, dds0}, lane_sequence);
...
bool MoveSequencePredictor::GetLateralPolynomial(const Obstacle&        obstacle,
                                                 const LaneSequence&    lane_sequence,
  ...
  const LanePoint& start_lane_point = lane_sequence.lane_segment(0).lane_point(0);
  ...
  double           cross_prod       = lane_heading_x * pos_delta_y - lane_heading_y * pos_delta_x;
...
MoveSequencePredictor::ComputeLonEndState(const std::array<double, 3>& init_s,
  ...
  if (t < FLAGS_double_precision) { return {v_end, FLAGS_prediction_duration}; }
...
void MoveSequencePredictor::GenerateCandidateTimes(std::vector<double>* candidate_times) {
