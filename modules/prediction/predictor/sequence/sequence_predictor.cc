...
bool SequencePredictor::Predict(const ADCTrajectoryContainer* adc_trajectory_container,
                                Obstacle*                     obstacle,
...
void SequencePredictor::FilterLaneSequences(const Feature&                feature,
  ...
  for (int i = 0; i < num_lane_sequence; ++i) {
    ...
    double distance =
        GetLaneChangeDistanceWithADC(sequence, ego_vehicle_ptr, adc_trajectory_container);
    ...
    if (distance > 0.0 && distance < FLAGS_lane_change_dist) {
      ...
      for (int j = 0; j < feature.polygon_point_size(); j++) {
        Eigen::Vector2d position(feature.polygon_point(j).x(), feature.polygon_point(j).y());
...
SequencePredictor::GetLaneChangeType(const std::string&  lane_id,
  ...
  if (lane_id == lane_change_id) {
    ...
    } else if (PredictionMap::IsRightNeighborLane(ptr_change_lane, ptr_current_lane)) {
...
void SequencePredictor::DrawConstantAccelerationTrajectory(const Obstacle&     obstacle,
                                                           const LaneSequence& lane_sequence,
  ...
  for (size_t i = 0; i < total_num; ++i) {
    ...
    if (speed < FLAGS_double_precision) { continue; }
...
double SequencePredictor::GetLaneSequenceCurvatureByS(const LaneSequence& lane_sequence,
                                                      const double        s) {
...
bool SequencePredictor::GetLongitudinalPolynomial(const Obstacle&                  obstacle,
                                                  const LaneSequence&              lane_sequence,
  ...
  int lane_point_start_idx = lane_sequence.lane_segment(lane_seg_start_idx).adc_lane_point_idx();
  if (lane_point_start_idx >= lane_sequence.lane_segment(lane_seg_start_idx).lane_point_size()) {
    lane_point_start_idx = lane_sequence.lane_segment(lane_seg_start_idx).lane_point_size() - 1;
  }
  double lane_heading =
      lane_sequence.lane_segment(lane_seg_start_idx).lane_point(lane_point_start_idx).heading();
...
bool SequencePredictor::GetLateralPolynomial(const Obstacle&        obstacle,
                                             const LaneSequence&    lane_sequence,
  ...
  const LanePoint& start_lane_point =
      lane_sequence.lane_segment(lane_seg_start_idx).lane_point(lane_point_start_idx);
