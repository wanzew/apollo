...
bool ExtrapolationPredictor::Predict(const ADCTrajectoryContainer* adc_trajectory_container,
                                     Obstacle*                     obstacle,
  ...
  if (feature_ptr->predicted_trajectory().empty()) {
    AERROR << "Obstacle [" << obstacle->id() << "] has no short-term trajectories.";
    return false;
  }
...
void ExtrapolationPredictor::PostProcess(Trajectory*       trajectory_ptr,
  ...
  double extraplation_speed = ComputeExtraplationSpeed(kNumTailPoint, *trajectory_ptr);
  if (lane_search_result.found) {
    ExtrapolateByLane(lane_search_result, extraplation_speed, trajectory_ptr, clusters_ptr);
  } else {
...
ExtrapolationPredictor::SearchExtrapolationLane(const Trajectory& trajectory,
  ...
  for (int i = num_trajectory_point - 1; i >= num_trajectory_point - num_tail_point; --i) {
    ...
    auto   lane_info_ptr =
        PredictionMap::GetMostLikelyCurrentLane(point_enu, radius, heading, angle_diff_threshold);
...
void ExtrapolationPredictor::ExtrapolateByLane(const LaneSearchResult& lane_search_result,
                                               const double            extraplation_speed,
  ...
  const TrajectoryPoint& last_point = trajectory_ptr->trajectory_point(num_trajectory_point - 1);

  Eigen::Vector2d position(last_point.path_point().x(), last_point.path_point().y());
  double          lane_s = 0.0;
  double          lane_l = 0.0;
  bool projected         = PredictionMap::GetProjection(position, lane_info_ptr, &lane_s, &lane_l);
  if (!projected) {
  ...
  double time_range         = FLAGS_prediction_trajectory_time_length - last_relative_time;
  double time_resolution    = FLAGS_prediction_trajectory_time_resolution;
  double length             = extraplation_speed * time_range;

  LaneGraph lane_graph = clusters_ptr->GetLaneGraph(lane_s, length, false, lane_info_ptr);
  ...
  for (int i = 1; i <= num_point_remained; ++i) {
    double          relative_time = last_relative_time + static_cast<double>(i) * time_resolution;
...
void ExtrapolationPredictor::ExtrapolateByFreeMove(const int    num_tail_point,
  ...
  while (relative_time < time_length) {
    ...
    const TrajectoryPoint& prev_point = trajectory_ptr->trajectory_point(prev_size - 1);
...
double ExtrapolationPredictor::ComputeExtraplationSpeed(const int         num_tail_point,
