...
JunctionPredictor::JunctionPredictor() { predictor_type_ = ObstacleConf::JUNCTION_PREDICTOR; }

bool JunctionPredictor::Predict(const ADCTrajectoryContainer* adc_trajectory_container,
                                Obstacle*                     obstacle,
  ...
  std::vector<JunctionExit> junction_exits = MostLikelyJunctions(latest_feature);
...
void JunctionPredictor::DrawJunctionTrajectoryPoints(
  ...
  const std::array<double, 2> start_x   = {feature.position().x(), feature.raw_velocity().x()};
  const std::array<double, 2> end_x     = {junction_exit.exit_position().x(),
                                       std::cos(junction_exit.exit_heading()) * speed};
  const std::array<double, 2> start_y   = {feature.position().y(), feature.raw_velocity().y()};
  ...
  Eigen::Vector2d pos(junction_exit.exit_position().x(), junction_exit.exit_position().y());
  double          lane_s = 0.0;
  double          lane_l = 0.0;
  double          theta  = M_PI;
  ...
  while (t <= total_time) {
    if (!PredictionMap::SmoothPointFromLane(lane_id, lane_s, 0.0, &pos, &theta)) {
    ...
    while (lane_s > PredictionMap::LaneById(lane_id)->total_length()) {
      ...
      if (PredictionMap::LaneById(lane_id)->lane().successor_id_size() < 1) { return; }
...
std::vector<JunctionExit> JunctionPredictor::MostLikelyJunctions(const Feature& feature) {
...
double JunctionPredictor::GetBestTime(const std::array<double, 2>& start_x,
  ...
  for (size_t i = 0; i < candidate_times.size(); ++i) {
    ...
    std::array<double, 4> x_coeffs           = ComputePolynomial<3>(start_x, end_x, time_to_exit);
    std::array<double, 4> y_coeffs           = ComputePolynomial<3>(start_y, end_y, time_to_exit);
...
double JunctionPredictor::CostFunction(const std::array<double, 4>& x_coeffs,
  ...
  while (t <= time_to_exit) {
    ...
    cost = std::max(cost, std::abs(x_1 * y_2 - y_1 * x_2) / std::hypot(x_1, y_1) + time_to_exit);
