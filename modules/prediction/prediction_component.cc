...
std::string PredictionComponent::Name() const { return FLAGS_prediction_module_name; }
...
bool PredictionComponent::Init() {
  ...
  container_writer_ =
      node_->CreateWriter<SubmoduleOutput>(prediction_conf.topic_conf().container_topic_name());
...
bool PredictionComponent::ContainerSubmoduleProcess(
  ...
  SubmoduleOutput submodule_output =
      obstacles_container_ptr->GetSubmoduleOutput(kHistorySize, frame_start_time);
...
bool PredictionComponent::PredictionEndToEndProc(
  ...
  MessageProcess::OnPerception(perception_msg, container_manager_, evaluator_manager_.get(),
