...
ObstaclesContainer::ObstaclesContainer(const SubmoduleOutput& submodule_output)
  ...
  std::unique_ptr<Obstacle> ptr_ego_vehicle(new Obstacle(std::move(ego_vehicle)));
  ptr_ego_vehicle->SetJunctionAnalyzer(&junction_analyzer_);
  ptr_obstacles_.Put(ego_vehicle.id(), std::move(ptr_ego_vehicle));

  curr_frame_movable_obstacle_ids_    = submodule_output.curr_frame_movable_obstacle_ids();
  curr_frame_unmovable_obstacle_ids_  = submodule_output.curr_frame_unmovable_obstacle_ids();
  curr_frame_considered_obstacle_ids_ = submodule_output.curr_frame_considered_obstacle_ids();
...
void ObstaclesContainer::Insert(const ::google::protobuf::Message& message) {
  ...
  switch (FLAGS_prediction_offline_mode) {
    ...
    case 2: {
      if (std::fabs(timestamp - timestamp_) > FLAGS_replay_timestamp_gap ||
          FeatureOutput::SizeOfDataForLearning() > FLAGS_max_num_dump_dataforlearn) {
    ...
    case 3: {
      if (std::fabs(timestamp - timestamp_) > FLAGS_replay_timestamp_gap ||
          FeatureOutput::SizeOfPredictionResult() > FLAGS_max_num_dump_feature) {
  ...
  ADEBUG << "Current timestamp is [" << std::fixed << std::setprecision(6) << timestamp_ << "]";
...
Obstacle* ObstaclesContainer::GetObstacle(const int id) {
  ...
  if (ptr_obstacle != nullptr) { return ptr_obstacle->get(); }
  return nullptr;
}
...
std::vector<int> ObstaclesContainer::curr_frame_obstacle_ids() {
  ...
  curr_frame_obs_ids.insert(curr_frame_obs_ids.end(), curr_frame_unmovable_obstacle_ids_.begin(),
...
void ObstaclesContainer::InsertPerceptionObstacle(const PerceptionObstacle& perception_obstacle,
  ...
  if (!IsMovable(perception_obstacle)) {
    ADEBUG << "Perception obstacle [" << perception_obstacle.id() << "] is unmovable.";
  ...
  if (obstacle_ptr != nullptr) {
    ADEBUG << "Current time = " << std::fixed << std::setprecision(6) << timestamp;
    obstacle_ptr->Insert(perception_obstacle, timestamp, id);
    ADEBUG << "Refresh obstacle [" << id << "]";
  } else {
    auto ptr_obstacle = Obstacle::Create(perception_obstacle, timestamp, id, clusters_.get());
...
void ObstaclesContainer::BuildJunctionFeature() {
  ...
  for (const int id : curr_frame_considered_obstacle_ids_) {
    ...
    if (obstacle_ptr->IsInJunction(junction_id)) {
      ADEBUG << "Build junction feature for obstacle [" << obstacle_ptr->id() << "] in junction ["
             << junction_id << "]";
...
bool ObstaclesContainer::IsMovable(const PerceptionObstacle& perception_obstacle) {
...
SubmoduleOutput
ObstaclesContainer::GetSubmoduleOutput(const size_t               history_size,
  ...
  if (ego_obstacle != nullptr) { container_output.InsertEgoVehicle(std::move(*ego_obstacle)); }

  container_output.set_curr_frame_movable_obstacle_ids(curr_frame_movable_obstacle_ids_);
  container_output.set_curr_frame_unmovable_obstacle_ids(curr_frame_unmovable_obstacle_ids_);
...
const Scenario& ObstaclesContainer::curr_scenario() const { return curr_scenario_; }

ObstacleClusters* ObstaclesContainer::GetClustersPtr() const { return clusters_.get(); }
JunctionAnalyzer* ObstaclesContainer::GetJunctionAnalyzer() { return &junction_analyzer_; }
