...
LaneGraph ObstacleClusters::GetLaneGraph(const double                    start_s,
                                         const double                    length,
...
LaneGraph
ObstacleClusters::GetLaneGraphWithoutMemorizing(const double                    start_s,
                                                const double                    length,
                                                bool                            is_on_lane,
                                                std::shared_ptr<const LaneInfo> lane_info_ptr) {
...
void ObstacleClusters::SortObstacles() {
  for (auto iter = lane_obstacles_.begin(); iter != lane_obstacles_.end(); ++iter) {
...
bool ObstacleClusters::ForwardNearbyObstacle(const LaneSequence&   lane_sequence,
  ...
  for (const LaneSegment& lane_segment : lane_sequence.lane_segment()) {
    ...
    for (const LaneObstacle& lane_obstacle : lane_obstacles_[lane_id]) {
      if (lane_obstacle.obstacle_id() == obstacle_id) { continue; }
...
bool ObstacleClusters::BackwardNearbyObstacle(const LaneSequence&   lane_sequence,
                                              const int             obstacle_id,
                                              const double          obstacle_s,
                                              const double          obstacle_l,
                                              NearbyObstacle* const nearby_obstacle_ptr) {
  ...
  // Search current lane
  if (lane_obstacles_.find(lane_id) != lane_obstacles_.end() && !lane_obstacles_[lane_id].empty()) {
    for (int i = static_cast<int>(lane_obstacles_[lane_id].size()) - 1; i >= 0; --i) {
  ...
  for (const auto& predecessor_lane_id : lane_info_ptr->lane().predecessor_id()) {
    ...
    double delta_s   = backward_obs.lane_s() - (obstacle_s + pred_lane_info_ptr->total_length());
      ...
      if (object.has_stop_sign_overlap_info()) {
        for (const auto& obj : overlap_info_ptr->overlap().object()) {
          ...
          if (obj.has_lane_overlap_info()) {
            if (!stop_sign.has_lane_s() || stop_sign.lane_s() > obj.lane_overlap_info().start_s()) {
