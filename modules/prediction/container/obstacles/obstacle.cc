...
Obstacle::Obstacle() {
  ...
  heading_filter_ =
      common::DigitalFilter{{1.0, 1.0 - heading_filter_param}, {heading_filter_param}};
}
...
bool Obstacle::IsNearJunction() {
  if (feature_history_.size() <= 0) { return false; }
...
void Obstacle::Insert(const PerceptionObstacle& perception_obstacle, const double timestamp) {
  if (feature_history_.size() > 0 && timestamp <= feature_history_.front().timestamp()) {
  ...
  Feature feature;
  if (SetId(perception_obstacle, &feature) == ErrorCode::PREDICTION_ERROR) { return; }
  if (SetType(perception_obstacle, &feature) == ErrorCode::PREDICTION_ERROR) { return; }
  ...
  if (!FLAGS_use_navigation_mode) {
    // Update KF
    if (!kf_motion_tracker_.IsInitialized()) { InitKFMotionTracker(feature); }
    UpdateKFMotionTracker(feature);
    if (type_ == PerceptionObstacle::PEDESTRIAN) {
      if (!kf_pedestrian_tracker_.IsInitialized()) { InitKFPedestrianTracker(feature); }
      UpdateKFPedestrianTracker(feature);
    }

    // Update obstacle status based on KF if enabled
    if (FLAGS_enable_kf_tracking) { UpdateStatus(&feature); }
  }
...
void Obstacle::UpdateStatus(Feature* feature) {
  ...
  if (FLAGS_adjust_velocity_by_position_shift) {
    UpdateVelocity(feature->theta(), &velocity_x, &velocity_y, &velocity_heading, &speed);
  }
  ...
  double acc   = acc_x * std::cos(velocity_heading) + acc_y * std::sin(velocity_heading);
  ...
  ADEBUG << "Obstacle [" << id_ << "] has tracked position [" << std::fixed << std::setprecision(6)
         << feature->t_position().x() << ", " << std::fixed << std::setprecision(6)
         << feature->t_position().y() << ", " << std::fixed << std::setprecision(6)
         << feature->t_position().z() << "]";
  ADEBUG << "Obstacle [" << id_ << "] has tracked velocity [" << std::fixed << std::setprecision(6)
         << feature->velocity().x() << ", " << std::fixed << std::setprecision(6)
         << feature->velocity().y() << ", " << std::fixed << std::setprecision(6)
         << feature->velocity().z() << "]";
  ADEBUG << "Obstacle [" << id_ << "] has tracked acceleration [" << std::fixed
         << std::setprecision(6) << feature->acceleration().x() << ", " << std::fixed
         << std::setprecision(6) << feature->acceleration().y() << ", " << std::fixed
         << std::setprecision(6) << feature->acceleration().z() << "]";
  ADEBUG << "Obstacle [" << id_ << "] has tracked velocity heading [" << std::fixed
         << std::setprecision(6) << feature->velocity_heading() << "].";

  // Update pedestrian motion belief
  if (type_ == PerceptionObstacle::PEDESTRIAN) {
    if (!kf_pedestrian_tracker_.IsInitialized()) {
      ADEBUG << "Obstacle [" << id_ << "] has not initialized pedestrian tracker.";
      return;
    }
    feature->mutable_t_position()->set_x(kf_pedestrian_tracker_.GetStateEstimate()(0, 0));
    feature->mutable_t_position()->set_y(kf_pedestrian_tracker_.GetStateEstimate()(1, 0));
    ADEBUG << "Obstacle [" << id_ << "] has tracked pedestrian position [" << std::setprecision(6)
           << feature->t_position().x() << std::fixed << ", " << std::setprecision(6)
           << feature->t_position().y() << std::fixed << ", " << std::setprecision(6)
...
ErrorCode Obstacle::SetId(const PerceptionObstacle& perception_obstacle, Feature* feature) {
...
ErrorCode Obstacle::SetType(const PerceptionObstacle& perception_obstacle, Feature* feature) {
...
void Obstacle::SetTimestamp(const PerceptionObstacle& perception_obstacle,
                            ...
                            Feature*                  feature) {
  double ts = timestamp;
  if (perception_obstacle.has_timestamp() && perception_obstacle.timestamp() > 0.0) {
...
void Obstacle::SetPosition(const PerceptionObstacle& perception_obstacle, Feature* feature) {
  ...
  if (perception_obstacle.has_position()) {
    if (perception_obstacle.position().has_x()) { x = perception_obstacle.position().x(); }
    if (perception_obstacle.position().has_y()) { y = perception_obstacle.position().y(); }
    if (perception_obstacle.position().has_z()) { z = perception_obstacle.position().z(); }
  }
  ...
  ADEBUG << "Obstacle [" << id_ << "] has position [" << std::fixed << std::setprecision(6) << x
         << ", " << std::fixed << std::setprecision(6) << y << ", " << std::fixed
...
void Obstacle::SetVelocity(const PerceptionObstacle& perception_obstacle, Feature* feature) {
  ...
  if (perception_obstacle.has_velocity()) {
    if (perception_obstacle.velocity().has_x()) { velocity_x = perception_obstacle.velocity().x(); }
    if (perception_obstacle.velocity().has_y()) { velocity_y = perception_obstacle.velocity().y(); }
    if (perception_obstacle.velocity().has_z()) { velocity_z = perception_obstacle.velocity().z(); }
  }

  double speed            = std::hypot(velocity_x, velocity_y);
  double velocity_heading = std::atan2(velocity_y, velocity_x);
  if (FLAGS_adjust_velocity_by_obstacle_heading) { velocity_heading = perception_obstacle.theta(); }
  ...
  if (!FLAGS_use_navigation_mode && FLAGS_adjust_velocity_by_position_shift && history_size() > 0) {
    ...
    double prev_obstacle_size =
        std::max(feature_history_.front().length(), feature_history_.front().width());
    double obstacle_size   = std::max(perception_obstacle.length(), perception_obstacle.width());
    double size_diff       = std::abs(obstacle_size - prev_obstacle_size);
    double shift_thred     = std::max(obstacle_size * FLAGS_valid_position_diff_rate_threshold,
    ...
    if (std::fabs(diff_x) > shift_thred && std::fabs(diff_y) > shift_thred &&
      ...
      double angle_diff = ::apollo::common::math::NormalizeAngle(shift_heading - velocity_heading);
    ...
    if (type_ == PerceptionObstacle::BICYCLE || type_ == PerceptionObstacle::PEDESTRIAN) {
...
void Obstacle::UpdateVelocity(const double theta,
  ...
  *speed            = std::hypot(*velocity_x, *velocity_y);
  double angle_diff = ::apollo::common::math::NormalizeAngle(*velocity_heading - theta);
  if (std::fabs(angle_diff) <= FLAGS_max_lane_angle_diff) {
...
void Obstacle::SetAcceleration(Feature* feature) {
  ...
  if (feature_history_.size() > 0) {
    ...
    if (curr_ts > prev_ts) {
      ...
      acc_x = common::math::Clamp(acc_x * damping_x, FLAGS_min_acc, FLAGS_max_acc);
      acc_y = common::math::Clamp(acc_y * damping_y, FLAGS_min_acc, FLAGS_max_acc);
      acc_z = common::math::Clamp(acc_z * damping_z, FLAGS_min_acc, FLAGS_max_acc);
...
void Obstacle::SetNearbyLanes(Feature* feature) {
  ...
  PredictionMap::NearbyLanesByCurrentLanes(point, theta, FLAGS_lane_search_radius, current_lanes_,
...
void Obstacle::SetLaneGraphFeature(Feature* feature) {
  ...
  double road_graph_distance =
      std::max(speed * FLAGS_prediction_duration +
                   0.5 * FLAGS_max_acc * FLAGS_prediction_duration * FLAGS_prediction_duration,
  ...
  for (auto& lane : feature->lane().current_lane_feature()) {
    ...
    const LaneGraph&                lane_graph =
        ObstacleClusters::GetLaneGraph(lane.lane_s(), road_graph_distance, lane_info);
    ...
    for (const auto& lane_seq : lane_graph.lane_sequence()) {
      ...
      feature->mutable_lane()->mutable_lane_graph()->add_lane_sequence()->CopyFrom(seq);
...
void Obstacle::SetLanePoints(Feature* feature) {
  ...
  for (int i = 0; i < lane_graph->lane_sequence_size(); ++i) {
    ...
    if (lane_sequence->lane_segment_size() <= 0) { continue; }
    int          lane_index   = 0;
...
void Obstacle::Trim() {
  if (feature_history_.size() < 2) { return; }
  int          count     = 0;
  const double latest_ts = feature_history_.front().timestamp();
  while (!feature_history_.empty() &&
         latest_ts - feature_history_.back().timestamp() >= FLAGS_max_history_time) {
    feature_history_.pop_back();
    ++count;
  }
  if (count > 0) {
    ADEBUG << "Obstacle [" << id_ << "] trimmed " << count << " historical features";
  }
}
