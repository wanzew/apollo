...
TEST_F(ObstacleClustersTest, ObstacleClusters) {
  ...
  const LaneGraph& lane_graph = ObstacleClusters::GetLaneGraph(start_s, length, lane);
  ...
  const LaneGraph& lane_graph_2 = ObstacleClusters::GetLaneGraph(start_s, length_2, lane);
