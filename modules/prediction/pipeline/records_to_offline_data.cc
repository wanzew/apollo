...
void GenerateDataForLearning() {
  ...
  if (FLAGS_prediction_offline_bags.empty()) { return; }

  PredictionConf prediction_conf;
  if (!cyber::common::GetProtoFromFile(FLAGS_prediction_conf_file, &prediction_conf)) {
    AERROR << "Unable to load adapter conf file: " << FLAGS_prediction_adapter_config_filename;
    return;
  }
  ADEBUG << "Adapter config file is loaded into: " << prediction_conf.ShortDebugString();
  ...
  if (!MessageProcess::Init(container_manager.get(), &evaluator_manager, &predictor_manager,
                            prediction_conf)) {
    return;
  }
  const std::vector<std::string> inputs = absl::StrSplit(FLAGS_prediction_offline_bags, ':');
  for (const auto& input : inputs) {
    ...
    AINFO << "For input " << input << ", found " << offline_bags.size() << "  rosbags to process";
    for (std::size_t i = 0; i < offline_bags.size(); ++i) {
      AINFO << "\tProcessing: [ " << i << " / " << offline_bags.size() << " ]: " << offline_bags[i];
      MessageProcess::ProcessOfflineData(prediction_conf, container_manager, &evaluator_manager,
