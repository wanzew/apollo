...
std::string PredictorSubmodule::Name() const { return FLAGS_evaluator_submodule_name; }
...
bool PredictorSubmodule::Init() {
  ...
  if (!MessageProcess::InitPredictors(predictor_manager_.get(), prediction_conf)) { return false; }
  predictor_writer_ =
      node_->CreateWriter<PredictionObstacles>(prediction_conf.topic_conf().prediction_topic());
  return true;
}
...
bool PredictorSubmodule::Proc(
  ...
  const apollo::common::Header&    perception_header     = perception_obstacles->header();
  const apollo::common::ErrorCode& perception_error_code = perception_obstacles->error_code();
  const apollo::cyber::Time&       frame_start_time      = submodule_output->frame_start_time();
  ...
  predictor_manager_->Run(*perception_obstacles, adc_trajectory_container.get(),
                          &obstacles_container);
  PredictionObstacles prediction_obstacles = predictor_manager_->prediction_obstacles();

  prediction_obstacles.set_end_timestamp(Clock::NowInSeconds());
  prediction_obstacles.mutable_header()->set_lidar_timestamp(perception_header.lidar_timestamp());
  prediction_obstacles.mutable_header()->set_camera_timestamp(perception_header.camera_timestamp());
  prediction_obstacles.mutable_header()->set_radar_timestamp(perception_header.radar_timestamp());
  ...
  ADEBUG << "End to end time = " << (end_time - frame_start_time).ToSecond() * 1000 << " ms";
