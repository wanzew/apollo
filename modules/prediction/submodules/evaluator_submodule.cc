...
std::string EvaluatorSubmodule::Name() const { return FLAGS_evaluator_submodule_name; }
...
bool EvaluatorSubmodule::Init() {
  ...
  if (!MessageProcess::InitEvaluators(evaluator_manager_.get(), prediction_conf)) { return false; }
  // TODO(kechxu) change topic name when finalized
  evaluator_writer_ =
      node_->CreateWriter<SubmoduleOutput>(prediction_conf.topic_conf().evaluator_topic_name());
  return true;
}
