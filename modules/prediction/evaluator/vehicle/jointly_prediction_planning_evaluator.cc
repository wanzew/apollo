...
bool JointlyPredictionPlanningEvaluator::VectornetProcessObstaclePosition(
  ...
  if (!ExtractObstaclesHistory(obstacle_ptr, obstacles_container, &target_pos_history,
                               &all_obs_length, &all_obs_pos_history, ptr_vector_mask)) {
    AERROR << "Obstacle [" << obstacle_ptr->id() << "] failed to extract obstacle history";
    return false;
  }
  ...
  for (int j = 0; j < 20; ++j) {
    ...
    if (j == 19 || (j > 0 && target_pos_history[j + 1].first == 0.0)) { break; }
    ptr_target_obs_pos_step->index_put_({19 - j, 0}, target_pos_history[j].first -
                                                         target_pos_history[j + 1].first);
    ptr_target_obs_pos_step->index_put_({19 - j, 1}, target_pos_history[j].second -
                                                         target_pos_history[j + 1].second);
  }

  int obs_num = obstacles_container->curr_frame_considered_obstacle_ids().size();
  for (int i = 0; i < obs_num; ++i) {
    ...
    for (int j = 0; j < 20; ++j) {
      ...
      ptr_all_obstacle_pos->index_put_({i, 19 - j, 0}, all_obs_pos_history[i][j].first);
      ptr_all_obstacle_pos->index_put_({i, 19 - j, 1}, all_obs_pos_history[i][j].second);
    }
...
bool JointlyPredictionPlanningEvaluator::VectornetProcessMapData(FeatureVector* map_feature,
                                                                 PidVector*     map_p_id,
                                                                 const int      obs_num,
  ...
  for (int i = 0; i < map_polyline_num && obs_num + i < 450; ++i) {
    ...
    if (one_polyline_vector_size < 50) {
      ptr_vector_mask->index_put_(
          {obs_num + i, torch::indexing::Slice(one_polyline_vector_size, torch::indexing::None)},
          1);
    }
  }
  ...
  for (int i = 0; i < map_polyline_num && i + obs_num < 450; ++i) {
    ptr_all_map_p_id->index_put_({i}, torch::from_blob(map_p_id->at(i).data(), {2}, opts));

    int one_polyline_vector_size = map_feature->at(i).size();
    for (int j = 0; j < one_polyline_vector_size && j < 50; ++j) {
      ptr_map_data->index_put_({i, j}, torch::from_blob(map_feature->at(i)[j].data(), {9}, opts));
    }
  }
...
bool JointlyPredictionPlanningEvaluator::Evaluate(
  ...
  if (!VectornetProcessObstaclePosition(obstacle_ptr, obstacles_container, &target_obstacle_pos,
  ...
  if (!vector_net_.query(center_point, heading, &map_feature, &map_p_id)) { return false; }
  ...
  int map_polyline_num = map_feature.size();
  int data_length      = ((obs_num + map_polyline_num) < 450) ? (obs_num + map_polyline_num) : 450;
  ...
  map_polyline_num_valid     = map_polyline_num_valid > 0 ? map_polyline_num_valid : 0;
  ...
  if (!VectornetProcessMapData(&map_feature, &map_p_id, obs_num, &map_data, &all_map_p_id,
                               &vector_mask)) {
  ...
  if (data_length < 450) {
    polyline_mask.index_put_({torch::indexing::Slice(data_length, torch::indexing::None)}, 1);
  }

  // Extend obs data to specific dimension
  torch::Tensor obs_pos_data =
      torch::cat({all_obstacle_pos.index({torch::indexing::Slice(),
  ...
  torch::Tensor obs_attr_other =
      torch::tensor({10.0, 4.0}).unsqueeze(0).unsqueeze(0).repeat({(obs_num - 1), 19, 1});
  torch::Tensor obs_attr = torch::cat({obs_attr_agent, obs_attr_other}, 0);
  // ADD obs id
  // add 500 to avoid same id as in map_info
  torch::Tensor obs_id =
      torch::arange(500, obs_num + 500).unsqueeze(1).repeat({1, 19}).unsqueeze(2);
  // Process obs data
  torch::Tensor obs_data_with_len  = torch::cat({obs_pos_data, obs_length}, 2);
  torch::Tensor obs_data_with_attr = torch::cat({obs_data_with_len, obs_attr}, 2);
  ...
  if (adc_traj_points_num < 1) {
    AERROR << "adc_traj points num is " << adc_traj_points_num << " adc traj points are not enough";
    return false;
  }
  ...
  double time_interval =
      obstacle_ptr->latest_feature().timestamp() - adc_traj.header().timestamp_sec();
  for (size_t i = 0; i < adc_traj_points_num - 1; ++i) {
    double delta_time = time_interval - adc_traj.trajectory_point(0).relative_time();
    adc_traj_points.emplace_back(InterpolateUsingLinearApproximation(
        adc_traj.trajectory_point(i), adc_traj.trajectory_point(i + 1), delta_time));
  }
  if (!ExtractADCTrajectory(&adc_traj_points, obstacle_ptr, &adc_traj_curr_pos)) {
  ...
  for (size_t j = 0; j < 30; ++j) {
    if (j > traj_points_num - 1) {
      adc_trajectory[0][j][0] = adc_traj_curr_pos[traj_points_num - 1].first;
      adc_trajectory[0][j][1] = adc_traj_curr_pos[traj_points_num - 1].second;
      adc_trajectory[0][j][2] = adc_traj_points[traj_points_num - 1].path_point().theta();
      adc_trajectory[0][j][3] = adc_traj_points[traj_points_num - 1].v();
      adc_trajectory[0][j][4] = adc_traj_points[traj_points_num - 1].a();
      adc_trajectory[0][j][5] = adc_traj_points[traj_points_num - 1].path_point().kappa();
    } else {
  ...
  auto                            X_value =
      c10::ivalue::Tuple::create({std::move(target_obstacle_pos.unsqueeze(0).to(device_)),
  ...
  torch_inputs.push_back(
      c10::ivalue::Tuple::create({X_value, std::move(adc_trajectory.to(device_))}));

  auto                          end_time_data_prep = std::chrono::system_clock::now();
  std::chrono::duration<double> diff_data_prep     = end_time_data_prep - start_time_data_prep;
  ADEBUG << "vectornet input tensor prepration used time: " << diff_data_prep.count() * 1000
         << " ms.";
  ...
  std::chrono::duration<double> diff_inference     = end_time_inference - start_time_inference;
  ADEBUG << "vectornet-interaction inference used time: " << diff_inference.count() * 1000
         << " ms.";
...
bool JointlyPredictionPlanningEvaluator::ExtractObstaclesHistory(
  ...
  for (std::size_t i = 0; i < obstacle_ptr->history_size() && i < 20; ++i) {
    ...
    target_pos_history->at(i) = WorldCoordToObjCoordNorth(
        std::make_pair(target_feature.position().x(), target_feature.position().y()), obs_curr_pos,
        obs_curr_heading);
  }
  all_obs_length->emplace_back(std::make_pair(obs_curr_feature.length(), obs_curr_feature.width()));
  ...
  for (int id : obstacles_container->curr_frame_considered_obstacle_ids()) {
    ...
    all_obs_length->emplace_back(
        std::make_pair(other_obs_curr_feature.length(), other_obs_curr_feature.width()));
    ...
    if (obs_his_size > 1) {
      vector_mask->index_put_(
          {cur_idx, torch::indexing::Slice(torch::indexing::None, -(obs_his_size - 1))}, 1);
    } else {
      vector_mask->index_put_({cur_idx, torch::indexing::Slice(torch::indexing::None, -1)}, 1);
    }
    ...
    for (size_t i = 0; i < obs_his_size; ++i) {
      ...
      pos_history[i] =
          WorldCoordToObjCoordNorth(std::make_pair(feature.position().x(), feature.position().y()),
...
bool JointlyPredictionPlanningEvaluator::ExtractADCTrajectory(
  ...
  for (size_t i = 0; i < 30; ++i) {
    if (i > adc_traj_points_num - 1) {
      adc_traj_curr_pos->at(i) = adc_traj_curr_pos->at(adc_traj_points_num - 1);
    } else {
      adc_traj_curr_pos->at(i) =
          WorldCoordToObjCoordNorth(std::make_pair(trajectory_points->at(i).path_point().x(),
...
void JointlyPredictionPlanningEvaluator::LoadModel() {
  if (FLAGS_use_cuda && torch::cuda::is_available()) {
    ...
    torch_vehicle_model_ = torch::jit::load(FLAGS_torch_vehicle_jointly_model_file, device_);
  } else {
    torch_vehicle_model_ = torch::jit::load(FLAGS_torch_vehicle_jointly_model_cpu_file, device_);
  }
  ...
  auto                            X_value = c10::ivalue::Tuple::create(
      {std::move(target_obstacle_pos.to(device_)), std::move(target_obstacle_pos_step.to(device_)),
