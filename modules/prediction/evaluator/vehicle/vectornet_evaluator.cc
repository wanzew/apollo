...
bool VectornetEvaluator::VectornetProcessObstaclePosition(Obstacle*           obstacle_ptr,
  ...
  if (!ExtractObstaclesHistory(obstacle_ptr, obstacles_container, &target_pos_history,
    ...
    ADEBUG << "Obstacle [" << obstacle_ptr->id() << "] failed to extract obstacle history";
    return false;
  }
  ...
  for (int i = 0; i < obs_num; ++i) {
    ...
    for (int j = 0; j < 20; ++j) {
      ...
      all_obstacle_pos.index_put_({i, 19 - j, 0}, all_obs_pos_history[i][j].first);
      all_obstacle_pos.index_put_({i, 19 - j, 1}, all_obs_pos_history[i][j].second);
    }

    ptr_all_obs_p_id->index_put_({i}, torch::from_blob(obs_p_id.data(), {2}, opts));
  ...
  torch::Tensor obs_data_with_len = torch::cat({obs_pos_data, obs_length_data}, 2);
  ...
  *ptr_obstacle_data = torch::cat({torch::zeros({obs_num, (50 - 19), 9}), obs_data_with_id}, 1);

  return true;
}

bool VectornetEvaluator::VectornetProcessMapData(FeatureVector* map_feature,
                                                 PidVector*     map_p_id,
                                                 const int      obs_num,
...
bool VectornetEvaluator::Evaluate(Obstacle* obstacle_ptr, ObstaclesContainer* obstacles_container) {
  ...
  common::PointENU center_point = common::util::PointFactory::ToPointENU(pos_x, pos_y);
  ;
  ...
  AINFO << "vectornet input tensor preparation used time: " << diff_data_prep.count() * 1000
        << " ms.";
  ...
  AINFO << "vectornet inference used time: " << diff_inference.count() * 1000 << " ms.";
  ...
  AINFO << "vectornet output process used time: " << diff_output_process.count() * 1000 << " ms.";
  return true;
}
...
void VectornetEvaluator::LoadModel() {
  if (FLAGS_use_cuda && torch::cuda::is_available()) {
    ...
    torch_vehicle_model_ = torch::jit::load(FLAGS_torch_vehicle_vectornet_file, device_);
  } else {
    torch_vehicle_model_ = torch::jit::load(FLAGS_torch_vehicle_vectornet_cpu_file, device_);
  }
