...
bool LaneScanningEvaluator::Evaluate(Obstacle*              obstacle_ptr,
  ...
  if (FLAGS_prediction_offline_mode == PredictionConstants::kDumpDataForLearning) {
    ...
    if (latest_feature_ptr->has_junction_feature()) { learning_data_tag = "vehicle_junction"; }
    FeatureOutput::InsertDataForLearning(*latest_feature_ptr, feature_values, string_feature_values,
  ...
  torch::Tensor torch_input = torch::zeros({1, static_cast<int>(feature_values.size())});
...
bool LaneScanningEvaluator::ExtractStringFeatures(
    const LaneGraph& lane_graph, std::vector<std::string>* const string_feature_values) {
...
bool LaneScanningEvaluator::ExtractFeatures(const Obstacle*      obstacle_ptr,
  ...
  if (!ExtractStaticEnvFeatures(obstacle_ptr, lane_graph_ptr, &static_feature_values,
  ...
  if (static_feature_values.size() %
      ...
      0) {
    AERROR << "Obstacle [" << id
           << "] has incorrect static env feature size: " << static_feature_values.size() << ".";
    return false;
  }
...
bool LaneScanningEvaluator::ExtractObstacleFeatures(const Obstacle*      obstacle_ptr,
                                                    std::vector<double>* feature_values) {
  // Sanity checks.
  CHECK_NOTNULL(obstacle_ptr);
  ...
  std::vector<std::vector<std::pair<double, double>>> polygon_points_history(
      ...
      std::vector<std::pair<double, double>>(max_num_poly_pt, std::make_pair(0.0, 0.0)));
  ...
  for (std::size_t i = 0;
    ...
    if (feature.has_position()) {
      pos_history[i] = std::make_pair(feature.position().x(), feature.position().y());
    } else {
      has_history[i] = 0.0;
    }
...
bool LaneScanningEvaluator::ExtractStaticEnvFeatures(
  ...
  for (int i = 0; i < lane_graph_ptr->lane_sequence_size(); ++i) {
    ...
    while (count >= SINGLE_LANE_FEATURE_SIZE * 2 &&
      ...
      double                  relative_l_new =
          2 * feature_values->operator[](s - 4) - feature_values->operator[](s - 8);
      double                                                      relative_s_new =
          2 * feature_values->operator[](s - 3) - feature_values->operator[](s - 7);
  ...
  size_t max_feature_size = LANE_POINTS_SIZE * SINGLE_LANE_FEATURE_SIZE * MAX_NUM_LANE;
...
void LaneScanningEvaluator::LoadModel() {
  ...
  torch_lane_scanning_model_ = torch::jit::load(FLAGS_torch_vehicle_lane_scanning_file, device_);
}

void LaneScanningEvaluator::ModelInference(const std::vector<torch::jit::IValue>& torch_inputs,
  ...
  for (size_t i = 0; i < SHORT_TERM_TRAJECTORY_SIZE; ++i) {
    ...
    double          dy = static_cast<double>(torch_output[0][0][i + SHORT_TERM_TRAJECTORY_SIZE]);
    Vec2d           offset(dx, dy);
