...
bool CruiseMLPEvaluator::Evaluate(Obstacle* obstacle_ptr, ObstaclesContainer* obstacles_container) {
  // Sanity checks.
  ...
  for (int i = 0; i < lane_graph_ptr->lane_sequence_size(); ++i) {
    ...
    if (FLAGS_prediction_offline_mode == PredictionConstants::kDumpDataForLearning) {
      ...
      SetInteractionFeatureValues(obstacle_ptr, obstacles_container, lane_sequence_ptr,
                                  &interaction_feature_values);
      if (interaction_feature_values.size() != INTERACTION_FEATURE_SIZE) {
        ADEBUG << "Obstacle [" << id << "] has fewer than "
               << "expected lane feature_values" << interaction_feature_values.size() << ".";
        return false;
      }
      ADEBUG << "Interaction feature size = " << interaction_feature_values.size();
      feature_values.insert(feature_values.end(), interaction_feature_values.begin(),
                            interaction_feature_values.end());
      FeatureOutput::InsertDataForLearning(*latest_feature_ptr, feature_values, "lane_scanning",
                                           lane_sequence_ptr);
    ...
    int                             input_dim =
        static_cast<int>(OBSTACLE_FEATURE_SIZE + SINGLE_LANE_FEATURE_SIZE * LANE_POINTS_SIZE);
...
void CruiseMLPEvaluator::ExtractFeatureValues(Obstacle*            obstacle_ptr,
  ...
  if (lane_feature_values.size() != SINGLE_LANE_FEATURE_SIZE * LANE_POINTS_SIZE) {
    ADEBUG << "Obstacle [" << id << "] has fewer than "
...
void CruiseMLPEvaluator::SetObstacleFeatureValues(const Obstacle*      obstacle_ptr,
                                                  std::vector<double>* feature_values) {
  ...
  for (std::size_t i = 0; i < obstacle_ptr->history_size(); ++i) {
    ...
    if (feature.timestamp() < obs_feature_history_start_time) { break; }
    if (!feature.has_lane()) { ADEBUG << "Feature has no lane. Quit."; }
    ...
    if (feature.has_lane() && feature.lane().has_lane_feature()) {
      ...
      dist_rbs.push_back(feature.lane().lane_feature().dist_to_right_boundary());
    ...
    if (i >= FLAGS_cruise_historical_frame_length) { continue; }
  ...
  double dist_lb_rate =
      (timestamps.size() > 1) ? (dist_lbs.front() - dist_lbs.back()) / time_diff : 0.0;
  double dist_rb_rate =
      (timestamps.size() > 1) ? (dist_rbs.front() - dist_rbs.back()) / time_diff : 0.0;

  double delta_t = 0.0;
  if (timestamps.size() > 1) {
    delta_t = (timestamps.front() - timestamps.back()) / static_cast<double>(timestamps.size() - 1);
  }
  ...
  if (static_cast<int>(speeds.size()) >= 3 * curr_size &&
    ...
    double speed_3rd_recent = ComputeMean(speeds, 2 * curr_size, 3 * curr_size - 1);
...
void CruiseMLPEvaluator::SetInteractionFeatureValues(Obstacle*            obstacle_ptr,
  ...
  if (!forward_obstacle.has_id()) {  // no forward obstacle
    ...
    Obstacle*      forward_obs_ptr = obstacles_container->GetObstacle(forward_obstacle.id());
  ...
  if (!backward_obstacle.has_id()) {  // no forward obstacle
    ...
    Obstacle*      backward_obs_ptr = obstacles_container->GetObstacle(backward_obstacle.id());
...
void CruiseMLPEvaluator::SetLaneFeatureValues(const Obstacle*      obstacle_ptr,
  ...
  for (int i = 0; i < lane_sequence_ptr->lane_segment_size(); ++i) {
    if (feature_values->size() >= SINGLE_LANE_FEATURE_SIZE * LANE_POINTS_SIZE) { break; }
    ...
    for (int j = 0; j < lane_segment.lane_point_size(); ++j) {
      ...
      std::pair<double, double> relative_s_l = WorldCoordToObjCoord(
          ...
          std::make_pair(feature.position().x(), feature.position().y()), heading);
  ...
  while (size >= 10 && size < SINGLE_LANE_FEATURE_SIZE * LANE_POINTS_SIZE) {
    double                  relative_l_new =
        2 * feature_values->operator[](size - 5) - feature_values->operator[](size - 10);
    double                                                         relative_s_new =
        2 * feature_values->operator[](size - 4) - feature_values->operator[](size - 9);
...
void CruiseMLPEvaluator::LoadModels() {
  ...
  torch_go_model_    = torch::jit::load(FLAGS_torch_vehicle_cruise_go_file, device_);
  torch_cutin_model_ = torch::jit::load(FLAGS_torch_vehicle_cruise_cutin_file, device_);
}

void CruiseMLPEvaluator::ModelInference(const std::vector<torch::jit::IValue>& torch_inputs,
  ...
  auto finish_time_tensor = torch_output_tuple->elements()[1].toTensor().to(torch::kCPU);
