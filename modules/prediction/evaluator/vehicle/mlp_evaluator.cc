...
void MLPEvaluator::Evaluate(Obstacle* obstacle_ptr) {
  ...
  for (int i = 0; i < lane_graph_ptr->lane_sequence_size(); ++i) {
    ...
    double centripetal_acc_probability =
        ValidationChecker::ProbabilityByCentripedalAcceleration(*lane_sequence_ptr, speed);
    probability *= centripetal_acc_probability;
    lane_sequence_ptr->set_probability(probability);
  }

  if (FLAGS_prediction_offline_mode) { FeatureOutput::Insert(*latest_feature_ptr); }
}
...
void MLPEvaluator::ExtractFeatureValues(Obstacle*            obstacle_ptr,
  ...
  if (FLAGS_prediction_offline_mode) { SaveOfflineFeatures(lane_sequence_ptr, *feature_values); }
}
...
void MLPEvaluator::SetObstacleFeatureValues(Obstacle*            obstacle_ptr,
  ...
  if (timestamps.size() > 1) {
    delta_t = (timestamps.front() - timestamps.back()) / (timestamps.size() - 1);
  }
