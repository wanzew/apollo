...
bool MLPEvaluator::Evaluate(Obstacle* obstacle_ptr, ObstaclesContainer* obstacles_container) {
  Clear();
  ...
  for (int i = 0; i < lane_graph_ptr->lane_sequence_size(); ++i) {
    ...
    if (FLAGS_prediction_offline_mode == PredictionConstants::kDumpDataForLearning &&
      ...
      FeatureOutput::InsertDataForLearning(*latest_feature_ptr, feature_values, "mlp",
                                           lane_sequence_ptr);
    ...
    double centripetal_acc_probability =
        ValidationChecker::ProbabilityByCentripetalAcceleration(*lane_sequence_ptr, speed);
...
void MLPEvaluator::SaveOfflineFeatures(LaneSequence*              sequence,
...
void MLPEvaluator::SetObstacleFeatureValues(Obstacle*            obstacle_ptr,
  ...
  double duration = obstacle_ptr->timestamp() - FLAGS_prediction_trajectory_time_length;
  int    count    = 0;
  for (std::size_t i = 0; i < obstacle_ptr->history_size(); ++i) {
    ...
    if (feature.timestamp() < duration) { break; }
  ...
  double acc = 0.0;
  if (speeds.size() >= 3 * curr_size && delta_t > std::numeric_limits<double>::epsilon()) {
    ...
    acc            = (speed_1 - 2 * speed_2 + speed_3) /
          (static_cast<float>(curr_size) * static_cast<float>(curr_size) * delta_t * delta_t);
  }
  ...
  if (hist_size >= 2 * curr_size && delta_t > std::numeric_limits<double>::epsilon()) {
    ...
    dist_lb_rate_curr   = (dist_lb_curr - dist_lb_prev) / (static_cast<float>(curr_size) * delta_t);
  }
  ...
  if (hist_size >= 2 * curr_size && delta_t > std::numeric_limits<double>::epsilon()) {
    ...
    dist_rb_rate_curr   = (dist_rb_curr - dist_rb_prev) / (static_cast<float>(curr_size) * delta_t);
  }
...
void MLPEvaluator::SetLaneFeatureValues(Obstacle*            obstacle_ptr,
  ...
  for (int i = 0; i < lane_sequence_ptr->lane_segment_size(); ++i) {
    if (feature_values->size() >= LANE_FEATURE_SIZE) { break; }
...
double MLPEvaluator::ComputeProbability(const std::vector<double>& feature_values) {
  ...
  for (int i = 0; i < model_ptr_->dim_input(); ++i) {
    ...
    layer_input.push_back(apollo::prediction::math_util::Normalize(feature_values[i], mean, std));
  }
  ...
  for (int i = 0; i < model_ptr_->num_layer(); ++i) {
    ...
    for (int col = 0; col < layer.layer_output_dim(); ++col) {
      ...
      if (layer.layer_activation_func() == Layer::RELU) {
        ...
        AERROR << "Undefined activation function [" << layer.layer_activation_func()
  ...
  if (layer_output.size() != 1) {
    AERROR << "Model output layer has incorrect # outputs: " << layer_output.size();
  } else {
