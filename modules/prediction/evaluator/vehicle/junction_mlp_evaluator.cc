...
bool JunctionMLPEvaluator::Evaluate(Obstacle*           obstacle_ptr,
  ...
  if (FLAGS_prediction_offline_mode == PredictionConstants::kDumpDataForLearning) {
    FeatureOutput::InsertDataForLearning(*latest_feature_ptr, feature_values, "junction", nullptr);
    ADEBUG << "Save extracted features for learning locally.";
    return true;  // Skip Compute probability for offline mode
  }
  std::vector<torch::jit::IValue> torch_inputs;
  int                             input_dim =
      static_cast<int>(OBSTACLE_FEATURE_SIZE + EGO_VEHICLE_FEATURE_SIZE + JUNCTION_FEATURE_SIZE);
  ...
  if (latest_feature_ptr->junction_feature().junction_exit_size() > 1) {
    ...
    for (int i = 0; i < 12; ++i) {
      probability.push_back(
          feature_values[OBSTACLE_FEATURE_SIZE + EGO_VEHICLE_FEATURE_SIZE + 8 * i]);
    }
  }
  for (double prob : probability) {
    latest_feature_ptr->mutable_junction_feature()->add_junction_mlp_probability(prob);
  }
  ...
  for (const JunctionExit& junction_exit : latest_feature_ptr->junction_feature().junction_exit()) {
  ...
  for (int i = 0; i < lane_graph_ptr->lane_sequence_size(); ++i) {
    ...
    for (const LaneSegment& lane_segment : lane_sequence_ptr->lane_segment()) {
      if (junction_exit_prob.find(lane_segment.lane_id()) != junction_exit_prob.end()) {
        lane_sequence_ptr->set_probability(junction_exit_prob[lane_segment.lane_id()]);
      }
    }
  }
  return true;
...
void JunctionMLPEvaluator::ExtractFeatureValues(Obstacle*            obstacle_ptr,
  ...
  SetEgoVehicleFeatureValues(obstacle_ptr, obstacles_container, &ego_vehicle_feature_values);
  if (ego_vehicle_feature_values.size() != EGO_VEHICLE_FEATURE_SIZE) {
    AERROR << "Obstacle [" << id << "] has fewer than "
           << "expected ego vehicle feature_values" << ego_vehicle_feature_values.size() << ".";
    return;
  }
  ...
  if (junction_feature_values.size() != JUNCTION_FEATURE_SIZE) {
    AERROR << "Obstacle [" << id << "] has fewer than "
           << "expected junction feature_values" << junction_feature_values.size() << ".";
    return;
  }

  feature_values->insert(feature_values->end(), obstacle_feature_values.begin(),
                         obstacle_feature_values.end());
  feature_values->insert(feature_values->end(), ego_vehicle_feature_values.begin(),
...
void JunctionMLPEvaluator::SetObstacleFeatureValues(Obstacle*            obstacle_ptr,
                                                    std::vector<double>* feature_values) {
  ...
  std::vector<std::pair<double, double>> pos_history(FLAGS_junction_historical_frame_length,
...
void JunctionMLPEvaluator::SetEgoVehicleFeatureValues(Obstacle*           obstacle_ptr,
                                                      ObstaclesContainer* obstacles_container,
                                                      std::vector<double>* const feature_values) {
  feature_values->clear();
  *feature_values            = std::vector<double>(4, 0.0);
  auto ego_pose_obstacle_ptr = obstacles_container->GetObstacle(FLAGS_ego_vehicle_id);
  if (ego_pose_obstacle_ptr == nullptr || ego_pose_obstacle_ptr->history_size() == 0) {
  ...
  apollo::common::math::Vec2d ego_relative_velocity(ego_velocity.x(), ego_velocity.y());
  ...
  ADEBUG << "ego relative pos = {" << ego_relative_position.x() << ", " << ego_relative_position.y()
         << "} "
         << "ego_relative_velocity = {" << ego_relative_velocity.x() << ", "
         << ego_relative_velocity.y() << "}";
}

void JunctionMLPEvaluator::SetJunctionFeatureValues(Obstacle*            obstacle_ptr,
                                                    std::vector<double>* feature_values) {
  feature_values->clear();
  ...
  double heading = std::atan2(feature_ptr->raw_velocity().y(), feature_ptr->raw_velocity().x());
  if (!feature_ptr->has_junction_feature()) {
    AERROR << "Obstacle [" << obstacle_ptr->id() << "] has no junction_feature.";
    return;
  }
  ...
  for (int i = 0; i < num_junction_exit; ++i) {
    const JunctionExit& junction_exit = feature_ptr->junction_feature().junction_exit(i);
    ...
    double diff_heading = apollo::common::math::AngleDiff(heading, junction_exit.exit_heading());
    ...
    std::array<double, 4> x_coeffs = ComputePolynomial<3>(start_x, end_x, exit_time);
    std::array<double, 4> y_coeffs = ComputePolynomial<3>(start_y, end_y, exit_time);
    double                t        = 0.0;
    double                cost     = 0.0;
    while (t <= exit_time) {
      ...
      // cost = curvature * v^2
      cost = std::max(cost, std::abs(x_1 * y_2 - y_1 * x_2) / std::hypot(x_1, y_1));
...
void JunctionMLPEvaluator::LoadModel() {
  ...
  torch_model_ = torch::jit::load(FLAGS_torch_vehicle_junction_mlp_file, device_);
}
