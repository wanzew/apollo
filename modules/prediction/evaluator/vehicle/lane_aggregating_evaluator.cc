...
void LaneAggregatingEvaluator::LoadModel() {
  ...
  torch_obstacle_encoding_ =
      torch::jit::load(FLAGS_torch_lane_aggregating_obstacle_encoding_file, device_);
  torch_lane_encoding_ = torch::jit::load(FLAGS_torch_lane_aggregating_lane_encoding_file, device_);
  torch_prediction_layer_ =
      torch::jit::load(FLAGS_torch_lane_aggregating_prediction_layer_file, device_);
}
...
bool LaneAggregatingEvaluator::Evaluate(Obstacle*           obstacle_ptr,
  ...
  if (!latest_feature_ptr->has_lane() || !latest_feature_ptr->lane().has_lane_graph_ordered()) {
    AERROR << "Obstacle [" << id << "] has no lane graph.";
    return false;
  }
  LaneGraph* lane_graph_ptr = latest_feature_ptr->mutable_lane()->mutable_lane_graph_ordered();
  CHECK_NOTNULL(lane_graph_ptr);
  if (lane_graph_ptr->lane_sequence().empty()) {
    AERROR << "Obstacle [" << id << "] has no lane sequences.";
    return false;
  }
  ADEBUG << "There are " << lane_graph_ptr->lane_sequence_size() << " lane sequences to scan.";
  ...
  if (obstacle_feature_values.size() != OBSTACLE_FEATURE_SIZE) {
    ADEBUG << "Obstacle [" << id << "] has fewer than "
           << "expected obstacle feature_values " << obstacle_feature_values.size() << ".";
    return false;
  }
  ...
  for (size_t i = 0; i < obstacle_feature_values.size(); ++i) {
    obstacle_encoding_inputs_tensor[0][i] = static_cast<float>(obstacle_feature_values[i]);
  }
  obstacle_encoding_inputs.push_back(std::move(obstacle_encoding_inputs_tensor));
  torch::Tensor obstalce_encoding =
      torch_obstacle_encoding_.forward(obstacle_encoding_inputs).toTensor().to(torch::kCPU);
  // 2. Encode the lane features.
  std::vector<std::vector<double>> lane_feature_values;
  std::vector<int>                 lane_sequence_idx_to_remove;
  if (!ExtractStaticEnvFeatures(obstacle_ptr, lane_graph_ptr, &lane_feature_values,
                                &lane_sequence_idx_to_remove)) {
    AERROR << "Failed to extract static environmental features around obs_id = " << id;
  }
  ...
  for (const auto& single_lane_feature_values : lane_feature_values) {
    if (single_lane_feature_values.size() !=
        SINGLE_LANE_FEATURE_SIZE * (LANE_POINTS_SIZE + BACKWARD_LANE_POINTS_SIZE)) {
    ...
    for (size_t i = 0; i < SINGLE_LANE_FEATURE_SIZE * LANE_POINTS_SIZE; ++i) {
      single_lane_encoding_inputs_tensor[0][i] = static_cast<float>(
          single_lane_feature_values[i + SINGLE_LANE_FEATURE_SIZE * BACKWARD_LANE_POINTS_SIZE]);
    }
    single_lane_encoding_inputs.push_back(std::move(single_lane_encoding_inputs_tensor));
    torch::Tensor single_lane_encoding =
        torch_lane_encoding_.forward(single_lane_encoding_inputs).toTensor().to(torch::kCPU);
  ...
  for (size_t i = 0; i < lane_encoding_list.size(); ++i) {
    ...
    torch::Tensor                   prediction_layer_inputs_tensor =
        torch::zeros({1, OBSTACLE_ENCODING_SIZE + SINGLE_LANE_ENCODING_SIZE + 1});
    ...
    for (size_t j = 0; j + 1 < SINGLE_LANE_ENCODING_SIZE; ++j) {
      prediction_layer_inputs_tensor[0][OBSTACLE_ENCODING_SIZE + j] = lane_encoding_list[i][0][j];
    }
    prediction_layer_inputs_tensor[0][OBSTACLE_ENCODING_SIZE + SINGLE_LANE_ENCODING_SIZE - 1] =
        static_cast<float>(
            lane_feature_values[i][SINGLE_LANE_FEATURE_SIZE * BACKWARD_LANE_POINTS_SIZE]);
    prediction_layer_inputs_tensor[0][OBSTACLE_ENCODING_SIZE + SINGLE_LANE_ENCODING_SIZE] = 0.0;
    ...
    // }
    prediction_layer_inputs.push_back(std::move(prediction_layer_inputs_tensor));
    torch::Tensor prediction_layer_output =
        torch_prediction_layer_.forward(prediction_layer_inputs).toTensor().to(torch::kCPU);
  ...
  for (int i = 0; i < lane_graph_ptr->lane_sequence_size(); ++i) {
    lane_graph_ptr->mutable_lane_sequence(i)->set_probability(output_probabilities[i]);
...
bool LaneAggregatingEvaluator::ExtractObstacleFeatures(const Obstacle*      obstacle_ptr,
  ...
  std::vector<std::pair<double, double>> pos_history(FLAGS_cruise_historical_frame_length,
                                                     std::make_pair(0.0, 0.0));
  std::vector<std::pair<double, double>> vel_history(FLAGS_cruise_historical_frame_length,
                                                     std::make_pair(0.0, 0.0));
  std::vector<std::pair<double, double>> acc_history(FLAGS_cruise_historical_frame_length,
                                                     std::make_pair(0.0, 0.0));
  std::vector<double> vel_heading_history(FLAGS_cruise_historical_frame_length, 0.0);
  std::vector<double> vel_heading_changing_rate_history(FLAGS_cruise_historical_frame_length, 0.0);
  ...
  ADEBUG << "Obstacle has " << obstacle_ptr->history_size() << " history timestamps.";
  for (std::size_t i = 0;
       i < std::min(obstacle_ptr->history_size(), FLAGS_cruise_historical_frame_length); ++i) {
    ...
    if (feature.has_velocity()) {
      auto vel_end =
          WorldCoordToObjCoord(std::make_pair(feature.velocity().x(), feature.velocity().y()),
                               obs_curr_pos, obs_curr_heading);
      auto vel_begin =
          WorldCoordToObjCoord(std::make_pair(0.0, 0.0), obs_curr_pos, obs_curr_heading);
      vel_history[i] =
          std::make_pair(vel_end.first - vel_begin.first, vel_end.second - vel_begin.second);
    } else {
      has_history[i] = 0.0;
    }
    ...
    if (feature.has_acceleration()) {
      auto acc_end = WorldCoordToObjCoord(
          std::make_pair(feature.acceleration().x(), feature.acceleration().y()), obs_curr_pos,
          obs_curr_heading);
      auto acc_begin =
          WorldCoordToObjCoord(std::make_pair(0.0, 0.0), obs_curr_pos, obs_curr_heading);
      acc_history[i] =
          std::make_pair(acc_end.first - acc_begin.first, acc_end.second - acc_begin.second);
    } else {
      has_history[i] = 0.0;
    }
    // Extract velocity heading info.
    if (feature.has_velocity_heading()) {
      vel_heading_history[i] = WorldAngleToObjAngle(feature.velocity_heading(), obs_curr_heading);
      if (i != 0) {
  ...
  for (std::size_t i = obstacle_ptr->history_size(); i < FLAGS_cruise_historical_frame_length;
       ++i) {
    has_history[i] = 0.0;
  }
...
bool LaneAggregatingEvaluator::ExtractStaticEnvFeatures(
  ...
  for (int i = 0; i < lane_graph_ptr->lane_sequence_size(); ++i) {
    ...
    for (int j = lane_sequence.adc_lane_segment_idx(); j >= 0; --j) {
      if (count >= SINGLE_LANE_FEATURE_SIZE * BACKWARD_LANE_POINTS_SIZE) { break; }
      ...
      if (j == lane_sequence.adc_lane_segment_idx()) {
        k_starting_idx =
            std::min(lane_segment.adc_lane_point_idx(), lane_segment.lane_point_size() - 1);
      }
      for (int k = k_starting_idx; k >= 0; --k) {
        ...
        std::pair<double, double> relative_s_l = WorldCoordToObjCoord(
            std::make_pair(lane_point.position().x(), lane_point.position().y()), obs_curr_pos,
            obs_curr_heading);
        double relative_ang = WorldAngleToObjAngle(lane_point.heading(), obs_curr_heading);
    ...
    while (count >= SINGLE_LANE_FEATURE_SIZE * 2 &&
      ...
      double relative_l_new   = 2 * backward_feature_values[s - 1] - backward_feature_values[s - 5];
      double relative_s_new   = 2 * backward_feature_values[s - 2] - backward_feature_values[s - 6];
    ...
    for (int j = static_cast<int>(backward_feature_values.size()) - 1; j >= 0; --j) {
      curr_feature_values.push_back(backward_feature_values[j]);
    }

    // Extract features from forward lane-points.
    count = 0;
    for (int j = lane_sequence.adc_lane_segment_idx(); j < lane_sequence.lane_segment_size(); ++j) {
      if (count >= SINGLE_LANE_FEATURE_SIZE * LANE_POINTS_SIZE) { break; }
      const LaneSegment& lane_segment   = lane_sequence.lane_segment(j);
    ...
    while (count >= SINGLE_LANE_FEATURE_SIZE * 2 &&
      ...
      double      relative_l_new   = 2 * curr_feature_values[s - 4] - curr_feature_values[s - 8];
      double      relative_s_new   = 2 * curr_feature_values[s - 3] - curr_feature_values[s - 7];
...
torch::Tensor LaneAggregatingEvaluator::AggregateLaneEncodings(
  ...
  torch::Tensor output_tensor      = torch::zeros({1, SINGLE_LANE_ENCODING_SIZE * 2});
...
torch::Tensor LaneAggregatingEvaluator::LaneEncodingMaxPooling(
  ...
  for (size_t i = 1; i < lane_encoding_list.size(); ++i) {
    for (size_t j = 0; j < SINGLE_LANE_ENCODING_SIZE; ++j) {
      output_tensor[0][j] = torch::max(output_tensor[0][j], lane_encoding_list[i][0][j]);
    }
  }
  return output_tensor;
}
...
std::vector<double>
LaneAggregatingEvaluator::StableSoftmax(const std::vector<double>& prediction_scores) {
