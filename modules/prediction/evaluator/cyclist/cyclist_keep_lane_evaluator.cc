...
bool CyclistKeepLaneEvaluator::Evaluate(Obstacle*           obstacle_ptr,
  ...
  std::string curr_lane_id = latest_feature_ptr->lane().lane_feature().lane_id();
...
double CyclistKeepLaneEvaluator::ComputeProbability(const std::string&  curr_lane_id,
  ...
  if (curr_lane_id == lane_seq_first_id) { return 1.0; }
  return 0.0;
}
