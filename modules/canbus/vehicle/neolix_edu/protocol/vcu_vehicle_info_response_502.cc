...
void Vcuvehicleinforesponse502::Parse(const std::uint8_t* bytes,
                                      int32_t             length,
                                      ChassisDetail*      chassis) const {
  chassis->mutable_neolix_edu()
      ->mutable_vcu_vehicle_info_response_502()
      ->set_vehicle_softwareversion_indicati(vehicle_softwareversion_indicati(bytes, length));
  chassis->mutable_neolix_edu()->mutable_vcu_vehicle_info_response_502()->set_project(
      project(bytes, length));
  chassis->mutable_neolix_edu()->mutable_vcu_vehicle_info_response_502()->set_manufacturer(
      manufacturer(bytes, length));
  chassis->mutable_neolix_edu()->mutable_vcu_vehicle_info_response_502()->set_year(
      year(bytes, length));
  chassis->mutable_neolix_edu()->mutable_vcu_vehicle_info_response_502()->set_month(
      month(bytes, length));
  chassis->mutable_neolix_edu()->mutable_vcu_vehicle_info_response_502()->set_day(
      day(bytes, length));
  chassis->mutable_neolix_edu()->mutable_vcu_vehicle_info_response_502()->set_vehicle_serial_number(
      vehicle_serial_number(bytes, length));
}
...
// 'physical_unit': ''}
int Vcuvehicleinforesponse502::vehicle_softwareversion_indicati(const std::uint8_t* bytes,
...
int Vcuvehicleinforesponse502::project(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 3);
...
int Vcuvehicleinforesponse502::manufacturer(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 3);
...
int Vcuvehicleinforesponse502::year(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 4);
  int32_t x = t0.get_byte(0, 8);
...
// 'order': 'motorola', 'physical_unit': ''}
int Vcuvehicleinforesponse502::month(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 5);
  int32_t x = t0.get_byte(4, 4);
...
// 'order': 'motorola', 'physical_unit': ''}
int Vcuvehicleinforesponse502::day(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 5);
  int32_t x = t0.get_byte(0, 4);
