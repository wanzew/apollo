...
class Aebrearwheelspeed354
 ...
 private:
  ...
  bool wheelspeed_rl_valid(const std::uint8_t* bytes, const int32_t length) const;
  ...
  // 'motorola', 'physical_unit': 'bit'}
  bool wheelspeed_rr_valid(const std::uint8_t* bytes, const int32_t length) const;
  ...
  double wheelspeed_rl_direct(const std::uint8_t* bytes, const int32_t length) const;

  // config detail: {'description': '0x0:Invalid;0x1:D;0x2:N;0x3:R', 'offset':
  // 0.0, 'precision': 1.0, 'len': 2, 'name': 'WheelSpeed_RR_Direct',
  // 'is_signed_var': False, 'physical_range': '[0.0|3.0]', 'bit': 55, 'type':
  // 'double', 'order': 'motorola', 'physical_unit': 'bit'}
  double wheelspeed_rr_direct(const std::uint8_t* bytes, const int32_t length) const;
  ...
  // 'physical_unit': ''}
  double alivecounter_rear(const std::uint8_t* bytes, const int32_t length) const;
