...
TEST_F(ZhongyunMessageManagerTest, GetSendProtocols) {
  ...
  EXPECT_NE(manager_.GetMutableProtocolDataById(Steeringcontrola2::ID), nullptr);
...
TEST_F(ZhongyunMessageManagerTest, GetRecvProtocols) {
  EXPECT_NE(manager_.GetMutableProtocolDataById(Enablestatefeedbackc3::ID), nullptr);
  EXPECT_NE(manager_.GetMutableProtocolDataById(Errorstatee1::ID), nullptr);
  EXPECT_NE(manager_.GetMutableProtocolDataById(Vehiclestatefeedback2c4::ID), nullptr);
  EXPECT_NE(manager_.GetMutableProtocolDataById(Vehiclestatefeedbackc1::ID), nullptr);
}
