...
void Fbs3237::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_wey()->mutable_fbs3_237()->set_engspd(engspd(bytes, length));
  chassis->mutable_wey()->mutable_fbs3_237()->set_accpedalpos(accpedalpos(bytes, length));
  chassis->mutable_wey()->mutable_fbs3_237()->set_epbswtichposition(
      epbswtichposition(bytes, length));
  chassis->mutable_wey()->mutable_fbs3_237()->set_currentgear(currentgear(bytes, length));
  chassis->mutable_wey()->mutable_fbs3_237()->set_eps_streeingmode(eps_streeingmode(bytes, length));
  ...
  chassis->mutable_wey()->mutable_fbs3_237()->set_epscurrmod(epscurrmod(bytes, length));
  // Added for response check
  chassis->mutable_check_response()->set_is_eps_online(eps_streeingmode(bytes, length) == 1);
}
...
Fbs3_237::EpbswtichpositionType Fbs3237::epbswtichposition(const std::uint8_t* bytes,
                                                           int32_t             length) const {
  Byte    t0(bytes + 3);
  int32_t x = t0.get_byte(6, 2);

  Fbs3_237::EpbswtichpositionType ret = static_cast<Fbs3_237::EpbswtichpositionType>(x);
  return ret;
}
...
Fbs3_237::CurrentgearType Fbs3237::currentgear(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 4);
  int32_t x = t0.get_byte(6, 2);
...
// 'order': 'motorola', 'physical_unit': ''}
Fbs3_237::Eps_streeingmodeType Fbs3237::eps_streeingmode(const std::uint8_t* bytes,
                                                         int32_t             length) const {
  Byte    t0(bytes + 4);
  int32_t x = t0.get_byte(0, 3);

  Fbs3_237::Eps_streeingmodeType ret = static_cast<Fbs3_237::Eps_streeingmodeType>(x);
  return ret;
}
...
double Fbs3237::epsdrvinputtrqvalue(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 5);
  int32_t x = t0.get_byte(0, 8);
...
double Fbs3237::epsconsumedcurrvalue(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 6);
  int32_t x = t0.get_byte(0, 8);
...
// 'type': 'enum', 'order': 'motorola', 'physical_unit': ''}
Fbs3_237::EpscurrmodType Fbs3237::epscurrmod(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 7);
