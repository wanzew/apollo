...
void Scu1301::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_ge3()->mutable_scu_1_301()->set_vin16(vin16(bytes, length));
  chassis->mutable_ge3()->mutable_scu_1_301()->set_scu_stopbutst(scu_stopbutst(bytes, length));
  chassis->mutable_ge3()->mutable_scu_1_301()->set_scu_drvmode(scu_drvmode(bytes, length));
  chassis->mutable_ge3()->mutable_scu_1_301()->set_scu_faultst(scu_faultst(bytes, length));
}
...
Scu_1_301::Scu_stopbutstType Scu1301::scu_stopbutst(const std::uint8_t* bytes,
  ...
  Scu_1_301::Scu_stopbutstType ret = static_cast<Scu_1_301::Scu_stopbutstType>(x);
  return ret;
}
...
Scu_1_301::Scu_drvmodeType Scu1301::scu_drvmode(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
...
// 'physical_unit': ''}
Scu_1_301::Scu_faultstType Scu1301::scu_faultst(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
