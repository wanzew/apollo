...
void Steerstatus512::Parse(const std::uint8_t* bytes,
                           int32_t             length,
  ...
  chassis->mutable_ch()->mutable_steer_status__512()->set_steer_err(steer_err(bytes, length));
  chassis->mutable_ch()->mutable_steer_status__512()->set_sensor_err(sensor_err(bytes, length));
  chassis->mutable_check_response()->set_is_eps_online(steer_angle_en_sts(bytes, length) == 1);
}
...
// 'physical_unit': ''}
Steer_status__512::Steer_angle_en_stsType
Steerstatus512::steer_angle_en_sts(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
...
double Steerstatus512::steer_angle_sts(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 2);
  int32_t x = t0.get_byte(0, 8);
...
Steer_status__512::Steer_errType Steerstatus512::steer_err(const std::uint8_t* bytes,
                                                           int32_t             length) const {
  Byte    t0(bytes + 3);
  int32_t x = t0.get_byte(0, 8);

  Steer_status__512::Steer_errType ret = static_cast<Steer_status__512::Steer_errType>(x);
  return ret;
}
...
// 'order': 'intel', 'physical_unit': ''}
Steer_status__512::Sensor_errType Steerstatus512::sensor_err(const std::uint8_t* bytes,
                                                             int32_t             length) const {
  Byte    t0(bytes + 4);
  int32_t x = t0.get_byte(0, 8);

  Steer_status__512::Sensor_errType ret = static_cast<Steer_status__512::Sensor_errType>(x);
  return ret;
}
}  // namespace ch
}  // namespace canbus
