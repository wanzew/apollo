...
void Llcmotionfeedback221::Parse(const std::uint8_t* bytes,
                                 int32_t             length,
                                 ChassisDetail*      chassis) const {
  chassis->mutable_transit()->mutable_llc_motionfeedback2_21()->set_llc_fbk_vehiclespeed(
      llc_fbk_vehiclespeed(bytes, length));
  chassis->mutable_transit()->mutable_llc_motionfeedback2_21()->set_llc_motionfeedback2_counter(
      llc_motionfeedback2_counter(bytes, length));
  chassis->mutable_transit()->mutable_llc_motionfeedback2_21()->set_llc_motionfeedback2_checksum(
      llc_motionfeedback2_checksum(bytes, length));
  chassis->mutable_transit()->mutable_llc_motionfeedback2_21()->set_llc_fbk_steeringrate(
      llc_fbk_steeringrate(bytes, length));
  chassis->mutable_transit()->mutable_llc_motionfeedback2_21()->set_llc_fbk_steeringangle(
...
double Llcmotionfeedback221::llc_fbk_vehiclespeed(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 5);
...
int Llcmotionfeedback221::llc_motionfeedback2_checksum(const std::uint8_t* bytes,
                                                       int32_t             length) const {
  Byte    t0(bytes + 7);
  int32_t x = t0.get_byte(0, 8);
