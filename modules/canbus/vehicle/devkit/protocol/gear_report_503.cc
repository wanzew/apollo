...
void Gearreport503::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_devkit()->mutable_gear_report_503()->set_gear_flt(gear_flt(bytes, length));
  chassis->mutable_devkit()->mutable_gear_report_503()->set_gear_actual(gear_actual(bytes, length));
}
...
Gear_report_503::Gear_fltType Gearreport503::gear_flt(const std::uint8_t* bytes,
  ...
  Gear_report_503::Gear_fltType ret = static_cast<Gear_report_503::Gear_fltType>(x);
  return ret;
}
...
// 'motorola', 'physical_unit': ''}
Gear_report_503::Gear_actualType Gearreport503::gear_actual(const std::uint8_t* bytes,
                                                            int32_t             length) const {
  Byte    t0(bytes + 0);
  int32_t x = t0.get_byte(0, 3);

  Gear_report_503::Gear_actualType ret = static_cast<Gear_report_503::Gear_actualType>(x);
  return ret;
}
}  // namespace devkit
}  // namespace canbus
