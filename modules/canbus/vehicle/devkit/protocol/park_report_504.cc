...
void Parkreport504::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_devkit()->mutable_park_report_504()->set_parking_actual(
      parking_actual(bytes, length));
  chassis->mutable_devkit()->mutable_park_report_504()->set_park_flt(park_flt(bytes, length));
}
...
// 'physical_unit': ''}
Park_report_504::Parking_actualType Parkreport504::parking_actual(const std::uint8_t* bytes,
                                                                  int32_t length) const {
  Byte    t0(bytes + 0);
  int32_t x = t0.get_byte(0, 1);

  Park_report_504::Parking_actualType ret = static_cast<Park_report_504::Parking_actualType>(x);
  return ret;
}
...
Park_report_504::Park_fltType Parkreport504::park_flt(const std::uint8_t* bytes,
  ...
  Park_report_504::Park_fltType ret = static_cast<Park_report_504::Park_fltType>(x);
  return ret;
}
}  // namespace devkit
}  // namespace canbus
