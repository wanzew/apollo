...
void Bmsreport512::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  ...
  chassis->mutable_devkit()->mutable_bms_report_512()->set_battery_soc(battery_soc(bytes, length));
}
...
double Bmsreport512::battery_current(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 2);
...
double Bmsreport512::battery_voltage(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
  int32_t x = t0.get_byte(0, 8);
