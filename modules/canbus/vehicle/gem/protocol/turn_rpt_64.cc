...
void Turnrpt64::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_gem()->mutable_turn_rpt_64()->set_manual_input(manual_input(bytes, length));
  chassis->mutable_gem()->mutable_turn_rpt_64()->set_commanded_value(
      commanded_value(bytes, length));
  chassis->mutable_gem()->mutable_turn_rpt_64()->set_output_value(output_value(bytes, length));
}
...
Turn_rpt_64::Manual_inputType Turnrpt64::manual_input(const std::uint8_t* bytes,
  ...
  Turn_rpt_64::Manual_inputType ret = static_cast<Turn_rpt_64::Manual_inputType>(x);
  return ret;
}
...
Turn_rpt_64::Commanded_valueType Turnrpt64::commanded_value(const std::uint8_t* bytes,
                                                            int32_t             length) const {
  Byte    t0(bytes + 1);
  int32_t x = t0.get_byte(0, 8);

  Turn_rpt_64::Commanded_valueType ret = static_cast<Turn_rpt_64::Commanded_valueType>(x);
  return ret;
}
...
Turn_rpt_64::Output_valueType Turnrpt64::output_value(const std::uint8_t* bytes,
  ...
  Turn_rpt_64::Output_valueType ret = static_cast<Turn_rpt_64::Output_valueType>(x);
  return ret;
}
}  // namespace gem
}  // namespace canbus
