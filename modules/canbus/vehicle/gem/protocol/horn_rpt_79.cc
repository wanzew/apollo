...
void Hornrpt79::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_gem()->mutable_horn_rpt_79()->set_output_value(output_value(bytes, length));
  chassis->mutable_gem()->mutable_horn_rpt_79()->set_commanded_value(
      commanded_value(bytes, length));
  chassis->mutable_gem()->mutable_horn_rpt_79()->set_manual_input(manual_input(bytes, length));
}
...
Horn_rpt_79::Output_valueType Hornrpt79::output_value(const std::uint8_t* bytes,
  ...
  Horn_rpt_79::Output_valueType ret = static_cast<Horn_rpt_79::Output_valueType>(x);
  return ret;
}
...
// 'motorola', 'physical_unit': ''}
Horn_rpt_79::Commanded_valueType Hornrpt79::commanded_value(const std::uint8_t* bytes,
                                                            int32_t             length) const {
  Byte    t0(bytes + 1);
  int32_t x = t0.get_byte(0, 8);

  Horn_rpt_79::Commanded_valueType ret = static_cast<Horn_rpt_79::Commanded_valueType>(x);
  return ret;
}
...
Horn_rpt_79::Manual_inputType Hornrpt79::manual_input(const std::uint8_t* bytes,
  ...
  Horn_rpt_79::Manual_inputType ret = static_cast<Horn_rpt_79::Manual_inputType>(x);
  return ret;
}
}  // namespace gem
}  // namespace canbus
