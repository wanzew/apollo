...
void Datetimerpt83::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_gem()->mutable_date_time_rpt_83()->set_time_second(time_second(bytes, length));
  chassis->mutable_gem()->mutable_date_time_rpt_83()->set_time_minute(time_minute(bytes, length));
  chassis->mutable_gem()->mutable_date_time_rpt_83()->set_time_hour(time_hour(bytes, length));
  chassis->mutable_gem()->mutable_date_time_rpt_83()->set_date_day(date_day(bytes, length));
  chassis->mutable_gem()->mutable_date_time_rpt_83()->set_date_month(date_month(bytes, length));
  chassis->mutable_gem()->mutable_date_time_rpt_83()->set_date_year(date_year(bytes, length));
}
...
int Datetimerpt83::time_second(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 5);
...
int Datetimerpt83::time_minute(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 4);
  int32_t x = t0.get_byte(0, 8);
