...
void Wiperrpt91::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_gem()->mutable_wiper_rpt_91()->set_output_value(output_value(bytes, length));
  chassis->mutable_gem()->mutable_wiper_rpt_91()->set_commanded_value(
      commanded_value(bytes, length));
  chassis->mutable_gem()->mutable_wiper_rpt_91()->set_manual_input(manual_input(bytes, length));
}
...
Wiper_rpt_91::Output_valueType Wiperrpt91::output_value(const std::uint8_t* bytes,
                                                        int32_t             length) const {
  Byte    t0(bytes + 2);
  int32_t x = t0.get_byte(0, 8);

  Wiper_rpt_91::Output_valueType ret = static_cast<Wiper_rpt_91::Output_valueType>(x);
  return ret;
}
...
Wiper_rpt_91::Commanded_valueType Wiperrpt91::commanded_value(const std::uint8_t* bytes,
                                                              int32_t             length) const {
  Byte    t0(bytes + 1);
  int32_t x = t0.get_byte(0, 8);

  Wiper_rpt_91::Commanded_valueType ret = static_cast<Wiper_rpt_91::Commanded_valueType>(x);
  return ret;
}
...
// 'type': 'enum', 'order': 'motorola', 'physical_unit': ''}
Wiper_rpt_91::Manual_inputType Wiperrpt91::manual_input(const std::uint8_t* bytes,
                                                        int32_t             length) const {
  Byte    t0(bytes + 0);
  int32_t x = t0.get_byte(0, 8);

  Wiper_rpt_91::Manual_inputType ret = static_cast<Wiper_rpt_91::Manual_inputType>(x);
  return ret;
}
}  // namespace gem
}  // namespace canbus
