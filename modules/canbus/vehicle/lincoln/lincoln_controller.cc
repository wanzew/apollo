...
Chassis LincolnController::chassis() {
  ...
  if (chassis_detail.has_eps() && chassis_detail.eps().has_steering_angle()) {
    chassis_.set_steering_percentage(chassis_detail.eps().steering_angle() * 100.0 /
  ...
  if (chassis_detail.license().has_vin()) {
    ...
    if (!received_vin_) {
      apollo::common::KVDB::Put("apollo:canbus:vin", chassis_detail.license().vin());
      received_vin_ = true;
    }
  }
...
bool LincolnController::CheckChassisError() {
  ...
  if (!chassis_detail.has_eps()) {
    AERROR_EVERY(100) << "ChassisDetail has NO eps." << chassis_detail.DebugString();
    return false;
  }
  bool steer_fault =
...
void LincolnController::SecurityDogThreadFunc() {
  ...
  int64_t start = common::time::AsInt64<common::time::micros>(common::time::Clock::Now());
  ...
  while (can_sender_->IsRunning()) {
    ...
    int64_t end = common::time::AsInt64<common::time::micros>(common::time::Clock::Now());
    std::chrono::duration<double, std::micro> elapsed{end - start};
    if (elapsed < default_period) {
      std::this_thread::sleep_for(default_period - elapsed);
      start = common::time::AsInt64<common::time::micros>(common::time::Clock::Now());
    } else {
