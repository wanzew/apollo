...
ErrorCode
LincolnController::Init(const VehicleParameter&                                params,
  ...
  // Sender part
  brake_60_ = dynamic_cast<Brake60*>(message_manager_->GetMutableProtocolDataById(Brake60::ID));
  ...
  throttle_62_ =
      dynamic_cast<Throttle62*>(message_manager_->GetMutableProtocolDataById(Throttle62::ID));
  ...
  steering_64_ =
      dynamic_cast<Steering64*>(message_manager_->GetMutableProtocolDataById(Steering64::ID));
  if (steering_64_ == nullptr) {
    AERROR << "Steering64 does not exist in the LincolnMessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }

  gear_66_ = dynamic_cast<Gear66*>(message_manager_->GetMutableProtocolDataById(Gear66::ID));
  if (gear_66_ == nullptr) {
    AERROR << "Gear66 does not exist in the LincolnMessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }
  turnsignal_68_ =
      dynamic_cast<Turnsignal68*>(message_manager_->GetMutableProtocolDataById(Turnsignal68::ID));
...
Chassis LincolnController::chassis() {
  ...
  if (chassis_detail.has_vehicle_spd() && chassis_detail.vehicle_spd().has_vehicle_spd()) {
    chassis_.set_speed_mps(static_cast<float>(chassis_detail.vehicle_spd().vehicle_spd()));
    ...
    chassis_.mutable_wheel_speed()->set_wheel_spd_rr(chassis_detail.vehicle_spd().wheel_spd_rr());
    ...
    chassis_.mutable_wheel_speed()->set_wheel_spd_rl(chassis_detail.vehicle_spd().wheel_spd_rl());
    ...
    chassis_.mutable_wheel_speed()->set_wheel_spd_fr(chassis_detail.vehicle_spd().wheel_spd_fr());
    ...
    chassis_.mutable_wheel_speed()->set_wheel_spd_fl(chassis_detail.vehicle_spd().wheel_spd_fl());
  ...
  if (chassis_detail.has_basic() && chassis_detail.basic().has_odo_meter()) {
    // odo_meter is in km
    chassis_.set_odometer_m(static_cast<float>(chassis_detail.basic().odo_meter() * 1000.0));
  } else {
  ...
  // 13
  if (chassis_detail.has_eps() && chassis_detail.epb().has_parking_brake_status()) {
    chassis_.set_parking_brake(chassis_detail.epb().parking_brake_status() == Epb::PBRAKE_ON);
  } else {
    chassis_.set_parking_brake(false);
  }

  // 14, 15
  if (chassis_detail.has_light() && chassis_detail.light().has_lincoln_lamp_type()) {
    chassis_.mutable_signal()->set_high_beam(chassis_detail.light().lincoln_lamp_type() ==
                                             Light::BEAM_HIGH);
  } else {
  ...
  if (chassis_detail.basic().has_gps_valid()) {
    chassis_.mutable_chassis_gps()->set_latitude(chassis_detail.basic().latitude());
    chassis_.mutable_chassis_gps()->set_longitude(chassis_detail.basic().longitude());
    chassis_.mutable_chassis_gps()->set_gps_valid(chassis_detail.basic().gps_valid());
    ...
    chassis_.mutable_chassis_gps()->set_minutes(chassis_detail.basic().minutes());
    chassis_.mutable_chassis_gps()->set_seconds(chassis_detail.basic().seconds());
    ...
    chassis_.mutable_chassis_gps()->set_is_gps_fault(chassis_detail.basic().is_gps_fault());
    chassis_.mutable_chassis_gps()->set_is_inferred(chassis_detail.basic().is_inferred());
    chassis_.mutable_chassis_gps()->set_altitude(chassis_detail.basic().altitude());
    chassis_.mutable_chassis_gps()->set_heading(chassis_detail.basic().heading());
    ...
    chassis_.mutable_chassis_gps()->set_quality(chassis_detail.basic().quality());
    chassis_.mutable_chassis_gps()->set_num_satellites(chassis_detail.basic().num_satellites());
    chassis_.mutable_chassis_gps()->set_gps_speed(chassis_detail.basic().gps_speed());
  } else {
  ...
  if (chassis_detail.vehicle_id().has_vin()) {
    ...
    if (!received_vin_) {
      apollo::common::KVDB::Put("apollo:canbus:vin", chassis_detail.vehicle_id().vin());
      received_vin_ = true;
    }
  }
...
void LincolnController::Emergency() {
  ...
  ResetProtocol();
  if (chassis_error_code() == Chassis::NO_ERROR) { set_chassis_error_code(Chassis::CHASSIS_ERROR); }
}
...
void LincolnController::Gear(Chassis::GearPosition ref_gear_position) {
  ...
  ChassisDetail chassis_detail;
  message_manager_->GetSensorData(&chassis_detail);
  const Chassis::GearPosition current_gear_position = chassis_detail.gear().gear_state();
  if (ref_gear_position == current_gear_position) { return; }
...
void LincolnController::Steer(double angle, double angle_spd) {
  ...
  steering_64_->set_steering_angle(real_angle)->set_steering_angle_speed(real_angle_spd);
}
...
void LincolnController::ResetProtocol() { message_manager_->ResetSendMessages(); }
...
bool LincolnController::CheckChassisError() {
  ...
  // Steer fault
  bool steer_fault =
      chassis_detail.eps().watchdog_fault() | chassis_detail.eps().channel_1_fault() |
      chassis_detail.eps().channel_2_fault() | chassis_detail.eps().calibration_fault() |
      chassis_detail.eps().connector_fault();

  chassis_error_mask |= ((chassis_detail.eps().watchdog_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.eps().channel_1_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.eps().channel_2_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.eps().calibration_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.eps().connector_fault()) << (++error_cnt));

  if (!chassis_detail.has_brake()) {
    AERROR_EVERY(100) << "ChassisDetail has NO brake." << chassis_detail.DebugString();
    return false;
  }
  // Brake fault
  bool brake_fault = chassis_detail.brake().watchdog_fault() |
                     chassis_detail.brake().channel_1_fault() |
                     chassis_detail.brake().channel_2_fault() | chassis_detail.brake().boo_fault() |
                     chassis_detail.brake().connector_fault();

  chassis_error_mask |= ((chassis_detail.brake().watchdog_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.brake().channel_1_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.brake().channel_2_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.brake().boo_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.brake().connector_fault()) << (++error_cnt));

  if (!chassis_detail.has_gas()) {
    AERROR_EVERY(100) << "ChassisDetail has NO gas." << chassis_detail.DebugString();
    return false;
  }
  // Throttle fault
  bool throttle_fault =
      chassis_detail.gas().watchdog_fault() | chassis_detail.gas().channel_1_fault() |
      chassis_detail.gas().channel_2_fault() | chassis_detail.gas().connector_fault();

  chassis_error_mask |= ((chassis_detail.gas().watchdog_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.gas().channel_1_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.gas().channel_2_fault()) << (++error_cnt));
  chassis_error_mask |= ((chassis_detail.gas().connector_fault()) << (++error_cnt));

  if (!chassis_detail.has_gear()) {
    AERROR_EVERY(100) << "ChassisDetail has NO gear." << chassis_detail.DebugString();
    return false;
  }
  // Gear fault
  bool gear_fault = chassis_detail.gear().canbus_fault();

  chassis_error_mask |= ((chassis_detail.gear().canbus_fault()) << (++error_cnt));

  set_chassis_error_mask(chassis_error_mask);

  if (steer_fault) {
    set_chassis_error_code(Chassis::CHASSIS_ERROR_ON_STEER);
    AERROR_EVERY(100) << "Steering fault detected: " << chassis_detail.eps().watchdog_fault()
  ...
  if (brake_fault) {
    ...
    AERROR_EVERY(100) << "Brake fault detected: " << chassis_detail.brake().watchdog_fault() << ", "
                      << chassis_detail.brake().channel_1_fault() << ", "
  ...
  if (throttle_fault) {
    ...
    AERROR_EVERY(100) << "Throttle fault detected: " << chassis_detail.gas().watchdog_fault()
  ...
  if (steer_fault || brake_fault || throttle_fault) { return true; }
...
void LincolnController::SecurityDogThreadFunc() {
  ...
  while (can_sender_->IsRunning()) {
    ...
    if (CheckChassisError()) { emergency_mode = true; }

    if (emergency_mode && mode != Chassis::EMERGENCY_MODE) { Emergency(); }
    ...
    if (elapsed < default_period) {
      ...
      AERROR_EVERY(100) << "Too much time consumption in LincolnController looping process:"
                        << elapsed.count();
      start = end;
    }
  }
...
void LincolnController::set_chassis_error_code(const Chassis::ErrorCode& error_code) {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  chassis_error_code_ = error_code;
}

bool LincolnController::CheckSafetyError(const ::apollo::canbus::ChassisDetail& chassis_detail) {
  bool safety_error = chassis_detail.safety().is_passenger_door_open() ||
