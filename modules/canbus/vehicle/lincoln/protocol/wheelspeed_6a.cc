...
void Wheelspeed6a::Parse(const std::uint8_t* bytes,
                         int32_t             length,
                         ChassisDetail*      chassis_detail) const {
  // how to set direction
  // what is "valid"
  // front left
  chassis_detail->mutable_vehicle_spd()->set_wheel_spd_fl(front_left_wheel_speed(bytes, length));
  ...
  // front right
  chassis_detail->mutable_vehicle_spd()->set_wheel_spd_fr(front_right_wheel_speed(bytes, length));
  ...
  // rear left
  chassis_detail->mutable_vehicle_spd()->set_wheel_spd_rl(rear_left_wheel_speed(bytes, length));
  ...
  // rear right
  chassis_detail->mutable_vehicle_spd()->set_wheel_spd_rr(rear_right_wheel_speed(bytes, length));
...
double Wheelspeed6a::front_left_wheel_speed(const std::uint8_t* bytes, int32_t length) const {
  DCHECK_GE(length, 2);
  return parse_two_frames(bytes[0], bytes[1]);
}

double Wheelspeed6a::front_right_wheel_speed(const std::uint8_t* bytes, int32_t length) const {
  DCHECK_GE(length, 4);
  return parse_two_frames(bytes[2], bytes[3]);
}

double Wheelspeed6a::rear_left_wheel_speed(const std::uint8_t* bytes, int32_t length) const {
  DCHECK_GE(length, 6);
  return parse_two_frames(bytes[4], bytes[5]);
}

double Wheelspeed6a::rear_right_wheel_speed(const std::uint8_t* bytes, int32_t length) const {
  DCHECK_GE(length, 8);
...
WheelSpeed::WheelSpeedType Wheelspeed6a::wheel_direction_convert(double wheel_speed) const {
