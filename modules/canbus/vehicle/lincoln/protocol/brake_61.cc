...
void Brake61::Parse(const std::uint8_t* bytes,
                    int32_t             length,
  ...
  chassis_detail->mutable_brake()->set_brake_output(pedal_output(bytes, length));
  ...
  chassis_detail->mutable_brake()->set_watchdog_source(watchdog_counter_source(bytes, length));
  chassis_detail->mutable_brake()->set_brake_enabled(is_enabled(bytes, length));
  chassis_detail->mutable_brake()->set_driver_override(is_driver_override(bytes, length));
  chassis_detail->mutable_brake()->set_driver_activity(is_driver_activity(bytes, length));
  chassis_detail->mutable_brake()->set_watchdog_fault(is_watchdog_counter_fault(bytes, length));
  chassis_detail->mutable_brake()->set_channel_1_fault(is_channel_1_fault(bytes, length));
  chassis_detail->mutable_brake()->set_channel_2_fault(is_channel_2_fault(bytes, length));
  chassis_detail->mutable_brake()->set_boo_fault(is_boo_switch_fault(bytes, length));
  chassis_detail->mutable_brake()->set_connector_fault(is_connector_fault(bytes, length));
  chassis_detail->mutable_check_response()->set_is_esp_online(!is_driver_override(bytes, length));
}
...
double Brake61::parse_two_frames(const std::uint8_t low_byte, const std::uint8_t high_byte) const {
...
bool Brake61::is_watchdog_counter_applying_brakes(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 6);
  return frame.is_bit_1(3);
}

int32_t Brake61::watchdog_counter_source(const std::uint8_t* bytes, int32_t length) const {
...
bool Brake61::is_driver_override(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 7);
  return frame.is_bit_1(1);
}

bool Brake61::is_driver_activity(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 7);
  return frame.is_bit_1(2);
}

bool Brake61::is_watchdog_counter_fault(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 7);
  return frame.is_bit_1(3);
}

bool Brake61::is_channel_1_fault(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 7);
  return frame.is_bit_1(4);
}

bool Brake61::is_channel_2_fault(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 7);
  return frame.is_bit_1(5);
}

bool Brake61::is_boo_switch_fault(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 7);
  return frame.is_bit_1(6);
}

bool Brake61::is_connector_fault(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 7);
