...
void Gps6e::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis_detail) const {
  ...
  chassis_detail->mutable_basic()->set_compass_direction(compass_direction(bytes, length));
  ...
  chassis_detail->mutable_basic()->set_is_inferred(is_inferred_position(bytes, length));
}
...
double Gps6e::compass_direction(const std::uint8_t* bytes, int32_t length) const {
  Byte    frame(bytes + 6);
  int32_t x = frame.get_byte(0, 4);
  return x * 45.000000;
}
...
bool Gps6e::is_inferred_position(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 7);
