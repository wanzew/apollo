...
void Surround73::Parse(const std::uint8_t* bytes,
                       int32_t             length,
  ...
  for (std::int8_t i = 0; i < kSonarNumbers; ++i) {
    chassis_detail->mutable_surround()->add_sonar_range(sonars(bytes, i, length / 2));
  }
}

bool Surround73::is_cross_traffic_alert_left(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 0);
...
bool Surround73::is_blind_spot_left_alert(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 0);
  return frame.is_bit_1(2);
}

bool Surround73::is_blind_spot_left_alert_enabled(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 0);
  return frame.is_bit_1(3);
}

bool Surround73::is_cross_traffic_alert_right(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 0);
...
bool Surround73::is_blind_spot_right_alert(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 0);
...
bool Surround73::sonar_enabled(const std::uint8_t* bytes, int32_t length) const {
  Byte frame(bytes + 7);
  return frame.is_bit_1(6);
}
...
double
Surround73::sonars(const std::uint8_t* bytes, std::uint8_t sonar_number, int32_t length) const {
