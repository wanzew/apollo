...
void Gps6f::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis_detail) const {
  ...
  switch (fix_quality(bytes, length)) {
    case 0: chassis_detail->mutable_basic()->set_quality(FIX_NO); break;
    case 1: chassis_detail->mutable_basic()->set_quality(FIX_2D); break;
    case 2: chassis_detail->mutable_basic()->set_quality(FIX_3D); break;
    default: chassis_detail->mutable_basic()->set_quality(FIX_INVALID); break;
  }
  chassis_detail->mutable_basic()->set_num_satellites(num_satellites(bytes, length));
}
