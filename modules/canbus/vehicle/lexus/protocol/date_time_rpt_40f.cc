...
void Datetimerpt40f::Parse(const std::uint8_t* bytes,
                           int32_t             length,
  ...
  chassis->mutable_lexus()->mutable_date_time_rpt_40f()->set_time_hour(time_hour(bytes, length));
  chassis->mutable_lexus()->mutable_date_time_rpt_40f()->set_date_day(date_day(bytes, length));
  chassis->mutable_lexus()->mutable_date_time_rpt_40f()->set_date_month(date_month(bytes, length));
  chassis->mutable_lexus()->mutable_date_time_rpt_40f()->set_date_year(date_year(bytes, length));
}
...
int Datetimerpt40f::time_second(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 5);
...
int Datetimerpt40f::time_minute(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 4);
  int32_t x = t0.get_byte(0, 8);
...
int Datetimerpt40f::date_month(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 1);
  int32_t x = t0.get_byte(0, 8);
