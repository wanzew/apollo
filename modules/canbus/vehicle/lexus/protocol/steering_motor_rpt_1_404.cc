...
void Steeringmotorrpt1404::Parse(const std::uint8_t* bytes,
                                 int32_t             length,
                                 ChassisDetail*      chassis) const {
  chassis->mutable_lexus()->mutable_steering_motor_rpt_1_404()->set_motor_current(
      motor_current(bytes, length));
  chassis->mutable_lexus()->mutable_steering_motor_rpt_1_404()->set_shaft_position(
