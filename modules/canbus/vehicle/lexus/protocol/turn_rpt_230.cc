...
#include "modules/canbus/vehicle/lexus/protocol/turn_rpt_230.h"
...
void Turnrpt230::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_lexus()->mutable_turn_rpt_230()->set_vehicle_fault(vehicle_fault(bytes, length));
  chassis->mutable_lexus()->mutable_turn_rpt_230()->set_pacmod_fault(pacmod_fault(bytes, length));
  ...
  chassis->mutable_lexus()->mutable_turn_rpt_230()->set_enabled(enabled(bytes, length));
  chassis->mutable_lexus()->mutable_turn_rpt_230()->set_command_output_fault(
      command_output_fault(bytes, length));
  chassis->mutable_lexus()->mutable_turn_rpt_230()->set_manual_input(manual_input(bytes, length));
  chassis->mutable_lexus()->mutable_turn_rpt_230()->set_commanded_value(
      commanded_value(bytes, length));
  chassis->mutable_lexus()->mutable_turn_rpt_230()->set_output_value(output_value(bytes, length));
}

// config detail: {'name': 'vehicle_fault', 'offset': 0.0, 'precision': 1.0,
// 'len': 1, 'is_signed_var': False, 'physical_range': '[0|1]', 'bit': 6,
// 'type': 'bool', 'order': 'motorola', 'physical_unit': ''}
bool Turnrpt230::vehicle_fault(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
...
// 'type': 'bool', 'order': 'motorola', 'physical_unit': ''}
bool Turnrpt230::override_active(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
...
bool Turnrpt230::output_reported_fault(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
...
bool Turnrpt230::input_output_fault(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
...
bool Turnrpt230::command_output_fault(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
...
// 'physical_unit': ''}
Turn_rpt_230::Manual_inputType Turnrpt230::manual_input(const std::uint8_t* bytes,
                                                        int32_t             length) const {
  Byte    t0(bytes + 1);
  int32_t x = t0.get_byte(0, 8);

  Turn_rpt_230::Manual_inputType ret = static_cast<Turn_rpt_230::Manual_inputType>(x);
  return ret;
}
...
Turn_rpt_230::Commanded_valueType Turnrpt230::commanded_value(const std::uint8_t* bytes,
                                                              int32_t             length) const {
  Byte    t0(bytes + 2);
  int32_t x = t0.get_byte(0, 8);

  Turn_rpt_230::Commanded_valueType ret = static_cast<Turn_rpt_230::Commanded_valueType>(x);
  return ret;
}
...
// 'physical_unit': ''}
Turn_rpt_230::Output_valueType Turnrpt230::output_value(const std::uint8_t* bytes,
                                                        int32_t             length) const {
  Byte    t0(bytes + 3);
  int32_t x = t0.get_byte(0, 8);

  Turn_rpt_230::Output_valueType ret = static_cast<Turn_rpt_230::Output_valueType>(x);
  return ret;
}
}  // namespace lexus
}  // namespace canbus
