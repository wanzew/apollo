...
void Turnauxrpt330::Parse(const std::uint8_t* bytes, int32_t length, ChassisDetail* chassis) const {
  chassis->mutable_lexus()->mutable_turn_aux_rpt_330()->set_pass_blinker_bulb_on_is_valid(
      pass_blinker_bulb_on_is_valid(bytes, length));
  chassis->mutable_lexus()->mutable_turn_aux_rpt_330()->set_pass_blinker_bulb_on(
      pass_blinker_bulb_on(bytes, length));
  chassis->mutable_lexus()->mutable_turn_aux_rpt_330()->set_driver_blinker_bulb_on_is_valid(
      driver_blinker_bulb_on_is_valid(bytes, length));
  chassis->mutable_lexus()->mutable_turn_aux_rpt_330()->set_driver_blinker_bulb_on(
