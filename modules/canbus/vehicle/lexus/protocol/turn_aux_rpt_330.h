...
class Turnauxrpt330
 ...
 private:
  ...
  // ''}
  bool pass_blinker_bulb_on_is_valid(const std::uint8_t* bytes, const int32_t length) const;

  // config detail: {'name': 'PASS_BLINKER_BULB_ON', 'offset': 0.0,
  // 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'physical_range':
  // '[0|1]', 'bit': 1, 'type': 'bool', 'order': 'motorola', 'physical_unit':
  // ''}
  bool pass_blinker_bulb_on(const std::uint8_t* bytes, const int32_t length) const;

  // config detail: {'name': 'DRIVER_BLINKER_BULB_ON_IS_VALID', 'offset': 0.0,
  // 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'physical_range':
  // '[0|1]', 'bit': 8, 'type': 'bool', 'order': 'motorola', 'physical_unit':
  // ''}
  bool driver_blinker_bulb_on_is_valid(const std::uint8_t* bytes, const int32_t length) const;

  // config detail: {'name': 'DRIVER_BLINKER_BULB_ON', 'offset': 0.0,
  // 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'physical_range':
  // '[0|1]', 'bit': 0, 'type': 'bool', 'order': 'motorola', 'physical_unit':
  // ''}
  bool driver_blinker_bulb_on(const std::uint8_t* bytes, const int32_t length) const;
};
