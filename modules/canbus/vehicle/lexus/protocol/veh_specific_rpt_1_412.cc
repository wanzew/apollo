...
void Vehspecificrpt1412::Parse(const std::uint8_t* bytes,
                               int32_t             length,
...
int Vehspecificrpt1412::shift_pos_2(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 1);
...
// 'type': 'int', 'order': 'motorola', 'physical_unit': ''}
int Vehspecificrpt1412::shift_pos_1(const std::uint8_t* bytes, int32_t length) const {
  Byte    t0(bytes + 0);
  int32_t x = t0.get_byte(0, 8);
