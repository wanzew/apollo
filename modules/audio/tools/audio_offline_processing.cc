...
void ProcessSingleRecordFile(const AudioConf&    audio_conf,
  ...
  while (reader.ReadMessage(&message)) {
    if (message.channel_name == audio_conf.topic_conf().audio_data_topic_name()) {
      AudioData audio_data;
      if (audio_data.ParseFromString(message.content)) {
        AudioDetection audio_detection;
        std::string    respeaker_extrinsics_file = audio_conf.respeaker_extrinsics_path();
        MessageProcess::OnMicrophone(audio_data, respeaker_extrinsics_file, audio_info,
                                     direction_detection, moving_detection, siren_detection,
                                     &audio_detection);
        writer.WriteMessage<AudioDetection>(audio_conf.topic_conf().audio_detection_topic_name(),
                                            audio_detection, message.time);
        AINFO << "Generate a new audio detection message.";
      }
    } else if (message.channel_name == audio_conf.topic_conf().audio_event_topic_name()) {
      AudioEvent audio_event;
      if (audio_event.ParseFromString(message.content)) {
        writer.WriteMessage<AudioEvent>(message.channel_name, audio_event, message.time);
        AINFO << "Save an audio even message.";
      }
    } else if (message.channel_name == audio_conf.topic_conf().localization_topic_name()) {
      LocalizationEstimate localization;
      if (localization.ParseFromString(message.content)) {
        writer.WriteMessage<LocalizationEstimate>(message.channel_name, localization, message.time);
        AINFO << "Save a localization message.";
      }
    } else if (message.channel_name == audio_conf.topic_conf().perception_topic_name()) {
...
void ProcessFolder() {
  ...
  if (!cyber::common::GetProtoFromFile(FLAGS_audio_conf_file, &audio_conf)) { return; }
  AudioInfo                audio_info;
  DirectionDetection       direction_detection;
  MovingDetection          moving_detection;
  SirenDetection           siren_detection;
  std::vector<std::string> offline_bags;
  GetRecordFileNames(boost::filesystem::path(FLAGS_audio_records_dir), &offline_bags);
  ...
  for (std::size_t i = 0; i < offline_bags.size(); ++i) {
    ...
    std::string        output_record_filepath = input_record_filepath + ".new_audio_detection";
    ProcessSingleRecordFile(audio_conf, input_record_filepath, output_record_filepath, &audio_info,
                            &direction_detection, &moving_detection, &siren_detection);
  }
}
