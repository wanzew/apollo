...
MovingResult MovingDetection::Detect(const std::vector<std::vector<double>>& signals) {
  ...
  if (approaching_count > departing_count) { return APPROACHING; }
  if (approaching_count < departing_count) { return DEPARTING; }
  return UNKNOWN;
}

MovingResult MovingDetection::DetectSingleChannel(const std::size_t          channel_index,
  ...
  while (static_cast<int>(signal_stats_[channel_index].size()) > kFrameNumStored) {
  ...
  if (power_result != UNKNOWN) { return power_result; }
  MovingResult top_frequency_result = AnalyzeTopFrequence(signal_stats_[channel_index]);
  return top_frequency_result;
}

MovingResult MovingDetection::AnalyzePower(const std::deque<SignalStat>& signal_stats) {
  ...
  if (first < second && second < third) { return APPROACHING; }
  if (first > second && second > third) { return DEPARTING; }
  return UNKNOWN;
}

MovingResult MovingDetection::AnalyzeTopFrequence(const std::deque<SignalStat>& signal_stats) {
  int n = static_cast<int>(signal_stats.size());
  if (n < 3) { return UNKNOWN; }
...
std::vector<std::complex<double>> MovingDetection::fft1d(const std::vector<double>& signal) {
...
MovingDetection::SignalStat
MovingDetection::GetSignalStat(const std::vector<std::complex<double>>& fft_results,
