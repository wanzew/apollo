...
void MessageProcess::OnMicrophone(const AudioData&    audio_data,
  ...
  auto direction_result = direction_detection->EstimateSoundSource(
      audio_info->GetSignals(audio_data.microphone_config().chunk()), respeaker_extrinsics_file,
      audio_data.microphone_config().sample_rate(), audio_data.microphone_config().mic_distance());
  *(audio_detection->mutable_position()) = direction_result.first;
  ...
  auto         signals       = audio_info->GetSignals(audio_data.microphone_config().chunk());
