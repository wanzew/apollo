...
MF::MeanFilter(const uint8 window_size)
...
double MF::Update(const double measurement) {
  ...
  if (values_.size() == window_size_) { RemoveEarliest(); }
  Insert(measurement);
  if (values_.size() > 2) {
    return (sum_ - GetMin() - GetMax()) / static_cast<double>(values_.size() - 2);
  } else {
...
void MF::RemoveEarliest() {
  ...
  values_.pop_front();
  sum_ -= removed;
  if (ShouldPopOldestCandidate(min_candidates_.front().first)) { min_candidates_.pop_front(); }
  if (ShouldPopOldestCandidate(max_candidates_.front().first)) { max_candidates_.pop_front(); }
}
