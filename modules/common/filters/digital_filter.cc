...
double DigitalFilter::UpdateLast(const double input) {
  ...
  if (diff < dead_zone_) { return last_; }
  last_ = input;
  return input;
}
...
const std::deque<double>& DigitalFilter::inputs_queue() const { return x_values_; }

const std::deque<double>& DigitalFilter::outputs_queue() const { return y_values_; }
