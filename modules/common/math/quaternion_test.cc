...
TEST(QuaternionTest, QuaternionToHeading) {
  ...
  EXPECT_DOUBLE_EQ(0, QuaternionToHeading(v, 0.0, 0.0, -v));          // Pointing to east.
  EXPECT_DOUBLE_EQ(M_PI_2, QuaternionToHeading(1.0, 0.0, 0.0, 0.0));  // Pointing to north.
  EXPECT_DOUBLE_EQ(-M_PI_2, QuaternionToHeading(0.0, 0.0, 0.0,
                                                1.0));           // Pointing to south.
  EXPECT_DOUBLE_EQ(-M_PI, QuaternionToHeading(v, 0.0, 0.0, v));  // Pointing to west.
  ...
  for (double heading : headings) {
    EXPECT_NEAR(NormalizeAngle(heading), QuaternionToHeading(HeadingToQuaternion<double>(heading)),
                1e-15);
  }
}
...
TEST(QuaternionTest, QuaternionRotate) {
  ...
  Eigen::Vector3d original(0.18112868882363914, 0.38614886414425986, -0.15861744649897938);
...
TEST(QuaternionTest, InverseQuaternionRotate) {
  ...
  q.set_qz(-0.99256254167039326);
  q.set_qw(-0.1199007240933047);
  Eigen::Vector3d rotated(-0.26184808017295008, -0.32827419468368224, -0.17535585973456849);
