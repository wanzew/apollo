...
double LineSegment2d::DistanceTo(const Vec2d& point) const {
  ...
  if (proj >= length_) { return point.DistanceTo(end_); }
...
double LineSegment2d::DistanceSquareTo(const Vec2d& point) const {
  if (length_ <= kMathEpsilon) { return point.DistanceSquareTo(start_); }
  const double x0   = point.x() - start_.x();
  const double y0   = point.y() - start_.y();
  const double proj = x0 * unit_direction_.x() + y0 * unit_direction_.y();
  if (proj <= 0.0) { return Square(x0) + Square(y0); }
  if (proj >= length_) { return point.DistanceSquareTo(end_); }
...
bool LineSegment2d::IsPointIn(const Vec2d& point) const {
  ...
  if (std::abs(prod) > kMathEpsilon) { return false; }
...
bool LineSegment2d::GetIntersect(const LineSegment2d& other_segment, Vec2d* const point) const {
  ...
  if (cc3 * cc4 >= -kMathEpsilon) { return false; }
...
std::string LineSegment2d::DebugString() const {
  return util::StrCat("segment2d ( start = ", start_.DebugString(), "  end = ", end_.DebugString(),
                      " )");
}
