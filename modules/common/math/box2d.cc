...
double PtSegDistance(double query_x,
  ...
  if (proj <= 0.0) { return hypot(x0, y0); }
  if (proj >= length * length) { return hypot(x0 - dx, y0 - dy); }
...
double Box2d::DistanceTo(const Vec2d& point) const {
  ...
  if (dx <= 0.0) { return std::max(0.0, dy); }
  if (dy <= 0.0) { return dx; }
  return hypot(dx, dy);
}
...
double Box2d::DistanceTo(const LineSegment2d& line_segment) const {
  ...
  if (gx1 == 1 && gy1 == 1) {
    switch (gx2 * 3 + gy2) {
      case 4: return PtSegDistance(box_x, box_y, x1, y1, x2, y2, line_segment.length());
      case 3:
    ...
    switch (gx2 * 3 + gy2) {
      case 4:
        return (x1 < x2) ? (x1 - box_x) :
                           PtSegDistance(box_x, box_y, x1, y1, x2, y2, line_segment.length());
      case 3: return std::min(x1, x2) - box_x;
      case 1:
      case -2:
        return CrossProd({x1, y1}, {x2, y2}, {box_x, box_y}) <= 0.0 ?
                   0.0 :
  ...
  ACHECK(0) << "unimplemented state: " << gx1 << " " << gy1 << " " << gx2 << " " << gy2;
  return 0.0;
}
...
std::string Box2d::DebugString() const {
  return absl::StrCat("box2d ( center = ", center_.DebugString(), "  heading = ", heading_,
