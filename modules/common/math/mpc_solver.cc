...
bool SolveLinearMPC(const Matrix&              matrix_a,
  ...
  for (unsigned int r = 1; r < horizon; ++r) {
    for (unsigned int c = 0; c < r; ++c) {
      matrix_k.block(r * matrix_b.rows(), c * matrix_b.cols(), matrix_b.rows(), matrix_b.cols()) =
          matrix_a_power[r - c - 1] * matrix_b;
    }
    matrix_k.block(r * matrix_b.rows(), r * matrix_b.cols(), matrix_b.rows(), matrix_b.cols()) =
        matrix_b;
  }
  ...
  for (unsigned int i = 0; i < horizon; ++i) {
    ...
    matrix_qq.block(i * matrix_q.rows(), i * matrix_q.rows(), matrix_q.rows(), matrix_q.rows()) =
        matrix_q;
    matrix_rr.block(i * matrix_r.rows(), i * matrix_r.rows(), matrix_r.cols(), matrix_r.cols()) =
        matrix_r;
  }
  ...
  for (unsigned int i = 1; i < horizon; ++i) {
    matrix_cc.block(i * matrix_c.rows(), 0, matrix_c.rows(), 1) =
        ...
        matrix_aa.block((i - 1) * matrix_a.rows(), 0, matrix_a.rows(), matrix_a.cols()) * matrix_c;
  }
  ...
  // Method 1: QPOASES
  Matrix matrix_inequality_constrain_ll = Matrix::Identity(matrix_ll.rows(), matrix_ll.rows());
  Matrix matrix_inequality_constrain_uu = Matrix::Identity(matrix_uu.rows(), matrix_uu.rows());
  ...
  std::unique_ptr<QpSolver> qp_solver(new ActiveSetQpSolver(
      matrix_m1, matrix_m2, matrix_inequality_constrain, matrix_inequality_boundary,
