...
bool ProjectByXSlow(const std::vector<Vec2d>& points,
                    double                    x,
                    double* const             min_y,
                    double* const             max_y) {
...
TEST(Polygon2dTest, polygon_IsPointIn) {
  ...
  EXPECT_EQ(poly1.DebugString(), "polygon2d (  num_points = 4  points = (vec2d ( x = 1  y = 0 ) "
...
TEST(Polygon2dTest, ConvexHull) {
  ...
  for (int iter = 0; iter < 10000; ++iter) {
    ...
    for (int x0 = 0; x0 < kRange; ++x0) {
      double min_y = 0.0;
      double max_y = 0.0;
      if (ProjectByXSlow(points, x0 + 0.5, &min_y, &max_y)) { area += max_y - min_y; }
    }
    Polygon2d polygon;
    if (area < 1e-3) {
...
TEST(Polygon2dTest, Overlap) {
  ...
  EXPECT_FALSE(
      poly1.GetOverlap(LineSegment2d({-1, 0}, {-1, 2}), &first_intersect, &last_intersect));
  EXPECT_FALSE(
      poly1.GetOverlap(LineSegment2d({-1, 1}, {-3, 1}), &first_intersect, &last_intersect));
  ...
  for (int iter = 0; iter < 10000; ++iter) {
    ...
    for (const auto& line_segment1 : polygon1.line_segments()) {
      for (const auto& line_segment2 : polygon2.line_segments()) {
        Vec2d pt;
        if (line_segment1.GetIntersect(line_segment2, &pt)) { key_points.push_back(pt.x()); }
      }
    }
    double area = 0;
...
TEST(Polygon2dTest, BoundingBox) {
  ...
  for (int iter = 0; iter < 1000; ++iter) {
    ...
    Polygon2d polygon;
    if (!Polygon2d::ComputeConvexHull(points, &polygon)) { continue; }
