...
ActiveSetQpSolver::ActiveSetQpSolver(const Eigen::MatrixXd& kernel_matrix,
    ...
    : QpSolver(kernel_matrix,
               offset,
               affine_inequality_matrix,
               affine_inequality_boundary,
               affine_equality_matrix,
               affine_equality_boundary)
    , num_constraint_(affine_equality_matrix_.rows() + affine_inequality_matrix_.rows())
...
bool ActiveSetQpSolver::Solve() {
  ...
  if (hessian_type_ == ::qpOASES::HST_POSDEF || hessian_type_ == ::qpOASES::HST_SEMIDEF) {
  ...
  if (!debug_info_) { qp_problem.setPrintLevel(qpOASES::PL_NONE); }
  if (kernel_matrix_.rows() != kernel_matrix_.cols()) {
    AERROR << "kernel_matrix_.rows() [" << kernel_matrix_.rows() << "] and kernel_matrix_.cols() ["
           << kernel_matrix_.cols() << "] should be identical.";
    return false;
  }
  // definition of qpOASESproblem
  const int kNumOfMatrixElements = kernel_matrix_.rows() * kernel_matrix_.cols();
  ...
  for (int r = 0; r < affine_inequality_matrix_.rows(); ++r) {
    ...
    constraint_upper_bound[r + affine_equality_boundary_.rows()] = constraint_upper_bound_;
  ...
  auto ret = qp_problem.init(h_matrix, g_matrix, affine_constraint_matrix, lower_bound, upper_bound,
...
void ActiveSetQpSolver::set_qp_eps_iter_ref(const double eps) { qp_eps_iter_ref_ = eps; }

void ActiveSetQpSolver::set_debug_info(const bool enable) { debug_info_ = enable; }
...
void ActiveSetQpSolver::set_constraint_upper_bound(const double la_upper_bound) {
  constraint_upper_bound_ = la_upper_bound;
}

void ActiveSetQpSolver::set_max_iteration(const int max_iter) { max_iteration_ = max_iter; }
...
double ActiveSetQpSolver::constraint_upper_bound() const { return constraint_upper_bound_; }
