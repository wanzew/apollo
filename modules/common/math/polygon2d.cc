...
double Polygon2d::DistanceTo(const LineSegment2d& line_segment) const {
  if (line_segment.length() <= kMathEpsilon) { return DistanceTo(line_segment.start()); }
  CHECK_GE(points_.size(), 3U);
  if (IsPointIn(line_segment.center())) { return 0.0; }
  if (std::any_of(line_segments_.begin(), line_segments_.end(), [&](const LineSegment2d& poly_seg) {
...
double Polygon2d::DistanceTo(const Polygon2d& polygon) const {
  ...
  if (IsPointIn(polygon.points()[0])) { return 0.0; }
  if (polygon.IsPointIn(points_[0])) { return 0.0; }
  double distance = std::numeric_limits<double>::infinity();
  for (int i = 0; i < num_points_; ++i) {
...
bool Polygon2d::IsPointIn(const Vec2d& point) const {
  ...
  for (int i = 0; i < num_points_; ++i) {
    if ((points_[i].y() > point.y()) != (points_[j].y() > point.y())) {
      ...
      if (points_[i].y() < points_[j].y() ? side > 0.0 : side < 0.0) { ++c; }
    }
    j = i;
  }
  return c & 1;
...
bool Polygon2d::Contains(const LineSegment2d& line_segment) const {
  if (line_segment.length() <= kMathEpsilon) { return IsPointIn(line_segment.start()); }
  CHECK_GE(points_.size(), 3U);
  if (!IsPointIn(line_segment.start())) { return false; }
  if (!IsPointIn(line_segment.end())) { return false; }
  if (!is_convex_) {
...
bool Polygon2d::Contains(const Polygon2d& polygon) const {
  CHECK_GE(points_.size(), 3U);
  if (area_ < polygon.area() - kMathEpsilon) { return false; }
  if (!IsPointIn(polygon.points()[0])) { return false; }
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d>& points, Polygon2d* const polygon) {
  ...
  std::sort(sorted_indices.begin(), sorted_indices.end(), [&](const int idx1, const int idx2) {
    ...
    if (std::abs(dx) > kMathEpsilon) { return dx < 0.0; }
    return pt1.y() < pt2.y();
  });
  int              count = 0;
...
bool Polygon2d::ClipConvexHull(const LineSegment2d&      line_segment,
  ...
  if (line_segment.length() <= kMathEpsilon) { return true; }
  ...
  for (size_t i = 0; i < n; ++i) {
    if (side[i] >= 0) { new_points.push_back((*points)[i]); }
...
bool Polygon2d::ComputeOverlap(const Polygon2d& other_polygon,
  ...
  for (int i = 0; i < num_points_; ++i) {
    if (!ClipConvexHull(line_segments_[i], &points)) { return false; }
  }
...
double Polygon2d::ComputeIoU(const Polygon2d& other_polygon) const {
  ...
  if (!ComputeOverlap(other_polygon, &overlap_polygon)) { return 0.0; }
...
std::vector<LineSegment2d> Polygon2d::GetAllOverlaps(const LineSegment2d& line_segment) const {
  ...
  if (line_segment.length() <= kMathEpsilon) {
    ...
    if (IsPointIn(line_segment.start())) { overlaps.push_back(line_segment); }
    return overlaps;
  }
  std::vector<double> projections;
  if (IsPointIn(line_segment.start())) { projections.push_back(0.0); }
  if (IsPointIn(line_segment.end())) { projections.push_back(line_segment.length()); }
  for (const auto& poly_seg : line_segments_) {
    Vec2d pt;
    if (poly_seg.GetIntersect(line_segment, &pt)) {
  ...
  for (int i = 0; i < num_points_; ++i) {
    ...
    if (diff <= kMathEpsilon) {
      ...
      for (int k = 0; k <= count; ++k) {
        const double angle =
            start_angle + diff * static_cast<double>(k) / static_cast<double>(count);
