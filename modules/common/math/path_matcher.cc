...
PathPoint PathMatcher::MatchToPath(const std::vector<PathPoint>& reference_line,
  ...
  auto func_distance_square = [](const PathPoint& point, const double x, const double y) {
    double dx = point.x() - x;
    double dy = point.y() - y;
    return dx * dx + dy * dy;
  };
  ...
  if (index_start == index_end) { return reference_line[index_start]; }
...
PathPoint PathMatcher::MatchToPath(const std::vector<PathPoint>& reference_line, const double s) {
  auto comp = [](const PathPoint& point, const double s) { return point.s() < s; };
