...
SplineInterpolate(const TrajectoryPoint& tp0, const TrajectoryPoint& tp1, const double t) {
  if (std::fabs(tp1.path_point().s() - tp0.path_point().s()) < 1.0e-4) { return tp1; }
  ...
  double s0     = 0.0;
  auto   func_v = [&dynamic_spline](const double t) { return dynamic_spline.Evaluate(0, t); };
  double s1     = IntegrateByGaussLegendre<5>(func_v, t0, t1);
  double s      = IntegrateByGaussLegendre<5>(func_v, t0, t);

  if (std::fabs(tp0.path_point().s() - s1) < 1.0e-4) { return tp1; }
