...
void VehicleModel::RearCenteredKinematicBicycleModel(const VehicleModelConfig& vehicle_model_config,
  ...
  double next_v   = cur_v;
  if (dt >= predicted_time_horizon) { dt = predicted_time_horizon; }
  ...
  predicted_vehicle_state->set_linear_acceleration(cur_vehicle_state.linear_acceleration());
}
...
VehicleState VehicleModel::Predict(const double        predicted_time_horizon,
  ...
  ACHECK(
      cyber::common::GetProtoFromFile(FLAGS_vehicle_model_config_filename, &vehicle_model_config))
      << "Failed to load vehicle model config file " << FLAGS_vehicle_model_config_filename;
