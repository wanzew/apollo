...
std::string filter_duplicates_str(
    int id, const char* str1, const char* str2, const char* str3, const char* str4) {
  // id is unused.
...
std::string filter_duplicates(int id) {
  ...
  std::stringstream ss_in;
  ss_in << "a a b b b c foo foo bar foobar foobar hello world hello hello world";
  std::set<std::string>              string_set;
  std::istream_iterator<std::string> beg(ss_in);
  std::istream_iterator<std::string> end;
  std::copy(beg, end, std::inserter(string_set, string_set.end()));
...
TEST(ThreadPool, filter_duplicates) {
  ...
  const unsigned int threads          = std::min(hardware_threads != 0 ? hardware_threads : 2, 50U);
  ThreadPool         p(threads);

  std::vector<std::future<std::string>> futures1, futures2;
  for (int i = 0; i < 1000; ++i) {
    futures1.push_back(
        std::move(p.Push(filter_duplicates_str, "thread pthread", "pthread thread good news",
