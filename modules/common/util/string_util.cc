...
std::string Base64Piece(const char in0, const char in1, const char in2) {
  ...
  if (in1) { out[2] = kBase64Array[(triplet >> 6) & 0x3f]; }
  if (in2) { out[3] = kBase64Array[triplet & 0x3f]; }
  return out;
}
...
std::string EncodeBase64(std::string_view in) {
  ...
  if (in_size % 3 == 1) { absl::StrAppend(&out, Base64Piece(in[in_size - 1], 0, 0)); }
  if (in_size % 3 == 2) { absl::StrAppend(&out, Base64Piece(in[in_size - 2], in[in_size - 1], 0)); }
  return out;
}
