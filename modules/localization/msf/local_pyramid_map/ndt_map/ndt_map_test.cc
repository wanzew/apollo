...
TEST(MapNdtTestSuite, matrix) {
  ...
  unsigned int node_size = static_cast<unsigned int>(distance / map_config.map_resolutions_[0]);
  ...
  for (unsigned int frame_idx = 0; frame_idx < pcd_poses.size(); ++frame_idx) {
    const std::string pcd_file_path = absl::StrCat(pcd_folder, "/", pcd_indices[frame_idx], ".pcd");
    velodyne::VelodyneFrame velodyne_frame;
    velodyne::LoadPcds(pcd_file_path, pcd_indices[frame_idx], pcd_poses[frame_idx], &velodyne_frame,
                       false);
    ...
    for (size_t k = 0; k < velodyne_frame.pt3ds.size(); ++k) {
      ...
      for (size_t res = 0; res < map_config.map_resolutions_.size(); ++res) {
        MapNodeIndex  index          = MapNodeIndex::GetMapNodeIndex(map_config, pt3d_global,
                                                           static_cast<unsigned int>(res), zone_id);
        NdtMapNode*   ndt_map_node   = static_cast<NdtMapNode*>(ndt_map.GetMapNodeSafe(index));
        ...
        int altitude_index = ndt_map_cells.CalAltitudeIndex(map_config.map_resolutions_z_[res],
                                                            static_cast<float>(pt3d_global[2]));
        const Eigen::Vector3f centroid(
            static_cast<float>(coord2d[0]) - static_cast<float>(left_top_corner[0]) -
                resolution * static_cast<float>(x),
            static_cast<float>(coord2d[1]) - static_cast<float>(left_top_corner[1]) -
                resolution * static_cast<float>(y),
            static_cast<float>(pt3d_global[2]) -
                static_cast<float>(altitude_index) * map_config.map_resolutions_z_[res]);

        // Add sample 3d points
        ndt_map_cells.AddSample(intensity, static_cast<float>(pt3d_global[2]),
                                map_config.map_resolutions_z_[res], centroid, false);
