...
size_t NdtMapSingleCell::LoadBinary(const unsigned char* buf) {
  ...
  const unsigned int* ui_buf =
      reinterpret_cast<const unsigned int*>(reinterpret_cast<const void*>(f_buf));
...
size_t NdtMapSingleCell::GetBinarySize() const {
  ...
  if (count_ >= minimum_points_threshold_) { sz += sizeof(float) * 9 + sizeof(unsigned char) * 1; }
  return sz;
}
...
void NdtMapSingleCell::AddSample(const float            intensity,
  ...
  intensity_var_ =
      (static_cast<float>(count_ - 1) * intensity_var_ + v1 * v2) / static_cast<float>(count_);

  if (is_road) { ++road_pt_count_; }
...
void NdtMapSingleCell::MergeCell(const float            intensity,
  ...
  intensity_var_ =
      intensity_var_ * p0 + intensity_var * p1 + intensity_diff * intensity_diff * p0 * p1;
...
void NdtMapSingleCell::MergeCell(const NdtMapSingleCell& cell_new) {
  MergeCell(cell_new.intensity_, cell_new.intensity_var_, cell_new.road_pt_count_, cell_new.count_,
            cell_new.centroid_, cell_new.centroid_average_cov_);
}

void NdtMapSingleCell::CentroidEigenSolver(const Eigen::Matrix3f& centroid_cov) {
  ...
  if (count_ >= minimum_points_threshold_) {
    ...
    if (eigen_val(0, 0) < min_covar_eigvalue) {
      ...
      if (eigen_val(1, 1) < min_covar_eigvalue) { eigen_val(1, 1) = min_covar_eigvalue; }
    }
    // Calculate inverse covariance
    centroid_icov_ = (centroid_evecs * eigen_val * centroid_evecs.inverse()).inverse();
...
int NdtMapCells::AddSample(const float            intensity,
                           const float            altitude,
  ...
  if (altitude_index > max_altitude_index_) { max_altitude_index_ = altitude_index; }
  if (altitude_index < min_altitude_index_) { min_altitude_index_ = altitude_index; }
  if (is_road) {
    auto got = std::find(road_cell_indices_.begin(), road_cell_indices_.end(), altitude_index);
    if (got == road_cell_indices_.end()) { road_cell_indices_.push_back(altitude_index); }
  }
...
int NdtMapCells::CalAltitudeIndex(const float resolution, const float altitude) {
  return static_cast<int>(altitude / resolution);
}

float NdtMapCells::CalAltitude(const float resolution, const int altitude_index) {
  return static_cast<float>(resolution * (static_cast<float>(altitude_index) + 0.5));
}
...
void NdtMapCells::Reduce(NdtMapCells* cell, const NdtMapCells& cell_new) {
  ...
  for (auto it_new = cell_new.road_cell_indices_.begin();
    ...
    auto got_it =
        std::find(cell->road_cell_indices_.begin(), cell->road_cell_indices_.end(), *it_new);
  ...
  for (unsigned int y = 0; y < rows_; ++y) {
    for (unsigned int x = 0; x < cols_; ++x) {
      ...
      intensity_img->at<unsigned char>(y, x) =
          (unsigned char)(map3d_cells_[id].cells_[min_altitude_index].intensity_);
    }
  }
  return true;
}
