...
unsigned int BaseMapNodeConfig::LoadBinary(const unsigned char* buf) {
  ...
  binary_size += sizeof(unsigned int) + sizeof(int) + sizeof(unsigned int) + sizeof(unsigned int) +
                 sizeof(unsigned int);
...
unsigned int BaseMapNodeConfig::CreateBinary(unsigned char* buf, size_t buf_size) const {
  unsigned int target_size = GetBinarySize();

  if (buf_size < target_size) { return 0; }
...
unsigned int BaseMapNodeConfig::GetBinarySize() const {
  ...
  // map_version
  if (has_map_version_) { binary_size += sizeof(uint16_t); }

  // body_md5
  if (has_body_md5_) { binary_size += sizeof(body_md5_); }
