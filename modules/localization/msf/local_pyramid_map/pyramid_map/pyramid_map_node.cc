...
void PyramidMapNode::Init(const BaseMapConfig* map_config) {
  ...
  map_matrix_handler_.reset(PyramidMapMatrixHandlerSelector::AllocPyramidMapMatrixHandler(
      map_node_config_->map_version_));
  compression_strategy_.reset(new ZlibStrategy());

  const PyramidMapConfig* pm_map_config = dynamic_cast<const PyramidMapConfig*>(map_config_);
...
bool PyramidMapNode::AddValueIfInBound(const Eigen::Vector3d& coordinate,
  ...
  if (is_success) {
    std::shared_ptr<PyramidMapMatrix> map_matrix =
        std::dynamic_pointer_cast<PyramidMapMatrix>(map_matrix_);
    map_matrix->AddSampleBase(static_cast<float>(intensity), static_cast<float>(coordinate[2]), y,
                              x, level);
    return true;
  } else {
    return false;
...
void PyramidMapNode::AddValueIfInBound(const std::vector<Eigen::Vector3d>& coordinates,
                                       const std::vector<unsigned char>&   intensity,
                                       unsigned int                        level) {
  if (coordinates.size() != intensity.size()) { return; }
...
bool PyramidMapNode::GetCoordinate(const Eigen::Vector2d& coordinate,
  ...
  unsigned int off_x =
      static_cast<unsigned int>((coordinate[0] - left_top_corner_[0]) / current_resolution);
  unsigned int off_y =
      static_cast<unsigned int>((coordinate[1] - left_top_corner_[1]) / current_resolution);
  if (off_x < map_matrix->GetCols(level) && off_y < map_matrix->GetRows(level)) {
    *x = off_x;
    *y = off_y;
    return true;
  }
...
Eigen::Vector2d
PyramidMapNode::GetCoordinate(unsigned int level, unsigned int x, unsigned int y) const {
  const PyramidMapConfig* map_config = dynamic_cast<const PyramidMapConfig*>(map_config_);

  float current_resolution = static_cast<float>(map_config->map_resolutions_[0] *
...
Eigen::Vector2d PyramidMapNode::GetCoordinate(unsigned int x, unsigned int y) const {
...
float PyramidMapNode::GetIntensitySafe(const Eigen::Vector3d& coordinate,
  ...
  if (is_success) {
    ...
    if (intensity_ptr != nullptr) { intensity = *intensity_ptr; }
  }
...
float PyramidMapNode::GetIntensityVarSafe(const Eigen::Vector3d& coordinate,
  ...
  if (is_success) {
    std::shared_ptr<PyramidMapMatrix> map_matrix =
        std::dynamic_pointer_cast<PyramidMapMatrix>(map_matrix_);
    const float* intensity_var_ptr = map_matrix->GetIntensityVarSafe(y, x, level);

    if (intensity_var_ptr != nullptr) { intensity_var = *intensity_var_ptr; }
  }

  return intensity_var;
}

float PyramidMapNode::GetAltitudeSafe(const Eigen::Vector3d& coordinate, unsigned int level) const {
  Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
  unsigned int    x          = 0;
  unsigned int    y          = 0;
  bool            is_success = GetCoordinate(coord2d, level, &x, &y);
  ...
  if (is_success) {
    ...
    if (altitude_ptr != nullptr) { altitude = *altitude_ptr; }
  }
...
float PyramidMapNode::GetAltitudeVarSafe(const Eigen::Vector3d& coordinate,
  ...
  if (is_success) {
    ...
    if (altitude_var_ptr != nullptr) { altitude_var = *altitude_var_ptr; }
  }
...
float PyramidMapNode::GetGroundAltitudeSafe(const Eigen::Vector3d& coordinate,
  ...
  if (is_success) {
    std::shared_ptr<PyramidMapMatrix> map_matrix =
        std::dynamic_pointer_cast<PyramidMapMatrix>(map_matrix_);
    const float* ground_altitude_ptr = map_matrix->GetGroundAltitudeSafe(y, x, level);

    if (ground_altitude_ptr != nullptr) { ground_altitude = *ground_altitude_ptr; }
  }
...
unsigned int PyramidMapNode::GetCountSafe(const Eigen::Vector3d& coordinate,
  ...
  if (is_success) {
    ...
    if (count_ptr != nullptr) { count = *count_ptr; }
  }

  return count;
}

unsigned int PyramidMapNode::GetGroundCountSafe(const Eigen::Vector3d& coordinate,
                                                unsigned int           level) const {
  Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
  unsigned int    x          = 0;
  unsigned int    y          = 0;
  ...
  if (is_success) {
    std::shared_ptr<PyramidMapMatrix> map_matrix =
        std::dynamic_pointer_cast<PyramidMapMatrix>(map_matrix_);
    const unsigned int* ground_count_ptr = map_matrix->GetGroundCountSafe(y, x, level);

    if (ground_count_ptr != nullptr) { ground_count = *ground_count_ptr; }
  }

  return ground_count;
}

float PyramidMapNode::GetIntensity(const Eigen::Vector3d& coordinate, unsigned int level) const {
  Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
  unsigned int    x          = 0;
  unsigned int    y          = 0;
  bool            is_success = GetCoordinate(coord2d, level, &x, &y);
...
float PyramidMapNode::GetIntensityVar(const Eigen::Vector3d& coordinate, unsigned int level) const {
  Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
  unsigned int    x          = 0;
  unsigned int    y          = 0;
  bool            is_success = GetCoordinate(coord2d, level, &x, &y);
...
float PyramidMapNode::GetAltitude(const Eigen::Vector3d& coordinate, unsigned int level) const {
  Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
  unsigned int    x          = 0;
  unsigned int    y          = 0;
  bool            is_success = GetCoordinate(coord2d, level, &x, &y);
...
float PyramidMapNode::GetAltitudeVar(const Eigen::Vector3d& coordinate, unsigned int level) const {
  Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
  unsigned int    x          = 0;
  unsigned int    y          = 0;
  bool            is_success = GetCoordinate(coord2d, level, &x, &y);
...
float PyramidMapNode::GetGroundAltitude(const Eigen::Vector3d& coordinate,
  ...
  if (is_success) {
    std::shared_ptr<PyramidMapMatrix> map_matrix =
        std::dynamic_pointer_cast<PyramidMapMatrix>(map_matrix_);
    const float* ground_altitude_ptr = map_matrix->GetGroundAltitude(y, x, level);
...
unsigned int PyramidMapNode::GetCount(const Eigen::Vector3d& coordinate, unsigned int level) const {
  Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
  unsigned int    x          = 0;
  unsigned int    y          = 0;
  bool            is_success = GetCoordinate(coord2d, level, &x, &y);
...
unsigned int PyramidMapNode::GetGroundCount(const Eigen::Vector3d& coordinate,
  ...
  if (is_success) {
    std::shared_ptr<PyramidMapMatrix> map_matrix =
        std::dynamic_pointer_cast<PyramidMapMatrix>(map_matrix_);
    const unsigned int* ground_count_ptr = map_matrix->GetGroundCount(y, x, level);
