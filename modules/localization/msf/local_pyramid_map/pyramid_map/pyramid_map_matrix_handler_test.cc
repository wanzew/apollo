...
TEST_F(PyramidMapMatrixHandlerTestSuite, LossyMapFullAltMatrixHandler) {
  ...
  BaseMapMatrixHandler* handler =
      PyramidMapMatrixHandlerSelector::AllocPyramidMapMatrixHandler(MapVersion::LOSSY_FULL_ALT_MAP);
  ...
  EXPECT_EQ(handler->CreateBinary(pm_matrix, buf.get(), expect_size), expect_size);
...
TEST_F(PyramidMapMatrixHandlerTestSuite, LosslessMapMatrixHandler) {
  // create handler
  BaseMapMatrixHandler* handler =
      PyramidMapMatrixHandlerSelector::AllocPyramidMapMatrixHandler(MapVersion::LOSSLESS_MAP);
  ...
  expect_size = 104;
  EXPECT_EQ(handler->GetBinarySize(pm_matrix), expect_size);
  EXPECT_EQ(handler->CreateBinary(pm_matrix, ground_buf.get(), expect_size - 1), 0);
  EXPECT_EQ(handler->CreateBinary(pm_matrix, ground_buf.get(), expect_size), expect_size);
...
TEST_F(PyramidMapMatrixHandlerTestSuite, PyramidLossyMapMatrixHandler) {
  // create handler
  BaseMapMatrixHandler* handler =
      PyramidMapMatrixHandlerSelector::AllocPyramidMapMatrixHandler(MapVersion::PYRAMID_LOSSY_MAP);

  // create pyramid map matrix
  std::unique_ptr<PyramidMapConfig> config(new PyramidMapConfig("pyramid_lossy_map"));
  config->SetMapNodeSize(2, 2);
  config->resolution_num_ = 1;
  std::shared_ptr<PyramidMapMatrix> pm_matrix(new PyramidMapMatrix());
  pm_matrix->Init(*config);
...
TEST_F(PyramidMapMatrixHandlerTestSuite, PyramidLosslessMapMatrixHandler) {
  // create handler
  BaseMapMatrixHandler* handler = PyramidMapMatrixHandlerSelector::AllocPyramidMapMatrixHandler(
      MapVersion::PYRAMID_LOSSLESS_MAP);

  // create pyramid map matrix
  std::unique_ptr<PyramidMapConfig> config(new PyramidMapConfig("pyramid_lossless_map"));
  config->SetMapNodeSize(2, 2);
  config->resolution_num_ = 1;
  std::shared_ptr<PyramidMapMatrix> pm_matrix(new PyramidMapMatrix());
  pm_matrix->Init(*config);
