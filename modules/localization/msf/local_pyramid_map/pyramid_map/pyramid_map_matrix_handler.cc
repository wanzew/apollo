...
PyramidMapMatrixHandlerSelector::AllocPyramidMapMatrixHandler(MapVersion version) {
  switch (version) {
    case MapVersion::LOSSY_FULL_ALT_MAP: return new LossyMapFullAltMatrixHandler();
    case MapVersion::LOSSLESS_MAP: return new LosslessMapMatrixHandler();
    case MapVersion::PYRAMID_LOSSY_MAP: return new PyramidLossyMapMatrixHandler();
    case MapVersion::PYRAMID_LOSSLESS_MAP: return new PyramidLosslessMapMatrixHandler();
    case MapVersion::UNKNOWN:
    default: AINFO << "Unknown map version!";
  }
  return nullptr;
}
...
void LossyMapMatrixHandler::DecodeIntensity(unsigned char data, float* intensity) const {
  *intensity = static_cast<float>(data);
}

uint16_t LossyMapMatrixHandler::EncodeIntensityVar(float var) const {
  var                      = std::sqrt(var);
  unsigned int encoded_var = static_cast<unsigned int>(
      static_cast<float>(var_range_) / (var * static_cast<float>(var_ratio_) + 1.0));
  if (encoded_var > var_range_) { encoded_var = var_range_; }
  if (encoded_var < 1) { encoded_var = 1; }
  return static_cast<uint16_t>(encoded_var);
}

void LossyMapMatrixHandler::DecodeIntensityVar(uint16_t data, float* var) const {
  *var = static_cast<float>(data);
  *var = (static_cast<float>(var_range_) / (*var) - 1.0f) / static_cast<float>(var_ratio_);
  *var = (*var) * (*var);
}
...
uint16_t LossyMapMatrixHandler::EncodeAltitude(float altitude,
  ...
  if (encoded_altitude > 0xffff) { encoded_altitude = 0xffff; }
  if (encoded_altitude < 0) { encoded_altitude = 0; }
  return static_cast<uint16_t>(encoded_altitude);
}

void LossyMapMatrixHandler::DecodeAltitude(uint16_t data,
                                           float    min_altitude,
...
unsigned char LossyMapMatrixHandler::EncodeCount(unsigned int count,
  ...
  if (encoded_count > count_range) { encoded_count = count_range; }
  return static_cast<unsigned char>(encoded_count);
}

void LossyMapMatrixHandler::DecodeCount(unsigned char data, unsigned int* count) const {
  if (data == 0) {
    *count = data;
  } else {
    *count = 1 << (data - 1);
...
size_t LossyMapFullAltMatrixHandler::LoadBinary(const unsigned char*           buf,
  ...
  if (  // matrix->get_resolution_num() == 1 &&
      ...
      rows == matrix->GetRows() && cols == matrix->GetCols() && matrix->HasIntensity() &&
      matrix->HasIntensityVar() && matrix->HasAltitude() && matrix->HasGroundAltitude() &&
      matrix->HasCount()) {
    matrix->Reset();
  } else {
  ...
  const float* float_p = reinterpret_cast<const float*>(reinterpret_cast<const void*>(uint_p));
  ...
  if (matrix->HasIntensityVar()) {
    ...
    for (unsigned int r = 0; r < rows; ++r) {
      for (unsigned int c = 0; c < cols; ++c) {
        ...
        DecodeIntensityVar(static_cast<uint16_t>(var), &(*intensity_var_matrix)[r][c]);
      }
    }
    uc_p += 2 * matrix_size;
  }
  ...
  if (matrix->HasAltitude()) {
    ...
    for (unsigned int r = 0; r < rows; ++r) {
      for (unsigned int c = 0; c < cols; ++c) {
        ...
        DecodeAltitude(static_cast<uint16_t>(alt), alt_avg_min_, alt_avg_interval_,
                       &(*altitude_matrix)[r][c]);
      }
    }
    uc_p += 2 * matrix_size;
  ...
  if (matrix->HasGroundAltitude()) {
    ...
    for (unsigned int r = 0; r < rows; ++r) {
      for (unsigned int c = 0; c < cols; ++c) {
        unsigned int alt = static_cast<unsigned int>(p_high[r * cols + c]);
        alt              = alt * 256u + static_cast<unsigned int>(p_low[r * cols + c]);
        DecodeAltitude(static_cast<uint16_t>(alt), ground_alt_min_, ground_alt_interval_,
...
size_t LossyMapFullAltMatrixHandler::CreateBinary(const std::shared_ptr<BaseMapMatrix> base_matrix,
                                                  unsigned char*                       buf,
                                                  size_t                               buf_size) {
  ...
  if (buf_size >= target_size) {
    ...
    if (matrix->HasAltitude() && matrix->HasCount()) {
      ...
      for (unsigned int y = 0; y < rows; ++y) {
        for (unsigned int x = 0; x < cols; ++x) {
          ...
          if (*altitude < alt_avg_min_) { alt_avg_min_ = *altitude; }
          if (*altitude > alt_avg_max_) { alt_avg_max_ = *altitude; }
        }
      }
    } else {
      alt_avg_min_ = 0.0;
    ...
    if (matrix->HasGroundAltitude() && matrix->HasGroundCount()) {
      ...
      for (unsigned int y = 0; y < rows; ++y) {
        for (unsigned int x = 0; x < cols; ++x) {
          ...
          if (*ground_count == 0) { continue; }
          if (*ground_altitude < ground_alt_min_) { ground_alt_min_ = *ground_altitude; }
          if (*ground_altitude > ground_alt_max_) { ground_alt_max_ = *ground_altitude; }
        }
      }
    } else {
      ground_alt_min_ = 0.0;
    ...
    *float_p = ground_alt_max_;
    ...
    for (unsigned int r = 0; r < rows; ++r) {
      for (unsigned int c = 0; c < cols; ++c) {
        const float* intensity = static_cast<const float*>(matrix->GetIntensitySafe(r, c));
    ...
    for (unsigned int r = 0; r < rows; ++r) {
      for (unsigned int c = 0; c < cols; ++c) {
        const float* intensity_var = static_cast<const float*>(matrix->GetIntensityVarSafe(r, c));
    ...
    for (unsigned int r = 0; r < rows; ++r) {
      for (unsigned int c = 0; c < cols; ++c) {
        ...
        uint16_t     alt             = EncodeAltitude(ga, ground_alt_min_, ground_alt_interval_);
        p_high[r * cols + c]         = static_cast<unsigned char>(alt / 256);
        p_low[r * cols + c]          = static_cast<unsigned char>(alt % 256);
      }
    }
...
size_t
LossyMapFullAltMatrixHandler::GetBinarySize(const std::shared_ptr<BaseMapMatrix> base_matrix) {
  const std::shared_ptr<PyramidMapMatrix> matrix =
      std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
...
size_t LosslessMapMatrixHandler::LoadBinary(const unsigned char*           buf,
  ...
  if (  // matrix->get_resolution_num() == 1 &&
        // matrix->get_resolution_ratio() == 2 &&
      rows == matrix->GetRowsSafe() && cols == matrix->GetColsSafe() && matrix->HasIntensity() &&
      matrix->HasIntensityVar() && matrix->HasAltitude() && matrix->HasAltitudeVar() &&
      matrix->HasGroundAltitude() && matrix->HasCount() && matrix->HasGroundCount()) {
    matrix->Reset();
  } else {
    ...
    for (unsigned int x = 0; x < cols; x++) {
      ...
      for (unsigned int i = 0; i < size; i++) {
        if (i == 0) {  // all points layer
          ...
          ++float_p;
          uint_p = reinterpret_cast<const unsigned int*>(reinterpret_cast<const void*>(float_p));
        ...
        for (unsigned int i = 0; i < layer_num; i++) {
          if (i == 0) {  // all points layer
            ...
            *float_p = (altitude_var != nullptr) ? *altitude_var : 0.0f;
            ++float_p;
            uint_p  = reinterpret_cast<unsigned int*>(reinterpret_cast<void*>(float_p));
...
size_t LosslessMapMatrixHandler::GetBinarySize(const std::shared_ptr<BaseMapMatrix> base_matrix) {
  const std::shared_ptr<PyramidMapMatrix> matrix =
      std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
  // assert(matrix->get_resolution_num() > 0);
  if (matrix->GetResolutionNum() == 0) {
...
size_t PyramidLossyMapMatrixHandler::LoadBinary(const unsigned char*           buf,
  ...
  const unsigned int* uint_p      = reinterpret_cast<const unsigned int*>(buf);  // resolution num
  ...
  // reset or init matrix
  if (resolution_num == matrix->GetResolutionNum() && ratio == matrix->GetResolutionRatio() &&
      rows == matrix->GetRowsSafe() && cols == matrix->GetColsSafe() &&
      has_intensity == matrix->HasIntensity() && has_intensity_var == matrix->HasIntensityVar() &&
      has_altitude == matrix->HasAltitude() && has_altitude_var == matrix->HasAltitudeVar() &&
      has_ground_altitude == matrix->HasGroundAltitude() && has_count == matrix->HasCount() &&
      has_ground_count == matrix->HasGroundCount()) {
    matrix->Reset();
  } else {
    matrix->Init(rows, cols, has_intensity, has_intensity_var, has_altitude, has_altitude_var,
                 has_ground_altitude, has_count, has_ground_count, resolution_num, ratio);
  }

  // alt min & ground alt min
  binary_size += sizeof(float) * 2;
  const float* float_p = reinterpret_cast<const float*>(reinterpret_cast<const void*>(uc_p));
  alt_avg_min_         = *float_p;
  ++float_p;
  ground_alt_min_ = *float_p;
  ++float_p;
  ...
  for (unsigned int l = 0; l < resolution_num; ++l) {
    ...
    if (matrix->HasAltitude()) {
      ...
      for (unsigned int r = 0; r < rows; ++r) {
        for (unsigned int c = 0; c < cols; ++c) {
          ...
          DecodeAltitude(alt, alt_avg_min_, alt_avg_interval_, &(*altitude_matrix)[r][c]);
        }
      }
      uc_p += 2 * matrix_size;
    }
    ...
    if (matrix->HasGroundAltitude()) {
      binary_size += sizeof(uint16_t) * matrix_size;
      FloatMatrix*         ground_altitude_matrix = matrix->GetGroundAltitudeMatrixSafe(l);
      const unsigned char* p_low                  = uc_p + matrix_size;
      const unsigned char* p_high                 = uc_p;
      for (unsigned int r = 0; r < rows; ++r) {
        for (unsigned int c = 0; c < cols; ++c) {
...
size_t PyramidLossyMapMatrixHandler::CreateBinary(const std::shared_ptr<BaseMapMatrix> base_matrix,
                                                  unsigned char*                       buf,
                                                  size_t                               buf_size) {
  const std::shared_ptr<PyramidMapMatrix> matrix =
      std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
  ...
  if (buf_size >= target_size) {
    ...
    unsigned int* uint_p = reinterpret_cast<unsigned int*>(buf);  // resolution num
    ...
    *uc_p = 0;
    ++uc_p;
    *uc_p = 0;
    if (matrix->HasIntensity()) { *uc_p |= 1; }
    if (matrix->HasIntensityVar()) { *uc_p |= 2; }
    if (matrix->HasAltitude()) { *uc_p |= 4; }
    if (matrix->HasAltitudeVar()) { *uc_p |= 8; }
    if (matrix->HasGroundAltitude()) { *uc_p |= 16; }
    if (matrix->HasCount()) { *uc_p |= 32; }
    if (matrix->HasGroundCount()) { *uc_p |= 64; }
    ++uc_p;
    ...
    *float_p = ground_alt_min_;
    ...
    for (unsigned int l = 0; l < resolution_num; ++l) {
      ...
      if (matrix->HasGroundCount()) {
        for (unsigned int r = 0; r < rows; ++r) {
          for (unsigned int c = 0; c < cols; ++c) {
            const unsigned int* ground_count = matrix->GetGroundCountSafe(r, c, l);
      ...
      if (matrix->HasAltitude()) {
        ...
        for (unsigned int r = 0; r < rows; ++r) {
          for (unsigned int c = 0; c < cols; ++c) {
            ...
            uint16_t     alt      = EncodeAltitude(*altitude, alt_avg_min_, alt_avg_interval_);
            p_high[r * cols + c]  = static_cast<unsigned char>(alt / 256);
            p_low[r * cols + c]   = static_cast<unsigned char>(alt % 256);
          }
        }
      ...
      if (matrix->HasGroundAltitude()) {
        ...
        for (unsigned int r = 0; r < rows; ++r) {
          for (unsigned int c = 0; c < cols; ++c) {
            const float* ground_altitude = matrix->GetGroundAltitudeSafe(r, c, l);
            uint16_t alt = EncodeAltitude(*ground_altitude, ground_alt_min_, ground_alt_interval_);
            p_high[r * cols + c] = static_cast<unsigned char>(alt / 256);
            p_low[r * cols + c]  = static_cast<unsigned char>(alt % 256);
          }
        }
...
size_t
PyramidLossyMapMatrixHandler::GetBinarySize(const std::shared_ptr<BaseMapMatrix> base_matrix) {
  const std::shared_ptr<PyramidMapMatrix> matrix =
      std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
  ...
  for (unsigned int l = 0; l < resolution_num; ++l) {
    unsigned int matrix_size = matrix->GetRowsSafe(l) * matrix->GetColsSafe(l);
    if (matrix->HasCount()) { target_size += sizeof(unsigned char) * matrix_size; }
    if (matrix->HasGroundCount()) { target_size += sizeof(unsigned char) * matrix_size; }
    if (matrix->HasIntensity()) { target_size += sizeof(unsigned char) * matrix_size; }
    if (matrix->HasIntensityVar()) { target_size += sizeof(uint16_t) * matrix_size; }
    if (matrix->HasAltitude()) { target_size += sizeof(uint16_t) * matrix_size; }
    if (matrix->HasAltitudeVar()) { target_size += sizeof(uint16_t) * matrix_size; }
    if (matrix->HasGroundAltitude()) { target_size += sizeof(uint16_t) * matrix_size; }
  }
...
size_t PyramidLosslessMapMatrixHandler::LoadBinary(const unsigned char*           buf,
  ...
  for (unsigned int l = 0; l < resolution_num; ++l) {
    ...
    float_p = reinterpret_cast<const float*>(reinterpret_cast<const void*>(uint_p));
  }
  ...
  if (buf_size >= target_size) {
    ...
    *uc_p = 0;
    ...
    for (unsigned int l = 0; l < resolution_num; ++l) {
      ...
      if (matrix->HasIntensityVar()) {
        const FloatMatrix* intensity_var_matrix = matrix->GetIntensityVarMatrixSafe(l);
        memcpy(float_p, (*intensity_var_matrix)[0], matrix_size * sizeof(float));
        float_p += matrix_size;
        // processed_size += matrix_size * sizeof(float);
      }
      if (matrix->HasAltitude()) {
      ...
      if (matrix->HasAltitudeVar()) {
        const FloatMatrix* altitude_var_matrix = matrix->GetAltitudeVarMatrixSafe(l);
        memcpy(float_p, (*altitude_var_matrix)[0], matrix_size * sizeof(float));
        float_p += matrix_size;
        // processed_size += matrix_size * sizeof(float);
      }
      if (matrix->HasGroundAltitude()) {
        const FloatMatrix* ground_altitude_matrix = matrix->GetGroundAltitudeMatrixSafe(l);
        memcpy(float_p, (*ground_altitude_matrix)[0], matrix_size * sizeof(float));
        float_p += matrix_size;
        // processed_size += matrix_size * sizeof(float);
      }
      ...
      if (matrix->HasGroundCount()) {
        const UIntMatrix* ground_count_matrix = matrix->GetGroundCountMatrixSafe(l);
        memcpy(uint_p, (*ground_count_matrix)[0], matrix_size * sizeof(unsigned int));
        uint_p += matrix_size;
        // processed_size += matrix_size * sizeof(unsigned int);
      }
...
size_t
PyramidLosslessMapMatrixHandler::GetBinarySize(const std::shared_ptr<BaseMapMatrix> base_matrix) {
  const std::shared_ptr<PyramidMapMatrix> matrix =
      std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
  ...
  // space for has_*
  size_t target_size =
      sizeof(unsigned int) * 2 + sizeof(unsigned int) * 2 + sizeof(unsigned char) * 4;

  for (unsigned int l = 0; l < resolution_num; ++l) {
    unsigned int matrix_size = matrix->GetRowsSafe(l) * matrix->GetColsSafe(l);
    if (matrix->HasIntensity()) { target_size += sizeof(float) * matrix_size; }
    if (matrix->HasIntensityVar()) { target_size += sizeof(float) * matrix_size; }
    if (matrix->HasAltitude()) { target_size += sizeof(float) * matrix_size; }
    if (matrix->HasAltitudeVar()) { target_size += sizeof(float) * matrix_size; }
    if (matrix->HasGroundAltitude()) { target_size += sizeof(float) * matrix_size; }
    if (matrix->HasCount()) { target_size += sizeof(unsigned int) * matrix_size; }
    if (matrix->HasGroundCount()) { target_size += sizeof(unsigned int) * matrix_size; }
  }
