...
bool OfflineLocalVisualizer::Init(const std::string& map_folder,
  ...
  map_param.set(map_config_.map_resolutions_, map_config_.map_node_size_x_,
                ...
                map_config_.map_range_.GetMinY(), map_config_.map_range_.GetMaxX(),
                map_config_.map_range_.GetMaxY());
  success = visual_engine_.Init(map_folder_, map_visual_folder_, map_param, resolution_id_,
  ...
  for (unsigned int idx = 0; idx < pcd_timestamps_.size(); ++idx) {
    ...
    if (pose_found_iter != lidar_poses_.end() && std_found_iter != lidar_stds_.end()) {
      ...
      lidar_loc_info.set(Eigen::Translation3d(lidar_pose.translation()),
                         Eigen::Quaterniond(lidar_pose.linear()), lidar_std, "Lidar.",
    ...
    if (pose_found_iter != gnss_poses_.end() && std_found_iter != gnss_stds_.end()) {
      ...
      gnss_loc_info.set(Eigen::Translation3d(gnss_pose.translation()), gnss_std, "GNSS.",
                        pcd_timestamps_[idx], idx + 1);
    }
    ...
    if (pose_found_iter != fusion_poses_.end() && std_found_iter != fusion_stds_.end()) {
      ...
      fusion_loc_info.set(Eigen::Translation3d(fusion_pose.translation()),
                          Eigen::Quaterniond(fusion_pose.linear()), fusion_std, "Fusion.",
                          pcd_timestamps_[idx], idx + 1);
    }
    ...
    apollo::localization::msf::velodyne::LoadPcds(pcd_file_path, idx, lidar_loc_info.pose, &pt3ds,
                                                  &intensities, false);

    ::apollo::common::EigenVector<LocalizatonInfo> loc_infos{lidar_loc_info, gnss_loc_info,
                                                             fusion_loc_info};
...
bool OfflineLocalVisualizer::LidarLocFileHandler(const std::vector<double>& pcd_timestamps) {
  ...
  PoseAndStdInterpolationByTime(poses, stds, timestamps, pcd_timestamps, &lidar_poses_,
                                &lidar_stds_);
...
bool OfflineLocalVisualizer::GnssLocFileHandler(const std::vector<double>& pcd_timestamps) {
  ...
  PoseAndStdInterpolationByTime(poses, stds, timestamps, pcd_timestamps, &gnss_poses_, &gnss_stds_);
...
bool OfflineLocalVisualizer::FusionLocFileHandler(const std::vector<double>& pcd_timestamps) {
  ...
  PoseAndStdInterpolationByTime(poses, stds, timestamps, pcd_timestamps, &fusion_poses_,
                                &fusion_stds_);
  return true;
}
...
bool OfflineLocalVisualizer::GetZoneIdFromMapFolder(const std::string& map_folder,
  ...
  if (iter_north == directory_end) {
    ...
    if (iter_south == directory_end) { return false; }
