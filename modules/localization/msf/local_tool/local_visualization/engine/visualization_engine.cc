...
bool MapImageKey::operator<(const MapImageKey& key) const {
  ...
  if (zone_id != key.zone_id) { return zone_id < key.zone_id; }
  if (node_north_id != key.node_north_id) { return node_north_id < key.node_north_id; }
...
void VisualizationEngine::Draw() {
  ...
  cv::resize(big_window_(cv::Rect(left_top_x, left_top_y, width, width)), image_window_,
             cv::Size(1024, 1024), 0, 0, CV_INTER_LINEAR);
    ...
    for (unsigned int k = 0; k < loc_info_num_; k++) {
      ...
      if (lt.x >= 0 && lt.y >= 0 && lt.x < 1024 * 3 && lt.y < 1024 * 3) {
        if (is_draw_car_ && loc_info.is_has_attitude) {
          ...
          cv::Mat mat_tem;
          cv::resize(car_img_mats_[i], mat_tem, cv::Size(48, 24), 0, 0, CV_INTER_LINEAR);
          cv::Mat rotated_mat;
          // AINFO << "yaw: " << yaw;
    ...
    for (unsigned int k = 0; k < loc_info_num_; k++) {
      ...
      if (loc_info.is_has_std) {
        ...
        if (lt.x >= 0 && lt.y >= 0 && lt.x < 1024 * 3 && lt.y < 1024 * 3) {
          ...
          cv::Size size(std::sqrt(std[0]) * 200 + 1.0, std::sqrt(std[1]) * 200 + 1.0);
...
void VisualizationEngine::CloudToMat(const Eigen::Affine3d&              cur_pose,
  ...
  cloud_img_lt_coord_[0] = cen[0] - map_param_.map_resolutions[resolution_id_] * (img_width / 2.0f);
...
void VisualizationEngine::CoordToImageKey(const Eigen::Vector2d& coord, MapImageKey* key) {
  ...
  int n =
      static_cast<int>((coord[0] - map_param_.map_min_x) /
                       (map_param_.map_node_size_x * map_param_.map_resolutions[resolution_id_]));
  int m =
      static_cast<int>((coord[1] - map_param_.map_min_y) /
                       (map_param_.map_node_size_y * map_param_.map_resolutions[resolution_id_]));
  int max_n =
      static_cast<int>((map_param_.map_max_x - map_param_.map_min_x) /
                       (map_param_.map_node_size_x * map_param_.map_resolutions[resolution_id_]));
