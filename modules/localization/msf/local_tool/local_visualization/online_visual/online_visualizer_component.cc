...
bool OnlineVisualizerComponent::InitIO() {
  // Lidar localization
  std::function<void(const std::shared_ptr<LocalizationEstimate>&)> lidar_local_call =
      std::bind(&OnlineVisualizerComponent::OnLidarLocalization, this, std::placeholders::_1);
  lidar_local_listener_ =
      this->node_->CreateReader<LocalizationEstimate>(lidar_local_topic_, lidar_local_call);

  // GNSS localization
  std::function<void(const std::shared_ptr<LocalizationEstimate>&)> gnss_local_call =
      std::bind(&OnlineVisualizerComponent::OnGNSSLocalization, this, std::placeholders::_1);
  gnss_local_listener_ =
      this->node_->CreateReader<LocalizationEstimate>(gnss_local_topic_, gnss_local_call);

  // Fusion localization
  std::function<void(const std::shared_ptr<LocalizationEstimate>&)> fusion_local_call =
      std::bind(&OnlineVisualizerComponent::OnFusionLocalization, this, std::placeholders::_1);
  fusion_local_listener_ =
      this->node_->CreateReader<LocalizationEstimate>(fusion_local_topic_, fusion_local_call);

  // Ndt Localization
  std::function<void(const std::shared_ptr<LocalizationEstimate>&)> ndt_local_call =
      std::bind(&OnlineVisualizerComponent::OnLidarLocalization, this, std::placeholders::_1);
  lidar_local_listener_ =
      this->node_->CreateReader<LocalizationEstimate>(ndt_local_topic_, ndt_local_call);

  return true;
}

bool OnlineVisualizerComponent::Proc(const std::shared_ptr<drivers::PointCloud>& msg) {
...
void OnlineVisualizerComponent::OnLidarLocalization(
  ...
  if (msg->has_uncertainty() && !std::isnan(msg->uncertainty().position_std_dev().x()) &&
