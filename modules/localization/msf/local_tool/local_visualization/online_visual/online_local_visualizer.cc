...
std::string OnlineLocalVisualizer::Name() const { return "localization_visualization"; }
...
Status OnlineLocalVisualizer::Start() {
  ...
  AdapterManager::AddPointCloudCallback(&OnlineLocalVisualizer::OnPointCloud, this);

  // Lidar localization
  if (!AdapterManager::GetLocalizationMsfLidar()) {
    buffer.ERROR("LocalizationMsfLidar input not initialized. Check your adapter.conf "
                 "file!");
    buffer.PrintLog();
    return Status(common::LOCALIZATION_ERROR_MSG, "no LocalizationMsfLidar adapter");
  }
  AdapterManager::AddLocalizationMsfLidarCallback(&OnlineLocalVisualizer::OnLidarLocalization,
                                                  this);

  // GNSS localization
  if (!AdapterManager::GetLocalizationMsfGnss()) {
    buffer.ERROR("LocalizationMsfGnss input not initialized. Check your adapter.conf "
                 "file!");
    buffer.PrintLog();
    return Status(common::LOCALIZATION_ERROR_MSG, "no LocalizationMsfGnss adapter");
  }
  AdapterManager::AddLocalizationMsfGnssCallback(&OnlineLocalVisualizer::OnGNSSLocalization, this);

  // Fusion localization
  if (!AdapterManager::GetLocalization()) {
    buffer.ERROR("Localization input not initialized. Check your adapter.conf file!");
    buffer.PrintLog();
    return Status(common::LOCALIZATION_ERROR_MSG, "no Localization adapter");
  }
  AdapterManager::AddLocalizationCallback(&OnlineLocalVisualizer::OnFusionLocalization, this);
...
void OnlineLocalVisualizer::Stop() { VisualizationManager::GetInstance().StopVisualization(); }

Status OnlineLocalVisualizer::Init() {
  InitParams();

  Eigen::Affine3d velodyne_extrinsic;
  bool            success = velodyne::LoadExtrinsic(lidar_extrinsic_file_, &velodyne_extrinsic);
  if (!success) {
  ...
  map_param.set(map_config.map_resolutions_, map_config.map_node_size_x_,
                ...
                map_config.map_range_.GetMinY(), map_config.map_range_.GetMaxX(),
                map_config.map_range_.GetMaxY());
  success = VisualizationManager::GetInstance().Init(map_folder_, map_visual_folder_,
                                                     velodyne_extrinsic, map_param);
  if (!success) { return Status(common::LOCALIZATION_ERROR, "visualization init failed"); }
...
void OnlineLocalVisualizer::OnPointCloud(const sensor_msgs::PointCloud2& message) {
...
void OnlineLocalVisualizer::OnLidarLocalization(const LocalizationEstimate& message) {
  ...
  if (message.has_uncertainty() && !std::isnan(message.uncertainty().position_std_dev().x()) &&
...
void OnlineLocalVisualizer::OnGNSSLocalization(const LocalizationEstimate& message) {
...
void OnlineLocalVisualizer::OnFusionLocalization(const LocalizationEstimate& message) {
...
void OnlineLocalVisualizer::ParsePointCloudMessage(const sensor_msgs::PointCloud2& message,
                                                   std::vector<Eigen::Vector3d>*   pt3ds,
                                                   std::vector<unsigned char>*     intensities) {
  pcl::PCLPointCloud2 pcl_cloud;
  pcl_conversions::toPCL(message, pcl_cloud);

  pcl::PointCloud<velodyne::PointXYZIT>::Ptr cloud(new pcl::PointCloud<velodyne::PointXYZIT>);
