...
int main(int argc, char** argv) {
  ...
  boost_desc.add_options()("help", "produce help message")(
      ...
      "provide the bag file")("out_folder", boost::program_options::value<std::string>(),
                              "provide the output folder")(
      "cloud_topic",
      boost::program_options::value<std::string>()->default_value(
          "/apollo/sensor/velodyne64/compensator/PointCloud2"),
      "provide point cloud2 topic")("gnss_loc_topic",
                                    boost::program_options::value<std::string>()->default_value(
      ...
      "fusion_loc_topic",
      boost::program_options::value<std::string>()->default_value("/apollo/localization/pose"),
      "provide fusion localization topic")(
      "odometry_loc_topic",
      boost::program_options::value<std::string>()->default_value("/apollo/sensor/gnss/odometry"),
  ...
  const std::vector<std::string> bag_files = boost_args["bag_files"].as<std::vector<std::string>>();
  const std::string              pcd_folder = boost_args["out_folder"].as<std::string>() + "/pcd";
  if (!boost::filesystem::exists(pcd_folder)) { boost::filesystem::create_directory(pcd_folder); }

  const std::string cloud_topic        = boost_args["cloud_topic"].as<std::string>();
  const std::string gnss_loc_topic     = boost_args["gnss_loc_topic"].as<std::string>();
  const std::string lidar_loc_topic    = boost_args["lidar_loc_topic"].as<std::string>();
  const std::string fusion_loc_topic   = boost_args["fusion_loc_topic"].as<std::string>();
  const std::string odometry_loc_topic = boost_args["odometry_loc_topic"].as<std::string>();

  std::unique_ptr<PCDExporter>      pcd_exporter(new PCDExporter(pcd_folder));
  std::unique_ptr<LocationExporter> loc_exporter(new LocationExporter(pcd_folder));

  CyberRecordReader reader;
  reader.Subscribe(cloud_topic,
                   [&](const std::string& msg) { pcd_exporter->CompensatedPcdCallback(msg); });

  reader.Subscribe(gnss_loc_topic,
                   [&](const std::string& msg) { loc_exporter->GnssLocCallback(msg); });

  reader.Subscribe(lidar_loc_topic,
                   [&](const std::string& msg) { loc_exporter->LidarLocCallback(msg); });

  reader.Subscribe(fusion_loc_topic,
                   [&](const std::string& msg) { loc_exporter->FusionLocCallback(msg); });

  reader.Subscribe(odometry_loc_topic,
                   [&](const std::string& msg) { loc_exporter->OdometryLocCallback(msg); });
