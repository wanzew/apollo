...
static bool LoadGnssAntennaExtrinsic(const std::string& file_path,
  ...
  if (config["leverarm"]) {
    if (config["leverarm"]["primary"]["offset"]) {
      (*imu_ant_offset)[0] = config["leverarm"]["primary"]["offset"]["x"].as<double>();
      (*imu_ant_offset)[1] = config["leverarm"]["primary"]["offset"]["y"].as<double>();
      (*imu_ant_offset)[2] = config["leverarm"]["primary"]["offset"]["z"].as<double>();
    }
    return true;
  }
  return false;
...
static void PoseAndStdInterpolationByTime(const EigenAffine3dVec&                  in_poses,
...
int main(int argc, char** argv) {
  ...
  boost_desc.add_options()("help", "produce help message")(
      ...
      "loc_file_a", boost::program_options::value<std::string>(),
      "provide gnss localization file.")("loc_file_b", boost::program_options::value<std::string>(),
                                         "provide lidar localization file.")(
      "compare_file", boost::program_options::value<std::string>(), "provide compare file.")(
      "imu_to_ant_offset_file", boost::program_options::value<std::string>()->default_value(""),
  ...
  const std::string loc_file_a   = in_folder + "/" + boost_args["loc_file_a"].as<std::string>();
  const std::string loc_file_b   = in_folder + "/" + boost_args["loc_file_b"].as<std::string>();
  const std::string compare_file = in_folder + "/" + boost_args["compare_file"].as<std::string>();
  const std::string imu_to_ant_offset_file = boost_args["imu_to_ant_offset_file"].as<std::string>();

  Eigen::Vector3d imu_ant_offset = Eigen::Vector3d::Zero();
  if (imu_to_ant_offset_file != "") {
    if (!LoadGnssAntennaExtrinsic(imu_to_ant_offset_file, &imu_ant_offset)) { return 0; }
  }

  EigenAffine3dVec    poses_a;
  EigenVector3dVec    stds_a;
  std::vector<double> timestamps_a;
  apollo::localization::msf::velodyne::LoadPosesAndStds(loc_file_a, &poses_a, &stds_a,
                                                        &timestamps_a);
  if (poses_a.empty()) { return 0; }

  EigenAffine3dVec    poses_b;
  EigenVector3dVec    stds_b;
  std::vector<double> timestamps_b;
  apollo::localization::msf::velodyne::LoadPosesAndStds(loc_file_b, &poses_b, &stds_b,
                                                        &timestamps_b);
  if (poses_b.empty()) { return 0; }
  ...
  PoseAndStdInterpolationByTime(poses_b, stds_b, timestamps_b, timestamps_a, &out_poses_b,
                                &out_stds_b);

  PoseAndStdInterpolationByTime(poses_a, stds_a, timestamps_a, timestamps_a, &out_poses_a,
                                &out_stds_a);

  if (out_poses_a.empty() || out_poses_b.empty()) { return 0; }
  ...
  for (unsigned int idx = 0; idx < timestamps_a.size(); idx++) {
    ...
    if (pose_a_found_iter != out_poses_a.end()) {
      ...
      apollo::common::math::EulerAnglesZXY<double> euler_a(quatd_a.w(), quatd_a.x(), quatd_a.y(),
                                                           quatd_a.z());
      ...
      if (pose_b_found_iter != out_poses_b.end()) {
        ...
        apollo::common::math::EulerAnglesZXY<double> euler_b(quatd_b.w(), quatd_b.x(), quatd_b.y(),
                                                             quatd_b.z());
  ...
  if (vec_idx.empty()) { return 0; }

  std::ofstream file(compare_file.c_str());
  for (unsigned int i = 0; i < vec_idx.size(); ++i) {
    file << vec_idx[i] << " " << std::setprecision(13) << vec_timestamp[i] << " " << vec_x_diff[i]
         << " " << vec_y_diff[i] << " " << vec_z_diff[i] << " " << vec_roll_diff[i] << " "
         << vec_pitch_diff[i] << " " << vec_yaw_diff[i] << std::endl;
  }
  file.close();
}
