...
int main(int argc, char** argv) {
  ...
  if (imu_to_ant_offset_file != "") {
    bool suc = LoadGnssAntennaExtrinsic(imu_to_ant_offset_file, &imu_ant_offset);
  ...
  apollo::localization::msf::OfflineLocalVisualizer::PoseAndStdInterpolationByTime(
      poses_b, stds_b, timestamps_b, timestamps_a, &out_poses_b, &out_stds_b);
  apollo::localization::msf::OfflineLocalVisualizer::PoseAndStdInterpolationByTime(
      poses_a, stds_a, timestamps_a, timestamps_a, &out_poses_a, &out_stds_a);

  if (out_poses_a.size() == 0 || out_poses_b.size() == 0) { return 0; }
