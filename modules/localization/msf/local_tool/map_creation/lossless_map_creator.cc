...
typedef apollo::localization::msf::FeatureXYPlane::PointCloudPtrT PclPointCloudPtrT;
  ...
  desc.add_options()("help", "product help message")(
      ...
      ("pcd_folders",
       boost::program_options::value<std::vector<std::string>>()
           ...
           ->composing()
           ->required(),
       "pcd folders(repeated)")("pose_files",
                                boost::program_options::value<std::vector<std::string>>()
                                    ->multitoken()
                                    ->composing()
                                    ->required(),
          ...
          "map folder")("zone_id", boost::program_options::value<int>()->required(), "zone id")(
          "coordinate_type", boost::program_options::value<std::string>()->required(),
          "coordinate type: UTM or LTM")("map_resolution_type",
...
int main(int argc, char** argv) {
  ...
  if (fabs(single_resolution_map - 0.03125) > 1e-8 && fabs(single_resolution_map - 0.0625) > 1e-8 &&
      fabs(single_resolution_map - 0.125) < 1e-8 && fabs(single_resolution_map - 0.25) < 1e-8 &&
      fabs(single_resolution_map - 0.5) < 1e-8 && fabs(single_resolution_map - 1.0) < 1e-8 &&
      fabs(single_resolution_map - 2.0) < 1e-8 && fabs(single_resolution_map - 4.0) < 1e-8 &&
      fabs(single_resolution_map - 8.0) < 1e-8 && fabs(single_resolution_map - 16.0) < 1e-8) {
  ...
  for (size_t i = 0; i < pose_files.size(); ++i) {
    apollo::localization::msf::velodyne::LoadPcdPoses(pose_files[i], &ieout_poses[i],
                                                      &time_stamps[i], &pcd_indices[i]);
  }

  PyramidMapConfig  conf("lossless_map");
  PyramidMap        map(&conf);
  PyramidMapConfig& loss_less_config = static_cast<PyramidMapConfig&>(map.GetMapConfig());
  ...
  if (strcasecmp(coordinate_type.c_str(), "UTM") == 0) {
    ...
    loss_less_config.map_range_ =
        apollo::localization::msf::Rect2D<double>(-1638400.0, -1638400.0, 1638400.0, 1638400.0);
  }

  // Output Config file
  char file_buf[1024];
  snprintf(file_buf, sizeof(file_buf), "%s/lossless_map/config.xml", map_base_folder.c_str());
  loss_less_config.Save(file_buf);

  snprintf(file_buf, sizeof(file_buf), "%s/lossless_map/config.txt", map_base_folder.c_str());
  ...
  if (file) {
    ...
    fprintf(file, "\nMap size: %lf %lf %lf %lf\n", loss_less_config.map_range_.GetMinX(),
            loss_less_config.map_range_.GetMinY(), loss_less_config.map_range_.GetMaxX(),
    ...
    for (size_t i = 0; i < loss_less_config.map_resolutions_.size(); ++i) {
      fprintf(file, "%u x %u, ",
              MapNodeIndex::GetMapIndexRangeNorth(loss_less_config, static_cast<unsigned int>(i)),
              MapNodeIndex::GetMapIndexRangeEast(loss_less_config, static_cast<unsigned int>(i)));
    }
  ...
  for (unsigned int trial = 0; trial < num_trials; ++trial) {
    for (unsigned int frame_idx = 0; frame_idx < ieout_poses[trial].size(); ++frame_idx) {
      ...
      std::string                                        pcd_file_path =
          absl::StrCat(pcd_folder_paths[trial], "/", pcd_indices[trial][frame_idx], ".pcd");
      const Eigen::Affine3d& pcd_pose = poses[trial_frame_idx];
      apollo::localization::msf::velodyne::LoadPcds(pcd_file_path, trial_frame_idx, pcd_pose,
                                                    &velodyne_frame, false);
      AINFO << "Loaded " << velodyne_frame.pt3ds.size() << "3D Points at Trial: " << trial
            << " Frame: " << trial_frame_idx << ".";
      ...
      for (size_t i = 0; i < velodyne_frame.pt3ds.size(); ++i) {
        ...
        MapNodeIndex     map_node_index =
            MapNodeIndex::GetMapNodeIndex(loss_less_config, pt3d_global, resolution_id, zone_id);
  ...
  for (unsigned int trial = 0; trial < num_trials; ++trial) {
    for (unsigned int i = 0; i < ieout_poses[trial].size(); ++i) {
      ...
      unsigned int           col           = 0;
      MapNodeIndex           map_node_index =
          MapNodeIndex::GetMapNodeIndex(loss_less_config, pt3d, resolution_id, zone_id);
      PyramidMapNode* map_node = dynamic_cast<PyramidMapNode*>(map.GetMapNodeSafe(map_node_index));
      map_node->GetCoordinate(pt3d, &col, &row);
      PyramidMapMatrix& map_matrix = dynamic_cast<PyramidMapMatrix&>(map_node->GetMapCellMatrix());
      ...
      if (use_plane_inliers_only) {
        ...
        if (*ground_count > 0) {
          ...
          VarianceOnline(&mean_height_diff, &var_height_diff, &count_height_diff, height_diff);
        }
      } else {
  ...
  map.GetMapConfig().map_ground_height_offset_ = static_cast<float>(mean_height_diff);
