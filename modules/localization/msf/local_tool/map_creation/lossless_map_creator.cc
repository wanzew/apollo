...
int main(int argc, char** argv) {
  ...
  LosslessMapConfig& loss_less_config = static_cast<LosslessMapConfig&>(map.GetConfig());
  ...
  if (file) {
    ...
    for (size_t i = 0; i < loss_less_config.map_resolutions_.size(); ++i) {
      fprintf(file, "%u x %u, ", MapNodeIndex::GetMapIndexRangeNorth(loss_less_config, i),
  ...
  for (unsigned int trial = 0; trial < num_trials; ++trial) {
    for (unsigned int frame_idx = 0; frame_idx < ieout_poses[trial].size(); ++frame_idx) {
      ...
      AERROR << "Loaded " << velodyne_frame.pt3ds.size() << "3D Points at Trial: " << trial
