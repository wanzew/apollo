...
int main(int argc, char** argv) {
  ...
  boost_desc.add_options()("help", "produce help message")(
      "input_poses_path", boost::program_options::value<std::string>(), "provide input poses path")(
      "ref_timestamps_path", boost::program_options::value<std::string>(),
      "provide reference timestamp path")("extrinsic_path",
  ...
  if (boost_args.count("help") || !boost_args.count("input_poses_path") ||
      !boost_args.count("ref_timestamps_path") || !boost_args.count("extrinsic_path") ||
      !boost_args.count("output_poses_path")) {
    AERROR << boost_desc;
    return 0;
  }

  std::string input_poses_path    = boost_args["input_poses_path"].as<std::string>();
  std::string ref_timestamps_path = boost_args["ref_timestamps_path"].as<std::string>();
  std::string extrinsic_path      = boost_args["extrinsic_path"].as<std::string>();
  std::string out_poses_path      = boost_args["output_poses_path"].as<std::string>();

  apollo::localization::msf::PosesInterpolation pose_interpolation;
  bool success = pose_interpolation.Init(input_poses_path, ref_timestamps_path, out_poses_path,
                                         extrinsic_path);

  if (success) { pose_interpolation.DoInterpolation(); }
