...
int main(int argc, char** argv) {
  ...
  std::list<MapNodeIndex> buf;
  apollo::localization::msf::GetAllMapIndex(src_map_folder, dst_map_folder, &buf);
  ...
  if (!lossy_map.SetMapFolderPath(dst_map_folder)) { AINFO << "lossy_map config xml not exist"; }

  int  index = 0;
  auto itr   = buf.begin();
  for (; itr != buf.end(); ++itr, ++index) {
    PyramidMapNode* lossless_node = static_cast<PyramidMapNode*>(lossless_map.GetMapNodeSafe(*itr));
    ...
    PyramidMapNode*   lossy_node   = static_cast<PyramidMapNode*>(lossy_map.GetMapNodeSafe(*itr));
    PyramidMapMatrix& lossy_matrix = static_cast<PyramidMapMatrix&>(lossy_node->GetMapCellMatrix());
    ...
    for (int row = 0; row < rows; ++row) {
      for (int col = 0; col < cols; ++col) {
        ...
        if (intensity) { lossy_matrix.SetIntensitySafe(*intensity, row, col); }
        if (intensity_var) { lossy_matrix.SetIntensityVarSafe(*intensity_var, row, col); }
        if (count) { lossy_matrix.SetCountSafe(*count, row, col); }
        if (altitude_avg) { lossy_matrix.SetAltitudeSafe(*altitude_avg, row, col); }
        if (altitude_var) { lossy_matrix.SetAltitudeVarSafe(*altitude_var, row, col); }
        if (altitude_ground) { lossy_matrix.SetGroundAltitudeSafe(*altitude_ground, row, col); }
        if (ground_count) { lossy_matrix.SetGroundCountSafe(*ground_count, row, col); }
      }
    }
