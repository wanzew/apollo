...
MSFLocalization::MSFLocalization()
    : monitor_logger_(apollo::common::monitor::MonitorMessageItem::LOCALIZATION)
...
void MSFLocalization::InitParams() {
  ...
  localization_param_.is_trans_gpstime_to_utctime = FLAGS_trans_gpstime_to_utctime;
  ...
  if (FLAGS_if_utm_zone_id_from_folder) {
    bool success =
        LoadZoneIdFromFolder(localization_param_.map_path, &localization_param_.utm_zone_id);
  ...
  AINFO << "imu_vehicle_quat: " << imu_vehicle_quat_.x() << " " << imu_vehicle_quat_.y() << " "
        << imu_vehicle_quat_.z() << " " << imu_vehicle_quat_.w();

  // common
  localization_param_.enable_lidar_localization = FLAGS_enable_lidar_localization;
  ...
  if (!FLAGS_if_imuant_from_file) {
    ...
    localization_param_.imu_to_ant_offset.uncertainty_x = FLAGS_imu_to_ant_offset_ux;
    localization_param_.imu_to_ant_offset.uncertainty_y = FLAGS_imu_to_ant_offset_uy;
    localization_param_.imu_to_ant_offset.uncertainty_z = FLAGS_imu_to_ant_offset_uz;
  } else {
    double offset_x      = 0.0;
    double offset_y      = 0.0;
    double offset_z      = 0.0;
    ...
    ACHECK(LoadGnssAntennaExtrinsic(FLAGS_ant_imu_leverarm_file, &offset_x, &offset_y, &offset_z,
  ...
  localization_param_.imu_delay_time_threshold_1 = FLAGS_imu_delay_time_threshold_1;
  localization_param_.imu_delay_time_threshold_2 = FLAGS_imu_delay_time_threshold_2;
  localization_param_.imu_delay_time_threshold_3 = FLAGS_imu_delay_time_threshold_3;

  localization_param_.imu_missing_time_threshold_1 = FLAGS_imu_missing_time_threshold_1;
  localization_param_.imu_missing_time_threshold_2 = FLAGS_imu_missing_time_threshold_2;
  localization_param_.imu_missing_time_threshold_3 = FLAGS_imu_missing_time_threshold_3;

  localization_param_.bestgnsspose_loss_time_threshold = FLAGS_bestgnsspose_loss_time_threshold;
  localization_param_.lidar_loss_time_threshold        = FLAGS_lidar_loss_time_threshold;

  localization_param_.localization_std_x_threshold_1 = FLAGS_localization_std_x_threshold_1;
  localization_param_.localization_std_y_threshold_1 = FLAGS_localization_std_y_threshold_1;

  localization_param_.localization_std_x_threshold_2 = FLAGS_localization_std_x_threshold_2;
  localization_param_.localization_std_y_threshold_2 = FLAGS_localization_std_y_threshold_2;
...
void MSFLocalization::OnPointCloud(const std::shared_ptr<drivers::PointCloud>& message) {
  ++pcd_msg_index_;
  if (pcd_msg_index_ % FLAGS_point_cloud_step != 0) { return; }
...
void MSFLocalization::OnRawImu(const std::shared_ptr<drivers::gnss::Imu>& imu_msg) {
  ...
  status_headerpb->set_timestamp_sec(result.localization().header().timestamp_sec());
  status.set_fusion_status(static_cast<MeasureState>(result.integ_status().integ_state));
...
void MSFLocalization::OnRawImuCache(const std::shared_ptr<drivers::gnss::Imu>& imu_msg) {
  if (imu_msg) {
...
void MSFLocalization::CompensateImuVehicleExtrinsic(LocalizationEstimate* local_result) {
  ...
  const Eigen::Quaternion<double>   quaternion(orientation.qw(), orientation.qx(), orientation.qy(),
                                             orientation.qz());
  ...
  posepb_loc->set_heading(
      common::math::QuaternionToHeading(quat_vehicle_world.w(), quat_vehicle_world.x(),
  ...
  common::math::EulerAnglesZXYd euler_angle(quat_vehicle_world.w(), quat_vehicle_world.x(),
                                            quat_vehicle_world.y(), quat_vehicle_world.z());
...
bool MSFLocalization::LoadGnssAntennaExtrinsic(const std::string& file_path,
                                               double*            offset_x,
                                               double*            offset_y,
                                               double*            offset_z,
    ...
    if (config["leverarm"]["primary"]["offset"]) {
      ...
      *offset_z = config["leverarm"]["primary"]["offset"]["z"].as<double>();
      ...
      if (config["leverarm"]["primary"]["uncertainty"]) {
        *uncertainty_x = config["leverarm"]["primary"]["uncertainty"]["x"].as<double>();
        *uncertainty_y = config["leverarm"]["primary"]["uncertainty"]["y"].as<double>();
        *uncertainty_z = config["leverarm"]["primary"]["uncertainty"]["z"].as<double>();
      }
      return true;
    }
  }
...
bool MSFLocalization::LoadZoneIdFromFolder(const std::string& folder_path, int* zone_id) {
