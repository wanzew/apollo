...
Status MSFLocalization::Start() {
  ...
  if (!AdapterManager::GetRawImu()) {
    buffer.ERROR("Raw IMU input not initialized. Check your adapter.conf file!");
    buffer.PrintLog();
  ...
  if (FLAGS_enable_gps_heading) {
    if (!AdapterManager::GetGnssHeading()) {
      buffer.ERROR("GnssHeading input not initialized. Check your adapter.conf file!");
      buffer.PrintLog();
      return Status(common::LOCALIZATION_ERROR_MSG, "no gnss heading adapter");
    }
    AdapterManager::AddGnssHeadingCallback(&MSFLocalization::OnGnssHeading, this);
  }
...
void MSFLocalization::InitParams() {
  ...
  if (FLAGS_if_utm_zone_id_from_folder) {
    bool success =
        LoadZoneIdFromFolder(localizaiton_param_.map_path, &localizaiton_param_.utm_zone_id);
  ...
  if (FLAGS_if_vehicle_imu_from_file) {
    ...
    std::string vehicle_imu_file = common::util::TranslatePath(FLAGS_vehicle_imu_file);
  ...
  // common
  localizaiton_param_.enable_lidar_localization = FLAGS_enable_lidar_localization;
  ...
  if (!FLAGS_if_imuant_from_file) {
    ...
    localizaiton_param_.imu_to_ant_offset.uncertainty_x = FLAGS_imu_to_ant_offset_ux;
    localizaiton_param_.imu_to_ant_offset.uncertainty_y = FLAGS_imu_to_ant_offset_uy;
    localizaiton_param_.imu_to_ant_offset.uncertainty_z = FLAGS_imu_to_ant_offset_uz;
  } else {
    double      offset_x              = 0.0;
    double      offset_y              = 0.0;
    double      offset_z              = 0.0;
    ...
    std::string ant_imu_leverarm_file = common::util::TranslatePath(FLAGS_ant_imu_leverarm_file);
    AERROR << "Ant imu lever arm file: " << ant_imu_leverarm_file;
    CHECK(LoadGnssAntennaExtrinsic(ant_imu_leverarm_file, &offset_x, &offset_y, &offset_z,
...
void MSFLocalization::OnPointCloud(const sensor_msgs::PointCloud2& message) {
  ...
  if (FLAGS_lidar_debug_log_flag) {
    ...
    for (auto itr = lidar_localization_list.begin(); itr != lidar_localization_list.end(); ++itr) {
      latest_lidar_localization_status_ = static_cast<MeasureState>(itr->state());
...
void MSFLocalization::OnRawImu(const drivers::gnss::Imu& imu_msg) {
  ...
  for (auto itr = integ_localization_list.begin(); itr != integ_localization_list.end(); ++itr) {
    // compose localization status
    LocalizationStatus      status;
    apollo::common::Header* status_headerpb = status.mutable_header();
    status_headerpb->set_timestamp_sec(itr->localization().header().timestamp_sec());
    ...
    if (itr->state() == msf::LocalizationMeasureState::OK ||
      ...
      const Eigen::Quaternion<double>   quaternion(orientation.qw(), orientation.qx(),
...
void MSFLocalization::OnGnssBestPose(const drivers::gnss::GnssBestPose& bestgnsspos_msg) {
  ...
  if (FLAGS_gnss_debug_log_flag) {
    ...
    for (auto itr = gnss_localization_list.begin(); itr != gnss_localization_list.end(); ++itr) {
      latest_gnss_localization_status_ = static_cast<MeasureState>(itr->state());
      if (itr->state() == msf::LocalizationMeasureState::OK ||
          itr->state() == msf::LocalizationMeasureState::VALID) {
...
void MSFLocalization::OnGnssHeading(const drivers::gnss::Heading& gnssheading_msg) {
...
bool MSFLocalization::LoadImuVehicleExtrinsic(const std::string& file_path,
                                              ...
                                              double*            quat_qz,
                                              double*            quat_qw) {
  if (!common::util::PathExists(file_path)) { return false; }
  YAML::Node config = YAML::LoadFile(file_path);
  if (config["transform"]) {
