...
void BaseMap::LoadMapNodes(std::set<MapNodeIndex>* map_ids) {
  ...
  itr = map_ids->begin();
  while (itr != map_ids->end()) {
    p_map_load_threads_->schedule(boost::bind(&BaseMap::LoadMapNodeThreadSafety, this, *itr, true));
    ++itr;
  }
...
void BaseMap::PreloadMapNodes(std::set<MapNodeIndex>* map_ids) {
  DCHECK_LE(static_cast<int>(map_ids->size()), map_node_cache_lvl2_->Capacity());
  // check in cacheL2
  typename std::set<MapNodeIndex>::iterator itr = map_ids->begin();
  while (itr != map_ids->end()) {
    boost::unique_lock<boost::recursive_mutex> lock(map_load_mutex_);
...
void BaseMap::PreloadMapArea(const Eigen::Vector3d& location,
  ...
  pt_top_left[0] = location[0] - (this->map_config_->map_node_size_x_ * map_pixel_resolution / 2.0);
  pt_top_left[1] = location[1] - (this->map_config_->map_node_size_y_ * map_pixel_resolution / 2.0);
  pt_top_left[2] = 0;
  MapNodeIndex map_id =
  ...
  pt_top_right[0] =
      location[0] + (this->map_config_->map_node_size_x_ * map_pixel_resolution / 2.0);
  ...
  pt_bottom_left[1] =
      location[1] + (this->map_config_->map_node_size_y_ * map_pixel_resolution / 2.0);
  pt_bottom_left[2] = 0;
  map_id =
