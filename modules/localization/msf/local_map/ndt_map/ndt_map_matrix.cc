...
unsigned int NdtMapSingleCell::LoadBinary(unsigned char* buf) {
  ...
  ++f_buf;
  unsigned int* ui_buf = reinterpret_cast<unsigned int*>(reinterpret_cast<void*>(f_buf));
...
unsigned int NdtMapSingleCell::CreateBinary(unsigned char* buf, unsigned int buf_size) const {
...
unsigned int NdtMapSingleCell::GetBinarySize() const {
  unsigned int sz = static_cast<unsigned int>(sizeof(float) * 2 + sizeof(unsigned int) * 2 +
...
unsigned int NdtMapCells::CreateBinary(unsigned char* buf, unsigned int buf_size) const {
  unsigned int target_size = GetBinarySize();
  if (buf_size >= target_size) {
    unsigned int* p = reinterpret_cast<unsigned int*>(buf);
...
unsigned int NdtMapCells::GetBinarySize() const {
  ...
  target_size += static_cast<unsigned int>(sizeof(int) * road_cell_indices_.size());
...
unsigned int NdtMapMatrix::CreateBinary(unsigned char* buf, unsigned int buf_size) const {
  unsigned int target_size = GetBinarySize();
  if (buf_size >= target_size) {
    unsigned int* p = reinterpret_cast<unsigned int*>(buf);
    *p              = rows_;
...
unsigned int NdtMapMatrix::GetBinarySize() const {
  unsigned int target_size = static_cast<unsigned int>(sizeof(unsigned int) * 2);
  for (unsigned int y = 0; y < rows_; ++y) {
    for (unsigned int x = 0; x < cols_; ++x) {
      const NdtMapCells& cell = GetMapCell(y, x);
      target_size += cell.GetBinarySize();
