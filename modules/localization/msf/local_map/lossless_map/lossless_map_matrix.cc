...
LosslessMapSingleCell& LosslessMapSingleCell::operator=(const LosslessMapSingleCell& ref) {
...
void LosslessMapSingleCell::AddSample(const float new_altitude, const float new_intensity) {
  ++count;
  float v1    = new_intensity - intensity;
  float value = v1 / count;
  intensity += value;
  float v2      = new_intensity - intensity;
  intensity_var = (static_cast<float>(count - 1) * intensity_var + v1 * v2) / count;

  v1    = new_altitude - altitude;
  value = v1 / count;
  altitude += value;
  v2           = new_altitude - altitude;
  altitude_var = (static_cast<float>(count - 1) * altitude_var + v1 * v2) / count;
}
...
unsigned int LosslessMapSingleCell::CreateBinary(unsigned char* buf, unsigned int buf_size) const {
...
void LosslessMapCell::SetValueLayer(double        altitude,
...
unsigned int LosslessMapCell::CreateBinary(unsigned char* buf, unsigned int buf_size) const {
  unsigned int target_size = GetBinarySize();
  if (buf_size >= target_size) {
    unsigned int* p = reinterpret_cast<unsigned int*>(buf);
    *p              = layer_num;
...
unsigned int LosslessMapCell::GetBinarySize() const {
  unsigned int target_size =
      sizeof(unsigned int);  // The size of the variable for the number of layers.
  for (size_t i = 0; i < layer_num; ++i) {
    const LosslessMapSingleCell& cell = map_cells[i];
...
unsigned int LosslessMapMatrix::CreateBinary(unsigned char* buf, unsigned int buf_size) const {
  unsigned int target_size = GetBinarySize();
  if (buf_size >= target_size) {
    unsigned int* p = reinterpret_cast<unsigned int*>(buf);
    *p              = rows_;
