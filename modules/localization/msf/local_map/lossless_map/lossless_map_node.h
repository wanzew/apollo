...
class LosslessMapNode : public BaseMapNode {
 public:
  ...
  unsigned char GetValue(unsigned int row, unsigned int col) const {
    return static_cast<LosslessMapMatrix*>(map_matrix_)->GetMapCell(row, col).GetValue();
  }
  /**@brief Get the map cell variance of the intensity. */
  float GetVar(unsigned int row, unsigned int col) const {
    return static_cast<LosslessMapMatrix*>(map_matrix_)->GetMapCell(row, col).GetVar();
  }
  /**@brief Get the map cell's average altitude. */
  float GetAlt(unsigned int row, unsigned int col) const {
    return static_cast<LosslessMapMatrix*>(map_matrix_)->GetMapCell(row, col).GetAlt();
  }
  /**@brief Get the map cell's variance of the altitude */
  float GetAltVar(unsigned int row, unsigned int col) const {
    return static_cast<LosslessMapMatrix*>(map_matrix_)->GetMapCell(row, col).GetAltVar();
  }
  /**@brief Get the map cell's count of the samples. */
  unsigned int GetCount(unsigned int row, unsigned int col) const {
    return static_cast<LosslessMapMatrix*>(map_matrix_)->GetMapCell(row, col).GetCount();
  }
  /**@brief Get the constant map cell given the coordinate. */
  inline const LosslessMapSingleCell& GetFirstMapCell(unsigned int row, unsigned int col) const {
    return static_cast<LosslessMapMatrix*>(map_matrix_)->GetMapCell(row, col).map_cells[0];
  }
  inline LosslessMapSingleCell& GetFirstMapCell(unsigned int row, unsigned int col) {
    return static_cast<LosslessMapMatrix*>(map_matrix_)->GetMapCell(row, col).map_cells[0];
  }
