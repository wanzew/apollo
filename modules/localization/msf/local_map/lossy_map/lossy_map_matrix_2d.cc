...
unsigned char LossyMapMatrix2D::EncodeIntensity(const LossyMapCell2D& cell) const {
...
void LossyMapMatrix2D::DecodeIntensity(unsigned char data, LossyMapCell2D* cell) const {
  cell->intensity = data;
}
...
uint16_t LossyMapMatrix2D::EncodeVar(const LossyMapCell2D& cell) const {
  ...
  var               = std::sqrt(var);
  int intensity_var = static_cast<int>(static_cast<const float>(var_range_) /
                                       (var * static_cast<const float>(var_ratio_) + 1.0));
  if (intensity_var > var_range_) { intensity_var = var_range_; }
  if (intensity_var < 1) { intensity_var = 1; }
  return static_cast<uint16_t>(intensity_var);
}
...
void LossyMapMatrix2D::DecodeVar(uint16_t data, LossyMapCell2D* cell) const {
  ...
  var                 = static_cast<float>((static_cast<const float>(var_range_) / var - 1.0) /
                           static_cast<const float>(var_ratio_));
  cell->intensity_var = var * var;
}

uint16_t LossyMapMatrix2D::EncodeAltitudeGround(const LossyMapCell2D& cell) const {
  float delta_alt = cell.altitude_ground - alt_ground_min_;
  delta_alt /= alt_ground_interval_;
  int ratio = static_cast<int>(delta_alt + 0.5);
  if (ratio >= ground_void_flag_) { ratio = ground_void_flag_ - 1; }
  if (ratio < 0) { ratio = 0; }
  return static_cast<uint16_t>(ratio);
}

void LossyMapMatrix2D::DecodeAltitudeGround(uint16_t data, LossyMapCell2D* cell) const {
  float ratio           = data;
...
void LossyMapMatrix2D::DecodeAltitudeAvg(uint16_t data, LossyMapCell2D* cell) const {
  float ratio    = data;
...
unsigned char LossyMapMatrix2D::EncodeCount(const LossyMapCell2D& cell) const {
  ...
  if (count_exp > count_range_) { count_exp = count_range_; }
  return static_cast<unsigned char>(count_exp);
}

void LossyMapMatrix2D::DecodeCount(unsigned char data, LossyMapCell2D* cell) const {
  int count_exp = data;
  if (count_exp == 0) {
...
unsigned int LossyMapMatrix2D::LoadBinary(unsigned char* buf) {
  ...
  for (unsigned int row = 0; row < rows_; ++row) {
    for (unsigned int col = 0; col < cols_; ++col) {
      ...
      var          = static_cast<uint16_t>(var * 256 + pp_low[row * cols_ + col]);
  ...
  for (unsigned int row = 0; row < rows_; ++row) {
    for (unsigned int col = 0; col < cols_; ++col) {
      ...
      alt                  = static_cast<uint16_t>(alt * 256 + pp_low[row * cols_ + col]);
...
unsigned int LossyMapMatrix2D::CreateBinary(unsigned char* buf, unsigned int buf_size) const {
  ...
  if (buf_size >= target_size) {
    ...
    *p = cols_;
    ...
    for (unsigned int y = 0; y < rows_; ++y) {
      for (unsigned int x = 0; x < cols_; ++x) {
        ...
        if (cell.altitude > alt_avg_max_) { alt_avg_max_ = cell.altitude; }
        if (cell.altitude < alt_avg_min_) { alt_avg_min_ = cell.altitude; }
      }
    }
    *pf = alt_avg_min_;
    ++pf;
    *pf = alt_avg_max_;
    ++pf;
    buf_size -= static_cast<unsigned int>(sizeof(float) * 2);
    ...
    for (unsigned int y = 0; y < rows_; ++y) {
      for (unsigned int x = 0; x < cols_; ++x) {
        const LossyMapCell2D& cell = map_cells_[y * cols_ + x];
        if (!cell.is_ground_useful) { continue; }
        if (cell.altitude_ground > alt_ground_max_) { alt_ground_max_ = cell.altitude_ground; }
        if (cell.altitude_ground < alt_ground_min_) { alt_ground_min_ = cell.altitude_ground; }
      }
    }
    *pf = alt_ground_min_;
    ++pf;
    *pf = alt_ground_max_;
    ...
    for (unsigned int row = 0; row < rows_; ++row) {
      for (unsigned int col = 0; col < cols_; ++col) {
        ...
        pp_high[row * cols_ + col] = static_cast<unsigned char>(var / 256);
        pp_low[row * cols_ + col]  = static_cast<unsigned char>(var % 256);
      }
    }
    pp += 2 * rows_ * cols_;
    ...
    for (unsigned int row = 0; row < rows_; ++row) {
      for (unsigned int col = 0; col < cols_; ++col) {
        ...
        pp_high[row * cols_ + col] = static_cast<unsigned char>(altitude / 256);
        pp_low[row * cols_ + col]  = static_cast<unsigned char>(altitude % 256);
      }
    }
    pp += 2 * rows_ * cols_;
...
unsigned int LossyMapMatrix2D::GetBinarySize() const {
  ...
  target_size +=
      static_cast<unsigned int>(rows_ * cols_ *
  ...
  *intensity_img = cv::Mat(cv::Size(cols_, rows_), CV_8UC1);
  ...
  for (unsigned int y = 0; y < rows_; ++y) {
    for (unsigned int x = 0; x < cols_; ++x) {
      ...
      intensity_img->at<unsigned char>(y, x) = (unsigned char)(map_cells_[id].intensity);
    }
  }
}
