...
void LocalizationLidarProcess::PcdProcess(const LidarFrame& lidar_frame) {
  ...
  if (forcast_integ_state_ != ForcastState::INCREMENT) { forcast_timer_ = -1; }
  ++forcast_timer_;

  locator_->SetDeltaYawLimit(
      ComputeDeltaYawLimit(forcast_timer_, 10, delta_yaw_limit_, init_delta_yaw_limit_));
  ...
  int ret = locator_->Update(pcd_index++, cur_predict_location_, velocity_, lidar_frame);
...
bool LocalizationLidarProcess::GetPredictPose(const double  lidar_time,
  ...
  if (lidar_status_ != LidarState::OK) {
    Pose init_pose;
    state = pose_forcastor_->GetBestForcastPose(lidar_time, -1, init_pose, &forcast_pose);
  } else {
    Pose init_pose;
    ...
    state = pose_forcastor_->GetBestForcastPose(lidar_time, pre_location_time_, init_pose,
                                                &forcast_pose);
  }
  ...
  Eigen::Quaterniond   quatd(forcast_pose.qw, forcast_pose.qx, forcast_pose.qy, forcast_pose.qz);
  Eigen::Translation3d transd(Eigen::Vector3d(forcast_pose.x, forcast_pose.y, forcast_pose.z));
...
void LocalizationLidarProcess::UpdateState(const int ret, const double time) {
  if (ret == 0) {  // OK
    ...
    double cur_location_std_area =
        std::sqrt(location_covariance_(0, 0)) * std::sqrt(location_covariance_(1, 1));
