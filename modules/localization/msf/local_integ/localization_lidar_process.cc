...
Status LocalizationLidarProcess::Init(const LocalizationIntegParam& params) {
  ...
  if (!success) {
    AERROR << "LocalizationLidar: Fail to access the lidar"
              " extrinsic file: "
           << lidar_extrinsic_file_;
    return Status(common::LOCALIZATION_ERROR_LIDAR, "Fail to access the lidar extrinsic file");
  }
  ...
  if (!locator_->Init(map_path_, lidar_filter_size_, lidar_filter_size_, utm_zone_id_)) {
    local_lidar_status_ = LocalLidarStatus::MSF_LOCAL_LIDAR_MAP_LOADING_FAILED;
    return Status(common::LOCALIZATION_ERROR_LIDAR, "Fail to load localization map!");
  }
...
double LocalizationLidarProcess::ComputeDeltaYawLimit(const int64_t index_cur,
                                                      const int64_t index_stable,
                                                      const double  limit_min,
                                                      const double  limit_max) {
  if (index_cur > index_stable) { return limit_min; }
...
void LocalizationLidarProcess::PcdProcess(const LidarFrame& lidar_frame) {
  ...
  if (forecast_integ_state_ != ForecastState::INCREMENT) { forecast_timer_ = -1; }
  ++forecast_timer_;

  locator_->SetDeltaYawLimit(
      ComputeDeltaYawLimit(forecast_timer_, 10, delta_yaw_limit_, init_delta_yaw_limit_));
  ...
  int ret =
      locator_->Update(pcd_index++, cur_predict_location_, velocity_, lidar_frame, if_use_avx_);
...
int LocalizationLidarProcess::GetResult(LocalizationEstimate* lidar_local_msg) {
  if (lidar_local_msg == nullptr) { return static_cast<int>(LidarState::NOT_VALID); }
  ...
  apollo::localization::Uncertainty* uncertainty = lidar_local_msg->mutable_uncertainty();
...
bool LocalizationLidarProcess::GetPredictPose(const double   lidar_time,
  ...
  if (latest_imu_time - lidar_time > imu_lidar_max_delay_time_) {
    AERROR << std::setprecision(16) << "LocalizationLidar GetPredictPose: "
           << "Lidar msg too old! "
           << "lidar time: " << lidar_time << "delay time: " << latest_imu_time - lidar_time;
    return false;
  }
  ...
  if (lidar_status_ != LidarState::OK) {
    Pose init_pose;
    state = pose_forecastor_->GetBestForcastPose(lidar_time, -1, init_pose, &forecast_pose);
  } else {
    Pose init_pose;
    ...
    state = pose_forecastor_->GetBestForcastPose(lidar_time, pre_location_time_, init_pose,
                                                 &forecast_pose);
  }
  ...
  Eigen::Quaterniond quatd(forecast_pose.qw, forecast_pose.qx, forecast_pose.qy, forecast_pose.qz);
  Eigen::Translation3d transd(Eigen::Vector3d(forecast_pose.x, forecast_pose.y, forecast_pose.z));
...
void LocalizationLidarProcess::UpdateState(const int ret, const double time) {
  if (ret == 0) {  // OK
    ...
    // check covariance
    double cur_location_std_area = std::sqrt(local_uncertainty_x * local_uncertainty_x +
                                             local_uncertainty_y * local_uncertainty_y);
    if (cur_location_std_area > unstable_threshold_ || location_score < 0.8) {
      local_lidar_quality_ = LocalLidarQuality::MSF_LOCAL_LIDAR_BAD;
    } else if (cur_location_std_area <= unstable_threshold_ && location_score < 0.85) {
      local_lidar_quality_ = LocalLidarQuality::MSF_LOCAL_LIDAR_NOT_BAD;
    } else if (cur_location_std_area <= unstable_threshold_ && location_score < 0.95) {
    ...
    if (unstable_count_ >= 2 && is_unstable_reset_) {
      ...
      lidar_status_ = LidarState::OK;
      if (out_map_count_ > 0) { --out_map_count_; }
    }
...
bool LocalizationLidarProcess::LoadLidarHeight(const std::string& file_path, LidarHeight* height) {
