...
bool MeasureRepublishProcess::NovatelBestgnssposProcess(const GnssBestPose& bestgnsspos_msg,
  ...
  if (is_sins_align) {
    ...
    if (is_using_novatel_heading_) {
      novatel_heading_mutex_.lock();
      double novatel_heading_time = novatel_heading_time_;
      novatel_heading_mutex_.unlock();
      double time_before_wait = ros::Time::now().toSec();

      while (novatel_heading_time < measure->time) {
  ...
  if (debug_log_flag_) {
    AINFO << std::setprecision(16) << "MeasureDataRepublish Debug Log: bestgnsspos msg: "
          << "[time:" << measure->time << "]"
...
void MeasureRepublishProcess::GnssLocalProcess(const MeasureData& gnss_local_msg,
  ...
  *measure = measure_data;
  ...
  if (debug_log_flag_) {
    AINFO << std::setprecision(16) << "MeasureDataRepublish Debug Log: rtkgnss msg: "
...
bool MeasureRepublishProcess::LidarLocalProcess(const LocalizationEstimate& lidar_local_msg,
  ...
  apollo::localization::msf::UtmXYToLatlon(lidar_local_msg.pose().position().x(),
  ...
  *measure = measure_data;
  ...
  if (debug_log_flag_) {
    AINFO << std::setprecision(16) << "MeasureDataRepublish Debug Log: lidarLocal msg: "
          << "[time:" << measure_data.time << "]"
          << "[x:" << measure_data.gnss_pos.longitude * RAD_TO_DEG << "]"
          << "[y:" << measure_data.gnss_pos.latitude * RAD_TO_DEG << "]"
          << "[z:" << measure_data.gnss_pos.height << "]"
...
bool MeasureRepublishProcess::IsSinsAlign() {
  ...
  if (!integ_pva_list_.empty()) { is_sins_align = integ_pva_list_.back().init_and_alignment; }
  integ_pva_mutex_.unlock();
...
bool MeasureRepublishProcess::CalculateVelFromBestgnsspose(const GnssBestPose& bestgnsspos_msg,
  ...
  if (!pre_yaw_from_vel) {
    ...
    AINFO << "yaw caculated from position difference: " << yaw_from_vel * RAD_TO_DEG;
    constexpr double rad_5deg = 5 * DEG_TO_RAD;
