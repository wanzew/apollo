...
void LocalizationGnssProcess::RawObservationProcess(
  ...
  double       leap_second_s =
      gnss_solver_->get_leap_second(raw_obs.gnss_week(), raw_obs.gnss_second_s());

  double sys_secs_to_gnss = Clock::NowInSeconds() - unix_to_gps + leap_second_s;
  double obs_secs         = raw_obs.gnss_second_s() + raw_obs.gnss_week() * sec_per_week;
  double obs_delay        = sys_secs_to_gnss - obs_secs;

  double obs_xyz[3] = {0.0};
  if (raw_obs.has_position_x() && raw_obs.has_position_y() && raw_obs.has_position_z()) {
  ...
  snprintf(message, max_msg_size,
           ...
           raw_obs.receiver_id(), raw_obs.gnss_second_s(), raw_obs.sat_obs_num(), obs_delay,
  ...
  double             new_obs_time = raw_obs.gnss_second_s() + raw_obs.gnss_week() * second_per_week;
  if (new_obs_time <= current_obs_time_) { return; }
  current_obs_time_ = new_obs_time;
  GnssPosition(&raw_obs_msg);
}

void LocalizationGnssProcess::RawEphemerisProcess(const drivers::gnss::GnssEphemeris& msg) {
  if (!msg.has_gnss_type()) { return; }
  auto gnss_orbit = msg;
  if (gnss_orbit.gnss_type() == drivers::gnss::GnssType::GLO_SYS) {
     ...
     */
    double leap_sec = gnss_solver_->get_leap_second(gnss_orbit.glonass_orbit().week_num(),
...
void LocalizationGnssProcess::IntegSinsPvaProcess(const InsPva& sins_pva_msg,
  ...
  if (!sins_pva_msg.init_and_alignment) { return; }

  sins_align_finish_ = true;

  // feed into GNSS-RTK Engine
  double sec_s  = sins_pva_msg.time;
  double llh[3] = {sins_pva_msg.pos.longitude, sins_pva_msg.pos.latitude, sins_pva_msg.pos.height};
  double velocity[3]  = {sins_pva_msg.vel.ve, sins_pva_msg.vel.vn, sins_pva_msg.vel.vu};
  double lever_arm[3] = {gnss_lever_arm_.arm_x, gnss_lever_arm_.arm_y, gnss_lever_arm_.arm_z};
  double euler[3]     = {sins_pva_msg.att.pitch, sins_pva_msg.att.roll, sins_pva_msg.att.yaw};
  ...
  gnss_solver_->motion_update(sec_s, llh, std_pos, velocity, std_vel, euler, lever_arm);
}
...
LocalizationMeasureState LocalizationGnssProcess::GetResult(MeasureData* gnss_msg) {
  ...
  // ros::Time::now().toSec();
  const unsigned int second_per_week = 604800;
  double sec_s = gnss_pnt_result_.gnss_week() * second_per_week + gnss_pnt_result_.gnss_second_s();
  gnss_msg->time       = sec_s;
  ...
  if (b_pos && b_vel) { gnss_msg->measure_type = MeasureType::GNSS_POS_VEL; }
...
bool LocalizationGnssProcess::DuplicateEph(const drivers::gnss::GnssEphemeris& raw_eph) {
  ...
  if (t_type != drivers::gnss::GnssType::GPS_SYS && t_type != drivers::gnss::GnssType::BDS_SYS &&
  ...
  if (map_gnss_eph_.find(temp) != map_gnss_eph_.end()) { return true; }
  map_gnss_eph_.insert(std::map<EphKey, drivers::gnss::GnssEphemeris>::value_type(temp, raw_eph));
  return false;
}

inline void LocalizationGnssProcess::LogPnt(const GnssPntResultMsg& rover_pnt, double ratio) {
  ...
  snprintf(print_infor, sizeof(print_infor),
           ...
           "3f%8.3f\n",
           rover_pnt.gnss_week(), rover_pnt.gnss_second_s(), static_cast<int>(rover_pnt.pnt_type()),
           rover_pnt.pos_x_m(), rover_pnt.pos_y_m(), rover_pnt.pos_z_m(),
           rover_pnt.sovled_sat_num(), rover_pnt.pdop(), ratio, rover_pnt.vel_x_m(),
           rover_pnt.vel_y_m(), rover_pnt.vel_z_m(), rover_pnt.std_pos_x_m(),
...
bool LocalizationGnssProcess::GnssPosition(EpochObservationMsg* raw_rover_obs) {
  ...
  if (b_solved < 0) { return false; }
  LogPnt(gnss_pnt_result_, gnss_solver_->get_ratio());
  if (!sins_align_finish_) { AWARN << "Sins-ekf has not converged or finished its alignment!"; }
  if (gnss_pnt_result_.has_std_pos_x_m() && gnss_pnt_result_.has_std_pos_y_m() &&
      gnss_pnt_result_.has_std_pos_z_m()) {
    double sigma = gnss_pnt_result_.std_pos_x_m() * gnss_pnt_result_.std_pos_x_m() +
