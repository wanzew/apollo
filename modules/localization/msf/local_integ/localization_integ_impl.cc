...
void LocalizationIntegImpl::RawImuProcessRfu(const ImuData& imu_data) { ImuProcessImpl(imu_data); }
...
void LocalizationIntegImpl::ImuProcessImpl(const ImuData& imu_data) {
  ...
  expert_.GetFusionStatus(integ_localization.mutable_msf_status(),
                          integ_localization.mutable_sensor_status(), &integ_status);
  ...
  Eigen::Vector3d orig_acceleration(corrected_imu.fb[0], corrected_imu.fb[1], corrected_imu.fb[2]);
  ...
  Eigen::Quaternion<double>         quaternion(orientation.qw(), orientation.qx(), orientation.qy(),
                                       orientation.qz());
  Eigen::Vector3d vec_acceleration = static_cast<Eigen::Vector3d>(quaternion * orig_acceleration);
  ...
  Eigen::Vector3d orig_angular_velocity(corrected_imu.wibb[0], corrected_imu.wibb[1],
                                        corrected_imu.wibb[2]);
  Eigen::Vector3d vec_angular_velocity =
      static_cast<Eigen::Vector3d>(quaternion.toRotationMatrix() * orig_angular_velocity);
  ...
  lastest_integ_localization_ = LocalizationResult(
      LocalizationMeasureState(static_cast<int>(state)), integ_localization, integ_status);
...
void LocalizationIntegImpl::TransferGnssMeasureToLocalization(const MeasureData&    measure,
  ...
  UTMCoor utm_xy;
  FrameTransform::LatlonToUtmXY(measure.gnss_pos.longitude, measure.gnss_pos.latitude, &utm_xy);
...
const LocalizationResult& LocalizationIntegImpl::GetLastestLidarLocalization() const {
  return lastest_lidar_localization_;
}

const LocalizationResult& LocalizationIntegImpl::GetLastestIntegLocalization() const {
  return lastest_integ_localization_;
}

const LocalizationResult& LocalizationIntegImpl::GetLastestGnssLocalization() const {
