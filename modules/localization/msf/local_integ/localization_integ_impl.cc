...
void LocalizationIntegImpl::PcdThreadLoop() {
  ...
  while (keep_lidar_running_.load()) {
    ...
    if (waiting_num > 2) { AWARN << waiting_num << " point cloud msg are waiting to process."; }
...
void LocalizationIntegImpl::ImuThreadLoop() {
  ...
  while (keep_imu_running_.load()) {
    ...
    if (waiting_num > 10) { AWARN << waiting_num << " imu msg are waiting to process."; }
...
void LocalizationIntegImpl::ImuProcessImpl(const ImuData& imu_data) {
  ...
  if (enable_lidar_localization_) { lidar_process_->RawImuProcess(imu_data); }
  ...
  Eigen::Vector3d orig_acceleration(imu_data.fb[0], imu_data.fb[1], imu_data.fb[2]);
  const apollo::common::Quaternion& orientation = integ_localization.pose().orientation();
  Eigen::Quaternion<double>         quaternion(orientation.qw(), orientation.qx(), orientation.qy(),
                                       orientation.qz());
  Eigen::Vector3d vec_acceleration = quaternion.toRotationMatrix() * orig_acceleration;

  apollo::common::Point3D* linear_acceleration = posepb_loc->mutable_linear_acceleration();
  ...
  apollo::common::Point3D* linear_acceleration_vrf = posepb_loc->mutable_linear_acceleration_vrf();
  ...
  Eigen::Vector3d orig_angular_velocity(imu_data.wibb[0], imu_data.wibb[1], imu_data.wibb[2]);
  Eigen::Vector3d vec_angular_velocity      = quaternion.toRotationMatrix() * orig_angular_velocity;
  apollo::common::Point3D* angular_velocity = posepb_loc->mutable_angular_velocity();
  ...
  apollo::common::Point3D* angular_velocity_vrf = posepb_loc->mutable_angular_velocity_vrf();
  ...
  integ_localization_list_.push_back(
      LocalizationResult(LocalizationMeasureState(static_cast<int>(state)), integ_localization));
...
void LocalizationIntegImpl::RawObservationProcess(
  ...
  if (is_use_gnss_bestpose_) { return; }

  // push process function to queue
  gnss_function_queue_mutex_.lock();
  gnss_function_queue_.push(std::function<void()>(
      std::bind(&LocalizationIntegImpl::RawObservationProcessImpl, this, raw_obs_msg)));
  gnss_function_signal_.notify_one();
...
void LocalizationIntegImpl::RawEphemerisProcess(
  ...
  gnss_function_queue_mutex_.lock();
  gnss_function_queue_.push(std::function<void()>(
      std::bind(&LocalizationIntegImpl::RawEphemerisProcessImpl, this, gnss_orbit_msg)));
  gnss_function_signal_.notify_one();
  gnss_function_queue_mutex_.unlock();
...
void LocalizationIntegImpl::GnssBestPoseProcess(
  ...
  gnss_function_queue_mutex_.lock();
  gnss_function_queue_.push(std::function<void()>(
      std::bind(&LocalizationIntegImpl::GnssBestPoseProcessImpl, this, bestgnsspos_msg)));
  gnss_function_signal_.notify_one();
  gnss_function_queue_mutex_.unlock();
...
void LocalizationIntegImpl::GnssHeadingProcess(const drivers::gnss::Heading& gnssheading_msg) {
  gnss_heading_function_queue_mutex_.lock();
  gnss_heading_function_queue_.push(std::function<void()>(
      std::bind(&LocalizationIntegImpl::GnssHeadingProcessImpl, this, gnssheading_msg)));
  gnss_heading_function_signal_.notify_one();
...
void LocalizationIntegImpl::GnssHeadingProcessImpl(const drivers::gnss::Heading& gnssheading_msg) {
  MeasureData measure;
  int         heading_status = 0;
  if (republish_process_->GnssHeadingProcess(gnssheading_msg, &measure, &heading_status)) {
...
void LocalizationIntegImpl::GnssThreadLoop() {
  ...
  while (keep_gnss_running_.load()) {
    ...
    if (waiting_num > 2) { AWARN << waiting_num << " gnss function are waiting to process."; }
...
void LocalizationIntegImpl::RawObservationProcessImpl(
  ...
  MeasureData measure;
  if (state == LocalizationMeasureState::OK || state == LocalizationMeasureState::VALID) {
  ...
  gnss_localization_list_.push_back(LocalizationResult(state, gnss_localization));
...
void LocalizationIntegImpl::TransferGnssMeasureToLocalization(const MeasureData&    measure,
...
void LocalizationIntegImpl::GetLastestLidarLocalization(LocalizationMeasureState* state,
                                                        LocalizationEstimate* lidar_localization) {
  CHECK_NOTNULL(state);
...
void LocalizationIntegImpl::GetLastestIntegLocalization(LocalizationMeasureState* state,
                                                        LocalizationEstimate* integ_localization) {
  CHECK_NOTNULL(state);
...
void LocalizationIntegImpl::GetLastestGnssLocalization(LocalizationMeasureState* state,
                                                       LocalizationEstimate* gnss_localization) {
  CHECK_NOTNULL(state);
...
void LocalizationIntegImpl::GetLidarLocalizationList(std::list<LocalizationResult>* results) {
...
void LocalizationIntegImpl::GetIntegLocalizationList(std::list<LocalizationResult>* results) {
  CHECK_NOTNULL(results);
...
void LocalizationIntegImpl::GetGnssLocalizationList(std::list<LocalizationResult>* results) {
  CHECK_NOTNULL(results);
