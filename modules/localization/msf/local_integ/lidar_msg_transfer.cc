...
void LidarMsgTransfer::Transfer(const sensor_msgs::PointCloud2& message, LidarFrame* lidar_frame) {
...
void LidarMsgTransfer::TransferCloud(const sensor_msgs::PointCloud2& lidar_data,
  ...
  if (FLAGS_lidar_debug_log_flag) {
    AINFO << std::setprecision(16) << "LidarMsgTransfer Debug Log: lidar msg. "
          ...
          << "[point_cnt:" << x_count_ << "]"
          << "[intensity_cnt:" << static_cast<unsigned int>(lidar_frame->intensities.size()) << "]";
  }
  return;
}

void LidarMsgTransfer::ParseCloudField(const sensor_msgs::PointCloud2& lidar_data) {
...
void LidarMsgTransfer::TransferOrganizedCloud32(const sensor_msgs::PointCloud2& lidar_data,
                                                LidarFrame*                     lidar_frame) {
  CHECK_NOTNULL(lidar_frame);
  ...
  for (uint32_t i = 0; i < lidar_data.height; ++i) {
    for (uint32_t j = 0; j < lidar_data.width; ++j) {
      ...
      pt3d[0]                = static_cast<const double>(
          *reinterpret_cast<const float*>(&lidar_data.data[offset + x_offset_]));
      pt3d[1] = static_cast<const double>(
          *reinterpret_cast<const float*>(&lidar_data.data[offset + y_offset_]));
      pt3d[2] = static_cast<const double>(
          *reinterpret_cast<const float*>(&lidar_data.data[offset + z_offset_]));
      if (!std::isnan(pt3d[0])) {
        unsigned char intensity =
            *reinterpret_cast<const unsigned char*>(&lidar_data.data[offset + i_offset_]);
...
void LidarMsgTransfer::TransferOrganizedCloud64(const sensor_msgs::PointCloud2& lidar_data,
                                                LidarFrame*                     lidar_frame) {
  CHECK_NOTNULL(lidar_frame);
  ...
  for (uint32_t i = 0; i < lidar_data.height; ++i) {
    for (uint32_t j = 0; j < lidar_data.width; ++j) {
      ...
      Eigen::Vector3d pt3d;
      uint32_t        offset = index * lidar_data.point_step;
      pt3d[0] = *reinterpret_cast<const double*>(&lidar_data.data[offset + x_offset_]);
      pt3d[1] = *reinterpret_cast<const double*>(&lidar_data.data[offset + y_offset_]);
      pt3d[2] = *reinterpret_cast<const double*>(&lidar_data.data[offset + z_offset_]);
      if (!std::isnan(pt3d[0])) {
        unsigned char intensity =
            *reinterpret_cast<const unsigned char*>(&lidar_data.data[offset + i_offset_]);
        lidar_frame->pt_xs.push_back(pt3d[0]);
...
void LidarMsgTransfer::TransferUnorganizedCloud32(const sensor_msgs::PointCloud2& lidar_data,
                                                  LidarFrame*                     lidar_frame) {
  CHECK_NOTNULL(lidar_frame);
...
void LidarMsgTransfer::TransferUnorganizedCloud64(const sensor_msgs::PointCloud2& lidar_data,
                                                  LidarFrame*                     lidar_frame) {
  CHECK_NOTNULL(lidar_frame);
