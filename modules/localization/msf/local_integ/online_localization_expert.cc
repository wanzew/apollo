...
void OnlineLocalizationExpert::AddFusionLocalization(const LocalizationEstimate& data) {
  SetLocalizationStatus(data);
}

void OnlineLocalizationExpert::AddLidarLocalization(const LocalizationEstimate& data) {
...
void OnlineLocalizationExpert::AddGnssBestPose(const drivers::gnss::GnssBestPose& msg,
...
void OnlineLocalizationExpert::CheckImuDelayStatus(const double& cur_imu_time) {
  ...
  if (delta_system_time > imu_delay_time_threshold_1_) {
    ADEBUG << std::setprecision(16) << "the imu delays " << delta_system_time
           << ", the current system time is " << cur_system_time << ", the imu timestamp is "
           << cur_imu_time;
...
void OnlineLocalizationExpert::CheckImuMissingStatus(const double& cur_imu_time) {
  sensor_status_.set_imu_missing_status(ImuMsgMissingStatus::IMU_MISSING_NORMAL);
  ...
  if (delta_time > imu_missing_time_threshold_1_) {
    ...
    ADEBUG << std::setprecision(16) << "Received imu message's time "
           ...
           << "the pre time and current time: " << pre_imu_time << " " << cur_imu_time;
    sensor_status_.set_imu_missing_status(ImuMsgMissingStatus::IMU_MISSING_ABNORMAL);
  }
  pre_imu_time = cur_imu_time;
}

void OnlineLocalizationExpert::CheckGnssLidarMsfStatus(const double& cur_imu_time) {
  std::lock_guard<std::mutex> lock(msf_status_mutex_);
  latest_gnsspos_timestamp_mutex_.lock();
  if (cur_imu_time - latest_gnsspos_timestamp_ > bestgnsspose_loss_time_threshold_) {
    AINFO << std::setprecision(15) << "The bestgnsspose msg loses more than 2 seconds: "
          << "imu time and latest gnss time: " << cur_imu_time << " " << latest_gnsspos_timestamp_;
  ...
  if (cur_imu_time - latest_lidar_timestamp_ > lidar_loss_time_threshold_) {
    AINFO << std::setprecision(15) << "The local lidar msg loses more than 2 seconds: "
...
void OnlineLocalizationExpert::SetLocalizationStatus(const LocalizationEstimate& data) {
  ...
  if (position_std.x() < localization_std_x_threshold_1_ &&
    ...
    switch (msf_status_.gnsspos_position_type()) {
      ...
      case apollo::localization::L1_INT:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SOL_LIDAR_GNSS);
          integ_status_.integ_state   = LocalizationIntegState::OK;
          integ_status_.state_message = "";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SOL_X_GNSS);
          integ_status_.integ_state   = LocalizationIntegState::WARNNING;
          integ_status_.state_message = "Warning: Lidar Localization Is Abnormal";
        }
        break;
      ...
      case apollo::localization::NARROW_FLOAT:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SOL_LIDAR_X);
          integ_status_.integ_state   = LocalizationIntegState::WARNNING;
          integ_status_.state_message = "Warning: GNSS Localization Is Unstable";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SOL_X_X);
          integ_status_.integ_state   = LocalizationIntegState::ERROR;
          integ_status_.state_message = "ERROR: Both GNSS and Lidar Localization Is Unstable";
        }
        break;
      case apollo::localization::SINGLE:
      case apollo::localization::PSRDIFF:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SOL_LIDAR_XX);
          integ_status_.integ_state   = LocalizationIntegState::WARNNING;
          integ_status_.state_message = "Warning: GNSS Localization Is Abnormal";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SOL_X_XX);
          integ_status_.integ_state   = LocalizationIntegState::ERROR;
          integ_status_.state_message = "ERROR: Both GNSS and Lidar Localization Is Abnormal";
        }
        break;
      default:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SOL_LIDAR_XXX);
          integ_status_.integ_state   = LocalizationIntegState::WARNNING;
          integ_status_.state_message = "WARNNING: GNSS Localization Is Invalid";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SOL_X_XXX);
          integ_status_.integ_state   = LocalizationIntegState::ERROR;
          integ_status_.state_message = "ERROR: Both GNSS and Lidar Localization Is Invalid";
        }
        break;
    }
    ...
    switch (msf_status_.gnsspos_position_type()) {
      ...
      case apollo::localization::L1_INT:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SSOL_LIDAR_GNSS);
          integ_status_.integ_state   = LocalizationIntegState::CRITIAL_ERROR;
          integ_status_.state_message = "CRITIAL_ERROR: Current Localization Is Unstable";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SSOL_X_GNSS);
          integ_status_.integ_state   = LocalizationIntegState::CRITIAL_ERROR;
          integ_status_.state_message = "CRITIAL_ERROR: Current Localization Is Unstable, "
      ...
      case apollo::localization::NARROW_FLOAT:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SSOL_LIDAR_X);
          integ_status_.integ_state   = LocalizationIntegState::CRITIAL_ERROR;
          integ_status_.state_message = "CRITIAL_ERROR: Current Localization Is Unstable, "
                                        "GNSS Localization is Abnormal";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SSOL_X_X);
          integ_status_.integ_state   = LocalizationIntegState::CRITIAL_ERROR;
          integ_status_.state_message = "CRITIAL_ERROR: Current Localization Is Unstable, "
      ...
      case apollo::localization::PSRDIFF:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SSOL_LIDAR_XX);
          integ_status_.integ_state   = LocalizationIntegState::CRITIAL_ERROR;
          integ_status_.state_message = "CRITIAL_ERROR: Current Localization Is Unstable, "
                                        "GNSS Localization Is Abnormal";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SSOL_X_XX);
          integ_status_.integ_state   = LocalizationIntegState::CRITIAL_ERROR;
          integ_status_.state_message = "CRITIAL_ERROR: Current Localization Is Unstable, "
                                        "Both GNSS And Lidar Localization Is Abnormal";
        }
      ...
      default:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SSOL_LIDAR_XXX);
          integ_status_.integ_state   = LocalizationIntegState::CRITIAL_ERROR;
          integ_status_.state_message = "CRITIAL_ERROR: Current Localization Is Unstable, "
                                        "GNSS Localization Is Abnormal";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_SSOL_X_XXX);
          integ_status_.integ_state   = LocalizationIntegState::CRITIAL_ERROR;
          integ_status_.state_message = "CRITIAL_ERROR: Current Localization Is Unstable, "
                                        "Both GNSS And Lidar Localization Is Abnormal";
        }
    ...
    switch (msf_status_.gnsspos_position_type()) {
      ...
      case apollo::localization::L1_INT:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_NOSOL_LIDAR_GNSS);
          integ_status_.integ_state   = LocalizationIntegState::FATAL_ERROR;
          integ_status_.state_message = "FATAL_ERROR: Current Localization Is NOT Available";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_NOSOL_X_GNSS);
          integ_status_.integ_state   = LocalizationIntegState::FATAL_ERROR;
          integ_status_.state_message = "FATAL_ERROR: Current Localization Is NOT Available, "
                                        "Lidar Localization Is Abnormal";
        }
      ...
      case apollo::localization::NARROW_FLOAT:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_NOSOL_LIDAR_X);
          integ_status_.integ_state   = LocalizationIntegState::FATAL_ERROR;
          integ_status_.state_message = "FATAL_ERROR: Current Localization Is NOT Available, "
                                        "GNSS Localization Is Unstable";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_NOSOL_X_X);
          integ_status_.integ_state   = LocalizationIntegState::FATAL_ERROR;
          integ_status_.state_message = "FATAL_ERROR: Current Localization Is NOT Available, "
      ...
      case apollo::localization::PSRDIFF:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_NOSOL_LIDAR_XX);
          integ_status_.integ_state   = LocalizationIntegState::FATAL_ERROR;
          integ_status_.state_message = "FATAL_ERROR: Current Localization Is NOT Available, "
                                        "GNSS Localization Is Abnormal";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_NOSOL_X_XX);
          integ_status_.integ_state   = LocalizationIntegState::FATAL_ERROR;
          integ_status_.state_message = "FATAL_ERROR: Current Localization Is NOT Available, "
                                        "Both GNSS And Lidar Localization Is Abnormal";
        }
      ...
      default:
        if (msf_status_.local_lidar_status() == apollo::localization::MSF_LOCAL_LIDAR_NORMAL) {
          msf_status_.set_msf_running_status(apollo::localization::MSF_NOSOL_LIDAR_XXX);
          integ_status_.integ_state   = LocalizationIntegState::FATAL_ERROR;
          integ_status_.state_message = "FATAL_ERROR: Current Localization Is NOT Available, "
                                        "GNSS Localization Is Invalid";
        } else {
          msf_status_.set_msf_running_status(apollo::localization::MSF_NOSOL_X_XXX);
          integ_status_.integ_state   = LocalizationIntegState::FATAL_ERROR;
          integ_status_.state_message = "FATAL_ERROR: Current Localization Is NOT Available, "
...
void OnlineLocalizationExpert::GetFusionStatus(MsfStatus*               msf_status,
                                               MsfSensorMsgStatus*      sensor_status,
                                               LocalizationIntegStatus* integ_status) {
  {
    std::unique_lock<std::mutex> lock(msf_status_mutex_);
    msf_status->set_local_lidar_consistency(msf_status_.local_lidar_consistency());
