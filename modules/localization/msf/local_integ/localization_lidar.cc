...
bool LocalizationLidar::Init(const std::string& map_path,
  ...
  lidar_locator_->Init(search_range_x_, search_range_y_, static_cast<float>(resolution_),
...
void LocalizationLidar::SetVelodyneExtrinsic(const Eigen::Affine3d& pose) {
  ...
  lidar_locator_->SetVelodyneExtrinsic(trans.x(), trans.y(), trans.z(), quat.x(), quat.y(),
                                       quat.z(), quat.w());
}
...
void LocalizationLidar::SetImageAlignMode(int mode) { lidar_locator_->SetImageAlignMode(mode); }

void LocalizationLidar::SetLocalizationMode(int mode) { lidar_locator_->SetLocalizationMode(mode); }

void LocalizationLidar::SetDeltaYawLimit(double limit) { lidar_locator_->SetDeltaYawLimit(limit); }
...
int LocalizationLidar::Update(const unsigned int     frame_idx,
  ...
  if (!is_map_loaded_) {
    map_.LoadMapArea(pose.translation(), resolution_id_, zone_id_, search_range_x_,
                     search_range_y_);
    is_map_loaded_ = true;
  ...
  lidar_locator_->SetMapNodeData(node_width, node_height, node_level_num,
                                 &(lidar_map_node_->intensities),
                                 &(lidar_map_node_->intensities_var), &(lidar_map_node_->altitudes),
                                 &(lidar_map_node_->count));
  lidar_locator_->SetMapNodeLeftTopCorner(map_left_top_corner_(0), map_left_top_corner_(1));

  // pass lidar points to locator
  int size = static_cast<int>(lidar_frame.pt_xs.size());
  lidar_locator_->SetPointCloudData(size, lidar_frame.pt_xs.data(), lidar_frame.pt_ys.data(),
                                    lidar_frame.pt_zs.data(), lidar_frame.intensities.data());

  // compute
  int error = lidar_locator_->Compute(pose_trans(0), pose_trans(1), pose_trans(2), pose_quat.x(),
                                      pose_quat.y(), pose_quat.z(), pose_quat.w(), use_avx);
...
void LocalizationLidar::RefineAltitudeFromMap(Eigen::Affine3d* pose) {
  ...
  MapNodeIndex index =
      MapNodeIndex::GetMapNodeIndex(map_.GetMapConfig(), lidar_trans, resolution_id_, zone_id_);
  PyramidMapNode*   node   = static_cast<PyramidMapNode*>(map_.GetMapNodeSafe(index));
  PyramidMapMatrix& matrix = static_cast<PyramidMapMatrix&>(node->GetMapCellMatrix());
  ...
  if (node->GetCoordinate(lidar_trans, &x, &y)) {
    ...
    if (count > 0) {
      if (matrix.HasGroundAltitude()) {
        ...
        if (ground_alt) { vehicle_ground_alt = *ground_alt; }
      } else {
        const float* altitude = matrix.GetAltitudeSafe(y, x);
        if (altitude) { vehicle_ground_alt = *altitude; }
      }
    } else {
...
void LocalizationLidar::ComposeMapNode(const Eigen::Vector3d& trans) {
  ...
  // top left corner
  map_node_idx[0][0] =
      MapNodeIndex::GetMapNodeIndex(map_.GetMapConfig(), left_top_corner, resolution_id_, zone_id_);
  // top right corner
  ...
  for (unsigned int y = 0; y < 2; ++y) {
    for (unsigned int x = 0; x < 2; ++x) {
      map_node[y][x] = static_cast<PyramidMapNode*>(map_.GetMapNodeSafe(map_node_idx[y][x]));
    }
  }
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      for (int y = 0; y < range_y; ++y) {
        ...
        for (int x = 0; x < range_x; ++x) {
          ...
          lidar_map_node_->intensities[dst_idx]     = (*intensity_matrix)[src_y + y][src_x + x];
          lidar_map_node_->intensities_var[dst_idx] = (*intensity_var_matrix)[src_y + y][src_x + x];
          lidar_map_node_->altitudes[dst_idx]       = (*altitude_matrix)[src_y + y][src_x + x];
          lidar_map_node_->count[dst_idx]           = (*count_matrix)[src_y + y][src_x + x];
        }
      }
    }
  }
