...
bool LocalizationLidar::Init(const std::string& map_path,
  ...
  lidar_locator_->Init(search_range_x_, search_range_y_, resolution_, node_size_x_, node_size_y_);
  return true;
}
...
int LocalizationLidar::Update(const unsigned int     frame_idx,
  ...
  int error = lidar_locator_->Compute(pose_trans(0), pose_trans(1), pose_trans(2), pose_quat.x(),
                                      pose_quat.y(), pose_quat.z(), pose_quat.w());
...
void LocalizationLidar::RefineAltitudeFromMap(Eigen::Affine3d* pose) {
  ...
  MapNodeIndex index =
      MapNodeIndex::GetMapNodeIndex(map_.GetConfig(), lidar_trans, resolution_id_, zone_id_);
  LossyMapNode*   node   = static_cast<LossyMapNode*>(map_.GetMapNodeSafe(index));
  LossyMapMatrix& matrix = static_cast<LossyMapMatrix&>(node->GetMapCellMatrix());
...
void LocalizationLidar::ComposeMapNode(const Eigen::Vector3d& trans) {
  ...
  // top left corner
  map_node_idx[0][0] =
      MapNodeIndex::GetMapNodeIndex(map_.GetConfig(), left_top_corner, resolution_id_, zone_id_);
  // top right corner
  ...
  for (unsigned int y = 0; y < 2; ++y) {
    for (unsigned int x = 0; x < 2; ++x) {
      map_node[y][x] = static_cast<LossyMapNode*>(map_.GetMapNodeSafe(map_node_idx[y][x]));
    }
  }
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 2; ++j) {
      ...
      LossyMapMatrix& map_cells = static_cast<LossyMapMatrix&>(map_node[i][j]->GetMapCellMatrix());
