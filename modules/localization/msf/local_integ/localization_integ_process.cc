...
void LocalizationIntegProcess::RawImuProcess(const ImuData& imu_msg) {
  ...
  if (delta_time > 0.1) {
    ...
    ADEBUG << std::setprecision(16) << "received imu message's time is eary than last imu message, "
           << "the pre time and current time: " << pre_imu_time << " " << cur_imu_time;
  }
...
void LocalizationIntegProcess::GetResult(IntegState* state, LocalizationEstimate* localization) {
  ...
  *state = integ_state_;
  ...
  if (debug_log_flag_ && *state != IntegState::NOT_INIT) {
    AINFO << std::setprecision(16) << "IntegratedLocalization Debug Log: integ_pose msg: "
          << "[time:" << ins_pva_.time << "]"
...
bool LocalizationIntegProcess::CheckIntegMeasureData(const MeasureData& measure_data) {
  ...
  if (debug_log_flag_) {
    AINFO << std::setprecision(16) << "IntegratedLocalization Debug Log: measure data: "
