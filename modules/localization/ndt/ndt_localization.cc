...
void NDTLocalization::Init() {
  ...
  std::string map_path_ = FLAGS_map_dir + "/" + FLAGS_ndt_map_dir + "/" + FLAGS_local_map_name;
  ...
  AINFO << "lidar extrinsics: " << velodyne_extrinsic_.translation().x() << ", "
        << velodyne_extrinsic_.translation().y() << ", " << velodyne_extrinsic_.translation().z()
  ...
  if (FLAGS_if_utm_zone_id_from_folder) {
    ...
    if (!success) { AWARN << "Can't load utm zone id from map folder, use default value."; }
  }
  ...
  lidar_locator_.SetOnlineCloudResolution(static_cast<float>(online_resolution_));
...
void NDTLocalization::OdometryCallback(const std::shared_ptr<localization::Gps>& odometry_msg) {
  ...
  if (time_delay > 0.1) {
    ...
    AINFO << "Odometry message's time is earlier than last one, "
          << "the pre time and cur time: " << pre_odometry_time << ", " << odometry_time;
  }
  pre_odometry_time             = odometry_time;
  Eigen::Affine3d odometry_pose = Eigen::Affine3d::Identity();
  if (odometry_msg->has_localization()) {
    odometry_pose.translation()[0] = odometry_msg->localization().position().x();
    odometry_pose.translation()[1] = odometry_msg->localization().position().y();
    odometry_pose.translation()[2] = odometry_msg->localization().position().z();
    Eigen::Quaterniond tmp_quat(odometry_msg->localization().orientation().qw(),
  ...
  if (ndt_debug_log_flag_) {
    AINFO << "NDTLocalization Debug Log: odometry msg: " << std::setprecision(15)
          << "time: " << odometry_time << ", "
  ...
  if (lidar_locator_.IsInitialized()) {
    localization_pose = pose_buffer_.UpdateOdometryPose(odometry_time, odometry_pose);
  }
  ComposeLocalizationEstimate(localization_pose, odometry_msg, &localization_result_);
...
void NDTLocalization::LidarCallback(const std::shared_ptr<drivers::PointCloud>& lidar_msg) {
  ...
  if (ndt_debug_log_flag_) {
    ...
    AINFO << "NDTLocalization Debug Log: odometry for lidar pose: " << std::setprecision(15)
          << "time: " << time_stamp << ", "
...
void NDTLocalization::GetLocalization(LocalizationEstimate* localization) const {
  *localization = localization_result_;
}

void NDTLocalization::GetLidarLocalization(LocalizationEstimate* localization) const {
  *localization = lidar_localization_result_;
}

void NDTLocalization::GetLocalizationStatus(LocalizationStatus* localization_status) const {
  *localization_status = localization_status_;
}

bool NDTLocalization::IsServiceStarted() { return is_service_started_; }

void NDTLocalization::FillLocalizationMsgHeader(LocalizationEstimate* localization) {
...
void NDTLocalization::ComposeLocalizationEstimate(
  ...
  double heading = common::math::QuaternionToHeading(quat.w(), quat.x(), quat.y(), quat.z());
...
bool NDTLocalization::QueryPoseFromTF(double time, Eigen::Affine3d* pose) {
  ...
  std::string err_msg       = "";
  bool        can_transform = tf_buffer_->canTransform(tf_target_frame_id_, tf_source_frame_id_,
                                                query_time, time_out, &err_msg);
  if (!can_transform) {
  ...
  try {
    query_transform =
        tf_buffer_->lookupTransform(tf_target_frame_id_, tf_source_frame_id_, query_time);
  ...
  Eigen::Quaterniond tmp_quat(
      query_transform.transform().rotation().qw(), query_transform.transform().rotation().qx(),
      query_transform.transform().rotation().qy(), query_transform.transform().rotation().qz());
  pose->linear() = tmp_quat.toRotationMatrix();
  return true;
}

void NDTLocalization::ComposeLocalizationStatus(const drivers::gnss::InsStat& status,
  ...
  if (ndt_score_ < warnning_ndt_score_ && pos_type == drivers::gnss::SolutionType::INS_RTKFIXED) {
    localization_status->set_fusion_status(MeasureState::OK);
    localization_status->set_state_message("");
  } else if (bad_score_count_ > bad_score_count_threshold_ || ndt_score_ > error_ndt_score_ ||
...
bool NDTLocalization::QueryPoseFromBuffer(double time, Eigen::Affine3d* pose) {
  ...
  // interpolation
  double v1           = (next_pose.timestamp - time) / (next_pose.timestamp - pre_pose.timestamp);
  double v2           = (time - pre_pose.timestamp) / (next_pose.timestamp - pre_pose.timestamp);
  pose->translation() = pre_pose.pose.translation() * v1 + next_pose.pose.translation() * v2;

  Eigen::Quaterniond pre_quat(pre_pose.pose.linear());

  common::math::EulerAnglesZXYd pre_euler(pre_quat.w(), pre_quat.x(), pre_quat.y(), pre_quat.z());

  Eigen::Quaterniond            next_quat(next_pose.pose.linear());
  common::math::EulerAnglesZXYd next_euler(next_quat.w(), next_quat.x(), next_quat.y(),
                                           next_quat.z());
...
bool NDTLocalization::ZeroOdometry(const Eigen::Affine3d& pose) {
  ...
  if (norm <= 0.01) { return true; }
  return false;
}

void NDTLocalization::LidarMsgTransfer(const std::shared_ptr<drivers::PointCloud>& msg,
                                       LidarFrame*                                 lidar_frame) {
  ...
  if (ndt_debug_log_flag_) {
    AINFO << std::setprecision(15) << "NDTLocalization Debug Log: velodyne msg. "
...
bool NDTLocalization::LoadLidarExtrinsic(const std::string& file_path,
  ...
  if (config["transform"]) {
    if (config["transform"]["translation"]) {
      lidar_extrinsic->translation()(0) = config["transform"]["translation"]["x"].as<double>();
      lidar_extrinsic->translation()(1) = config["transform"]["translation"]["y"].as<double>();
      lidar_extrinsic->translation()(2) = config["transform"]["translation"]["z"].as<double>();
      if (config["transform"]["rotation"]) {
        ...
        lidar_extrinsic->linear() = Eigen::Quaterniond(qw, qx, qy, qz).toRotationMatrix();
        return true;
      }
    }
  }
...
bool NDTLocalization::LoadLidarHeight(const std::string& file_path, LidarHeight* height) {
  CHECK_NOTNULL(height);

  if (!cyber::common::PathExists(file_path)) { return false; }

  YAML::Node config = YAML::LoadFile(file_path);
  if (config["vehicle"]) {
    if (config["vehicle"]["parameters"]) {
      height->height     = config["vehicle"]["parameters"]["height"].as<double>();
      height->height_var = config["vehicle"]["parameters"]["height_var"].as<double>();
      return true;
    }
  }
  return false;
...
bool NDTLocalization::LoadZoneIdFromFolder(const std::string& folder_path, int* zone_id) {
...
bool NDTLocalization::FindNearestOdometryStatus(const double            odometry_timestamp,
  ...
  for (auto itr = odometry_status_list.begin(); itr != odometry_status_list.end(); ++itr) {
  ...
  if (nearest_itr == odometry_status_list.end()) { return false; }

  if (timestamp_diff_sec > odometry_status_time_diff_threshold_) { return false; }
