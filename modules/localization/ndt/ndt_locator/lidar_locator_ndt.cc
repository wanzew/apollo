...
void LidarLocatorNdt::Init(const Eigen::Affine3d& init_location,
  ...
  if (!is_map_loaded_) {
    ...
    map_.LoadMapArea(location_.translation(), resolution_id_, zone_id_, filter_x_, filter_y_);
    AINFO << "Locator map pre-loading is done.";
    is_map_loaded_ = true;
  }

  // set filter
  filter_x_ = static_cast<int>(static_cast<float>(FLAGS_ndt_filter_size_x) /
                               map_.GetMapConfig().map_resolutions_[resolution_id_]);
  filter_y_ = static_cast<int>(static_cast<float>(FLAGS_ndt_filter_size_y) /
                               map_.GetMapConfig().map_resolutions_[resolution_id_]);
...
void LidarLocatorNdt::LoadMap(const Eigen::Affine3d& init_location,
  ...
  map_.InitMapNodeCaches(12, 24);
  map_.AttachMapNodePool(&map_preload_node_pool_);
  map_.LoadMapArea(location_.translation(), resolution_id, zone_id, filter_x_, filter_y_);
  AINFO << "Locator map pre-loading is done.";
  is_map_loaded_ = true;
}

void LidarLocatorNdt::SetMapFolderPath(const std::string folder_path) {
  if (!map_.SetMapFolderPath(folder_path)) { AERROR << "Map folder is invalid!"; }
}
...
int LidarLocatorNdt::Update(unsigned int           frame_idx,
                            const Eigen::Affine3d& pose,
                            const LidarFrame&      lidar_frame) {
  // Increasement from INSPVA
  Eigen::Vector3d      trans_diff      = pose.translation() - pre_input_location_.translation();
  Eigen::Vector3d      trans_pre_local = pre_estimate_location_.translation() + trans_diff;
  ...
  AINFO << "original pose: " << std::setprecision(15) << pose.translation()[0] << ", "
        << pose.translation()[1] << ", " << pose.translation()[2] << ", " << pose_qbn.x() << ", "
        << pose_qbn.y() << ", " << pose_qbn.z() << ", " << pose_qbn.w();
...
// Pre-load the map nodes
  ...
  bool map_is_ready =
      map_.LoadMapArea(center_pose.translation(), resolution_id_, zone_id_, filter_x_, filter_y_);
  map_.PreloadMapArea(center_pose.translation(), trans_diff, resolution_id_, zone_id_);
#endif
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr online_points(new pcl::PointCloud<pcl::PointXYZ>());
  for (unsigned int i = 0; i < lidar_frame.pt_xs.size(); ++i) {
    pcl::PointXYZ p(lidar_frame.pt_xs[i], lidar_frame.pt_ys[i], lidar_frame.pt_zs[i]);
    online_points->push_back(p);
  }

  // Filter online points
  AINFO << "Online point cloud leaf size: " << proj_reslution_;
  pcl::PointCloud<pcl::PointXYZ>::Ptr online_points_filtered(new pcl::PointCloud<pcl::PointXYZ>());
  ...
  ComposeMapCells(left_top_coord2d, zone_id_, resolution_id_,
                  map_.GetMapConfig().map_resolutions_[resolution_id_], transform.inverse());

  // Convert map pointcloud to local corrdinate
  pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_map_point_cloud(new pcl::PointCloud<pcl::PointXYZ>());
...
Eigen::Matrix3d LidarLocatorNdt::GetLocationCovariance() const { return location_covariance_; }

void LidarLocatorNdt::ComposeMapCells(const Eigen::Vector2d& left_top_coord2d,
                                      int                    zone_id,
                                      unsigned int           resolution_id,
  ...
  MapNodeIndex map_id =
      MapNodeIndex::GetMapNodeIndex(map_.GetMapConfig(), coord2d, resolution_id, zone_id);
  NdtMapNode* map_node_lt = dynamic_cast<NdtMapNode*>(map_.GetMapNodeSafe(map_id));
  ...
  int top_center_end_x =
      static_cast<int>(map_coord_x) - 1 + 2 * static_cast<int>(filter_size_x / 2);
  ...
  int top_right_end_x =
      2 * static_cast<int>(filter_size_x / 2) - (top_left_end_x - top_left_start_x + 1) - 1;
  ...
  int middle_left_end_y =
      static_cast<int>(map_coord_y) - 1 + 2 * static_cast<int>(filter_size_y / 2);
  ...
  int bottom_left_end_y =
      2 * static_cast<int>(filter_size_y / 2) - (top_left_end_y - top_left_start_y + 1) - 1;
  node_zone.clear();
  ...
  start_x_indices[2] = start_x_indices[1] + top_center_end_x - top_center_start_x + 1;
  start_y_indices[0] = 0;
  start_y_indices[1] = top_left_end_y - top_left_start_y + 1;
  start_y_indices[2] = start_y_indices[1] + middle_center_end_y - middle_center_start_y + 1;

  std::vector<std::pair<int, int>> composed_map_indices;
  for (int y = 0; y < 3; ++y) {
    for (int x = 0; x < 3; ++x) {
      composed_map_indices.push_back(std::make_pair(start_x_indices[x], start_y_indices[y]));
    }
  }
    ...
    for (int x = 0; x < 3; ++x) {
      if (map_nodes_zones[y * 3 + x][2] - map_nodes_zones[y * 3 + x][0] >= 0 &&
        ...
        if (x == 0 && y == 0) {
          ...
          coord2d_xy[0] = coord2d_center[0] + static_cast<double>(x * map_node_size_x) *
                                                  static_cast<double>(map_pixel_resolution);
          coord2d_xy[1] = coord2d_center[1] + static_cast<double>(y * map_node_size_y) *
                                                  static_cast<double>(map_pixel_resolution);

          MapNodeIndex map_id      = MapNodeIndex::GetMapNodeIndex(map_.GetMapConfig(), coord2d_xy,
                                                              resolution_id, zone_id);
          NdtMapNode*  map_node_xy = dynamic_cast<NdtMapNode*>(map_.GetMapNodeSafe(map_id));
          assert(map_.IsMapNodeExist(map_id));
          map_node_ptr = map_node_xy;
        }

        // get map matrix
        NdtMapMatrix& map_cells = dynamic_cast<NdtMapMatrix&>(map_node_ptr->GetMapCellMatrix());

        // start obtain cells in MapNdtMatrix
        const Eigen::Vector2d& left_top_corner = map_node_ptr->GetLeftTopCorner();
        double                 resolution      = map_node_ptr->GetMapResolution();
        double                 resolution_z    = map_node_ptr->GetMapResolutionZ();
        for (int map_y = map_nodes_zones[y * 3 + x][1]; map_y <= map_nodes_zones[y * 3 + x][3];
             ++map_y) {
          for (int map_x = map_nodes_zones[y * 3 + x][0]; map_x <= map_nodes_zones[y * 3 + x][2];
               ++map_x) {
            ...
            if (cell_ndt.cells_.size() > 0) {
              for (auto it = cell_ndt.cells_.begin(); it != cell_ndt.cells_.end(); ++it) {
                ...
                if (cell_count >= 6) {
                  ...
                  eigen_point(0) =
                      left_top_corner[0] + map_x * resolution + it->second.centroid_[0];
                  eigen_point(1) =
                      left_top_corner[1] + map_y * resolution + it->second.centroid_[1];
                  eigen_point(2) = resolution_z * it->first + it->second.centroid_[2];
