...
void RTKLocalization::GpsCallback(const std::shared_ptr<localization::Gps>& gps_msg) {
...

  {
    ...
    if (imu_list_.empty()) {
      ...
      if (service_started_) { monitor_logger_.ERROR("IMU message buffer is empty."); }
      return;
    }
  }

  {
    ...
    if (gps_status_list_.empty()) {
      ...
      if (service_started_) { monitor_logger_.ERROR("Gps status message buffer is empty."); }
      return;
    }
  }

  // publish localization messages
  PrepareLocalizationMsg(*gps_msg, &last_localization_result_, &last_localization_status_result_);
  service_started_ = true;
  if (service_started_time == 0.0) { service_started_time = Clock::NowInSeconds(); }
...
void RTKLocalization::GpsStatusCallback(const std::shared_ptr<drivers::gnss::InsStat>& status_msg) {
  std::unique_lock<std::mutex> lock(gps_status_list_mutex_);
...
void RTKLocalization::GetLocalizationStatus(LocalizationStatus* localization_status) {
  *localization_status = last_localization_status_result_;
}

void RTKLocalization::RunWatchDog(double gps_timestamp) {
  if (!enable_watch_dog_) { return; }
  ...
  int64_t gps_delay_cycle_cnt = static_cast<int64_t>(gps_delay_sec * localization_publish_freq_);
  ...
  if (gps_delay_cycle_cnt > report_threshold_err_num_ &&
    ...
    msg_delay = true;
    std::stringstream ss;
    ss << "Raw GPS Message Delay. GPS message is " << gps_delay_cycle_cnt << " cycle "
  ...
  lock.unlock();
  double  imu_delay_sec       = Clock::NowInSeconds() - imu_msg.header().timestamp_sec();
  int64_t imu_delay_cycle_cnt = static_cast<int64_t>(imu_delay_sec * localization_publish_freq_);
  if (imu_delay_cycle_cnt > report_threshold_err_num_ &&
      static_cast<int>(gps_service_delay) > service_delay_threshold) {
    msg_delay = true;
    std::stringstream ss;
    ss << "Raw IMU Message Delay. IMU message is " << imu_delay_cycle_cnt << " cycle "
...
void RTKLocalization::PrepareLocalizationMsg(const localization::Gps& gps_msg,
...
void RTKLocalization::FillLocalizationStatusMsg(const drivers::gnss::InsStat& status,
                                                LocalizationStatus*           localization_status) {
...
bool RTKLocalization::InterpolateIMU(const CorrectedImu& imu1,
  ...
  if (timestamp_sec < imu1.header().timestamp_sec()) {
    AERROR << "[InterpolateIMU1]: the given time stamp[" << FORMAT_TIMESTAMP(timestamp_sec)
           << "] is older than the 1st message[" << FORMAT_TIMESTAMP(imu1.header().timestamp_sec())
           << "]";
    *imu_msg = imu1;
...
bool RTKLocalization::FindNearestGpsStatus(const double            gps_timestamp_sec,
  ...
  for (auto itr = gps_status_list.begin(); itr != gps_status_list.end(); ++itr) {
  ...
  if (nearest_itr == gps_status_list.end()) { return false; }

  if (timestamp_diff_sec > gps_status_time_diff_threshold_) { return false; }
