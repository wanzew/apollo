...
Status RTKLocalization::Start() {
  ...
  if (!AdapterManager::GetGps()) {
    buffer.ERROR() << "GPS input not initialized. Check file " << FLAGS_rtk_adapter_config_file;
    buffer.PrintLog();
...
void RTKLocalization::OnTimer(const ros::TimerEvent& event) {
  double time_delay = common::time::ToSecond(Clock::Now()) - last_received_timestamp_sec_;
  common::monitor::MonitorLogBuffer buffer(&monitor_logger_);
  if (FLAGS_enable_gps_timestamp && time_delay > FLAGS_gps_time_delay_tolerance) {
  ...
  if (AdapterManager::GetGps()->Empty()) {
    ...
    if (service_started_) { buffer.ERROR("GPS message buffer is empty."); }
    return;
  }
  if (AdapterManager::GetImu()->Empty()) {
    AERROR << "IMU message buffer is empty.";
    if (service_started_) { buffer.ERROR("IMU message buffer is empty."); }
    return;
  }
...
template <class T>
T RTKLocalization::InterpolateXYZ(const T& p1, const T& p2, const double frac1) {
  T      p;
...
bool RTKLocalization::FindMatchingIMU(const double gps_timestamp_sec, CorrectedImu* imu_msg) {
  ...
  for (; imu_it != imu_adapter->end(); ++imu_it) {
    if ((*imu_it)->header().timestamp_sec() - gps_timestamp_sec > FLAGS_timestamp_sec_tolerance) {
      break;
    }
  }
  ...
  if (imu_it != imu_adapter->end()) {  // found one
    if (imu_it == imu_adapter->begin()) {
      AERROR << "IMU queue too short or request too old. "
             << "Oldest timestamp[" << imu_adapter->GetOldestObserved().header().timestamp_sec()
             << "], Newest timestamp[" << imu_adapter->GetLatestObserved().header().timestamp_sec()
...
bool RTKLocalization::InterpolateIMU(const CorrectedImu& imu1,
  ...
  if (!(imu1.has_header() && imu1.header().has_timestamp_sec() && imu2.has_header() &&
        imu2.header().has_timestamp_sec())) {
    AERROR << "imu1 and imu2 has no header or no timestamp_sec in header";
    return false;
  }
  if (timestamp_sec - imu1.header().timestamp_sec() < FLAGS_timestamp_sec_tolerance) {
    AERROR << "[InterpolateIMU]: the given time stamp[" << timestamp_sec
           << "] is older than the 1st message[" << imu1.header().timestamp_sec() << "]";
    *imu_msg = imu1;
  } else if (timestamp_sec - imu2.header().timestamp_sec() > FLAGS_timestamp_sec_tolerance) {
    AERROR << "[InterpolateIMU]: the given time stamp[" << timestamp_sec
           << "] is newer than the 2nd message[" << imu2.header().timestamp_sec() << "]";
    *imu_msg = imu1;
  } else {
    *imu_msg = imu1;
    ...
    if (fabs(time_diff) >= 0.001) {
      ...
      if (imu1.has_imu() && imu1.imu().has_angular_velocity() && imu2.has_imu() &&
          imu2.imu().has_angular_velocity()) {
        auto val =
            InterpolateXYZ(imu1.imu().angular_velocity(), imu2.imu().angular_velocity(), frac1);
        imu_msg->mutable_imu()->mutable_angular_velocity()->CopyFrom(val);
      }

      if (imu1.has_imu() && imu1.imu().has_linear_acceleration() && imu2.has_imu() &&
...
void RTKLocalization::PrepareLocalizationMsg(LocalizationEstimate* localization) {
  ...
  if (FLAGS_enable_gps_imu_interprolate) {
    ...
    if (!FindMatchingIMU(gps_time_stamp, &imu_msg)) { imu_valid = false; }
  } else {
    imu_msg = AdapterManager::GetImu()->GetLatestObserved();
  }

  if (imu_valid && fabs(gps_msg.header().timestamp_sec() - imu_msg.header().timestamp_sec()) >
                       FLAGS_gps_imu_timestamp_sec_diff_tolerance) {
    // not the same time stamp, 20ms threshold
    AERROR << "[PrepareLocalizationMsg]: time stamp of GPS[" << gps_msg.header().timestamp_sec()
           << "] is different from timestamp of IMU[" << imu_msg.header().timestamp_sec() << "]";
  }

  ComposeLocalizationMsg(gps_msg, imu_msg, localization);
}

void RTKLocalization::ComposeLocalizationMsg(const localization::Gps&          gps_msg,
                                             const localization::CorrectedImu& imu_msg,
                                             LocalizationEstimate*             localization) {
  localization->Clear();

  // header
  AdapterManager::FillLocalizationHeader(FLAGS_localization_module_name, localization);
  if (FLAGS_enable_gps_timestamp) {
    // copy time stamp, do NOT use Clock::Now()
    localization->mutable_header()->set_timestamp_sec(gps_msg.header().timestamp_sec());
  }
  ...
  if (gps_msg.has_localization()) {
    ...
    if (pose.has_orientation()) {
      ...
      double heading =
          common::math::QuaternionToHeading(pose.orientation().qw(), pose.orientation().qx(),
    ...
    if (imu.has_linear_acceleration()) {
      if (FLAGS_enable_map_reference_unify) {
        if (localization->pose().has_orientation()) {
          ...
          Vector3d orig(imu.linear_acceleration().x(), imu.linear_acceleration().y(),
        ...
        mutable_pose->mutable_linear_acceleration()->CopyFrom(imu.linear_acceleration());
      }
    }
    ...
    if (imu.has_angular_velocity()) {
      if (FLAGS_enable_map_reference_unify) {
        if (localization->pose().has_orientation()) {
          ...
          // angular_velocity_vf
          mutable_pose->mutable_angular_velocity_vrf()->CopyFrom(imu.angular_velocity());
        } else {
          AERROR << "[PrepareLocalizationMsg]: "
                 << "fail to convert angular_velocity";
        }
      } else {
        mutable_pose->mutable_angular_velocity()->CopyFrom(imu.angular_velocity());
      }
    }
...
void RTKLocalization::RunWatchDog() {
  ...
  bool msg_lost = false;
  if (FLAGS_enable_gps_timestamp && (gps_delay_cycle_cnt > FLAGS_report_threshold_err_num)) {
    msg_lost = true;

    buffer.ERROR() << "Raw GPS Message Lost. GPS message is " << gps_delay_cycle_cnt << " cycle "
                   << gps_delay_sec << " sec behind current time.";
    buffer.PrintLog();
  }

  // check IMU time stamp against ROS timer
  double imu_delay_sec = common::time::ToSecond(Clock::Now()) -
  ...
  if (FLAGS_enable_gps_timestamp && imu_delay_cycle_cnt > FLAGS_report_threshold_err_num) {
    msg_lost = true;

    buffer.ERROR() << "Raw GPS Message Lost. IMU message is " << imu_delay_cycle_cnt << " cycle "
                   << imu_delay_sec << " sec behind current time.";
    buffer.PrintLog();
  }

  // to prevent it from beeping continuously
  if (msg_lost && (last_reported_timestamp_sec_ < 1. ||
                   common::time::ToSecond(Clock::Now()) > last_reported_timestamp_sec_ + 1.)) {
