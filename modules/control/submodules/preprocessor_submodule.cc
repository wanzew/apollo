...
std::string PreprocessorSubmodule::Name() const { return FLAGS_preprocessor_submodule_name; }
...
bool PreprocessorSubmodule::Init() {
  ...
  // Preprocessor writer
  preprocessor_writer_ = node_->CreateWriter<Preprocessor>(FLAGS_control_preprocessor_topic);
...
bool PreprocessorSubmodule::Proc(const std::shared_ptr<LocalView>& local_view) {
  ...
  // handling estop
  auto* preprocessor_status = control_preprocessor.mutable_header()->mutable_status();

  control_preprocessor.mutable_local_view()->CopyFrom(*local_view);

  Status status = ProducePreprocessorStatus(&control_preprocessor);
  AERROR_IF(!status.ok()) << "Failed to produce control preprocessor:" << status.error_message();
  ...
  static apollo::common::LatencyRecorder latency_recorder(FLAGS_control_preprocessor_topic);
  latency_recorder.AppendLatencyRecord(control_preprocessor.header().lidar_timestamp(), start_time,
                                       end_time);
...
Status PreprocessorSubmodule::ProducePreprocessorStatus(Preprocessor* control_preprocessor) {
  ...
  if (!status.ok()) {
    ...
    mutable_engage_advice->set_advice(apollo::common::EngageAdvice::DISALLOW_ENGAGE);
  ...
  if (!status_ts.ok()) {
    ...
    if (control_preprocessor->local_view().chassis().driving_mode() !=
      ...
      control_preprocessor->mutable_engage_advice()->set_reason(status.error_message());
    }
    return status;
  }
  control_preprocessor->mutable_engage_advice()->set_advice(
  ...
  estop_ = control_common_conf_.enable_persistent_estop() ?
               estop_ || control_preprocessor->local_view().trajectory().estop().is_estop() :
  ...
  if (FLAGS_enable_gear_drive_negative_speed_protection) {
    ...
    if (control_preprocessor->local_view().chassis().gear_location() == Chassis::GEAR_DRIVE &&
        first_trajectory_point.v() < -1 * kEpsilon) {
      return Status(
          ErrorCode::CONTROL_ESTOP_ERROR,
          absl::StrCat(
              "estop for empty planning trajectory, planning headers: ",
              control_preprocessor->local_view().trajectory().header().ShortDebugString()));
    }
  }
...
Status PreprocessorSubmodule::CheckInput(LocalView* local_view) {
  ...
  }
  {
    ...
    for (auto trajectory_point : local_view->trajectory().trajectory_point()) {
      if (std::abs(trajectory_point.v()) < control_common_conf_.minimum_speed_resolution() &&
          std::abs(trajectory_point.a()) < control_common_conf_.max_acceleration_when_stopped()) {
...
Status PreprocessorSubmodule::CheckTimestamp(const LocalView& local_view) {
  ...
  if (chassis_diff >
      (control_common_conf_.max_chassis_miss_num() * control_common_conf_.chassis_period())) {
  ...
  if (trajectory_diff >
      (control_common_conf_.max_planning_miss_num() * control_common_conf_.trajectory_period())) {
