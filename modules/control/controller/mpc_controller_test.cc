...
class MPCControllerTest : public ::testing::Test, MPCController {
 public:
  virtual void SetupTestCase() {
    FLAGS_v                 = 4;
    FLAGS_control_conf_file = "modules/control/testdata/mpc_controller_test/lincoln.pb.txt ";
    ControlConf control_conf;
    CHECK(apollo::common::util::GetProtoFromFile(FLAGS_control_conf_file, &control_conf));
  ...
  void ComputeLateralErrors(const double              x,
    ...
    MPCController::ComputeLateralErrors(x, y, theta, linear_v, angular_v, trajectory_analyzer,
                                        debug);
  }
 ...
 protected:
  ...
  PlanningTrajectoryPb LoadPlanningTrajectoryPb(const std::string& filename) {
    ...
    CHECK(apollo::common::util::GetProtoFromFile(filename, &planning_trajectory_pb))
        << "Failed to open file " << filename;
...
TEST_F(MPCControllerTest, ComputeLateralErrors) {
  auto localization_pb = LoadLocalizaionPb("modules/control/testdata/mpc_controller_test/"
                                           "1_localization.pb.txt");
  auto chassis_pb = LoadChassisPb("modules/control/testdata/mpc_controller_test/1_chassis.pb.txt");
  FLAGS_enable_map_reference_unify = false;
  auto* vehicle_state              = VehicleStateProvider::instance();
  vehicle_state->Update(localization_pb, chassis_pb);

  auto planning_trajectory_pb =
      LoadPlanningTrajectoryPb("modules/control/testdata/mpc_controller_test/"
                               "1_planning.pb.txt");
