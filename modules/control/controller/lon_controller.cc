...
Status LonController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  if (control_conf_ == nullptr) {
    ...
    return Status(ErrorCode::CONTROL_INIT_ERROR, "Failed to load LonController conf");
  }
  injector_                                    = injector;
...
Status
LonController::ComputeControlCommand(const localization::LocalizationEstimate* localization,
  ...
  if (FLAGS_enable_speed_station_preview) {
    ...
    station_error_limited =
        common::math::Clamp(debug->station_error(), -station_error_limit, station_error_limit);
  }
  ...
  if (enable_leadlag) { speed_offset = station_leadlag_controller_.Control(speed_offset, ts); }
  ...
  speed_controller_input_limited = common::math::Clamp(
      speed_controller_input, -speed_controller_input_limit, speed_controller_input_limit);
  ...
  double slope_offset_compensation =
      digital_filter_pitch_angle_.Filter(GRA_ACC * std::sin(injector_->vehicle_state()->pitch()));

  if (std::isnan(slope_offset_compensation)) { slope_offset_compensation = 0; }
  ...
  double throttle_lowerbound =
      std::max(vehicle_param_.throttle_deadzone(), lon_controller_conf.throttle_minimum_action());
  double brake_lowerbound =
      std::max(vehicle_param_.brake_deadzone(), lon_controller_conf.brake_minimum_action());
  ...
  if (FLAGS_enable_csv_debug && speed_log_file_ != nullptr) {
    fprintf(speed_log_file_,
            ...
            debug->station_reference(), debug->station_error(), station_error_limited,
            ...
            debug->preview_speed_error(), debug->preview_acceleration_reference(),
            acceleration_cmd_closeloop, acceleration_cmd, debug->acceleration_lookup(),
...
void LonController::ComputeLongitudinalErrors(const TrajectoryAnalyzer* trajectory_analyzer,
  ...
  trajectory_analyzer->ToTrajectoryFrame(vehicle_state->x(), vehicle_state->y(),
                                         ...
                                         matched_point, &s_matched, &s_dot_matched, &d_matched,
                                         &d_dot_matched);
  ...
  double heading_error =
      common::math::NormalizeAngle(vehicle_state->heading() - matched_point.theta());
...
void LonController::SetDigitalFilter(double                 ts,
                                     double                 cutoff_freq,
