...
LonController::LonController()
  ...
  if (FLAGS_enable_csv_debug) {
    ...
    char   name_buffer[80];
    std::time(&rawtime);
    strftime(name_buffer, 80, "/tmp/speed_log__%F_%H%M%S.csv", localtime(&rawtime));
...
LonController::ComputeControlCommand(const localization::LocalizationEstimate* localization,
  ...
  if (preview_time < 0.0) {
    const auto error_msg =
        common::util::StrCat("Preview time set as: ", preview_time, " less than 0");
    AERROR << error_msg;
  ...
  if (isnan(slope_offset_compenstaion)) { slope_offset_compenstaion = 0; }
  ...
  if ((std::fabs(debug->preview_acceleration_reference()) <= FLAGS_max_acceleration_when_stopped &&
  ...
  if (calibration_value >= 0) {
    throttle_cmd = calibration_value > throttle_deadzone ? calibration_value : throttle_deadzone;
    brake_cmd    = 0.0;
  } else {
    throttle_cmd = 0.0;
    brake_cmd    = -calibration_value > brake_deadzone ? -calibration_value : brake_deadzone;
  }
