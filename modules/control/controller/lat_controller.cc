...
std::string GetLogFileName() {
  ...
  char   name_buffer[80];
  std::time(&raw_time);
  strftime(name_buffer, 80, "/tmp/steer_log_simple_optimal_%F_%H%M%S.csv", localtime(&raw_time));
...
void LatController::ProcessLogs(const SimpleLateralDebug* debug, const canbus::Chassis* chassis) {
  ...
  const std::string log_str = StrCat(
      ...
      StrCat(debug->heading_error_rate(), ",", debug->lateral_error_rate(), ",", debug->curvature(),
             ",", debug->steer_angle(), ","),
      StrCat(debug->steer_angle_feedforward(), ",", debug->steer_angle_lateral_contribution(), ",",
             debug->steer_angle_lateral_rate_contribution(), ",",
             debug->steer_angle_heading_contribution(), ","),
      StrCat(debug->steer_angle_heading_rate_contribution(), ",", debug->steer_angle_feedback(),
...
void LatController::InitializeFilters(const ControlConf* control_conf) {
  ...
  lateral_error_filter_ =
      common::MeanFilter(control_conf->lat_controller_conf().mean_filter_window_size());
...
Status LatController::Init(const ControlConf* control_conf) {
  ...
  if (matrix_size != q_param_size) {
    const auto error_msg = StrCat("lateral controller error: matrix_q size: ", q_param_size,
...
void LatController::LoadLatGainScheduler(const LatControllerConf& lat_controller_conf) {
  ...
  CHECK(lat_err_interpolation_->Init(xy1)) << "Fail to load lateral error gain scheduler";

  heading_err_interpolation_.reset(new Interpolation1D);
  CHECK(heading_err_interpolation_->Init(xy2)) << "Fail to load heading error gain scheduler";
}
...
LatController::ComputeControlCommand(const localization::LocalizationEstimate* localization,
  ...
  double steer_angle =
      common::math::Clamp(steer_angle_feedback + steer_angle_feedforward, -100.0, 100.0);
  ...
  if (VehicleStateProvider::instance()->linear_velocity() < FLAGS_lock_steer_speed &&
...
double LatController::ComputeFeedForward(double ref_curvature) const {
  ...
  const double steer_angle_feedforwardterm =
      ...
      180 / M_PI * steer_transmission_ratio_ / steer_single_direction_max_degree_ * 100;
...
void LatController::ComputeLateralErrors(const double              x,
  ...
  double heading_error = common::math::NormalizeAngle(theta - target_point.path_point().theta());
  if (FLAGS_enable_navigation_mode_handlilng) {
