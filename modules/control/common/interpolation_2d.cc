...
double Interpolation2D::Interpolate(const KeyType& xy) const {
  ...
  if (itr_before != xyz_.begin()) { --itr_before; }
...
double Interpolation2D::InterpolateYz(const std::map<double, double>& yz_table, double y) const {
  if (yz_table.empty()) {
  ...
  if (itr_before != yz_table.begin()) { --itr_before; }
