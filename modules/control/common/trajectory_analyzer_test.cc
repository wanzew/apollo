...
TEST_F(TrajectoryAnalyzerTest, QueryNearestPointByAbsoluteTimeInterpolation) {
  ...
  TrajectoryPoint point_2      = trajectory_analyzer.QueryNearestPointByAbsoluteTime(current_time);
  EXPECT_NEAR(point_2.path_point().x(), 1.0, 1e-6);

  current_time            = timestamp_ + 50.0;
  TrajectoryPoint point_4 = trajectory_analyzer.QueryNearestPointByAbsoluteTime(current_time);
  EXPECT_NEAR(point_4.path_point().x(), 1.4, 1e-6);

  current_time            = timestamp_ + 0.03;
  TrajectoryPoint point_6 = trajectory_analyzer.QueryNearestPointByAbsoluteTime(current_time);
