...
double MracController::Control(const double command,
                               const Matrix state,
  ...
  saturation_status_reference_ =
      BoundOutput(state_reference_(0, 0), state_reference_(0, 1), &state_bounded);
  ...
  UpdateAdaption(&gain_state_adaption_, state_action_, gamma_state_adaption_ * gamma_ratio_state_);

  UpdateAdaption(&gain_input_adaption_, input_desired_, gamma_input_adaption_ * gamma_ratio_input_);
  ...
  double control_unbounded = gain_state_adaption_.col(0).transpose() * state_action_.col(0) +
                             gain_input_adaption_(0, 0) * input_desired_(0, 0);

  double control             = 0.0;
  saturation_status_control_ = BoundOutput(control_unbounded, control_previous_, &control);
...
void MracController::Init(const MracConf&     mrac_conf,
                          const LatencyParam& latency_param,
                          const double        dt) {
  control_previous_            = 0.0;
  saturation_status_control_   = 0;
  saturation_status_reference_ = 0;
  ...
  reference_model_enabled_ = (SetReferenceModel(mrac_conf).ok() && BuildReferenceModel().ok());
  ...
  adaption_model_enabled_ = (SetAdaptionModel(mrac_conf).ok() && BuildAdaptionModel().ok());
...
Status MracController::SetReferenceModel(const MracConf& mrac_conf) {
  ...
  if (((mrac_conf.reference_time_constant() < Epsilon && model_order_ == 1)) ||
      ((mrac_conf.reference_natural_frequency() < Epsilon && model_order_ == 2))) {
    const auto error_msg =
        absl::StrCat("mrac controller error: reference model time-constant parameter: ",
                     mrac_conf.reference_time_constant(),
                     "and natural frequency parameter: ", mrac_conf.reference_natural_frequency(),
...
Status MracController::BuildReferenceModel() {
  if (model_order_ > 2) {
    const auto error_msg = absl::StrCat("mrac controller error: reference model order ",
...
Status MracController::BuildAdaptionModel() {
  if (model_order_ > 2) {
    const auto error_msg = absl::StrCat("mrac controller error: adaption model order ",
                                        model_order_, " is beyond the designed range");
    AERROR << error_msg;
    return Status(ErrorCode::CONTROL_INIT_ERROR, error_msg);
  }
...
bool MracController::CheckLyapunovPD(const Matrix matrix_a, const Matrix matrix_p) const {
  ...
  return (matrix_q.isApprox(matrix_q.transpose()) && llt_matrix_q.info() != Eigen::NumericalIssue);
}

void MracController::EstimateInitialGains(const LatencyParam& latency_param) {
  const double rise_time_estimate     = latency_param.dead_time() + latency_param.rise_time();
  const double settling_time_estimate = latency_param.dead_time() + latency_param.settling_time();
  ...
  if (model_order_ == 1 && (rise_time_estimate >= ts_ || settling_time_estimate >= ts_)) {
    const double tau_estimate =
        (rise_time_estimate >= ts_) ? rise_time_estimate / 2.2 : settling_time_estimate / 4.0;
    ...
    gain_state_adaption_init_(0, 0) =
        (matrix_a_reference_(0, 0) - matrix_a_estimate(0, 0)) / matrix_b_estimate(0, 0);
    gain_input_adaption_init_(0, 0) = matrix_b_reference_(0, 0) / matrix_b_estimate(0, 0);
    ...
    if (adaption_clamping_enabled) {
      ...
      gain_state_clamping_(0, 0) =
          (matrix_a_clamping(0, 0) - matrix_a_estimate(0, 0)) / matrix_b_estimate(0, 0);
      gain_input_clamping_(0, 0) = matrix_b_clamping(0, 0) / matrix_b_estimate(0, 0);
    }
  } else if (model_order_ == 2 && (rise_time_estimate >= ts_ && settling_time_estimate >= ts_)) {
    const double wn_estimate   = 1.8 / rise_time_estimate;
    const double zeta_estimate = 4.6 / (rise_time_estimate * settling_time_estimate);
    ...
    gain_input_adaption_init_.col(0) =
        (common::math::PseudoInverse<double, 2, 1>(matrix_b_estimate) * matrix_b_reference_)
            .transpose();
  } else {
...
void MracController::UpdateReference() {
  ...
  state_reference_.col(0) =
      ...
      ((matrix_i + ts_ * 0.5 * matrix_a_reference_) * state_reference_.col(1) +
       ts_ * 0.5 * matrix_b_reference_ * (input_desired_(0, 0) + input_desired_(0, 1)));
}

void MracController::UpdateAdaption(Matrix*      law_adp,
                                    const Matrix state_adp,
                                    const Matrix gain_adp) {
  Matrix state_error = state_action_ - state_reference_;
  law_adp->col(0) =
      law_adp->col(1) - 0.5 * ts_ * gain_adp *
...
void MracController::AntiWindupCompensation(const double control_command,
  ...
  offset_windup(0, 0) =
      ((control_command > bound_command_) ? bound_command_ - control_command : 0.0) +
      ((control_command < -bound_command_) ? -bound_command_ - control_command : 0.0);
  if (model_order_ > 1) {
    offset_windup(1, 0) = ((control_command > previous_command + bound_command_rate_ * ts_) ?
...
int MracController::BoundOutput(const double output_unbounded,
  ...
  if (output_unbounded > bound_command_ ||
    ...
    *output = (bound_command_ < previous_output + bound_command_rate_ * ts_) ?
    ...
    status = (bound_command_ < previous_output + bound_command_rate_ * ts_) ? 1 : 2;
...
void MracController::SetInitialReferenceState(const Matrix& state_reference_init) {
  if (state_reference_init.rows() != model_order_ || state_reference_init.cols() != 1) {
...
void MracController::SetInitialActionState(const Matrix& state_action_init) {
  if (state_action_init.rows() != model_order_ || state_action_init.cols() != 1) {
...
void MracController::SetInitialStateAdaptionGain(const Matrix& gain_state_adaption_init) {
  if (gain_state_adaption_init.rows() != model_order_ || gain_state_adaption_init.cols() != 1) {
    AWARN << "failed to set the initial state adaption gains, due to the given "
             "state size: "
          << gain_state_adaption_init.rows() << " x " << gain_state_adaption_init.cols()
          << " doesn't match the model order: " << model_order_;
  } else {
...
void MracController::SetInitialInputAdaptionGain(const double gain_input_adaption_init) {
  gain_input_adaption_(0, 1) = gain_input_adaption_init;
}

void MracController::SetInitialNonlinearAdaptionGain(const double gain_nonlinear_adaption_init) {
...
double MracController::NonlinearAdaptionRate() const { return gamma_ratio_nonlinear_; }

int MracController::ReferenceSaturationStatus() const { return saturation_status_reference_; }

int MracController::ControlSaturationStatus() const { return saturation_status_control_; }

Matrix MracController::CurrentReferenceState() const { return state_reference_; }

Matrix MracController::CurrentStateAdaptionGain() const { return gain_state_adaption_; }

Matrix MracController::CurrentInputAdaptionGain() const { return gain_input_adaption_; }

Matrix MracController::CurrentNonlinearAdaptionGain() const { return gain_nonlinear_adaption_; }
