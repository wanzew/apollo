...
bool Interpolation1D::Init(const DataType& xy) {
  ...
  spline_.reset(
      new Eigen::Spline<double, 1>(Eigen::SplineFitting<Eigen::Spline<double, 1>>::Interpolate(
          y.transpose(),
          // No more than cubic spline, but accept short vectors.
          static_cast<Eigen::DenseIndex>(std::min<size_t>(x.size() - 1, 3)), ScaledValues(x))));
  return true;
}
...
Eigen::RowVectorXd Interpolation1D::ScaledValues(Eigen::VectorXd const& x_vec) const {
