...
bool LidarExChecker::GetExtrinsics() {
  ...
  if (!tf2_buffer.canTransform("novatel", "velodyne64", ros::Time(0), ros::Duration(100),
                               &err_msg)) {
...
void LidarExChecker::VisualizeClouds() {
  ...
  for (uint32_t i = 0; i < clouds_.size(); ++i) {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr tf_cld_ptr(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> handler(tf_cld_ptr, r, g, b);
...
void LidarExChecker::OnPointCloud(const sensor_msgs::PointCloud2& message) {
  ...
  if (enough_data_) {
    ...
    if (bottom_redundant_cloud_count_ == 50) { VisualizeClouds(); }
    return;
  }

  if (position_type_ != 56) { return; }
  ...
  if ((position - last_position_).norm() < capture_distance_) { return; }
  ...
  for (uint32_t i = 0; i < cld.points.size(); ++i) {
    if (pcl_isfinite(cld.points[i].x)) { tmp_cld.push_back(cld.points[i]); }
  }
  cld = tmp_cld;
