...
double StGapEstimator::EstimateSafeYieldingGap() { return FLAGS_yield_distance; }

// TODO(Jinyun): add more variables to overtaking gap calculation
double StGapEstimator::EstimateProperOvertakingGap(const double target_obs_speed,
                                                   const double adc_speed) {
  const double overtake_distance_s =
      std::fmax(std::fmax(adc_speed, target_obs_speed) * kOvertakeTimeBuffer, kMinOvertakeDistance);
...
double StGapEstimator::EstimateProperFollowingGap(const double adc_speed) {
  return std::fmax(adc_speed * FLAGS_follow_time_buffer, FLAGS_follow_min_distance);
}

// TODO(Jinyun): add more variables to yielding gap calculation
double StGapEstimator::EstimateProperYieldingGap() { return FLAGS_yield_distance; }
