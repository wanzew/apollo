...
class StGraphPoint {
 public:
  ...
  void SetObstacleCost(const float obs_cost);  // given obstacle info, get the cost;
  void SetTotalCost(const float total_cost);   // total cost
  void SetPrePoint(const StGraphPoint& pre_point);

 private:
  STPoint             point_;
  const StGraphPoint* pre_point_ = nullptr;
  std::uint32_t       index_s_   = 0;  // 根据配置文件，暂且理解成[0-150) 150个整数
  std::uint32_t       index_t_   = 0;  // 根据配置文件，暂且理解成[0-8)     8个整数
