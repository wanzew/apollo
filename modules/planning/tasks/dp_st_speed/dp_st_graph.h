...
class DpStGraph {
 ...
 private:
  ...
  float                  CalculateEdgeCostForThirdCol(const uint32_t curr_r,
                                                      ...
                                                      const float    speed_limit);
  void                   GetColRange(const StGraphPoint& point, int* highest_row, int* lowest_row);

 private:
  const StGraphData& st_graph_data_;
  DpStSpeedConfig    dp_st_speed_config_;  // dp st configuration
  const std::vector<const PathObstacle*>&
      obstacles_;  // obstacles based on the current reference line
  ...
  DpStCost                dp_st_cost_;  // cost utility with configuration;
