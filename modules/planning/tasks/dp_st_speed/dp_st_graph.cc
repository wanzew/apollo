  ...
  for (size_t c = 0; c < cost_table_.size(); ++c) {
    ...
    for (uint32_t r = next_lowest_row; r <= next_highest_row; ++r) {
      if (FLAGS_enable_multi_thread_in_dp_st_graph) {
        PlanningThreadPool::instance()->Push(std::bind(&DpStGraph::CalculateCostAt, this, c, r));
      } else {
        CalculateCostAt(c, r);
      }
    }
...
void DpStGraph::GetRowRange(const StGraphPoint& point,
  ...
  *next_highest_row = point.index_s() + static_cast<int>(delta_s_upper_bound / unit_s_);
  if (*next_highest_row >= max_s_size) { *next_highest_row = max_s_size; }

  const float delta_s_lower_bound =
      std::fmax(0.0, v0 * unit_t_ + vehicle_param_.max_deceleration() * speed_coeff);
  *next_lowest_row = point.index_s() + static_cast<int>(delta_s_lower_bound / unit_s_);
...
void DpStGraph::CalculateCostAt(const uint32_t c, const uint32_t r) {
  ...
  if (cost_cr.obstacle_cost() > std::numeric_limits<float>::max()) { return; }
  ...
  for (uint32_t r_pre = r_low; r_pre <= r; ++r_pre) {
    ...
    const float curr_a =
        (cost_cr.index_s() * unit_s_ + pre_col[r_pre].pre_point()->index_s() * unit_s_ -
         2 * pre_col[r_pre].index_s() * unit_s_) /
        (unit_t_ * unit_t_);
    if (curr_a > vehicle_param_.max_acceleration() || curr_a < vehicle_param_.max_deceleration()) {
      continue;
    }
    if (CheckOverlapOnDpStGraph(st_graph_data_.st_boundaries(), cost_cr, pre_col[r_pre])) {
      continue;
