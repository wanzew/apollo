...
Status PiecewiseJerkSpeedNonlinearOptimizer::Process(const PathData&        path_data,
  ...
  speed_data->clear();
  speed_data->AppendSpeedPoint(distance[0], 0.0, velocity[0], acceleration[0], 0.0);
  for (int i = 1; i < num_of_knots_; ++i) {
    // Avoid the very last points when already stopped
    if (velocity[i] < 0.0) { break; }
    speed_data->AppendSpeedPoint(distance[i], delta_t_ * i, velocity[i], acceleration[i],
...
Status PiecewiseJerkSpeedNonlinearOptimizer::SetUpStatesAndBounds(const PathData&  path_data,
  ...
  // Set s_dot bounary
  s_dot_max_ = std::fmax(FLAGS_planning_upper_speed_limit, st_graph_data.init_point().v());
  ...
  if (FLAGS_use_soft_bound_in_nonlinear_speed_opt) {
    ...
    for (int i = 0; i < num_of_knots_; ++i) {
      ...
      for (const STBoundary* boundary : st_graph_data.st_boundaries()) {
        double s_lower = 0.0;
        double s_upper = 0.0;
        if (!boundary->GetUnblockSRange(curr_t, &s_upper, &s_lower)) { continue; }
        SpeedPoint sp;
        switch (boundary->boundary_type()) {
          ...
          case STBoundary::BoundaryType::FOLLOW:
            s_upper_bound = std::fmin(s_upper_bound, s_upper - FLAGS_follow_min_distance);
            ...
            s_soft_upper_bound =
                std::fmin(s_soft_upper_bound, s_upper - FLAGS_follow_min_distance -
...
bool PiecewiseJerkSpeedNonlinearOptimizer::CheckSpeedLimitFeasibility() {
  ...
  if (init_speed_limit + kEpsilon < s_dot_init_) {
    AERROR << "speed limit [" << init_speed_limit << "] lower than initial speed[" << s_dot_init_
           << "]";
    return false;
  }
  return true;
...
Status PiecewiseJerkSpeedNonlinearOptimizer::SmoothSpeedLimit() {
  ...
  PiecewiseJerkPathProblem piecewise_jerk_problem(speed_ref.size(), delta_s, init_state);
  ...
  PiecewiseJerkTrajectory1d smoothed_speed_limit(opt_x.front(), opt_dx.front(), opt_ddx.front());
...
Status PiecewiseJerkSpeedNonlinearOptimizer::SmoothPathCurvature(const PathData& path_data) {
  ...
  for (double path_s = cartesian_path.front().s(); path_s < cartesian_path.back().s() + delta_s;
       path_s += delta_s) {
  ...
  std::array<double, 3>    init_state      = {path_init_point.kappa(), path_init_point.dkappa(),
                                      path_init_point.ddkappa()};
  PiecewiseJerkPathProblem piecewise_jerk_problem(path_curvature.size(), delta_s, init_state);
  ...
  PiecewiseJerkTrajectory1d smoothed_path_curvature(opt_x.front(), opt_dx.front(), opt_ddx.front());
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByQP(SpeedData* const     speed_data,
                                                          std::vector<double>* distance,
                                                          std::vector<double>* velocity,
                                                          std::vector<double>* acceleration) {
  std::array<double, 3>     init_states = {s_init_, s_dot_init_, s_ddot_init_};
  PiecewiseJerkSpeedProblem piecewise_jerk_problem(num_of_knots_, delta_t_, init_states);
  piecewise_jerk_problem.set_dx_bounds(0.0,
                                       std::fmax(FLAGS_planning_upper_speed_limit, init_states[1]));
  ...
  if (!piecewise_jerk_problem.Optimize()) {
    const std::string msg = "Speed Optimization by Quadratic Programming failed. "
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByNLP(std::vector<double>* distance,
                                                           std::vector<double>* velocity,
                                                           std::vector<double>* acceleration) {
  ...
  auto ptr_interface = new PiecewiseJerkSpeedNonlinearIpoptInterface(
      s_init_, s_dot_init_, s_ddot_init_, delta_t_, num_of_knots_, total_length_, s_dot_max_,
      s_ddot_min_, s_ddot_max_, s_dddot_min_, s_dddot_max_);
  ...
  if (config.use_warm_start()) {
    ...
    for (std::size_t i = 0; i < size; ++i) {
      warm_start.emplace_back(std::initializer_list<double>(
          {warm_start_distance[i], warm_start_velocity[i], warm_start_acceleration[i]}));
    }
