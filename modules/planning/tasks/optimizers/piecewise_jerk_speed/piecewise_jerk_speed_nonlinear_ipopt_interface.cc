...
bool PiecewiseJerkSpeedNonlinearIpoptInterface::get_starting_point(int     n,
                                                                   bool    init_x,
                                                                   double* x,
                                                                   bool    init_z,
                                                                   double* z_L,
...
bool PiecewiseJerkSpeedNonlinearIpoptInterface::eval_f(int           n,
                                                       const double* x,
                                                       bool          new_x,
                                                       double&       obj_value) {
  obj_value = 0.0;
...
bool PiecewiseJerkSpeedNonlinearIpoptInterface::eval_grad_f(int           n,
  ...
  for (int i = 1; i + 1 < num_of_points_; ++i) {
    grad_f[a_offset_ + i] +=
        c * (2.0 * x[a_offset_ + i] - x[a_offset_ + i + 1] - x[a_offset_ + i - 1]);
  }
  grad_f[a_offset_ + num_of_points_ - 1] +=
      c * (x[a_offset_ + num_of_points_ - 1] - x[a_offset_ + num_of_points_ - 2]);
...
bool PiecewiseJerkSpeedNonlinearIpoptInterface::eval_g(
...
bool PiecewiseJerkSpeedNonlinearIpoptInterface::eval_h(int           n,
                                                       const double* x,
                                                       bool          new_x,
                                                       double        obj_factor,
                                                       int           m,
...
void PiecewiseJerkSpeedNonlinearIpoptInterface::finalize_solution(
  ...
  if (use_soft_safety_bound_) {
    ...
    ADEBUG << "lower soft s boundary average intrustion is [" << lower_s_mean_intrusion
           << "] with highest value of [" << lower_s_highest_intrusion << "] at time ["
           << delta_t_ * static_cast<double>(lower_s_highest_intrustion_index) << "].";
    ADEBUG << "upper soft s boundary average intrustion is [" << upper_s_mean_intrusion
           << "] with highest value of [" << upper_s_highest_intrusion << "] at time ["
...
int PiecewiseJerkSpeedNonlinearIpoptInterface::to_hash_key(const int i, const int j) const {
...
void PiecewiseJerkSpeedNonlinearIpoptInterface::set_w_reference_spatial_distance(
