...
class PiecewiseJerkSpeedNonlinearOptimizer : public SpeedOptimizer {
 ...
 private:
  ...
  double delta_t_      = 0.0;  // st problem dimensions
  double total_length_ = 0.0;
  double total_time_   = 0.0;
  int    num_of_knots_ = 0;
  double s_init_       = 0.0;  // st initial values
  double s_dot_init_   = 0.0;
  double s_ddot_init_  = 0.0;
  double s_dot_max_    = 0.0;  // st states dynamically feasibility bounds
  ...
  SpeedLimit                             speed_limit_;           // speed limits
  PiecewiseJerkTrajectory1d              smoothed_speed_limit_;  //
  PiecewiseJerkTrajectory1d smoothed_path_curvature_;  // smoothed path curvature profile as a
                                                       // function of traversal distance
  SpeedData reference_speed_data_;                     // reference speed profile
  double    cruise_speed_;                             // reference cruise speed
};
