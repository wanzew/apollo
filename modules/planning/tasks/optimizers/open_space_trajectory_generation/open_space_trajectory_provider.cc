...
OpenSpaceTrajectoryProvider::OpenSpaceTrajectoryProvider(
  ...
  open_space_trajectory_optimizer_.reset(new OpenSpaceTrajectoryOptimizer(
      config.open_space_trajectory_provider_config().open_space_trajectory_optimizer_config()));
}

OpenSpaceTrajectoryProvider::~OpenSpaceTrajectoryProvider() {
  if (FLAGS_enable_open_space_planner_thread) { Stop(); }
}

void OpenSpaceTrajectoryProvider::Stop() {
  if (FLAGS_enable_open_space_planner_thread) {
    is_generation_thread_stop_.store(true);
    if (thread_init_flag_) { task_future_.get(); }
...
Status OpenSpaceTrajectoryProvider::Process() {
  ...
  auto trajectory_data = frame_->mutable_open_space_info()->mutable_stitched_trajectory_result();
  ...
  if (!IsVehicleStopDueToFallBack(previous_frame->open_space_info().fallback_flag(),
                                  vehicle_state)) {
    const auto& previous_planning = previous_frame->open_space_info().stitched_trajectory_result();
    const auto& previous_planning_header =
        previous_frame->current_frame_planned_trajectory().header().timestamp_sec();
    ...
    stitching_trajectory =
        TrajectoryStitcher::ComputeReinitStitchingTrajectory(planning_cycle_time, vehicle_state);
  ...
  if (FLAGS_enable_open_space_planner_thread) {
...

    {
      ...
      thread_data_.obstacles_vertices_vec = open_space_info.obstacles_vertices_vec();
      thread_data_.XYbounds               = open_space_info.ROI_xy_boundary();
      data_ready_.store(true);
    }

    // Check vehicle state
    if (IsVehicleNearDestination(vehicle_state, open_space_info.open_space_end_pose(),
    ...
    if (trajectory_error_) {
      ...
      if (optimizer_thread_counter > 1000) {
        return Status(ErrorCode::PLANNING_ERROR, "open_space_optimizer failed too many times");
      }
    }
    ...
    if (previous_frame->open_space_info().open_space_provider_success()) {
      ...
      return Status(ErrorCode::OK, "Waiting for open_space_trajectory_optimizer in "
    ...
    // Check vehicle state
    if (IsVehicleNearDestination(vehicle_state, end_pose, rotate_angle, translate_origin)) {
      GenerateStopTrajectory(trajectory_data);
      return Status(ErrorCode::OK, "Vehicle is near to destination");
    }
    ...
    Status status = open_space_trajectory_optimizer_->Plan(
        ...
        obstacles_edges_num, obstacles_A, obstacles_b, obstacles_vertices_vec, &time_latency);
...
void OpenSpaceTrajectoryProvider::GenerateTrajectoryThread() {
  while (!is_generation_thread_stop_) {
    if (!trajectory_updated_ && data_ready_) {
      ...
      double time_latency;
      Status status = open_space_trajectory_optimizer_->Plan(
          thread_data.stitching_trajectory, thread_data.end_pose, thread_data.XYbounds,
          thread_data.rotate_angle, thread_data.translate_origin, thread_data.obstacles_edges_num,
          thread_data.obstacles_A, thread_data.obstacles_b, thread_data.obstacles_vertices_vec,
          &time_latency);
      frame_->mutable_open_space_info()->set_time_latency(time_latency);
      if (status == Status::OK()) {
        std::lock_guard<std::mutex> lock(open_space_mutex_);
...
bool OpenSpaceTrajectoryProvider::IsVehicleNearDestination(
  ...
  double distance_to_vehicle2     = std::sqrt((vehicle_state.x() - end_pose_to_world_frame.x()) *
  ...
  double distance_to_vehicle1 =
      std::sqrt((vehicle_state.x() - end_pose[0]) * (vehicle_state.x() - end_pose[0]) +
                (vehicle_state.y() - end_pose[1]) * (vehicle_state.y() - end_pose[1]));
  double distance_to_vehicle = std::sqrt((vehicle_state.x() - end_pose_to_world_frame.x()) *
                                             (vehicle_state.x() - end_pose_to_world_frame.x()) +
                                         (vehicle_state.y() - end_pose_to_world_frame.y()) *
                                             (vehicle_state.y() - end_pose_to_world_frame.y()));
  double theta_to_vehicle =
      std::abs(common::math::AngleDiff(vehicle_state.heading(), end_theta_to_world_frame));
  ...
  distance_to_vehicle =
      std::min(std::min(distance_to_vehicle, distance_to_vehicle1), distance_to_vehicle2);
  theta_to_vehicle = std::min(theta_to_vehicle, std::abs(vehicle_state.heading()));
...
bool OpenSpaceTrajectoryProvider::IsVehicleStopDueToFallBack(
  ...
  if (!is_on_fallback) { return false; }
...
void OpenSpaceTrajectoryProvider::GenerateStopTrajectory(
  ...
  double standstill_acceleration = frame_->vehicle_state().linear_velocity() >= -vEpsilon ?
...
void OpenSpaceTrajectoryProvider::LoadResult(DiscretizedTrajectory* const trajectory_data) {
  ...
  open_space_trajectory_optimizer_->GetOptimizedTrajectory(optimizer_trajectory_ptr);
  open_space_trajectory_optimizer_->GetStitchingTrajectory(stitching_trajectory_ptr);
  ...
  double stitching_point_relative_time = stitching_trajectory_ptr->back().relative_time();
  double stitching_point_relative_s    = stitching_trajectory_ptr->back().path_point().s();
  for (size_t i = 0; i < optimizer_trajectory_size; ++i) {
    optimizer_trajectory_ptr->at(i).set_relative_time(
        optimizer_trajectory_ptr->at(i).relative_time() + stitching_point_relative_time);
    optimizer_trajectory_ptr->at(i).mutable_path_point()->set_s(
        optimizer_trajectory_ptr->at(i).path_point().s() + stitching_point_relative_s);
  }
  *(trajectory_data) = *(optimizer_trajectory_ptr);

  // Last point in stitching trajectory is already in optimized trajectory, so
  // it is deleted
  frame_->mutable_open_space_info()->mutable_stitching_trajectory_data()->pop_back();
  trajectory_data->PrependTrajectoryPoints(frame_->open_space_info().stitching_trajectory_data());
...
void OpenSpaceTrajectoryProvider::ReuseLastFrameDebug(const Frame* last_frame) {
  ...
  ptr_debug->mutable_planning_data()->mutable_open_space()->MergeFrom(
      last_frame->open_space_info().debug_instance().planning_data().open_space());
}
