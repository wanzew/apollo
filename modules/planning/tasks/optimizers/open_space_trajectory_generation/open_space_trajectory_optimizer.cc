...
OpenSpaceTrajectoryOptimizer::OpenSpaceTrajectoryOptimizer(
  ...
  distance_approach_.reset(new DistanceApproachProblem(config.planner_open_space_config()));
...
Status
OpenSpaceTrajectoryOptimizer::Plan(const std::vector<common::TrajectoryPoint>& stitching_trajectory,
  ...
  if (IsInitPointNearDestination(stitching_trajectory.back(), end_pose, rotate_angle,
                                 translate_origin)) {
    ADEBUG << "Planning init point is close to destination, skip new "
              "trajectory generation";
    return Status(ErrorCode::OK, "Planning init point is close to destination, skip new "
                                 "trajectory generation");
  }
  ...
  const common::TrajectoryPoint trajectory_stitching_point = stitching_trajectory.back();
  ...
  if (warm_start_->Plan(init_x, init_y, init_phi, end_pose[0], end_pose[1], end_pose[2], XYbounds,
    ...
    return Status(ErrorCode::PLANNING_ERROR, "State warm start problem failed to solve");
  }
  ...
  if (FLAGS_enable_parallel_trajectory_smoothing) {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (config_.planner_open_space_config().enable_check_parallel_trajectory()) {
        AINFO << "trajectory id: " << i;
        AINFO << "trajectory partitioned size: " << xWS_vec[i].cols();
        AINFO << "initial point: " << xWS_vec[i].col(0).transpose();
        AINFO << "ending point: " << xWS_vec[i].col(xWS_vec[i].cols() - 1).transpose();
      }
      ...
      if (FLAGS_use_iterative_anchoring_smoother) {
        if (!GenerateDecoupledTraj(xWS_vec[i], last_time_u(1, 0), init_v, obstacles_vertices_vec,
        ...
        if (!GenerateDistanceApproachTraj(
                xWS_vec[i], uWS_vec[i], XYbounds, obstacles_edges_num, obstacles_A, obstacles_b,
                obstacles_vertices_vec, last_time_u, init_v, &state_result_ds_vec[i],
                &control_result_ds_vec[i], &time_result_ds_vec[i], &l_warm_up_vec[i],
                &n_warm_up_vec[i], &dual_l_result_ds_vec[i], &dual_n_result_ds_vec[i])) {
          ADEBUG << "Smoother fail at " << i << "th trajectory with index starts from 0";
          ADEBUG << i << "th trajectory size is " << xWS_vec[i].cols();
        ...
        ADEBUG << i << "th trajectory size is " << xWS_vec[i].cols();
        ADEBUG << "average time spend: " << time_diff_ms / xWS_vec[i].cols() << " ms per point.";
        ADEBUG << "average time spend after smooth: "
               << time_diff_ms / state_result_ds_vec[i].cols() << " ms per point.";
        ADEBUG << i << "th smoothed trajectory size is " << state_result_ds_vec[i].cols();
      }
      ...
      ADEBUG << "Open space trajectory smoothing total time: " << smoother_diff.count() * 1000.0
             << " ms at the " << i << "th trajectory.";
      ADEBUG << "The " << i << "th trajectory pre-smoothing size is " << xWS_vec[i].cols()
             << "; post-smoothing size is " << state_result_ds_vec[i].cols();
    }

    // Retrive the trajectory in one piece
    CombineTrajectories(xWS_vec, uWS_vec, state_result_ds_vec, control_result_ds_vec,
                        time_result_ds_vec, l_warm_up_vec, n_warm_up_vec, dual_l_result_ds_vec,
                        dual_n_result_ds_vec, &xWS, &uWS, &state_result_ds, &control_result_ds,
                        &time_result_ds, &l_warm_up, &n_warm_up, &dual_l_result_ds,
                        &dual_n_result_ds);
    ...
    if (!GenerateDistanceApproachTraj(
            ...
            obstacles_vertices_vec, last_time_u, init_v, &state_result_ds, &control_result_ds,
            &time_result_ds, &l_warm_up, &n_warm_up, &dual_l_result_ds, &dual_n_result_ds)) {
      return Status(ErrorCode::PLANNING_ERROR,
                    "distance approach smoothing problem failed to solve");
    }
  }
  ...
  if (FLAGS_enable_record_debug) {
    ...
    RecordDebugInfo(trajectory_stitching_point, translate_origin, rotate_angle, end_pose, xWS, uWS,
                    l_warm_up, n_warm_up, dual_l_result_ds, dual_n_result_ds, state_result_ds,
                    control_result_ds, time_result_ds, XYbounds, obstacles_vertices_vec);
  }

  // rescale the states to the world frame
  size_t state_size = state_result_ds.cols();
  for (size_t i = 0; i < state_size; ++i) {
    PathPointDeNormalizing(rotate_angle, translate_origin, &(state_result_ds(0, i)),
  ...
  ADEBUG << "open space trajectory smoother total time: " << diff.count() * 1000.0 << " ms.";
...
void OpenSpaceTrajectoryOptimizer::RecordDebugInfo(
  ...
  open_space_debug_.mutable_trajectory_stitching_point()->CopyFrom(trajectory_stitching_point);
  ...
  auto*  warm_start_trajectory = open_space_debug_.mutable_warm_start_trajectory();
  for (size_t i = 0; i < horizon; ++i) {
    auto* warm_start_point = warm_start_trajectory->add_vehicle_motion_point();
    warm_start_point->mutable_trajectory_point()->mutable_path_point()->set_x(xWS(0, i));
    warm_start_point->mutable_trajectory_point()->mutable_path_point()->set_y(xWS(1, i));
    warm_start_point->mutable_trajectory_point()->mutable_path_point()->set_theta(xWS(2, i));
  ...
  auto* warm_start_point = warm_start_trajectory->add_vehicle_motion_point();
  warm_start_point->mutable_trajectory_point()->mutable_path_point()->set_x(xWS(0, horizon));
  warm_start_point->mutable_trajectory_point()->mutable_path_point()->set_y(xWS(1, horizon));
  warm_start_point->mutable_trajectory_point()->mutable_path_point()->set_theta(xWS(2, horizon));
  ...
  for (size_t i = 0; i < horizon; ++i) {
    ...
    smoothed_point->mutable_trajectory_point()->mutable_path_point()->set_x(state_result_ds(0, i));
    smoothed_point->mutable_trajectory_point()->mutable_path_point()->set_y(state_result_ds(1, i));
  ...
  smoothed_point->mutable_trajectory_point()->set_v(state_result_ds(3, horizon));
...
bool OpenSpaceTrajectoryOptimizer::IsInitPointNearDestination(
  ...
  double distance_to_init_point      = std::sqrt((path_point.x() - end_pose_to_world_frame.x()) *
...
void OpenSpaceTrajectoryOptimizer::PathPointNormalizing(
    double rotate_angle, const Vec2d& translate_origin, double* x, double* y, double* phi) {
...
void OpenSpaceTrajectoryOptimizer::LoadTrajectory(const Eigen::MatrixXd& state_result,
...
void OpenSpaceTrajectoryOptimizer::UseWarmStartAsResult(const Eigen::MatrixXd& xWS,
                                                        const Eigen::MatrixXd& uWS,
                                                        const Eigen::MatrixXd& l_warm_up,
                                                        const Eigen::MatrixXd& n_warm_up,
                                                        Eigen::MatrixXd*       state_result_ds,
...
bool OpenSpaceTrajectoryOptimizer::GenerateDistanceApproachTraj(
  ...
  if (FLAGS_use_dual_variable_warm_start) {
    if (dual_variable_warm_start_->Solve(horizon, ts, ego, obstacles_num, obstacles_edges_num,
                                         obstacles_A, obstacles_b, xWS, l_warm_up, n_warm_up,
                                         &s_warm_up)) {
    ...
    *l_warm_up = 0.5 * Eigen::MatrixXd::Ones(obstacles_edges_num.sum(), horizon + 1);
    *n_warm_up = 0.5 * Eigen::MatrixXd::Ones(4 * obstacles_num, horizon + 1);
  }

  // Distance approach trajectory smoothing
  if (distance_approach_->Solve(x0, xF, last_time_u, horizon, ts, ego, xWS, uWS, *l_warm_up,
    ...
    if (FLAGS_enable_smoother_failsafe) {
      UseWarmStartAsResult(xWS, uWS, *l_warm_up, *n_warm_up, state_result_ds, control_result_ds,
                           time_result_ds, dual_l_result_ds, dual_n_result_ds);
    } else {
      return false;
    }
...
void OpenSpaceTrajectoryOptimizer::LoadHybridAstarResultInEigen(HybridAStartResult* result,
                                                                Eigen::MatrixXd*    xWS,
                                                                Eigen::MatrixXd*    uWS) {
  ...
  Eigen::VectorXd x = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result->x.data(), horizon + 1);
  Eigen::VectorXd y = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result->y.data(), horizon + 1);
  Eigen::VectorXd phi =
      Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result->phi.data(), horizon + 1);
  Eigen::VectorXd v = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result->v.data(), horizon + 1);
  Eigen::VectorXd steer =
      Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result->steer.data(), horizon);
  Eigen::VectorXd a = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result->a.data(), horizon);
...
void OpenSpaceTrajectoryOptimizer::CombineTrajectories(
  ...
  Eigen::MatrixXd xWS_ = Eigen::MatrixXd::Zero(xWS_vec[0].rows(), warm_start_state_size);
  Eigen::MatrixXd uWS_ = Eigen::MatrixXd::Zero(uWS_vec[0].rows(), warm_start_control_size);
  ...
  Eigen::MatrixXd control_result_ds_ =
      Eigen::MatrixXd::Zero(control_result_ds_vec[0].rows(), smoothed_control_size);
  Eigen::MatrixXd time_result_ds_ = Eigen::MatrixXd::Zero(time_result_ds_vec[0].rows(), time_size);
  Eigen::MatrixXd l_warm_up_ = Eigen::MatrixXd::Zero(l_warm_up_vec[0].rows(), l_warm_start_size);
  Eigen::MatrixXd n_warm_up_ = Eigen::MatrixXd::Zero(n_warm_up_vec[0].rows(), n_warm_start_size);
...
bool OpenSpaceTrajectoryOptimizer::GenerateDecoupledTraj(
  ...
  if (!iterative_anchoring_smoother_->Smooth(xWS, init_a, init_v, obstacles_vertices_vec,
                                             &smoothed_trajectory)) {
    return false;
  }

  LoadResult(smoothed_trajectory, state_result_dc, control_result_dc, time_result_dc);
  return true;
}

// TODO(Jinyun): tmp interface, will refactor
void OpenSpaceTrajectoryOptimizer::LoadResult(const DiscretizedTrajectory& discretized_trajectory,
                                              Eigen::MatrixXd*             state_result_dc,
                                              Eigen::MatrixXd*             control_result_dc,
                                              Eigen::MatrixXd*             time_result_dc) {
  ...
  *time_result_dc    = Eigen::MatrixXd::Zero(1, points_size - 1);
  ...
  const double wheel_base =
      common::VehicleConfigHelper::Instance()->GetConfig().vehicle_param().wheel_base();
  for (size_t i = 0; i + 1 < points_size; ++i) {
    control_result(0, i) = std::atan(discretized_trajectory[i].path_point().kappa() * wheel_base);
    control_result(1, i) = discretized_trajectory[i].a();
    time_result(0, i) =
        discretized_trajectory[i + 1].relative_time() - discretized_trajectory[i].relative_time();
  }
}
