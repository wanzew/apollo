...
bool CheckOverlapOnDpStGraph(const std::vector<const STBoundary*>& boundaries,
                             const StGraphPoint&                   p1,
                             const StGraphPoint&                   p2) {
  if (FLAGS_use_st_drivable_boundary) { return false; }
  for (const auto* boundary : boundaries) {
    ...
    if (boundary->HasOverlap({p1.point(), p2.point()})) { return true; }
  }
  return false;
}
}  // namespace
...
GriddedPathTimeGraph::GriddedPathTimeGraph(const StGraphData&                  st_graph_data,
    ...
    , dp_st_cost_(dp_config,
                  st_graph_data_.total_time_by_conf(),
                  st_graph_data_.path_length(),
                  obstacles,
                  st_graph_data_.st_drivable_boundary(),
                  init_point_) {
...
Status GriddedPathTimeGraph::Search(SpeedData* const speed_data) {
  ...
  for (const auto& boundary : st_graph_data_.st_boundaries()) {
    ...
    if (boundary->IsPointInBoundary({0.0, 0.0}) ||
      ...
      dimension_t_ =
          static_cast<uint32_t>(std::ceil(total_length_t_ / static_cast<double>(unit_t_))) + 1;
...
Status GriddedPathTimeGraph::InitCostTable() {
  ...
  double sparse_length_s =
      total_length_s_ - static_cast<double>(dense_dimension_s_ - 1) * dense_unit_s_;
...
void GriddedPathTimeGraph::GetRowRange(const StGraphPoint& point,
  ...
  const double s_upper_bound =
      v0 * unit_t_ + acc_coeff * max_acceleration_ * t_squared + point.point().s();
...
void GriddedPathTimeGraph::CalculateCostAt(const std::shared_ptr<StGraphMessage>& msg) {
  ...
  if (cost_cr.obstacle_cost() > std::numeric_limits<double>::max()) { return; }
  ...
  if (c == 1) {
    ...
    if (acc < max_deceleration_ || acc > max_acceleration_) { return; }
    ...
    if (CheckOverlapOnDpStGraph(st_graph_data_.st_boundaries(), cost_cr, cost_init)) { return; }
  ...
  const double            pre_lowest_s =
      cost_cr.point().s() - FLAGS_planning_upper_speed_limit * (1 + kSpeedRangeBuffer) * unit_t_;
  ...
  if (pre_lowest_itr == spatial_distance_by_index_.end()) {
    ...
    r_low =
        static_cast<uint32_t>(std::distance(spatial_distance_by_index_.begin(), pre_lowest_itr));
  }
  ...
  if (c == 2) {
    for (uint32_t i = 0; i < r_pre_size; ++i) {
      ...
      const double curr_a = 2 *
                            ...
                            unit_t_;
      if (curr_a < max_deceleration_ || curr_a > max_acceleration_) { continue; }
  ...
  for (uint32_t i = 0; i < r_pre_size; ++i) {
    ...
    const double curr_a = 2 *
                          ((cost_cr.point().s() - pre_col[r_pre].point().s()) / unit_t_ -
                           pre_col[r_pre].GetOptimalSpeed()) /
                          unit_t_;
    if (curr_a > max_acceleration_ || curr_a < max_deceleration_) { continue; }
