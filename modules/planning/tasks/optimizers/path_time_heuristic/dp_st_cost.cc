...
DpStCost::DpStCost(const DpStSpeedOptimizerConfig&     config,
  ...
  const auto dimension_t =
      static_cast<uint32_t>(std::ceil(total_t / static_cast<double>(unit_t_))) + 1;
...
void DpStCost::AddToKeepClearRange(const std::vector<const Obstacle*>& obstacles) {
  for (const auto& obstacle : obstacles) {
    if (obstacle->path_st_boundary().IsEmpty()) { continue; }
...
void DpStCost::SortAndMergeRange(std::vector<std::pair<double, double>>* keep_clear_range) {
  if (!keep_clear_range || keep_clear_range->empty()) { return; }
  ...
  while (j < keep_clear_range->size()) {
    if (keep_clear_range->at(i).second < keep_clear_range->at(j).first) {
      ...
      keep_clear_range->at(i).second =
          std::max(keep_clear_range->at(i).second, keep_clear_range->at(j).second);
      ++j;
    }
  }
...
bool DpStCost::InKeepClearRange(double s) const {
  for (const auto& p : keep_clear_range_) {
    if (p.first <= s && p.second >= s) { return true; }
  }
  return false;
}
...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point) {
  ...
  if (FLAGS_use_st_drivable_boundary) {
    ...
    if (s > upper_bound || s < lower_bound) { return kInf; }
  }
  ...
  for (const auto* obstacle : obstacles_) {
    ...
    if (obstacle->LongitudinalDecision().has_stop()) { continue; }

    auto boundary = obstacle->path_st_boundary();

    if (boundary.min_s() > FLAGS_speed_lon_decision_horizon) { continue; }
    if (t < boundary.min_t() || t > boundary.max_t()) { continue; }
    if (boundary.IsPointInBoundary(st_graph_point.point())) { return kInf; }
    double s_upper = 0.0;
    double s_lower = 0.0;
...
double DpStCost::GetSpeedCost(const STPoint& first,
  ...
  if (FLAGS_enable_dp_reference_speed) {
    ...
    cost += config_.reference_speed_penalty() * config_.default_speed_cost() * fabs(diff_speed) *
            unit_t_;
  }
...
double DpStCost::GetAccelCost(const double accel) {
  ...
  if (accel_key >= accel_cost_.size()) { return kInf; }
...
double DpStCost::JerkCost(const double jerk) {
  ...
  if (jerk_key >= jerk_cost_.size()) { return kInf; }
...
double DpStCost::GetJerkCostByFourPoints(const STPoint& first,
  ...
  double jerk =
      (fourth.s() - 3 * third.s() + 3 * second.s() - first.s()) / (unit_t_ * unit_t_ * unit_t_);
