...
bool TrajectoryCost::IsOffRoad(const double ref_s,
                               const double l,
                               const double dl,
                               const bool   is_change_lane_path) {
  static constexpr double kIgnoreDistance = 5.0;
  if (ref_s - init_sl_point_.s() < kIgnoreDistance) { return false; }
  ...
  const double r_w    = (param.left_edge_to_center() + param.right_edge_to_center()) / 2.0;
...
ComparableCost TrajectoryCost::CalculateDynamicObstacleCost(const QuinticPolynomialCurve1d& curve,
  ...
  ComparableCost obstacle_cost;
  if (dynamic_obstacle_boxes_.empty()) { return obstacle_cost; }
...
ComparableCost TrajectoryCost::GetCostFromObsSL(const double      adc_s,
                                                const double      adc_l,
                                                const SLBoundary& obs_sl_boundary) {
  const auto& vehicle_param = common::VehicleConfigHelper::Instance()->GetConfig().vehicle_param();

  ComparableCost obstacle_cost;
  if (obs_sl_boundary.start_l() * obs_sl_boundary.end_l() <= 0.0) { return obstacle_cost; }
  ...
  const double delta_l =
      std::fmax(adc_right_l - obs_sl_boundary.end_l(), obs_sl_boundary.start_l() - adc_left_l);
  /*
...
Box2d TrajectoryCost::GetBoxFromSLPoint(const common::SLPoint& sl, const double dl) const {
  Vec2d xy_point;
