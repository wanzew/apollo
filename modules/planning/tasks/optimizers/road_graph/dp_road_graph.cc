...
bool DpRoadGraph::GenerateMinCostPath(const std::vector<const Obstacle*>& obstacles,
  ...
  for (size_t level = 1; level < path_waypoints.size(); ++level) {
    ...
    for (size_t i = 0; i < level_points.size(); ++i) {
      ...
      auto msg =
          std::make_shared<RoadGraphMessage>(prev_dp_nodes, level, total_level, &trajectory_cost,
                                             &front, &(graph_nodes.back().back()));
...
void DpRoadGraph::UpdateNode(const std::shared_ptr<RoadGraphMessage>& msg) {
  ...
  if (reference_line_info_.IsChangeLanePath() && msg->level >= 2) {
    ...
    const auto cost = msg->trajectory_cost->Calculate(
        curve, init_sl_point_.s(), msg->cur_node->sl_point.s(), msg->level, msg->total_level);
