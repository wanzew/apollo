...
common::Status PiecewiseJerkPathOptimizer::Process(const SpeedData&               speed_data,
  ...
  const auto& config =
      reference_line_info_->IsChangeLanePath() ?
          config_.piecewise_jerk_path_optimizer_config().lane_change_path_config() :
  ...
  for (const auto& path_boundary : path_boundaries) {
    ...
    if (path_boundary.label().find("regular") != std::string::npos && path_boundary_size < 2) {
      continue;
    }

    int max_iter = 4000;
    // lower max_iter for regular/self/
    if (path_boundary.label().find("self") != std::string::npos) { max_iter = 4000; }
    ...
    if (path_boundary.label().find("regular") != std::string::npos &&
      ...
      for (size_t i = 0; i < path_reference_size; ++i) {
        ...
        reference_line.XYToSL(
            common::util::PointFactory::ToPointENU(reference_path_data.path_reference().at(i).x(),
...
PiecewiseJerkPathOptimizer::ConvertPathPointRefFromFrontAxeToRearAxe(const PathData& path_data) {
...
bool PiecewiseJerkPathOptimizer::OptimizePath(
  ...
  if (end_state[0] != 0 && !is_valid_path_reference) {
    ...
    const auto&         pull_over_type =
        injector_->planning_context()->planning_status().pull_over().pull_over_type();
  ...
  if (is_valid_path_reference) {
    ...
    const double peak_value =
        config_.piecewise_jerk_path_optimizer_config().path_reference_l_weight();
    ...
    piecewise_jerk_problem.set_x_ref(std::move(weight_x_ref_vec), std::move(path_reference_l_ref));
  }
...
FrenetFramePath PiecewiseJerkPathOptimizer::ToPiecewiseJerkPath(const std::vector<double>& x,
...
double PiecewiseJerkPathOptimizer::EstimateJerkBoundary(const double vehicle_speed,
                                                        const double axis_distance,
                                                        const double max_yaw_rate) const {
  return max_yaw_rate / axis_distance / vehicle_speed;
}

double PiecewiseJerkPathOptimizer::GaussianWeighting(const double x,
  ...
  ADEBUG << peak_weighting * exp(-0.5 * (x - peak_weighting_x) * (x - peak_weighting_x));
