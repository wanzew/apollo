...
Status PathAssessmentDecider::Process(Frame* const             frame,
  ...
  ADEBUG << "Time for path validity checking: " << diff.count() * 1000 << " msec.";
  ...
  for (size_t i = 0; i != valid_path_data.size(); ++i) {
    ...
    if (curr_path_data.path_label().find("fallback") != std::string::npos) {
      ...
      if (!curr_path_data.Empty()) {
        if (cnt != i) { valid_path_data[cnt] = curr_path_data; }
        ++cnt;
      }
      continue;
    }
  ...
  ADEBUG << "Using '" << valid_path_data.front().path_label() << "' path out of "
         << valid_path_data.size() << " path(s)";
  if (valid_path_data.front().path_label().find("fallback") != std::string::npos) {
  ...
  *(reference_line_info->mutable_path_data()) = valid_path_data.front();
  ...
  ADEBUG << "Time for optimal path selection: " << diff.count() * 1000 << " msec.";
  ...
  if (reference_line_info->path_data().path_label().find("self") != std::string::npos) {
    ...
    if (able_to_use_self_lane_counter < 0) { able_to_use_self_lane_counter = 0; }
  ...
  if (mutable_path_decider_status->is_in_path_lane_borrow_scenario()) {
    ...
    for (const auto& lane_borrow_direction : path_decider_status.decided_side_pass_direction()) {
      if (lane_borrow_direction == PathDeciderStatus::LEFT_BORROW &&
          reference_line_info->path_data().path_label().find("left") != std::string::npos) {
        left_borrow = true;
      }
      if (lane_borrow_direction == PathDeciderStatus::RIGHT_BORROW &&
          reference_line_info->path_data().path_label().find("right") != std::string::npos) {
        right_borrow = true;
      }
    }
...
bool ComparePathData(const PathData& lhs, const PathData& rhs, const Obstacle* blocking_obstacle) {
  ...
  if (lhs_is_regular != rhs_is_regular) { return lhs_is_regular; }
  // Select longer path.
  ...
  if (lhs_on_selflane || rhs_on_selflane) {
    if (std::fabs(lhs_path_length - rhs_path_length) > kSelfPathLengthComparisonTolerance) {
  ...
  if (std::abs(lhs_on_reverse - rhs_on_reverse) > 6) { return lhs_on_reverse < rhs_on_reverse; }
...
bool PathAssessmentDecider::IsValidRegularPath(const ReferenceLineInfo& reference_line_info,
...
bool PathAssessmentDecider::IsValidFallbackPath(const ReferenceLineInfo& reference_line_info,
                                                const PathData&          path_data) {
  // Basic sanity checks.
  if (path_data.Empty()) {
...
void PathAssessmentDecider::SetPathInfo(const ReferenceLineInfo& reference_line_info,
...
void PathAssessmentDecider::TrimTailingOutLanePoints(PathData* const path_data) {
  ...
  while (!path_point_decision.empty() &&
         std::get<1>(path_point_decision.back()) != PathData::PathPointType::IN_LANE) {
    if (std::get<1>(path_point_decision.back()) == PathData::PathPointType::OUT_ON_FORWARD_LANE) {
...
bool PathAssessmentDecider::IsGreatlyOffRoad(const ReferenceLineInfo& reference_line_info,
                                             const PathData&          path_data) {
  ...
  for (const auto& frenet_path_point : frenet_path) {
    ...
    if (reference_line_info.reference_line().GetRoadWidth(frenet_path_point.s(), &road_left_width,
                                                          &road_right_width)) {
...
bool PathAssessmentDecider::IsCollidingWithStaticObstacles(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    obstacle_polygons.push_back(Polygon2d({Vec2d(obstacle_sl.start_s(), obstacle_sl.start_l()),
  ...
  for (size_t i = 0; i < path_data.discretized_path().size(); ++i) {
    if (path_data.frenet_frame_path().back().s() - path_data.frenet_frame_path()[i].s() <
    ...
    const auto& vehicle_box = common::VehicleConfigHelper::Instance()->GetBoundingBox(path_point);
    ...
    for (const auto& corner_point : ABCDpoints) {
      ...
      if (!reference_line_info.reference_line().XYToSL(corner_point, &curr_point_sl)) {
      ...
      for (const auto& obstacle_polygon : obstacle_polygons) {
        if (obstacle_polygon.IsPointIn(curr_point)) {
          ADEBUG << "ADC is colliding with obstacle at path s = " << path_point.s();
          return true;
        }
      }
    }
...
bool PathAssessmentDecider::IsStopOnReverseNeighborLane(
  ...
  std::vector<common::SLPoint> all_stop_point_sl = reference_line_info.GetAllStopDecisionSLPoint();
  if (all_stop_point_sl.empty()) { return false; }

  double                  check_s            = 0.0;
  static constexpr double kLookForwardBuffer = 5.0;  // filter out sidepass stop fence
  const double            adc_end_s          = reference_line_info.AdcSlBoundary().end_s();
  for (const auto& stop_point_sl : all_stop_point_sl) {
    if (stop_point_sl.s() - adc_end_s < kLookForwardBuffer) { continue; }
    check_s = stop_point_sl.s();
    break;
  }
  if (check_s <= 0.0) { return false; }

  double lane_left_width  = 0.0;
  double lane_right_width = 0.0;
  if (!reference_line_info.reference_line().GetLaneWidth(check_s, &lane_left_width,
                                                         &lane_right_width)) {
    return false;
  }
  ...
  ADEBUG << "path_point_sl[" << path_point_sl.s() << ", " << path_point_sl.l()
         << "] lane_left_width[" << lane_left_width << "] lane_right_width[" << lane_right_width
         << "]";
  ...
  if (path_data.path_label().find("left") != std::string::npos &&
    ...
    if (reference_line_info.GetNeighborLaneInfo(ReferenceLineInfo::LaneType::LeftReverse,
                                                path_point_sl.s(), &neighbor_lane_id,
                                                &neighbor_lane_width)) {
      ADEBUG << "stop path point at LeftReverse neighbor lane[" << neighbor_lane_id.id() << "]";
      return true;
    }
  } else if (path_data.path_label().find("right") != std::string::npos &&
             path_point_sl.l() < -lane_right_width) {
    if (reference_line_info.GetNeighborLaneInfo(ReferenceLineInfo::LaneType::RightReverse,
                                                path_point_sl.s(), &neighbor_lane_id,
                                                &neighbor_lane_width)) {
      ADEBUG << "stop path point at RightReverse neighbor lane[" << neighbor_lane_id.id() << "]";
      return true;
    }
  }
  return false;
...
void PathAssessmentDecider::InitPathPointDecision(
    const PathData& path_data, std::vector<PathPointDecision>* const path_point_decision) {
  // Sanity checks.
  ...
  for (const auto& frenet_path_point : path_data.frenet_frame_path()) {
    path_point_decision->emplace_back(frenet_path_point.s(), PathData::PathPointType::UNKNOWN,
...
void PathAssessmentDecider::SetPathPointType(
  ...
  for (size_t i = 0; i < discrete_path.size(); ++i) {
    ...
    Box2d ego_box({rear_center_path_point.x(), rear_center_path_point.y()}, ego_theta, ego_length,
                  ego_width);
    ...
    if (!reference_line_info.reference_line().GetSLBoundary(ego_box, &ego_sl_boundary)) {
    ...
    if (reference_line_info.reference_line().GetLaneWidth(middle_s, &lane_left_width,
                                                          &lane_right_width)) {
      ...
      if (is_lane_change_path) {
        ...
        if (ego_sl_boundary.start_l() > lane_left_width ||
          ...
          std::get<1>((*path_point_decision)[i]) = PathData::PathPointType::IN_LANE;
          ...
          std::get<1>((*path_point_decision)[i]) = PathData::PathPointType::OUT_ON_FORWARD_LANE;
        }
      } else {
        ...
        if (ego_sl_boundary.end_l() > lane_left_width + in_and_out_lane_hysteresis_buffer ||
          ...
          if (path_data.path_label().find("reverse") != std::string::npos) {
            std::get<1>((*path_point_decision)[i]) = PathData::PathPointType::OUT_ON_REVERSE_LANE;
          } else if (path_data.path_label().find("forward") != std::string::npos) {
            std::get<1>((*path_point_decision)[i]) = PathData::PathPointType::OUT_ON_FORWARD_LANE;
          } else {
            std::get<1>((*path_point_decision)[i]) = PathData::PathPointType::UNKNOWN;
          }
          ...
          std::get<1>((*path_point_decision)[i]) = PathData::PathPointType::IN_LANE;
          if (is_prev_point_out_lane) { is_prev_point_out_lane = false; }
        }
      }
    } else {
...
void PathAssessmentDecider::SetObstacleDistance(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    if (obstacle_box.area() < kMinObstacleArea) { continue; }
  ...
  for (size_t i = 0; i < discrete_path.size(); ++i) {
    ...
    for (const auto& obstacle_polygon : obstacle_polygons) {
      ...
      min_distance_to_obstacles  = std::min(min_distance_to_obstacles, distance_to_vehicle);
    }
    std::get<2>((*path_point_decision)[i]) = min_distance_to_obstacles;
  }
}

void PathAssessmentDecider::RecordDebugInfo(const PathData&          path_data,
...
int ContainsOutOnReverseLane(const std::vector<PathPointDecision>& path_point_decision) {
  ...
  for (const auto& curr_decision : path_point_decision) {
    if (std::get<1>(curr_decision) == PathData::PathPointType::OUT_ON_REVERSE_LANE) { ++ret; }
  }
  return ret;
}
...
int GetBackToInLaneIndex(const std::vector<PathPointDecision>& path_point_decision) {
  ...
  for (int i = static_cast<int>(path_point_decision.size()) - 1; i >= 0; --i) {
    if (std::get<1>(path_point_decision[i]) != PathData::PathPointType::IN_LANE) { return i; }
  }
  return 0;
}
