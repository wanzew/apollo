...
Status CreepDecider::Process(Frame* frame, ReferenceLineInfo* reference_line_info) {
  CHECK_NOTNULL(frame);
  ...
  const std::string stop_sign_overlap_id =
      injector_->planning_context()->planning_status().stop_sign().current_stop_sign_overlap_id();
  // traffic light
  ...
  if (injector_->planning_context()
    ...
    yield_sign_overlap_id =
        injector_->planning_context()->planning_status().yield_sign().current_yield_sign_overlap_id(
            0);
  }
  ...
  if (!stop_sign_overlap_id.empty()) {
    ...
    if (current_stop_sign_overlap) {
      stop_line_s =
          current_stop_sign_overlap->end_s + FindCreepDistance(*frame, *reference_line_info);
    ...
    PathOverlap* current_traffic_light_overlap = scenario::util::GetOverlapOnReferenceLine(
        *reference_line_info, current_traffic_light_overlap_id, ReferenceLineInfo::SIGNAL);
    if (current_traffic_light_overlap) {
      stop_line_s =
          current_traffic_light_overlap->end_s + FindCreepDistance(*frame, *reference_line_info);
      current_overlap_id = current_traffic_light_overlap_id;
    }
  } else if (!yield_sign_overlap_id.empty()) {
    // get overlap along reference line
    PathOverlap* current_yield_sign_overlap = scenario::util::GetOverlapOnReferenceLine(
        *reference_line_info, yield_sign_overlap_id, ReferenceLineInfo::YIELD_SIGN);
    if (current_yield_sign_overlap) {
      stop_line_s =
          current_yield_sign_overlap->end_s + FindCreepDistance(*frame, *reference_line_info);
  ...
  if (stop_line_s > 0.0) {
    ...
    const double creep_stop_s = stop_line_s + FindCreepDistance(*frame, *reference_line_info);
    const std::vector<std::string> wait_for_obstacles;
    util::BuildStopDecision(virtual_obstacle_id, creep_stop_s,
                            config_.creep_decider_config().stop_distance(),
                            StopReasonCode::STOP_REASON_CREEPER, wait_for_obstacles, "CreepDecider",
...
double CreepDecider::FindCreepDistance(const Frame&             frame,
...
bool CreepDecider::CheckCreepDone(const Frame&             frame,
  ...
  double creep_stop_s = traffic_sign_overlap_end_s + FindCreepDistance(frame, reference_line_info);

  const double distance = creep_stop_s - reference_line_info.AdcSlBoundary().end_s();
  if (distance < creep_config.max_valid_stop_distance() || wait_time_sec >= timeout_sec) {
    ...
    for (auto* obstacle : reference_line_info.path_decision().obstacles().Items()) {
      ...
      if (obstacle->reference_line_st_boundary().min_t() < creep_config.min_boundary_t()) {
        ...
        if (obstacle_traveled_s < kepsilon &&
            obstacle->reference_line_st_boundary().min_t() < creep_config.ignore_max_st_min_t() &&
            obstacle->reference_line_st_boundary().min_s() > creep_config.ignore_min_st_min_s()) {
          continue;
        }
        all_far_away = false;
        break;
    ...
    auto* creep_decider_status =
        injector_->planning_context()->mutable_planning_status()->mutable_creep_decider();
