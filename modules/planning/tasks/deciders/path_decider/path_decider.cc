...
bool PathDecider::MakeStaticObstacleDecision(const PathData&     path_data,
  ...
  for (const auto* obstacle : path_decision->obstacles().Items()) {
    ...
    if (obstacle->Id() == blocking_obstacle_id && !injector_->planning_context()
                                                       ->planning_status()
                                                       .path_decider()
      ...
      *object_decision.mutable_stop() = GenerateObjectStopDecision(*obstacle);
      path_decision->AddLongitudinalDecision("PathDecider/blocking_obstacle", obstacle->Id(),
                                             object_decision);
      continue;
    }
    ...
    double min_nudge_l = half_width + config_.path_decider_config().static_obstacle_buffer() / 2.0;
    ...
    if (curr_l - lateral_radius > sl_boundary.end_l() ||
      ...
      *object_decision.mutable_stop() = GenerateObjectStopDecision(*obstacle);
      ...
      if (sl_boundary.end_l() < curr_l - min_nudge_l) {  // &&
        ...
        object_nudge_ptr->set_distance_l(config_.path_decider_config().static_obstacle_buffer());
        path_decision->AddLateralDecision("PathDecider/left-nudge", obstacle->Id(),
                                          object_decision);
...
ObjectStop PathDecider::GenerateObjectStopDecision(const Obstacle& obstacle) const {
  ...
  double stop_distance =
      obstacle.MinRadiusStopDistance(VehicleConfigHelper::GetConfig().vehicle_param());
