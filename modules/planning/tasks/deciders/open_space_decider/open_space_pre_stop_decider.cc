...
Status OpenSpacePreStopDecider::Process(Frame* frame, ReferenceLineInfo* reference_line_info) {
  ...
  open_space_pre_stop_decider_config_ = config_.open_space_pre_stop_decider_config();
  double      target_s                = 0.0;
...
bool OpenSpacePreStopDecider::CheckPullOverPreStop(Frame* const             frame,
...
bool OpenSpacePreStopDecider::CheckParkingSpotPreStop(Frame* const             frame,
                                                      ReferenceLineInfo* const reference_line_info,
                                                      double*                  target_s) {
...
bool OpenSpacePreStopDecider::CheckDeadEndPreStop(Frame* const             frame,
                                                  ReferenceLineInfo* const reference_line_info,
                                                  double*                  target_s) {
...
void OpenSpacePreStopDecider::SetDeadEndStopFence(const double             target_s,
                                                  Frame* const             frame,
                                                  ReferenceLineInfo* const reference_line_info) {
  double stop_line_s             = 0.0;
  double stop_distance_to_target = open_space_pre_stop_decider_config_.stop_distance_to_target();
  ...
  util::BuildStopDecision(stop_wall_id, stop_line_s, 0.0,
                          StopReasonCode::STOP_REASON_PRE_OPEN_SPACE_STOP, wait_for_obstacles,
                          "OpenSpacePreStopDecider", frame, reference_line_info);
}

void OpenSpacePreStopDecider::SetParkingSpotStopFence(
    const double target_s, Frame* const frame, ReferenceLineInfo* const reference_line_info) {
  const auto&         nearby_path      = reference_line_info->reference_line().map_path();
  ...
  if (target_vehicle_offset > stop_distance_to_target) {
    ...
    if (!frame->open_space_info().pre_stop_rightaway_flag()) {
      ...
      stop_line_s =
          adc_front_edge_s + open_space_pre_stop_decider_config_.rightaway_stop_distance();
      ...
      *(frame->mutable_open_space_info()->mutable_pre_stop_rightaway_point()) =
      ...
      nearby_path.GetNearestPoint(frame->open_space_info().pre_stop_rightaway_point(),
...
void OpenSpacePreStopDecider::SetPullOverStopFence(const double             target_s,
                                                   Frame* const             frame,
                                                   ReferenceLineInfo* const reference_line_info) {
  const auto&         nearby_path      = reference_line_info->reference_line().map_path();
  const double        adc_front_edge_s = reference_line_info->AdcSlBoundary().end_s();
