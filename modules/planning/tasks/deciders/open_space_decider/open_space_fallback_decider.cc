...
bool OpenSpaceFallbackDecider::QuardraticFormulaLowerSolution(const double a,
  ...
  *sol = std::abs(std::min(sol1, sol2));
  ADEBUG << "QuardraticFormulaLowerSolution finished with sol: " << *sol << "sol1: " << sol1
         << ", sol2: " << sol2 << "a: " << a << "b: " << b << "c: " << c;
  return true;
}
...
Status OpenSpaceFallbackDecider::Process(Frame* frame) {
  ...
  if (!IsCollisionFreeTrajectory(frame->open_space_info().chosen_partitioned_trajectory(),
    ...
    auto fallback_start_point = fallback_trajectory_pair_candidate.first[fallback_start_index];
    const auto& vehicle_state = injector_->vehicle_state()->vehicle_state();
    fallback_start_point.set_v(vehicle_state.linear_velocity());

    *(frame_->mutable_open_space_info()->mutable_future_collision_point()) = future_collision_point;

    // min stop distance: (max_acc)
    double min_stop_distance = 0.5 * fallback_start_point.v() * fallback_start_point.v() / 4.0;
    ...
    if (fallback_trajectory_pair_candidate.second == canbus::Chassis::GEAR_REVERSE) {
      stop_deceleration = std::min(fallback_start_point.v() * fallback_start_point.v() /
                                       (2.0 * (stop_distance + 1e-6)),
                                   vehicle_max_acc);
      stop_distance     = std::min(-1 * min_stop_distance, stop_distance);
    } else {
      stop_deceleration = std::max(-fallback_start_point.v() * fallback_start_point.v() /
                                       (2.0 * (stop_distance + 1e-6)),
                                   vehicle_max_dec);
    ...
    for (size_t i = fallback_start_index;
      ...
      if (std::abs(fallback_trajectory_pair_candidate.first[i].path_point().s()) >=
    ...
    for (size_t i = 0; i < fallback_start_index; ++i) {
      fallback_trajectory_pair_candidate.first[i].set_v(fallback_start_point.v());
    ...
    if (fallback_start_index == stop_index) {
      ...
      for (int i = 0; i < 20; ++i) {
        ...
        trajectory_point.set_relative_time(
            i * 0.5 + 0.5 + fallback_trajectory_pair_candidate.first[stop_index].relative_time());
        fallback_trajectory_pair_candidate.first.AppendTrajectoryPoint(trajectory_point);
      }
    ...
    ADEBUG << "before change, size : " << fallback_trajectory_pair_candidate.first.size()
    ...
    for (size_t i = fallback_start_index; i <= stop_index; ++i) {
      ...
      double temp_v            = 0.0;
      double c = -2.0 * fallback_trajectory_pair_candidate.first[i].path_point().s();

      if (QuardraticFormulaLowerSolution(stop_deceleration, 2.0 * fallback_start_point.v(), c,
                                         &new_relative_time) &&
          std::abs(fallback_trajectory_pair_candidate.first[i].path_point().s()) <=
              std::abs(stop_distance)) {
        ADEBUG << "new_relative_time" << new_relative_time;
        temp_v = fallback_start_point.v() + stop_deceleration * new_relative_time;
        // speed limit
        if (std::abs(temp_v) < 1.0) {
          fallback_trajectory_pair_candidate.first[i].set_v(temp_v);
        } else {
          fallback_trajectory_pair_candidate.first[i].set_v(temp_v / std::abs(temp_v) * 1.0);
        }
        fallback_trajectory_pair_candidate.first[i].set_a(stop_deceleration);

        fallback_trajectory_pair_candidate.first[i].set_relative_time(new_relative_time);
      } else {
        AINFO << "QuardraticFormulaLowerSolution solving failed, stop "
                 "immediately!";

        if (i != 0) {
          fallback_trajectory_pair_candidate.first[i].mutable_path_point()->CopyFrom(
          ...
          fallback_trajectory_pair_candidate.first[i].set_relative_time(
              fallback_trajectory_pair_candidate.first[i - 1].relative_time() + 0.5);
        } else {
          fallback_trajectory_pair_candidate.first[i].set_v(0.0);
          fallback_trajectory_pair_candidate.first[i].set_a(0.0);
        }
    ...
    ADEBUG << "fallback start point after changes: " << fallback_start_point.DebugString();
...
void OpenSpaceFallbackDecider::BuildPredictedEnvironment(
  ...
  while (relative_time <
         config_.open_space_fallback_decider_config().open_space_prediction_time_period()) {
...
bool OpenSpaceFallbackDecider::IsCollisionFreeTrajectory(
  ...
  *current_index = trajectory_pb.QueryLowerBoundPoint(0.0);
  ...
  for (size_t i = *current_index; i < point_size; ++i) {
    ...
    Box2d ego_box({trajectory_point.path_point().x(), trajectory_point.path_point().y()}, ego_theta,
