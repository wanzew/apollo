...
Status OpenSpaceRoiDecider::Process(Frame* frame) {
  if (frame == nullptr) {
    const std::string msg = "Invalid frame, fail to process the OpenSpaceRoiDecider.";
    AERROR << msg;
  ...
  if (roi_type == OpenSpaceRoiDeciderConfig::PARKING) {
    ...
    if (!GetDeadEndBoundary(frame, dead_end_vertices, nearby_path, &roi_boundary)) {
...
// get origin from ADC
void OpenSpaceRoiDecider::SetOriginFromADC(Frame* const frame, const hdmap::Path& nearby_path) {
  // get ADC box
...
void OpenSpaceRoiDecider::SetDeadEndOrigin(Frame* const              frame,
  ...
  frame->mutable_open_space_info()->mutable_origin_point()->set_x(first_point.x());
  frame->mutable_open_space_info()->mutable_origin_point()->set_y(first_point.y());
}

void OpenSpaceRoiDecider::SetOrigin(Frame* const                              frame,
...
void OpenSpaceRoiDecider::SetDeadEndPose(Frame* const              frame,
                                         const std::vector<Vec2d>& dead_end_vertices) {
...
void OpenSpaceRoiDecider::SetParkingSpotEndPose(
  ...
  const bool   parking_inwards      = config_.open_space_roi_decider_config().parking_inwards();
  const double top_to_down_distance = left_top.y() - left_down.y();
  if (parking_spot_heading > common::math::kMathEpsilon) {
    if (parking_inwards) {
      end_y = left_down.y() -
              (std::max(3.0 * -top_to_down_distance / 4.0, vehicle_params_.front_edge_to_center()) +
               parking_depth_buffer);

    } else {
      end_y = left_down.y() -
              (std::max(-top_to_down_distance / 4.0, vehicle_params_.back_edge_to_center()) +
               parking_depth_buffer);
    }
  } else {
    if (parking_inwards) {
  ...
  if (plot_type == ParkingSpaceType::PARALLEL_PARKING) {
    ...
    end_x = left_top.x() + vehicle_params_.back_edge_to_center() + parllel_park_end_x_buffer;
  }
  auto* end_pose = frame->mutable_open_space_info()->mutable_open_space_end_pose();
  ...
  if (config_.open_space_roi_decider_config().parking_inwards()) {
    ...
    end_pose->push_back(common::math::NormalizeAngle(parking_spot_heading + M_PI));
  }
  end_pose->push_back(0.0);
}
...
void OpenSpaceRoiDecider::SetParkAndGoEndPose(Frame* const frame) {
  const double kSTargetBuffer = config_.open_space_roi_decider_config().end_pose_s_distance();
  ...
  const auto reference_line_info =
      std::min_element(reference_line_list.begin(), reference_line_list.end(),
...
void OpenSpaceRoiDecider::GetRoadBoundary(const hdmap::Path&         nearby_path,
  ...
  double start_s =
      center_line_s - config_.open_space_roi_decider_config().roi_longitudinal_range_start();
  double end_s =
      center_line_s + config_.open_space_roi_decider_config().roi_longitudinal_range_end();
  ...
  while (check_point_s <= end_s) {
    ...
    bool is_anchor_point =
        check_point_s == start_s || check_point_s == end_s || is_center_lane_heading_change;
    // Add key points to the left-half boundary
    AddBoundaryKeyPoint(nearby_path, check_point_s, start_s, end_s, is_anchor_point, true,
                        center_lane_boundary_left, left_lane_boundary, center_lane_s_left,
                        left_lane_road_width);
    // Add key points to the right-half boundary
    AddBoundaryKeyPoint(nearby_path, check_point_s, start_s, end_s, is_anchor_point, false,
                        center_lane_boundary_right, right_lane_boundary, center_lane_s_right,
                        right_lane_road_width);
    if (check_point_s == end_s) { break; }
    index += 1.0;
    check_point_s =
        start_s + index * config_.open_space_roi_decider_config().roi_line_segment_length();
...
void OpenSpaceRoiDecider::GetRoadBoundaryFromMap(const hdmap::Path&   nearby_path,
                                                 const double         center_line_s,
  ...
  while (check_point_s <= end_s) {
    ...
    check_point_s =
        check_point_s + config_.open_space_roi_decider_config().roi_line_segment_length_from_map();
  }
    ...
    sort(left_lane_boundary->begin(), left_lane_boundary->end(),
         [](const Vec2d& first_pt, const Vec2d& second_pt) {
           return first_pt.x() < second_pt.x() ||
                  (first_pt.x() == second_pt.x() && first_pt.y() < second_pt.y());
         });
...
void OpenSpaceRoiDecider::AddBoundaryKeyPoint(const hdmap::Path&   nearby_path,
  ...
  const double previous_distance_s = std::min(
      config_.open_space_roi_decider_config().roi_line_segment_length(), check_point_s - start_s);
  const double next_distance_s = std::min(
      config_.open_space_roi_decider_config().roi_line_segment_length(), end_s - check_point_s);
  ...
  double current_road_width          = is_left_curb ? nearby_path.GetRoadLeftWidth(check_point_s) :
  ...
  if (is_anchor_point) {
    ...
    Vec2d curb_lane_point =
        Vec2d(current_road_width * point_vec_cos, current_road_width * point_vec_sin);
  ...
  double previous_road_width =
      is_left_curb ? nearby_path.GetRoadLeftWidth(check_point_s - previous_distance_s) :
  ...
  if (std::abs(current_curb_point_delta_theta) >
      config_.open_space_roi_decider_config().curb_heading_tangent_change_upper_limit()) {
    double point_vec_cos = is_left_curb ? std::cos(current_check_point_heading + M_PI / 2.0) :
                                          std::cos(current_check_point_heading - M_PI / 2.0);
    double point_vec_sin = is_left_curb ? std::sin(current_check_point_heading + M_PI / 2.0) :
                                          std::sin(current_check_point_heading - M_PI / 2.0);
...
void OpenSpaceRoiDecider::GetInLaneEndPoint(LaneInfoConstPtr lane_info,
  ...
  const auto& left_boundary_segment  = lane_info->lane().left_boundary().curve().segment();
  const auto& right_boundary_segment = lane_info->lane().right_boundary().curve().segment();
  size_t      lane_points_num        = left_boundary_segment[0].line_segment().point_size();
  *left_boundary_point  = left_boundary_segment[0].line_segment().point(lane_points_num - 1);
  *right_boundary_point = right_boundary_segment[0].line_segment().point(lane_points_num - 1);
}
...
void OpenSpaceRoiDecider::GetInLaneBoundaryPoints(LaneInfoConstPtr       lane_info,
  ...
  for (size_t i = 0; i < lane_points_num; ++i) {
    PointENU left_point  = left_boundary_segment[0].line_segment().point().at(i);
    PointENU right_point = right_boundary_segment[0].line_segment().point().at(i);
    left_points.push_back(left_point);
    right_points.push_back(right_point);
    Vec2d left_point_v  = {left_boundary_segment[0].line_segment().point().at(i).x(),
                          left_boundary_segment[0].line_segment().point().at(i).y()};
    Vec2d right_point_v = {right_boundary_segment[0].line_segment().point().at(i).x(),
  ...
  std::reverse(In_right_boundary_points->begin(), In_right_boundary_points->end());
}
...
void OpenSpaceRoiDecider::GetOutLaneBoundaryPoints(
  ...
  Vec2d                 target_position = {routing_target_point_.x(), routing_target_point_.y()};
  ...
  std::reverse(Out_left_boundary_points->begin(), Out_left_boundary_points->end());
}
...
bool OpenSpaceRoiDecider::GetDeadEndBoundary(
  ...
  hdmap_->GetNearestLane(routing_end_point_, &start_nearest_lane, &nearest_s, &nearest_l);
  ...
  hdmap_->GetNearestLane(routing_target_point_, &end_nearest_lane, &nearest_s, &nearest_l);
  GetOutLaneStartPoint(end_nearest_lane, &left_start_point, &right_start_point);
  ...
  hdmap_->GetNearestLane(routing_target_point_, &target_lane, &nearest_s, &nearest_l);
  std::vector<PointENU> Out_left_boundary_points;
  std::vector<PointENU> Out_right_boundary_points;
  GetOutLaneBoundaryPoints(target_lane, nearby_path, &Out_left_boundary_points,
                           &Out_right_boundary_points);

  std::vector<Vec2d> point_boundary;
  for (size_t i = 0; i < In_right_boundary_points.size(); ++i) {
    point_boundary.push_back({In_right_boundary_points[i].x(), In_right_boundary_points[i].y()});
  }
  ...
  for (size_t i = 0; i < Out_right_boundary_points.size(); ++i) {
    point_boundary.push_back({Out_right_boundary_points[i].x(), Out_right_boundary_points[i].y()});
  }
  for (size_t i = 0; i < Out_left_boundary_points.size(); ++i) {
    point_boundary.push_back({Out_left_boundary_points[i].x(), Out_left_boundary_points[i].y()});
  }
  ...
  for (size_t i = 0; i < In_left_boundary_points.size(); ++i) {
    point_boundary.push_back({In_left_boundary_points[i].x(), In_left_boundary_points[i].y()});
  }
  point_boundary.push_back({In_right_boundary_points[0].x(), In_right_boundary_points[0].y()});
  const auto& origin_point   = frame->open_space_info().origin_point();
  const auto& origin_heading = frame->open_space_info().origin_heading();
  ...
  // Get xy boundary
  auto                xminmax = std::minmax_element(point_boundary.begin(), point_boundary.end(),
                                     [](const Vec2d& a, const Vec2d& b) { return a.x() < b.x(); });
  auto                yminmax = std::minmax_element(point_boundary.begin(), point_boundary.end(),
                                     [](const Vec2d& a, const Vec2d& b) { return a.y() < b.y(); });
  std::vector<double> ROI_xy_boundary{xminmax.first->x(), xminmax.second->x(), yminmax.first->y(),
                                      yminmax.second->y()};
...
bool OpenSpaceRoiDecider::GetParkingBoundary(
  ...
  GetRoadBoundary(nearby_path, center_line_s, origin_point, origin_heading, &left_lane_boundary,
                  &right_lane_boundary, &center_lane_boundary_left, &center_lane_boundary_right,
  ...
  if (average_l < 0) {
    ...
    for (size_t i = point_right_to_right_top_connor_index; i < right_lane_boundary_last_index;
         i++) {
      std::vector<Vec2d> segment{right_lane_boundary[i], right_lane_boundary[i + 1]};
      roi_parking_boundary->push_back(segment);
    }
    ...
    if (point_right_to_right_top_connor_index > 0) { --point_right_to_right_top_connor_index; }
    auto point_right_to_right_top_connor_itr =
    ...
    std::reverse_copy(point_left_to_left_top_connor_itr, left_lane_boundary.end(),
                      std::back_inserter(boundary_points));
...
bool OpenSpaceRoiDecider::GetPullOverBoundary(
  ...
  // Fuse line segments into convex contraints
  if (!FuseLineSegments(roi_parking_boundary)) { return false; }
  // Get xy boundary
  auto                xminmax = std::minmax_element(boundary_points.begin(), boundary_points.end(),
                                     [](const Vec2d& a, const Vec2d& b) { return a.x() < b.x(); });
  auto                yminmax = std::minmax_element(boundary_points.begin(), boundary_points.end(),
...
bool OpenSpaceRoiDecider::GetParkAndGoBoundary(
  ...
  if (FLAGS_use_road_boundary_from_map) {
    GetRoadBoundaryFromMap(nearby_path, center_line_s, origin_point, origin_heading,
...
bool OpenSpaceRoiDecider::GetParkingSpot(Frame* const          frame,
  ...
  if (previous_open_space_info.target_parking_lane() != nullptr &&
    ...
    int status = HDMapUtil::BaseMap().GetNearestLaneWithHeading(
        point, 10.0, vehicle_state_.heading(), M_PI / 2.0, &nearest_lane, &vehicle_lane_s,
        &vehicle_lane_l);
    if (status != 0) {
  ...
  LaneSegment nearest_lanesegment = LaneSegment(nearest_lane, nearest_lane->accumulate_s().front(),
  ...
  if (next_lanes_num != 0) {
    for (int i = 0; i < next_lanes_num; ++i) {
      ...
      LaneSegment next_lanesegment = LaneSegment(next_lane, next_lane->accumulate_s().front(),
      ...
      *nearby_path = Path(segments_vector);
      ...
      if (target_parking_spot != nullptr) { break; }
    }
  } else {
    segments_vector.push_back(nearest_lanesegment);
    *nearby_path = Path(segments_vector);
  ...
  if (plot_type == ParkingSpaceType::PARALLEL_PARKING) {
    ...
    double extend_right_x_buffer = config_.open_space_roi_decider_config().extend_right_x_buffer();
    double extend_left_x_buffer  = config_.open_space_roi_decider_config().extend_left_x_buffer();
...
bool OpenSpaceRoiDecider::GetPullOverSpot(Frame* const                        frame,
...
void OpenSpaceRoiDecider::SearchTargetParkingSpotOnPath(
    const hdmap::Path& nearby_path, ParkingSpaceInfoConstPtr* target_parking_spot) {
...
bool OpenSpaceRoiDecider::CheckDistanceToParkingSpot(
  ...
  nearby_path.GetNearestPoint(left_bottom_point, &left_bottom_point_s, &left_bottom_point_l);
  nearby_path.GetNearestPoint(right_bottom_point, &right_bottom_point_s, &right_bottom_point_l);
  ...
  if (std::abs((left_bottom_point_s + right_bottom_point_s) / 2 - vehicle_point_s) <
...
bool OpenSpaceRoiDecider::FuseLineSegments(
  ...
  while (cur_segment != line_segments_vec->end() - 1) {
    ...
    if (CrossProd(cur_second_to_last_point, cur_last_point, next_second_point) < 0.0) {
      ...
      if (next_segment->empty()) { line_segments_vec->erase(next_segment); }
    } else {
      ++cur_segment;
    }
  }
...
bool OpenSpaceRoiDecider::FormulateBoundaryConstraints(
    const std::vector<std::vector<common::math::Vec2d>>& roi_parking_boundary, Frame* const frame) {
...
bool OpenSpaceRoiDecider::LoadObstacleInVertices(
  ...
  if (config_.open_space_roi_decider_config().enable_perception_obstacles()) {
    if (perception_obstacles_num == 0) { ADEBUG << "no obstacle given by perception"; }
    ...
    for (const auto& obstacle : obstacles_by_frame_->Items()) {
      if (FilterOutObstacle(*frame, *obstacle)) { continue; }
...
bool OpenSpaceRoiDecider::FilterOutObstacle(const Frame& frame, const Obstacle& obstacle) {
  if (obstacle.IsVirtual()) { return true; }
  ...
  const double filtering_distance =
      config_.open_space_roi_decider_config().perception_obstacle_filtering_distance();
...
bool OpenSpaceRoiDecider::LoadObstacleInHyperPlanes(Frame* const frame) {
  ...
  *(frame->mutable_open_space_info()->mutable_obstacles_b()) =
  ...
  if (!GetHyperPlanes(frame->open_space_info().obstacles_num(),
...
bool OpenSpaceRoiDecider::IsInParkingLot(const double          adc_init_x,
...
void OpenSpaceRoiDecider::GetParkSpotFromMap(ParkingSpaceInfoConstPtr parking_lot,
                                             std::array<Vec2d, 4>*    vertices) {
  // left or right of the parking lot is decided when viewing the parking spot
  // open upward
