...
RssDecider::RssDecider(const TaskConfig& config)
    : Task(config) {}

apollo::common::Status RssDecider::Execute(Frame* frame, ReferenceLineInfo* reference_line_info) {
...
Status RssDecider::Process(Frame* frame, ReferenceLineInfo* reference_line_info) {
  ...
  if (reference_line_info->path_data().Empty() || reference_line_info->speed_data().empty()) {
  ...
  for (const auto* obstacle : path_decision->obstacles().Items()) {
    ...
    bool is_on_road =
        reference_line_info->reference_line().HasOverlap(obstacle->PerceptionBoundingBox());
    if (!is_on_road) { continue; }
    const auto& obstacle_sl = obstacle->PerceptionSLBoundary();
    if (obstacle_sl.end_s() <= ego_v_s_start) { continue; }
  ...
  if (front_obstacle_distance >= FLAGS_rss_max_front_obstacle_distance) {
    ...
    reference_line_info->mutable_rss_info()->set_cur_dist_lon(front_obstacle_distance);
    reference_line_info->mutable_rss_info()->set_rss_safe_dist_lon(front_obstacle_distance);
  ...
  occupiedRegion_leading.lonRange.minimum = ParametricValue(nearest_obs_s_start / lane_length);
  occupiedRegion_leading.lonRange.maximum = ParametricValue(nearest_obs_s_end / lane_length);
  ...
  rss_world_info.OR_front_lon_min = static_cast<double>(occupiedRegion_leading.lonRange.minimum);
  rss_world_info.OR_front_lon_max = static_cast<double>(occupiedRegion_leading.lonRange.maximum);
  rss_world_info.OR_front_lat_min = static_cast<double>(occupiedRegion_leading.latRange.minimum);
  rss_world_info.OR_front_lat_max = static_cast<double>(occupiedRegion_leading.latRange.maximum);
  ...
  occupiedRegion_following.lonRange.minimum = ParametricValue(ego_v_s_start / lane_length);
  occupiedRegion_following.lonRange.maximum = ParametricValue(ego_v_s_end / lane_length);
  ...
  occupiedRegion_following.latRange.maximum =
      ParametricValue(std::abs(nearest_obs_l_start - lane_leftmost) / lane_width);

  followingObject.occupiedRegions.push_back(occupiedRegion_following);

  rss_world_info.adc_vel         = adc_velocity;
  rss_world_info.OR_rear_lon_min = static_cast<double>(occupiedRegion_following.lonRange.minimum);
  rss_world_info.OR_rear_lon_max = static_cast<double>(occupiedRegion_following.lonRange.maximum);
  rss_world_info.OR_rear_lat_min = static_cast<double>(occupiedRegion_following.latRange.minimum);
  rss_world_info.OR_rear_lat_max = static_cast<double>(occupiedRegion_following.latRange.maximum);
  ...
  rss_world_info.laneSeg_len_min   = static_cast<double>(laneSegment.length.minimum);
  rss_world_info.laneSeg_len_max   = static_cast<double>(laneSegment.length.maximum);
  rss_world_info.laneSeg_width_min = static_cast<double>(laneSegment.width.minimum);
  rss_world_info.laneSeg_width_max = static_cast<double>(laneSegment.width.maximum);
  ...
  bool                               rss_result =
      ::ad_rss::core::RssSituationExtraction::extractSituations(worldModel, situationVector);
  if (!rss_result) {
  ...
  rss_result = RssResponse.provideProperResponse(responseStateVector, properResponse);
  ...
  Distance const      currentLonDistance = situationVector[0].relativePosition.longitudinalDistance;
  Distance            safeLonDistance;
  VehicleState const& leadingVehicleState   = situationVector[0].otherVehicleState;
  VehicleState const& followingVehicleState = situationVector[0].egoVehicleState;

  rss_result = ::ad_rss::situation::calculateSafeLongitudinalDistanceSameDirection(
      leadingVehicleState, followingVehicleState, safeLonDistance);
  if (!rss_result) {
    rss_world_info.err_code = ("ad_rss::calculateSafeLongitudinalDistanceSameDirection failed");
    rss_dump_world_info(rss_world_info);
    return Status(ErrorCode::PLANNING_ERROR, rss_world_info.err_code);
  }
  ...
  if (responseStateVector[0].longitudinalState.isSafe) {
    ...
    if (FLAGS_enable_rss_fallback) { reference_line_info->mutable_speed_data()->clear(); }
  }
  ...
  ADEBUG << " longitudinalState.isSafe: " << responseStateVector[0].longitudinalState.isSafe;
  ADEBUG << " lateralStateLeft.isSafe: " << responseStateVector[0].lateralStateLeft.isSafe;
  ADEBUG << " lateralStateRight.isSafe: " << responseStateVector[0].lateralStateRight.isSafe;
  ADEBUG << " is_rss_safe : " << reference_line_info->rss_info().is_rss_safe();
  ADEBUG << " cur_dist_lon: " << reference_line_info->rss_info().cur_dist_lon();
  ADEBUG << " is_rss_safe : " << reference_line_info->rss_info().is_rss_safe();
  ADEBUG << " cur_dist_lon: " << reference_line_info->rss_info().cur_dist_lon();
  ADEBUG << " rss_safe_dist_lon: " << reference_line_info->rss_info().rss_safe_dist_lon();
...
void RssDecider::rss_config_default_dynamics(::ad_rss::world::Dynamics* dynamics) {
...
void RssDecider::rss_dump_world_info(const struct rss_world_model_struct& rss_info) {
  AERROR << " RSS_INFO :"
         << " front_obs_dist: " << rss_info.front_obs_dist
         << " obs_s_start: " << rss_info.obs_s_start << " obs_s_end: " << rss_info.obs_s_end
         << " obs_l_start: " << rss_info.obs_l_start << " obs_l_end: " << rss_info.obs_l_end
         << " obs_speed: " << rss_info.obs_speed << " ego_v_s_start: " << rss_info.ego_v_s_start
         << " ego_v_s_end: " << rss_info.ego_v_s_end << " ego_v_l_start: " << rss_info.ego_v_l_start
         << " ego_v_l_end: " << rss_info.ego_v_l_end << " lane_leftmost: " << rss_info.lane_leftmost
         << " lane_rightmost: " << rss_info.lane_rightmost << " lane_width: " << rss_info.lane_width
         ...
         << " OR_rear_lat_max: " << rss_info.OR_rear_lat_max << " adc_vel: " << rss_info.adc_vel
