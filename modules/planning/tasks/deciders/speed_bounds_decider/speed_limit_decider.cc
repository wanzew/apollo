...
Status SpeedLimitDecider::GetSpeedLimits(const IndexedList<std::string, Obstacle>& obstacles,
  ...
  for (uint32_t i = 0; i < discretized_path.size(); ++i) {
    ...
    for (const auto* ptr_obstacle : obstacles.Items()) {
      if (ptr_obstacle->IsVirtual()) { continue; }
      if (!ptr_obstacle->LateralDecision().has_nudge()) { continue; }
      ...
      if (vehicle_front_s < obstacle_back_s || vehicle_back_s > obstacle_front_s) { continue; }
      ...
      if (is_close_on_left || is_close_on_right) {
        ...
        if (ptr_obstacle->IsStatic()) {
          ...
          nudge_speed_ratio = speed_bounds_config_.dynamic_obs_nudge_speed_ratio();
        }
    ...
    if (FLAGS_enable_nudge_slowdown) {
      curr_speed_limit =
          std::fmax(speed_bounds_config_.lowest_speed(),
                    std::min({speed_limit_from_reference_line, speed_limit_from_centripetal_acc,
