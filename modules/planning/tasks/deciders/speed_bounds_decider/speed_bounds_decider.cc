...
Status SpeedBoundsDecider::Process(Frame* const             frame,
  ...
  if (!FLAGS_use_st_drivable_boundary) { path_decision->EraseStBoundaries(); }
  ...
  ADEBUG << "Time for ST Boundary Mapping = " << diff.count() * 1000 << " msec.";
  ...
  st_graph_data->LoadData(boundaries, min_s_on_st_boundaries, init_point, speed_limit,
                          reference_line_info->GetCruiseSpeed(), path_data_length,
...
double SpeedBoundsDecider::SetSpeedFallbackDistance(PathDecision* const path_decision) {
  ...
  for (auto* obstacle : path_decision->obstacles().Items()) {
    ...
    if (st_boundary.IsEmpty()) { continue; }
    ...
    if (left_bottom_point_s - right_bottom_point_s > kEpsilon) {
      if (min_s_reverse > lowest_s) { min_s_reverse = lowest_s; }
