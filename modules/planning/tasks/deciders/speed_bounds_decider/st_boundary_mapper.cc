...
bool STBoundaryMapper::MapStopDecision(Obstacle*                 stop_obstacle,
  ...
  if (stop_ref_s > path_data_.frenet_frame_path().back().s()) {
    ...
    if (!path_data_.GetPathPointWithRefS(stop_ref_s, &stop_point)) { return false; }
...
void STBoundaryMapper::ComputeSTBoundary(Obstacle* obstacle) const {
  if (FLAGS_use_st_drivable_boundary) { return; }
  std::vector<STPoint> lower_points;
  std::vector<STPoint> upper_points;

  if (!GetOverlapBoundaryPoints(path_data_.discretized_path(), *obstacle, &upper_points,
                                &lower_points)) {
    return;
  }
...
bool STBoundaryMapper::GetOverlapBoundaryPoints(const std::vector<PathPoint>& path_points,
  ...
  if (trajectory.trajectory_point().empty()) {
    ...
    for (const auto& curr_point_on_path : path_points) {
      if (curr_point_on_path.s() > planning_max_distance_) { break; }
      ...
      if (CheckOverlap(curr_point_on_path, obs_box, l_buffer)) {
        ...
        double       high_s =
            std::fmin(planning_max_distance_, curr_point_on_path.s() + forward_distance);
    ...
    for (int i = 0; i < trajectory.trajectory_point_size(); ++i) {
      ...
      auto         path_len    = std::min(FLAGS_max_trajectory_len, discretized_path.Length());
...
void STBoundaryMapper::ComputeSTBoundaryWithDecision(Obstacle*                 obstacle,
  ...
  if (decision.has_follow()) {
    ...
    boundary =
        STBoundary::CreateInstance(lower_points, upper_points).ExpandByS(characteristic_length);
...
bool STBoundaryMapper::CheckOverlap(const PathPoint& path_point,
  ...
  Box2d adc_box(ego_center_map_frame, path_point.theta(), vehicle_param_.length(),
