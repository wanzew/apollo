...
LearningModelInferenceTask::LearningModelInferenceTask(
  ...
  trajectory_imitation_inference_ = std::make_unique<TrajectoryImitationLibtorchInference>(
      config.learning_model_inference_task_config());
}

Status LearningModelInferenceTask::Execute(Frame* frame, ReferenceLineInfo* reference_line_info) {
  CHECK_NOTNULL(frame);
...
Status LearningModelInferenceTask::Process(Frame* frame) {
  ...
  if (!injector_->learning_based_data() ||
    ...
    if (config.allow_empty_learning_based_data()) { return Status::OK(); }
    return Status(ErrorCode::PLANNING_ERROR, msg);
  }

  LearningDataFrame learning_data_frame;
  learning_data_frame.CopyFrom(*(injector_->learning_based_data()->GetLatestLearningDataFrame()));

  ADEBUG << "LearningModelInferenceTask: frame_num[" << learning_data_frame.frame_num()
         << "] adc_trajectory_point_size[" << learning_data_frame.adc_trajectory_point_size()
         << "]";

  if (learning_data_frame.adc_trajectory_point_size() <= 0) {
    const std::string msg = absl::StrCat("learning_data adc_trajectory_point empty. frame_num[",
                                         learning_data_frame.frame_num(), "]");
    AERROR << msg;
    // hybrid model will use rule based planning when learning model output is
    // not ready
    if (config.allow_empty_output_trajectory()) { return Status::OK(); }
    return Status(ErrorCode::PLANNING_ERROR, msg);
  }

  const double start_point_timestamp_sec =
      learning_data_frame.adc_trajectory_point(learning_data_frame.adc_trajectory_point_size() - 1)
          .timestamp_sec();
  ...
  trajectory_evaluator.EvaluateADCTrajectory(start_point_timestamp_sec, config.trajectory_delta_t(),
                                             &learning_data_frame);

  // evaluate obstacle trajectory
  trajectory_evaluator.EvaluateObstacleTrajectory(
      start_point_timestamp_sec, config.trajectory_delta_t(), &learning_data_frame);
  ...
  if (!trajectory_imitation_inference_->LoadModel()) {
    const std::string msg =
        absl::StrCat("TrajectoryImitationInference LoadModel() failed. frame_num[",
                     learning_data_frame.frame_num(), "]");
    AERROR << msg;
    return Status(ErrorCode::PLANNING_ERROR, msg);
  }
  if (!trajectory_imitation_inference_->DoInference(&learning_data_frame)) {
    const std::string msg =
        absl::StrCat("TrajectoryImitationLibtorchInference Inference failed. frame_num[",
                     learning_data_frame.frame_num(), "]");
    AERROR << msg;
    return Status(ErrorCode::PLANNING_ERROR, msg);
  }
  ...
  ADEBUG << "   adc_future_trajectory_point_size[" << adc_future_trajectory_point_size << "]";
  if (adc_future_trajectory_point_size < 10) {
    const std::string msg = absl::StrCat(
        "too short adc_future_trajectory_point. frame_num[", learning_data_frame.frame_num(),
        "] adc_future_trajectory_point_size[", adc_future_trajectory_point_size, "]");
    AERROR << msg;
    return Status(ErrorCode::PLANNING_ERROR, msg);
  }
  ...
  for (const auto& tp : learning_data_frame.output().adc_future_trajectory_point()) {
    future_trajectory.push_back(tp);
  }

  TrajectoryPointFeature tp;
  const int              last = learning_data_frame.adc_trajectory_point_size() - 1;
  tp.set_timestamp_sec(learning_data_frame.adc_trajectory_point(last).timestamp_sec());
  ...
  trajectory_evaluator.EvaluateADCFutureTrajectory(
      learning_data_frame.frame_num(), future_trajectory, start_point_timestamp_sec,
      kADCFutureTrajectoryDeltaTime, &evaluated_future_trajectory);

  // convert to common::TrajectoryPoint
  std::vector<common::TrajectoryPoint> adc_future_trajectory;
  ConvertADCFutureTrajectory(evaluated_future_trajectory, &adc_future_trajectory);
  ADEBUG << "adc_future_trajectory_size: " << adc_future_trajectory.size();

  injector_->learning_based_data()->set_learning_data_adc_future_trajectory_points(
...
void LearningModelInferenceTask::ConvertADCFutureTrajectory(
  ...
  for (size_t i = 0; i + 1 < adc_future_trajectory->size(); ++i) {
    ...
    const auto&  next_relative_time = (*adc_future_trajectory)[i + 1].relative_time();
    const double cur_a              = (next_v - cur_v) / (next_relative_time - cur_relative_time);
  ...
  for (size_t i = 0; i + 1 < adc_future_trajectory->size(); ++i) {
    ...
    const auto&  next_relative_time = (*adc_future_trajectory)[i + 1].relative_time();
    const double cur_da             = (next_a - cur_a) / (next_relative_time - cur_relative_time);
  ...
  for (size_t i = 0; i + 1 < adc_future_trajectory->size(); ++i) {
    ...
    const double cur_kappa =
        apollo::common::math::NormalizeAngle(next_path_point.theta() - cur_path_point->theta()) /
  ...
  for (size_t i = 0; i + 1 < adc_future_trajectory->size(); ++i) {
    auto*        cur_path_point  = (*adc_future_trajectory)[i].mutable_path_point();
    const auto&  next_path_point = (*adc_future_trajectory)[i + 1].path_point();
    const double cur_dkappa      = (next_path_point.kappa() - cur_path_point->kappa()) /
                              (next_path_point.s() - cur_path_point->s());
  ...
  for (size_t i = 0; i + 1 < adc_future_trajectory->size(); ++i) {
    auto*        cur_path_point  = (*adc_future_trajectory)[i].mutable_path_point();
    const auto&  next_path_point = (*adc_future_trajectory)[i + 1].path_point();
    const double cur_ddkappa     = (next_path_point.dkappa() - cur_path_point->dkappa()) /
                               (next_path_point.s() - cur_path_point->s());
