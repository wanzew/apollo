...
TEST_F(SpeedLimitDeciderTest, get_centric_acc_limit) {
  ...
  SLBoundary        sl;
  SpeedLimitDecider speed_limit_decider(sl, config, *reference_line_, path_data_);
  ...
  while (kappa < 0.2) {
    ...
    if (v > config.high_speed_threshold()) {
      EXPECT_DOUBLE_EQ(acc_limit, config.high_speed_centric_acceleration_limit());
    } else if (v < config.low_speed_threshold()) {
      EXPECT_DOUBLE_EQ(acc_limit, config.low_speed_centric_acceleration_limit());
    } else {
