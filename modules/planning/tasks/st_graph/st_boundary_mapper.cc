...
Status StBoundaryMapper::CreateStBoundary(PathDecision* path_decision) const {
  ...
  for (const auto* const_path_obstacle : path_obstacles.Items()) {
    ...
    if (!path_obstacle->HasLongitudinalDecision()) {
      MapWithoutDecision(path_obstacle);
      continue;
    }
    ...
    if (decision.has_stop()) {
      const double stop_s =
          path_obstacle->PerceptionSLBoundary().start_s() + decision.stop().distance_s();
    ...
    // follow/overtake/yield decision
      ...
      if (!MapWithDecision(path_obstacle, decision).ok()) {
        ...
        return Status(ErrorCode::PLANNING_ERROR, "Fail to map overtake/yield decision");
      }
    } else {
...
Status StBoundaryMapper::CreateStBoundaryWithHistory(const ObjectDecisions& history_decisions,
  ...
  for (const auto& history_decision : history_decisions.decision()) {
    for (const auto& decision : history_decision.object_decision()) {
      if (PathObstacle::IsLongitudinalDecision(decision) && !decision.has_ignore()) {
  ...
  for (const auto* const_path_obstacle : path_obstacles.Items()) {
    ...
    if (!path_obstacle->HasLongitudinalDecision()) {
      if (!MapWithoutDecision(path_obstacle).ok()) {
        std::string msg =
            StrCat("Fail to map obstacle ", path_obstacle->Id(), " without decision.");
        AERROR << msg;
        return Status(ErrorCode::PLANNING_ERROR, msg);
      }
      if (path_obstacle->st_boundary().IsEmpty() || decision.has_ignore()) { continue; }
    }
    if (path_obstacle->HasLongitudinalDecision()) {
    ...
    if (decision.has_stop()) {
      ...
      if (stop_s + stop_buff < adc_sl_boundary_.end_s()) {
        AERROR << "Invalid stop decision. not stop at behind of current "
                  "position. stop_s : "
               << stop_s << ", and current adc_s is; " << adc_sl_boundary_.end_s();
        return Status(ErrorCode::PLANNING_ERROR, "invalid decision");
      }
      if (stop_s < min_stop_s) {
        stop_obstacle = path_obstacle;
...
bool StBoundaryMapper::MapStopDecision(PathObstacle*             stop_obstacle,
  ...
  const double s_max = std::fmax(s_min, std::fmax(planning_distance_, reference_line_.Length()));

  std::vector<std::pair<STPoint, STPoint>> point_pairs;
  point_pairs.emplace_back(STPoint(s_min, 0.0), STPoint(s_max, 0.0));
  point_pairs.emplace_back(STPoint(s_min, planning_time_),
...
bool StBoundaryMapper::GetOverlapBoundaryPoints(const std::vector<PathPoint>& path_points,
  ...
  if (trajectory.trajectory_point_size() == 0) {
    ...
    for (const auto& curr_point_on_path : path_points) {
      if (curr_point_on_path.s() > planning_distance_) { break; }

      // 因为目标是静止的，所以obs_box就只有一个框
    ...
    if (path_points.size() > 2 * default_num_point) {
      ...
      for (size_t i = 0; i < path_points.size(); ++i) {
        if (i % ratio == 0) { sampled_path_points.push_back(path_points[i]); }
      }
    ...
    for (int i = 0; i < trajectory.trajectory_point_size(); ++i) {
      ...
      for (double path_s = 0.0; path_s < discretized_path.Length(); path_s += step_length) {
        ...
        if (CheckOverlap(curr_adc_path_point, obs_box, st_boundary_config_.boundary_buffer())) {
          ...
          while (low_s < high_s) {
            if (find_low && find_high) { break; }
            if (!find_low) {
              const auto& point_low =
                  discretized_path.Evaluate(low_s + discretized_path.StartPoint().s());
              if (!CheckOverlap(point_low, obs_box, st_boundary_config_.boundary_buffer())) {
            ...
            if (!find_high) {
              const auto& point_high =
                  discretized_path.Evaluate(high_s + discretized_path.StartPoint().s());
              if (!CheckOverlap(point_high, obs_box, st_boundary_config_.boundary_buffer())) {
          ...
          if (find_high && find_low) {
            lower_points->emplace_back(low_s - st_boundary_config_.point_extension(),
                                       trajectory_point_time);
            upper_points->emplace_back(high_s + st_boundary_config_.point_extension(),
                                       trajectory_point_time);
          }
          break;
        }
...
Status StBoundaryMapper::MapWithDecision(PathObstacle*             path_obstacle,
  ...
  DCHECK(decision.has_follow() || decision.has_yield() || decision.has_overtake());
  ...
  if (decision.has_follow() && lower_points.back().t() < planning_time_) {
    ...
    double       extend_lower_s =
        diff_s / diff_t * (planning_time_ - lower_points.front().t()) + lower_points.front().s();
  ...
  // error
    ...
    DCHECK(false) << "Obj decision should be either yield or overtake: " << decision.DebugString();
  }
...
bool StBoundaryMapper::CheckOverlap(const PathPoint& path_point,
  ...
  Vec2d vec_to_center =
      Vec2d((vehicle_param_.front_edge_to_center() - vehicle_param_.back_edge_to_center()) / 2.0,
  ...
  const Box2d adc_box = Box2d(center, path_point.theta(), vehicle_param_.length() + 2 * buffer,
