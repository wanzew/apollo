...
void SpeedLimitDecider::GetAvgKappa(const std::vector<common::PathPoint>& path_points,
  ...
  int    start = 0;
  int    end   = 0;
  while (end < static_cast<int>(path_points.size()) && end - start < kHalfNumPoints + 1) {
...
Status
SpeedLimitDecider::GetSpeedLimits(const IndexedList<std::string, PathObstacle>& path_obstacles,
  ...
  const auto& discretized_path_points = path_data_.discretized_path().path_points();
  ...
  for (uint32_t i = 0; i < discretized_path_points.size(); ++i) {
    ...
    if (frenet_point_s > reference_line_.Length()) {
      AWARN << "path length [" << frenet_point_s << "] is LARGER than reference_line_ length ["
    ...
    if (i + 1 < discretized_path_points.size()) {
      ...
      const double centri_jerk = std::fabs(avg_kappa[i + 1] - avg_kappa[i]) / (ds + kEpsilon);
      centri_jerk_speed_limit =
          std::fmax(10.0, st_boundary_config_.centri_jerk_speed_coeff() / centri_jerk);
    }
    ...
    for (const auto* const_path_obstacle : path_obstacles.Items()) {
      if (const_path_obstacle->obstacle()->IsVirtual()) { continue; }
      if (!const_path_obstacle->LateralDecision().has_nudge()) { continue; }
      ...
      bool is_close_on_left = (nudge.type() == ObjectNudge::LEFT_NUDGE) &&
      ...
      bool is_close_on_right = (nudge.type() == ObjectNudge::RIGHT_NUDGE) &&
      ...
      if (is_close_on_left || is_close_on_right) {
        ...
        nudge_obstacle_speed_limit = nudge_speed_ratio * speed_limit_on_reference_line;
        break;
      }
    }
    ...
    if (FLAGS_enable_nudge_slowdown) {
      ...
      curr_speed_limit = std::fmax(
          ...
          common::util::MinElement(std::vector<double>{
              speed_limit_on_reference_line, centri_acc_speed_limit, centri_jerk_speed_limit}));
    }
...
double SpeedLimitDecider::GetCentricAccLimit(const double kappa) const {
  ...
  const double h_v_acc = st_boundary_config_.high_speed_centric_acceleration_limit();
  const double l_v_acc = st_boundary_config_.low_speed_centric_acceleration_limit();
  ...
  if (kappa < kMinKappaEpsilon) { return h_v_acc; }
