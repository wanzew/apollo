...
bool PolyStGraph::GenerateMinCostSpeedProfile(const std::vector<std::vector<STPoint>>& points,
                                              const std::vector<const PathObstacle*>&  obstacles,
                                              PolyStGraphNode* const min_cost_node) {
  CHECK_NOTNULL(min_cost_node);
  PolyStGraphNode  start_node = {STPoint(0.0, 0.0), init_point_.v(), init_point_.a()};
  ...
  for (const auto& level : points) {
    for (const auto& st_point : level) {
      ...
      for (double v = 0; v < speed_limit + kEpsilon; v += speed_limit / num_speed) {
        PolyStGraphNode node = {st_point, v, 0.0};
        node.speed_profile =
            QuarticPolynomialCurve1d(0.0, start_node.speed, start_node.accel, node.st_point.s(),
                                     node.speed, node.st_point.t());
        const double c = cost.Calculate(node.speed_profile, st_point.t(), min_cost);
        if (c < min_cost) {
          *min_cost_node = node;
          min_cost       = c;
        }
...
bool PolyStGraph::SampleStPoints(std::vector<std::vector<STPoint>>* const points) {
