...
ChangeLane::ChangeLane(const TrafficRuleConfig& config)
    : TrafficRule(config) {}
...
bool ChangeLane::FilterObstacles(ReferenceLineInfo* reference_line_info) {
  ...
  for (const auto* path_obstacle : path_decision->path_obstacles().Items()) {
    ...
    const auto& last_point = *(obstacle->Trajectory().trajectory_point().rbegin());
    if (last_point.v() < config_.change_lane().min_guard_speed()) { continue; }
    if (!reference_line.IsOnRoad(last_point.path_point())) { continue; }
    SLPoint last_sl;
    if (!reference_line.XYToSL(last_point.path_point(), &last_sl)) { continue; }
  ...
  if (first_guard_vehicle) { guard_obstacles_.push_back(first_guard_vehicle); }
  return true;
}

bool ChangeLane::CreateGuardObstacle(const ReferenceLineInfo* reference_line_info,
                                     Obstacle*                obstacle) {
  if (!obstacle || !obstacle->HasTrajectory()) { return false; }
  const auto& last_point = *(obstacle->Trajectory().trajectory_point().rbegin());

  const double kStepDistance  = obstacle->PerceptionBoundingBox().length();
  double       extend_v       = std::max(last_point.v(), config_.change_lane().min_guard_speed());
  const double time_delta     = kStepDistance / extend_v;
  const auto&  reference_line = reference_line_info->reference_line();
  const double end_s =
      std::min(reference_line.Length(), reference_line_info->AdcSlBoundary().end_s() +
                                            config_.change_lane().guard_distance());
  SLPoint sl_point;
  if (!reference_line.XYToSL(last_point.path_point(), &sl_point)) { return false; }
  ...
  for (double t = last_point.relative_time() + time_delta; ref_s < end_s;
    ...
    Vec2d xy_point;
    if (!reference_line.SLToXY(common::util::MakeSLPoint(ref_s, sl_point.l()), &xy_point)) {
      return false;
    }
...
Status ChangeLane::ApplyRule(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
  ...
  if (reference_line_info->Lanes().IsOnSegment()) { return Status::OK(); }
  ...
  if (!overtake_obstacles_.empty()) {
    ...
    for (const auto* path_obstacle : overtake_obstacles_) {
      ...
      path_decision->AddLongitudinalDecision(TrafficRuleConfig::RuleId_Name(Id()),
...
ObjectDecisionType ChangeLane::CreateOvertakeDecision(const ReferenceLine& reference_line,
