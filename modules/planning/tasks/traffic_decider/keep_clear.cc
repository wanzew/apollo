...
Status KeepClear::ApplyRule(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
  ...
  if (config_.keep_clear().enable_keep_clear_zone()) {
    ...
    for (const auto& keep_clear_overlap : keep_clear_overlaps) {
      ...
      if (BuildKeepClearObstacle(frame, reference_line_info,
                                 const_cast<PathOverlap*>(&keep_clear_overlap), obstacle_id)) {
  ...
  if (config_.keep_clear().enable_junction()) {
    ...
    for (const auto& junction_overlap : junction_overlaps) {
      const auto obstacle_id = KEEP_CLEAR_JUNCTION_VO_ID_PREFIX + junction_overlap.object_id;

      if (BuildKeepClearObstacle(frame, reference_line_info,
                                 const_cast<PathOverlap*>(&junction_overlap), obstacle_id)) {
        ADEBUG << "KEEP_CLAER for junction[" << junction_overlap.object_id << "] s["
               << junction_overlap.start_s << ", " << junction_overlap.end_s << "] BUILD";
      }
    }
  }
...
bool KeepClear::BuildKeepClearObstacle(Frame* const             frame,
  ...
  if (adc_front_edge_s - keep_clear_overlap->start_s > config_.keep_clear().min_pass_s_distance()) {
    ADEBUG << "adc inside keep_clear zone[" << keep_clear_overlap->object_id << "] s["
           << keep_clear_overlap->start_s << ", " << keep_clear_overlap->end_s
           << "] adc_front_edge_s[" << adc_front_edge_s << "]. skip this keep clear zone";
    return false;
  }
