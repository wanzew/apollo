 ...
 * @brief: make decision
 */
void StopSign::MakeDecisions(Frame* frame, ReferenceLineInfo* const reference_line_info) {
  CHECK_NOTNULL(frame);
  CHECK_NOTNULL(reference_line_info);
  ...
  if (stop_status_ == StopSignStatus::DRIVE || stop_status_ == StopSignStatus::STOP ||
    ...
    if (stop_status_ == StopSignStatus::DRIVE) {
      for (const auto* path_obstacle : path_decision->path_obstacles().Items()) {
        ...
        AddWatchVehicle(*path_obstacle, &watch_vehicles);
               ...
               (stop_status_ == StopSignStatus::STOP || stop_status_ == StopSignStatus::WAIT)) {
      // get all vehicles currently watched
      std::vector<std::string> watch_vehicle_ids;
      for (StopSignLaneVehicles::iterator it = watch_vehicles.begin(); it != watch_vehicles.end();
           ++it) {
        std::copy(it->second.begin(), it->second.end(), std::back_inserter(watch_vehicle_ids));
      }
  ...
  if (stop_status_ == StopSignStatus::STOP_DONE) {
    ...
    for (const PathOverlap& stop_sign_overlap : stop_sign_overlaps) {
      if (stop_sign_status.stop_sign_id() == stop_sign_overlap.object_id) {
        if (adc_back_edge_s - stop_sign_overlap.end_s > config_.stop_sign().min_pass_s_distance()) {
          GetPlanningStatus()->clear_stop_sign();
        }
        break;
      }
    ...
    // creep: stop decition
    double creep_stop_s =
        next_stop_sign_overlap_.end_s + config_.stop_sign().creep().creep_distance_to_stop_line();
    const std::string stop_wall_id = STOP_SIGN_CREEP_VO_ID_PREFIX + stop_sign_id;
    ...
    // stop decision
    const std::string stop_wall_id = STOP_SIGN_VO_ID_PREFIX + next_stop_sign_overlap_.object_id;
    BuildStopDecision(frame, reference_line_info, stop_wall_id, next_stop_sign_overlap_.start_s,
...
bool StopSign::FindNextStopSign(ReferenceLineInfo* const reference_line_info) {
  ...
  for (const PathOverlap& stop_sign_overlap : stop_sign_overlaps) {
    ...
    if (adc_front_edge_s - stop_sign_overlap.end_s <= config_.stop_sign().min_pass_s_distance() &&
  ...
  if (next_stop_sign_overlap_.object_id.empty()) { return false; }

  next_stop_sign_ =
      HDMapUtil::BaseMap().GetStopSignById(hdmap::MakeMapId(next_stop_sign_overlap_.object_id));
  if (!next_stop_sign_) {
  ...
  if (stop_sign_status.stop_sign_id() != next_stop_sign_->id().id()) {
    GetPlanningStatus()->clear_stop_sign();
    GetPlanningStatus()->mutable_stop_sign()->set_stop_sign_id(next_stop_sign_->id().id());
  }
...
int StopSign::GetAssociatedLanes(const StopSignInfo& stop_sign_info) {
  ...
  HDMapUtil::BaseMap().GetStopSignAssociatedStopSigns(stop_sign_info.id(), &associated_stop_signs);
  ...
  for (const auto stop_sign : associated_stop_signs) {
    ...
    for (const auto& lane_id : associated_lane_ids) {
      ...
      for (auto stop_sign_overlap : stop_sign_overlaps) {
        auto over_lap_info = stop_sign_overlap->GetObjectOverlapInfo(stop_sign.get()->id());
...
bool StopSign::CheckCreep(const hdmap::StopSignInfo& stop_sign_info) {
  if (config_.stop_sign().creep().enabled()) {
    ...
    if (associated_lanes_.size() <= 1) { return true; }
  }
  return false;
}
...
bool StopSign::CheckCreepDone(ReferenceLineInfo* const reference_line_info) {
  ...
  if (distance < config_.stop_sign().creep().max_valid_stop_distance()) {
    ...
    for (auto* path_obstacle : reference_line_info->path_decision()->path_obstacles().Items()) {
...
int StopSign::ProcessStopStatus(ReferenceLineInfo* const reference_line_info,
  ...
  if (!stop_sign_status->has_status()) { stop_sign_status->set_status(StopSignStatus::UNKNOWN); }
  stop_status_ = stop_sign_status->status();

  // adjust status
  double adc_front_edge_s  = reference_line_info->AdcSlBoundary().end_s();
  double stop_line_start_s = next_stop_sign_overlap_.start_s;
  if (stop_line_start_s - adc_front_edge_s > config_.stop_sign().max_valid_stop_distance()) {
  ...
  ADEBUG << "stop_start_time: " << stop_start_time << "; wait_time: " << wait_time;
  ...
  switch (stop_status_) {
    ...
    case StopSignStatus::STOP:
      if (wait_time >= config_.stop_sign().stop_duration()) {
        if (watch_vehicles != nullptr && !watch_vehicles->empty()) {
          ...
          stop_status_ =
              CheckCreep(stop_sign_info) ? StopSignStatus::CREEP : StopSignStatus::STOP_DONE;
        }
      }
      break;
    ...
    case StopSignStatus::CREEP:
      if (CheckCreepDone(reference_line_info)) { stop_status_ = StopSignStatus::STOP_DONE; }
      break;
    case StopSignStatus::STOP_DONE: break;
    default: break;
  }
...
bool StopSign::CheckADCkStop(ReferenceLineInfo* const reference_line_info) {
  ...
  double adc_front_edge_s                     = reference_line_info->AdcSlBoundary().end_s();
  double stop_line_start_s                    = next_stop_sign_overlap_.start_s;
  double distance_stop_line_to_adc_front_edge = stop_line_start_s - adc_front_edge_s;
  ADEBUG << "distance_stop_line_to_adc_front_edge[" << distance_stop_line_to_adc_front_edge
         << "]; stop_line_start_s[" << stop_line_start_s << "]; adc_front_edge_s["
         << adc_front_edge_s << "]";

  if (distance_stop_line_to_adc_front_edge > config_.stop_sign().max_valid_stop_distance()) {
...
int StopSign::GetWatchVehicles(const StopSignInfo&   stop_sign_info,
  ...
  for (int i = 0; i < stop_sign_status.lane_watch_vehicles_size(); ++i) {
    ...
    ADEBUG << "GetWatchVehicles watch_vehicles: lane_id[" << associated_lane_id << "] vehicle[" << s
           << "]";
  }
...
int StopSign::UpdateWatchVehicles(StopSignLaneVehicles* watch_vehicles) {
  ...
  for (auto it = watch_vehicles->begin(); it != watch_vehicles->end(); ++it) {
    ...
    ADEBUG << "UpdateWatchVehicles watch_vehicles: lane_id[" << it->first << "] vehicle[" << s
           << "]";
  }
...
int StopSign::AddWatchVehicle(const PathObstacle&   path_obstacle,
  ...
  const PerceptionObstacle& perception_obstacle = path_obstacle.obstacle()->Perception();
  ...
  if (HDMapUtil::BaseMap().GetNearestLaneWithHeading(point, 5.0, perception_obstacle.theta(),
                                                     M_PI / 3.0, &obstacle_lane, &obstacle_s,
                                                     &obstacle_l) != 0) {
    ADEBUG << "obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name
           << "]: Failed to find nearest lane from map for position: " << point.DebugString()
  ...
  auto        assoc_lane_it    = std::find_if(
      ...
      [&obstable_lane_id](std::pair<LaneInfoConstPtr, OverlapInfoConstPtr>& assc_lane) {
        return assc_lane.first.get()->id().id() == obstable_lane_id;
      });
  if (assoc_lane_it == associated_lanes_.end()) {
    ADEBUG << "obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name << "] lane_id["
           << obstable_lane_id << "] not associated with current stop_sign. skip";
    return -1;
  }
  ...
  */
  ...
  auto over_lap_info = assoc_lane_it->second.get()->GetObjectOverlapInfo(obstacle_lane.get()->id());
  if (over_lap_info == nullptr) {
  ...
  if (distance_to_stop_line > config_.stop_sign().watch_vehicle_max_valid_stop_distance()) {
    ADEBUG << "obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name
           << "] distance_to_stop_line[" << distance_to_stop_line << "]; stop_line_s" << stop_line_s
           << "]; obstacle_end_s[" << obstacle_end_s << "] too far from stop line. skip";
    return -1;
  }

  // use a vector since motocycles/bicycles can be more than one
  std::vector<std::string> vehicles = (*watch_vehicles)[obstacle_lane->id().id()];
  if (std::find(vehicles.begin(), vehicles.end(), obstacle_id) == vehicles.end()) {
    ADEBUG << "AddWatchVehicle: lane[" << obstacle_lane->id().id() << "] obstacle_id["
           << obstacle_id << "]";
 ...
 */
int StopSign::RemoveWatchVehicle(const PathObstacle&             path_obstacle,
  ...
  if (std::find(watch_vehicle_ids.begin(), watch_vehicle_ids.end(), obstacle_id) ==
  ...
  if (assoc_lane_it != associated_lanes_.end()) {
    ...
    auto over_lap_info =
        assoc_lane_it->second.get()->GetObjectOverlapInfo(obstacle_lane.get()->id());
    if (over_lap_info == nullptr) {
      AERROR << "can't find over_lap_info for id: " << obstable_lane_id;
      return -1;
    }
    ...
    if (distance_pass_stop_line > config_.stop_sign().min_pass_s_distance() && !is_path_cross) {
      ...
      ADEBUG << "obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name
             << "] distance_pass_stop_line[" << distance_pass_stop_line << "] stop_line_end_s["
             << stop_line_end_s << "] obstacle_end_s[" << obstacle_end_s << "] is_path_cross["
             << is_path_cross << "] passed stop sign, AND path not crosses. "
    ...
    if (!is_path_cross) {
      erase = true;
      ADEBUG << "obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name
             << "] obstable_lane_id[" << obstable_lane_id << "] is_path_cross[" << is_path_cross
...
int StopSign::ClearWatchVehicle(ReferenceLineInfo* const reference_line_info,
  ...
  const auto& path_obstacles = reference_line_info->path_decision()->path_obstacles().Items();
...
int StopSign::BuildStopDecision(Frame*                   frame,
  ...
  auto* obstacle = frame->CreateStopObstacle(reference_line_info, stop_wall_id, stop_line_s);
  if (!obstacle) {
