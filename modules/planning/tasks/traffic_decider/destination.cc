 ...
 * @brief: make decision
 */
void Destination::MakeDecisions(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
  CHECK_NOTNULL(frame);
  CHECK_NOTNULL(reference_line_info);

  if (FLAGS_use_navigation_mode) { return; }

  if (!frame->is_near_destination()) { return; }
...
int Destination::BuildStopDecision(Frame* frame, ReferenceLineInfo* const reference_line_info) {
  ...
  double      dest_lane_s = std::max(0.0, routing_end.s() - FLAGS_virtual_stop_wall_length -
...
bool Destination::CheckPullOver(ReferenceLineInfo* const reference_line_info,
  ...
  // check dest OnRoad
  double dest_lane_s = std::max(0.0, lane_s - FLAGS_virtual_stop_wall_length -
                                         config_.destination().stop_distance());
  *dest_point        = dest_lane->GetSmoothPoint(dest_lane_s);
  if (!reference_line.IsOnRoad(*dest_point)) { return false; }
  ...
  ADEBUG << "adc_front_edge_s[" << adc_front_edge_s << "] distance_to_dest[" << distance_to_dest
