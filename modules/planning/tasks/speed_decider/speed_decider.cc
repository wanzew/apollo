...
apollo::common::Status SpeedDecider::Execute(Frame* frame, ReferenceLineInfo* reference_line_info) {
...
SpeedDecider::StPosition SpeedDecider::GetStPosition(const SpeedData&  speed_profile,
  ...
  if (st_boundary.IsEmpty()) { return st_position; }
  ...
  for (size_t i = 0; i + 1 < speed_profile.speed_vector().size(); ++i) {
    const STPoint curr_st(speed_profile.speed_vector()[i].s(), speed_profile.speed_vector()[i].t());
...
bool SpeedDecider::IsFollowTooClose(const PathObstacle& path_obstacle) const {
  if (!path_obstacle.IsBlockingObstacle()) { return false; }

  if (path_obstacle.st_boundary().min_t() > 0.0) { return false; }
  ...
  const double distance  = path_obstacle.st_boundary().min_s() - FLAGS_min_stop_distance_obstacle;
    ...
    switch (position) {
      case BELOW:
        if (boundary.boundary_type() == StBoundary::BoundaryType::KEEP_CLEAR) {
          ...
          if (CreateStopDecision(*path_obstacle, &stop_decision, -FLAGS_stop_line_stop_distance)) {
            path_obstacle->AddLongitudinalDecision("dp_st_graph/keep_clear", stop_decision);
          }
        } else if (CheckIsFollowByT(boundary) &&
                   (boundary.max_t() - boundary.min_t() > FLAGS_follow_min_time_sec)) {
          ...
          if (IsFollowTooClose(*path_obstacle)) {
            ...
            if (CreateStopDecision(*path_obstacle, &stop_decision,
              ...
              path_obstacle->AddLongitudinalDecision("dp_st_graph/too_close", stop_decision);
            }
      ...
      case ABOVE:
        if (boundary.boundary_type() == StBoundary::BoundaryType::KEEP_CLEAR) {
          ...
          if (CreateOvertakeDecision(*path_obstacle, &overtake_decision)) {
            path_obstacle->AddLongitudinalDecision("dp_st_graph/overtake", overtake_decision);
          }
        }
        break;
      case CROSS:
        if (obstacle->IsBlockingObstacle()) {
          ...
          if (CreateStopDecision(*path_obstacle, &stop_decision,
                                 -FLAGS_min_stop_distance_obstacle)) {
            path_obstacle->AddLongitudinalDecision("dp_st_graph/cross", stop_decision);
          }
          const std::string msg =
              "Failed to find a solution for crossing obstacle:" + obstacle->Id();
          AERROR << msg;
          return Status(ErrorCode::PLANNING_ERROR, msg);
        }
        break;
      default: AERROR << "Unknown position:" << position;
    }
    AppendIgnoreDecision(path_obstacle);
  }
  return Status::OK();
...
bool SpeedDecider::CreateStopDecision(const PathObstacle&       path_obstacle,
  ...
  PerceptionObstacle::Type obstacle_type = path_obstacle.obstacle()->Perception().type();
  ADEBUG << "STOP: obstacle_id[" << path_obstacle.obstacle()->Id() << "] obstacle_type["
         << PerceptionObstacle_Type_Name(obstacle_type) << "]";

  return true;
}

bool SpeedDecider::CreateFollowDecision(const PathObstacle&       path_obstacle,
  ...
  PerceptionObstacle::Type obstacle_type = path_obstacle.obstacle()->Perception().type();
  ADEBUG << "FOLLOW: obstacle_id[" << path_obstacle.obstacle()->Id() << "] obstacle_type["
         << PerceptionObstacle_Type_Name(obstacle_type) << "]";

  return true;
}

bool SpeedDecider::CreateYieldDecision(const PathObstacle&       path_obstacle,
  ...
  switch (obstacle_type) {
    ...
    case PerceptionObstacle::BICYCLE:
      ...
      break;
    default: yield_distance = FLAGS_yield_distance; break;
  }
  ...
  ADEBUG << "YIELD: obstacle_id[" << path_obstacle.obstacle()->Id() << "] obstacle_type["
         << PerceptionObstacle_Type_Name(obstacle_type) << "]";

  return true;
}

bool SpeedDecider::CreateOvertakeDecision(const PathObstacle&       path_obstacle,
  ...
  PerceptionObstacle::Type obstacle_type = path_obstacle.obstacle()->Perception().type();
  ADEBUG << "OVERTAKE: obstacle_id[" << path_obstacle.obstacle()->Id() << "] obstacle_type["
         << PerceptionObstacle_Type_Name(obstacle_type) << "]";
...
bool SpeedDecider::CheckIsFollowByT(const StBoundary& boundary) const {
  if (boundary.BottomLeftPoint().s() > boundary.BottomRightPoint().s()) { return false; }
