...
class ObstacleContainer {
 public:
  ...
  bool VPresentationObstacle(const double* ROI_distance_approach_parking_boundary) {
    ...
    for (size_t i = 0; i < obstacles_num_; i++) {
      ...
      for (int j = 0; j < obstacles_edges_num_(i, 0) + 1; j++) {
        Vec2d vertice = Vec2d(ROI_distance_approach_parking_boundary[index],
  ...
  bool HPresentationObstacle() {
    ...
    if (!ObsHRep(obstacles_num_, obstacles_edges_num_, obstacles_vertices_vec_, &obstacles_A_,
                 &obstacles_b_)) {
  ...
  const std::vector<std::vector<Vec2d>>& GetObstacleVec() const { return obstacles_vertices_vec_; }
  ...
  const Eigen::MatrixXi& GetObstaclesEdgesNum() const { return obstacles_edges_num_; }
...
HybridAStar* CreateHybridAPtr() {
  ...
  ACHECK(apollo::cyber::common::GetProtoFromFile(FLAGS_planner_open_space_config_filename,
...
ObstacleContainer* DistanceCreateObstaclesPtr() { return new ObstacleContainer(); }
...
bool DistanceSmoothing(const apollo::planning::PlannerOpenSpaceConfig& planner_open_space_config,
  ...
  if (horizon_ <= 10 && horizon_ > 2 && planner_open_space_config.enable_linear_interpolation()) {
    ...
    phi = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(hybrid_a_star_result->phi.data(),
                                                        horizon_ + 1);
    v = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(hybrid_a_star_result->v.data(), horizon_ + 1);
    steer =
        Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(hybrid_a_star_result->steer.data(), horizon_);
  ...
  if (FLAGS_use_dual_variable_warm_start) {
    bool dual_variable_warm_start_status = dual_variable_warm_start_ptr->Solve(
        horizon_, ts_, ego_, obstacles.GetObstaclesNum(), obstacles.GetObstaclesEdgesNum(),
        obstacles.GetAMatrix(), obstacles.GetbMatrix(), xWS, &l_warm_up, &n_warm_up, &s_warm_up);
  ...
  bool status = distance_approach_ptr->Solve(
      x0, xF, last_time_u, horizon_, ts_, ego_, xWS, uWS, l_warm_up, n_warm_up, s_warm_up, XYbounds,
      obstacles.GetObstaclesNum(), obstacles.GetObstaclesEdgesNum(), obstacles.GetAMatrix(),
      obstacles.GetbMatrix(), state_result_ds_, control_result_ds_, time_result_ds_,
      dual_l_result_ds_, dual_n_result_ds_);
...
bool DistancePlan(HybridAStar*       hybridA_ptr,
  ...
  if (!hybridA_ptr->Plan(sx, sy, sphi, ex, ey, ephi, XYbounds_, obstacles_ptr->GetObstacleVec(),
                         &hybrid_astar_result)) {
  ...
  if (FLAGS_enable_parallel_trajectory_smoothing) {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (!DistanceSmoothing(planner_open_space_config_, *obstacles_ptr, piece_wise_sx,
                             piece_wise_sy, piece_wise_sphi, piece_wise_ex, piece_wise_ey,
                             piece_wise_ephi, XYbounds_, &partition_trajectories[i],
    ...
    *(result_ptr->GetIpoptTime())         = ipopt_total;
    ...
    if (!DistanceSmoothing(planner_open_space_config_, *obstacles_ptr, sx, sy, sphi, ex, ey, ephi,
                           XYbounds_, &hybrid_astar_result, &state_result_ds, &control_result_ds,
                           &time_result_ds, &dual_l_result_ds, &dual_n_result_ds, dual_total,
                           ipopt_total)) {
      return false;
    }
    *(result_ptr->PrepareHybridAResult()) = hybrid_astar_result;
