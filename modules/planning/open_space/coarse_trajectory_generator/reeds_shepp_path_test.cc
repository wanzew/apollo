...
class reeds_shepp : public ::testing::Test {
 public:
  virtual void SetUp() {
    ASSERT_TRUE(cyber::common::GetProtoFromFile(FLAGS_planner_open_space_config_filename,
                                                &planner_open_space_config_));
    vehicle_param_ = common::VehicleConfigHelper::GetConfig().vehicle_param();
    reedshepp_test =
        std::unique_ptr<ReedShepp>(new ReedShepp(vehicle_param_, planner_open_space_config_));
  ...
  void check(std::shared_ptr<Node3d>        start_node,
    ...
    for (size_t i = 1; i < (*optimal_path).x.size(); ++i) {
      double gold_interval =
          std::sqrt(planner_open_space_config_.warm_start_config().step_size() *
                        planner_open_space_config_.warm_start_config().step_size() +
                    planner_open_space_config_.warm_start_config().step_size() *
                        planner_open_space_config_.warm_start_config().step_size());
      double interval = std::sqrt(((*optimal_path).x.at(i) - (*optimal_path).x.at(i - 1)) *
...
TEST_F(reeds_shepp, test_set_1) {
  std::shared_ptr<Node3d> start_node = std::shared_ptr<Node3d>(
      new Node3d(0.0, 0.0, 10.0 * M_PI / 180.0, XYbounds_, planner_open_space_config_));
  std::shared_ptr<Node3d> end_node = std::shared_ptr<Node3d>(
      new Node3d(7.0, -8.0, 50.0 * M_PI / 180.0, XYbounds_, planner_open_space_config_));
...
TEST_F(reeds_shepp, test_set_3) {
  std::shared_ptr<Node3d> start_node = std::shared_ptr<Node3d>(
      new Node3d(0.0, 10.0, -10.0 * M_PI / 180.0, XYbounds_, planner_open_space_config_));
  std::shared_ptr<Node3d> end_node = std::shared_ptr<Node3d>(
      new Node3d(-7.0, -8.0, -50.0 * M_PI / 180.0, XYbounds_, planner_open_space_config_));
  std::shared_ptr<ReedSheppPath> optimal_path = std::shared_ptr<ReedSheppPath>(new ReedSheppPath());
  if (!reedshepp_test->ShortestRSP(start_node, end_node, optimal_path)) {
...
TEST_F(reeds_shepp, test_set_4) {
  std::shared_ptr<Node3d> start_node = std::shared_ptr<Node3d>(
      new Node3d(0.0, 10.0, -10.0 * M_PI / 180.0, XYbounds_, planner_open_space_config_));
  std::shared_ptr<Node3d> end_node = std::shared_ptr<Node3d>(
      new Node3d(-7.0, -8.0, 150.0 * M_PI / 180.0, XYbounds_, planner_open_space_config_));
  std::shared_ptr<ReedSheppPath> optimal_path = std::shared_ptr<ReedSheppPath>(new ReedSheppPath());
  if (!reedshepp_test->ShortestRSP(start_node, end_node, optimal_path)) {
    ADEBUG << "generating short RSP not successful";
  }
