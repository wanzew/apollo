...
ReedShepp::ReedShepp(const common::VehicleParam&   vehicle_param,
  ...
  max_kappa_ = std::tan(vehicle_param_.max_steer_angle() / vehicle_param_.steer_ratio()) /
...
std::pair<double, double> ReedShepp::calc_tau_omega(
    const double u, const double v, const double xi, const double eta, const double phi) {
...
bool ReedShepp::ShortestRSP(const std::shared_ptr<Node3d>  start_node,
  ...
  if (std::abs(all_possible_paths[optimal_path_index].x.back() - end_node->GetX()) > 1e-3 ||
      std::abs(all_possible_paths[optimal_path_index].y.back() - end_node->GetY()) > 1e-3 ||
      std::abs(all_possible_paths[optimal_path_index].phi.back() - end_node->GetPhi()) > 1e-3) {
    ADEBUG << "RSP end position not right";
    for (size_t i = 0; i < all_possible_paths[optimal_path_index].segs_types.size(); ++i) {
      ADEBUG << "types are " << all_possible_paths[optimal_path_index].segs_types[i];
    }
    ADEBUG << "x, y, phi are: " << all_possible_paths[optimal_path_index].x.back() << ", "
...
bool ReedShepp::GenerateRSP(const std::shared_ptr<Node3d> start_node,
  ...
  if (!SCS(x, y, dphi, all_possible_paths)) { ADEBUG << "Fail at SCS"; }
  if (!CSC(x, y, dphi, all_possible_paths)) { ADEBUG << "Fail at CSC"; }
  if (!CCC(x, y, dphi, all_possible_paths)) { ADEBUG << "Fail at CCC"; }
  if (!CCCC(x, y, dphi, all_possible_paths)) { ADEBUG << "Fail at CCCC"; }
  if (!CCSC(x, y, dphi, all_possible_paths)) { ADEBUG << "Fail at CCSC"; }
  if (!CCSCC(x, y, dphi, all_possible_paths)) { ADEBUG << "Fail at CCSCC"; }
...
bool ReedShepp::SCS(const double                x,
                    const double                y,
                    const double                phi,
                    std::vector<ReedSheppPath>* all_possible_paths) {
  RSPParam SLS_param;
...
bool ReedShepp::CSC(const double                x,
                    const double                y,
                    const double                phi,
                    std::vector<ReedSheppPath>* all_possible_paths) {
  RSPParam LSL1_param;
  ...
  char   LSL1_types[]    = "LSL";
  if (LSL1_param.flag && !SetRSP(3, LSL1_lengths, LSL1_types, all_possible_paths)) {
  ...
  char   LSL2_types[]    = "LSL";
  if (LSL2_param.flag && !SetRSP(3, LSL2_lengths, LSL2_types, all_possible_paths)) {
  ...
  char   LSL3_types[]    = "RSR";
  if (LSL3_param.flag && !SetRSP(3, LSL3_lengths, LSL3_types, all_possible_paths)) {
  ...
  char   LSL4_types[]    = "RSR";
  if (LSL4_param.flag && !SetRSP(3, LSL4_lengths, LSL4_types, all_possible_paths)) {
  ...
  char   LSR1_types[]    = "LSR";
  if (LSR1_param.flag && !SetRSP(3, LSR1_lengths, LSR1_types, all_possible_paths)) {
  ...
  char   LSR2_types[]    = "LSR";
  if (LSR2_param.flag && !SetRSP(3, LSR2_lengths, LSR2_types, all_possible_paths)) {
  ...
  char   LSR3_types[]    = "RSL";
  if (LSR3_param.flag && !SetRSP(3, LSR3_lengths, LSR3_types, all_possible_paths)) {
  ...
  char   LSR4_types[]    = "RSL";
  if (LSR4_param.flag && !SetRSP(3, LSR4_lengths, LSR4_types, all_possible_paths)) {
...
bool ReedShepp::CCC(const double                x,
                    const double                y,
                    const double                phi,
                    std::vector<ReedSheppPath>* all_possible_paths) {
  RSPParam LRL1_param;
  ...
  char   LRL1_types[]    = "LRL";
  if (LRL1_param.flag && !SetRSP(3, LRL1_lengths, LRL1_types, all_possible_paths)) {
  ...
  char   LRL2_types[]    = "LRL";
  if (LRL2_param.flag && !SetRSP(3, LRL2_lengths, LRL2_types, all_possible_paths)) {
  ...
  char   LRL3_types[]    = "RLR";
  if (LRL3_param.flag && !SetRSP(3, LRL3_lengths, LRL3_types, all_possible_paths)) {
  ...
  char   LRL4_types[]    = "RLR";
  if (LRL4_param.flag && !SetRSP(3, LRL4_lengths, LRL4_types, all_possible_paths)) {
  ...
  char   LRL5_types[]    = "LRL";
  if (LRL5_param.flag && !SetRSP(3, LRL5_lengths, LRL5_types, all_possible_paths)) {
  ...
  char   LRL6_types[]    = "LRL";
  if (LRL6_param.flag && !SetRSP(3, LRL6_lengths, LRL6_types, all_possible_paths)) {
  ...
  char   LRL7_types[]    = "RLR";
  if (LRL7_param.flag && !SetRSP(3, LRL7_lengths, LRL7_types, all_possible_paths)) {
  ...
  char   LRL8_types[]    = "RLR";
  if (LRL8_param.flag && !SetRSP(3, LRL8_lengths, LRL8_types, all_possible_paths)) {
...
bool ReedShepp::CCCC(const double                x,
                     const double                y,
                     const double                phi,
                     std::vector<ReedSheppPath>* all_possible_paths) {
  ...
  if (LRLRn1_param.flag && !SetRSP(4, LRLRn1_lengths, LRLRn1_types, all_possible_paths)) {
  ...
  if (LRLRn2_param.flag && !SetRSP(4, LRLRn2_lengths, LRLRn2_types, all_possible_paths)) {
  ...
  if (LRLRn3_param.flag && !SetRSP(4, LRLRn3_lengths, LRLRn3_types, all_possible_paths)) {
  ...
  if (LRLRn4_param.flag && !SetRSP(4, LRLRn4_lengths, LRLRn4_types, all_possible_paths)) {
  ...
  if (LRLRp1_param.flag && !SetRSP(4, LRLRp1_lengths, LRLRp1_types, all_possible_paths)) {
  ...
  if (LRLRp2_param.flag && !SetRSP(4, LRLRp2_lengths, LRLRp2_types, all_possible_paths)) {
  ...
  if (LRLRp3_param.flag && !SetRSP(4, LRLRp3_lengths, LRLRp3_types, all_possible_paths)) {
  ...
  if (LRLRp4_param.flag && !SetRSP(4, LRLRp4_lengths, LRLRp4_types, all_possible_paths)) {
...
bool ReedShepp::CCSC(const double                x,
                     const double                y,
                     const double                phi,
                     std::vector<ReedSheppPath>* all_possible_paths) {
  ...
  if (LRSL1_param.flag && !SetRSP(4, LRSL1_lengths, LRSL1_types, all_possible_paths)) {
  ...
  if (LRSL2_param.flag && !SetRSP(4, LRSL2_lengths, LRSL2_types, all_possible_paths)) {
  ...
  if (LRSL3_param.flag && !SetRSP(4, LRSL3_lengths, LRSL3_types, all_possible_paths)) {
  ...
  if (LRSL4_param.flag && !SetRSP(4, LRSL4_lengths, LRSL4_types, all_possible_paths)) {
  ...
  if (LRSR1_param.flag && !SetRSP(4, LRSR1_lengths, LRSR1_types, all_possible_paths)) {
  ...
  if (LRSR2_param.flag && !SetRSP(4, LRSR2_lengths, LRSR2_types, all_possible_paths)) {
  ...
  if (LRSR3_param.flag && !SetRSP(4, LRSR3_lengths, LRSR3_types, all_possible_paths)) {
  ...
  if (LRSR4_param.flag && !SetRSP(4, LRSR4_lengths, LRSR4_types, all_possible_paths)) {
  ...
  if (LRSL5_param.flag && !SetRSP(4, LRSL5_lengths, LRSL5_types, all_possible_paths)) {
    ADEBUG << "Fail at SetRSP with LRLRn_param";
    return false;
  }
  ...
  if (LRSL6_param.flag && !SetRSP(4, LRSL6_lengths, LRSL6_types, all_possible_paths)) {
  ...
  if (LRSL7_param.flag && !SetRSP(4, LRSL7_lengths, LRSL7_types, all_possible_paths)) {
  ...
  if (LRSL8_param.flag && !SetRSP(4, LRSL8_lengths, LRSL8_types, all_possible_paths)) {
  ...
  if (LRSR5_param.flag && !SetRSP(4, LRSR5_lengths, LRSR5_types, all_possible_paths)) {
  ...
  if (LRSR6_param.flag && !SetRSP(4, LRSR6_lengths, LRSR6_types, all_possible_paths)) {
  ...
  if (LRSR7_param.flag && !SetRSP(4, LRSR7_lengths, LRSR7_types, all_possible_paths)) {
  ...
  if (LRSR8_param.flag && !SetRSP(4, LRSR8_lengths, LRSR8_types, all_possible_paths)) {
...
bool ReedShepp::CCSCC(const double                x,
                      const double                y,
                      const double                phi,
                      std::vector<ReedSheppPath>* all_possible_paths) {
  RSPParam LRSLR1_param;
  LRSLR(x, y, phi, &LRSLR1_param);
  double LRSLR1_lengths[5] = {LRSLR1_param.t, -0.5 * M_PI, LRSLR1_param.u, -0.5 * M_PI,
                              LRSLR1_param.v};
  char   LRSLR1_types[]    = "LRSLR";
  if (LRSLR1_param.flag && !SetRSP(5, LRSLR1_lengths, LRSLR1_types, all_possible_paths)) {
  ...
  double LRSLR2_lengths[5] = {-LRSLR2_param.t, 0.5 * M_PI, -LRSLR2_param.u, 0.5 * M_PI,
                              -LRSLR2_param.v};
  char   LRSLR2_types[]    = "LRSLR";
  if (LRSLR2_param.flag && !SetRSP(5, LRSLR2_lengths, LRSLR2_types, all_possible_paths)) {
  ...
  double LRSLR3_lengths[5] = {LRSLR3_param.t, -0.5 * M_PI, LRSLR3_param.u, -0.5 * M_PI,
                              LRSLR3_param.v};
  char   LRSLR3_types[]    = "RLSRL";
  if (LRSLR3_param.flag && !SetRSP(5, LRSLR3_lengths, LRSLR3_types, all_possible_paths)) {
  ...
  double LRSLR4_lengths[5] = {-LRSLR4_param.t, 0.5 * M_PI, -LRSLR4_param.u, 0.5 * M_PI,
                              -LRSLR4_param.v};
  char   LRSLR4_types[]    = "RLSRL";
  if (LRSLR4_param.flag && !SetRSP(5, LRSLR4_lengths, LRSLR4_types, all_possible_paths)) {
...
bool ReedShepp::SetRSP(const int                   size,
...
bool ReedShepp::GenerateLocalConfigurations(const std::shared_ptr<Node3d> start_node,
                                            const std::shared_ptr<Node3d> end_node,
                                            ReedSheppPath*                shortest_path) {
  double step_scaled = planner_open_space_config_.warm_start_config().step_size() * max_kappa_;

  size_t point_num =
      static_cast<size_t>(std::floor(shortest_path->total_length / step_scaled +
  ...
  for (size_t i = 0; i < px.size(); ++i) {
    shortest_path->x.push_back(std::cos(-start_node->GetPhi()) * px.at(i) +
                               std::sin(-start_node->GetPhi()) * py.at(i) + start_node->GetX());
    shortest_path->y.push_back(-std::sin(-start_node->GetPhi()) * px.at(i) +
                               std::cos(-start_node->GetPhi()) * py.at(i) + start_node->GetY());
    shortest_path->phi.push_back(common::math::NormalizeAngle(pphi.at(i) + start_node->GetPhi()));
  }
...
void ReedShepp::Interpolation(const int             index,
                              const double          pd,
                              const char            m,
                              const double          ox,
                              const double          oy,
...
#pragma omp parallel for schedule(dynamic, 2) num_threads(8)
  for (int i = 0; i < RSP_nums; ++i) {
    ...
    if (i > 2 && i % 2) { x_param = -1.0; }
    if (i > 2 && i % 4 < 2) { y_param = -1.0; }
