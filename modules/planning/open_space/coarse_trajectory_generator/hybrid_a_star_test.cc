...
class HybridATest : public ::testing::Test {
 public:
  virtual void SetUp() {
    ...
    hybrid_test = std::unique_ptr<HybridAStar>(new HybridAStar(planner_open_space_config_));
  }
...
TEST_F(HybridATest, test1) {
  ...
  ASSERT_TRUE(hybrid_test->Plan(sx, sy, sphi, ex, ey, ephi, XYbounds_, obstacles_list, &result));
}
}  // namespace planning
}  // namespace apollo
