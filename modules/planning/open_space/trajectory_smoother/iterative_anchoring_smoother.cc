...
bool IterativeAnchoringSmoother::Smooth(
  ...
  if (!SmoothPath(interpolated_warm_start_path, bounds, &smoothed_path_points)) { return false; }
  ...
  ADEBUG << "iterative anchoring path smoother time: " << path_smooth_diff.count() * 1000.0
         << " ms.";
  ...
  ADEBUG << "iterative anchoring speed smoother time: " << speed_smooth_diff.count() * 1000.0
         << " ms.";
...
void IterativeAnchoringSmoother::AdjustStartEndHeading(
    const Eigen::MatrixXd& xWS, std::vector<std::pair<double, double>>* const point2d) {
  // Sanity check
  ...
  const double second_last_to_last_s = std::sqrt(second_last_to_last_dx * second_last_to_last_dx +
...
bool IterativeAnchoringSmoother::ReAnchoring(const std::vector<size_t>& colliding_point_index,
  ...
  CHECK_GT(path_points->size(),
           *(std::max_element(colliding_point_index.begin(), colliding_point_index.end())));
  ...
  const size_t reanchoring_trails_num = static_cast<size_t>(
      planner_open_space_config_.iterative_anchoring_smoother_config().reanchoring_trails_num());
  const double reanchoring_pos_stddev =
      planner_open_space_config_.iterative_anchoring_smoother_config().reanchoring_pos_stddev();
  const double reanchoring_length_stddev =
      planner_open_space_config_.iterative_anchoring_smoother_config().reanchoring_length_stddev();
  ...
  for (const auto index : colliding_point_index) {
    ...
    for (size_t i = 0; i < reanchoring_trails_num; ++i) {
      ...
      for (size_t j = index - 1; j < index + 2; ++j) {
        ...
        Box2d ego_box({path_points->at(j).x() + center_shift_distance_ * std::cos(heading),
      ...
      if (is_colliding) {
        ...
        if (index == 1) {
          ...
          const double delta_s =
              std::abs(path_points->at(index).s() - path_points->at(index - 1).s());
          ...
          const double delta_s =
              std::abs(path_points->at(index + 1).s() - path_points->at(index).s());
          double rand_dev         = common::math::Clamp(length_dis(gen), 0.8, -0.8);
          double adjusted_delta_s = delta_s * (1.0 + rand_dev);
          path_points->at(index).set_x(path_points->at(index + 1).x() +
                                       adjusted_delta_s * std::cos(adjust_theta));
          ...
          double rand_dev_x = common::math::Clamp(pos_dis(gen), 2.0 * reanchoring_pos_stddev,
                                                  -2.0 * reanchoring_pos_stddev);
          double rand_dev_y = common::math::Clamp(pos_dis(gen), 2.0 * reanchoring_pos_stddev,
                                                  -2.0 * reanchoring_pos_stddev);
...
bool IterativeAnchoringSmoother::GenerateInitialBounds(const DiscretizedPath& path_points,
  ...
  const bool estimate_bound =
      planner_open_space_config_.iterative_anchoring_smoother_config().estimate_bound();
  const double default_bound =
      planner_open_space_config_.iterative_anchoring_smoother_config().default_bound();
  const double vehicle_shortest_dimension =
      planner_open_space_config_.iterative_anchoring_smoother_config().vehicle_shortest_dimension();
...
bool IterativeAnchoringSmoother::SmoothPath(const DiscretizedPath&     raw_path_points,
...
bool IterativeAnchoringSmoother::CheckCollisionAvoidance(
    const DiscretizedPath& path_points, std::vector<size_t>* colliding_point_index) {
  ...
  for (size_t i = 0; i < path_points_size; ++i) {
    ...
    if (skip_checking) { continue; }

    const double heading =
        gear_ ? path_points[i].theta() : NormalizeAngle(path_points[i].theta() + M_PI);
    Box2d ego_box({path_points[i].x() + center_shift_distance_ * std::cos(heading),
  ...
  if (!colliding_point_index->empty()) { return false; }
  return true;
}

void IterativeAnchoringSmoother::AdjustPathBounds(const std::vector<size_t>& colliding_point_index,
                                                  std::vector<double>*       bounds) {
  CHECK_NOTNULL(bounds);

  const double collision_decrease_ratio =
      planner_open_space_config_.iterative_anchoring_smoother_config().collision_decrease_ratio();
  ...
  if (enforce_initial_kappa_) { bounds->at(2) = 0.0; }
}
...
bool IterativeAnchoringSmoother::SetPathProfile(
  ...
  if (!DiscretePointsMath::ComputePathProfile(point2d, &headings, &accumulated_s, &kappas,
                                              &dkappas)) {
    return false;
  }
...
bool IterativeAnchoringSmoother::SmoothSpeed(const double init_a,
  ...
  const double max_forward_v =
      planner_open_space_config_.iterative_anchoring_smoother_config().max_forward_v();
  const double max_reverse_v =
      planner_open_space_config_.iterative_anchoring_smoother_config().max_reverse_v();
  const double max_forward_acc =
      planner_open_space_config_.iterative_anchoring_smoother_config().max_forward_acc();
  const double max_reverse_acc =
      planner_open_space_config_.iterative_anchoring_smoother_config().max_reverse_acc();
  const double max_acc_jerk =
      planner_open_space_config_.iterative_anchoring_smoother_config().max_acc_jerk();
  const double delta_t = planner_open_space_config_.iterative_anchoring_smoother_config().delta_t();
  ...
  auto s_curve_config =
      planner_open_space_config_.iterative_anchoring_smoother_config().s_curve_config();
  ...
  piecewise_jerk_problem.set_x_ref(s_curve_config.ref_s_weight(), std::move(x_ref));
  ...
  for (size_t i = 1; i < num_of_knots; ++i) {
    ...
    smoothed_speeds->AppendSpeedPoint(s[i], delta_t * static_cast<double>(i), ds[i], dds[i],
...
bool IterativeAnchoringSmoother::CombinePathAndSpeed(
  ...
  for (double cur_rel_time = 0.0; cur_rel_time < time_horizon;
    ...
    if (speed_point.s() > path_points.Length()) { break; }
...
void IterativeAnchoringSmoother::AdjustPathAndSpeedByGear(
    DiscretizedTrajectory* discretized_trajectory) {
  if (gear_) { return; }
  std::for_each(discretized_trajectory->begin(), discretized_trajectory->end(),
...
bool IterativeAnchoringSmoother::GenerateStopProfileFromPolynomial(const double init_acc,
  ...
  for (double t = 2.0; t <= kMaxT; t += kUnitT) {
    ...
    if (!IsValidPolynomialProfile(curve)) { continue; }
...
bool IterativeAnchoringSmoother::IsValidPolynomialProfile(const QuinticPolynomialCurve1d& curve) {
  for (double evaluate_t = 0.1; evaluate_t <= curve.ParamLength(); evaluate_t += 0.2) {
    ...
    if (v < -kEpsilon || a > 1.0) { return false; }
  }
  return true;
}

double IterativeAnchoringSmoother::CalcHeadings(const DiscretizedPath& path_points,
                                                const size_t           index) {
