...
void DualVariableWarmStartIPOPTInterfaceTest::ProblemSetup() {
  ...
  ptop_.reset(new DualVariableWarmStartIPOPTInterface(horizon_, ts_, ego_, obstacles_edges_num_,
                                                      obstacles_num_, obstacles_A_, obstacles_b_,
                                                      xWS, planner_open_space_config_));
}

TEST_F(DualVariableWarmStartIPOPTInterfaceTest, initilization) { EXPECT_NE(ptop_, nullptr); }
...
TEST_F(DualVariableWarmStartIPOPTInterfaceTest, get_starting_point) {
  ...
  bool   res = ptop_->get_starting_point(kNumOfVariables, init_x, x, init_z, z_L, z_U,
