...
void DistanceApproachIPOPTCUDAInterfaceTest::ProblemSetup() {
  ...
  ptop_.reset(new DistanceApproachIPOPTCUDAInterface(
      horizon_, ts_, ego_, xWS_, uWS_, l_warm_up_, n_warm_up_, x0_, xf_, last_time_u_, XYbounds_,
      obstacles_edges_num_, obstacles_num_, obstacles_A_, obstacles_b_,
      planner_open_space_config_));
}

TEST_F(DistanceApproachIPOPTCUDAInterfaceTest, initilization) { EXPECT_NE(ptop_, nullptr); }
