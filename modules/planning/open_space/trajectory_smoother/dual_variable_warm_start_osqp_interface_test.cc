...
class DualVariableWarmStartOSQPInterfaceTest : public ::testing::Test {
 public:
  virtual void SetUp() {
    FLAGS_planner_open_space_config_filename = "/apollo/modules/planning/testdata/conf/"
...
void DualVariableWarmStartOSQPInterfaceTest::ProblemSetup() {
  ...
  ptop_.reset(new DualVariableWarmStartOSQPInterface(horizon_, ts_, ego_, obstacles_edges_num_,
                                                     obstacles_num_, obstacles_A_, obstacles_b_,
                                                     xWS, planner_open_space_config_));
  pt_gt_ = new DualVariableWarmStartIPOPTQPInterface(horizon_, ts_, ego_, obstacles_edges_num_,
                                                     obstacles_num_, obstacles_A_, obstacles_b_,
                                                     xWS, planner_open_space_config_);
}

TEST_F(DualVariableWarmStartOSQPInterfaceTest, initilization) { EXPECT_NE(ptop_, nullptr); }
...
TEST_F(DualVariableWarmStartOSQPInterfaceTest, optimize) {
  ...
  auto                                     ipopt_config_tmp =
      planner_open_space_config_.dual_variable_warm_start_config().ipopt_config();

  app->Options()->SetIntegerValue("print_level", ipopt_config_tmp.ipopt_print_level());
  app->Options()->SetIntegerValue("mumps_mem_percent", ipopt_config_tmp.mumps_mem_percent());
  app->Options()->SetNumericValue("mumps_pivtol", ipopt_config_tmp.mumps_pivtol());
  app->Options()->SetIntegerValue("max_iter", ipopt_config_tmp.ipopt_max_iter());
  ...
  app->Options()->SetStringValue("alpha_for_y", ipopt_config_tmp.ipopt_alpha_for_y());
  ...
  for (int r = 0; r < obstacles_edges_sum; ++r) {
    for (int c = 0; c < static_cast<int>(horizon_) + 1; ++c) {
      EXPECT_NEAR(l_warm_up(r, c), l_warm_up_gt(r, c), 1e-6) << "r: " << r << ", c: " << c;
    }
  }
  for (int r = 0; r < 4 * static_cast<int>(obstacles_num_); ++r) {
    for (int c = 0; c < static_cast<int>(horizon_) + 1; ++c) {
      EXPECT_NEAR(n_warm_up(r, c), n_warm_up_gt(r, c), 1e-6) << "r: " << r << ",c: " << c;
    }
  }
}
}  // namespace planning
