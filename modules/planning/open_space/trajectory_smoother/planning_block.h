...
#define DATA_TRANSFER_INST(type)                                                                   \
  TEMPLATE_ROUTINE_INSTANCE(bool, data_transfer(type* dst, const type* src, const int size))
...
__global__ void fill_lower_left_gpu(
    int* iRow, int* jCol, unsigned int* rind_L, unsigned int* cind_L, const int nnz_L);

template <typename T>
__global__ void data_transfer_gpu(T* dst, const T* src, const int size);

bool fill_lower_left(
    int* iRow, int* jCol, unsigned int* rind_L, unsigned int* cind_L, const int nnz_L);
template <typename T>
