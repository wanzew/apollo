...
DistanceApproachIPOPTRelaxEndSlackInterface::DistanceApproachIPOPTRelaxEndSlackInterface(
...
bool DistanceApproachIPOPTRelaxEndSlackInterface::get_nlp_info(
  ...
  num_of_variables_   = n1 + n2 + n3 + lambda_horizon_ + miu_horizon_ + slack_horizon_;
...
bool DistanceApproachIPOPTRelaxEndSlackInterface::get_starting_point(int     n,
                                                                     bool    init_x,
                                                                     double* x,
                                                                     bool    init_z,
                                                                     double* z_L,
...
bool DistanceApproachIPOPTRelaxEndSlackInterface::eval_f(int           n,
                                                         const double* x,
                                                         bool          new_x,
                                                         double&       obj_value) {
...
bool DistanceApproachIPOPTRelaxEndSlackInterface::eval_h(int           n,
                                                         const double* x,
                                                         bool          new_x,
                                                         double        obj_factor,
                                                         int           m,
...
//***************    start ADOL-C part ***********************************
template <class T>
void DistanceApproachIPOPTRelaxEndSlackInterface::eval_obj(int n, const T* x, T* obj_value) {
  // Objective is :
  // min control inputs
  // min input rate
  ...
  *obj_value += weight_stitching_steer_ * last_time_steer_rate * last_time_steer_rate +
  ...
  for (int i = 0; i < 4; ++i) {
    *obj_value += weight_end_state_ * (x[state_start_index_ + 4 * horizon_ + i] - xf_(i, 0)) *
...
template <class T>
void DistanceApproachIPOPTRelaxEndSlackInterface::eval_constraints(int n, const T* x, int m, T* g) {
  // state start index
  int state_index = state_start_index_;
...
void DistanceApproachIPOPTRelaxEndSlackInterface::generate_tapes(int  n,
                                                                 int  m,
                                                                 int* nnz_jac_g,
                                                                 int* nnz_h_lag) {
