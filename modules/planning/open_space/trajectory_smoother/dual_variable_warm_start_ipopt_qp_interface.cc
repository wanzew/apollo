...
bool DualVariableWarmStartIPOPTQPInterface::get_starting_point(int     n,
                                                               bool    init_x,
                                                               double* x,
                                                               bool    init_z,
                                                               double* z_L,
...
bool DualVariableWarmStartIPOPTQPInterface::get_bounds_info(
...
bool DualVariableWarmStartIPOPTQPInterface::eval_f(int           n,
                                                   const double* x,
                                                   bool          new_x,
                                                   double&       obj_value) {
...
bool DualVariableWarmStartIPOPTQPInterface::eval_grad_f(int           n,
                                                        const double* x,
                                                        bool          new_x,
                                                        double*       grad_f) {
...
bool DualVariableWarmStartIPOPTQPInterface::eval_g(
    int n, const double* x, bool new_x, int m, double* g) {
  eval_constraints(n, x, m, g);
  return true;
}

bool DualVariableWarmStartIPOPTQPInterface::eval_jac_g(
...
bool DualVariableWarmStartIPOPTQPInterface::eval_h(int           n,
                                                   const double* x,
                                                   bool          new_x,
                                                   double        obj_factor,
                                                   int           m,
...
void DualVariableWarmStartIPOPTQPInterface::check_solution(const Eigen::MatrixXd& l_warm_up,
  ...
  for (int i = 0; i < lambda_horizon_; ++i) {
    if (g_l[constraint_index] > g[constraint_index] ||
      ...
      AERROR << "lambda box constraint fails, "
             << "constraint_index: " << constraint_index << ", g: " << g[constraint_index];
    }
...
template <class T>
bool DualVariableWarmStartIPOPTQPInterface::eval_obj(int n, const T* x, T* obj_value) {
  ADEBUG << "eval_obj";
  *obj_value  = 0.0;
  int l_index = l_start_index_;
  for (int i = 0; i < horizon_ + 1; ++i) {
...
template <class T>
bool DualVariableWarmStartIPOPTQPInterface::eval_constraints(int n, const T* x, int m, T* g) {
  ...
  for (int i = 0; i < horizon_ + 1; ++i) {
    ...
    for (int j = 0; j < obstacles_num_; ++j) {
      ...
      g[constraint_index + 2] = -tmp3 + (xWS_(0, i) + cos(xWS_(2, i)) * offset_) * tmp1 +
...
void DualVariableWarmStartIPOPTQPInterface::generate_tapes(int n, int m, int* nnz_h_lag) {
