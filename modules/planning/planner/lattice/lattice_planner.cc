...
void ComputeInitFrenetState(const PathPoint&       matched_point,
  ...
  CartesianFrenetConverter::cartesian_to_frenet(
      ...
      matched_point.kappa(), matched_point.dkappa(), cartesian_state.path_point().x(),
      cartesian_state.path_point().y(), cartesian_state.v(), cartesian_state.a(),
      cartesian_state.path_point().theta(), cartesian_state.path_point().kappa(), ptr_s, ptr_d);
}

}  // namespace

Status LatticePlanner::Plan(const TrajectoryPoint& planning_start_point, Frame* frame) {
  ...
  for (auto& reference_line_info : frame->reference_line_info()) {
    ...
    auto status = PlanOnReferenceLine(planning_start_point, frame, &reference_line_info);

    if (status != Status::OK()) {
      if (reference_line_info.IsChangeLanePath()) {
        AERROR << "Planner failed to change lane to " << reference_line_info.Lanes().Id();
      } else {
  ...
  if (success_line_count > 0) { return Status::OK(); }
  return Status(ErrorCode::PLANNING_ERROR, "Failed to plan on any reference line.");
}
...
Status LatticePlanner::PlanOnReferenceLine(const TrajectoryPoint& planning_init_point,
  ...
  auto ptr_prediction_querier =
      std::make_shared<PredictionQuerier>(frame->obstacles(), ptr_reference_line);

  // 4. parse the decision and get the planning target.
  auto ptr_path_time_graph = std::make_shared<PathTimeGraph>(
      ptr_prediction_querier->GetObstacles(), *ptr_reference_line, reference_line_info, init_s[0],
      init_s[0] + FLAGS_decision_horizon, 0.0, FLAGS_trajectory_time_length, init_d);
  ...
  ADEBUG << "number of trajectory pairs = " << trajectory_evaluator.num_of_trajectory_pairs()
  ...
  CollisionChecker collision_checker(frame->obstacles(), init_s[0], init_d[0], *ptr_reference_line,
                                     reference_line_info, ptr_path_time_graph);
  ...
  while (trajectory_evaluator.has_more_trajectory_pairs()) {
    ...
    if (FLAGS_enable_auto_tuning) {
      trajectory_pair_cost_components = trajectory_evaluator.top_trajectory_pair_component_cost();
    ...
    if (result != ConstraintChecker::Result::VALID) {
      ...
      switch (result) {
        ...
        case ConstraintChecker::Result::LON_ACCELERATION_OUT_OF_BOUND:
          ...
          break;
        case ConstraintChecker::Result::LON_JERK_OUT_OF_BOUND: lon_jerk_failure_count += 1; break;
        case ConstraintChecker::Result::CURVATURE_OUT_OF_BOUND: curvature_failure_count += 1; break;
        case ConstraintChecker::Result::LAT_ACCELERATION_OUT_OF_BOUND:
          lat_acc_failure_count += 1;
          break;
        case ConstraintChecker::Result::LAT_JERK_OUT_OF_BOUND: lat_jerk_failure_count += 1; break;
    ...
    if (AdapterManager::GetLocalization() == nullptr) {
      ...
      if (!MapFutureTrajectoryToSL(future_trajectory, *ptr_reference_line, &lon_future_trajectory,
      ...
      trajectory_evaluator.EvaluateDiscreteTrajectory(planning_target, lon_future_trajectory,
                                                      lat_future_trajectory,
      ...
      for (double student_cost_component : trajectory_pair_cost_components) {
        auto_tuning_data.mutable_student_component()->add_cost_component(student_cost_component);
      }

      for (double teacher_cost_component : future_traj_component_cost) {
        auto_tuning_data.mutable_teacher_component()->add_cost_component(teacher_cost_component);
      }

      ptr_debug->mutable_auto_tuning_training_data()->CopyFrom(auto_tuning_data);
    }
    ...
    ADEBUG << "Starting Lon. State: s = " << init_s[0] << " ds = " << init_s[1]
           << " dds = " << init_s[2];
    // cast
    auto lattice_traj_ptr = std::dynamic_pointer_cast<LatticeTrajectory1d>(trajectory_pair.first);
    if (!lattice_traj_ptr) { ADEBUG << "Dynamically casting trajectory1d ptr. failed."; }
  ...
  ADEBUG << "Trajectory_Evaluation_Time = " << (Clock::NowInSeconds() - current_time) * 1000;
  ...
  ADEBUG << "Total_Lattice_Planning_Frame_Time = " << (Clock::NowInSeconds() - start_time) * 1000;
  ...
  if (num_lattice_traj > 0) {
    ...
    if (FLAGS_enable_backup_trajectory) {
      ...
      BackupTrajectoryGenerator backup_trajectory_generator(
          ...
          std::make_shared<CollisionChecker>(collision_checker), &trajectory1d_generator);
...
bool LatticePlanner::MapFutureTrajectoryToSL(
  ...
  for (const common::TrajectoryPoint& trajectory_point : future_trajectory.trajectory_points()) {
    const PathPoint& path_point = trajectory_point.path_point();
    PathPoint        matched_point =
        PathMatcher::MatchToPath(discretized_reference_line, path_point.x(), path_point.y());
