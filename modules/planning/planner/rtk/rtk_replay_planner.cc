...
RTKReplayPlanner::RTKReplayPlanner(const std::shared_ptr<DependencyInjector>& injector)
...
Status RTKReplayPlanner::PlanOnReferenceLine(const TrajectoryPoint& planning_init_point,
  ...
  if (complete_rtk_trajectory_.empty() || complete_rtk_trajectory_.size() < 2) {
    const std::string msg = "RTKReplayPlanner doesn't have a recorded trajectory or "
  ...
  std::uint32_t forward_buffer = static_cast<std::uint32_t>(FLAGS_rtk_trajectory_forward);
  // end_index is excluded.
  std::uint32_t end_index = std::min<std::uint32_t>(
      static_cast<std::uint32_t>(complete_rtk_trajectory_.size()), matched_index + forward_buffer);
  ...
  while (trajectory_points.size() < static_cast<size_t>(FLAGS_rtk_trajectory_forward)) {
