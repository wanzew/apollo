...
void NaviPlanner::RegisterTasks() {
  task_factory_.Register(NAVI_PATH_DECIDER, []() -> Task* { return new NaviPathDecider(); });
  task_factory_.Register(NAVI_SPEED_DECIDER, []() -> Task* { return new NaviSpeedDecider(); });
...
Status NaviPlanner::Plan(const TrajectoryPoint& planning_init_point, Frame* frame) {
  // NaviPlanner is only used in navigation mode based on the real-time relative
  // map.
  if (!FLAGS_use_navigation_mode) {
    std::string msg = "NaviPlanner is only used in navigation mode.";
...
void NaviPlanner::GenerateFallbackSpeedProfile(const ReferenceLineInfo* reference_line_info,
