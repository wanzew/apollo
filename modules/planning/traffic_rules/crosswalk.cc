...
void Crosswalk::MakeDecisions(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
  ...
  for (auto crosswalk_overlap : crosswalk_overlaps_) {
    ...
    if (adc_front_edge_s - crosswalk_overlap->end_s > config_.crosswalk().min_pass_s_distance()) {
      ...
      ADEBUG << "SKIP: crosswalk_id[" << crosswalk_id << "] crosswalk_overlap_end_s["
             << crosswalk_overlap->end_s << "] adc_front_edge_s[" << adc_front_edge_s
    ...
    for (const auto* obstacle : path_decision->obstacles().Items()) {
      ...
      bool stop =
          CheckStopForObstacle(reference_line_info, crosswalk_ptr, *obstacle, stop_deceleration);
      ...
      if (stop && !is_on_lane &&
          crosswalk_overlap->start_s - adc_front_edge_s <= kStartWatchTimerDistance) {
        ...
        if (obstacle_speed <= kMaxStopSpeed) {
          if (crosswalk_stop_timer[crosswalk_id].count(obstacle_id) < 1) {
            ...
            double stop_time =
                Clock::NowInSeconds() - crosswalk_stop_timer[crosswalk_id][obstacle_id];
  ...
  for (auto crosswalk_to_stop : crosswalks_to_stop) {
    ...
    util::BuildStopDecision(
        virtual_obstacle_id, crosswalk_overlap->start_s, config_.crosswalk().stop_distance(),
  ...
  if (firsts_crosswalk_to_stop) {
    ...
    for (const auto& timer : crosswalk_stop_timer[crosswalk]) {
      ...
      ADEBUG << "UPDATE stop_time: id[" << crosswalk << "] obstacle_id[" << timer.first
             << "] stop_timestamp[" << timer.second << "]";
    }
...
bool Crosswalk::CheckStopForObstacle(ReferenceLineInfo* const    reference_line_info,
  ...
  ADEBUG << "obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name << "] crosswalk_id["
         << crosswalk_id << "] obstacle_l[" << obstacle_sl_point.l()
         << "] within_expanded_crosswalk_area[" << in_expanded_crosswalk << "] obstacle_l_distance["
         << obstacle_l_distance << "] on_lane[" << is_on_lane << "] is_on_road[" << is_on_road
  ...
  if (obstacle_l_distance >= config_.crosswalk().stop_loose_l_distance()) {
  ...
  } else if (obstacle_l_distance <= config_.crosswalk().stop_strict_l_distance()) {
    if (is_on_road) {
      ...
      if (obstacle_sl_point.s() > adc_end_edge_s) {
        ...
        ADEBUG << "need_stop(<=l1): obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name
               << "] s[" << obstacle_sl_point.s() << "] adc_end_edge_s[ " << adc_end_edge_s
      ...
      if (is_path_cross) {
        ...
        const auto adc_path_point = Vec2d(injector_->ego_info()->start_point().path_point().x(),
    ...
    if (is_path_cross) { stop = true; }
    ADEBUG << "need_stop(between l1 & l2): obstacle_id[" << obstacle_id << "] type["
           << obstacle_type_name << "] obstacle_l_distance[" << obstacle_l_distance
