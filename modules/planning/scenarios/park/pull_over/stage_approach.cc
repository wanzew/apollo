...
PullOverStageApproach::PullOverStageApproach(const ScenarioConfig::StageConfig&         config,
                                             const std::shared_ptr<DependencyInjector>& injector)
    : Stage(config, injector) {}

Stage::StageStatus PullOverStageApproach::Process(const TrajectoryPoint& planning_init_point,
                                                  Frame*                 frame) {
  ...
  if (!plan_ok) { AERROR << "PullOverStageApproach planning error"; }

  const auto&                    reference_line_info = frame->reference_line_info().front();
  scenario::util::PullOverStatus status =
      scenario::util::CheckADCPullOver(injector_->vehicle_state(), reference_line_info,
                                       scenario_config_, injector_->planning_context());

  if (status == scenario::util::PASS_DESTINATION || status == scenario::util::PARK_COMPLETE) {
  ...
  if (!candidate_path_data.empty()) {
    for (const auto& path_data : candidate_path_data) {
      if (path_data.path_label().find("pullover") == std::string::npos) { break; }

      for (size_t i = path_data.discretized_path().size(); i >= 1; --i) {
        if (path_data.frenet_frame_path().back().s() - path_data.frenet_frame_path()[i - 1].s() <
        ...
        scenario::util::PullOverStatus status     = scenario::util::CheckADCPullOverPathPoint(
            reference_line_info, scenario_config_, path_point, injector_->planning_context());
        if (status == scenario::util::PARK_FAIL) { path_fail = true; }
        break;
      }
    }
  }
  ...
  if (path_fail) {
    ...
    if (pull_over_status.has_position() && pull_over_status.position().has_x() &&
      ...
      const double stop_line_s =
          pull_over_sl.s() - scenario_config_.s_distance_to_stop_for_open_space_parking();
      const std::string              virtual_obstacle_id = "DEST_PULL_OVER_PREPARKING";
      const std::vector<std::string> wait_for_obstacle_ids;
      planning::util::BuildStopDecision(virtual_obstacle_id, stop_line_s, 1.0,
                                        StopReasonCode::STOP_REASON_PREPARKING,
                                        wait_for_obstacle_ids, "PULL-OVER-scenario", frame,
                                        &(frame->mutable_reference_line_info()->front()));

      ADEBUG << "Build a stop fence to pause ADC at a better position: id[" << virtual_obstacle_id
             << "] s[" << stop_line_s << "]";
      ...
      double angle = common::math::AngleDiff(pull_over_status.theta(), ref_point.heading());
      if (distance <= kPreparkingStopDistance && angle <= kPreparkingAngleDiff) {
        return FinishStage(false);
      }
    }
  }
