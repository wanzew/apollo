...
PullOverStatus CheckADCPullOver(const common::VehicleStateProvider* vehicle_state_provider,
  ...
  const auto& pull_over_status = planning_context->planning_status().pull_over();
  ...
  const bool position_check = CheckPullOverPositionBySL(
      reference_line_info, scenario_config, adc_position, vehicle_state_provider->heading(),
      target_position, pull_over_status.theta(), true);
...
PullOverStatus CheckADCPullOverPathPoint(const ReferenceLineInfo&      reference_line_info,
  ...
  const common::math::Vec2d target_position = {pull_over_status.position().x(),
                                               pull_over_status.position().y()};
  const bool                position_check  = CheckPullOverPositionBySL(
      reference_line_info, scenario_config, {path_point.x(), path_point.y()}, path_point.theta(),
...
bool CheckPullOverPositionBySL(const ReferenceLineInfo&      reference_line_info,
  ...
  const double theta_diff = std::fabs(common::math::NormalizeAngle(target_theta - adc_theta));

  ADEBUG << "adc_position_s[" << adc_position_sl.s() << "] adc_position_l[" << adc_position_sl.l()
         << "] target_s[" << target_sl.s() << "] target_l[" << target_sl.l() << "] s_diff["
         << s_diff << "] l_diff[" << l_diff << "] theta_diff[" << theta_diff << "]";
  ...
  if (check_s) {
    ret = (ret && s_diff >= 0 && s_diff <= scenario_config.max_s_error_to_end_point());
  }
...
bool CheckADCReadyToCruise(const common::VehicleStateProvider* vehicle_state_provider,
  ...
  bool is_near_front_obstacle         = CheckADCSurroundObstacles(adc_position, adc_heading, frame,
                                                          scenario_config.front_obstacle_buffer());
  bool heading_align_w_reference_line = CheckADCHeading(
      adc_position, adc_heading, *reference_line_info, scenario_config.heading_buffer());
  ADEBUG << "is_near_front_obstacle: " << is_near_front_obstacle;
  ADEBUG << "heading_align_w_reference_line: " << heading_align_w_reference_line;
  // check gear status
  ...
  return ((vehicle_status->gear() == canbus::Chassis::GEAR_DRIVE ||
           std::fabs(vehicle_status->vehicle_state().linear_velocity()) < kMinSpeed) &&
...
bool CheckADCSurroundObstacles(const common::math::Vec2d adc_position,
  ...
  // ADC box
  Box2d  adc_box(adc_position, adc_heading, adc_length + front_obstacle_buffer, adc_width);
  double shift_distance =
      front_obstacle_buffer / 2 + vehicle_config.vehicle_param().back_edge_to_center();
  Vec2d shift_vec{shift_distance * std::cos(adc_heading), shift_distance * std::sin(adc_heading)};
  ...
  for (const auto& obstacle : obstacles) {
    ...
    const Polygon2d& nudge_polygon =
        obstacle_polygon.ExpandByDistance(std::fabs(FLAGS_static_obstacle_nudge_l_buffer));
...
bool CheckADCHeading(const common::math::Vec2d adc_position,
  ...
  const auto reference_point = reference_line.GetReferencePoint(adc_position_sl.s());
  const auto path_point      = reference_point.ToPathPoint(adc_position_sl.s());
  AINFO << "heading difference: " << common::math::NormalizeAngle(adc_heading - path_point.theta());
  double angle_difference = common::math::NormalizeAngle(adc_heading - path_point.theta());
  if (angle_difference > -1.0 * (heading_diff_to_reference_line - kReducedHeadingBuffer) &&
