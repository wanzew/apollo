...
void StopSignUnprotectedScenario::Init() {
  ...
  hdmap::StopSignInfoConstPtr stop_sign =
      HDMapUtil::BaseMap().GetStopSignById(hdmap::MakeMapId(stop_sign_overlap_id));
  if (!stop_sign) {
...
void StopSignUnprotectedScenario::RegisterStages() {
  ...
  s_stage_factory_.Register(ScenarioConfig::STOP_SIGN_UNPROTECTED_PRE_STOP,
                            [](const ScenarioConfig::StageConfig&         config,
                               const std::shared_ptr<DependencyInjector>& injector) -> Stage* {
                              return new StopSignUnprotectedStagePreStop(config, injector);
                            });
  s_stage_factory_.Register(ScenarioConfig::STOP_SIGN_UNPROTECTED_STOP,
                            [](const ScenarioConfig::StageConfig&         config,
                               const std::shared_ptr<DependencyInjector>& injector) -> Stage* {
  ...
  s_stage_factory_.Register(ScenarioConfig::STOP_SIGN_UNPROTECTED_INTERSECTION_CRUISE,
                            [](const ScenarioConfig::StageConfig&         config,
                               const std::shared_ptr<DependencyInjector>& injector) -> Stage* {
...
StopSignUnprotectedScenario::CreateStage(const ScenarioConfig::StageConfig&         stage_config,
                                         const std::shared_ptr<DependencyInjector>& injector) {
 ...
 */
int StopSignUnprotectedScenario::GetAssociatedLanes(const StopSignInfo& stop_sign_info) {
    ...
    for (const auto& lane_id : associated_lane_ids) {
      ...
      for (const auto& stop_sign_overlap : lane->stop_signs()) {
        ...
        if (over_lap_info != nullptr) {
          context_.associated_lanes.push_back(std::make_pair(lane, stop_sign_overlap));
