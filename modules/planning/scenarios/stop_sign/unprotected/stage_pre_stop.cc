...
Stage::StageStatus
StopSignUnprotectedStagePreStop::Process(const TrajectoryPoint& planning_init_point, Frame* frame) {
  ...
  if (!plan_ok) { AERROR << "StopSignUnprotectedStagePreStop planning error"; }
  ...
  const double distance_adc_pass_stop_sign = adc_front_edge_s - current_stop_sign_overlap->start_s;
  ...
  for (const auto& vehicle : watch_vehicles) {
    std::copy(vehicle.second.begin(), vehicle.second.end(), std::back_inserter(watch_vehicle_ids));
 ...
 */
int StopSignUnprotectedStagePreStop::AddWatchVehicle(const Obstacle&       obstacle,
                                                     StopSignLaneVehicles* watch_vehicles) {
  CHECK_NOTNULL(watch_vehicles);

  const PerceptionObstacle& perception_obstacle    = obstacle.Perception();
  const std::string&        perception_obstacle_id = std::to_string(perception_obstacle.id());
  ...
  if (obstacle_type != PerceptionObstacle::UNKNOWN &&
    ...
    ADEBUG << "obstacle_id[" << perception_obstacle_id << "] type[" << obstacle_type_name
           << "]. skip";
    return 0;
  }

  const auto       point = common::util::PointFactory::ToPointENU(perception_obstacle.position());
  ...
  if (assoc_lane_it == GetContext()->associated_lanes.end()) {
    ADEBUG << "obstacle_id[" << perception_obstacle_id << "] type[" << obstacle_type_name
           << "] lane_id[" << obstable_lane_id << "] not associated with current stop_sign. skip";
    return -1;
  }
  ...
  if (distance_to_stop_line > scenario_config_.watch_vehicle_max_valid_stop_distance()) {
    ADEBUG << "obstacle_id[" << perception_obstacle_id << "] type[" << obstacle_type_name
  ...
  if (std::find(vehicles.begin(), vehicles.end(), perception_obstacle_id) == vehicles.end()) {
    ADEBUG << "AddWatchVehicle: lane[" << obstacle_lane->id().id() << "] obstacle_id["
           << perception_obstacle_id << "]";
    (*watch_vehicles)[obstacle_lane->id().id()].push_back(perception_obstacle_id);
  }
 ...
 */
bool StopSignUnprotectedStagePreStop::CheckADCStop(const double adc_front_edge_s,
  ...
  const double distance_stop_line_to_adc_front_edge = stop_line_s - adc_front_edge_s;
