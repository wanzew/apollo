...
Stage::StageStatus
EmergencyPullOverStageSlowDown::Process(const TrajectoryPoint& planning_init_point, Frame* frame) {
  ...
  if (!plan_ok) { AERROR << "EmergencyPullOverStageSlowDown planning error"; }

  // check slow enough
  static constexpr double kSpeedTolarence = 1.0;
  if (adc_speed - target_slow_down_speed <= kSpeedTolarence) { return FinishStage(); }
