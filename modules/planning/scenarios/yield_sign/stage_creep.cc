...
Stage::StageStatus YieldSignStageCreep::Process(const TrajectoryPoint& planning_init_point,
                                                Frame*                 frame) {
  ADEBUG << "stage: Creep";
  CHECK_NOTNULL(frame);
  ...
  if (!plan_ok) { AERROR << "YieldSignStageCreep planning error"; }

  if (GetContext()->current_yield_sign_overlap_ids.empty()) { return FinishScenario(); }

  const auto&       reference_line_info   = frame->reference_line_info().front();
  const std::string yield_sign_overlap_id = GetContext()->current_yield_sign_overlap_ids[0];
  ...
  if (!current_yield_sign_overlap) { return FinishScenario(); }
  ...
  double creep_stop_s   = yield_sign_end_s + task->FindCreepDistance(*frame, reference_line_info);
  ...
  if (task->CheckCreepDone(*frame, reference_line_info, yield_sign_end_s, wait_time, timeout_sec)) {
    return FinishStage();
  }
...
Stage::StageStatus YieldSignStageCreep::FinishStage() { return FinishScenario(); }
