...
class Scenario {
 public:
  ...
  ScenarioConfig::ScenarioType scenario_type() const { return config_.scenario_type(); }
   ...
   */
  virtual std::unique_ptr<Stage>
  CreateStage(const ScenarioConfig::StageConfig&         stage_config,
  ...
  virtual bool IsTransferable(const Scenario& other_scenario, const Frame& frame) { return true; }
  ...
  const ScenarioConfig::StageType GetStage() const {
    return current_stage_ ? current_stage_->stage_type() : ScenarioConfig::NO_STAGE;
  }
 ...
 protected:
  ...
  std::unordered_map<ScenarioConfig::StageType, const ScenarioConfig::StageConfig*, std::hash<int>>
                                      stage_config_map_;
