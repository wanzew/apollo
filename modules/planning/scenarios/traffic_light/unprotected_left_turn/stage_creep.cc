...
Stage::StageStatus
TrafficLightUnprotectedLeftTurnStageCreep::Process(const TrajectoryPoint& planning_init_point,
                                                   Frame*                 frame) {
  ADEBUG << "stage: Creep";
  CHECK_NOTNULL(frame);
  ...
  if (!plan_ok) { AERROR << "TrafficLightUnprotectedLeftTurnStageCreep planning error"; }
