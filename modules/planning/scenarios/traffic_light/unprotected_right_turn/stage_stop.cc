...
Stage::StageStatus
TrafficLightUnprotectedRightTurnStageStop::Process(const TrajectoryPoint& planning_init_point,
                                                   Frame*                 frame) {
  ADEBUG << "stage: Stop";
  CHECK_NOTNULL(frame);
  ...
  if (!plan_ok) { AERROR << "TrafficLightRightTurnUnprotectedStop planning error"; }
  ...
  if (traffic_light_all_stop && traffic_light_all_green) { return FinishStage(true); }
  ...
  if (!traffic_light_no_right_turn_on_red) {
    if (traffic_light_all_stop && !traffic_light_all_green) {
      ...
      if (distance_adc_pass_stop_line > scenario_config_.min_pass_s_distance()) {
      ...
      if (scenario_config_.enable_right_turn_on_red()) {
        ...
        if (GetContext()->stop_start_time == 0.0) {
          ...
          if (wait_time > scenario_config_.red_light_right_turn_stop_duration_sec()) {
            return FinishStage(false);
          }
        }
      }
...
bool TrafficLightUnprotectedRightTurnStageStop::CheckTrafficLightNoRightTurnOnRed(
  ...
  if (!traffic_light_ptr) { return false; }

  const auto& signal = traffic_light_ptr->signal();
  for (int i = 0; i < signal.sign_info_size(); i++) {
    if (signal.sign_info(i).type() == hdmap::SignInfo::NO_RIGHT_TURN_ON_RED) { return true; }
  }
...
Stage::StageStatus
TrafficLightUnprotectedRightTurnStageStop::FinishStage(const bool protected_mode) {
  if (protected_mode) {
    // intersection_cruise
