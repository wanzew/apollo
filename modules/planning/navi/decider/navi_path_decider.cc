...
NaviPathDecider::NaviPathDecider()
    : Task("NaviPathDecider") {
...
bool NaviPathDecider::Init(const PlanningConfig& config) {
  ...
  for (const auto& item : later_shift_config_talbe.lateral_shift()) {
    ...
    if (speed_to_shift_param_table_.find(max_speed_level) == speed_to_shift_param_table_.end()) {
      speed_to_shift_param_table_.emplace(
          std::make_pair(max_speed_level, std::make_tuple(item.min_distance(), item.max_distance(),
  ...
  for (const auto& data : speed_to_shift_param_table_) {
    ...
    AINFO << "[max_sped : " << max_speed << " ,min_distance : " << std::get<0>(shift_param)
...
Status NaviPathDecider::Execute(Frame* frame, ReferenceLineInfo* const reference_line_info) {
  Task::Execute(frame, reference_line_info);
  vehicle_state_              = frame->vehicle_state();
  cur_reference_line_lane_id_ = reference_line_info->Lanes().Id();
  auto ret = Process(reference_line_info->reference_line(), frame->PlanningStartPoint(),
                     frame->obstacles(), reference_line_info->path_decision(),
...
apollo::common::Status NaviPathDecider::Process(const ReferenceLine&                reference_line,
  ...
  size_t path_len =
      static_cast<size_t>(std::ceil(init_point.v() * config_.min_look_forward_time()));
  ...
  if (!GetBasicPathData(reference_line, &path_points) || path_points.size() < path_len) {
    AERROR << "Get path points from reference line failed.";
    return Status(apollo::common::ErrorCode::PLANNING_ERROR, "NaviPathDecider GetLocalPath");
  }
  ...
  if (reference_line_info_->IsChangeLanePath() && reference_line_info_->IsNeighborLanePath()) {
    ...
    bool bRet = reference_line.GetLaneWidth(stop_point_s, &lane_left_width, &lane_right_width);
    if (bRet) {
      target_start_path_point_y = init_basic_path_y - lane_right_width + KSideBuffer;
    } else {
      AERROR << "Pull over failed, because cannot get the right lane width";
      return Status(apollo::common::ErrorCode::PLANNING_ERROR, "NaviPathDecider can not pull over");
    }
  ...
  double start_point_y = SmoothInitY(init_basic_path_y, target_start_path_point_y);
  ...
  ADEBUG << "in current plan, adc latteral to ref line shift distance : " << start_point_y
  ...
  double           path_l_cost =
      std::fabs(init_basic_path_y) / KDefaultDoubleLaneWidth * config_.path_l_cost();
  ...
  if (!path_data->SetDiscretizedPath(discretized_path)) {
    ...
    return Status(apollo::common::ErrorCode::PLANNING_ERROR, "NaviPathDecider SetDiscretizedPath");
  }
  last_lane_id_to_adc_project_y_[cur_reference_line_lane_id_] = init_basic_path_y;
...
void NaviPathDecider::RecordDebugInfo(const PathData& path_data) {
  ...
  auto*       ptr_optimized_path =
      reference_line_info_->mutable_debug()->mutable_planning_data()->add_path();
  ptr_optimized_path->set_name(Name());
  ptr_optimized_path->mutable_path_point()->CopyFrom({path_points.begin(), path_points.end()});
}

bool NaviPathDecider::GetBasicPathData(const ReferenceLine&                  reference_line,
  ...
  if (reference_line.reference_points().size() < kMinRefPointNum) {
    AERROR << "Reference line points are not enough to generate path trajectory.";
    return false;
  }
  ...
  for (double s = start_plan_point_project_s; s < reference_line_len; s += unit_s) {
...
void NaviPathDecider::ShiftY(const double                          shift_distance,
                             std::vector<common::PathPoint>* const path_points) {
  CHECK_NOTNULL(path_points);
...
double NaviPathDecider::SmoothInitY(const double actual_ref_init_y,
  ...
  // get shift diretion
  double shift_direction = target_path_init_y < 0.0 ? -kPositiveSign : kPositiveSign;
  ...
  if (plan_point_to_target_distance > min_init_y_) {
    shift_distance =
        (plan_point_to_target_distance < max_init_y_) ? plan_point_to_target_distance : max_init_y_;

    // accurate to the centimeter scale
    start_position_y = shift_direction * std::floor(shift_distance * 100.0) * 0.01;
  }
...
bool NaviPathDecider::IsSafeChangeLane(const ReferenceLine& reference_line,
  ...
  const auto&       adc_param        = common::VehicleConfigHelper::GetConfig().vehicle_param();
  common::PathPoint start_plan_point = frame_->PlanningStartPoint().path_point();
  Vec2d             adc_position(start_plan_point.x(), start_plan_point.y());
  Vec2d vec_to_center((adc_param.front_edge_to_center() - adc_param.back_edge_to_center()) / 2.0,
                      (adc_param.left_edge_to_center() - adc_param.right_edge_to_center()) / 2.0);
  Vec2d adc_center(adc_position + vec_to_center.rotate(start_plan_point.theta()));
  Box2d adc_box(adc_center, start_plan_point.theta(), adc_param.length(), adc_param.width());
  ...
  for (const auto* path_obstacle : path_decision.path_obstacles().Items()) {
    ...
    if (sl_boundary.start_l() < -kLateralShift || sl_boundary.end_l() > kLateralShift) { continue; }
    ...
    const double kForwardSafeDistance = std::max(
        ...
        ((vehicle_state_.linear_velocity() - path_obstacle->obstacle()->Speed()) * kSafeTime));
    const double kBackwardSafeDistance = std::max(
        kBackwardMinSafeDistance,
        ((path_obstacle->obstacle()->Speed() - vehicle_state_.linear_velocity()) * kSafeTime));
    if (sl_boundary.end_s() > adc_sl_boundary.start_s() - kBackwardSafeDistance &&
        sl_boundary.start_s() < adc_sl_boundary.end_s() + kForwardSafeDistance) {
      return false;
    }
  }
...
double NaviPathDecider::NudgeProcess(const ReferenceLine&                  reference_line,
  ...
  double              nudge_distance     = obstacle_decider.GetNudgeDistance(
      obstacles, reference_line, path_decision, path_data_points, &lane_obstacles_num);
  ...
  if (std::fabs(nudge_distance) > KNudgeEpsilon) {
    ...
    if (last_lane_id_to_nudge_flag_.find(cur_reference_line_lane_id_) !=
      ...
      last_plan_has_nudge = last_lane_id_to_nudge_flag_[cur_reference_line_lane_id_];
    }
...
void NaviPathDecider::CalculateShiftParam() {
  ...
  auto   max_speed_level_itr =
      std::upper_bound(max_speed_levels_.begin(), max_speed_levels_.end(), max_adc_speed);
