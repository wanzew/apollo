...
bool NaviPathDecider::Init(const PlanningConfig& config) {
  ...
  for (const auto& item : move_dest_lane_config_talbe.lateral_shift()) {
    ...
    if (move_dest_lane_config_talbe_.find(max_speed_level) == move_dest_lane_config_talbe_.end()) {
...
apollo::common::Status NaviPathDecider::Process(const ReferenceLine&                reference_line,
  ...
  if (!GetBasicPathData(reference_line, &path_points)) {
    ...
    return Status(apollo::common::ErrorCode::PLANNING_ERROR, "NaviPathDecider GetBasicPathData");
  }
  ...
  ADEBUG << "in current plan cycle, adc to ref line distance : " << dest_ref_line_y
         << "lane id : " << cur_reference_line_lane_id_;
...
void NaviPathDecider::MoveToDestLane(const double                          dest_ref_line_y,
                                     std::vector<common::PathPoint>* const path_points) {
  double dest_lateral_distance = std::fabs(dest_ref_line_y);
  if (dest_lateral_distance < max_keep_lane_distance_) { return; }
  ...
  lateral_shift_value = lateral_shift_value > 0.0 ?
  ...
  std::transform(path_points->begin(), path_points->end(), path_points->begin(),
                 [lateral_shift_value](common::PathPoint& old_path_point) {
                   ...
                   double            new_path_point_y = old_path_point.y() + lateral_shift_value;
                   new_path_point.set_y(new_path_point_y);
                   return new_path_point;
                 });
}

void NaviPathDecider::KeepLane(const double                          dest_ref_line_y,
                               std::vector<common::PathPoint>* const path_points) {
  double dest_lateral_distance = std::fabs(dest_ref_line_y);
  if (dest_lateral_distance <= max_keep_lane_distance_) {
    ...
    double actual_dest_point_y =
        NudgeProcess(reference_line, *path_points, obstacles, *path_decision, vehicle_state_);
    ...
    if (actual_dest_lateral_distance > min_keep_lane_offset_) {
      ...
      lateral_shift_value =
          (actual_dest_lateral_distance <
           max_keep_lane_shift_y_ + min_keep_lane_offset_ - keep_lane_shift_compensation_) ?
    ...
    ADEBUG << "in current plan cycle keep lane, actual dest : " << actual_dest_point_y
...
bool NaviPathDecider::GetBasicPathData(const ReferenceLine&                  reference_line,
  ...
  if (reference_line_len < path_len) {
    AERROR << "Reference line is too short to generate path trajectory( s = " << reference_line_len
           << ").";
    return false;
  }
  ...
  auto start_plan_point_project =
      reference_line.GetReferencePoint(start_plan_point_.x(), start_plan_point_.y());
  common::SLPoint sl_point;
  if (!reference_line.XYToSL(start_plan_point_project.ToPathPoint(0.0), &sl_point)) {
...
bool NaviPathDecider::IsSafeChangeLane(const ReferenceLine& reference_line,
  ...
  Vec2d adc_center(adc_position + vec_to_center.rotate(start_plan_point_.theta()));
  Box2d adc_box(adc_center, start_plan_point_.theta(), adc_param.length(), adc_param.width());
...
double NaviPathDecider::NudgeProcess(const ReferenceLine&                  reference_line,
  ...
  double                  nudge_distance =
      obstacle_decider_.GetNudgeDistance(obstacles, reference_line, path_decision, path_data_points,
...
double NaviPathDecider::CalculateDistanceToDestLane() {
  ...
  double max_adc_speed = start_plan_v_ + start_plan_a_ * 1.0 / FLAGS_planning_loop_rate;
