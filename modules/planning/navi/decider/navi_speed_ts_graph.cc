...
NaviSpeedTsGraph::NaviSpeedTsGraph() { Reset(kDefaultSStep, kDefaultSMax, 0.0, 0.0, 0.0); }

void NaviSpeedTsGraph::Reset(
    double s_step, double s_max, double start_v, double start_a, double start_da) {
  CHECK_GT(s_step, 0.0);
...
void NaviSpeedTsGraph::UpdateRangeConstraints(double                        start_s,
                                              double                        end_s,
...
void NaviSpeedTsGraph::UpdateObstacleConstraints(double distance,
  ...
  if (distance > safe_distance && distance - safe_distance < kSafeDistanceSmooth &&
  ...
  auto vm = std::max(cruise_speed * r0, 0.0) + (1.0 + following_accel_ratio * r0) * v1;
...
Status NaviSpeedTsGraph::Solve(std::vector<NaviSpeedTsPoint>* output) {
  ...
  for (ssize_t i = constraints_.size() - 2; i >= 0; i--) {
    ...
    cur.v_max = std::min(std::sqrt(next.v_max * next.v_max + 2 * next.b_max * s_step_), cur.v_max);
  ...
  for (size_t i = 1; i < constraints_.size(); i++) {
    ...
    auto&       cur  = constraints_[i];
    cur.v_max = std::min(std::sqrt(prev.v_max * prev.v_max + 2 * cur.a_max * s_step_), cur.v_max);
    cur.v_preffered = std::min(cur.v_max, cur.v_preffered);
  }
  ...
  for (ssize_t i = constraints_.size() - 2; i >= 0; i--) {
    ...
    auto&       cur  = constraints_[i];
    cur.v_preffered =
        std::min(std::sqrt(next.v_preffered * next.v_preffered + 2 * next.b_preffered * s_step_),
                 cur.v_preffered);
  }
  ...
  for (size_t i = 1; i < constraints_.size(); i++) {
    ...
    auto&       cur  = constraints_[i];
    cur.v_preffered =
        std::min(std::sqrt(prev.v_preffered * prev.v_preffered + 2 * cur.a_preffered * s_step_),
                 cur.v_preffered);
  }
  ...
  for (size_t i = 0; i < points.size(); i++) {
    ...
    if (std::abs(point.t - kInfinityValue) < kDoubleEpsilon) points.resize(i + 1);
  }
