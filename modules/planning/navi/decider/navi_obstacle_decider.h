...
class NaviObstacleDecider : public NaviTask {
 ...
 private:
   ...
   */
  void AddObstacleOffsetDirection(const common::PathPoint&              projection_point,
                                  const std::vector<common::PathPoint>& path_data_points,
   ...
   */
  bool IsNeedFilterObstacle(const Obstacle*                       current_obstacle,
   ...
   */
  double GetObstacleActualOffsetDistance(std::map<double, double>::iterator iter,
   ...
   */
  void SmoothNudgeDistance(const common::VehicleState& vehicle_state, double* nudge_dist);
...
inline void NaviObstacleDecider::SetLastNudgeDistance(double dist) { last_nudge_dist_ = dist; }
}  // namespace planning
}  // namespace apollo
