...
int NaviObstacleDecider::ProcessPathObstacle(const std::vector<const Obstacle*>&   obstacles,
                                             const std::vector<common::PathPoint>& path_data_points,
  ...
  for (const auto& current_obstacle : obstacles) {
    if (current_obstacle->Perception().has_velocity() &&
      ...
      if (current_obstacle->Perception().velocity().x() > 0.0) { continue; }
    ...
    if (fabs(dest_ptr->PerceptionSLBoundary().start_l()) < (min_lane_width / 2)) {
    ...
    if (dist < (kMaxNudgeDistance + current_obstacle->Perception().width() +
      ...
      if ((proj_len == 0) || (proj_len >= path_data_points.back().s())) { continue; }
      JudgePointLeftOrRight(projection_point, path_data_points, current_obstacle, proj_len, &dist);
...
double NaviObstacleDecider::GetNudgeDistance(const std::vector<const Obstacle*>&   obstacles,
  ...
  if (routing_y <= 0.0) {
    left_nudge_lane = min_lane_width / 2.0 - fabs(routing_y) - VehicleParam().left_edge_to_center();
    right_nedge_lane =
        -1.0 * (min_lane_width / 2.0 + fabs(routing_y) - VehicleParam().right_edge_to_center());
  } else {
    left_nudge_lane = min_lane_width / 2.0 + fabs(routing_y) - VehicleParam().left_edge_to_center();
    right_nedge_lane =
        -1.0 * (min_lane_width / 2.0 - fabs(routing_y) - VehicleParam().right_edge_to_center());
  ...
  *lane_obstacles_num =
      ProcessPathObstacle(obstacles, path_data_points, path_decision, min_lane_width);
  for (auto iter = obstacle_lat_dist_.begin(); iter != obstacle_lat_dist_.end(); iter++) {
    ...
    ADEBUG << "get obstacle width : " << obs_width << "get lattitude distance : " << lat_dist;
    auto actual_dist = fabs(lat_dist) - obs_width / 2.0 - VehicleParam().left_edge_to_center();
    if ((actual_dist > kMinNudgeDistance) && (actual_dist < kMaxNudgeDistance)) {
...
void NaviObstacleDecider::GetUnsafeObstaclesInfo(
  ...
  if (reference_line_y < 0.0) {
    unsafe_refline_pos_y = reference_line_y - VehicleParam().right_edge_to_center() - kSafeDistance;
    ...
    unsafe_refline_pos_y = reference_line_y + VehicleParam().left_edge_to_center() + kSafeDistance;
    unsafe_car_pos_y     = -1.0 * (VehicleParam().left_edge_to_center() + kSafeDistance);
  ...
  for (const auto& iter : obstacles) {
    ...
    if ((obstacle_y > unsafe_range.first) && (obstacle_y < unsafe_range.second)) {
      auto projection_point = PathMatcher::MatchToPath(
          path_data_points, iter->Perception().position().x(), iter->Perception().position().y());
      ...
      unsafe_obstacle_info_.emplace_back(iter->Id(), projection_point.s(), project_velocity);
    }
  }
}
}  // namespace planning
