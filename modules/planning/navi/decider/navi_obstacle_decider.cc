...
NaviObstacleDecider::NaviObstacleDecider()
    : NaviTask("NaviObstacleDecider") {}

bool NaviObstacleDecider::Init(const PlanningConfig& config) {
  PlannerNaviConfig planner_navi_conf = config.navigation_planning_config().planner_navi_config();
  config_                             = planner_navi_conf.navi_obstacle_decider_config();
  return true;
}

double NaviObstacleDecider::GetMinLaneWidth(const std::vector<common::PathPoint>& path_data_points,
  ...
  for (const auto& path_data_point : path_data_points) {
    bool ret =
        reference_line.GetLaneWidth(path_data_point.s(), &lane_left_width, &lane_right_width);
    if (ret) {
      double lane_width = lane_left_width + lane_right_width;
      if (lane_width < min_lane_width) { min_lane_width = lane_width; }
    }
  }
...
void NaviObstacleDecider::AddObstacleOffsetDirection(
  ...
  auto d = ((current_obstacle->Perception().position().x() - p1.x()) * (p2.y() - p1.y())) -
           ((current_obstacle->Perception().position().y() - p1.y()) * (p2.x() - p1.x()));
  if (d > 0) { *dist = *dist * -1; }
}
...
bool NaviObstacleDecider::IsNeedFilterObstacle(
  ...
  *projection_point_ptr =
      PathMatcher::MatchToPath(path_data_points, current_obstacle->Perception().position().x(),
  ...
  if ((projection_point_ptr->s() - vehicle_projection_point.s()) >
      (config_.judge_dis_coeff() * vehicle_state.linear_velocity() + config_.basis_dis_value())) {
    return is_filter;
  }
  double vehicle_frontedge_position = vehicle_projection_point.s() + VehicleParam().length();
  ...
  double obstacle_end_position =
      projection_point_ptr->s() + current_obstacle->Perception().length() / 2.0;
  if ((vehicle_backedge_position - obstacle_end_position) > config_.safe_distance()) {
    return is_filter;
  }
  if ((obstacle_start_position - vehicle_frontedge_position) > config_.safe_distance()) {
    if (!current_obstacle->IsStatic()) {
      if (current_obstacle->Perception().velocity().x() > 0.0) { return is_filter; }
    }
  }
  is_filter = false;
  return is_filter;
...
void NaviObstacleDecider::ProcessObstacle(const std::vector<const Obstacle*>&   obstacles,
                                          const std::vector<common::PathPoint>& path_data_points,
                                          const PathDecision&                   path_decision,
                                          const double                          min_lane_width,
                                          const common::VehicleState&           vehicle_state) {
  auto func_distance = [](const PathPoint& point, const double x, const double y) {
    double dx = point.x() - x;
    double dy = point.y() - y;
  ...
  PathPoint vehicle_projection_point = PathMatcher::MatchToPath(path_data_points, 0, 0);
  for (const auto& current_obstacle : obstacles) {
    bool is_continue = IsNeedFilterObstacle(current_obstacle, vehicle_projection_point,
                                            path_data_points, vehicle_state, &projection_point);
    if (is_continue) { continue; }
    auto dist = func_distance(projection_point, current_obstacle->Perception().position().x(),
                              current_obstacle->Perception().position().y());

    if (dist < (config_.max_nudge_distance() + current_obstacle->Perception().width() +
                VehicleParam().left_edge_to_center())) {
      auto proj_len = projection_point.s();
      if (std::fabs(proj_len) <= kEpislon || proj_len >= path_data_points.back().s()) { continue; }
      AddObstacleOffsetDirection(projection_point, path_data_points, current_obstacle, proj_len,
                                 &dist);
      obstacle_lat_dist_.emplace(
          std::pair<double, double>(current_obstacle->Perception().width(), dist));
    }
  }
}

double NaviObstacleDecider::GetObstacleActualOffsetDistance(std::map<double, double>::iterator iter,
                                                            const double right_nudge_lane,
                                                            const double left_nudge_lane,
                                                            int*         lane_obstacles_num) {
  auto obs_width = iter->first;
  auto lat_dist  = iter->second;
  ADEBUG << "get obstacle width : " << obs_width << "get latitude distance : " << lat_dist;
  auto actual_dist = std::fabs(lat_dist) - obs_width / 2.0 - VehicleParam().left_edge_to_center();
  ...
  if ((last_lane_obstacles_num_ != 0) && (*lane_obstacles_num == 0) && (!is_obstacle_stable_)) {
  ...
  ADEBUG << "last_nudge_dist : " << last_nudge_dist_ << "lat_dist : " << lat_dist
...
void NaviObstacleDecider::RecordLastNudgeDistance(const double nudge_dist) {
  ...
  if (std::fabs(nudge_dist) > tolerance) {
    if (std::fabs(nudge_dist) > std::fabs(last_nudge_dist_)) { last_nudge_dist_ = nudge_dist; }
    no_nudge_num_ = 0;
  } else {
    ++no_nudge_num_;
  }

  if (no_nudge_num_ >= config_.cycles_number()) { last_nudge_dist_ = 0.0; }
}

void NaviObstacleDecider::SmoothNudgeDistance(const common::VehicleState& vehicle_state,
                                              double*                     nudge_dist) {
  ...
  if (limit_speed_num_ < config_.cycles_number()) { *nudge_dist = 0; }
  if (std::fabs(*nudge_dist) > config_.nudge_allow_tolerance()) {
    ++eliminate_clutter_num_;
  } else {
    eliminate_clutter_num_ = 0;
  }
  if (eliminate_clutter_num_ < config_.cycles_number()) { *nudge_dist = 0; }
  ADEBUG << "eliminate_clutter_num_: " << eliminate_clutter_num_;
}

double NaviObstacleDecider::GetNudgeDistance(const std::vector<const Obstacle*>&   obstacles,
                                             const ReferenceLine&                  reference_line,
                                             const PathDecision&                   path_decision,
                                             const std::vector<common::PathPoint>& path_data_points,
                                             const common::VehicleState&           vehicle_state,
  ...
  if (routing_y <= 0.0) {
    left_nudge_lane =
        min_lane_width / 2.0 - std::fabs(routing_y) - VehicleParam().left_edge_to_center();
  ...
  ProcessObstacle(obstacles, path_data_points, path_decision, min_lane_width, vehicle_state);
  for (auto iter = obstacle_lat_dist_.begin(); iter != obstacle_lat_dist_.end(); ++iter) {
    auto actual_dist = GetObstacleActualOffsetDistance(iter, right_nudge_lane, left_nudge_lane,
                                                       lane_obstacles_num);
    auto lat_dist    = iter->second;
    if (actual_dist > config_.min_nudge_distance() && actual_dist < config_.max_nudge_distance()) {
      auto need_nudge_dist = config_.max_nudge_distance() - actual_dist;
      if (lat_dist >= 0.0) {
        right_nudge_obstacle = -1.0 * std::max(std::fabs(right_nudge_obstacle), need_nudge_dist);
      } else {
        left_nudge_obstacle = std::max(std::fabs(left_nudge_obstacle), need_nudge_dist);
      }
    }
  }
  ...
  if (std::fabs(left_nudge_obstacle) > kEpislon && std::fabs(right_nudge_obstacle) <= kEpislon) {
...
void NaviObstacleDecider::KeepNudgePosition(const double nudge_dist, int* lane_obstacles_num) {
  if (std::fabs(nudge_dist) > config_.nudge_allow_tolerance() &&
      std::fabs(last_nudge_dist_) < config_.nudge_allow_tolerance() && !keep_nudge_flag_) {
    cycles_count_    = 0;
...
void NaviObstacleDecider::GetUnsafeObstaclesInfo(
  ...
  if (reference_line_y < 0.0) {
    unsafe_refline_pos_y = reference_line_y - VehicleParam().right_edge_to_center() -
                           config_.speed_decider_detect_range();
    unsafe_car_pos_y = VehicleParam().right_edge_to_center() + config_.speed_decider_detect_range();
    unsafe_range     = std::make_pair(unsafe_refline_pos_y, unsafe_car_pos_y);
  } else {
    unsafe_refline_pos_y = reference_line_y + VehicleParam().left_edge_to_center() +
                           config_.speed_decider_detect_range();
    unsafe_car_pos_y =
        -1.0 * (VehicleParam().left_edge_to_center() + config_.speed_decider_detect_range());
  ...
  for (const auto& iter : obstacles) {
    ...
    if ((obstacle_y > unsafe_range.first && obstacle_y < unsafe_range.second) ||
        std::abs(iter->Perception().velocity().y()) > config_.lateral_velocity_value()) {
      auto projection_point =
          PathMatcher::MatchToPath(path_data_points, iter->Perception().position().x(), obstacle_y);
      if (vehicle_projection_point.s() >= projection_point.s()) { continue; }
      auto front_distance =
          (projection_point.s() - iter->Perception().length() / 2.0) - vehicle_projection_point.s();
      auto ref_theta        = projection_point.theta();
      auto project_velocity = iter->Perception().velocity().x() * std::cos(ref_theta) +
                              iter->Perception().velocity().y() * std::sin(ref_theta);
      ADEBUG << "Lateral speed : " << iter->Perception().velocity().y();
      unsafe_obstacle_info_.emplace_back(iter->Id(), front_distance, project_velocity);
    }
  }
}
}  // namespace planning
