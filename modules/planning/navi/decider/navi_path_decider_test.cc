...
class NaviPathDeciderTest : public ::testing::Test {
 public:
  ...
  static void GeneratePathData(double                                s,
                               double                                init_y,
                               double                                kappa,
                               std::vector<common::PathPoint>* const path_points) {
    for (double x = 0.0, y = init_y; x < s; ++x) {
      path_points->clear();
      path_points->push_back(PointFactory::ToPathPoint(x, y, 0.0, 0.0, 0.0, kappa));
    }
  }

  static void InitPlannigConfig(PlanningConfig* const plannig_config) {
    auto* navi_planner_config =
        plannig_config->mutable_navigation_planning_config()->mutable_planner_navi_config();
    auto* navi_path_decider_config = navi_planner_config->mutable_navi_path_decider_config();
...
TEST_F(NaviPathDeciderTest, MoveToDestLane) {
  ...
  auto navi_path_decider_cfg =
      config.navigation_planning_config().planner_navi_config().navi_path_decider_config();
...
TEST_F(NaviPathDeciderTest, KeepLane) {
  ...
  navi_path_decider.reference_line_info_ =
      new ReferenceLineInfo(vehicle_state, plan_start_point, ref_line, route_segments);
  LocalView local_view;
  navi_path_decider.frame_ = new Frame(1, local_view, plan_start_point, vehicle_state, nullptr);
