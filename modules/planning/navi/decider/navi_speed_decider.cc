...
NaviSpeedDecider::NaviSpeedDecider()
    : NaviTask("NaviSpeedDecider") {}

bool NaviSpeedDecider::Init(const PlanningConfig& planning_config) {
  CHECK_GT(FLAGS_planning_upper_speed_limit, 0.0);
  NavigationPlanningConfig config = planning_config.navigation_planning_config();
  ACHECK(config.has_planner_navi_config());
  ACHECK(config.planner_navi_config().has_navi_speed_decider_config());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_preferred_accel());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_preferred_decel());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_max_accel());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_max_decel());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_preferred_jerk());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_obstacle_buffer());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_safe_distance_base());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_safe_distance_ratio());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_following_accel_ratio());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_soft_centric_accel_limit());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_hard_centric_accel_limit());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_hard_speed_limit());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_hard_accel_limit());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_enable_safe_path());
  ACHECK(
      config.planner_navi_config().navi_speed_decider_config().has_enable_planning_start_point());
  ACHECK(config.planner_navi_config()
             .navi_speed_decider_config()
             .has_enable_accel_auto_compensation());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_kappa_preview());
  ACHECK(config.planner_navi_config().navi_speed_decider_config().has_kappa_threshold());

  max_speed_ = FLAGS_planning_upper_speed_limit;
  preferred_accel_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().preferred_accel());
  preferred_decel_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().preferred_decel());
  preferred_jerk_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().preferred_jerk());
  max_accel_       = std::abs(config.planner_navi_config().navi_speed_decider_config().max_accel());
  ...
  obstacle_buffer_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().obstacle_buffer());
  safe_distance_base_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().safe_distance_base());
  safe_distance_ratio_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().safe_distance_ratio());
  following_accel_ratio_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().following_accel_ratio());
  soft_centric_accel_limit_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().soft_centric_accel_limit());
  hard_centric_accel_limit_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().hard_centric_accel_limit());
  soft_centric_accel_limit_ = std::min(hard_centric_accel_limit_, soft_centric_accel_limit_);
  hard_speed_limit_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().hard_speed_limit());
  hard_accel_limit_ =
      std::abs(config.planner_navi_config().navi_speed_decider_config().hard_accel_limit());
  enable_safe_path_ = config.planner_navi_config().navi_speed_decider_config().enable_safe_path();
  enable_planning_start_point_ =
      config.planner_navi_config().navi_speed_decider_config().enable_planning_start_point();
  enable_accel_auto_compensation_ =
      config.planner_navi_config().navi_speed_decider_config().enable_accel_auto_compensation();
  kappa_preview_   = config.planner_navi_config().navi_speed_decider_config().kappa_preview();
  kappa_threshold_ = config.planner_navi_config().navi_speed_decider_config().kappa_threshold();

  return obstacle_decider_.Init(planning_config);
}

Status NaviSpeedDecider::Execute(Frame* frame, ReferenceLineInfo* reference_line_info) {
  NaviTask::Execute(frame, reference_line_info);

  // get cruise speed
  const auto& planning_target = reference_line_info_->planning_target();
  preferred_speed_ =
      planning_target.has_cruise_speed() ? std::abs(planning_target.cruise_speed()) : 0.0;
  ...
  auto        expected_v           = planning_start_point.has_v() ? planning_start_point.v() : 0.0;
  auto        expected_a           = planning_start_point.has_a() ? planning_start_point.a() : 0.0;

  // current status of vehicle
  const auto& vehicle_state = frame->vehicle_state();
  auto current_v = vehicle_state.has_linear_velocity() ? vehicle_state.linear_velocity() : 0.0;
  auto current_a =
      vehicle_state.has_linear_acceleration() ? vehicle_state.linear_acceleration() : 0.0;
  ...
  if (enable_accel_auto_compensation_) {
    if (prev_v_ > 0.0 && current_v > 0.0 && prev_v_ > expected_v) {
      ...
      decel_compensation_ratio_ = Clamp(decel_compensation_ratio_, 1.0, kDecelCompensationLimit);
  ...
  if (ret != Status::OK()) {
    ...
    AERROR << "Reference Line " << reference_line_info->Lanes().Id() << " is not drivable after "
           << Name();
  }
...
Status NaviSpeedDecider::MakeSpeedDecision(
  ...
  ADEBUG << "start to make speed decision,  start_v: " << start_v << " start_a: " << start_a
         << " start_da: " << start_da << " start_s: " << start_s
  ...
  auto s_step =
      planning_length > kTsGraphSStep ? kTsGraphSStep : planning_length / kFallbackSpeedPointNum;
  ...
  ret = AddObstaclesConstraints(start_v, planning_length, path_points, obstacles, find_obstacle);
  if (ret != Status::OK()) {
  ...
  for (auto& ts_point : ts_points) {
    if (ts_point.s > kSpeedPointSLimit || ts_point.t > kSpeedPointTimeLimit) break;
    ...
    speed_data->AppendSpeedPoint(ts_point.s + start_s, ts_point.t, ts_point.v, ts_point.a,
                                 ts_point.da);
  }
...
Status NaviSpeedDecider::AddObstaclesConstraints(
  ...
  auto        front_edge_to_center  = vehicle_config.vehicle_param().front_edge_to_center();
  ...
  auto get_safe_distance = [&](double v) -> double {
    return safe_distance_ratio_ * v + safe_distance_base_ + front_edge_to_center + obstacle_buffer_;
  };
  ...
  for (const auto& info : obstacle_decider_.UnsafeObstacles()) {
    ...
    if (obstacle != nullptr) {
      ...
      AINFO << "obstacle with id: " << id << " s: " << s << " distance: " << obstacle_distance
            << " speed: " << obstacle_speed << " safe_distance: " << safe_distance;

      ts_graph_.UpdateObstacleConstraints(obstacle_distance, safe_distance, following_accel_ratio_,
...
Status
NaviSpeedDecider::AddCentricAccelerationConstraints(const std::vector<PathPoint>& path_points) {
  ...
  for (size_t i = 1; i < path_points.size(); i++) {
    ...
    auto v_preffered =
        std::min(std::sqrt(soft_centric_accel_limit_ / kappa), std::numeric_limits<double>::max());
    auto v_max =
        std::min(std::sqrt(hard_centric_accel_limit_ / kappa), std::numeric_limits<double>::max());
  ...
  for (size_t i = 0; i < c_limits.size(); i++) {
    for (size_t j = i; j - i < (size_t)(kappa_preview_ / kTsGraphSStep) && j < c_limits.size(); j++)
      c_limits[i].v_preffered = std::min(c_limits[j].v_preffered, c_limits[i].v_preffered);
  }
