...
bool NaviSpeedDecider::Init(const PlanningConfig& config) {
  ...
  CHECK(config.navi_planner_config().navi_speed_decider_config().has_preferred_accel());
  CHECK(config.navi_planner_config().navi_speed_decider_config().has_preferred_decel());
  CHECK(config.navi_planner_config().navi_speed_decider_config().has_max_accel());
  CHECK(config.navi_planner_config().navi_speed_decider_config().has_max_decel());
  CHECK(config.navi_planner_config().navi_speed_decider_config().has_obstacle_buffer());
  CHECK(config.navi_planner_config().navi_speed_decider_config().has_safe_distance_base());
  CHECK(config.navi_planner_config().navi_speed_decider_config().has_safe_distance_ratio());

  preferred_accel_ =
      std::abs(config.navi_planner_config().navi_speed_decider_config().preferred_accel());
  preferred_decel_ =
      std::abs(config.navi_planner_config().navi_speed_decider_config().preferred_decel());
  max_accel_       = std::abs(config.navi_planner_config().navi_speed_decider_config().max_accel());
  max_decel_       = std::abs(config.navi_planner_config().navi_speed_decider_config().max_decel());
  preferred_accel_ = std::min(max_accel_, preferred_accel_);
  preferred_decel_ = std::min(max_decel_, preferred_accel_);

  obstacle_buffer_ =
      std::abs(config.navi_planner_config().navi_speed_decider_config().obstacle_buffer());
  safe_distance_base_ =
      std::abs(config.navi_planner_config().navi_speed_decider_config().safe_distance_base());
  safe_distance_ratio_ =
      std::abs(config.navi_planner_config().navi_speed_decider_config().safe_distance_ratio());
...
Status NaviSpeedDecider::Execute(Frame* frame, ReferenceLineInfo* reference_line_info) {
  ...
  auto ret = MakeSpeedDecision(
      start_v, start_a, start_da, kTsGraphSMax, path_data_points, frame_->obstacles(),
...
Status NaviSpeedDecider::MakeSpeedDecision(
  ...
  // init t-s graph
  ts_graph_.Reset(kTsGraphSStep, planning_length,
                  [&](double v) { return safe_distance_ratio_ * v + safe_distance_base_; });
  ...
  ret = AddObstaclesConstraints(start_v, path_data_points, obstacles, find_obstacle);
  if (ret != Status::OK()) {
  ...
  for (const auto& ts_point : ts_points)
    speed_data->AppendSpeedPoint(ts_point.s, ts_point.t, ts_point.v, ts_point.a, ts_point.da);
...
Status NaviSpeedDecider::AddObstaclesConstraints(
  ...
  for (const auto& info : obstacle_decider_.UnsafeObstacles()) {
    ...
    if (obstacle != nullptr) {
      ...
      auto obstacle_distance = obstacle_aa_box.min_x() - front_edge_to_center - obstacle_buffer_;
      obstacle_distance      = std::max(0.0, obstacle_distance);

      double rel_speed = 0.0;
      if (obstacle->Perception().has_velocity() && obstacle->Perception().velocity().has_x())
