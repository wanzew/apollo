...
TEST(NaviSpeedDeciderTest, CreateSpeedDataForStaticObstacle) {
  ...
  std::string id = "1";
  obstacle_buf.emplace(id, Obstacle(id, perception_obstacle, ObstaclePriority::NORMAL, false));
...
TEST(NaviSpeedDeciderTest, CreateSpeedDataForCurve) {
  ...
  for (size_t i = 1; i <= 5; i++) {
    s += 1.0;
    path_points.emplace_back(GenPathPoint(s, 0.03 * static_cast<double>(5 - i)));
  }
  s += 1.0;
  path_points.emplace_back(GenPathPoint(s, 0.0));
  s += 10.0;
