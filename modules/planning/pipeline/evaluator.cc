...
DEFINE_double(trajectory_delta_t, 0.2, "delta time(sec) between trajectory points");
...
void Evaluator::Init() {
  ...
  EvaluatorLogger::GetStream() << "UTC date and time: " << std::asctime(std::gmtime(&now))
...
void Evaluator::Close() {
  ...
  EvaluatorLogger::GetStream() << "Time elapsed(sec): " << elapsed_seconds.count() << std::endl
                               << std::endl;
}
...
void Evaluator::Evaluate(const std::string& source_file) {
  ...
  const std::string& source_filename = source_file.substr(source_file.find_last_of("/") + 1);
  ...
  for (int i = 0; i < learning_data_.learning_data_frame_size(); ++i) {
    ...
    if (learning_data_frame->adc_trajectory_point_size() <= 0) { continue; }
    ...
    trajectory_evaluator_.EvaluateADCTrajectory(start_point_timestamp_sec, FLAGS_trajectory_delta_t,
                                                learning_data_frame);

    // evaluate adc future trajectory
    std::vector<TrajectoryPointFeature> adc_future_trajectory;
    for (const auto& tp :
         learning_data_.learning_data_frame(i).output().adc_future_trajectory_point()) {
    ...
    trajectory_evaluator_.EvaluateADCFutureTrajectory(
        learning_data_.learning_data_frame(i).frame_num(), adc_future_trajectory,
        start_point_timestamp_sec, FLAGS_trajectory_delta_t, &evaluated_adc_future_trajectory);
    ...
    for (const auto& tp : evaluated_adc_future_trajectory) {
      auto adc_future_trajectory_point =
          learning_data_frame->mutable_output()->add_adc_future_trajectory_point();
      adc_future_trajectory_point->set_timestamp_sec(tp.timestamp_sec());
      adc_future_trajectory_point->mutable_trajectory_point()->CopyFrom(tp.trajectory_point());
    }

    // evaluate obstacle trajectory
    trajectory_evaluator_.EvaluateObstacleTrajectory(start_point_timestamp_sec,
                                                     FLAGS_trajectory_delta_t, learning_data_frame);

    // evaluate obstacle prediction trajectory
    trajectory_evaluator_.EvaluateObstaclePredictionTrajectory(
        start_point_timestamp_sec, FLAGS_trajectory_delta_t, learning_data_frame);
  }
