...
void GenerateLearningData() {
  ...
  if (FLAGS_planning_offline_bags.empty()) { return; }
  ...
  ACHECK(cyber::common::GetProtoFromFile(planning_config_file, &planning_config))
      << "failed to load planning config file " << planning_config_file;

  MessageProcess message_process;
  if (!message_process.Init(planning_config)) { return; }

  const std::vector<std::string> inputs = absl::StrSplit(FLAGS_planning_offline_bags, ':');
