...
SpiralReferenceLineSmoother::SpiralReferenceLineSmoother(const ReferenceLineSmootherConfig& config)
    : ReferenceLineSmoother(config) {}

bool SpiralReferenceLineSmoother::Smooth(const ReferenceLine& raw_reference_line,
  ...
  if (anchor_points_.empty()) {
    ...
    uint32_t num_of_pieces = std::max(1u, static_cast<uint32_t>(length / piecewise_length));
    ...
    Smooth(raw_point2d, &opt_theta, &opt_kappa, &opt_dkappa, &opt_s, &opt_x, &opt_y);
  } else {
    ...
    if (start_index == 0) {
      ...
      fixed_start_theta_  = common::math::NormalizeAngle(start_anchor_point.path_point.theta());
      ...
      opt_theta.insert(opt_theta.begin(), overhead_theta.begin(), overhead_theta.end());
      opt_kappa.insert(opt_kappa.begin(), overhead_kappa.begin(), overhead_kappa.end());
      opt_dkappa.insert(opt_dkappa.begin(), overhead_dkappa.begin(), overhead_dkappa.end());
      ...
      std::for_each(opt_x.begin(), opt_x.end(), [this](double& x) { x += zero_x_; });

      std::for_each(opt_y.begin(), opt_y.end(), [this](double& y) { y += zero_y_; });
    }
  }

  std::vector<common::PathPoint> smoothed_point2d =
      Interpolate(opt_theta, opt_kappa, opt_dkappa, opt_s, opt_x, opt_y, config_.resolution());
  ...
  for (const auto& p : smoothed_point2d) {
    ...
    if (!raw_reference_line.XYToSL(p, &ref_sl_point)) { return false; }
    if (ref_sl_point.s() < 0 || ref_sl_point.s() > raw_reference_line.Length()) { continue; }

    ReferencePoint rlp = raw_reference_line.GetReferencePoint(ref_sl_point.s());
    ref_points.emplace_back(ReferencePoint(
        hdmap::MapPathPoint(common::math::Vec2d(p.x(), p.y()), heading, rlp.lane_waypoints()),
        kappa, dkappa));
  }
...
int SpiralReferenceLineSmoother::SmoothStandAlone(std::vector<Eigen::Vector2d> point2d,
  ...
  app->Options()->SetNumericValue("acceptable_tol", config_.spiral().opt_acceptable_tol());
...
bool SpiralReferenceLineSmoother::Smooth(std::vector<Eigen::Vector2d> point2d,
  ...
  app->Options()->SetIntegerValue("max_iter", config_.spiral().max_iteration());
  app->Options()->SetIntegerValue("acceptable_iter", config_.spiral().opt_acceptable_iteration());
  app->Options()->SetNumericValue("tol", config_.spiral().opt_tol());
  app->Options()->SetNumericValue("acceptable_tol", config_.spiral().opt_acceptable_tol());
...
SpiralReferenceLineSmoother::Interpolate(const std::vector<double>& theta,
  ...
  common::PathPoint              first_point =
      to_path_point(x.front(), y.front(), start_s, theta.front(), kappa.front(), dkappa.front());
...
std::vector<common::PathPoint>
SpiralReferenceLineSmoother::Interpolate(const double start_x,
  ...
  for (size_t i = 1; i <= num_of_points; ++i) {
    const double inter_s = delta_s / static_cast<double>(num_of_points) * static_cast<double>(i);
    ...
    const double theta  = common::math::NormalizeAngle(spiral_curve.Evaluate(0, inter_s));
    const double kappa  = spiral_curve.Evaluate(1, inter_s);
    const double dkappa = spiral_curve.Evaluate(2, inter_s);

    auto path_point =
        to_path_point(start_x + dx, start_y + dy, start_s + inter_s, theta, kappa, dkappa);
    path_points.push_back(std::move(path_point));
  }
  return path_points;
}

common::PathPoint SpiralReferenceLineSmoother::to_path_point(const double x,
                                                             const double y,
                                                             const double s,
                                                             const double theta,
                                                             const double kappa,
...
void SpiralReferenceLineSmoother::SetAnchorPoints(const std::vector<AnchorPoint>& anchor_points) {
  ...
  std::for_each(anchor_points_.begin(), anchor_points_.end(), [this](AnchorPoint& p) {
