...
bool SpiralReferenceLineSmoother::Smooth(const ReferenceLine& raw_reference_line,
  ...
  for (const auto& p : smoothed_point2d) {
    ...
    if (!raw_reference_line.XYToSL({p.x(), p.y()}, &ref_sl_point)) { return false; }
  ...
  *smoothed_reference_line   = ReferenceLine(ref_points);
  ...
  ADEBUG << "Spiral reference line smoother time: " << (end_timestamp - start_timestamp) * 1000
         << " ms.";
  return true;
}
...
bool SpiralReferenceLineSmoother::Smooth(std::vector<Eigen::Vector2d> point2d,
  ...
  ptop->set_element_weight_curve_length(config_.spiral().opt_weight_curve_length());
