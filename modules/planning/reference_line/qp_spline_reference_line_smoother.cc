...
QpSplineReferenceLineSmoother::QpSplineReferenceLineSmoother(
  ...
  spline_solver_.reset(new Spline2dSolver(t_knots_, config.qp_spline().spline_order()));
}

void QpSplineReferenceLineSmoother::Clear() { t_knots_.clear(); }

bool QpSplineReferenceLineSmoother::Smooth(const ReferenceLine& raw_reference_line,
  ...
  for (std::uint32_t i = 0; i < config_.num_of_total_points() && t < end_t; ++i, t += resolution) {
    const double heading = std::atan2(spline.DerivativeY(t), spline.DerivativeX(t));
    const double kappa =
        CurveMath::ComputeCurvature(spline.DerivativeX(t), spline.SecondDerivativeX(t),
                                    spline.DerivativeY(t), spline.SecondDerivativeY(t));
    const double dkappa = CurveMath::ComputeCurvatureDerivative(
        spline.DerivativeX(t), spline.SecondDerivativeX(t), spline.ThirdDerivativeX(t),
        spline.DerivativeY(t), spline.SecondDerivativeY(t), spline.ThirdDerivativeY(t));

    std::pair<double, double> xy = spline(t);
    xy.first += ref_x_;
    xy.second += ref_y_;
    common::SLPoint ref_sl_point;
    if (!raw_reference_line.XYToSL({xy.first, xy.second}, &ref_sl_point)) { return false; }
    ...
    ref_points.emplace_back(ReferencePoint(
        hdmap::MapPathPoint(common::math::Vec2d(xy.first, xy.second), heading, new_lane_waypoints),
        kappa, dkappa));
  }
...
bool QpSplineReferenceLineSmoother::Sampling() {
  ...
  uint32_t num_spline =
      std::max(1u, static_cast<uint32_t>(length / config_.qp_spline().max_spline_length() + 0.5));
...
bool QpSplineReferenceLineSmoother::AddConstraint() {
  ...
  if (!spline_constraint->Add2dBoundary(evaluated_t, headings, xy_points, longitudinal_bound,
                                        lateral_bound)) {
  ...
  if (!spline_constraint->AddPointAngleConstraint(evaluated_t.front(), headings.front())) {
...
bool QpSplineReferenceLineSmoother::AddKernel() {
  ...
  if (config_.qp_spline().second_derivative_weight() > 0.0) {
    kernel->AddSecondOrderDerivativeMatrix(config_.qp_spline().second_derivative_weight());
  }
  if (config_.qp_spline().third_derivative_weight() > 0.0) {
    kernel->AddThirdOrderDerivativeMatrix(config_.qp_spline().third_derivative_weight());
  }
...
void QpSplineReferenceLineSmoother::SetAnchorPoints(const std::vector<AnchorPoint>& anchor_points) {
