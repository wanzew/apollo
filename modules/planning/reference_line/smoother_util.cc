...
class SmootherUtil {
 public:
  explicit SmootherUtil(const std::string& filename)
    ...
    CHECK(common::util::GetProtoFromFile(FLAGS_smoother_config_filename, &config_))
  ...
  bool Smooth() {
    ...
    std::size_t i = 1;
    {
      ...
      auto smoother_ptr = std::make_unique<SpiralReferenceLineSmoother>(config_);
  ...
  void Export(const std::string& filename) {
    ...
    for (std::size_t i = 1; i + 1 < ref_points_.size(); ++i) {
      ...
      ofs << std::fixed << "{\"kappa\": " << point.kappa() << ", \"s\": " << s
          ...
          << ", \"y\":" << point.y() << ", \"dkappa\":" << point.dkappa() << "}" << std::endl;
