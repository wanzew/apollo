...
ReferenceLine::ReferenceLine(const std::vector<ReferencePoint>& reference_points)
    ...
    , map_path_(std::move(
          std::vector<hdmap::MapPathPoint>(reference_points.begin(), reference_points.end()))) {
...
ReferenceLine::ReferenceLine(const MapPath& hdmap_path)
  ...
  for (const auto& point : hdmap_path.path_points()) {
    ...
    reference_points_.emplace_back(hdmap::MapPathPoint(point, point.heading(), lane_waypoint), 0.0,
                                   0.0);
  }
  CHECK_EQ(static_cast<size_t>(map_path_.num_points()), reference_points_.size());
}
...
bool ReferenceLine::Stitch(const ReferenceLine& other) {
  ...
  if (last_join) {
    ...
    reference_points_.insert(reference_points_.end(), other_points.begin() + end_i,
                             other_points.end());
  }
  map_path_ = MapPath(std::move(
      std::vector<hdmap::MapPathPoint>(reference_points_.begin(), reference_points_.end())));
  return true;
}
...
bool ReferenceLine::Segment(const double s, const double look_backward, const double look_forward) {
  ...
  map_path_ =
      MapPath(std::vector<hdmap::MapPathPoint>(reference_points_.begin(), reference_points_.end()));
  return true;
}

common::FrenetFramePoint ReferenceLine::GetFrenetPoint(const common::PathPoint& path_point) const {
  if (reference_points_.empty()) { return common::FrenetFramePoint(); }
...
ReferenceLine::ToFrenetFrame(const common::TrajectoryPoint& traj_point) const {
  ...
  CartesianFrenetConverter::cartesian_to_frenet(
      ...
      ref_point.dkappa(), traj_point.path_point().x(), traj_point.path_point().y(), traj_point.v(),
...
ReferencePoint ReferenceLine::GetNearestReferencePoint(const double s) const {
  ...
  if (it_lower == accumulated_s.begin()) { return reference_points_.front(); }
...
ReferencePoint ReferenceLine::GetReferencePoint(const double s) const {
  ...
  if (next_index >= reference_points_.size()) { next_index = reference_points_.size() - 1; }
...
ReferencePoint ReferenceLine::GetReferencePoint(const double x, const double y) const {
  ...
  auto func_distance_square = [](const ReferencePoint& point, const double x, const double y) {
    double dx = point.x() - x;
    double dy = point.y() - y;
    return dx * dx + dy * dy;
  };
  ...
  double s = ReferenceLine::FindMinDistancePoint(reference_points_[index_start], s0,
                                                 reference_points_[index_end], s1, x, y);

  return Interpolate(reference_points_[index_start], s0, reference_points_[index_end], s1, s);
}
...
ReferencePoint ReferenceLine::InterpolateWithMatchedIndex(const ReferencePoint&    p0,
  ...
  if (std::fabs(s0 - s1) < common::math::kMathEpsilon) { return p0; }
...
ReferencePoint ReferenceLine::Interpolate(const ReferencePoint& p0,
  ...
  return ReferencePoint(hdmap::MapPathPoint({x, y}, heading, waypoints), kappa, dkappa);
}
...
bool ReferenceLine::GetLaneWidth(const double  s,
                                 double* const lane_left_width,
                                 double* const lane_right_width) const {
  if (map_path_.path_points().empty()) { return false; }

  if (!map_path_.GetLaneWidth(s, lane_left_width, lane_right_width)) { return false; }
  return true;
}
...
bool ReferenceLine::GetRoadWidth(const double  s,
...
void ReferenceLine::GetLaneFromS(const double                          s,
...
double ReferenceLine::GetDrivingWidth(const SLBoundary& sl_boundary) const {
  ...
  double driving_width =
      std::max(lane_left_width - sl_boundary.end_l(), lane_right_width + sl_boundary.start_l());
...
bool ReferenceLine::IsOnLane(const SLBoundary& sl_boundary) const {
  if (sl_boundary.end_s() < 0 || sl_boundary.start_s() > Length()) { return false; }
...
bool ReferenceLine::IsOnLane(const SLPoint& sl_point) const {
  if (sl_point.s() <= 0 || sl_point.s() > map_path_.length()) { return false; }
  double left_width  = 0.0;
  double right_width = 0.0;

  if (!GetLaneWidth(sl_point.s(), &left_width, &right_width)) { return false; }

  return sl_point.l() >= -right_width && sl_point.l() <= left_width;
}

bool ReferenceLine::IsBlockRoad(const common::math::Box2d& box2d, double gap) const {
...
bool ReferenceLine::IsOnRoad(const SLBoundary& sl_boundary) const {
  ...
  return sl_boundary.start_l() <= road_left_width && sl_boundary.end_l() >= -road_right_width;
}
...
bool ReferenceLine::IsOnRoad(const SLPoint& sl_point) const {
  ...
  if (!GetRoadWidth(sl_point.s(), &road_left_width, &road_right_width)) { return false; }
...
bool ReferenceLine::GetApproximateSLBoundary(const common::math::Box2d& box,
  ...
  double l        = 0.0;
  double distance = 0.0;
  if (!map_path_.GetProjectionWithHueristicParams(box.center(), start_s, end_s, &s, &l,
                                                  &distance)) {
...
bool ReferenceLine::GetSLBoundary(const common::math::Box2d& box,
  ...
  for (size_t i = 0; i < corners.size(); ++i) {
    ...
    *sl_boundary->add_boundary_point() = sl_corners[index0];
    ...
    if (v0.CrossProd(v1) < 0.0) { *sl_boundary->add_boundary_point() = sl_point_mid; }
  }
...
bool ReferenceLine::HasOverlap(const common::math::Box2d& box) const {
  ...
  if (sl_boundary.end_s() < 0 || sl_boundary.start_s() > Length()) { return false; }
  if (sl_boundary.start_l() * sl_boundary.end_l() < 0) { return false; }
...
std::string ReferenceLine::DebugString() const {
  const auto limit =
      std::min(reference_points_.size(), static_cast<size_t>(FLAGS_trajectory_point_num_for_debug));
  return absl::StrCat("point num:", reference_points_.size(),
                      absl::StrJoin(reference_points_.begin(), reference_points_.begin() + limit,
...
double ReferenceLine::GetSpeedLimitFromS(const double s) const {
  for (const auto& speed_limit : speed_limit_) {
    if (s >= speed_limit.start_s && s <= speed_limit.end_s) { return speed_limit.speed_limit; }
  }
  ...
  if (!speed_limit_found) {
    ...
    if (road_type == hdmap::Road::HIGHWAY) { speed_limit = FLAGS_default_highway_speed_limit; }
  }
...
void ReferenceLine::AddSpeedLimit(double start_s, double end_s, double speed_limit) {
  ...
  speed_limit_.clear();
  if (end_s > start_s) { new_speed_limit.emplace_back(start_s, end_s, speed_limit); }
  for (const auto& limit : new_speed_limit) {
    if (limit.start_s < limit.end_s) { speed_limit_.emplace_back(limit); }
  }
  std::sort(speed_limit_.begin(), speed_limit_.end(), [](const SpeedLimit& a, const SpeedLimit& b) {
