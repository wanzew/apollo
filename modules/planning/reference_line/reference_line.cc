...
bool ReferenceLine::Stitch(const ReferenceLine& other) {
  ...
  bool last_join =
      last_sl.s() > 0 && last_sl.s() < other.Length() && std::fabs(last_sl.l()) < kStitchingError;
  ...
  if (!first_join && !last_join) {
    ...
    if (!XYToSL(other_points.front(), &other_first)) {
      AERROR << "Could not project point : " << other_points.front().DebugString();
      return false;
    }
    bool other_on_current = other_first.s() >= 0 && other_first.s() < Length() &&
                            std::fabs(other_first.l()) < kStitchingError;
    if (other_on_current) { return true; }
...
bool ReferenceLine::Shrink(const common::math::Vec2d& point,
  ...
  if (sl.s() > look_backward) {
    auto it_lower =
        std::lower_bound(accumulated_s.begin(), accumulated_s.end(), sl.s() - look_backward);
...
ReferencePoint ReferenceLine::GetReferencePoint(const double x, const double y) const {
  ...
  uint32_t index_end   = (index_min + 1 == reference_points_.size() ? index_min : index_min + 1);
...
std::string ReferenceLine::DebugString() const {
  ...
  return apollo::common::util::StrCat(
      ...
      apollo::common::util::PrintDebugStringIter(reference_points_.begin(),
...
void ReferenceLine::AddSpeedLimit(const hdmap::SpeedControl& speed_control) {
  SLBoundary sl_boundary;
  if (GetSLBoundary(speed_control.polygon(), &sl_boundary) && IsOnRoad(sl_boundary)) {
    AddSpeedLimit(sl_boundary.start_s(), sl_boundary.end_s(), speed_control.speed_limit());
  }
}
