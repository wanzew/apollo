  ...
  for (std::size_t i = 0; i + 1 < num_of_points_; ++i) {
    ...
    for (std::size_t j = 0; j < num_of_internal_points_; ++j) {
      ...
      grad_f[index0 + 2] += weight_dkappa_ * 2.0 * dkappa *
                            spiral_curve.DeriveDKappaDerivative(QuinticSpiralPath::DKAPPA0, ratio);
      ...
      grad_f[index1 + 2] += weight_dkappa_ * 2.0 * dkappa *
                            spiral_curve.DeriveDKappaDerivative(QuinticSpiralPath::DKAPPA1, ratio);

      grad_f[variable_offset + i] +=
          weight_dkappa_ * 2.0 * dkappa *
          spiral_curve.DeriveDKappaDerivative(QuinticSpiralPath::DELTA_S, ratio);

      double d2kappa = spiral_curve.Evaluate(3, ratio * delta_s);
      grad_f[index0] += weight_d2kappa_ * 2.0 * d2kappa *
                        spiral_curve.DeriveD2KappaDerivative(QuinticSpiralPath::THETA0, ratio);
      grad_f[index0 + 1] += weight_d2kappa_ * 2.0 * d2kappa *
                            spiral_curve.DeriveD2KappaDerivative(QuinticSpiralPath::KAPPA0, ratio);
      grad_f[index0 + 2] += weight_d2kappa_ * 2.0 * d2kappa *
                            spiral_curve.DeriveD2KappaDerivative(QuinticSpiralPath::DKAPPA0, ratio);

      grad_f[index1] += weight_d2kappa_ * 2.0 * d2kappa *
                        spiral_curve.DeriveD2KappaDerivative(QuinticSpiralPath::THETA1, ratio);
      grad_f[index1 + 1] += weight_d2kappa_ * 2.0 * d2kappa *
                            spiral_curve.DeriveD2KappaDerivative(QuinticSpiralPath::KAPPA1, ratio);
      grad_f[index1 + 2] += weight_d2kappa_ * 2.0 * d2kappa *
                            spiral_curve.DeriveD2KappaDerivative(QuinticSpiralPath::DKAPPA1, ratio);

      grad_f[variable_offset + i] +=
          weight_d2kappa_ * 2.0 * d2kappa *
          spiral_curve.DeriveD2KappaDerivative(QuinticSpiralPath::DELTA_S, ratio);
    }
  }
  return true;
}
...
bool SpiralProblemInterface::eval_g(int n, const double* x, bool new_x, int m, double* g) {
  ...
  for (std::size_t i = 0; i + 1 < num_of_points_; ++i) {
    ...
    double x_diff =
        x[index1 + 3] - x[index0 + 3] - spiral_curve.ComputeCartesianDeviationX<N>(delta_s);
    g[i * 2] = x_diff * x_diff;

    double y_diff =
        x[index1 + 4] - x[index0 + 4] - spiral_curve.ComputeCartesianDeviationY<N>(delta_s);
...
bool SpiralProblemInterface::eval_jac_g(
  ...
  if (values == NULL) {
    ...
    for (std::size_t i = 0; i + 1 < num_of_points_; ++i) {
      ...
      auto pos_theta0  = spiral_curve.DeriveCartesianDeviation<N>(QuinticSpiralPath::THETA0);
      auto pos_kappa0  = spiral_curve.DeriveCartesianDeviation<N>(QuinticSpiralPath::KAPPA0);
      auto pos_dkappa0 = spiral_curve.DeriveCartesianDeviation<N>(QuinticSpiralPath::DKAPPA0);

      auto pos_theta1  = spiral_curve.DeriveCartesianDeviation<N>(QuinticSpiralPath::THETA1);
      auto pos_kappa1  = spiral_curve.DeriveCartesianDeviation<N>(QuinticSpiralPath::KAPPA1);
      auto pos_dkappa1 = spiral_curve.DeriveCartesianDeviation<N>(QuinticSpiralPath::DKAPPA1);

      auto pos_delta_s = spiral_curve.DeriveCartesianDeviation<N>(QuinticSpiralPath::DELTA_S);
...
void SpiralProblemInterface::set_element_weight_d2kappa(const double weight_d2kappa) {
