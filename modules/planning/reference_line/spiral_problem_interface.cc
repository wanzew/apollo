...
namespace {
auto THETA0  = QuinticSpiralPathWithDerivation<SpiralProblemInterface::N>::THETA0;
auto KAPPA0  = QuinticSpiralPathWithDerivation<SpiralProblemInterface::N>::KAPPA0;
auto DKAPPA0 = QuinticSpiralPathWithDerivation<SpiralProblemInterface::N>::DKAPPA0;
auto THETA1  = QuinticSpiralPathWithDerivation<SpiralProblemInterface::N>::THETA1;
auto KAPPA1  = QuinticSpiralPathWithDerivation<SpiralProblemInterface::N>::KAPPA1;
auto DKAPPA1 = QuinticSpiralPathWithDerivation<SpiralProblemInterface::N>::DKAPPA1;
auto DELTA_S = QuinticSpiralPathWithDerivation<SpiralProblemInterface::N>::DELTA_S;
}  // namespace

SpiralProblemInterface::SpiralProblemInterface(std::vector<Eigen::Vector2d> points)
  ...
  for (int i = 1; i < num_of_points_; ++i) {
    double theta_diff = common::math::AngleDiff(relative_theta_.back(), normalized_theta[i]);
...
void SpiralProblemInterface::get_optimization_results(std::vector<double>* ptr_theta,
...
bool SpiralProblemInterface::get_nlp_info(
...
bool SpiralProblemInterface::get_bounds_info(
  ...
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    x_l[variable_offset + i] = point_distances_[i] - 2.0 * default_max_point_deviation_;
  ...
  for (int i = 0; i < num_of_points_; ++i) {
    ...
    g_u[constraint_offset + i] = default_max_point_deviation_ * default_max_point_deviation_;
  }
  return true;
}

bool SpiralProblemInterface::get_starting_point(int     n,
                                                bool    init_x,
                                                double* x,
                                                bool    init_z,
                                                double* z_L,
...
bool SpiralProblemInterface::eval_f(int n, const double* x, bool new_x, double& obj_value) {
  CHECK_EQ(n, num_of_variables_);
  if (new_x) { update_piecewise_spiral_paths(x, n); }
  ...
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    ...
    for (int j = 0; j < num_of_internal_points_; ++j) {
      double ratio = static_cast<double>(j) / static_cast<double>(num_of_internal_points_);
  ...
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    ...
    for (int j = 0; j < num_of_internal_points_; ++j) {
      ...
      grad_f[index0] += weight_kappa_ * 2.0 * kappa *
                        spiral_curve.DeriveKappaDerivative(THETA0, j, num_of_internal_points_);
      grad_f[index0 + 1] += weight_kappa_ * 2.0 * kappa *
                            spiral_curve.DeriveKappaDerivative(KAPPA0, j, num_of_internal_points_);
      grad_f[index0 + 2] += weight_kappa_ * 2.0 * kappa *
                            spiral_curve.DeriveKappaDerivative(DKAPPA0, j, num_of_internal_points_);

      grad_f[index1] += weight_kappa_ * 2.0 * kappa *
                        spiral_curve.DeriveKappaDerivative(THETA1, j, num_of_internal_points_);
      grad_f[index1 + 1] += weight_kappa_ * 2.0 * kappa *
                            spiral_curve.DeriveKappaDerivative(KAPPA1, j, num_of_internal_points_);
      grad_f[index1 + 2] += weight_kappa_ * 2.0 * kappa *
                            spiral_curve.DeriveKappaDerivative(DKAPPA1, j, num_of_internal_points_);

      grad_f[variable_offset + i] +=
          weight_kappa_ * 2.0 * kappa *
          spiral_curve.DeriveKappaDerivative(DELTA_S, j, num_of_internal_points_);

      double dkappa = spiral_curve.Evaluate(2, s);
      grad_f[index0] += weight_dkappa_ * 2.0 * dkappa *
                        spiral_curve.DeriveDKappaDerivative(THETA0, j, num_of_internal_points_);
      grad_f[index0 + 1] += weight_dkappa_ * 2.0 * dkappa *
                            spiral_curve.DeriveDKappaDerivative(KAPPA0, j, num_of_internal_points_);
      grad_f[index0 + 2] +=
          weight_dkappa_ * 2.0 * dkappa *
          spiral_curve.DeriveDKappaDerivative(DKAPPA0, j, num_of_internal_points_);

      grad_f[index1] += weight_dkappa_ * 2.0 * dkappa *
                        spiral_curve.DeriveDKappaDerivative(THETA1, j, num_of_internal_points_);
      grad_f[index1 + 1] += weight_dkappa_ * 2.0 * dkappa *
                            spiral_curve.DeriveDKappaDerivative(KAPPA1, j, num_of_internal_points_);
      grad_f[index1 + 2] +=
          weight_dkappa_ * 2.0 * dkappa *
          spiral_curve.DeriveDKappaDerivative(DKAPPA1, j, num_of_internal_points_);

      grad_f[variable_offset + i] +=
          weight_dkappa_ * 2.0 * dkappa *
          spiral_curve.DeriveDKappaDerivative(DELTA_S, j, num_of_internal_points_);
    }
  }
  return true;
}

bool SpiralProblemInterface::eval_g(int n, const double* x, bool new_x, int m, double* g) {
  CHECK_EQ(n, num_of_variables_);
  CHECK_EQ(m, num_of_constraints_);
  ...
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    ...
    double x_diff =
        x[index1 + 3] - x[index0 + 3] - spiral_curve.ComputeCartesianDeviationX(delta_s);
    g[i * 2] = x_diff * x_diff;

    double y_diff =
        x[index1 + 4] - x[index0 + 4] - spiral_curve.ComputeCartesianDeviationY(delta_s);
...
bool SpiralProblemInterface::eval_jac_g(
...
bool SpiralProblemInterface::eval_h(int           n,
                                    const double* x,
                                    bool          new_x,
                                    double        obj_factor,
                                    int           m,
...
void SpiralProblemInterface::finalize_solution(Ipopt::SolverReturn               status,
                                               int                               n,
                                               const double*                     x,
                                               const double*                     z_L,
                                               const double*                     z_U,
...
void SpiralProblemInterface::update_piecewise_spiral_paths(const double* x, const int n) {
  int variable_offset = num_of_points_ * 5;
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    int index0 = i * 5;
    int index1 = (i + 1) * 5;
    ...
    piecewise_paths_[i]           = std::move(QuinticSpiralPathWithDerivation<N>(x0, x1, delta_s));
  }
}
...
void SpiralProblemInterface::set_start_point(
...
void SpiralProblemInterface::set_end_point(
    const double x, const double y, const double theta, const double kappa, const double dkappa) {
...
void SpiralProblemInterface::set_end_point_position(const double x, const double y) {
  has_fixed_end_point_position_ = true;
  end_x_                        = x;
  end_y_                        = y;
}

void SpiralProblemInterface::set_element_weight_curve_length(const double weight_curve_length) {
...
void SpiralProblemInterface::set_element_weight_dkappa(const double weight_dkappa) {
