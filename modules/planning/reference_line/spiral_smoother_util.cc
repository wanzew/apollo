...
DEFINE_double(minimum_point_spacing, 5.0, "The minimum distance for input points.");
...
class SpiralSmootherUtil {
 public:
  static std::vector<Eigen::Vector2d> ReadCoordinatesFromFile(const std::string& filename) {
    std::vector<Eigen::Vector2d> raw_points;
    std::ifstream                ifs(filename.c_str(), std::ifstream::in);
    std::string                  point_str;

    auto spacing_thres = FLAGS_minimum_point_spacing * FLAGS_minimum_point_spacing;
    ...
    while (std::getline(ifs, point_str)) {
      ...
      if (raw_points.size() > 0) {
        ...
        auto dx = x - last_x;
        auto dy = y - last_y;
        if (dx * dx + dy * dy < spacing_thres) { continue; }
      }
  ...
  static bool Smooth(std::vector<Eigen::Vector2d>    raw_points,
    ...
    for (const Eigen::Vector2d& p : raw_points) {
      ...
      if (d.norm() < FLAGS_minimum_point_spacing) { continue; }
    ...
    ACHECK(cyber::common::GetProtoFromFile("modules/planning/conf/spiral_smoother_config.pb.txt",
                                           &config));
  ...
  static void Export(const std::string&                    filename,
    ...
    for (size_t i = 1; i + 1 < smoothed_points.size(); ++i) {
      ...
      ofs << std::fixed << "{\"kappa\": " << point.kappa() << ", \"s\": " << point.s()
          << ", \"theta\": " << point.theta() << ", \"x\":" << point.x() << ", \"y\":" << point.y()
          << ", \"dkappa\":" << point.dkappa() << "}";
    }
    ofs.close();
...
int main(int argc, char* argv[]) {
  ...
  std::vector<Eigen::Vector2d> raw_points =
      apollo::planning::SpiralSmootherUtil::ReadCoordinatesFromFile(FLAGS_input_file);

  std::vector<apollo::common::PathPoint> smooth_points;
  auto res = apollo::planning::SpiralSmootherUtil::Smooth(raw_points, &smooth_points);
  if (!res) { AERROR << "Failed to smooth a the line"; }

  if (FLAGS_output_file.empty()) {
    FLAGS_output_file = FLAGS_input_file + ".smoothed";
    AINFO << "Output file not provided, set to: " << FLAGS_output_file;
  }
  apollo::planning::SpiralSmootherUtil::Export(FLAGS_output_file, smooth_points);
  return 0;
}
