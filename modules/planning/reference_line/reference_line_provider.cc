...
ReferenceLineProvider::ReferenceLineProvider(
  ...
  ACHECK(cyber::common::GetProtoFromFile(FLAGS_smoother_config_filename, &smoother_config_))
...
void ReferenceLineProvider::Wait() {
  is_stop_ = true;
  if (FLAGS_enable_reference_line_provider_thread) { task_future_.wait(); }
}

void ReferenceLineProvider::Stop() {
  is_stop_ = true;
  if (FLAGS_enable_reference_line_provider_thread) { task_future_.get(); }
}
...
void ReferenceLineProvider::UpdateReferenceLine(
  ...
  if (reference_lines_.size() != reference_lines.size()) {
    ...
    for (auto iter = reference_lines.begin();
         iter != reference_lines.end() && segment_iter != route_segments.end() &&
...
bool ReferenceLineProvider::GetReferenceLinesFromRelativeMap(
    std::list<ReferenceLine>* reference_lines, std::list<hdmap::RouteSegments>* segments) {
  ...
  ADEBUG << "relative_map_->navigation_path_size = " << relative_map_->navigation_path_size();
  for (const auto& path_pair : relative_map_->navigation_path()) {
    const auto     lane_id  = path_pair.first;
  ...
  for (const auto& path_pair : relative_map_->navigation_path()) {
    ...
    if (is_lane_change_needed) {
      if (lane_id == nearest_neighbor_lane_id) {
        ADEBUG << "adc lane_id = " << adc_lane_id << " nearest_neighbor_lane_id = " << lane_id;
    ...
    for (auto iter = segments->begin(); iter != segments->end();) {
      ...
      if (!SmoothRouteSegment(*iter, &reference_lines->back())) {
        ...
        if (!reference_lines->back().XYToSL(vehicle_state, &sl)) {
          AWARN << "Failed to project point: {" << vehicle_state.x() << "," << vehicle_state.y()
...
bool ReferenceLineProvider::Shrink(const common::SLPoint& sl,
  ...
  if (sl.s() > FLAGS_look_backward_distance * 1.5) {
    ADEBUG << "reference line back side is " << sl.s()
           << ", shrink reference line: origin length: " << reference_line->Length();
  ...
  while (last_index < ref_points.size() &&
         AngleDiff(cur_heading, ref_points[last_index].heading()) < kMaxHeadingDiff) {
    ++last_index;
  }
  --last_index;
...
AnchorPoint ReferenceLineProvider::GetAnchorPoint(const ReferenceLine& reference_line,
  ...
  if (is_lane_width_safe) { effective_width = 0.5 * safe_lane_width; }
...
bool ReferenceLineProvider::SmoothPrefixedReferenceLine(const ReferenceLine& prefix_ref,
  ...
  for (auto& point : anchor_points) {
    common::SLPoint sl_point;
    if (!prefix_ref.XYToSL(point.path_point, &sl_point)) { continue; }
