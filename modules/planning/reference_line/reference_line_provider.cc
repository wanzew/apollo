...
ReferenceLineProvider::ReferenceLineProvider(const hdmap::HDMap* base_map) {
  if (!FLAGS_use_navigation_mode) { pnc_map_.reset(new hdmap::PncMap(base_map)); }
  ...
  if (smoother_config_.has_qp_spline()) {
    ...
    CHECK(false) << "unknown smoother config " << smoother_config_.DebugString();
  }
...
bool ReferenceLineProvider::Start() {
  if (FLAGS_use_navigation_mode) { return true; }
  if (!is_initialized_) {
...
void ReferenceLineProvider::Stop() {
  is_stop_ = true;
  if (FLAGS_enable_reference_line_provider_thread && thread_ && thread_->joinable()) {
    thread_->join();
  }
}
...
void ReferenceLineProvider::UpdateReferenceLine(
  ...
  if (reference_lines.size() != route_segments.size()) {
    AERROR << "The calculated reference line size(" << reference_lines.size()
           << ") and route_segments size(" << route_segments.size() << ") are different";
    return;
  }
...
void ReferenceLineProvider::GenerateThread() {
  ...
  while (!is_stop_) {
    ...
    std::this_thread::sleep_for(std::chrono::duration<double, std::milli>(kSleepTime));
...
double ReferenceLineProvider::LastTimeDelay() {
  if (FLAGS_enable_reference_line_provider_thread && !FLAGS_use_navigation_mode) {
    std::lock_guard<std::mutex> lock(reference_lines_mutex_);
...
bool ReferenceLineProvider::GetReferenceLines(std::list<ReferenceLine>*        reference_lines,
  ...
  if (FLAGS_use_navigation_mode) {
    ...
    bool   result     = GetReferenceLinesFromRelativeMap(
        AdapterManager::GetRelativeMap()->GetLatestObserved(), reference_lines, segments);
    if (!result) { AERROR << "Failed to get reference line from relative map"; }
    double end_time        = Clock::NowInSeconds();
    last_calculation_time_ = end_time - start_time;
    return result;
  }
  ...
  if (FLAGS_enable_reference_line_provider_thread) {
    ...
    if (!reference_lines_.empty()) {
      ...
      if (reference_line_history_.empty()) { return false; }
...
bool ReferenceLineProvider::GetReferenceLinesFromRelativeMap(
  ...
  for (const auto& path_pair : relative_map.navigation_path()) {
    ...
    ADEBUG << "lane_id = " << lane_id << " priority = " << priority
           << " adc_lane_id = " << adc_lane_id << " adc_lane_priority = " << adc_lane_priority;
  ...
  if (!high_priority_lane_pairs.empty()) {
    std::sort(
        high_priority_lane_pairs.begin(), high_priority_lane_pairs.end(),
        [](const LaneIdPair& left, const LaneIdPair& right) { return left.second < right.second; });
  ...
  if (is_lane_change_needed) {
    ...
    if (left_neighbor_lane_ids.end() != std::find(left_neighbor_lane_ids.begin(),
                                                  left_neighbor_lane_ids.end(),
    ...
    } else if (right_neighbor_lane_ids.end() != std::find(right_neighbor_lane_ids.begin(),
      ...
      // the neareast_neighbor_lane_id
      neareast_neighbor_lane_id =
          adc_lane_way_point.lane->lane().right_neighbor_forward_lane_id(0).id();
    }
  }
  ...
  for (const auto& path_pair : relative_map.navigation_path()) {
    ...
    if (is_lane_change_needed) {
      if (lane_id == neareast_neighbor_lane_id) {
        ADEBUG << "adc lane_id = " << adc_lane_id << " neareast_neighbor_lane_id = " << lane_id;
    ...
    for (const auto& path_point : path_points) {
      ref_points.emplace_back(MapPathPoint{Vec2d{path_point.x(), path_point.y()},
                                           path_point.theta(),
...
bool ReferenceLineProvider::GetNearestWayPointFromNavigationPath(
  ...
  const int status =
      hdmap->GetLanesWithHeading(point, kMaxDistance, state.heading(), M_PI / 2.0, &lanes);
  if (status < 0) {
  ...
  if (valid_lanes.empty()) {
    AERROR << "no valid lane found within " << kMaxDistance << " meters with heading "
           << state.heading();
    return false;
  }
...
bool ReferenceLineProvider::CreateRouteSegments(const common::VehicleState& vehicle_state,
  ...
  {
    std::lock_guard<std::mutex> lock(pnc_map_mutex_);
    if (!pnc_map_->GetRouteSegments(vehicle_state, look_backward_distance, look_forward_distance,
                                    segments)) {
  ...
  if (FLAGS_prioritize_change_lane) { PrioritzeChangeLane(segments); }
  return !segments->empty();
}
...
bool ReferenceLineProvider::CreateReferenceLine(std::list<ReferenceLine>*        reference_lines,
  ...
  if (!CreateRouteSegments(vehicle_state, look_backward_distance, look_forward_distance,
                           segments)) {
...
bool ReferenceLineProvider::ExtendReferenceLine(const VehicleState& state,
  ...
  while (prev_segment != route_segments_.end()) {
    if (prev_segment->IsConnectedSegment(*segments)) { break; }
    ++prev_segment;
    ++prev_ref;
  }
  ...
  if (remain_s > look_forward_required_distance) {
    ...
    *reference_line = *prev_ref;
    ADEBUG << "Reference line remain " << remain_s << ", which is more than required "
  ...
  *segments = shifted_segments;
  ...
  if (sl.s() > FLAGS_look_backward_distance * 1.5) {
    ADEBUG << "reference line back side is " << sl.s()
           << ", shrink reference line: origin lenght: " << reference_line->Length();
...
AnchorPoint ReferenceLineProvider::GetAnchorPoint(const ReferenceLine& reference_line,
  ...
  if (total_width > adc_width * smoother_config_.wide_lane_threshold_factor()) {
    if (smoother_config_.driving_side() == ReferenceLineSmootherConfig::RIGHT) {
      shifted_left_width =
          adc_half_width + adc_width * smoother_config_.wide_lane_shift_remain_factor();
    } else {
      shifted_left_width =
          std::fmax(adc_half_width,
                    total_width - (adc_half_width +
  ...
  anchor.path_point        = ref_point.ToPathPoint(s);
  double effective_width   = std::min(shifted_left_width, shifted_right_width) - adc_half_width -
...
bool ReferenceLineProvider::SmoothPrefixedReferenceLine(const ReferenceLine& prefix_ref,
  ...
  for (auto& point : anchor_points) {
    ...
    if (sl_point.s() < 0 || sl_point.s() > prefix_ref.Length()) { continue; }
