...
void NaviPlanning::RunOnce(const LocalView& local_view, ADCTrajectory* const trajectory_pb) {
  ...
  if (last_vehicle_config_.is_valid_ && vehicle_config.is_valid_) {
    ...
    TrajectoryStitcher::TransformLastPublishedTrajectory(x_diff_veh, y_diff_veh, theta_diff,
  ...
  if (start_timestamp - vehicle_state.timestamp() < FLAGS_message_latency_threshold) {
    auto future_xy = injector_->vehicle_state()->EstimateFuturePosition(start_timestamp -
  ...
  if (FLAGS_enable_record_debug) { frame_->RecordInputDebug(trajectory_pb->mutable_debug()); }
  trajectory_pb->mutable_latency_stats()->set_init_frame_time_ms(Clock::NowInSeconds() -
                                                                 start_timestamp);
  if (!status.ok()) {
    ...
    if (FLAGS_publish_estop) {
      ...
      trajectory_pb->mutable_decision()->mutable_main_decision()->mutable_not_ready()->set_reason(
          status.ToString());
      status.Save(trajectory_pb->mutable_header()->mutable_status());
      // TODO(all): integrate reverse gear
      trajectory_pb->set_gear(canbus::Chassis::GEAR_DRIVE);
    ...
    if (drvie_action != DrivingAction::NONE) {
      ...
      switch (drvie_action) {
        case DrivingAction::FOLLOW: {
          ...
          if (!current_lane_id.empty()) { target_lane_id_ = current_lane_id; }
          break;
        }
        case DrivingAction::CHANGE_LEFT: {
          ...
          if (!lane_info_group.empty()) { target_lane_id_ = lane_info_group.front().first; }
          break;
        }
...
std::string NaviPlanning::GetCurrentLaneId() {
  ...
  for (auto& ref_line_info : ref_line_info_group) {
    ...
    if (ref_line.IsOnLane(adc_position)) { current_lane_id = lane_id; }
  }
...
void NaviPlanning::GetLeftNeighborLanesInfo(
  ...
  for (auto& ref_line_info : ref_line_info_group) {
    common::math::Vec2d adc_position(vehicle_state.x(), vehicle_state.y());
    auto&               ref_line = ref_line_info.reference_line();
    if (ref_line.IsOnLane(adc_position)) { continue; }
    auto   lane_id   = ref_line_info.Lanes().Id();
    auto   ref_point = ref_line.GetReferencePoint(vehicle_state.x(), vehicle_state.y());
    double y         = ref_point.y();
    // in FLU positive on the left
    if (y > 0.0) { lane_info_group->emplace_back(lane_id, y); }
  }
...
Status NaviPlanning::Plan(const double                        current_time_stamp,
  ...
  auto status = planner_->Plan(stitching_trajectory.back(), frame_.get(), trajectory_pb);
  ...
  best_ref_info->ExportDecision(trajectory_pb->mutable_decision(), injector_->planning_context());
  ...
  **/
  ...
  for (size_t i = 0; i < last_publishable_trajectory_->NumOfPoints(); ++i) {
    ...
    ADEBUG << last_publishable_trajectory_->TrajectoryPointAt(i).ShortDebugString();
  }
...
NaviPlanning::VehicleConfig NaviPlanning::ComputeVehicleConfigFromLocalization(
  ...
  if (!localization.pose().has_position()) { return vehicle_config; }
  ...
  if (localization.pose().has_heading()) {
    ...
    vehicle_config.theta_ = common::math::QuaternionToHeading(orientation.qw(), orientation.qx(),
...
bool NaviPlanning::CheckPlanningConfig(const PlanningConfig& config) {
  if (!config.has_navigation_planning_config()) { return false; }
