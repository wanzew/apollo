...
void PlanningTestBase::SetUpTestCase() {
  ...
  FLAGS_traffic_rule_config_filename = "/apollo/modules/planning/conf/traffic_rule_config.pb.txt";
  FLAGS_smoother_config_filename = "/apollo/modules/planning/conf/qp_spline_smoother_config.pb.txt";
  FLAGS_map_dir                  = "/apollo/modules/planning/testdata";
  FLAGS_test_localization_file   = "";
  FLAGS_test_chassis_file        = "";
  FLAGS_test_routing_response_file  = "";
  FLAGS_test_planning_config_file   = "/apollo/modules/planning/conf/planning_config.pb.txt";
...
bool PlanningTestBase::FeedTestData() {
  ...
  if (!apollo::cyber::common::GetProtoFromFile(FLAGS_test_data_dir + "/" + FLAGS_test_chassis_file,
                                               &chassis)) {
  ...
  if (!apollo::cyber::common::GetProtoFromFile(
          FLAGS_test_data_dir + "/" + FLAGS_test_localization_file, &localization)) {
  ...
  if (!apollo::cyber::common::GetProtoFromFile(
          FLAGS_test_data_dir + "/" + FLAGS_test_prediction_file, &prediction)) {
  ...
  if (!apollo::cyber::common::GetProtoFromFile(
          FLAGS_test_data_dir + "/" + FLAGS_test_routing_response_file, &routing_response)) {
  ...
  if (!apollo::cyber::common::GetProtoFromFile(
          FLAGS_test_data_dir + "/" + FLAGS_test_traffic_light_file, &traffic_light_detection)) {
    AERROR << "failed to load file: " << FLAGS_test_traffic_light_file;
    return false;
  }

  local_view_.prediction_obstacles  = std::make_shared<PredictionObstacles>(prediction);
  local_view_.chassis               = std::make_shared<Chassis>(chassis);
  local_view_.localization_estimate = std::make_shared<LocalizationEstimate>(localization);
  local_view_.routing               = std::make_shared<routing::RoutingResponse>(routing_response);
  local_view_.traffic_light = std::make_shared<TrafficLightDetection>(traffic_light_detection);
...
void PlanningTestBase::SetUp() {
  ...
  ACHECK(cyber::common::GetProtoFromFile(FLAGS_test_planning_config_file, &config_))
      << "failed to load planning config file " << FLAGS_test_planning_config_file;

  ACHECK(planning_->Init(config_).ok()) << "Failed to init planning module";

  if (!FLAGS_test_previous_planning_file.empty()) {
    const auto prev_planning_file = FLAGS_test_data_dir + "/" + FLAGS_test_previous_planning_file;
    ADCTrajectory prev_planning;
    ACHECK(cyber::common::GetProtoFromFile(prev_planning_file, &prev_planning));
    planning_->last_publishable_trajectory_.reset(new PublishableTrajectory(prev_planning));
  }
  for (auto& config : *(planning_->traffic_rule_configs_.mutable_config())) {
    auto iter = rule_enabled_.find(config.rule_id());
    if (iter != rule_enabled_.end()) { config.set_enabled(iter->second); }
  }
}
...
void PlanningTestBase::TrimPlanning(ADCTrajectory* origin, bool no_trajectory_point) {
...
bool PlanningTestBase::RunPlanning(const std::string& test_case_name,
  ...
  if (FLAGS_test_update_golden_log) {
    ...
    if (!load_success || !common::util::IsProtoEqual(golden_result, adc_trajectory_)) {
...
bool PlanningTestBase::IsValidTrajectory(const ADCTrajectory& trajectory) {
  for (int i = 0; i < trajectory.trajectory_point_size(); ++i) {
    ...
    const double kMaxAccelThreshold = FLAGS_longitudinal_acceleration_upper_bound;
    ...
    if (point.a() > kMaxAccelThreshold || point.a() < kMinAccelThreshold) {
      AERROR << "Invalid trajectory point because accel out of range: " << point.DebugString();
      return false;
    }
    ...
    if (i > 0) {
      ...
      if (point.path_point().s() + kPathSEpsilon < last_point.path_point().s()) {
        AERROR << "Invalid trajectory point because s value error. last point: "
               << last_point.DebugString() << ", curr point: " << point.DebugString();
        return false;
      }
    }
  }
...
TrafficRuleConfig*
PlanningTestBase::GetTrafficRuleConfig(const TrafficRuleConfig::RuleId& rule_id) {
  for (auto& config : *planning_->traffic_rule_configs_.mutable_config()) {
    if (config.rule_id() == rule_id) { return &config; }
  }
  return nullptr;
}
