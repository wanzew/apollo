...
class BackupTrajectoryGenerator {
 public:
  ...
  BackupTrajectoryGenerator(const std::array<double, 3>&             init_s,
  ...
  GenerateTrajectory(const std::vector<common::PathPoint>& discretized_ref_points);
 ...
 private:
  ...
  struct CostComparator
      : public std::binary_function<const Trajectory1dPair&, const Trajectory1dPair&, bool> {
    bool operator()(const Trajectory1dPair& left, const Trajectory1dPair& right) const {
      ...
      if (s_dot_left < s_dot_right) { return true; }
      return false;
    }
  };

  std::priority_queue<Trajectory1dPair, std::vector<Trajectory1dPair>, CostComparator>
