...
class TrajectoryEvaluator {
 ...
 public:
  ...
  double EvaluateDiscreteTrajectory(const PlanningTarget&                          planning_target,
 ...
 private:
  ...
  double LatOffsetCost(const std::vector<apollo::common::FrenetFramePoint> sl_points) const;

  double LatComfortCost(const std::shared_ptr<Curve1d>& lon_trajectory,
                        const std::shared_ptr<Curve1d>& lat_trajectory) const;

  double LatComfortCost(const std::vector<apollo::common::FrenetFramePoint>& sl_points) const;

  double LonComfortCost(const std::shared_ptr<Curve1d>& lon_trajectory) const;

  double LonComfortCost(const std::vector<apollo::common::SpeedPoint>& st_points) const;

  double LonCollisionCost(const std::shared_ptr<Curve1d>& lon_trajectory) const;

  double LonCollisionCost(const std::vector<apollo::common::SpeedPoint>& st_points) const;

  double LonObjectiveCost(const std::shared_ptr<Curve1d>& lon_trajectory,
                          const PlanningTarget&           planning_target,
                          const std::vector<double>&      ref_s_dot) const;

  double LonObjectiveCost(const std::vector<apollo::common::SpeedPoint>& st_points,
                          const PlanningTarget&                          planning_target,
  ...
  double
  CentripetalAccelerationCost(const std::vector<apollo::common::SpeedPoint>& st_points) const;

  std::vector<double> ComputeLongitudinalGuideVelocity(const PlanningTarget& planning_target) const;

  bool InterpolateDenseStPoints(const std::vector<apollo::common::SpeedPoint>& st_points,
                                double                                         t,
                                double*                                        traj_s) const;
  ...
  struct CostComponentComparator : public std::binary_function<const PairCostWithComponents&,
                                                               const PairCostWithComponents&,
                                                               bool> {
