...
DiscretizedTrajectory TrajectoryCombiner::Combine(const std::vector<PathPoint>& reference_line,
  ...
  while (t_param < FLAGS_trajectory_time_length) {
    ...
    double s_dot  = std::max(FLAGS_lattice_epsilon, lon_trajectory.Evaluate(1, t_param));
    ...
    CartesianFrenetConverter::frenet_to_cartesian(rs, rx, ry, rtheta, rkappa, rdkappa, s_conditions,
                                                  d_conditions, &x, &y, &theta, &kappa, &v, &a);
