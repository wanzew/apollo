...
TrajectoryEvaluator::TrajectoryEvaluator(const std::array<double, 3>&            init_s,
  ...
  if (planning_target.has_stop_point()) { stop_point = planning_target.stop_point().s(); }
  for (const auto& lon_trajectory : lon_trajectories) {
    double lon_end_s = lon_trajectory->Evaluate(0, end_time);
    if (init_s[0] < stop_point && lon_end_s + FLAGS_lattice_stop_buffer > stop_point) { continue; }

    if (!ConstraintChecker1d::IsValidLongitudinalTrajectory(*lon_trajectory)) { continue; }
    for (const auto& lat_trajectory : lat_trajectories) {
      ...
      */
      if (!FLAGS_enable_auto_tuning) {
        ...
        double cost = Evaluate(planning_target, lon_trajectory, lat_trajectory, &cost_components);
  ...
  if (!FLAGS_enable_auto_tuning) {
    ...
    ADEBUG << "Number of valid 1d trajectory pairs: " << cost_queue_with_components_.size();
  }
}
...
double TrajectoryEvaluator::Evaluate(const PlanningTarget&  planning_target,
  ...
  double evaluation_horizon =
      std::min(FLAGS_decision_horizon, lon_trajectory->Evaluate(0, lon_trajectory->ParamLength()));
  ...
  return lon_objective_cost * FLAGS_weight_lon_objective + lon_jerk_cost * FLAGS_weight_lon_jerk +
         lon_collision_cost * FLAGS_weight_lon_collision +
         centripetal_acc_cost * FLAGS_weight_centripetal_acceleration +
         lat_offset_cost * FLAGS_weight_lat_offset + lat_comfort_cost * FLAGS_weight_lat_comfort;
}

double
TrajectoryEvaluator::EvaluateDiscreteTrajectory(const PlanningTarget&          planning_target,
  ...
  double lon_objective_cost   = LonObjectiveCost(st_points, planning_target, reference_s_dot_);
...
double TrajectoryEvaluator::LatOffsetCost(const std::vector<FrenetFramePoint> sl_points) const {
  if (sl_points.size() == 0) { return 0.0; }
...
double TrajectoryEvaluator::LatComfortCost(const PtrTrajectory1d& lon_trajectory,
                                           const PtrTrajectory1d& lat_trajectory) const {
  double max_cost = 0.0;
  for (double t = 0.0; t < FLAGS_trajectory_time_length; t += FLAGS_trajectory_time_resolution) {
...
double TrajectoryEvaluator::LatComfortCost(const std::vector<FrenetFramePoint>& sl_points) const {
  if (sl_points.size() < 2) { return 0.0; }
  ...
  for (std::size_t i = 1; i + 1 < sl_points.size(); ++i) {
    ...
    double s_dotdot =
        (sl_points[i + 1].s() - 2.0 * sl_points[i].s() + sl_points[i - 1].s()) / (dt * dt);
...
double TrajectoryEvaluator::LonComfortCost(const std::vector<SpeedPoint>& st_points) const {
  if (st_points.size() < 2) { return 0.0; }
  double cost_sqr_sum = 0.0;
  double cost_abs_sum = 0.0;
  for (std::size_t i = 0; i < st_points.size() - 1; ++i) {
    ...
    if (std::fabs(t1 - t2) <= FLAGS_lattice_epsilon) { continue; }
...
double TrajectoryEvaluator::LonObjectiveCost(const PtrTrajectory1d&     lon_trajectory,
  ...
  double speed_cost          = speed_cost_sqr_sum / (speed_cost_weight_sum + FLAGS_lattice_epsilon);
...
double TrajectoryEvaluator::LonObjectiveCost(const std::vector<SpeedPoint>& st_points,
                                             const PlanningTarget&          planning_target,
                                             const std::vector<double>&     ref_s_dots) const {
  if (st_points.size() < 1) { return 0.0; }
  ...
  for (std::size_t i = 0; i < ref_s_dots.size(); ++i) {
    ...
    while (point_idx + 1 < st_points.size()) {
      if (st_points[point_idx + 1].t() <= t) { ++point_idx; }
    }
...
double TrajectoryEvaluator::LonCollisionCost(const PtrTrajectory1d& lon_trajectory) const {
  double cost_sqr_sum = 0.0;
  double cost_abs_sum = 0.0;
  for (std::size_t i = 0; i < path_time_intervals_.size(); ++i) {
    const auto& pt_interval = path_time_intervals_[i];
    if (pt_interval.empty()) { continue; }
    double t      = i * FLAGS_trajectory_time_resolution;
...
double
TrajectoryEvaluator::LonCollisionCost(const std::vector<common::SpeedPoint>& st_points) const {
  double cost_sqr_sum = 0.0;
  double cost_abs_sum = 0.0;
  for (std::size_t i = 0; i < path_time_intervals_.size(); ++i) {
    const auto& pt_interval = path_time_intervals_[i];
...
double
TrajectoryEvaluator::CentripetalAccelerationCost(const PtrTrajectory1d& lon_trajectory) const {
  ...
  return centripetal_acc_sqr_sum / (centripetal_acc_sum + FLAGS_lattice_epsilon);
}

double
TrajectoryEvaluator::CentripetalAccelerationCost(const std::vector<SpeedPoint>& st_points) const {
  double centripetal_acc_sum     = 0.0;
  double centripetal_acc_sqr_sum = 0.0;
...
std::vector<double>
TrajectoryEvaluator::ComputeLongitudinalGuideVelocity(const PlanningTarget& planning_target) const {
  ...
  if (!planning_target.has_stop_point()) {
    ...
    lon_traj.AppendSegment(0.0, FLAGS_trajectory_time_length + +FLAGS_lattice_epsilon);
    ...
    double a_comfort =
        FLAGS_longitudinal_acceleration_upper_bound * FLAGS_comfort_acceleration_factor;
    double d_comfort =
        -FLAGS_longitudinal_acceleration_lower_bound * FLAGS_comfort_acceleration_factor;

    std::shared_ptr<Trajectory1d> lon_ref_trajectory =
        PiecewiseBrakingTrajectoryGenerator::Generate(
            planning_target.stop_point().s(), init_s_[0], planning_target.cruise_speed(),
            init_s_[1], a_comfort, d_comfort, FLAGS_trajectory_time_length + FLAGS_lattice_epsilon);
...
bool TrajectoryEvaluator::InterpolateDenseStPoints(const std::vector<SpeedPoint>& st_points,
                                                   double                         t,
                                                   double*                        traj_s) const {
