...
bool TrajectoryEvaluator::has_more_trajectory_pairs() const { return !cost_queue_.empty(); }

size_t TrajectoryEvaluator::num_of_trajectory_pairs() const { return cost_queue_.size(); }
...
double TrajectoryEvaluator::top_trajectory_pair_cost() const { return cost_queue_.top().second; }
...
double TrajectoryEvaluator::Evaluate(const PlanningTarget&  planning_target,
  ...
  double              evaluation_horizon = std::min(FLAGS_speed_lon_decision_horizon,
...
double TrajectoryEvaluator::LonObjectiveCost(const PtrTrajectory1d&     lon_trajectory,
  ...
  double speed_cost = speed_cost_sqr_sum / (speed_cost_weight_sum + FLAGS_numerical_epsilon);
