...
void BackupTrajectoryGenerator::GenerateTrajectory1dPairs(const State& init_s,
  ...
  for (const auto dds : dds_condidates) {
    lon_trajectories.emplace_back(new ConstantDecelerationTrajectory1d(init_s[0], init_s[1], dds));
  }

  std::vector<std::shared_ptr<Curve1d>> lat_trajectories;
  ptr_trajectory1d_generator_->GenerateLateralTrajectoryBundle(&lat_trajectories);
...
DiscretizedTrajectory BackupTrajectoryGenerator::GenerateTrajectory(
  ...
  while (trajectory_pair_pqueue_.size() > 1) {
    ...
    DiscretizedTrajectory trajectory = TrajectoryCombiner::Combine(
        discretized_ref_points, *top_pair.first, *top_pair.second, init_relative_time_);
    if (!ptr_collision_checker_->InCollision(trajectory)) { return trajectory; }
  }
  auto top_pair = trajectory_pair_pqueue_.top();
  return TrajectoryCombiner::Combine(discretized_ref_points, *top_pair.first, *top_pair.second,
                                     init_relative_time_);
}
