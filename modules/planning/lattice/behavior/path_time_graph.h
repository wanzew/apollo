...
class PathTimeGraph {
 public:
  ...
  bool GetPathTimeObstacle(const std::string& obstacle_id, PathTimeObstacle* path_time_obstacle);

  std::vector<std::pair<double, double>> GetPathBlockingIntervals(const double t) const;

  std::vector<std::vector<std::pair<double, double>>>
  GetPathBlockingIntervals(const double t_start, const double t_end, const double t_resolution);

  std::pair<double, double> get_path_range() const;

  std::pair<double, double> get_time_range() const;

  std::vector<PathTimePoint> GetObstacleSurroundingPoints(const std::string& obstacle_id,
                                                          const double       s_dist,
  ...
  GetLateralBounds(const double s_start, const double s_end, const double s_resolution);
 ...
 private:
  ...
  SLBoundary
  ComputeObstacleBoundary(const std::vector<common::math::Vec2d>& vertices,
                          const std::vector<common::PathPoint>&   discretized_ref_points) const;

  PathTimePoint
  SetPathTimePoint(const std::string& obstacle_id, const double s, const double t) const;
  ...
  void UpdateLateralBoundsByObstacle(const SLBoundary&                             sl_boundary,
