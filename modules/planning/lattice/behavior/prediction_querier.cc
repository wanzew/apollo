...
std::vector<const Obstacle*> PredictionQuerier::GetObstacles() const { return obstacles_; }
...
double PredictionQuerier::ProjectVelocityAlongReferenceLine(const std::string& obstacle_id,
  ...
  if (num_traj_point < 2) { return 0.0; }
  ...
  auto matched_it = std::lower_bound(
      trajectory.trajectory_point().begin(), trajectory.trajectory_point().end(), t,
      [](const common::TrajectoryPoint& p, const double t) { return p.relative_time() < t; });
