...
double PiecewiseTrajectory1d::Evaluate(const std::uint32_t order, const double param) const {
  auto it_lower =
      std::lower_bound(accumulated_param_lengths_.begin(), accumulated_param_lengths_.end(), param);
...
void PiecewiseTrajectory1d::PopSegment() {
  if (trajectory_segments_.empty()) { return; }
  trajectory_segments_.pop_back();
  accumulated_param_lengths_.pop_back();
}

std::size_t PiecewiseTrajectory1d::NumOfSegments() const { return trajectory_segments_.size(); }
