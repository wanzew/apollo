...
PiecewiseLinearGenerator::PiecewiseLinearGenerator(const uint32_t num_of_segments,
...
PiecewiseLinearConstraint* PiecewiseLinearGenerator::mutable_constraint() { return &constraint_; }

PiecewiseLinearKernel* PiecewiseLinearGenerator::mutable_kernel() { return &kernel_; }
...
bool PiecewiseLinearGenerator::Solve() {
  ...
  const Eigen::MatrixXd& inequality_constraint_matrix = constraint_.inequality_constraint_matrix();
  const Eigen::MatrixXd& inequality_constraint_boundary =
      constraint_.inequality_constraint_boundary();

  const Eigen::MatrixXd& equality_constraint_matrix   = constraint_.equality_constraint_matrix();
  const Eigen::MatrixXd& equality_constraint_boundary = constraint_.equality_constraint_boundary();

  qp_solver_.reset(new apollo::common::math::ActiveSetQpSolver(
      kernel_matrix, offset, inequality_constraint_matrix, inequality_constraint_boundary,
      equality_constraint_matrix, equality_constraint_boundary));

  qp_solver_->EnableCholeskyRefactorisation(1);
  qp_solver_->set_pos_definite_hessian();

  if (!qp_solver_->Solve()) { return false; }
