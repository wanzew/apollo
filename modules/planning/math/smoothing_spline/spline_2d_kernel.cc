...
const Eigen::MatrixXd Spline2dKernel::kernel_matrix() const { return kernel_matrix_ * 2.0; }
...
void Spline2dKernel::AddNthDerivativeKernelMatrix(const uint32_t n, const double weight) {
  for (uint32_t i = 0; i + 1 < t_knots_.size(); ++i) {
    ...
    Eigen::MatrixXd cur_kernel = SplineSegKernel::instance()->NthDerivativeKernel(
                                     n, num_params, t_knots_[i + 1] - t_knots_[i]) *
                                 weight;
    kernel_matrix_.block(2 * i * num_params, 2 * i * num_params, num_params, num_params) +=
        cur_kernel;
    kernel_matrix_.block((2 * i + 1) * num_params, (2 * i + 1) * num_params, num_params,
...
bool Spline2dKernel::AddReferenceLineKernelMatrix(const std::vector<double>& t_coord,
                                                  ...
                                                  const double               weight) {
  if (ref_points.size() != t_coord.size()) { return false; }
  ...
  for (uint32_t i = 0; i < t_coord.size(); ++i) {
    ...
    kernel_matrix_.block((2 * cur_index) * num_params, (2 * cur_index) * num_params, num_params,
                         num_params) += weight * ref_kernel;
    kernel_matrix_.block((2 * cur_index + 1) * num_params, (2 * cur_index + 1) * num_params,
