 ...
 **/
bool Spline2dConstraint::Add2dBoundary(const std::vector<double>& t_coord,
...
bool Spline2dConstraint::Add2dDerivativeBoundary(const std::vector<double>& t_coord,
                                                 const std::vector<double>& angle,
                                                 const std::vector<Vec2d>&  ref_point,
                                                 const std::vector<double>& longitudinal_bound,
                                                 const std::vector<double>& lateral_bound) {
...
bool Spline2dConstraint::AddPointKthOrderDerivativeConstraint(const double t,
...
bool Spline2dConstraint::AddPointAngleConstraint(const double t, const double angle) {
  ...
  if (normalized_angle < 0) { normalized_angle += M_PI * 2; }

  if (normalized_angle > (M_PI / 2) && normalized_angle < (M_PI * 1.5)) { x_sign = -1; }

  if (normalized_angle >= M_PI) { y_sign = -1; }
  ...
  if (!AddEqualityConstraint(affine_equality, affine_boundary)) { return false; }
...
bool Spline2dConstraint::AddSmoothConstraint() {
  if (t_knots_.size() < 3) { return false; }
...
bool Spline2dConstraint::AddDerivativeSmoothConstraint() {
  if (t_knots_.size() < 3) { return true; }
...
std::vector<double> Spline2dConstraint::AffineDerivativeCoef(const double angle,
...
std::vector<double> Spline2dConstraint::AffineSecondDerivativeCoef(const double angle,
                                                                   const double t) const {
  const uint32_t      num_params = spline_order_ + 1;
  std::vector<double> result(num_params * 2, 0.0);
  double              x_coef  = -common::math::sin(common::math::Angle16::from_rad(angle));
...
std::vector<double> Spline2dConstraint::AffineThirdDerivativeCoef(const double angle,
                                                                  const double t) const {
  const uint32_t      num_params = spline_order_ + 1;
  std::vector<double> result(num_params * 2, 0.0);
  double              x_coef  = -common::math::sin(common::math::Angle16::from_rad(angle));
...
double Spline2dConstraint::SignDistance(const Vec2d& xy_point, const double angle) const {
  return common::math::InnerProd(xy_point.x(), xy_point.y(),
