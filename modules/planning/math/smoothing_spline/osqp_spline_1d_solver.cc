...
bool OsqpSpline1dSolver::Solve() {
  ...
  int constraint_num =
      static_cast<int>(inequality_constraint_boundary.rows() + equality_constraint_boundary.rows());
  ...
  data_->P = csc_matrix(data_->n, data_->n, P_data.size(), P_data.data(), P_indices.data(),
                        P_indptr.data());
  data_->q = q;
  data_->A = csc_matrix(data_->m, data_->n, A_data.size(), A_data.data(), A_indices.data(),
                        A_indptr.data());
  data_->l = l;
  data_->u = u;
