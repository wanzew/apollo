...
QuinticSpiralPath::QuinticSpiralPath(const double x0,
  ...
  coef_deriv_[5][6] = 30.0 * x0 / p6 + 12.0 * dx0 / p5 + 1.5 * ddx0 / p4 - 30.0 * x1 / p6 +
                      12.0 * dx1 / p5 - 1.5 * ddx1 / p4;
...
double QuinticSpiralPath::DeriveTheta(const std::size_t param_index, const double r) const {
  double s  = param_ * r;
  double s2 = s * s;
  double s3 = s2 * s;
  double s4 = s2 * s2;
