...
void CosThetaIpoptInterface::get_optimization_results(std::vector<double>* ptr_x,
                                                      std::vector<double>* ptr_y) const {
  *ptr_x = opt_x_;
  *ptr_y = opt_y_;
}

bool CosThetaIpoptInterface::get_nlp_info(
...
bool CosThetaIpoptInterface::get_bounds_info(
...
bool CosThetaIpoptInterface::get_starting_point(int     n,
                                                bool    init_x,
                                                double* x,
                                                bool    init_z,
                                                double* z_L,
...
bool CosThetaIpoptInterface::eval_grad_f(int n, const double* x, bool new_x, double* grad_f) {
  ...
  for (size_t i = 0; i < num_of_points_ - 2; ++i) {
    ...
    double q3      = ((2 * x[index] - 2 * x[index + 2]) *
                 ((x[index] - x[index + 2]) * (x[index + 2] - x[index + 4]) +
                  (x[index + 1] - x[index + 3]) * (x[index + 3] - x[index + 5])));
    double q4      = ((2 * x[index + 1] - 2 * x[index + 3]) *
                 ((x[index] - x[index + 2]) * (x[index + 2] - x[index + 4]) +
                  (x[index + 1] - x[index + 3]) * (x[index + 3] - x[index + 5])));
    double q5      = ((2 * x[index + 2] - 2 * x[index + 4]) *
                 ((x[index] - x[index + 2]) * (x[index + 2] - x[index + 4]) +
                  (x[index + 1] - x[index + 3]) * (x[index + 3] - x[index + 5])));
    double q6      = ((2 * x[index + 3] - 2 * x[index + 5]) *
                 ((x[index] - x[index + 2]) * (x[index + 2] - x[index + 4]) +
                  (x[index + 1] - x[index + 3]) * (x[index + 3] - x[index + 5])));
    ...
    grad_f[index + 3] += -weight_cos_included_angle_ *
                         ((x[index + 1] - 2 * x[index + 3] + x[index + 5]) / (sqrt_q1 * sqrt_q2) +
...
bool CosThetaIpoptInterface::eval_jac_g(
...
bool CosThetaIpoptInterface::eval_h(int           n,
                                    const double* x,
                                    bool          new_x,
                                    double        obj_factor,
                                    int           m,
  ...
  if (use_automatic_differentiation_) {
    if (values == nullptr) {
      ...
      sparse_hess(tag_L, n, 1, const_cast<double*>(x), &nnz_L_, &rind_L_, &cind_L_, &hessval_,
                  options_L_);
    ...
    for (size_t i = 0; i < num_of_points_ - 2; ++i) {
      ...
      values[idx_map_[std::make_pair(topleft + 2, topleft)]] +=
          ...
          (1 / sqrt_q1q2 + q4 / q1_sqrt_q1q2 - (q5 * q7) / (2 * q1_sqrt_q1q2) -
           ...
           (q8 * q13) / (2 * sqrt_q1_q2_sqrt_q2) + (q5 * q8 * q4) / (4 * q1_sqrt_q1_q2_sqrt_q2));
      ...
      values[idx_map_[std::make_pair(topleft + 1, topleft + 1)]] +=
          obj_factor * (-weight_cos_included_angle_) *
          ((3 * q6 * q6 * q4) / (4 * square_q1 * sqrt_q1q2) - q4 / q1_sqrt_q1q2 -
           (q6 * q11) / q1_sqrt_q1q2);
      ...
      values[idx_map_[std::make_pair(topleft + 4, topleft + 2)]] +=
          ...
          (1 / sqrt_q1q2 + q4 / (sqrt_q1_q2_sqrt_q2) + (q8 * q7) / (2 * sqrt_q1_q2_sqrt_q2) -
           (3 * q8 * q8 * q4) / (4 * sqrt_q1 * square_q2 * sqrt_q2) -
           (q5 * q12) / (2 * q1_sqrt_q1q2) + (q8 * q12) / (2 * sqrt_q1_q2_sqrt_q2) +
           (q5 * q8 * q4) / (4 * q1_sqrt_q1_q2_sqrt_q2));
      ...
      values[idx_map_[std::make_pair(topleft + 5, topleft + 3)]] +=
          ...
          (1 / sqrt_q1q2 + q4 / (sqrt_q1_q2_sqrt_q2) + (q9 * q10) / (2 * sqrt_q1_q2_sqrt_q2) -
           (3 * q9 * q9 * q4) / (4 * sqrt_q1 * square_q2 * sqrt_q2) -
           (q6 * q14) / (2 * q1_sqrt_q1q2) + (q9 * q14) / (2 * sqrt_q1_q2_sqrt_q2) +
           (q6 * q9 * q4) / (4 * q1_sqrt_q1_q2_sqrt_q2));
      ...
      values[idx_map_[std::make_pair(topleft + 5, topleft + 4)]] +=
          obj_factor * (-weight_cos_included_angle_) *
          ((3 * q8 * q9 * q4) / (4 * sqrt_q1 * square_q2 * sqrt_q2) -
           (q9 * q12) / (2 * sqrt_q1_q2_sqrt_q2) - (q8 * q14) / (2 * sqrt_q1_q2_sqrt_q2));
...
void CosThetaIpoptInterface::finalize_solution(Ipopt::SolverReturn               status,
                                               int                               n,
                                               const double*                     x,
                                               const double*                     z_L,
                                               const double*                     z_U,
...
void CosThetaIpoptInterface::hessian_strcuture() {
  ...
  for (size_t i = 0; i < 6; ++i) {
    for (size_t j = 0; j <= i; ++j) {
      idx_map_.insert(
          std::pair<std::pair<size_t, size_t>, size_t>(std::pair<size_t, size_t>(i, j), index));
      index++;
    }
  }
...
bool CosThetaIpoptInterface::eval_obj(int n, const T* x, T* obj_value) {
  *obj_value = 0.0;
  ...
  for (size_t i = 0; i < num_of_points_ - 1; ++i) {
    ...
    *obj_value +=
        weight_length_ * ((x[findex] - x[nindex]) * (x[findex] - x[nindex]) +
