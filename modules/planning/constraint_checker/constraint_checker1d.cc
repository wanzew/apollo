...
inline bool
fuzzy_within(const double v, const double lower, const double upper, const double e = 1.0e-4) {
...
bool ConstraintChecker1d::IsValidLongitudinalTrajectory(const Curve1d& lon_trajectory) {
  ...
  while (t < lon_trajectory.ParamLength()) {
    ...
    if (!fuzzy_within(v, FLAGS_speed_lower_bound, FLAGS_speed_upper_bound)) { return false; }
...
bool ConstraintChecker1d::IsValidLateralTrajectory(const Curve1d& lat_trajectory,
  ...
  while (t < lon_trajectory.ParamLength()) {
    ...
    double a = 0.0;
    if (s < lat_trajectory.ParamLength()) { a = d2d_ds2 * ds_dt * ds_dt + dd_ds * d2s_dt2; }
    ...
    if (!fuzzy_within(j, -FLAGS_lateral_jerk_bound, FLAGS_lateral_jerk_bound)) { return false; }
    t += FLAGS_trajectory_time_resolution;
  }
  return true;
}
