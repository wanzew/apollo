...
OnLanePlanning::~OnLanePlanning() {
  if (reference_line_provider_) { reference_line_provider_->Stop(); }
  planner_->Stop();
...
Status OnLanePlanning::Init(const PlanningConfig& config) {
  ...
  ACHECK(apollo::cyber::common::GetProtoFromFile(FLAGS_traffic_rule_config_filename,
  ...
  reference_line_provider_ =
      std::make_unique<ReferenceLineProvider>(injector_->vehicle_state(), hdmap_);
...
Status OnLanePlanning::InitFrame(const uint32_t         sequence_num,
  ...
  frame_.reset(new Frame(sequence_num, local_view_, planning_start_point, vehicle_state,
  ...
  for (auto& ref_line : reference_lines) {
    if (!ref_line.Segment(Vec2d(vehicle_state.x(), vehicle_state.y()), FLAGS_look_backward_distance,
                          forward_limit)) {
  ...
  for (auto& seg : segments) {
    if (!seg.Shrink(Vec2d(vehicle_state.x(), vehicle_state.y()), FLAGS_look_backward_distance,
                    forward_limit)) {
...
bool OnLanePlanning::JudgeCarInDeadEndJunction(std::vector<JunctionInfoConstPtr>* junctions,
  ...
  if (junction_num <= 0) { return false; }
...
void OnLanePlanning::RunOnce(const LocalView& local_view, ADCTrajectory* const ptr_trajectory_pb) {
  ...
  Status status =
      injector_->vehicle_state()->Update(*local_view_.localization_estimate, *local_view_.chassis);

  VehicleState vehicle_state = injector_->vehicle_state()->vehicle_state();
  size_t       waypoint_num  = local_view_.routing->routing_request().waypoint().size();
  if (local_view_.routing->routing_request().dead_end_info().dead_end_routing_type() ==
      routing::ROUTING_IN) {
    dead_end_point_ = local_view_.routing->routing_request().waypoint().at(waypoint_num - 1).pose();
  } else if (local_view_.routing->routing_request().dead_end_info().dead_end_routing_type() ==
             routing::ROUTING_OUT) {
    dead_end_point_ = local_view_.routing->routing_request().waypoint().at(0).pose();
  }
  ...
  if (FLAGS_enable_record_debug) { frame_->RecordInputDebug(ptr_trajectory_pb->mutable_debug()); }
  ptr_trajectory_pb->mutable_latency_stats()->set_init_frame_time_ms(Clock::NowInSeconds() -
                                                                     start_timestamp);
...
void OnLanePlanning::ExportReferenceLineDebug(planning_internal::Debug* debug) {
  if (!FLAGS_enable_record_debug) { return; }
  for (auto& reference_line_info : *frame_->mutable_reference_line_info()) {
    ...
    const auto& reference_line_path   = reference_line_info.reference_line().GetMapPath();
    const auto  sample_s              = 0.1;
    const auto  reference_line_length = reference_line_info.reference_line().Length();
    ...
    for (double s = 0.0; s < reference_line_length; s += sample_s) {
      ...
      if (left_width < adc_half_width || right_width < adc_half_width) { is_off_road = true; }
      if (left_width < minimum_boundary) { minimum_boundary = left_width; }
      if (right_width < minimum_boundary) { minimum_boundary = right_width; }
      ++sample_count;
    }
...
Status OnLanePlanning::Plan(const double                        current_time_stamp,
  ...
  if (frame_->open_space_info().is_on_open_space_trajectory()) {
    ...
    if (injector_->vehicle_state()->vehicle_state().driving_mode() !=
      ...
      engage_advice->set_reason("Ready to engage when staring with OPEN_SPACE_PLANNER");
    } else {
    ...
    ptr_trajectory_pb->mutable_decision()->mutable_main_decision()->mutable_parking()->set_status(
    ...
    if (!best_ref_info) {
      ...
      AERROR << msg;
      if (last_publishable_trajectory_) { last_publishable_trajectory_->Clear(); }
      return Status(ErrorCode::PLANNING_ERROR, msg);
    }
    ...
    if (FLAGS_export_chart) {
      ...
      if (failed_ref_info) { ExportFailedLaneChangeSTChart(failed_ref_info->debug(), ptr_debug); }
    }
    ...
    if (FLAGS_enable_record_debug) {
      ...
      const auto& reference_points = best_ref_info->reference_line().reference_points();
      double      s                = 0.0;
      double      prev_x           = 0.0;
      double      prev_y           = 0.0;
    ...
    last_publishable_trajectory_.reset(
        new PublishableTrajectory(current_time_stamp, best_ref_info->trajectory()));
...
bool OnLanePlanning::CheckPlanningConfig(const PlanningConfig& config) {
  if (!config.has_standard_planning_config()) { return false; }
  if (!config.standard_planning_config().has_planner_public_road_config()) { return false; }
  // TODO(All): check other config params
  return true;
}

void PopulateChartOptions(double      x_min,
                          double      x_max,
                          std::string x_label,
                          double      y_min,
                          double      y_max,
...
void AddSTGraph(const STGraphDebug& st_graph, Chart* chart) {
  ...
  PopulateChartOptions(-2.0, 10.0, "t (second)", -10.0, 220.0, "s (meter)", false, chart);
...
void AddSLFrame(const SLFrameDebug& sl_frame, Chart* chart) {
  ...
  PopulateChartOptions(0.0, 80.0, "s (meter)", -8.0, 8.0, "l (meter)", false, chart);
...
void AddSpeedPlan(const ::google::protobuf::RepeatedPtrField<SpeedPlan>& speed_plans,
  ...
  PopulateChartOptions(0.0, 80.0, "s (meter)", 0.0, 50.0, "v (m/s)", false, chart);
...
void OnLanePlanning::ExportFailedLaneChangeSTChart(const planning_internal::Debug& debug_info,
...
void OnLanePlanning::ExportOnLaneChart(const planning_internal::Debug& debug_info,
                                       planning_internal::Debug*       debug_chart) {
  const auto& src_data = debug_info.planning_data();
  auto*       dst_data = debug_chart->mutable_planning_data();
  for (const auto& st_graph : src_data.st_graph()) {
...
void OnLanePlanning::ExportOpenSpaceChart(const planning_internal::Debug& debug_info,
...
void OnLanePlanning::AddOpenSpaceOptimizerResult(const planning_internal::Debug& debug_info,
                                                 planning_internal::Debug*       debug_chart) {
  ...
  PopulateChartOptions(open_space_debug.xy_boundary(0) - 1.0, open_space_debug.xy_boundary(1) + 1.0,
                       "x (meter)", open_space_debug.xy_boundary(2) - 1.0,
                       open_space_debug.xy_boundary(3) + 1.0, "y (meter)", true, chart);
  ...
  for (const auto& obstacle : open_space_debug.obstacles()) {
    ...
    obstacle_index += 1;
    for (int vertice_index = 0; vertice_index < obstacle.vertices_x_coords_size();
         vertice_index++) {
...
void OnLanePlanning::AddPartitionedTrajectory(const planning_internal::Debug& debug_info,
                                              planning_internal::Debug*       debug_chart) {
  // if open space info provider success run
  if (!frame_->open_space_info().open_space_provider_success()) { return; }

  const auto& open_space_debug    = debug_info.planning_data().open_space();
  const auto& chosen_trajectories = open_space_debug.chosen_trajectory().trajectory();
  if (chosen_trajectories.empty() || chosen_trajectories[0].trajectory_point().empty()) { return; }
  ...
  for (const auto& partitioned_trajectory :
    ...
    partition_line->set_label(absl::StrCat("Partitioned ", partitioned_trajectory_label));
  ...
  trajectory_stitching_point->set_x(open_space_debug.trajectory_stitching_point().path_point().x());
  trajectory_stitching_point->set_y(open_space_debug.trajectory_stitching_point().path_point().y());
  // Set chartJS's dataset properties
  ...
  if (open_space_debug.is_fallback_trajectory()) {
    ...
    future_collision_point->set_x(open_space_debug.future_collision_point().path_point().x());
    future_collision_point->set_y(open_space_debug.future_collision_point().path_point().y());
    // Set chartJS's dataset properties
    ...
    const auto& fallback_trajectories = open_space_debug.fallback_trajectory().trajectory();
    if (fallback_trajectories.empty() || fallback_trajectories[0].trajectory_point().empty()) {
      return;
    }
...
void OnLanePlanning::AddStitchSpeedProfile(planning_internal::Debug* debug_chart) {
  ...
  for (const auto& point : last_trajectory.trajectory_point()) {
    auto* point_debug = speed_profile->add_point();
    point_debug->set_x(point.relative_time() + last_trajectory.header().timestamp_sec());
    point_debug->set_y(point.v());
  }
  // Set chartJS's dataset properties
...
void OnLanePlanning::AddPublishedSpeed(const ADCTrajectory&      trajectory_pb,
  ...
  for (const auto& point : trajectory_pb.trajectory_point()) {
    auto* point_debug = speed_profile->add_point();
    point_debug->set_x(point.relative_time() + trajectory_pb.header().timestamp_sec());
    if (trajectory_pb.gear() == canbus::Chassis::GEAR_DRIVE) { point_debug->set_y(point.v()); }
    if (trajectory_pb.gear() == canbus::Chassis::GEAR_REVERSE) { point_debug->set_y(-point.v()); }
  }
  // Set chartJS's dataset properties
  auto* speed_profile_properties             = speed_profile->mutable_properties();
  (*speed_profile_properties)["borderWidth"] = "2";
...
void OnLanePlanning::AddPublishedAcceleration(const ADCTrajectory&      trajectory_pb,
  ...
  for (const auto& point : trajectory_pb.trajectory_point()) {
    ...
    point_debug->set_x(point.relative_time() + trajectory_pb.header().timestamp_sec());
    if (trajectory_pb.gear() == canbus::Chassis::GEAR_DRIVE) point_debug->set_y(point.a());
    if (trajectory_pb.gear() == canbus::Chassis::GEAR_REVERSE) point_debug->set_y(-point.a());
  }
  // Set chartJS's dataset properties
