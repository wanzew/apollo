...
void FeatureOutput::InsertLearningDataFrame(const std::string&       record_file,
  ...
  if (learning_data_.learning_data_frame_size() >= FLAGS_learning_data_frame_num_per_file) {
...
void FeatureOutput::WriteLearningData(const std::string& record_file) {
  ...
  const std::string dest_file = absl::StrCat(FLAGS_planning_data_dir, "/", src_file_name, ".",
...
void FeatureOutput::WriteRemainderiLearningData(const std::string& record_file) {
  if (learning_data_.learning_data_frame_size() > 0) { WriteLearningData(record_file); }
}
