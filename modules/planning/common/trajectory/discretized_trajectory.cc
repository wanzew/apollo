...
TrajectoryPoint DiscretizedTrajectory::Evaluate(const double relative_time) const {
  ...
  auto it_lower =
      std::lower_bound(trajectory_points_.begin(), trajectory_points_.end(), relative_time, comp);
...
std::uint32_t DiscretizedTrajectory::QueryLowerBoundPoint(const double relative_time) const {
  ...
  auto it_lower =
      std::lower_bound(trajectory_points_.begin(), trajectory_points_.end(), relative_time, func);
  return std::distance(trajectory_points_.begin(), it_lower);
}

std::uint32_t DiscretizedTrajectory::QueryNearestPoint(const common::math::Vec2d& position) const {
  ...
  for (std::uint32_t i = 0; i < trajectory_points_.size(); ++i) {
    const common::math::Vec2d curr_point(trajectory_points_[i].path_point().x(),
...
void DiscretizedTrajectory::AppendTrajectoryPoint(const TrajectoryPoint& trajectory_point) {
  if (!trajectory_points_.empty()) {
    CHECK_GT(trajectory_point.relative_time(), trajectory_points_.back().relative_time());
  }
  trajectory_points_.push_back(trajectory_point);
}

const TrajectoryPoint& DiscretizedTrajectory::TrajectoryPointAt(const std::uint32_t index) const {
...
double DiscretizedTrajectory::GetTemporalLength() const {
  CHECK(!trajectory_points_.empty());
  return trajectory_points_.back().relative_time() - trajectory_points_.front().relative_time();
}

double DiscretizedTrajectory::GetSpatialLength() const {
  CHECK(!trajectory_points_.empty());
  return trajectory_points_.back().path_point().s() - trajectory_points_.front().path_point().s();
}
