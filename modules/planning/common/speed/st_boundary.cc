...
std::string StBoundary::TypeName(BoundaryType type) {
  ...
  AWARN << "Unkown boundary type " << static_cast<int>(type) << ", treated as UNKNOWN";
  return "UNKNOWN";
}
...
bool StBoundary::IsValid(const std::vector<std::pair<STPoint, STPoint>>& point_pairs) const {
  if (point_pairs.size() < 2) {
    AERROR << "point_pairs.size() must > 2. current point_pairs.size() = " << point_pairs.size();
    return false;
  }
...
StBoundary StBoundary::ExpandByS(const double s) const {
  ...
  for (size_t i = 0; i < lower_points_.size(); ++i) {
    point_pairs.emplace_back(STPoint(lower_points_[i].y() - s, lower_points_[i].x()),
...
StBoundary StBoundary::ExpandByT(const double t) const {
  ...
  point_pairs.emplace_back(STPoint(lower_points_[0].y() - t * lower_left_delta_s / left_delta_t,
                                   lower_points_[0].x() - t),
  ...
  *s_lower = 0.0;
  ...
  if (boundary_type_ == BoundaryType::STOP || boundary_type_ == BoundaryType::YIELD ||
    ...
    *s_lower = std::fmax(*s_lower, upper_cross_s);
    ...
    AERROR << "boundary_type is not supported. boundary_type: " << static_cast<int>(boundary_type_);
    return false;
  }
  return true;
}
...
bool StBoundary::GetBoundarySRange(const double curr_time, double* s_upper, double* s_lower) const {
  ...
  const double r =
      (curr_time - upper_points_[left].t()) / (upper_points_[right].t() - upper_points_[left].t());
