...
STBoundary STBoundary::CreateInstanceAccurate(const std::vector<STPoint>& lower_points,
                                              const std::vector<STPoint>& upper_points) {
  if (lower_points.size() != upper_points.size() || lower_points.size() < 2) {
    return STBoundary();
  }
...
std::string STBoundary::TypeName(BoundaryType type) {
  ...
  AWARN << "Unknown boundary type " << static_cast<int>(type) << ", treated as UNKNOWN";
  return "UNKNOWN";
}
...
bool STBoundary::GetUnblockSRange(const double curr_time, double* s_upper, double* s_lower) const {
  ...
  *s_lower = 0.0;
  if (curr_time < min_t_ || curr_time > max_t_) { return true; }
  ...
  if (curr_time > upper_points_[right].t()) { return true; }
  ...
  double upper_cross_s =
      upper_points_[left].s() + r * (upper_points_[right].s() - upper_points_[left].s());
  double lower_cross_s =
      lower_points_[left].s() + r * (lower_points_[right].s() - lower_points_[left].s());
  ...
  if (boundary_type_ == BoundaryType::STOP || boundary_type_ == BoundaryType::YIELD ||
    ...
    *s_lower = std::fmax(*s_lower, upper_cross_s);
    ...
    ADEBUG << "boundary_type is not supported. boundary_type: " << static_cast<int>(boundary_type_);
    return false;
  }
  return true;
}
...
bool STBoundary::GetBoundarySRange(const double curr_time, double* s_upper, double* s_lower) const {
  CHECK_NOTNULL(s_upper);
  CHECK_NOTNULL(s_lower);
  if (curr_time < min_t_ || curr_time > max_t_) { return false; }
...
bool STBoundary::GetBoundarySlopes(const double curr_time,
                                   double*      ds_upper,
                                   double*      ds_lower) const {
  if (ds_upper == nullptr || ds_lower == nullptr) { return false; }
  if (curr_time < min_t_ || curr_time > max_t_) { return false; }
...
bool STBoundary::IsPointInBoundary(const STPoint& st_point) const {
  if (st_point.t() <= min_t_ || st_point.t() >= max_t_) { return false; }
  size_t left  = 0;
  size_t right = 0;
  ...
  const double check_upper =
      common::math::CrossProd(st_point, upper_points_[left], upper_points_[right]);
  const double check_lower =
      common::math::CrossProd(st_point, lower_points_[left], lower_points_[right]);

  return (check_upper * check_lower < 0);
}

STBoundary STBoundary::ExpandByS(const double s) const {
  if (lower_points_.empty()) { return STBoundary(); }
  std::vector<std::pair<STPoint, STPoint>> point_pairs;
...
STBoundary STBoundary::ExpandByT(const double t) const {
  ...
  point_pairs.emplace_back(STPoint(lower_points_[0].s() - t * lower_left_delta_s / left_delta_t,
                                   lower_points_[0].t() - t),
  ...
  point_pairs.emplace_back(
      STPoint(lower_points_.back().s() + t * lower_right_delta_s / right_delta_t,
              lower_points_.back().t() + t),
      STPoint(upper_points_.back().s() + t * upper_right_delta_s / right_delta_t,
...
STBoundary STBoundary::CutOffByT(const double t) const {
  ...
  for (size_t i = 0; i < lower_points_.size() && i < upper_points_.size(); ++i) {
    if (lower_points_[i].t() < t) { continue; }
    ...
    if (i + 1 != point_pairs.size()) {
      ...
      if (std::fmax(curr_lower.t(), curr_upper.t()) + kMinDeltaT >=
        ...
        AERROR << "Latter points should have larger t: "
               ...
               << curr_upper.DebugString() << "] next_lower[" << next_lower.DebugString()
...
void STBoundary::RemoveRedundantPoints(std::vector<std::pair<STPoint, STPoint>>* point_pairs) {
  ...
  while (i < point_pairs->size() && j + 1 < point_pairs->size()) {
    ...
    if (!IsPointNear(lower_seg, point_pairs->at(j).first, kMaxDist) ||
      ...
      ++i;
      if (i != j) { point_pairs->at(i) = point_pairs->at(j); }
    }
    ++j;
  }
