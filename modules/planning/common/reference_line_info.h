...
class ReferenceLineInfo {
 public:
  ...
  bool
  AdjustTrajectoryWhichStartsFromCurrentPos(const common::TrajectoryPoint& planning_start_point,
  ...
  ADCTrajectory::TrajectoryType trajectory_type() const { return trajectory_type_; }
  ...
  void set_path_reusable(const bool path_reusable) { path_reusable_ = path_reusable; }
 ...
 private:
  ...
  void SetTurnSignalBasedOnLaneTurnType(common::VehicleSignal* vehicle_signal) const;

  void ExportVehicleSignal(common::VehicleSignal* vehicle_signal) const;

  bool IsIrrelevantObstacle(const Obstacle& obstacle);

  void MakeDecision(DecisionResult* decision_result, PlanningContext* planning_context) const;
