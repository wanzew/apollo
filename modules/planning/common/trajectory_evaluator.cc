...
void TrajectoryEvaluator::WriteLog(const std::string& msg) {
  AERROR << msg;
  if (FLAGS_planning_offline_learning) { EvaluatorLogger::GetStream() << msg << std::endl; }
}

void TrajectoryEvaluator::EvaluateTrajectoryByTime(
    const int                                                           frame_num,
    const std::string&                                                  obstacle_id,
    const std::vector<std::pair<double, CommonTrajectoryPointFeature>>& trajectory,
    const double                                                        start_point_timestamp_sec,
    const double                                                        delta_time,
    std::vector<TrajectoryPointFeature>*                                evaluated_trajectory) {
  if (trajectory.empty() ||
      (fabs(trajectory.front().first - start_point_timestamp_sec) < delta_time &&
       fabs(trajectory.back().first - start_point_timestamp_sec) < delta_time)) {
    return;
  }
  ...
  for (const auto& tp : updated_trajectory) {
    ...
    if (tp.relative_time() > last_relative_time) {
      ...
      const std::string msg = absl::StrCat(
          "DISCARD trajectory point: frame_num[", frame_num, "] obstacle_id[", obstacle_id,
          "] last_relative_time[", last_relative_time, "] relatice_time[", tp.relative_time(),
          "] relative_time_diff[", tp.relative_time() - last_relative_time, "]");
      WriteLog(msg);
    }
  }

  const int low_bound =
      std::max(-150.0, ceil(updated_trajectory.front().relative_time() / delta_time));
  const int high_bound =
      std::min(150.0, floor(updated_trajectory.back().relative_time() / delta_time));
  ADEBUG << "frame_num[" << frame_num << "] obstacle_id[" << obstacle_id << "] low[" << low_bound
         << "] high[" << high_bound << "]";
...
void TrajectoryEvaluator::EvaluateADCTrajectory(const double       start_point_timestamp_sec,
                                                const double       delta_time,
  ...
  for (const auto& adc_tp : learning_data_frame->adc_trajectory_point()) {
    trajectory.push_back(std::make_pair(adc_tp.timestamp_sec(), adc_tp.trajectory_point()));
  }

  if (trajectory.size() <= 1) {
    const std::string msg =
        absl::StrCat("too short adc_trajectory_point. frame_num[", learning_data_frame->frame_num(),
  ...
  if (fabs(trajectory.front().first - start_point_timestamp_sec) <= delta_time) {
    ...
    adc_trajectory_point->mutable_trajectory_point()->CopyFrom(trajectory.back().second);

    const std::string msg =
        absl::StrCat("too short adc_trajectory. frame_num[", learning_data_frame->frame_num(),
                     "] size[", trajectory.size(), "] timestamp_diff[",
  ...
  std::vector<TrajectoryPointFeature> evaluated_trajectory;
  EvaluateTrajectoryByTime(learning_data_frame->frame_num(), "adc_trajectory", trajectory,
                           start_point_timestamp_sec, delta_time, &evaluated_trajectory);
  ADEBUG << "frame_num[" << learning_data_frame->frame_num() << "] orig adc_trajectory["
         << trajectory.size() << "] evaluated_trajectory_size[" << evaluated_trajectory.size()
         << "]";
  ...
  for (const auto& tp : evaluated_trajectory) {
    if (tp.trajectory_point().relative_time() <= 0.0 &&
      ...
      adc_trajectory_point->mutable_trajectory_point()->CopyFrom(tp.trajectory_point());
    } else {
      const std::string msg =
          absl::StrCat("DISCARD adc_trajectory_point. frame_num[", learning_data_frame->frame_num(),
...
void TrajectoryEvaluator::EvaluateADCFutureTrajectory(
  ...
  for (const auto& adc_future_trajectory_point : adc_future_trajectory) {
    trajectory.push_back(std::make_pair(adc_future_trajectory_point.timestamp_sec(),
                                        adc_future_trajectory_point.trajectory_point()));
  }

  if (trajectory.size() <= 1) {
    const std::string msg = absl::StrCat("too short adc_future_trajectory. frame_num[", frame_num,
                                         "] size[", trajectory.size(), "]");
    WriteLog(msg);
    return;
  }
  ...
  ADEBUG << "frame_num[" << frame_num << "] orig adc_future_trajectory[" << trajectory.size()
         << "] evaluated_trajectory_size[" << evaluated_trajectory.size() << "]";

  if (evaluated_trajectory.empty()) {
    const std::string msg =
        absl::StrCat("WARNING: adc_future_trajectory not long enough. ", "frame_num[", frame_num,
    ...
    if (time_range < FLAGS_trajectory_time_length) {
      const std::string msg =
          absl::StrCat("WARNING: adc_future_trajectory not long enough. ", "frame_num[", frame_num,
                       "] size[", evaluated_trajectory.size(), "] time_range[", time_range, "]");
      WriteLog(msg);
    }
  }
  ...
  for (const auto& tp : evaluated_trajectory) {
    if (tp.trajectory_point().relative_time() > 0.0 &&
      ...
      const std::string msg =
          absl::StrCat("DISCARD adc_future_trajectory_point. frame_num[", frame_num, "] size[",
...
void TrajectoryEvaluator::EvaluateObstacleTrajectory(const double       start_point_timestamp_sec,
                                                     const double       delta_time,
                                                     LearningDataFrame* learning_data_frame) {
  for (int i = 0; i < learning_data_frame->obstacle_size(); ++i) {
    const int  obstacle_id         = learning_data_frame->obstacle(i).id();
    const auto obstacle_trajectory = learning_data_frame->obstacle(i).obstacle_trajectory();
    std::vector<std::pair<double, CommonTrajectoryPointFeature>> trajectory;
    for (const auto& perception_obstacle : obstacle_trajectory.perception_obstacle_history()) {
      CommonTrajectoryPointFeature trajectory_point;
      trajectory_point.mutable_path_point()->set_x(perception_obstacle.position().x());
      trajectory_point.mutable_path_point()->set_y(perception_obstacle.position().y());
      trajectory_point.mutable_path_point()->set_z(perception_obstacle.position().z());
      trajectory_point.mutable_path_point()->set_theta(perception_obstacle.theta());

      const double v =
          std::sqrt(perception_obstacle.velocity().x() * perception_obstacle.velocity().x() +
                    perception_obstacle.velocity().y() * perception_obstacle.velocity().y());
      trajectory_point.set_v(v);

      const double a = std::sqrt(
          perception_obstacle.acceleration().x() * perception_obstacle.acceleration().x() +
          perception_obstacle.acceleration().y() * perception_obstacle.acceleration().y());
      trajectory_point.set_a(a);

      trajectory.push_back(std::make_pair(perception_obstacle.timestamp_sec(), trajectory_point));
    }
    ...
    if (trajectory.size() == 1 ||
      ...
      ADEBUG << "too short obstacle_trajectory. frame_num[" << learning_data_frame->frame_num()
             << "] obstacle_id[" << obstacle_id << "] size[" << trajectory.size()
             << "] timestamp_diff[" << start_point_timestamp_sec - trajectory.front().first
             << "] time_range[" << fabs(trajectory.front().first - trajectory.back().first) << "]";
      ...
      trajectory_point.mutable_trajectory_point()->CopyFrom(trajectory.front().second);
      evaluated_trajectory.push_back(trajectory_point);
    } else {
      EvaluateTrajectoryByTime(learning_data_frame->frame_num(), std::to_string(obstacle_id),
                               trajectory, start_point_timestamp_sec, delta_time,
                               &evaluated_trajectory);

      ADEBUG << "frame_num[" << learning_data_frame->frame_num() << "] obstacle_id[" << obstacle_id
             << "] orig obstacle_trajectory[" << trajectory.size() << "] evaluated_trajectory_size["
    ...
    for (const auto& tp : evaluated_trajectory) {
      ...
      evaluated_trajectory_point->mutable_trajectory_point()->CopyFrom(tp.trajectory_point());
    }
  }
}
...
void TrajectoryEvaluator::EvaluateObstaclePredictionTrajectory(
  ...
  for (int i = 0; i < learning_data_frame->obstacle_size(); ++i) {
    const int  obstacle_id         = learning_data_frame->obstacle(i).id();
    const auto obstacle_prediction = learning_data_frame->obstacle(i).obstacle_prediction();
    for (int j = 0; j < obstacle_prediction.trajectory_size(); ++j) {
      const auto obstacle_prediction_trajectory = obstacle_prediction.trajectory(j);

      std::vector<std::pair<double, CommonTrajectoryPointFeature>> trajectory;
      for (const auto& trajectory_point : obstacle_prediction_trajectory.trajectory_point()) {
        const double timestamp_sec = obstacle_prediction.timestamp_sec() +
                                     trajectory_point.trajectory_point().relative_time();
        trajectory.push_back(std::make_pair(timestamp_sec, trajectory_point.trajectory_point()));
      }
      if (fabs(trajectory.back().first - start_point_timestamp_sec) <= delta_time) {
        ADEBUG << "too short obstacle_prediction_trajectory. frame_num["
               << learning_data_frame->frame_num() << "] obstacle_id[" << obstacle_id << "] size["
      ...
      for (const auto& tp : evaluated_trajectory) {
        auto obstacle_prediction_trajectory_point = learning_data_frame->mutable_obstacle(i)
                                                        ->mutable_obstacle_prediction()
                                                        ->mutable_trajectory(j)
                                                        ->add_trajectory_point();
        obstacle_prediction_trajectory_point->set_timestamp_sec(tp.timestamp_sec());
        obstacle_prediction_trajectory_point->mutable_trajectory_point()->CopyFrom(
            tp.trajectory_point());
      }
    }
  }
...
void TrajectoryEvaluator::Convert(const common::TrajectoryPoint& tp,
  ...
  trajectory_point->mutable_trajectory_point()->set_relative_time(tp.relative_time());
  trajectory_point->mutable_trajectory_point()->mutable_gaussian_info()->CopyFrom(
      tp.gaussian_info());
}
