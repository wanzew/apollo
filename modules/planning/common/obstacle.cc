...
Obstacle::Obstacle(const std::string& id, const PerceptionObstacle& perception_obstacle)
    : id_(id)
  ...
  CHECK(common::math::Polygon2d::ComputeConvexHull(polygon_points, &perception_polygon_))
...
bool Obstacle::HasTrajectory() const { return trajectory_.trajectory_point_size() > 0; }
...
bool Obstacle::IsStaticObstacle(const PerceptionObstacle& perception_obstacle) {
  if (perception_obstacle.type() == PerceptionObstacle::UNKNOWN_UNMOVABLE) { return true; }
  auto moving_speed =
...
bool Obstacle::IsVirtualObstacle(const PerceptionObstacle& perception_obstacle) {
...
const prediction::Trajectory& Obstacle::Trajectory() const { return trajectory_; }

const PerceptionObstacle& Obstacle::Perception() const { return perception_obstacle_; }
