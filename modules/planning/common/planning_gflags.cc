...
DEFINE_double(look_backward_distance,
              30,
              "look backward this distance when creating reference line from routing");

DEFINE_double(look_forward_short_distance,
              150,
              "short look forward this distance when creating reference line "
              "from routing when ADC is slow");
DEFINE_double(look_forward_long_distance,
              250,
...
DEFINE_double(reference_line_lateral_buffer,
              0.5,
...
DEFINE_bool(enable_change_lane_decider, false, "True to use change lane state machine decider.");
DEFINE_double(change_lane_speed_relax_percentage,
              0.05,
              "The percentage of change lane speed relaxation.");
DEFINE_bool(enable_side_vehicle_st_boundary, false, "Add st boundary of side vehicle in st graph.");

DEFINE_int32(max_history_frame_num, 1, "The maximum history frame number");

DEFINE_double(max_collision_distance,
              0.1,
              "considered as collision if distance (meters) is smaller than or "
              "equal to this (meters)");

DEFINE_bool(ignore_overlapped_obstacle,
            false,
            "ingore obstacle that overlapps with ADC. Only enable this flag "
            "when you found fake obstacle result from poorly lidar");

DEFINE_double(replan_lateral_distance_threshold, 5.0, "The distance threshold of replan");
DEFINE_double(replan_longitudinal_distance_threshold, 5.0, "The distance threshold of replan");
DEFINE_bool(estimate_current_vehicle_state, true, "Estimate current vehicle state.");
...
DEFINE_double(planning_lower_speed_limit, 5.0, "Minimum speed (m/s) in planning.");
...
DEFINE_bool(enable_side_radar, false, "If there is no radar on the side,ignore it");
...
DEFINE_double(nudge_distance_obstacle, 0.5, "minimum distance to nudge a obstacle (meters)");
...
DEFINE_double(yield_distance_pedestrian_bycicle,
              5.0,
...
DEFINE_double(signal_light_min_pass_s_distance,
              4.0,
...
/// thread pool
DEFINE_int32(num_thread_planning_thread_pool, 5, "num of thread used in planning thread pool.");
DEFINE_bool(use_multi_thread_to_add_obstacles, false, "use multiple thread to add obstacles.");
DEFINE_bool(enable_multi_thread_in_dp_poly_path,
            false,
...
DEFINE_double(decision_horizon, 200.0, "Longitudinal horizon for decision making");
DEFINE_uint32(num_velocity_sample, 6, "The number of velocity samples in end condition sampler.");
...
DEFINE_double(lat_collision_buffer, 0.2, "The lateral buffer to keep distance to other vehicles");
DEFINE_uint32(num_sample_follow_per_timestamp,
              3,
...
DEFINE_double(weight_lon_collision, 5.0, "Weight of logitudinal collision cost");
...
DEFINE_double(max_s_lateral_optimization, 50.0, "The maximal s for lateral optimization.");
...
DEFINE_bool(use_planning_fallback, true, "Use fallback trajectory for planning.");

// navigation mode
DEFINE_double(navigation_fallback_cruise_time,
              8.0,
