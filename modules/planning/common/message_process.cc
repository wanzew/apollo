...
void MessageProcess::Close() {
  ...
  if (FLAGS_planning_offline_learning) {
    ...
    const std::string msg =
        absl::StrCat("Total learning_data_frame number: ", total_learning_data_frame_num_);
    AINFO << msg;
    log_file_ << msg << std::endl;
    auto                          end_time        = std::chrono::system_clock::now();
    std::chrono::duration<double> elapsed_seconds = end_time - start_time_;
    log_file_ << "Time elapsed(sec): " << elapsed_seconds.count() << std::endl << std::endl;
    log_file_.close();
  }
}
...
void MessageProcess::OnLocalization(const LocalizationEstimate& le) {
  ...
  const double time_diff = le.header().timestamp_sec() - last_localization_message_timestamp_sec_;
  ...
  if (time_diff >= (1.0 * 2 / FLAGS_planning_loop_rate)) {
    const std::string msg =
        absl::StrCat("missing localization too long: time_stamp[", le.header().timestamp_sec(),
                     "] time_diff[", time_diff, "]");
    AERROR << msg;
    if (FLAGS_planning_offline_learning) { log_file_ << msg << std::endl; }
  }
  last_localization_message_timestamp_sec_ = le.header().timestamp_sec();
  ...
  if (GenerateLearningDataFrame(&learning_data_frame)) {
    ...
    if (FLAGS_planning_offline_learning) {
      ...
      // online
      injector_->learning_based_data()->InsertLearningDataFrame(learning_data_frame);
    }
  }
}

void MessageProcess::OnPrediction(const PredictionObstacles& prediction_obstacles) {
  prediction_obstacles_map_.clear();
  for (int i = 0; i < prediction_obstacles.prediction_obstacle_size(); ++i) {
    const auto& prediction_obstacle = prediction_obstacles.prediction_obstacle(i);
  ...
  */
  ...
  for (const auto& m : prediction_obstacles_map_) {
    ...
    obstacle_trajectory_point.mutable_position()->CopyFrom(perception_obstale.position());
    obstacle_trajectory_point.set_theta(perception_obstale.theta());
    obstacle_trajectory_point.mutable_velocity()->CopyFrom(perception_obstale.velocity());
    ...
    obstacle_trajectory_point.mutable_acceleration()->CopyFrom(perception_obstale.acceleration());
    ...
    if (obstacle_history_map_[m.first].empty() ||
      ...
      const double time_diff = obstacle_trajectory_point.timestamp_sec() -
                               obstacle_history_map_[m.first].back().timestamp_sec();
      const std::string msg =
          absl::StrCat("DISCARD: obstacle_id[", m.first, "] last_timestamp_sec[",
                       obstacle_history_map_[m.first].back().timestamp_sec(), "] timestamp_sec[",
                       obstacle_trajectory_point.timestamp_sec(), "] time_diff[", time_diff, "]");
      AERROR << msg;
      if (FLAGS_planning_offline_learning) { log_file_ << msg << std::endl; }
    }
    ...
    while (!obstacle_history.empty()) {
      const double time_distance =
          obstacle_history.back().timestamp_sec() - obstacle_history.front().timestamp_sec();
      if (time_distance < FLAGS_learning_data_obstacle_history_time_sec) { break; }
      obstacle_history.pop_front();
    }
  }
}

void MessageProcess::OnRoutingResponse(const apollo::routing::RoutingResponse& routing_response) {
  ADEBUG << "routing_response received at frame[" << total_learning_data_frame_num_ << "]";
  routing_response_.CopyFrom(routing_response);
}

void MessageProcess::OnStoryTelling(const apollo::storytelling::Stories& stories) {
  // clear area
...
void MessageProcess::OnTrafficLightDetection(const TrafficLightDetection& traffic_light_detection) {
  // AINFO << "traffic_light_detection received at frame["
  //      << total_learning_data_frame_num_ << "]";
  traffic_light_detection_message_timestamp_ = traffic_light_detection.header().timestamp_sec();
  ...
  for (int i = 0; i < traffic_light_detection.traffic_light_size(); ++i) {
    ...
    traffic_light.set_confidence(traffic_light_detection.traffic_light(i).confidence());
    traffic_light.set_tracking_time(traffic_light_detection.traffic_light(i).tracking_time());
    traffic_light.set_remaining_time(traffic_light_detection.traffic_light(i).remaining_time());
...
void MessageProcess::ProcessOfflineData(const std::string& record_file) {
  ...
  while (reader.ReadMessage(&message)) {
    if (message.channel_name == planning_config_.topic_config().chassis_topic()) {
      Chassis chassis;
      if (chassis.ParseFromString(message.content)) { OnChassis(chassis); }
    } else if (message.channel_name == planning_config_.topic_config().localization_topic()) {
      LocalizationEstimate localization;
      if (localization.ParseFromString(message.content)) { OnLocalization(localization); }
    } else if (message.channel_name == planning_config_.topic_config().hmi_status_topic()) {
      HMIStatus hmi_status;
      if (hmi_status.ParseFromString(message.content)) { OnHMIStatus(hmi_status); }
    } else if (message.channel_name == planning_config_.topic_config().prediction_topic()) {
      PredictionObstacles prediction_obstacles;
      if (prediction_obstacles.ParseFromString(message.content)) {
        OnPrediction(prediction_obstacles);
      }
    } else if (message.channel_name == planning_config_.topic_config().routing_response_topic()) {
      RoutingResponse routing_response;
      if (routing_response.ParseFromString(message.content)) {
        OnRoutingResponse(routing_response);
      }
    } else if (message.channel_name == planning_config_.topic_config().story_telling_topic()) {
      Stories stories;
      if (stories.ParseFromString(message.content)) { OnStoryTelling(stories); }
    } else if (message.channel_name ==
               planning_config_.topic_config().traffic_light_detection_topic()) {
...
bool MessageProcess::GetADCCurrentRoutingIndex(int*    adc_road_index,
  ...
  apollo::hdmap::HDMapUtil::BaseMapPtr()->GetLanes(pose.position(), kRadius, &lanes);
    ...
    for (int i = 0; i < routing_response_.road_size(); ++i) {
      *adc_passage_s = 0;
      for (int j = 0; j < routing_response_.road(i).passage_size(); ++j) {
        double passage_s = 0;
        for (int k = 0; k < routing_response_.road(i).passage(j).segment_size(); ++k) {
...
MessageProcess::GetCurrentLane(const apollo::common::PointENU& position) {
  constexpr double                                            kRadiusUnit = 0.1;
  std::vector<std::shared_ptr<const apollo::hdmap::LaneInfo>> lanes;
  for (int i = 1; i <= 10; ++i) {
    apollo::hdmap::HDMapUtil::BaseMapPtr()->GetLanes(position, i * kRadiusUnit, &lanes);
  ...
  for (auto& lane : lanes) {
    for (int i = 0; i < routing_response_.road_size(); ++i) {
      for (int j = 0; j < routing_response_.road(i).passage_size(); ++j) {
        for (int k = 0; k < routing_response_.road(i).passage(j).segment_size(); ++k) {
          if (lane->id().id() == routing_response_.road(i).passage(j).segment(k).id()) {
            return lane;
          }
        }
      }
...
int MessageProcess::GetADCCurrentInfo(ADCCurrentInfo* adc_curr_info) {
  ...
  adc_curr_info->adc_cur_velocity_ =
      std::make_pair(adc_cur_pose.linear_velocity().x(), adc_cur_pose.linear_velocity().y());
  adc_curr_info->adc_cur_acc_     = std::make_pair(adc_cur_pose.linear_acceleration().x(),
                                               adc_cur_pose.linear_acceleration().y());
  adc_curr_info->adc_cur_heading_ = adc_cur_pose.heading();
  return 1;
}

void MessageProcess::GenerateObstacleTrajectory(const int             frame_num,
  ...
  for (const auto& obj_traj_point : obstacle_history) {
    auto perception_obstacle_history = obstacle_trajectory->add_perception_obstacle_history();
    perception_obstacle_history->set_timestamp_sec(obj_traj_point.timestamp_sec());

    // convert position to relative coordinate
    const auto& relative_posistion = util::WorldCoordToObjCoord(
        std::make_pair(obj_traj_point.position().x(), obj_traj_point.position().y()),
    ...
    const double relative_theta =
        util::WorldAngleToObjAngle(obj_traj_point.theta(), adc_curr_info.adc_cur_heading_);
    ...
    const auto& relative_velocity = util::WorldCoordToObjCoord(
        std::make_pair(obj_traj_point.velocity().x(), obj_traj_point.velocity().y()),
    ...
    const auto& relative_acc = util::WorldCoordToObjCoord(
        std::make_pair(obj_traj_point.acceleration().x(), obj_traj_point.acceleration().y()),
    ...
    for (int i = 0; i < obj_traj_point.polygon_point_size(); ++i) {
      ...
      const auto& relative_point = util::WorldCoordToObjCoord(
          std::make_pair(obj_traj_point.polygon_point(i).x(), obj_traj_point.polygon_point(i).y()),
          adc_curr_info.adc_cur_position_, adc_curr_info.adc_cur_heading_);
...
void MessageProcess::GenerateObstaclePrediction(const int                 frame_num,
  ...
  obstacle_prediction->set_predicted_period(prediction_obstacle.predicted_period());
  obstacle_prediction->mutable_intent()->CopyFrom(prediction_obstacle.intent());
  obstacle_prediction->mutable_priority()->CopyFrom(prediction_obstacle.priority());
  ...
  for (int i = 0; i < prediction_obstacle.trajectory_size(); ++i) {
    ...
    for (int j = 0; j < obstacle_trajectory.trajectory_point_size(); ++j) {
      const auto& obstacle_trajectory_point = obstacle_trajectory.trajectory_point(j);
      ...
      if (trajectory->trajectory_point_size() > 0) {
        ...
        if (obstacle_trajectory_point.relative_time() < last_relative_time) {
          const std::string msg = absl::StrCat(
              "DISCARD prediction trajectory point: frame_num[", frame_num, "] obstacle_id[",
              obstacle_id, "] last_relative_time[", last_relative_time, "] relative_time[",
      ...
      auto path_point = trajectory_point->mutable_trajectory_point()->mutable_path_point();
      ...
      const double relative_theta = util::WorldAngleToObjAngle(
          obstacle_trajectory_point.path_point().theta(), adc_curr_info.adc_cur_heading_);
      ...
      const double timestamp_sec =
          prediction_obstacle.timestamp() + obstacle_trajectory_point.relative_time();
      ...
      tp->mutable_gaussian_info()->CopyFrom(obstacle_trajectory_point.gaussian_info());
    }
  }
}

void MessageProcess::GenerateObstacleFeature(LearningDataFrame* learning_data_frame) {
  ADCCurrentInfo adc_curr_info;
  if (GetADCCurrentInfo(&adc_curr_info) == -1) {
    const std::string msg = absl::StrCat("fail to get ADC current info: frame_num[",
  ...
  for (const auto& m : prediction_obstacles_map_) {
    ...
    GenerateObstacleTrajectory(frame_num, m.first, adc_curr_info, obstacle_feature);

    // obstacle prediction
    GenerateObstaclePrediction(frame_num, m.second, adc_curr_info, obstacle_feature);
  }
}

bool MessageProcess::GenerateLocalRouting(const int                 frame_num,
                                          RoutingResponseFeature*   local_routing,
                                          std::vector<std::string>* local_routing_lane_ids) {
  local_routing->Clear();
  local_routing_lane_ids->clear();

  if (routing_response_.road_size() == 0 || routing_response_.road(0).passage_size() == 0 ||
      routing_response_.road(0).passage(0).segment_size() == 0) {
    const std::string msg =
        absl::StrCat("DISCARD: invalid routing_response. frame_num[", frame_num, "]");
    AERROR << msg;
    if (FLAGS_planning_offline_learning) { log_file_ << msg << std::endl; }
    return false;
  }
  ...
  for (int i = 0; i < routing_response_.road_size(); ++i) {
    ...
    for (int j = 0; j < routing_response_.road(i).passage_size(); ++j) {
      ADEBUG << "   passage: segment_size[" << routing_response_.road(i).passage(j).segment_size()
             << "]";
    ...
    road_lengths.push_back(std::make_pair(routing_response_.road(i).id(), road_length));
  ...
  */
  ...
  int    adc_passage_index;
  double adc_passage_s;
  if (!GetADCCurrentRoutingIndex(&adc_road_index, &adc_passage_index, &adc_passage_s) ||
      adc_road_index < 0 || adc_passage_index < 0 || adc_passage_s < 0) {
    // reset localization history
    localizations_.clear();

    const std::string msg =
        absl::StrCat("DISCARD: fail to locate ADC on routing. frame_num[", frame_num, "]");
    AERROR << msg;
    if (FLAGS_planning_offline_learning) { log_file_ << msg << std::endl; }
    return false;
  }
  ADEBUG << "adc_road_index[" << adc_road_index << "] adc_passage_index[" << adc_passage_index
  ...
  for (int i = adc_road_index; i >= 0; --i) {
    const double road_length = (i == adc_road_index ? adc_passage_s : road_lengths[i].second);
    if (backward_length > road_length) {
      ...
      ADEBUG << "local_routing_start_road_index[" << local_routing_start_road_index
  ...
  double local_routing_end_road_s     = road_lengths[local_routing_end_road_index].second;
  ...
  for (int i = adc_road_index; i < routing_response_.road_size(); ++i) {
    const double road_length =
        (i == adc_road_index ? road_lengths[i].second - adc_passage_s : road_lengths[i].second);
    if (forwardward_length > road_length) {
      forwardward_length -= road_length;
    } else {
      local_routing_end_road_index = i;
      local_routing_end_road_s =
          (i == adc_road_index ? adc_passage_s + forwardward_length : forwardward_length);
  ...
  ADEBUG << "local_routing: start_road_index[" << local_routing_start_road_index
         << "] start_road_s[" << local_routing_start_road_s << "] end_road_index["
         << local_routing_end_road_index << "] end_road_s[" << local_routing_end_road_s << "]";

  bool local_routing_end  = false;
  int  last_passage_index = adc_passage_index;
  for (int i = local_routing_start_road_index; i <= local_routing_end_road_index; ++i) {
    ...
    for (int j = 0; j < road.passage_size(); ++j) {
      ...
      for (int k = 0; k < passage.segment_size(); ++k) {
        ...
        // first road
        if (i == local_routing_start_road_index && road_s < local_routing_start_road_s) {
          continue;
        }

        local_routing_passage->add_segment()->CopyFrom(lane_segment);
        ADEBUG << "ADD road[" << i << "] id[" << road.id() << "] passage[" << j << "] id["
               << lane_segment.id() << "] length[" << lane_segment.end_s() - lane_segment.start_s()
               << "]";
        ...
        if (i == adc_road_index) {
          ...
          if (j == adc_passage_index) {
            ...
            ADEBUG << "ADD local_routing_lane_ids: road[" << i << "] passage[" << j
                   << "]: " << lane_segment.id();
          ...
          if (road.passage_size() == 1) {
            ...
            if (i < adc_road_index) {
              ...
              if (j == adc_passage_index ||
                ...
                ADEBUG << "ADD local_routing_lane_ids: road[" << i << "] passage[" << j
                       << "] adc_passage_index[" << adc_passage_index << "] passage_size["
              ...
              if (j == last_passage_index + 1 ||
                  (j == road.passage_size() - 1 && j < last_passage_index + 1)) {
                ADEBUG << "ADD local_routing_lane_ids: road[" << i << "] passage[" << j
                       << "] last_passage_index[" << last_passage_index << "] passage_size["
                       << road.passage_size() << "]: " << lane_segment.id();
                local_routing_lane_ids->push_back(lane_segment.id());
                last_passage_index = j;
              }
        ...
        // last road
        if (i == local_routing_end_road_index && road_s >= local_routing_end_road_s) {
  ...
  if (FLAGS_planning_offline_learning) {
    for (size_t i = 0; i < local_routing_lane_ids->size(); ++i) {
      ...
      const auto&       lane = hdmap::HDMapUtil::BaseMap().GetLaneById(hdmap::MakeMapId(lane_id));
      if (lane == nullptr) {
        const std::string msg =
            absl::StrCat("DISCARD: fail to find local_routing_lane on map. frame_num[", frame_num,
                         "] lane[", lane_id, "]");
        AERROR << msg;
        if (FLAGS_planning_offline_learning) { log_file_ << msg << std::endl; }
        return false;
...
void MessageProcess::GenerateRoutingFeature(const RoutingResponseFeature&   local_routing,
  ...
  for (int i = 0; i < routing_response_.road_size(); ++i) {
    routing->mutable_routing_response()->add_road()->CopyFrom(routing_response_.road(i));
  }
  ...
  if (local_routing_lane_id_size == 0) {
    const std::string msg = absl::StrCat("empty local_routing. frame_num[", frame_num, "]");
    AERROR << msg;
    if (FLAGS_planning_offline_learning) { log_file_ << msg << std::endl; }
  }
  if (local_routing_lane_id_size > 100) {
    const std::string msg =
        absl::StrCat("LARGE local_routing. frame_num[", frame_num, "] local_routing_lane_id_size[",
...
void MessageProcess::GenerateTrafficLightDetectionFeature(LearningDataFrame* learning_data_frame) {
  auto traffic_light_detection = learning_data_frame->mutable_traffic_light_detection();
  traffic_light_detection->set_message_timestamp_sec(traffic_light_detection_message_timestamp_);
...
void MessageProcess::GenerateADCTrajectoryPoints(
    const std::list<LocalizationEstimate>& localizations, LearningDataFrame* learning_data_frame) {
  ...
  for (const auto& localization_sample : localization_samples) {
    ...
    adc_trajectory_point.set_timestamp_sec(localization_sample.measurement_time());
    ...
    const double v = std::sqrt(pose.linear_velocity().x() * pose.linear_velocity().x() +
                               pose.linear_velocity().y() * pose.linear_velocity().y());
    trajectory_point->set_v(v);

    const double a = std::sqrt(pose.linear_acceleration().x() * pose.linear_acceleration().x() +
    ...
    if (lane != nullptr) { lane_turn = lane->lane().turn(); }
    ...
    if (FLAGS_planning_offline_learning) {
      ...
      if (trajectory_point_index > 0) {
        auto& next_point =
            adc_trajectory_points[trajectory_point_index - 1].trajectory_point().path_point();
      ...
      if (HDMapUtil::BaseMap().GetClearAreas(hdmap_point, kSearchRadius, &clear_areas) == 0 &&
          clear_areas.size() > 0) {
        clear_area_id       = clear_areas.front()->id().id();
        clear_area_distance = 0.0;
      } else {
        if (!clear_area_id.empty()) { clear_area_distance += point_distance; }
      }
      ...
      if (HDMapUtil::BaseMap().GetCrosswalks(hdmap_point, kSearchRadius, &crosswalks) == 0 &&
        ...
        if (!crosswalk_id.empty()) { crosswalk_distance += point_distance; }
      }
      ...
      if (HDMapUtil::BaseMap().GetPNCJunctions(hdmap_point, kSearchRadius, &pnc_junctions) == 0 &&
          pnc_junctions.size() > 0) {
        pnc_junction_id       = pnc_junctions.front()->id().id();
        pnc_junction_distance = 0.0;
      } else {
        if (!pnc_junction_id.empty()) { pnc_junction_distance += point_distance; }
      }
      ...
      if (HDMapUtil::BaseMap().GetSignals(hdmap_point, kSearchRadius, &signals) == 0 &&
          signals.size() > 0) {
        signal_id       = signals.front()->id().id();
        signal_distance = 0.0;
      } else {
        if (!signal_id.empty()) { signal_distance += point_distance; }
      }
      if (!signal_id.empty()) {
      ...
      if (HDMapUtil::BaseMap().GetStopSigns(hdmap_point, kSearchRadius, &stop_signs) == 0 &&
          stop_signs.size() > 0) {
        stop_sign_id       = stop_signs.front()->id().id();
        stop_sign_distance = 0.0;
      } else {
        if (!stop_sign_id.empty()) { stop_sign_distance += point_distance; }
      }
      ...
      if (HDMapUtil::BaseMap().GetYieldSigns(hdmap_point, kSearchRadius, &yield_signs) == 0 &&
          yield_signs.size() > 0) {
        yield_sign_id       = yield_signs.front()->id().id();
        yield_sign_distance = 0.0;
      } else {
        if (!yield_sign_id.empty()) { yield_sign_distance += point_distance; }
      }
  ...
  if (adc_trajectory_points.size() <= 5) {
    const std::string msg =
        absl::StrCat("too few adc_trajectory_points: frame_num[", learning_data_frame->frame_num(),
...
bool MessageProcess::GenerateLearningDataFrame(LearningDataFrame* learning_data_frame) {
  ...
  learning_data_frame->set_message_timestamp_sec(localizations_.back().header().timestamp_sec());
  ...
  localization->set_message_timestamp_sec(localizations_.back().header().timestamp_sec());
  const auto& pose = localizations_.back().pose();
  ...
  localization->mutable_linear_acceleration()->CopyFrom(pose.linear_acceleration());
  ...
  // add routing
  GenerateRoutingFeature(local_routing, local_routing_lane_ids, learning_data_frame);
  ...
  ADEBUG << "MessageProcess: start_timestamp[" << start_timestamp << "] end_timestamp["
         << end_timestamp << "] time_diff_ms[" << time_diff_ms << "]";
  return true;
}
