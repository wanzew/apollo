...
class FrenetFramePath {
 public:
  ...
  explicit FrenetFramePath(const std::vector<common::FrenetFramePoint>& sl_points);
 ...
 private:
  ...
  static bool UpperBoundComparator(const double s, const common::FrenetFramePoint& p) {
    return s < p.s();
  }
