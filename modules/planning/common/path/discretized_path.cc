...
std::vector<PathPoint>::const_iterator DiscretizedPath::QueryLowerBound(const double path_s) const {
...
PathPoint DiscretizedPath::EvaluateReverse(const double path_s) const {
  ...
  if (it_upper == begin()) { return front(); }
  if (it_upper == end()) { return back(); }
...
std::vector<PathPoint>::const_iterator DiscretizedPath::QueryUpperBound(const double path_s) const {
  auto func = [](const double path_s, const PathPoint& tp) { return tp.s() < path_s; };
