...
bool DecisionData::IsValidTrajectory(const prediction::Trajectory& trajectory) {
  for (const auto& point : trajectory.trajectory_point()) {
    if (!IsValidTrajectoryPoint(point)) {
      AERROR << " TrajectoryPoint: " << trajectory.ShortDebugString() << " is NOT valid.";
      return false;
    }
  }
  return true;
...
DecisionData::DecisionData(const prediction::PredictionObstacles& prediction_obstacles,
...
std::vector<Obstacle*> DecisionData::GetObstacleByType(const VirtualObjectType& type) {
  std::lock_guard<std::mutex> lock(transaction_mutex_);

  std::unordered_set<std::string> ids = GetObstacleIdByType(type);
  if (ids.empty()) { return std::vector<Obstacle*>(); }
...
std::unordered_set<std::string> DecisionData::GetObstacleIdByType(const VirtualObjectType& type) {
  std::lock_guard<std::mutex> lock(mutex_);
  return common::util::FindWithDefault(virtual_obstacle_id_map_, type, {});
}

const std::vector<Obstacle*>& DecisionData::GetStaticObstacle() const { return static_obstacle_; }

const std::vector<Obstacle*>& DecisionData::GetDynamicObstacle() const { return dynamic_obstacle_; }

const std::vector<Obstacle*>& DecisionData::GetVirtualObstacle() const { return virtual_obstacle_; }

const std::vector<Obstacle*>& DecisionData::GetPracticalObstacle() const {
  return practical_obstacle_;
}

const std::vector<Obstacle*>& DecisionData::GetAllObstacle() const { return all_obstacle_; }
...
bool DecisionData::CreateVirtualObstacle(const ReferencePoint&    point,
  ...
  if (!reference_line_.XYToSL(point, &sl_point)) { return false; }
...
bool DecisionData::CreateVirtualObstacle(const common::math::Box2d& obstacle_box,
                                         const VirtualObjectType&   type,
                                         std::string* const         id) {
