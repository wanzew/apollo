...
LagPrediction::LagPrediction(uint32_t min_appear_num, uint32_t max_disappear_num)
  ...
  if (AdapterManager::GetPredictionConfig().message_history_limit() <
    ...
    AWARN << "Prediction adapter history limit is "
          ...
          << ", but an obstacle need to be observed at least " << min_appear_num_ << " times";
    return;
  }
}
...
void LagPrediction::GetLaggedPrediction(PredictionObstacles* obstacles) const {
  ...
  if (!AdapterManager::GetPrediction() || AdapterManager::GetPrediction()->Empty()) { return; }
  ...
  bool apply_lag =
      std::distance(prediction.begin(), prediction.end()) >= static_cast<int32_t>(min_appear_num_);
  for (const auto& iter : obstacle_lag_info) {
    if (apply_lag && iter.second.count < min_appear_num_) { continue; }
    if (apply_lag && iter.second.last_observed_seq > max_disappear_num_) { continue; }
    AddObstacleToPrediction(timestamp - iter.second.last_observed_time, *(iter.second.obstacle_ptr),
                            obstacles);
  }
}
...
void LagPrediction::AddObstacleToPrediction(double                                delay_sec,
  ...
  obstacle->mutable_perception_obstacle()->CopyFrom(history_obstacle.perception_obstacle());
  for (const auto& hist_trajectory : history_obstacle.trajectory()) {
    ...
    for (const auto& hist_point : hist_trajectory.trajectory_point()) {
      if (hist_point.relative_time() < delay_sec) { continue; }
