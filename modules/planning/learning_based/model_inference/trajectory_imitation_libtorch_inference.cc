...
bool TrajectoryImitationLibtorchInference::LoadCNNModel() {
  ...
  try {
    auto torch_output_tensor = model_.forward(torch_inputs).toTensor().to(torch::kCPU);
...
bool TrajectoryImitationLibtorchInference::LoadCNNLSTMModel() {
  ...
  torch_inputs.push_back(c10::ivalue::Tuple::create(
      {std::move(input_feature_tensor.to(device_)), std::move(current_v.to(device_))}));
  try {
    auto torch_output_tensor = model_.forward(torch_inputs).toTensor().to(torch::kCPU);
  } catch (const c10::Error& e) {
...
bool TrajectoryImitationLibtorchInference::LoadModel() {
  ...
  switch (config_.model_type()) {
    case LearningModelInferenceTaskConfig::CNN: {
      if (!LoadCNNModel()) { return false; }
      break;
    }
    case LearningModelInferenceTaskConfig::CNN_LSTM: {
      if (!LoadCNNLSTMModel()) { return false; }
      break;
    }
    default: {
...
void TrajectoryImitationLibtorchInference::output_postprocessing(
    const at::Tensor& torch_output_tensor, LearningDataFrame* const learning_data_frame) {
  const int    past_points_size = learning_data_frame->adc_trajectory_point_size();
  const auto&  cur_traj_point   = learning_data_frame->adc_trajectory_point(past_points_size - 1);
  ...
  ADEBUG << "cur_x[" << cur_x << "], cur_y[" << cur_y << "], cur_heading[" << cur_heading
         << "], cur_v[" << cur_traj_point.trajectory_point().v() << "]";
  ...
  for (int i = 0; i < torch_output_tensor.size(1); ++i) {
    ...
    ADEBUG << "dx[" << dx << "], dy[" << dy << "], dtheta[" << dtheta << "], v[" << v << "]";
    ...
    const double heading = apollo::common::math::NormalizeAngle(dtheta + cur_heading);

    auto* traj_point = learning_data_frame->mutable_output()->add_adc_future_trajectory_point();
    ...
    traj_point->mutable_trajectory_point()->mutable_path_point()->set_theta(heading);
    traj_point->mutable_trajectory_point()->set_v(v);
    traj_point->mutable_trajectory_point()->set_relative_time(delta_t * (i + 1));
  }
}
...
bool TrajectoryImitationLibtorchInference::DoCNNMODELInference(
  ...
  if (!BirdviewImgFeatureRenderer::Instance()->RenderMultiChannelEnv(*learning_data_frame,
                                                                     &input_feature)) {
  ...
  ADEBUG << "trajectory imitation model input renderer used time: " << rendering_diff.count() * 1000
         << " ms.";
  ...
  torch::Tensor input_feature_tensor = torch::from_blob(
      ...
      {1, input_feature_float.rows, input_feature_float.cols, input_feature_float.channels()});
  ...
  std::vector<torch::jit::IValue> torch_inputs;
  torch_inputs.push_back(input_feature_tensor.to(device_));
  at::Tensor torch_output_tensor = model_.forward(torch_inputs).toTensor().to(torch::kCPU);

  auto                          inference_end_time = std::chrono::system_clock::now();
  std::chrono::duration<double> inference_diff     = inference_end_time - inference_start_time;
  ADEBUG << "trajectory imitation model inference used time: " << inference_diff.count() * 1000
         << " ms.";
...
bool TrajectoryImitationLibtorchInference::DoCNNLSTMMODELInference(
  ...
  const auto& current_traj_point =
      learning_data_frame
          ->adc_trajectory_point(learning_data_frame->adc_trajectory_point_size() - 1)
          .trajectory_point();
  ...
  std::vector<torch::jit::IValue> torch_inputs;
  torch_inputs.push_back(c10::ivalue::Tuple::create(
      {std::move(input_feature_tensor.to(device_)), std::move(current_v_tensor.to(device_))}));
  at::Tensor torch_output_tensor = model_.forward(torch_inputs).toTensor().to(torch::kCPU);
...
bool TrajectoryImitationLibtorchInference::DoInference(
  ...
  switch (config_.model_type()) {
    case LearningModelInferenceTaskConfig::CNN: {
      if (!DoCNNMODELInference(learning_data_frame)) { return false; }
      break;
    }
    case LearningModelInferenceTaskConfig::CNN_LSTM: {
      if (!DoCNNLSTMMODELInference(learning_data_frame)) { return false; }
      break;
    }
    default: {
      AERROR << "Configured model type not defined and implemented";
