...
double
AutotuningSpeedMLPModel::Evaluate(const autotuning::TrajectoryFeature& trajectory_feature) const {
  return 0.0;
}
...
void AutotuningSpeedMLPModel::FlattenFeatures(const autotuning::TrajectoryFeature& feature,
  ...
  for (int i = 0; i < row_count; ++i) {
    FlattenFeatures(feature.point_feature(i).speed_input_feature(), i, flat_feature);
  }
}
...
void AutotuningSpeedMLPModel::FlattenFeatures(
  ...
  for (int i = 0; i < speed_point_feature.overtake_obs_feature_size(); ++i) {
    const auto& overtake_obs_feature = speed_point_feature.overtake_obs_feature(i);
  ...
  for (int i = 0; i < speed_point_feature.virtual_obs_feature_size(); ++i) {
    const auto& virtual_obs_feature = speed_point_feature.virtual_obs_feature(i);
  ...
  // looping obstacles to find the value minimize the distance
  for (int i = 0; i < speed_point_feature.sidepass_front_obs_feature_size(); ++i) {
    const auto& sidepass_front_obs_feature = speed_point_feature.sidepass_front_obs_feature(i);
    if (sidepass_front_distance > sidepass_front_obs_feature.longitudinal_distance()) {
      sidepass_front_distance = sidepass_front_obs_feature.longitudinal_distance();
      sidepass_front_v_rel    = sidepass_front_obs_feature.relative_v();
    }
  }
  // looping obstacles to find the value minimize the distance
  for (int i = 0; i < speed_point_feature.sidepass_rear_obs_feature_size(); ++i) {
    const auto& sidepass_rear_obs_feature = speed_point_feature.sidepass_rear_obs_feature(i);
    if (sidepass_rear_distance > sidepass_rear_obs_feature.longitudinal_distance()) {
      sidepass_rear_distance = sidepass_rear_obs_feature.longitudinal_distance();
