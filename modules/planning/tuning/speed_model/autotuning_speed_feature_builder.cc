...
Status AutotuningSpeedFeatureBuilder::BuildFeature(
  ...
  if (n != raw_feature.point_feature_size()) {
    const std::string msg =
        absl::StrCat("raw and input feature size mismatch. raw: ", raw_feature.point_feature_size(),
                     "; input: ", n);
    AERROR << msg;
    return Status(ErrorCode::PLANNING_ERROR, msg);
  }
...
Status AutotuningSpeedFeatureBuilder::BuildPointFeature(
  ...
  speed_feature->set_speed_limit(raw_point_feature.speed_feature().speed_limit());
  ...
  for (int i = 0; i < raw_point_feature.speed_feature().virtual_decision_size(); ++i) {
    const auto& obj_decision   = raw_point_feature.speed_feature().virtual_decision(i);
  ...
  for (int i = 0; i < raw_point_feature.speed_feature().sidepass_front_size(); ++i) {
    const auto& obj_decision   = raw_point_feature.speed_feature().sidepass_front(i);
    auto*       input_decision = speed_feature->add_sidepass_front_obs_feature();
    map_sidepass_obs_feature(obj_decision, input_decision);
  }
  for (int i = 0; i < raw_point_feature.speed_feature().sidepass_rear_size(); ++i) {
    const auto& obj_decision   = raw_point_feature.speed_feature().sidepass_rear(i);
...
void AutotuningSpeedFeatureBuilder::map_obstacle_feature(
  ...
  input_feature->set_longitudinal_distance(std::fabs(obj_raw_feature.relative_s()));
