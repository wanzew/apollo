  ...
  if (!IsCloseEnough(back_node.EndS(), back_node.FullLength())) {
    if (!range_manager.Find(back_node.GetTopoNode())) {
      if (IsCloseEnough(next_passage.nodes.back().EndS(), next_passage.nodes.back().FullLength())) {
        ...
        double temp_s = next_passage.nodes.back().EndS() / next_passage.nodes.back().FullLength() *
...
void LaneNodesToPassageRegion(const std::vector<NodeWithRange>::const_iterator first,
...
double CalculateDistance(const std::vector<NodeWithRange>& nodes) {
  ...
  for (size_t i = 1; i < nodes.size(); ++i) {
    auto edge = nodes.at(i - 1).GetTopoNode()->GetOutEdgeTo(nodes.at(i).GetTopoNode());
    if (edge->Type() != TET_FORWARD) { continue; }
...
void ResultGenerator::AddRoadSegment(const std::vector<PassageInfo>&            passages,
  ...
  for (std::size_t i = start.first; i <= end.first && i < passages.size(); ++i) {
    auto* passage    = road->add_passage();
    auto  start_iter = passages[i].nodes.cbegin() + (i == start.first ? start.second : 0);
    auto  end_iter =
        passages[i].nodes.cbegin() + (i == end.first ? end.second : passages[i].nodes.size());
...
void ResultGenerator::CreateRoadSegments(const std::vector<PassageInfo>& passages,
  ...
  for (std::size_t i = 0; i < passages.size(); ++i) {
    ...
    for (std::size_t j = 0; j < curr_nodes.size(); ++j) {
      if ((i + 1 < passages.size() &&
        ...
        if (in_change_lane) { AddRoadSegment(passages, start_index, {i, j}, result); }
  ...
  if (in_change_lane) {
    AddRoadSegment(passages, start_index, {passages.size() - 1, passages.back().nodes.size()},
                   result);
  }
}
