...
bool ResultGenerator::ExtractBasicPassages(const std::vector<NodeWithRange>& nodes,
  ...
  for (size_t i = 1; i < nodes.size(); ++i) {
    ...
    if (edge == nullptr) {
      AERROR << "Get null pointer to edge from " << nodes.at(i - 1).LaneId() << " to "
             << nodes.at(i).LaneId();
      return false;
    }
    if (edge->Type() == TET_LEFT || edge->Type() == TET_RIGHT) {
      auto change_lane_type = LEFT;
      if (edge->Type() == TET_RIGHT) { change_lane_type = RIGHT; }
...
bool ResultGenerator::IsReachableFromWithChangeLane(const TopoNode*    from_node,
...
void ResultGenerator::ExtendBackward(const TopoRangeManager& range_manager,
    ...
    if (!range_manager.Find(front_node.GetTopoNode())) {
      if (IsCloseEnough(prev_passage.nodes.front().StartS(), 0.0)) {
        ...
        double temp_s = prev_passage.nodes.front().StartS() /
                        prev_passage.nodes.front().FullLength() * front_node.FullLength();
  ...
  while (allowed_to_explore) {
    ...
    for (const auto& edge : curr_passage->nodes.front().GetTopoNode()->InFromPreEdge()) {
      const auto& pred_node = edge->FromNode();

      // if pred node has been inserted
      if (ContainsKey(node_set_of_curr_passage, pred_node)) { continue; }
      // if pred node is reachable from prev passage
      NodeWithRange reachable_node(pred_node, 0, 1);
      if (IsReachableToWithChangeLane(pred_node, prev_passage, &reachable_node)) {
...
void ResultGenerator::ExtendForward(const TopoRangeManager& range_manager,
    ...
    if (!range_manager.Find(back_node.GetTopoNode())) {
      if (IsCloseEnough(next_passage.nodes.back().EndS(), next_passage.nodes.back().FullLength())) {
        ...
        double adjusted_end_s = next_passage.nodes.back().EndS() /
                                next_passage.nodes.back().FullLength() * back_node.FullLength();
        if (adjusted_end_s > back_node.StartS()) {
          ...
          ADEBUG << "ExtendForward: orig_end_s[" << back_node.EndS() << "] adjusted_end_s["
                 << adjusted_end_s << "]";
        }
      }
    } else {
  ...
  while (allowed_to_explore) {
    ...
    for (const auto& edge : curr_passage->nodes.back().GetTopoNode()->OutToSucEdge()) {
      ...
      if (ContainsKey(node_set_of_curr_passage, succ_node)) { continue; }
      ...
      if (IsReachableFromWithChangeLane(succ_node, next_passage, &reachable_node)) {
        ...
        if (succ_range != nullptr && !succ_range->empty()) {
          ...
          if (IsCloseEnough(reachable_node.EndS(), reachable_node.FullLength())) {
            ...
            double push_end_s =
                reachable_node.EndS() / reachable_node.FullLength() * succ_node->Length();
...
void ResultGenerator::ExtendPassages(const TopoRangeManager&         range_manager,
  ...
  for (int i = 0; i < passage_num; ++i) {
    ...
    if (i > 0) { ExtendBackward(range_manager, passages->at(i - 1), &(passages->at(i))); }
  }
...
void LaneNodesToPassageRegion(const std::vector<NodeWithRange>::const_iterator begin,
...
double CalculateDistance(const std::vector<NodeWithRange>& nodes) {
  ...
  for (size_t i = 1; i < nodes.size(); ++i) {
    auto edge = nodes.at(i - 1).GetTopoNode()->GetOutEdgeTo(nodes.at(i).GetTopoNode());
    if (edge == nullptr || edge->Type() != TET_FORWARD) { continue; }
...
void PrintDebugInfo(const std::string&                             road_id,
  ...
  for (size_t j = 0; j < nodes.size(); ++j) {
    ...
    for (const auto& node : nodes[j]) {
      AINFO << "\t\t" << node.LaneId() << "   (" << node.StartS() << ", " << node.EndS() << ")";
    }
  }
}

bool ResultGenerator::GeneratePassageRegion(const std::string&                map_version,
                                            const RoutingRequest&             request,
                                            const std::vector<NodeWithRange>& nodes,
                                            const TopoRangeManager&           range_manager,
                                            RoutingResponse* const            result) {
  if (!GeneratePassageRegion(nodes, range_manager, result)) { return false; }
...
bool ResultGenerator::GeneratePassageRegion(const std::vector<NodeWithRange>& nodes,
                                            const TopoRangeManager&           range_manager,
                                            RoutingResponse* const            result) {
  std::vector<PassageInfo> passages;
  if (!ExtractBasicPassages(nodes, &passages)) { return false; }
...
void ResultGenerator::AddRoadSegment(const std::vector<PassageInfo>&            passages,
  ...
  for (std::size_t i = start_index.first; i <= end_index.first && i < passages.size(); ++i) {
    ...
    ADEBUG << "start node: " << node_begin_iter->LaneId() << ": " << node_begin_iter->StartS()
           << "; " << node_begin_iter->EndS();
    const size_t node_end_index =
        (i == end_index.first ? std::min(end_index.second, passages[i].nodes.size() - 1) :
                                passages[i].nodes.size() - 1);
    const auto node_last_iter = passages[i].nodes.cbegin() + node_end_index;
    ADEBUG << "last node: " << node_last_iter->LaneId() << ": " << node_last_iter->StartS() << "; "
           << node_last_iter->EndS();
...
void ResultGenerator::CreateRoadSegments(const std::vector<PassageInfo>& passages,
                                         RoutingResponse*                result) {
  ...
  for (std::size_t i = 0; i < passages.size(); ++i) {
    ...
    for (std::size_t j = 0; j < curr_nodes.size(); ++j) {
      if ((i + 1 < passages.size() &&
           IsReachableToWithChangeLane(curr_nodes[j].GetTopoNode(), passages[i + 1],
                                       &fake_node_range)) ||
          (i > 0 && IsReachableFromWithChangeLane(curr_nodes[j].GetTopoNode(), passages[i - 1],
                                                  &fake_node_range))) {
        if (!in_change_lane) {
          start_index    = {i, j};
          in_change_lane = true;
        ...
        ADEBUG << "start_index(" << i << ", " << j << ") end_index(" << i << ", " << j << ")";
  ...
  if (in_change_lane) {
    ADEBUG << "start_index(" << start_index.first << ", " << start_index.second << ") end_index("
           << passages.size() - 1 << ", " << passages.back().nodes.size() - 1 << ")";
    AddRoadSegment(passages, start_index, {passages.size() - 1, passages.back().nodes.size() - 1},
                   result);
  }
}
