...
struct SearchNode {
  ...
  bool operator==(const SearchNode& node) const { return topo_node == node.topo_node; }
};
...
bool AdjustLaneChangeBackward(std::vector<const TopoNode*>* const result_node_vec) {
  for (int i = static_cast<int>(result_node_vec->size()) - 2; i > 0; --i) {
    ...
    if (from_to_edge->Type() != TopoEdgeType::TET_FORWARD) {
      if (base_node->EndS() - base_node->StartS() < from_node->EndS() - from_node->StartS()) {
        continue;
      }
      ...
      for (const auto* edge : out_edges) {
        ...
        if (candidate_node == from_node) { continue; }
      ...
      if (largest_node == nullptr) { return false; }
      if (largest_node != from_node) { result_node_vec->at(i) = largest_node; }
    }
  }
  return true;
}
...
bool AdjustLaneChangeForward(std::vector<const TopoNode*>* const result_node_vec) {
  for (size_t i = 1; i < result_node_vec->size() - 1; ++i) {
    ...
    if (from_to_edge->Type() != TopoEdgeType::TET_FORWARD) {
      if (base_node->EndS() - base_node->StartS() < to_node->EndS() - to_node->StartS()) {
        continue;
      }
      std::vector<const TopoNode*> candidate_set;
      ...
      for (const auto* edge : in_edges) {
        ...
        if (candidate_node == to_node) { continue; }
      ...
      const auto* largest_node = GetLargestNode(candidate_set);
      if (largest_node == nullptr) { return false; }
      if (largest_node != to_node) { result_node_vec->at(i) = largest_node; }
    }
  }
  return true;
}
...
bool AdjustLaneChange(std::vector<const TopoNode*>* const result_node_vec) {
  if (result_node_vec->size() < 3) { return true; }
...
bool AStarStrategy::Search(const TopoGraph*                  graph,
  ...
  while (!open_set_detail.empty()) {
    ...
    for (const auto* edge : next_edge_set) {
      ...
      if (open_set_.count(to_node) != 0 && tentative_g_score >= g_score_[to_node]) { continue; }
...
double AStarStrategy::GetResidualS(const TopoNode* node) {
  ...
  if (iter != enter_s_.end()) {
    if (iter->second > node->EndS()) { return 0.0; }
    start_s = iter->second;
  } else {
  ...
  if (succ_node != nullptr) { end_s = succ_node->EndS(); }
  return (end_s - start_s);
}

double AStarStrategy::GetResidualS(const TopoEdge* edge, const TopoNode* to_node) {
  if (edge->Type() == TopoEdgeType::TET_FORWARD) { return std::numeric_limits<double>::max(); }
