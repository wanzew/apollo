...
bool TopoNode::IsOutRangeEnough(const std::vector<NodeSRange>& range_vec,
                                double                         start_s,
                                double                         end_s) {
  if (!NodeSRange::IsEnoughForChangeLane(start_s, end_s)) { return false; }
  ...
  if (start_index < 0 || end_index < 0) { return false; }
  if (index_diff > 1) { return true; }
  ...
  if (index_diff == 0) { return NodeSRange::IsEnoughForChangeLane(pre_s_s, suc_e_s); }
  return false;
}
...
void TopoNode::Init() {
  ...
  ConvertOutRange(pb_node_.left_out(), start_s_, end_s_, &left_out_sorted_range_,
  ...
  ConvertOutRange(pb_node_.right_out(), start_s_, end_s_, &right_out_sorted_range_,
...
const hdmap::Curve& TopoNode::CentralCurve() const { return pb_node_.central_curve(); }

const common::PointENU& TopoNode::AnchorPoint() const { return anchor_point_; }

const std::vector<NodeSRange>& TopoNode::LeftOutRange() const { return left_out_sorted_range_; }

const std::vector<NodeSRange>& TopoNode::RightOutRange() const { return right_out_sorted_range_; }
...
const std::unordered_set<const TopoEdge*>& TopoNode::InFromLeftOrRightEdge() const {
...
const std::unordered_set<const TopoEdge*>& TopoNode::OutToLeftOrRightEdge() const {
...
bool TopoNode::IsOverlapEnough(const TopoNode* sub_node, const TopoEdge* edge_for_type) const {
  if (edge_for_type->Type() == TET_LEFT) {
    return (is_left_range_enough_ &&
            IsOutRangeEnough(left_out_sorted_range_, sub_node->StartS(), sub_node->EndS()));
  }
  if (edge_for_type->Type() == TET_RIGHT) {
    return (is_right_range_enough_ &&
            IsOutRangeEnough(right_out_sorted_range_, sub_node->StartS(), sub_node->EndS()));
  }
...
void TopoNode::AddInEdge(const TopoEdge* edge) {
  if (edge->ToNode() != this) { return; }
  if (in_edge_map_.count(edge->FromNode()) != 0) { return; }
  switch (edge->Type()) {
    case TET_LEFT:
    ...
    case TET_RIGHT:
      ...
      in_from_left_or_right_edge_set_.insert(edge);
      break;
    default: in_from_pre_edge_set_.insert(edge); break;
  }
...
void TopoNode::AddOutEdge(const TopoEdge* edge) {
  if (edge->FromNode() != this) { return; }
  if (out_edge_map_.count(edge->ToNode()) != 0) { return; }
  switch (edge->Type()) {
    case TET_LEFT:
    ...
    case TET_RIGHT:
      ...
      out_to_left_or_right_edge_set_.insert(edge);
      break;
    default: out_to_suc_edge_set_.insert(edge); break;
  }
...
TopoEdge::TopoEdge(const Edge& edge, const TopoNode* from_node, const TopoNode* to_node)
    : pb_edge_(edge)
...
const std::string& TopoEdge::FromLaneId() const { return pb_edge_.from_lane_id(); }

const std::string& TopoEdge::ToLaneId() const { return pb_edge_.to_lane_id(); }

TopoEdgeType TopoEdge::Type() const {
  if (pb_edge_.direction_type() == Edge::LEFT) { return TET_LEFT; }
  if (pb_edge_.direction_type() == Edge::RIGHT) { return TET_RIGHT; }
  return TET_FORWARD;
}
}  // namespace routing
}  // namespace apollo
