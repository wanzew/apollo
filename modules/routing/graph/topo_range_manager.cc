...
bool TopoRangeManager::Find(const TopoNode* node) const { return ContainsKey(range_map_, node); }
...
void TopoRangeManager::PrintDebugInfo() const {
  for (const auto& map : range_map_) {
    for (const auto& range : map.second) {
      AINFO << "black lane id: " << map.first->LaneId() << ", start s: " << range.StartS()
            << ", end s: " << range.EndS();
    }
  }
}
...
void TopoRangeManager::SortAndMerge() {
  for (auto& iter : range_map_) {
    ...
    iter.second.clear();
    iter.second.insert(iter.second.begin(), merged_range_vec.begin(), merged_range_vec.end());
  }
}
