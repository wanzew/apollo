...
double GetLengthbyRate(double cur_s, double cur_total_length, double target_length) {
...
void NodeCreator::GetPbNode(const Lane&          lane,
...
void NodeCreator::AddOutBoundary(const LaneBoundary&                 bound,
                                 double                              lane_length,
                                 RepeatedPtrField<CurveRange>* const out_range) {
  for (int i = 0; i < bound.boundary_type_size(); ++i) {
    if (!IsAllowedOut(bound.boundary_type(i))) { continue; }
    CurveRange* range = out_range->Add();
    range->mutable_start()->set_s(
        GetLengthbyRate(bound.boundary_type(i).s(), bound.length(), lane_length));
    if (i != bound.boundary_type_size() - 1) {
      range->mutable_end()->set_s(
          GetLengthbyRate(bound.boundary_type(i + 1).s(), bound.length(), lane_length));
    } else {
...
void NodeCreator::InitNodeInfo(const Lane&          lane,
                               const std::string&   road_id,
                               Node* const          node,
                               const RoutingConfig* routingconfig) {
  ...
  if (!lane.has_junction_id() || lane.left_neighbor_forward_lane_id_size() > 0 ||
      lane.right_neighbor_forward_lane_id_size() > 0) {
    node->set_is_virtual(false);
  }
}

void NodeCreator::InitNodeCost(const Lane&          lane,
                               Node* const          node,
                               const RoutingConfig* routingconfig) {
  double lane_length = GetLaneLength(lane);
  double speed_limit = (lane.has_speed_limit()) ? lane.speed_limit() : routingconfig->base_speed();
