...
bool GraphCreator::Create() {
  ...
  for (const auto& lane : pbmap_.lane()) {
    ...
    if (iter != road_id_map_.end()) {
      NodeCreator::GetPbNode(lane, iter->second, graph_.add_node(), routing_conf_);
    } else {
  ...
  if (!EndWith(dump_topo_file_path_, ".bin") && !EndWith(dump_topo_file_path_, ".txt")) {
...
void GraphCreator::AddEdge(const Node&                 from_node,
  ...
  for (const auto& to_id : to_node_vec) {
    ...
    EdgeCreator::GetPbEdge(from_node, to_node, type, graph_.add_edge(), routing_conf_);
  }
}
