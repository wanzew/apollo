...
class ChannelVerifyAgent {
 ...
 private:
  void StartCheck(ChannelVerifyRequest* request, ChannelVerifyResponse* response);
  void AsyncCheck(const std::string& records_path);
  void DoCheck(const std::string& records_path);
  void CheckResult(ChannelVerifyRequest* request, ChannelVerifyResponse* response);
  void StopCheck(ChannelVerifyRequest* request, ChannelVerifyResponse* response);
  void Reset();
  void SetState(ChannelVerifyAgentState state);
  ChannelVerifyAgentState GetState() const;
  int                     AddTopicLack(VerifyResult*                   result,
                                       const std::string&              record_path,
                                       std::vector<std::string> const& lack_channels);
  int                     AddInadequateRate(VerifyResult*                                           result,
