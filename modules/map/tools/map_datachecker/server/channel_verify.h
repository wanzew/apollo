...
class ChannelVerify {
 ...
 private:
  ...
  std::shared_ptr<CyberRecordInfo> GetRecordInfo(const std::string& record_path) const;
  int                              IncrementalCheck(const std::vector<std::string>& records_path);
  std::vector<std::string> GetRecordsPath(const std::string& record_dir_or_record_full_path) const;
  bool                     IsRecordChecked(const std::string& record_path);
  OneRecordChannelCheckResult CheckRecordChannels(const std::string& record_path);
  void                        Reset();
