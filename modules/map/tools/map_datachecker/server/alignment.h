...
class Alignment {
 public:
  ...
  virtual void UpdateBadPoseInfo(const FramePose& pose) { UpdatePoseInfo(pose, sp_bad_pose_info_); }
  ...
  virtual bool IsGoodPose(const std::vector<FramePose>& poses, int pose_index) {
    ...
    if (sp_conf_->position_type_range.find(position_type) != sp_conf_->position_type_range.end() &&
        diff_age >= sp_conf_->diff_age_range.first && diff_age <= sp_conf_->diff_age_range.second &&
 ...
 protected:
  ...
  int TimeToIndex(const std::vector<FramePose>& poses, double time) {
    ...
    if (size == 0 || time <= 0) { return -1; }

    for (size_t i = 0; i < size; ++i) {
      if (poses[i].time_stamp >= time) { return static_cast<int>(i); }
    }
