...
double LapsChecker::GetConfidence() {
  ...
  AINFO << "current confidence: " << res << ",confidence thresh:" << sp_conf_->laps_rate_thresh;
...
void LapsChecker::DoCheck() {
  ...
  ret = CheckParams();
  if (ret < 0) { AINFO << "check_params failed"; }
  SetProgress(0.1);

  AINFO << "check->setup_grids_map";
  ret = SetupGridsMap();
  if (ret < 0) { AINFO << "setup_grids_map failed"; }
  SetProgress(0.5);
  AINFO << "check->setup_grids_map done";

  AINFO << "check->check_laps";
  ret = CheckLaps();
  if (ret < 0) { AINFO << "check_laps failed"; }
  SetProgress(1.0);
...
int LapsChecker::CheckParams() {
  ...
  if (n_pose < sp_conf_->laps_frames_thresh) { return -1; }
  return 0;
}
    ...
    for (int x = 0; x < width; x++) {
      ...
      for (size_t i = 0; i < t_size; ++i) {
        ...
        for (size_t j = 1; j < stamps.size(); ++j) {
          if (stamps[j] - stamps[j - 1] > thresh_in_sec) { segment++; }
        }
        if (segment <= possible_max_laps_) { confidence_[segment] += 1; }
      }
    }
  }
...
int LapsChecker::GatherTimestamps(std::vector<double>* sp_stamps,
                                  double               alpha,
                                  int                  center_x,
                                  int                  center_y) {
  ...
  const size_t end_y   = std::min(static_cast<int>(height) - 1, center_y + search_r);
  const size_t start_x = std::max(0, center_x - search_r);
  const size_t end_x   = std::min(static_cast<int>(width) - 1, center_x + search_r);
        ...
        if (std::abs(alpha - grid[i].alpha) < sp_conf_->laps_alpha_err_thresh) {
          ...
          for (size_t j = 0; j < idxs.size(); ++j) {
            if (idxs[j] >= static_cast<int>(poses_.size())) {
              AINFO << "index error, index: " << idxs[j] << ", pose size: " << poses_.size();
            } else {
...
int LapsChecker::GetMinMax() {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (tx < minx_) { minx_ = tx; }
    if (tx > maxx_) { maxx_ = tx; }
    if (ty < miny_) { miny_ = ty; }
    if (ty > maxy_) { maxy_ = ty; }
  }
  return 0;
}
...
int LapsChecker::GetPassedGrid(int               pose_index,
...
double LapsChecker::Slope(double x1, double y1, double x2, double y2) {
  if (std::abs(x1 - x2) < 1e-6) { return std::numeric_limits<double>::max(); }
  if (std::abs(y1 - y2) < 1e-6) { return 0.0; }
  return (y2 - y1) / (x2 - x1);
}
