...
grpc::Status ChannelVerifyAgent::ProcessGrpcRequest(grpc::ServerContext*   context,
  ...
  switch (request->cmd()) {
    ...
    case CmdType::STOP:
      ...
      break;
    default: response->set_code(ErrorCode::ERROR_REQUEST); AERROR << "command error";
  }
...
void ChannelVerifyAgent::AsyncCheck(const std::string& records_path) {
  ...
  std::thread doctor_strange([=]() {
    ...
    while (true) {
      ...
      if (std::this_thread::get_id() != check_thread_id_) { break; }
    }
    stopped_ = true;
  });
...
int ChannelVerifyAgent::AddTopicLack(VerifyResult*                   result,
...
FrameRate* ChannelVerifyAgent::FindRates(VerifyResult* result, const std::string& channel) {
  for (FrameRate& rate : *result->mutable_rates()) {
    if (rate.topic() == channel) { return &rate; }
  }
  return nullptr;
}
...
void ChannelVerifyAgent::CheckResult(ChannelVerifyRequest*  request,
  ...
  for (CheckResultIterator it = sp_check_result_->begin(); it != sp_check_result_->end(); ++it) {
    int res = 0;
    // write rate
    res = AddInadequateRate(result, it->record_path, it->inadequate_rate);
    if (res > 0) { response->set_code(ErrorCode::ERROR_CHANNEL_VERIFY_RATES_ABNORMAL); }
    // write topic lack
    res = AddTopicLack(result, it->record_path, it->lack_channels);
    if (res > 0) { response->set_code(ErrorCode::ERROR_CHANNEL_VERIFY_TOPIC_LACK); }
  }
}

void ChannelVerifyAgent::StopCheck(ChannelVerifyRequest* request, ChannelVerifyResponse* response) {
  std::lock_guard<std::mutex> guard(stop_mutex_);
  need_stop_ = true;
  response->set_code(ErrorCode::SUCCESS);
  SetState(ChannelVerifyAgentState::IDLE);
  VerifyResult* result = response->mutable_result();
  if (sp_check_result_ == nullptr) { return; }
  for (CheckResultIterator it = sp_check_result_->begin(); it != sp_check_result_->end(); ++it) {
    int res = 0;
    // write rate
    res = AddInadequateRate(result, it->record_path, it->inadequate_rate);
...
void ChannelVerifyAgent::SetState(ChannelVerifyAgentState state) { state_ = state; }
