  ...
  if (config_.lane_source() == NavigationLaneConfig::OFFLINE_GENERATED && navigation_line_num > 0) {
    ...
    for (auto iter = navigation_path_list_.begin(); iter != navigation_path_list_.end(); ++iter) {
      ...
      if (prev_iter != navigation_path_list_.end()) {
        ...
        width = common::math::Clamp(width, FLAGS_min_lane_half_width, FLAGS_max_lane_half_width);
...
void NavigationLane::MergeNavigationLineAndLaneMarker(const int line_index, common::Path* path) {
  CHECK_NOTNULL(path);
  ...
  for (int i = 0; i < path->path_point_size(); ++i) {
    ...
    *point = common::util::GetWeightedAverageOfTwoPathPoints(*point, lane_maker_point, kWeight,
                                                             1 - kWeight);
  }
}
...
bool NavigationLane::ConvertNavigationLineToPath(const int line_index, common::Path* path) {
  CHECK_NOTNULL(path);
  ...
  path->set_name("Path from navigation line index " + std::to_string(line_index));
  ...
  for (int i = std::max(0, current_project_index - 3); i < navigation_path.path_point_size(); ++i) {
    ...
    double flu_x = 0.0;
    double flu_y = 0.0;
    common::math::RotateAxis(original_pose_.heading(), enu_x, enu_y, &flu_x, &flu_y);

    point->set_x(flu_x);
    point->set_y(flu_y);
    point->set_theta(common::math::NormalizeAngle(common::math::NormalizeAngle(point->theta()) -
                                                  original_pose_.heading()));
    const double accumulated_s = navigation_path.path_point(i).s() - ref_s;
    point->set_s(accumulated_s);

    if (accumulated_s > FLAGS_max_len_from_navigation_line) { break; }
  }
...
ProjIndexPair NavigationLane::UpdateProjectionIndex(const common::Path& path, int line_index) {
  ...
  if (FLAGS_enable_cyclic_rerouting) {
    ...
    if (current_project_index > near_end_size && current_project_index < path_size) {
      if (DistanceXY(path.path_point(0), path.path_point(current_project_index)) <
  ...
  if (check_distance_func(index)) { return std::make_pair(index, min_d); }
...
void NavigationLane::ConvertLaneMarkerToPath(const perception::LaneMarkers& lane_marker,
  ...
  double path_c2 =
      (left_lane.c2_curvature() * left_quality + right_lane.c2_curvature() * right_quality) /
      quality_divider;
  ...
  perceived_left_width_ =
      (std::fabs(left_lane.c0_position()) + std::fabs(right_lane.c0_position())) / 2.0;
  perceived_left_width_  = common::math::Clamp(perceived_left_width_, FLAGS_min_lane_half_width,
                                              FLAGS_max_lane_half_width);
  perceived_right_width_ = perceived_left_width_;
}

bool NavigationLane::CreateMap(const MapGenerationParam& map_config, MapMsg* map_msg) const {
  ...
  auto create_map_func = [&](const NaviPathTuple& navi_path_tuple) {
    ...
    lane->mutable_id()->set_id(std::to_string(navi_path->path_priority()) + "_" + path.name());
  ...
  for (int i = 0; i < lane_num; ++i) {
    ...
    if (i > 0) { lane->add_left_neighbor_forward_lane_id()->CopyFrom(hdmap->lane(i - 1).id()); }
    if (i < lane_num - 1) {
