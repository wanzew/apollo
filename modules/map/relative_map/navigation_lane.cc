...
bool CreateSingleLaneMap(
  ...
  lane->mutable_id()->set_id(absl::StrCat(navi_path->path_priority(), "_", path.name()));
  ...
  if (FLAGS_relative_map_generate_left_boundray) {
    ...
    left_segment = left_boundary->mutable_curve()->add_segment()->mutable_line_segment();
  }
  ...
  auto* right_segment = right_boundary->mutable_curve()->add_segment()->mutable_line_segment();
  ...
  for (const auto& path_point : path.path_point()) {
    ...
    if (FLAGS_relative_map_generate_left_boundray) {
      ...
      left_segment->add_point()->CopyFrom(
          *point + lane_left_width * Vec2d::CreateUnitVec2d(path_point.theta() + M_PI_2));
    }
    ...
    right_segment->add_point()->CopyFrom(
        *point + lane_right_width * Vec2d::CreateUnitVec2d(path_point.theta() - M_PI_2));
  }
  return true;
}
}  // namespace
...
void NavigationLane::SetConfig(const NavigationLaneConfig& config) { config_ = config; }
...
void NavigationLane::UpdateNavigationInfo(const NavigationInfo& navigation_path) {
  ...
  if (FLAGS_enable_cyclic_rerouting) { UpdateStitchIndexInfo(); }
}
...
bool NavigationLane::GeneratePath() {
  ...
  auto generate_path_on_perception_func = [this, &lane_marker]() {
    ...
    double left_width = perceived_left_width_ > 0.0 ? perceived_left_width_ : default_left_width_;
    double right_width =
        perceived_right_width_ > 0.0 ? perceived_right_width_ : default_right_width_;
    current_navi_path_tuple_ = std::make_tuple(0, left_width, right_width, current_navi_path);
  };
  ...
  if (config_.lane_source() == NavigationLaneConfig::OFFLINE_GENERATED && navigation_line_num > 0) {
    ...
    for (int i = 0; i < navigation_line_num; ++i) {
      ...
      if (ConvertNavigationLineToPath(i, path)) {
        current_navi_path->set_path_priority(navigation_info_.navigation_path(i).path_priority());
        navigation_path_list_.emplace_back(i, default_left_width_, default_right_width_,
                                           current_navi_path);
      }
    }
    ...
    // y-coordinate.
    navigation_path_list_.sort([](const NaviPathTuple& left, const NaviPathTuple& right) {
    ...
    for (const auto& navi_path_tuple : navigation_path_list_) {
      ...
      if (item_iter != last_project_index_map_.end()) { current_d = item_iter->second.second; }
      if (current_d < min_d) {
        min_d                    = current_d;
    ...
    MergeNavigationLineAndLaneMarker(std::get<0>(current_navi_path_tuple_), path);
    ...
    if (!IsFloatEqual(left_width, default_left_width_) &&
      ...
      left_width  = left_width > default_left_width_ ? left_width - min_d : left_width + min_d;
    ...
    auto curr_navi_path_iter =
        std::find_if(std::begin(navigation_path_list_), std::end(navigation_path_list_),
    ...
    for (auto iter = navigation_path_list_.begin(); iter != navigation_path_list_.end(); ++iter) {
      ...
      if (prev_iter != navigation_path_list_.end()) {
        ...
        average_point_size    = std::min(
            average_point_size, std::min(curr_path.path_point_size(), prev_path.path_point_size()));
      ...
      if (next_iter != navigation_path_list_.end()) {
        ...
        average_point_size    = std::min(
            average_point_size, std::min(curr_path.path_point_size(), next_path.path_point_size()));
        double lateral_distance_sum = 0.0;
        for (int i = 0; i < average_point_size; ++i) {
...
double NavigationLane::EvaluateCubicPolynomial(
...
void NavigationLane::MergeNavigationLineAndLaneMarker(const int           line_index,
  ...
  ConvertLaneMarkerToPath(perception_obstacles_.lane_marker(), &lane_marker_path);

  // If the size of lane marker path points is smaller than 2, merging is not
  // required.
  if (lane_marker_path.path_point_size() < 2) { return; }
  ...
  for (int i = 0; i < path->path_point_size(); ++i) {
    ...
    double s     = point->s();
    auto   lane_maker_point =
        GetPathPointByS(lane_marker_path, lane_marker_index, s, &lane_marker_index);
    ...
    *point = common::util::GetWeightedAverageOfTwoPathPoints(
        *point, lane_maker_point, navigation_line_weight, 1 - navigation_line_weight);
  }
}
...
common::PathPoint NavigationLane::GetPathPointByS(const common::Path& path,
  ...
  *matched_index = i;
  ...
  const double r =
      (s - path.path_point(i).s()) / (path.path_point(i + 1).s() - path.path_point(i).s());
  auto p = common::util::GetWeightedAverageOfTwoPathPoints(path.path_point(i),
                                                           path.path_point(i + 1), 1 - r, r);
  return p;
}
...
bool NavigationLane::ConvertNavigationLineToPath(const int line_index, common::Path* const path) {
  ...
  if (!navigation_info_.navigation_path(line_index).has_path() ||
      navigation_info_.navigation_path(line_index).path().path_point_size() == 0) {
    // path is empty
    return false;
  }
  path->set_name(absl::StrCat("Path from navigation line index ", line_index));
  const auto& navigation_path = navigation_info_.navigation_path(line_index).path();
  ...
  if (current_project_index < 0 || current_project_index >= navigation_path.path_point_size()) {
    AERROR << "Invalid projection index " << current_project_index << " in line " << line_index;
  ...
  auto         enu_to_flu_func = [this, dx, dy](const double enu_x, const double enu_y,
                                        const double enu_theta, double* flu_x, double* flu_y,
                                        double* flu_theta) {
    if (flu_x != nullptr && flu_y != nullptr) {
      Eigen::Vector2d flu_coordinate =
          common::math::RotateVector2d({enu_x + dx, enu_y + dy}, -original_pose_.heading());
    ...
    if (flu_theta != nullptr) {
      *flu_theta = common::math::NormalizeAngle(common::math::NormalizeAngle(enu_theta) -
  ...
  auto gen_navi_path_loop_func = [this, &navigation_path, &enu_to_flu_func](
    ...
    for (int i = start; i < end; ++i) {
      ...
      enu_to_flu_func(point->x(), point->y(), point->theta(), &flu_x, &flu_y, &flu_theta);

      point->set_x(flu_x);
      point->set_y(flu_y);
      point->set_theta(flu_theta);
      const double accumulated_s = navigation_path.path_point(i).s() - ref_s + ref_s_base;
      point->set_s(accumulated_s);

      if (accumulated_s > max_length) { break; }
    }
  };
  ...
  if (FLAGS_enable_cyclic_rerouting && dist < config_.max_len_from_navigation_line()) {
    auto item_iter = stitch_index_map_.find(line_index);
    if (item_iter != stitch_index_map_.end()) {
      int stitch_start_index = std::max(item_iter->second.first, item_iter->second.second);
      stitch_start_index     = std::max(current_project_index, stitch_start_index);
      stitch_start_index     = std::min(navigation_path.path_point_size() - 1, stitch_start_index);

      int stitch_end_index = std::min(item_iter->second.first, item_iter->second.second);
      ...
      double length = navigation_path.path_point(stitch_start_index).s() -
                      navigation_path.path_point(current_project_index).s();
      gen_navi_path_loop_func(std::max(0, current_project_index - 3), stitch_start_index + 1, 0.0,
                              length, path);
      if (length > config_.max_len_from_navigation_line()) { return true; }
      gen_navi_path_loop_func(stitch_end_index, navigation_path.path_point_size(), length,
  ...
  gen_navi_path_loop_func(std::max(0, current_project_index - 3), navigation_path.path_point_size(),
                          0.0, config_.max_len_from_navigation_line(), path);
  return true;
}
...
ProjIndexPair NavigationLane::UpdateProjectionIndex(const common::Path& path,
  ...
  if (path.path_point_size() < 2) { return std::make_pair(-1, std::numeric_limits<double>::max()); }
  ...
  auto check_distance_func = [this, &path, &path_size](const int project_index,
    ...
    //  sin(angle)
    if (project_index < 0 || project_index > path_size - 1) { return false; }
    ...
    double flu_x     = (enu_x - x_shift) * cos_angle + (enu_y - y_shift) * sin_angle;
    double flu_y     = (enu_y - y_shift) * cos_angle - (enu_x - x_shift) * sin_angle;

    if (project_distance != nullptr) { *project_distance = std::fabs(flu_y); }
  ...
  for (int i = current_project_index; i + 1 < path_size; ++i) {
    ...
    if (current_project_index != 0 && d > kMaxDistance) { break; }
  }
  ...
  if (check_distance_func(index, &min_d)) {
    if (FLAGS_enable_cyclic_rerouting) {
      ...
      auto item_iter = stitch_index_map_.find(line_index);
      if (item_iter != stitch_index_map_.end()) {
        int start_index = std::max(item_iter->second.first, item_iter->second.second);
        int end_index   = std::min(item_iter->second.first, item_iter->second.second);
...
double NavigationLane::GetKappa(const double c1, const double c2, const double c3, const double x) {
...
void NavigationLane::ConvertLaneMarkerToPath(const perception::LaneMarkers& lane_marker,
  ...
  double path_c3 =
      (left_lane.c3_curvature_derivative() + right_lane.c3_curvature_derivative()) / 2.0;
  ...
  for (double z = start_s; z <= path_range; z += unit_z) {
    ...
    point->set_s(accumulated_s);
    point->set_theta(std::atan2(3 * path_c3 * x1 * x1 + 2 * path_c2 * x1 + path_c1, 1));
...
bool NavigationLane::CreateMap(const MapGenerationParam& map_config, MapMsg* const map_msg) const {
  ...
  if (navigation_path_list_.empty()) {
    if (std::get<3>(current_navi_path_tuple_) != nullptr) {
      ...
      return CreateSingleLaneMap(current_navi_path_tuple_, map_config, perception_obstacles_, hdmap,
                                 navigation_path);
    } else {
      return false;
    }
  ...
  int fail_num                              = 0;
  FLAGS_relative_map_generate_left_boundray = true;
  for (auto iter = navigation_path_list_.cbegin(); iter != navigation_path_list_.cend(); ++iter) {
    std::size_t index = std::distance(navigation_path_list_.cbegin(), iter);
    if (!CreateSingleLaneMap(*iter, map_config, perception_obstacles_, hdmap, navigation_path)) {
    ...
    if (lane_index > 0) {
      auto* left_boundary = hdmap->mutable_lane(lane_index)->mutable_left_boundary();
      left_boundary->CopyFrom(hdmap->lane(lane_index - 1).right_boundary());
      auto* left_sample = hdmap->mutable_lane(lane_index)->mutable_left_sample();
  ...
  right_edge->mutable_curve()->CopyFrom(hdmap->lane(lane_num - 1).right_boundary().curve());

  // Set neighbor information for each lane
  if (lane_num < 2) { return true; }
  for (int i = 0; i < lane_num; ++i) {
    auto* lane = hdmap->mutable_lane(i);
    if (i > 0) {
      lane->add_left_neighbor_forward_lane_id()->CopyFrom(hdmap->lane(i - 1).id());
      ADEBUG << "Left neighbor is: " << hdmap->lane(i - 1).id().id();
    }
    if (i < lane_num - 1) {
      lane->add_right_neighbor_forward_lane_id()->CopyFrom(hdmap->lane(i + 1).id());
...
void NavigationLane::UpdateStitchIndexInfo() {
  ...
  int navigation_line_num = navigation_info_.navigation_path_size();
  if (navigation_line_num <= 0) { return; }
  ...
  for (int i = 0; i < navigation_line_num; ++i) {
    ...
    for (int m = 0; m < path_size; ++m) {
      ...
      if (forward_s > config_.max_len_from_navigation_line()) { break; }

      for (int n = path_size - 1; n >= 0; --n) {
        double reverse_s = end_s - navigation_path.path_point(n).s();
        if (reverse_s > config_.max_len_from_navigation_line()) { break; }
        if (m == n) { break; }

        double current_distance =
            DistanceXY(navigation_path.path_point(m), navigation_path.path_point(n));
    ...
    if (min_distance < config_.min_lane_half_width()) {
      AINFO << "The stitching pair is: (" << min_index_pair.first << ", " << min_index_pair.second
            << ") for the navigation line: " << i;
