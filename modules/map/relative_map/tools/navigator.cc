...
int main(int argc, char** argv) {
  ...
  AINFO << "The navigator configuration filename is: " << FLAGS_navigator_config_filename;
  ...
  for (const std::string& filename : navigation_line_filenames) {
    ...
    if (!GetNavigationPathFromFile(filename, navigator_config, navigation_path)) {
...
bool GetNavigationPathFromFile(const std::string&     filename,
  ...
  while (std::getline(ifs, line_str)) {
    try {
      ...
      bool not_down_sampling = navigator_config.enable_navigator_downsample() ?
      ...
      if (not_down_sampling || !navigation_path->has_path()) {
        ...
        ADEBUG << "down_sample_x: " << json_obj["x"] << ", down_sample_y: " << json_obj["y"];
      }
      ++original_points_num;
    } catch (const std::exception& e) {
  ...
  AINFO << "The number of original points is: " << original_points_num
        << " and the number of down sampled points is: " << down_sampled_points_num
