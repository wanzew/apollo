...
class MapPathPoint : public common::math::Vec2d {
 public:
  ...
  MapPathPoint(const common::math::Vec2d& point,
               double                     heading,
  ...
  GetPointsFromLane(LaneInfoConstPtr lane, const double start_s, const double end_s);
...
class PathApproximation {
 public:
  ...
  PathApproximation(const Path& path, const double max_error)
      ...
      , max_sqr_error_(max_error * max_error) {
    Init(path);
  }
  ...
  bool GetProjection(const Path&                path,
...
class Path {
 public:
  ...
  bool
       GetNearestPoint(const common::math::Vec2d& point, double* accumulate_s, double* lateral) const;
  bool GetNearestPoint(const common::math::Vec2d& point,
                       double*                    accumulate_s,
                       double*                    lateral,
                       double*                    distance) const;
  ...
  const std::vector<PathOverlap>& dead_end_overlaps() const { return dead_end_overlaps_; }
