...
TEST(TestSuite, LaneSegment) {
  Lane lane1;
  {
    ...
    auto* line_segment = lane1.mutable_central_curve()->add_segment()->mutable_line_segment();
  ...
  Lane             lane2;
  {
    ...
    auto* line_segment = lane2.mutable_central_curve()->add_segment()->mutable_line_segment();
    *line_segment->add_point() = MakePoint(0, 0, 0);
    *line_segment->add_point() = MakePoint(0, 3, 0);
...
TEST(TestSuite, hdmap_line_path) {
  ...
  auto* line_segment         = lane.mutable_central_curve()->add_segment()->mutable_line_segment();
  *line_segment->add_point() = MakePoint(0, 0, 0);
  *line_segment->add_point() = MakePoint(0, 3, 0);
  lane.set_length(3.0);
  ...
  *lane.add_right_sample() = MakeSample(3.0, 5.0);
  ...
  std::vector<MapPathPoint> points{MapPathPoint({0, 0}, M_PI_2, LaneWaypoint(lane_info, 0)),
  ...
  double lateral;
  double distance;
  EXPECT_TRUE(path.GetNearestPoint({0.3, -1}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 0.0, 1e-6);
  EXPECT_NEAR(lateral, -0.3, 1e-6);
  EXPECT_NEAR(distance, hypot(0.3, 1.0), 1e-6);
  EXPECT_TRUE(path.GetNearestPoint({-0.5, 1.5}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 1.5, 1e-6);
  EXPECT_NEAR(lateral, 0.5, 1e-6);
  EXPECT_NEAR(distance, 0.5, 1e-6);
  EXPECT_TRUE(path.GetNearestPoint({0.0, 3.5}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 3.0, 1e-6);
  EXPECT_NEAR(lateral, 0.0, 1e-6);
  EXPECT_NEAR(distance, 0.5, 1e-6);

  EXPECT_TRUE(path.GetProjection({0.3, -1}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, -1.0, 1e-6);
  EXPECT_NEAR(lateral, -0.3, 1e-6);
  EXPECT_NEAR(distance, hypot(0.3, 1.0), 1e-6);
  EXPECT_TRUE(path.GetProjection({-0.5, 1.5}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 1.5, 1e-6);
  EXPECT_NEAR(lateral, 0.5, 1e-6);
  EXPECT_NEAR(distance, 0.5, 1e-6);
  EXPECT_TRUE(path.GetProjection({0.0, 3.5}, &accumulate_s, &lateral, &distance));
...
TEST(TestSuite, hdmap_curvy_path) {
  std::vector<MapPathPoint> points{MakeMapPathPoint(2, 0), MakeMapPathPoint(2, 1),
  ...
  double lateral;
  double distance;
  EXPECT_TRUE(path.GetNearestPoint({1.5, 0.5}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 0.5, 1e-6);
  EXPECT_NEAR(lateral, 0.5, 1e-6);
  EXPECT_NEAR(distance, 0.5, 1e-6);
  EXPECT_TRUE(path.GetNearestPoint({2.5, 1.1}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 1.0, 1e-6);
  EXPECT_NEAR(distance, hypot(0.5, 0.1), 1e-6);
  EXPECT_TRUE(path.GetNearestPoint({1.6, 1.6}, &accumulate_s, &lateral, &distance));
  ...
  EXPECT_TRUE(path.GetProjection({1.5, 0.5}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 0.5, 1e-6);
  EXPECT_NEAR(lateral, 0.5, 1e-6);
  EXPECT_NEAR(distance, 0.5, 1e-6);
  EXPECT_TRUE(path.GetProjection({2.5, 1.1}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 1.0, 1e-6);
  EXPECT_NEAR(distance, hypot(0.5, 0.1), 1e-6);
  EXPECT_TRUE(path.GetProjection({1.6, 1.6}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 1.0 + 0.5 * sqrt(2.0), 1e-6);
  EXPECT_NEAR(lateral, -0.1 * sqrt(2.0), 1e-6);
  EXPECT_NEAR(distance, 0.1 * sqrt(2.0), 1e-6);
  ...
  EXPECT_TRUE(other_path.GetProjection({1.5, 0.5}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 0.5, 1e-6);
  EXPECT_NEAR(lateral, 0.5, 1e-6);
  EXPECT_NEAR(distance, 0.5, 1e-6);
  EXPECT_TRUE(other_path.GetProjection({2.5, 1.1}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 1.0, 1e-6);
  EXPECT_NEAR(distance, hypot(0.5, 0.1), 1e-6);
  EXPECT_TRUE(other_path.GetProjection({1.6, 1.6}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, 1.0 + 0.5 * sqrt(2.0), 1e-6);
  EXPECT_NEAR(lateral, -0.1 * sqrt(2.0), 1e-6);
  EXPECT_NEAR(distance, 0.1 * sqrt(2.0), 1e-6);
}
...
TEST(TestSuite, hdmap_circle_path) {
  ...
  for (int i = 0; i <= kNumSegments; ++i) {
    const double p = M_PI_2 * static_cast<double>(i) / static_cast<double>(kNumSegments);
  ...
  double       lateral;
  double       distance;
  EXPECT_TRUE(path.GetProjection({kRadius + 1, -1}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, -1.0, kLargeEps);
  EXPECT_NEAR(lateral, -1.0, kLargeEps);
  EXPECT_NEAR(distance, sqrt(2.0), 1e-6);

  EXPECT_TRUE(path.GetProjection({-1, kRadius - 1}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, total_length + 1.0, kLargeEps);
  EXPECT_NEAR(lateral, 1.0, kLargeEps);
  EXPECT_NEAR(distance, sqrt(2.0), 1e-6);

  EXPECT_TRUE(path.GetProjection({kRadius / sqrt(2.0) + 1, kRadius / sqrt(2.0) + 1}, &accumulate_s,
                                 &lateral, &distance));
  EXPECT_NEAR(accumulate_s, total_length / 2.0, 1e-6);
  EXPECT_NEAR(lateral, -sqrt(2.0), 1e-6);
  EXPECT_NEAR(distance, sqrt(2.0), 1e-6);

  EXPECT_TRUE(path.GetProjection({kRadius / sqrt(2.0), kRadius / sqrt(2.0)}, &accumulate_s,
                                 &lateral, &distance));
  EXPECT_NEAR(accumulate_s, total_length / 2.0, 1e-6);
  EXPECT_NEAR(lateral, 0.0, 1e-6);
  ...
  for (int case_id = 0; case_id < 10000; ++case_id) {
    ...
    EXPECT_TRUE(path.GetNearestPoint({x, y}, &accumulate_s, &lateral, &distance));

    double other_accumulate_s;
    double other_lateral;
    double other_distance;
    EXPECT_TRUE(path_no_approximation.GetNearestPoint({x, y}, &other_accumulate_s, &other_lateral,
                                                      &other_distance));

    EXPECT_NEAR(distance, other_distance, 1e-6);
    EXPECT_NEAR(path.GetSmoothPoint(accumulate_s).DistanceTo({x, y}), distance, 1e-6);
  }
...
TEST(TestSuite, hdmap_jerky_path) {
  ...
  for (int path_id = 0; path_id < kNumPaths; ++path_id) {
    ...
    for (int i = 0; i <= num_segments; ++i) {
      ...
      sum_x += RandomDouble(average_segment_length * 0.1, average_segment_length * 1.9);
    }
    ...
    double lateral;
    double distance;
    EXPECT_TRUE(path.GetProjection(points[0], &accumulate_s, &lateral, &distance));
    EXPECT_NEAR(accumulate_s, 0.0, 1e-6);
    EXPECT_NEAR(lateral, 0.0, 1e-6);
    EXPECT_NEAR(distance, 0.0, 1e-6);
    EXPECT_TRUE(path.GetProjection(points[num_segments], &accumulate_s, &lateral, &distance));
...
TEST(TestSuite, hdmap_s_path) {
  ...
  for (int i = 0; i <= kNumSegments; ++i) {
    if (i <= kNumSegments / 2) {
      const double p =
          -M_PI_2 + 2.0 * M_PI * static_cast<double>(i) / static_cast<double>(kNumSegments);
      points.push_back(MakeMapPathPoint(kRadius * cos(p), kRadius * (sin(p) - 1.0)));
    } else {
      const double p =
          M_PI_2 - 2.0 * M_PI * static_cast<double>(i) / static_cast<double>(kNumSegments);
      points.push_back(MakeMapPathPoint(kRadius * cos(p), kRadius * (sin(p) + 1.0)));
  ...
  double       lateral;
  double       distance;
  EXPECT_TRUE(path.GetProjection({-1, -2.0 * kRadius - 1}, &accumulate_s, &lateral, &distance));
  EXPECT_NEAR(accumulate_s, -1.0, kLargeEps);
  EXPECT_NEAR(lateral, -1.0, kLargeEps);
  EXPECT_NEAR(distance, sqrt(2.0), 1e-6);
...
TEST(TestSuite, hdmap_path_get_smooth_point) {
  ...
  for (int i = 0; i < kNumSegments; ++i) {
    ...
    auto* segment = lane.mutable_central_curve()->add_segment()->mutable_line_segment();
  ...
  for (int i = 0; i <= kNumSegments; ++i) {
    points[i].set_heading((i < kNumSegments) ? (points[i + 1] - points[i]).Angle() :
                                               (points[i] - points[i - 1]).Angle());
    if (i > 0) { points[i].add_lane_waypoint(LaneWaypoint(lanes[i - 1], segment_length)); }
    if (i < kNumSegments) { points[i].add_lane_waypoint(LaneWaypoint(lanes[i], 0.0)); }
  }
  ...
  for (int i = 0; i <= kNumSegments; ++i) {
    ...
    if (i == 0) {
      ...
      EXPECT_EQ(point.lane_waypoints().size(), 1);
      EXPECT_EQ(point.lane_waypoints()[0].lane->id().id(), std::to_string(i - 1));
    ...
    if (i < kNumSegments) {
      for (int case_id = 0; case_id < 20; ++case_id) {
        ...
        EXPECT_NEAR(point.x(), points[i].x() + offset * cos(points[i].heading()), 1e-6);
        EXPECT_NEAR(point.y(), points[i].y() + offset * sin(points[i].heading()), 1e-6);
        EXPECT_NEAR(point.heading(), points[i].heading(), 1e-6);
        EXPECT_EQ(point.lane_waypoints().size(), 1);
        EXPECT_EQ(point.lane_waypoints()[0].lane->id().id(), std::to_string(i));
