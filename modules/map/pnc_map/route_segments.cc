...
const LaneWaypoint& RouteSegments::RouteEndWaypoint() const { return route_end_waypoint_; }

bool RouteSegments::IsOnSegment() const { return is_on_segment_; }

void RouteSegments::SetIsOnSegment(bool on_segment) { is_on_segment_ = on_segment; }

bool RouteSegments::IsNeighborSegment() const { return is_neighbor_; }

void RouteSegments::SetIsNeighborSegment(bool is_neighbor) { is_neighbor_ = is_neighbor; }
...
bool RouteSegments::IsConnectedSegment(const RouteSegments& other) const {
  ...
  if (IsWaypointOnSegment(other.FirstWaypoint())) { return true; }
  if (IsWaypointOnSegment(other.LastWaypoint())) { return true; }
  if (other.IsWaypointOnSegment(FirstWaypoint())) { return true; }
  if (other.IsWaypointOnSegment(LastWaypoint())) { return true; }
  return false;
}
...
bool RouteSegments::Shrink(const double s, const double look_backward, const double look_forward) {
  LaneWaypoint waypoint;
  if (!GetWaypoint(s, &waypoint)) { return false; }
  return Shrink(s, waypoint, look_backward, look_forward);
}

bool RouteSegments::Shrink(const double        s,
                           const LaneWaypoint& waypoint,
                           const double        look_backward,
                           const double        look_forward) {
  double acc_s = 0.0;
  ...
  if (iter->Length() < kSegmentationEpsilon) { ++iter; }
  erase(begin(), iter);
...
bool RouteSegments::GetProjection(const common::math::Vec2d& point,
  ...
  for (auto iter = begin(); iter != end(); accumulated_s += (iter->end_s - iter->start_s), ++iter) {
    ...
    if (!iter->lane->GetProjection(point, &lane_s, &lane_l)) {
      AERROR << "Failed to get projection from point " << point.DebugString() << " on lane "
             << iter->lane->id().id();
      return false;
    }
...
void RouteSegments::SetPreviousAction(routing::ChangeLaneType action) { previous_action_ = action; }

routing::ChangeLaneType RouteSegments::PreviousAction() const { return previous_action_; }

void RouteSegments::SetNextAction(routing::ChangeLaneType action) { next_action_ = action; }

routing::ChangeLaneType RouteSegments::NextAction() const { return next_action_; }

bool RouteSegments::IsWaypointOnSegment(const LaneWaypoint& waypoint) const {
  for (auto iter = begin(); iter != end(); ++iter) {
    if (WithinLaneSegment(*iter, waypoint)) { return true; }
  }
  return false;
}
...
bool RouteSegments::CanDriveFrom(const LaneWaypoint& waypoint) const {
  ...
  if (IsWaypointOnSegment(waypoint)) { return true; }
  ...
  if (std::fabs(route_sl.l()) > 2 * kMaxLaneWidth) { return false; }
