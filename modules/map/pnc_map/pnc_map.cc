...
bool PncMap::GetNearestPointFromRouting(const VehicleState& state, LaneWaypoint* waypoint) const {
  ...
  const int                     status =
      hdmap_->GetLanesWithHeading(point, kMaxDistance, state.heading(), M_PI / 2.0, &lanes);
  if (status < 0) {
  ...
  for (const auto& lane : valid_lanes) {
    if (range_lane_ids_.count(lane->id().id()) == 0) { continue; }
    {
      double s = 0.0;
      double l = 0.0;
      if (!lane->GetProjection({point.x(), point.y()}, &s, &l)) { return false; }
...
LaneInfoConstPtr PncMap::GetRouteSuccessor(LaneInfoConstPtr lane) const {
  if (lane->lane().successor_id_size() == 0) { return nullptr; }
...
LaneInfoConstPtr PncMap::GetRoutePredecessor(LaneInfoConstPtr lane) const {
  if (lane->lane().predecessor_id_size() == 0) { return nullptr; }
...
bool PncMap::CreatePathFromLaneSegments(const RouteSegments& segments, Path* const path) {
  ...
  if (points.size() < 2) {
    AWARN << "Cannot create path from " << points.size() << " points. Expecting more than 2.";
    return false;
  }
