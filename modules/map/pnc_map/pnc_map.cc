...
PncMap::PncMap(const HDMap* hdmap)
    : hdmap_(hdmap) {}
...
std::vector<routing::LaneWaypoint> PncMap::FutureRouteWaypoints() const {
  ...
  return std::vector<routing::LaneWaypoint>(waypoints.begin() + next_routing_waypoint_index_,
                                            waypoints.end());
}
...
void PncMap::UpdateRoutingRange(int adc_index) {
  ...
  while (range_end_ < static_cast<int>(route_indices_.size())) {
    ...
    if (range_lane_ids_.count(lane_id) != 0) { break; }
...
bool PncMap::UpdateVehicleState(const VehicleState& vehicle_state) {
  ...
  if (!adc_state_.has_x() ||
      (common::util::DistanceXY(adc_state_, vehicle_state) >
       FLAGS_replan_lateral_distance_threshold + FLAGS_replan_longitudinal_distance_threshold)) {
...
const routing::RoutingResponse& PncMap::routing_response() const { return routing_; }
...
bool PncMap::ValidateRouting(const RoutingResponse& routing) {
  ...
  if (!routing.has_routing_request() || routing.routing_request().waypoint_size() < 2) {
...
int PncMap::SearchForwardWaypointIndex(int start, const LaneWaypoint& waypoint) const {
...
int PncMap::SearchBackwardWaypointIndex(int start, const LaneWaypoint& waypoint) const {
  int i = std::min(static_cast<int>(route_indices_.size() - 1), start);
  while (i >= 0 && !RouteSegments::WithinLaneSegment(route_indices_[i].segment, waypoint)) {
    --i;
  }
  return i;
}
...
int PncMap::GetWaypointIndex(const LaneWaypoint& waypoint) const {
  ...
  if (forward_index == adc_route_index_ || forward_index == adc_route_index_ + 1) {
    return forward_index;
  }
  auto backward_index = SearchBackwardWaypointIndex(adc_route_index_, waypoint);
  if (backward_index < 0) { return forward_index; }

  return (backward_index + 1 == adc_route_index_) ? backward_index : forward_index;
}
...
bool PncMap::GetRouteSegments(const VehicleState&             vehicle_state,
  ...
  return GetRouteSegments(vehicle_state, look_backward_distance, look_forward_distance,
                          route_segments);
}
...
bool PncMap::GetRouteSegments(const VehicleState&             vehicle_state,
  ...
  for (const int index : drive_passages) {
    ...
    if (!ExtendSegments(segments, sl.s() - backward_length, sl.s() + forward_length,
                        &route_segments->back())) {
      AERROR << "Failed to extend segments with s=" << sl.s() << ", backward: " << backward_length
...
bool PncMap::GetNearestPointFromRouting(const VehicleState& state, LaneWaypoint* waypoint) const {
  ...
  const int status = hdmap_->GetLanesWithHeading(point, kMaxDistance, state.heading(),
  ...
  std::copy_if(
      ...
      [&](LaneInfoConstPtr ptr) { return range_lane_ids_.count(ptr->lane().id().id()) > 0; });
  if (valid_lanes.empty()) {
    std::copy_if(
        lanes.begin(), lanes.end(), std::back_inserter(valid_lanes),
        [&](LaneInfoConstPtr ptr) { return all_lane_ids_.count(ptr->lane().id().id()) > 0; });
  }

  // Get nearest_waypoints for current position
  double min_distance = std::numeric_limits<double>::infinity();
  for (const auto& lane : valid_lanes) {
    if (range_lane_ids_.count(lane->id().id()) == 0) { continue; }
    {
      double s = 0.0;
      double l = 0.0;
...
LaneInfoConstPtr PncMap::GetRouteSuccessor(LaneInfoConstPtr lane) const {
  if (lane->lane().successor_id().empty()) { return nullptr; }
...
LaneInfoConstPtr PncMap::GetRoutePredecessor(LaneInfoConstPtr lane) const {
  if (lane->lane().predecessor_id().empty()) { return nullptr; }
...
bool PncMap::ExtendSegments(const RouteSegments& segments,
  ...
  if (start_s < 0) {
    ...
    while (extend_s > kRouteEpsilon) {
      if (s <= kRouteEpsilon) {
        ...
        if (lane == nullptr || unique_lanes.find(lane->id().id()) != unique_lanes.end()) { break; }
        s = lane->total_length();
      } else {
    ...
    truncated_segments->insert(truncated_segments->begin(), extended_lane_segments.rbegin(),
  ...
  for (const auto& lane_segment : segments) {
    const double adjusted_start_s =
        std::max(start_s - router_s + lane_segment.start_s, lane_segment.start_s);
    ...
    if (router_s > end_s) { break; }
  }
  if (found_loop) { return true; }
  ...
  while (router_s < end_s - kRouteEpsilon) {
    ...
    if (last_lane == nullptr || unique_lanes.find(last_lane->id().id()) != unique_lanes.end()) {
      break;
    }
...
void PncMap::AppendLaneToPoints(LaneInfoConstPtr                 lane,
                                const double                     start_s,
                                const double                     end_s,
                                std::vector<MapPathPoint>* const points) {
  if (points == nullptr || start_s >= end_s) { return; }
