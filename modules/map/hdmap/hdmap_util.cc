...
std::string FindFirstExist(const std::string& dir, const std::string& files) {
  const auto candidates = apollo::common::util::StringTokenizer::Split(files, "|");
  for (const auto& filename : candidates) {
    const std::string file_path = apollo::common::util::StrCat(FLAGS_map_dir, "/", filename);
    if (apollo::common::util::PathExists(file_path)) { return file_path; }
  }
...
const SpeedControls* GetSpeedControls() {
  ...
  if (speed_control_ == nullptr) {
    ...
    if (!common::util::GetProtoFromFile(SpeedControlFile(), speed_control_.get())) {
...
const HDMap* HDMapUtil::BaseMapPtr() {
  if (FLAGS_use_navigation_mode) {
    ...
    if (base_map_ != nullptr && base_map_seq_ == latest.header().sequence_num()) {
