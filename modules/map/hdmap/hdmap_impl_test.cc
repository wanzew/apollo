...
class HDMapImplTestSuite : public ::testing::Test {
 public:
  HDMapImplTestSuite() { EXPECT_EQ(0, hdmap_impl_.LoadMapFromFile(kMapFilename)); }
...
TEST_F(HDMapImplTestSuite, GetCrosswalkById) {
  ...
  CrosswalkInfoConstPtr crosswalk_ptr = hdmap_impl_.GetCrosswalkById(crosswalk_id);
...
TEST_F(HDMapImplTestSuite, GetStopSignById) {
  ...
  StopSignInfoConstPtr stop_sign_ptr = hdmap_impl_.GetStopSignById(stop_sign_id);
...
TEST_F(HDMapImplTestSuite, GetYieldSignById) {
  ...
  YieldSignInfoConstPtr yield_sign_ptr = hdmap_impl_.GetYieldSignById(yield_sign_id);
...
TEST_F(HDMapImplTestSuite, GetClearAreaById) {
  ...
  ClearAreaInfoConstPtr clear_area_ptr = hdmap_impl_.GetClearAreaById(clear_area_id);
...
TEST_F(HDMapImplTestSuite, GetNearestLaneWithHeading) {
  ...
  EXPECT_EQ(-1, hdmap_impl_.GetNearestLaneWithHeading(point, 1e-6, 0.86, 0.2, &nearest_lane,
                                                      &nearest_s, &nearest_l));

  EXPECT_EQ(0, hdmap_impl_.GetNearestLaneWithHeading(point, 5, -2.35, 1.0, &nearest_lane,
                                                     &nearest_s, &nearest_l));
...
TEST_F(HDMapImplTestSuite, GetLanesWithHeading) {
  ...
  std::vector<LaneInfoConstPtr> lanes;
  EXPECT_EQ(-1, hdmap_impl_.GetLanesWithHeading(point, 1e-6, 0.86, 0.2, &lanes));
...
TEST_F(HDMapImplTestSuite, GetRoadBoundaries) {
  ...
  EXPECT_EQ(-1, hdmap_impl_.GetRoadBoundaries(point, 3.0, &road_boundaries, &junctions));

  point.set_x(586434.75);
  point.set_y(4140746.94);
  point.set_z(0.0);
  EXPECT_EQ(0, hdmap_impl_.GetRoadBoundaries(point, 2.0, &road_boundaries, &junctions));
  ...
  point.set_y(4140746.94);
  point.set_z(0.0);
  EXPECT_EQ(0, hdmap_impl_.GetRoadBoundaries(point, 4.5, &road_boundaries, &junctions));
  EXPECT_EQ(1, road_boundaries.size());
  EXPECT_EQ(1, junctions.size());
}
...
TEST_F(HDMapImplTestSuite, GetForwardNearestSignalsOnLane) {
  ...
  point.set_z(0.0);
  std::vector<SignalInfoConstPtr> signals;
  EXPECT_EQ(0, hdmap_impl_.GetForwardNearestSignalsOnLane(point, 10.0, &signals));
  EXPECT_EQ(1, signals.size());
  EXPECT_EQ("1278", signals[0]->id().id());

  EXPECT_EQ(0, hdmap_impl_.GetForwardNearestSignalsOnLane(point, 3.0, &signals));
  EXPECT_EQ(0, signals.size());
...
TEST_F(HDMapImplTestSuite, GetRoadBounrariesAndJunctions) {
  ...
  std::vector<RoadRoiPtr>           roads_roi;
  std::vector<JunctionInfoConstPtr> junctions;
  ASSERT_EQ(0, hdmap_impl_.GetRoadBoundaries(point, distance, &roads_roi, &junctions));
  ASSERT_EQ(5, roads_roi.size());
...
TEST_F(HDMapImplTestSuite, GetLocalMap) {
  ...
  const std::string time_since_epoch =
  ...
  local_map.Clear();
  ACHECK(cyber::common::GetProtoFromFile(output_bin_file, &local_map)) << "failed to load map";
