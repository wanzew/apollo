...
void ProtoOrganizer::GetJunctionElements(const std::vector<JunctionInternal>& junctions) {
...
void ProtoOrganizer::GetLaneObjectOverlapElements(
    const std::string& lane_id, const std::vector<OverlapWithLane>& overlap_with_lanes) {
  for (auto& overlap_object : overlap_with_lanes) {
    ...
    object_overlap->mutable_lane_overlap_info()->set_start_s(overlap_object.start_s);
    object_overlap->mutable_lane_overlap_info()->set_end_s(overlap_object.end_s);
    object_overlap->mutable_lane_overlap_info()->set_is_merge(overlap_object.is_merge);
    if (!overlap_object.region_overlap_id.empty()) {
      object_overlap->mutable_lane_overlap_info()->mutable_region_overlap_id()->set_id(
    ...
    if (proto_data_.pb_crosswalks.count(object_id) > 0) {
      ...
      if (!overlap_object.region_overlap_id.empty()) {
        object_overlap->mutable_crosswalk_overlap_info()->mutable_region_overlap_id()->set_id(
            overlap_object.region_overlap_id);
      }
...
void ProtoOrganizer::GetLaneSignalOverlapElements(
  ...
  for (auto& overlap_signal : overlap_with_lanes) {
    ...
    PbObjectOverlapInfo* object_overlap = overlap.add_object();
    object_overlap->mutable_id()->set_id(lane_id);
    object_overlap->mutable_lane_overlap_info()->set_start_s(overlap_signal.start_s);
    object_overlap->mutable_lane_overlap_info()->set_end_s(overlap_signal.end_s);
    object_overlap->mutable_lane_overlap_info()->set_is_merge(overlap_signal.is_merge);
    object_overlap = overlap.add_object();
    object_overlap->mutable_id()->set_id(object_id);
    if (proto_data_.pb_signals.count(object_id) > 0) {
      ...
      proto_data_.pb_yield_signs[object_id].add_overlap_id()->set_id(overlap_id);
...
void ProtoOrganizer::GetLaneJunctionOverlapElements(
  ...
  for (auto& overlap_junction : overlap_with_lanes) {
    ...
    PbObjectOverlapInfo* object_overlap = overlap.add_object();
    object_overlap->mutable_id()->set_id(lane_id);
    object_overlap->mutable_lane_overlap_info()->set_start_s(overlap_junction.start_s);
    object_overlap->mutable_lane_overlap_info()->set_end_s(overlap_junction.end_s);
    object_overlap->mutable_lane_overlap_info()->set_is_merge(overlap_junction.is_merge);
    object_overlap = overlap.add_object();
    object_overlap->mutable_id()->set_id(object_id);
    if (proto_data_.pb_junctions.count(object_id) > 0) {
...
void ProtoOrganizer::GetLaneLaneOverlapElements(
    const std::unordered_map<std::pair<std::string, std::string>, OverlapWithLane, PairHash>&
        lane_lane_overlaps) {
  ...
  for (auto& overlap_lane_pair : lane_lane_overlaps) {
    ...
    if (close_set.count(unique_object_id) > 0) { continue; }
    ...
    PbObjectOverlapInfo* object_overlap = overlap.add_object();
    object_overlap->mutable_id()->set_id(lane_id);
    object_overlap->mutable_lane_overlap_info()->set_start_s(overlap_lane.start_s);
    object_overlap->mutable_lane_overlap_info()->set_end_s(overlap_lane.end_s);
    object_overlap->mutable_lane_overlap_info()->set_is_merge(overlap_lane.is_merge);
    object_overlap = overlap.add_object();
    object_overlap->mutable_id()->set_id(object_id);
    ...
    proto_data_.pb_lanes[lane_id].add_overlap_id()->set_id(overlap_id);
    auto& lane_lane_overlap = lane_lane_overlaps.at(make_pair(object_id, lane_id));
    object_overlap->mutable_lane_overlap_info()->set_start_s(lane_lane_overlap.start_s);
    object_overlap->mutable_lane_overlap_info()->set_end_s(lane_lane_overlap.end_s);
    object_overlap->mutable_lane_overlap_info()->set_is_merge(lane_lane_overlap.is_merge);
...
void ProtoOrganizer::GetJunctionObjectOverlapElements(
  ...
  for (auto& junction_internal : junctions) {
    ...
    for (auto& overlap_junction : junction_internal.overlap_with_junctions) {
      ...
      proto_data_.pb_junctions[junction_id].add_overlap_id()->set_id(overlap_id);
      proto_data_.pb_overlaps[overlap_id] = overlap;
    }
  }
}

void ProtoOrganizer::GetOverlapElements(const std::vector<RoadInternal>&     roads,
                                        const std::vector<JunctionInternal>& junctions) {
  std::unordered_map<std::pair<std::string, std::string>, OverlapWithLane, PairHash>
      lane_lane_overlaps;
  // overlap
  for (auto& road_internal : roads) {
    for (auto& road_section : road_internal.sections) {
      for (auto& lane_internal : road_section.lanes) {
        ...
        GetLaneJunctionOverlapElements(lane_id, lane_internal.overlap_junctions);
        for (auto& overlap_lane : lane_internal.overlap_lanes) {
          lane_lane_overlaps[make_pair(lane_id, overlap_lane.object_id)] = overlap_lane;
        }
      }
    }
  }
...
void ProtoOrganizer::OutputData(apollo::hdmap::Map* pb_map) {
  ...
  AINFO << "hdmap statistics: roads-" << proto_data_.pb_roads.size() << ",lanes-"
        << proto_data_.pb_lanes.size() << ",crosswalks-" << proto_data_.pb_crosswalks.size()
        << ",parking spaces-" << proto_data_.pb_parking_spaces.size() << ",clear areas-"
        << proto_data_.pb_clear_areas.size() << ",speed bumps-" << proto_data_.pb_speed_bumps.size()
        << ",signals-" << proto_data_.pb_signals.size() << ",stop signs-"
        << proto_data_.pb_stop_signs.size() << ",yield signs-" << proto_data_.pb_yield_signs.size()
        << ",pnc-junctions-" << proto_data_.pb_pnc_junctions.size() << ",junctions-"
        << proto_data_.pb_junctions.size() << ",overlaps-" << proto_data_.pb_rsus.size() << ",rsus-"
