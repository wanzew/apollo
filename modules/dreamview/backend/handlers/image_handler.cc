...
void ImageHandler::OnImage(const sensor_msgs::Image& image) {
  ...
  apollo::perception::traffic_light::Yuyv2rgb(yuv, mat.data, image.height * image.width);

  cv::cvtColor(mat, mat, CV_RGB2BGR);

  cv::resize(
      mat, mat,
      cv::Size(image.width * ImageHandler::kImageScale, image.height * ImageHandler::kImageScale),
      0, 0, CV_INTER_LINEAR);
...
void ImageHandler::OnImage(const sensor_msgs::CompressedImage& image) {
  ...
  try {
    ...
    cv::imencode(".jpg", current_image->image, send_buffer_, std::vector<int>() /* params */);
    cvar_.notify_all();
...
void ImageHandler::OnImageFront(const sensor_msgs::Image& image) {
  if (FLAGS_use_navigation_mode) { OnImage(image); }
}
