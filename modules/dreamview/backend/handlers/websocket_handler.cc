...
bool WebSocketHandler::SendData(Connection*        conn,
  ...
  // closed and removed.
  int ret;
  PERF_BLOCK(StrCat(name_, ": Writing ", data.size(), " bytes via websocket took"), 0.1) {
  ...
  if (ret != static_cast<int>(data.size())) {
    ...
    if (data.size() == 0 && ret == 2) { return true; }
    ...
    if (ret == 0) {
      ...
      msg = StrCat("Expect to send ", data.size(), " bytes. But sent ", ret, " bytes");
    }
...
bool WebSocketHandler::handleData(
  ...
  if ((bits & 0x0F) == WEBSOCKET_OPCODE_CONNECTION_CLOSE) { return false; }
  ...
  if (is_final_fragment) {
    switch (current_opcode_) {
      case WEBSOCKET_OPCODE_TEXT: result = handleJsonData(conn, data_.str()); break;
      case WEBSOCKET_OPCODE_BINARY: result = handleBinaryData(conn, data_.str()); break;
