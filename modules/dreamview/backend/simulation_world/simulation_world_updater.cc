...
void SimulationWorldUpdater::RegisterMessageHandlers() {
  ...
  map_ws_->RegisterMessageHandler(
      "RetrieveMapData", [this](const Json& json, WebSocketHandler::Connection* conn) {
  ...
  map_ws_->RegisterMessageHandler("RetrieveRelativeMapData",
  ...
  websocket_->RegisterMessageHandler(
      "Binary", [this](const std::string& data, WebSocketHandler::Connection* conn) {
  ...
  websocket_->RegisterMessageHandler("RetrieveMapElementIdsByRadius",
                                     [this](const Json& json, WebSocketHandler::Connection* conn) {
  ...
  websocket_->RegisterMessageHandler("CheckRoutingPoint",
                                     [this](const Json& json, WebSocketHandler::Connection* conn) {
  ...
  websocket_->RegisterMessageHandler(
      "SendRoutingRequest", [this](const Json& json, WebSocketHandler::Connection* conn) {
        ...
        if (succeed) {
          ...
          sim_world_service_.PublishMonitorMessage(MonitorMessageItem::ERROR,
                                                   "Failed to send a routing request.");
        }
      });
  ...
  websocket_->RegisterMessageHandler(
      ...
      [this](const Json& json, WebSocketHandler::Connection* conn) {
        ...
        if (result.contains("error")) {
          AINFO << result["error"];
          sim_world_service_.PublishMonitorMessage(MonitorMessageItem::ERROR, result["error"]);
        } else {
          auto  task                       = std::make_shared<Task>();
          auto* dead_junction_routing_task = task->mutable_dead_end_routing_task();
          bool  succeed = ConstructDeadJunctionRoutingTask(result, dead_junction_routing_task);
          if (succeed) {
            task->set_task_name("dead_end_junction_routing_task");
            task->set_task_type(apollo::task_manager::TaskType::DEAD_END_ROUTING);
            sim_world_service_.PublishTask(task);
            AINFO << task->DebugString();
            sim_world_service_.PublishMonitorMessage(MonitorMessageItem::INFO,
                                                     "dead junction routing task sent.");
          } else {
  ...
  websocket_->RegisterMessageHandler(
      ...
      [this](const Json& json, WebSocketHandler::Connection* conn) {
        ...
        if (!ContainsKey(json, "cycleNumber") || !json.find("cycleNumber")->is_number()) {
        ...
        if (succeed) {
          cycle_routing_task->set_cycle_num(static_cast<int>(json["cycleNumber"]));
          ...
          sim_world_service_.PublishMonitorMessage(MonitorMessageItem::INFO,
                                                   "Default cycle routing request sent.");
        } else {
          sim_world_service_.PublishMonitorMessage(
              MonitorMessageItem::ERROR, "Failed to send a default cycle routing request.");
        }
      });

  websocket_->RegisterMessageHandler(
      "SendParkingRoutingRequest", [this](const Json& json, WebSocketHandler::Connection* conn) {
        auto  task                 = std::make_shared<Task>();
  ...
  websocket_->RegisterMessageHandler(
      "RequestSimulationWorld", [this](const Json& json, WebSocketHandler::Connection* conn) {
        if (!sim_world_service_.ReadyToPush()) {
          AWARN_EVERY(100) << "Not sending simulation world as the data is not ready!";
          return;
        }
  ...
  websocket_->RegisterMessageHandler("RequestRoutePath",
                                     [this](const Json& json, WebSocketHandler::Connection* conn) {
  ...
  websocket_->RegisterMessageHandler(
      "GetDefaultEndPoint", [this](const Json& json, WebSocketHandler::Connection* conn) {
        Json response;
        ...
        if (LoadPOI()) {
          for (const auto& landmark : poi_.landmark()) {
            ...
            Json parking_info = apollo::common::util::JsonUtil::ProtoToTypedJson(
  ...
  websocket_->RegisterMessageHandler(
      "GetDefaultRoutings", [this](const Json& json, WebSocketHandler::Connection* conn) {
        Json response;
        response["type"]      = "DefaultRoutings";
        response["threshold"] = FLAGS_loop_routing_end_to_start_distance_threshold;
  ...
  websocket_->RegisterMessageHandler("Reset",
                                     [this](const Json& json, WebSocketHandler::Connection* conn) {
                                       sim_world_service_.SetToClear();
                                       sim_control_->Reset();
                                     });

  websocket_->RegisterMessageHandler("Dump",
                                     [this](const Json& json, WebSocketHandler::Connection* conn) {
                                       sim_world_service_.DumpMessages();
                                     });

  websocket_->RegisterMessageHandler("ToggleSimControl",
                                     [this](const Json& json, WebSocketHandler::Connection* conn) {
  ...
  websocket_->RegisterMessageHandler("RequestDataCollectionProgress",
                                     [this](const Json& json, WebSocketHandler::Connection* conn) {
                                       auto* monitors =
                                           FuelMonitorManager::Instance()->GetCurrentMonitors();
                                       if (monitors) {
  ...
  websocket_->RegisterMessageHandler(
      "GetParkingRoutingDistance", [this](const Json& json, WebSocketHandler::Connection* conn) {
        Json response;
  ...
  websocket_->RegisterMessageHandler(
      "RequestPreprocessProgress", [this](const Json& json, WebSocketHandler::Connection* conn) {
  ...
  websocket_->RegisterMessageHandler(
      "SaveDefaultRouting", [this](const Json& json, WebSocketHandler::Connection* conn) {
        bool succeed = AddDefaultRouting(json);
        if (succeed) {
          sim_world_service_.PublishMonitorMessage(MonitorMessageItem::INFO,
                                                   "Successfully add default routing.");
          if (!default_routing_) { AERROR << "Failed to add a default routing" << std::endl; }
          Json response = JsonUtil::ProtoToTypedJson("AddDefaultRoutingPath", *default_routing_);
          websocket_->SendData(conn, response.dump());
        } else {
          sim_world_service_.PublishMonitorMessage(MonitorMessageItem::ERROR,
                                                   "Failed to add a default routing.");
        }
      });

  camera_ws_->RegisterMessageHandler("RequestCameraData",
                                     [this](const Json& json, WebSocketHandler::Connection* conn) {
                                       if (!perception_camera_updater_->IsEnabled()) { return; }
                                       std::string to_send;
...
Json SimulationWorldUpdater::GetPointJsonFromLaneWaypoint(
  ...
  if (waypoint.has_heading()) { point["heading"] = waypoint.heading(); }
  return point;
}
...
bool SimulationWorldUpdater::ConstructRoutingRequest(const Json&     json,
  ...
  if (ContainsKey(start, "heading")) {
    if (!map_service_->ConstructLaneWayPointWithHeading(start["x"], start["y"], start["heading"],
                                                        routing_request->add_waypoint())) {
      AERROR << "Failed to prepare a routing request with heading: " << start["heading"]
             << " cannot locate start point on map.";
      return false;
    }
  } else if (ContainsKey(start, "id")) {
    if (!map_service_->ConstructLaneWayPointWithLaneId(start["x"], start["y"], start["id"],
                                                       routing_request->add_waypoint())) {
      AERROR << "Failed to prepare a routing request with lane id: " << start["id"]
  ...
  if (iter != json.end() && iter->is_array()) {
    ...
    for (size_t i = 0; i < iter->size(); ++i) {
      ...
      if (!map_service_->ConstructLaneWayPoint(point["x"], point["y"], waypoint->Add())) {
  ...
  if (ContainsKey(end, "id")) {
    if (!map_service_->ConstructLaneWayPointWithLaneId(end["x"], end["y"], end["id"],
                                                       routing_request->add_waypoint())) {
      AERROR << "Failed to prepare a routing request with lane id: " << end["id"]
             << " cannot locate end point on map.";
      return false;
    }
  } else {
  ...
  if (ContainsKey(json, "parkingInfo")) {
    ...
    if (ContainsKey(json, "cornerPoints")) {
      ...
      auto* points     = requested_parking_info->mutable_corner_point()->mutable_point();
...
bool SimulationWorldUpdater::ConstructParkingRoutingTask(const Json&         json,
  ...
  bool succeed = ConstructRoutingRequest(json, routing_request);
  if (succeed) {
    parking_routing_task->set_lane_width(static_cast<double>(json["laneWidth"]));
    return true;
  }
  return false;
}
...
bool SimulationWorldUpdater::ConstructDeadJunctionRoutingTask(
  ...
  auto* routing_request_in = dead_junction_routing_task->mutable_routing_request_in();
  ...
  auto* routing_request_out = dead_junction_routing_task->mutable_routing_request_out();
...
bool SimulationWorldUpdater::ValidateCoordinate(const nlohmann::json& json) {
  ...
  if (json.find("x")->is_number() && json.find("y")->is_number()) { return true; }
...
void SimulationWorldUpdater::OnTimer() {
...

  {
    ...
    sim_world_service_.GetWireFormatString(FLAGS_sim_map_radius, &simulation_world_,
                                           &simulation_world_with_planning_data_);
    sim_world_service_.GetRelativeMap().SerializeToString(&relative_map_string_);
  }
}

bool SimulationWorldUpdater::LoadPOI() {
  if (GetProtoFromASCIIFile(EndWayPointFile(), &poi_)) { return true; }
...
bool SimulationWorldUpdater::LoadDefaultRoutings() {
  if (GetProtoFromASCIIFile(DefaultRoutingFile(), &default_routings_)) { return true; }

  AWARN << "Failed to load default routings of DefaultRoutings from " << DefaultRoutingFile();
  return false;
}
...
bool SimulationWorldUpdater::AddDefaultRouting(const Json& json) {
  ...
  if (iter != json.end() && iter->is_array()) {
    for (size_t i = 0; i < iter->size(); ++i) {
      ...
      if (ContainsKey(point, "heading")) { p->set_heading(point["heading"]); }
    }
  }
