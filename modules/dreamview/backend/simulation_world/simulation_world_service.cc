...
double CalculateAcceleration(const Point3D&                              acceleration,
  ...
  // deceleration.
  double projection = acceleration.x() * velocity.x() + acceleration.y() * velocity.y();
  ...
  if (std::signbit(projection)) { magnitude = -magnitude; }

  // Negate the value if gear is reverse
  if (gear_location == Chassis::GEAR_REVERSE) { magnitude = -magnitude; }

  return magnitude;
}

Object::DisengageType DeduceDisengageType(const Chassis& chassis) {
  if (chassis.error_code() != Chassis::NO_ERROR) { return Object::DISENGAGE_CHASSIS_ERROR; }

  switch (chassis.driving_mode()) {
    case Chassis::COMPLETE_AUTO_DRIVE: return Object::DISENGAGE_NONE;
    case Chassis::COMPLETE_MANUAL: return Object::DISENGAGE_MANUAL;
    case Chassis::AUTO_STEER_ONLY: return Object::DISENGAGE_AUTO_STEER_ONLY;
    case Chassis::AUTO_SPEED_ONLY: return Object::DISENGAGE_AUTO_SPEED_ONLY;
    case Chassis::EMERGENCY_MODE: return Object::DISENGAGE_EMERGENCY;
    default: return Object::DISENGAGE_UNKNOWN;
  }
}
...
void SetObstacleType(const PerceptionObstacle::Type    obstacle_type,
                     ...
                     Object*                           world_object) {
  if (world_object == nullptr) { return; }

  switch (obstacle_type) {
    case PerceptionObstacle::UNKNOWN: world_object->set_type(Object_Type_UNKNOWN); break;
    ...
    case PerceptionObstacle::UNKNOWN_UNMOVABLE:
      ...
      break;
    case PerceptionObstacle::PEDESTRIAN: world_object->set_type(Object_Type_PEDESTRIAN); break;
    case PerceptionObstacle::BICYCLE: world_object->set_type(Object_Type_BICYCLE); break;
    case PerceptionObstacle::VEHICLE: world_object->set_type(Object_Type_VEHICLE); break;
    default: world_object->set_type(Object_Type_VIRTUAL);
  }
...
void SetStopReason(const StopReasonCode& reason_code, Decision* decision) {
  switch (reason_code) {
    ...
    case StopReasonCode::STOP_REASON_PULL_OVER:
      ...
      break;
    default: AWARN << "Unrecognizable stop reason code:" << reason_code;
  }
}

void UpdateTurnSignal(const apollo::common::VehicleSignal& signal, Object* auto_driving_car) {
  if (signal.turn_signal() == apollo::common::VehicleSignal::TURN_LEFT) {
    auto_driving_car->set_current_signal("LEFT");
  } else if (signal.turn_signal() == apollo::common::VehicleSignal::TURN_RIGHT) {
...
SimulationWorldService::SimulationWorldService(const MapService* map_service,
  ...
  if (routing_from_file) { ReadRoutingFromFile(FLAGS_routing_response_file); }
...
void SimulationWorldService::InitReaders() {
  routing_request_reader_  = node_->CreateReader<RoutingRequest>(FLAGS_routing_request_topic);
  routing_response_reader_ = node_->CreateReader<RoutingResponse>(FLAGS_routing_response_topic);
  chassis_reader_          = node_->CreateReader<Chassis>(FLAGS_chassis_topic);
  gps_reader_              = node_->CreateReader<Gps>(FLAGS_gps_topic);
  localization_reader_     = node_->CreateReader<LocalizationEstimate>(FLAGS_localization_topic);
  ...
  perception_traffic_light_reader_ =
      node_->CreateReader<TrafficLightDetection>(FLAGS_traffic_light_detection_topic);
  prediction_obstacle_reader_ = node_->CreateReader<PredictionObstacles>(FLAGS_prediction_topic);
  planning_reader_            = node_->CreateReader<ADCTrajectory>(FLAGS_planning_trajectory_topic);
  control_command_reader_     = node_->CreateReader<ControlCommand>(FLAGS_control_command_topic);
  navigation_reader_          = node_->CreateReader<NavigationInfo>(FLAGS_navigation_topic);
  ...
  audio_detection_reader_     = node_->CreateReader<AudioDetection>(FLAGS_audio_detection_topic);

  audio_event_reader_ = node_->CreateReader<AudioEvent>(
      FLAGS_audio_event_topic, [this](const std::shared_ptr<AudioEvent>& audio_event) {
  ...
  drive_event_reader_ = node_->CreateReader<DriveEvent>(
      FLAGS_drive_event_topic, [this](const std::shared_ptr<DriveEvent>& drive_event) {
        this->PublishMonitorMessage(MonitorMessageItem::WARN, drive_event->event());
      });
  cyber::ReaderConfig monitor_message_reader_config;
  monitor_message_reader_config.channel_name       = FLAGS_monitor_topic;
  monitor_message_reader_config.pending_queue_size = FLAGS_monitor_msg_pending_queue_size;
...
void SimulationWorldService::InitWriters() {
  navigation_writer_ = node_->CreateWriter<NavigationInfo>(FLAGS_navigation_topic);
  ...
  {  // configure QoS for routing request writer
    ...
    routing_request_writer_ = node_->CreateWriter<RoutingRequest>(routing_request_attr);
  }

  routing_response_writer_ = node_->CreateWriter<RoutingResponse>(FLAGS_routing_response_topic);
...
void SimulationWorldService::UpdateDelays() {
  ...
  delays->set_perception_obstacle(SecToMs(perception_obstacle_reader_->GetDelaySec()));
  ...
  delays->set_traffic_light(SecToMs(perception_traffic_light_reader_->GetDelaySec()));
...
void SimulationWorldService::GetWireFormatString(double       radius,
...
void SimulationWorldService::GetMapElementIds(double radius, MapElementIds* ids) const {
...
const Map& SimulationWorldService::GetRelativeMap() const { return relative_map_; }

template <>
void SimulationWorldService::UpdateSimulationWorld(const LocalizationEstimate& localization) {
  Object*     auto_driving_car = world_.mutable_auto_driving_car();
  const auto& pose             = localization.pose();

  // Updates position with the input localization message.
  auto_driving_car->set_position_x(pose.position().x() + map_service_->GetXOffset());
  auto_driving_car->set_position_y(pose.position().y() + map_service_->GetYOffset());
  auto_driving_car->set_heading(pose.heading());

  // Updates acceleration with the input localization message.
  auto_driving_car->set_speed_acceleration(
      CalculateAcceleration(pose.linear_acceleration(), pose.linear_velocity(), gear_location_));
...
void SimulationWorldService::UpdateSimulationWorld(const Gps& gps) {
  if (gps.header().module_name() == "ShadowLocalization") {
    ...
    shadow_localization_position->set_position_x(pose.position().x() + map_service_->GetXOffset());
    shadow_localization_position->set_position_y(pose.position().y() + map_service_->GetYOffset());
    ...
    const auto& pose = gps.localization();
    gps_position->set_position_x(pose.position().x() + map_service_->GetXOffset());
    gps_position->set_position_y(pose.position().y() + map_service_->GetYOffset());
...
void SimulationWorldService::UpdateSimulationWorld(const Chassis& chassis) {
  ...
  if (gear_location_ == Chassis::GEAR_REVERSE) { speed = -speed; }
  ...
  if (angle_percentage > 100 || angle_percentage < -100) { angle_percentage = 0; }
  auto_driving_car->set_steering_percentage(angle_percentage);

  double steering_angle = angle_percentage / 100.0 * vehicle_param.max_steer_angle();
  auto_driving_car->set_steering_angle(steering_angle);

  double kappa =
      std::tan(steering_angle / vehicle_param.steer_ratio()) / vehicle_param.wheel_base();
...
template <>
void SimulationWorldService::UpdateSimulationWorld(const AudioDetection& audio_detection) {
  world_.set_is_siren_on(audio_detection.is_siren());
}

Object& SimulationWorldService::CreateWorldObjectIfAbsent(const PerceptionObstacle& obstacle) {
...
void SimulationWorldService::CreateWorldObjectFromSensorMeasurement(const SensorMeasurement& sensor,
                                                                    Object* world_object) {
...
void SimulationWorldService::SetObstaclePolygon(const PerceptionObstacle& obstacle,
                                                Object*                   world_object) {
  if (world_object == nullptr) { return; }
...
void SimulationWorldService::SetObstacleSensorMeasurements(const PerceptionObstacle& obstacle,
                                                           Object*                   world_object) {
  if (world_object == nullptr) { return; }
  for (const auto& sensor : obstacle.measurements()) {
    Object* obj =
        (*(world_.mutable_sensor_measurements()))[sensor.sensor_id()].add_sensor_measurement();
...
void SimulationWorldService::SetObstacleSource(
  ...
  if (world_object == nullptr || !obstacle.has_source()) { return; }
...
void SimulationWorldService::UpdateSimulationWorld(const PerceptionObstacles& obstacles) {
  for (const auto& obstacle : obstacles.perception_obstacle()) {
    ...
    if (obstacles.has_cipv_info() && (obstacles.cipv_info().cipv_id() == obstacle.id())) {
...
void SimulationWorldService::UpdatePlanningTrajectory(const ADCTrajectory& trajectory) {
  // Collect trajectory
  ...
  for (const TrajectoryPoint& point : trajectory.trajectory_point()) {
    ...
    trajectory_point->set_position_x(point.path_point().x() + map_service_->GetXOffset());
    trajectory_point->set_position_y(point.path_point().y() + map_service_->GetYOffset());
  ...
  if (trajectory.has_engage_advice()) {
    world_.set_engage_advice(EngageAdvice_Advice_Name(trajectory.engage_advice().advice()));
  }
}
...
void SimulationWorldService::UpdateRSSInfo(const ADCTrajectory& trajectory) {
  if (trajectory.has_rss_info()) {
    if (trajectory.rss_info().is_rss_safe()) {
      ...
      const double next_rss_safe_dist = trajectory.rss_info().rss_safe_dist_lon();
      // Not update RSS message if data keeps same.
      if (std::fabs(current_real_dist_ - next_real_dist) < common::math::kMathEpsilon &&
          std::fabs(current_rss_safe_dist_ - next_rss_safe_dist) < common::math::kMathEpsilon) {
        return;
      }
...
void SimulationWorldService::UpdateMainStopDecision(
  ...
  if (main_decision.has_not_ready()) {
    ...
    if (stop.has_reason_code()) { SetStopReason(stop.reason_code(), decision); }
  }
...
bool SimulationWorldService::LocateMarker(const apollo::planning::ObjectDecisionType& decision,
...
void SimulationWorldService::FindNudgeRegion(const apollo::planning::ObjectDecisionType& decision,
  ...
  const std::vector<apollo::common::math::Vec2d>& nudge_points = nudge_polygon.points();
...
void SimulationWorldService::UpdateDecision(const DecisionResult& decision_res,
                                            double                header_time) {
  // Update turn signal.
  UpdateTurnSignal(decision_res.vehicle_signal(), world_.mutable_auto_driving_car());
  ...
  if (main_decision.has_not_ready() || main_decision.has_estop() || main_decision.has_stop()) {
  ...
  for (const auto& obj_decision : decision_res.object_decision().decision()) {
    if (obj_decision.has_perception_id()) {
        ...
        if (decision.has_stop() || decision.has_follow() || decision.has_yield() ||
          ...
          if (decision.has_stop()) {
            ...
            for (auto obstacle_id : decision.stop().wait_for_obstacle()) {
              const std::vector<std::string> id_segments = absl::StrSplit(obstacle_id, '_');
              if (id_segments.size() > 0) { obj_map_[id_segments[0]].set_yielded_obstacle(true); }
            }
          }
        } else if (decision.has_nudge()) {
          if (world_obj.polygon_point().empty()) {
            if (world_obj.type() == Object_Type_VIRTUAL) {
              AWARN << "No current perception object with id=" << id << " for nudge decision";
            } else {
      ...
      world_obj.set_timestamp_sec(std::max(world_obj.timestamp_sec(), header_time));
    }
  }
}
...
void SimulationWorldService::DownsamplePath(const common::Path& path,
  ...
  for (const size_t index : sampled_indices) {
    *downsampled_path->add_path_point() = path.path_point(static_cast<int>(index));
  }
}
...
void SimulationWorldService::UpdatePlanningData(const PlanningData& data) {
  ...
  // Update scenario
  if (data.has_scenario()) { planning_data->mutable_scenario()->CopyFrom(data.scenario()); }

  // Update init point
  if (data.has_init_point()) {
    auto& planning_path_point  = data.init_point().path_point();
    auto* world_obj_path_point = planning_data->mutable_init_point()->mutable_path_point();
    world_obj_path_point->set_x(planning_path_point.x() + map_service_->GetXOffset());
    world_obj_path_point->set_y(planning_path_point.y() + map_service_->GetYOffset());
  ...
  for (auto& graph : data.st_graph()) {
    ...
    if (graph.has_kernel_cruise_ref()) {
      st_graph->mutable_kernel_cruise_ref()->CopyFrom(graph.kernel_cruise_ref());
    }
    if (graph.has_kernel_follow_ref()) {
      st_graph->mutable_kernel_follow_ref()->CopyFrom(graph.kernel_follow_ref());
    }
    ...
    // max 10 points
    size_t profile_downsample_interval = std::max(1, (graph.speed_profile_size() / 200));
    profile_downsample_interval        = std::min(profile_downsample_interval, max_interval);
    DownsampleSpeedPointsByInterval(graph.speed_profile(), profile_downsample_interval,
                                    st_graph->mutable_speed_profile());

    size_t limit_downsample_interval = std::max(1, (graph.speed_limit_size() / 200));
    limit_downsample_interval        = std::min(limit_downsample_interval, max_interval);
    DownsampleSpeedPointsByInterval(graph.speed_limit(), limit_downsample_interval,
  ...
  if (data.has_pull_over()) { planning_data->mutable_pull_over()->CopyFrom(data.pull_over()); }
  ...
  if (data.has_signal_light() && data.signal_light().signal_size() > 0) {
    ...
    for (auto& signal : data.signal_light().signal()) {
      switch (signal.color()) {
        ...
        case TrafficLight::BLACK: current_signal = signal.color(); break;
        case TrafficLight::GREEN: green_light_count++; break;
        default: break;
      }
    }
    ...
    world_.mutable_traffic_signal()->set_current_signal(TrafficLight_Color_Name(current_signal));
  }
}

template <>
void SimulationWorldService::UpdateSimulationWorld(const ADCTrajectory& trajectory) {
...
void SimulationWorldService::CreatePredictionTrajectory(const PredictionObstacle& obstacle,
                                                        Object*                   world_object) {
  for (const auto& traj : obstacle.trajectory()) {
    ...
    for (auto index : sampled_indices) {
      ...
      if (traj_point.has_gaussian_info()) {
        const apollo::common::GaussianInfo& gaussian = traj_point.gaussian_info();
...
template <>
void SimulationWorldService::UpdateSimulationWorld(const PredictionObstacles& obstacles) {
  for (const auto& obstacle : obstacles.prediction_obstacle()) {
    ...
    world_obj.set_timestamp_sec(std::max(obstacle.timestamp(), world_obj.timestamp_sec()));
  }
}

template <>
void SimulationWorldService::UpdateSimulationWorld(const RoutingResponse& routing_response) {
  {
  ...
  if (!map_service_->GetPathsFromRouting(routing_response, &paths)) { return; }
  ...
  for (const Path& path : paths) {
    ...
    auto sampled_indices = DownsampleByAngle(path.path_points(), kAngleThreshold);
...
Json SimulationWorldService::GetRoutePathAsJson() const {
  ...
  for (const auto& route_path : route_paths) {
    ...
    for (const auto& route_point : route_path.point()) {
      path["point"].push_back(
          {{"x", route_point.x()}, {"y", route_point.y()}, {"z", route_point.z()}});
    }
...
void SimulationWorldService::ReadRoutingFromFile(const std::string& routing_response_file) {
  auto routing_response = std::make_shared<RoutingResponse>();
  if (!GetProtoFromFile(routing_response_file, routing_response.get())) {
    AWARN << "Unable to read routing response from file: " << routing_response_file;
    return;
  }
...
template <>
void SimulationWorldService::UpdateSimulationWorld(const ControlCommand& control_command) {
...
template <>
void SimulationWorldService::UpdateSimulationWorld(const NavigationInfo& navigation_info) {
...
template <>
void SimulationWorldService::UpdateSimulationWorld(const MonitorMessage& monitor_msg) {
  const int updated_size =
      std::min(monitor_msg.item_size(), SimulationWorldService::kMaxMonitorItems);
  ...
  int remove_size = world_.notification_size() - SimulationWorldService::kMaxMonitorItems;
  if (remove_size > 0) {
    auto* notifications = world_.mutable_notification();
    notifications->erase(notifications->begin(), notifications->begin() + remove_size);
  }
}
...
void SimulationWorldService::PublishMonitorMessage(
    apollo::common::monitor::MonitorMessageItem::LogLevel log_level, const std::string& msg) {
