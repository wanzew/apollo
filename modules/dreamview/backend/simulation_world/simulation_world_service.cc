...
double CalculateAcceleration(const Point3D& acceleration, const Point3D& velocity) {
...
void DownsampleCurve(Curve* curve) {
  if (curve->segment_size() == 0) { return; }
...
void SimulationWorldService::Update() {
  ...
  obj_map_.clear();
  world_.clear_object();
  UpdateWithLatestObserved("PerceptionObstacles", AdapterManager::GetPerceptionObstacles());
  UpdateWithLatestObserved("PerceptionTrafficLight", AdapterManager::GetTrafficLightDetection(),
                           false);
  UpdateWithLatestObserved("PredictionObstacles", AdapterManager::GetPrediction());
  UpdateWithLatestObserved("Planning", AdapterManager::GetPlanning());
  UpdateWithLatestObserved("ControlCommand", AdapterManager::GetControlCommand());
...
void SimulationWorldService::UpdateDelays() {
  ...
  delays->set_localization(SecToMs(AdapterManager::GetLocalization()->GetDelaySec()));
  delays->set_perception_obstacle(SecToMs(AdapterManager::GetPerceptionObstacles()->GetDelaySec()));
  delays->set_planning(SecToMs(AdapterManager::GetPlanning()->GetDelaySec()));
  delays->set_prediction(SecToMs(AdapterManager::GetPrediction()->GetDelaySec()));
  delays->set_traffic_light(SecToMs(AdapterManager::GetTrafficLightDetection()->GetDelaySec()));
  delays->set_control(SecToMs(AdapterManager::GetControlCommand()->GetDelaySec()));
}
...
void SimulationWorldService::UpdateSimulationWorld(const LocalizationEstimate& localization) {
  ...
  auto_driving_car->set_speed_acceleration(
      CalculateAcceleration(pose.linear_acceleration(), pose.linear_velocity()));
...
void SimulationWorldService::UpdateSimulationWorld(const Chassis& chassis) {
  ...
  double kappa = std::tan(steering_angle / vehicle_param.steer_ratio()) / vehicle_param.length();
...
void SimulationWorldService::UpdateSimulationWorld(
  ...
  if (traffic_light_detection.traffic_light_size() > 0) {
    ...
    signal->set_current_signal(apollo::perception::TrafficLight_Color_Name(traffic_light.color()));
  } else {
...
void SimulationWorldService::UpdatePlanningTrajectory(const ADCTrajectory& trajectory) {
  ...
  for (const TrajectoryPoint& point : trajectory.trajectory_point()) {
    ...
    if (collecting_started || point.relative_time() + header_time >= cutoff_time) {
...
void SimulationWorldService::UpdateSimulationWorld(const ADCTrajectory& trajectory) {
  ...
  world_.mutable_latency()->set_planning(trajectory.latency_stats().total_time_ms());
}
...
template <>
void SimulationWorldService::UpdateSimulationWorld(const DriveEvent& drive_event) {
...
void SimulationWorldService::RegisterMessageCallbacks() {
  AdapterManager::AddDriveEventCallback(&SimulationWorldService::UpdateSimulationWorld, this);
  AdapterManager::AddMonitorCallback(&SimulationWorldService::UpdateSimulationWorld, this);
  AdapterManager::AddRoutingResponseCallback(&SimulationWorldService::UpdateSimulationWorld, this);
}
