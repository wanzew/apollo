...
class AllLatest : public DataFusion<M0, M1, M2, M3> {
  using FusionDataType = std::
      tuple<std::shared_ptr<M0>, std::shared_ptr<M1>, std::shared_ptr<M2>, std::shared_ptr<M3>>;
 ...
 public:
  AllLatest(const ChannelBuffer<M0>& buffer_0,
    ...
    buffer_m0_.Buffer()->SetFusionCallback([this](const std::shared_ptr<M0>& m0) {
      ...
      if (!buffer_m1_.Latest(m1) || !buffer_m2_.Latest(m2) || !buffer_m3_.Latest(m3)) { return; }
