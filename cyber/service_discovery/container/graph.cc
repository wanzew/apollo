...
Vertice::Vertice(const std::string& val)
    : value_(val) {}
...
FlowDirection Graph::GetDirectionOf(const Vertice& lhs, const Vertice& rhs) {
  ...
  ReadLockGuard<AtomicRWLock> lock(rw_lock_);
  if (list_.count(lhs.GetKey()) == 0 || list_.count(rhs.GetKey()) == 0) { return UNREACHABLE; }
...
void Graph::InsertCompleteEdge(const Edge& e) {
  ...
  if (list_.find(src_v_k) == list_.end()) { list_[src_v_k] = VerticeSet(); }
  auto& dst_v_k = e.dst().GetKey();
  if (list_.find(dst_v_k) == list_.end()) { list_[dst_v_k] = VerticeSet(); }
