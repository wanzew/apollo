...
LibPathSharedLibVector::iterator FindLoadedLibrary(const std::string& library_path) {
  ...
  for (itr = opened_libraries.begin(); itr != opened_libraries.end(); ++itr) {
    if (itr->first == library_path) { break; }
  }
  return itr;
}
...
bool IsLibraryLoaded(const std::string& library_path, ClassLoader* class_loader) {
  ...
  if (is_lib_loaded_by_anyone && num_lib_class_factory_objs == 0) { return true; }
...
bool LoadLibrary(const std::string& library_path, ClassLoader* loader) {
  ...
  if (num_lib_objs == 0) { AWARN << "Class factory objs counts is 0, maybe registerclass failed."; }
...
void UnloadLibrary(const std::string& library_path, ClassLoader* loader) {
  {
    ...
    } catch (const std::runtime_error& e) { AERROR << "Library unload error: " << e.what(); }
  }
}
