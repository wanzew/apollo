...
bool RecordWriter::WriteMessage(const SingleMessage& message) {
  ...
  if (segment_begin_time_ == 0) { segment_begin_time_ = message.time(); }
  if (segment_begin_time_ > message.time()) { segment_begin_time_ = message.time(); }
...
void RecordWriter::OnNewMessage(const std::string& channel_name) {
  ...
  if (iter != channel_message_number_map_.end()) { iter->second++; }
}
...
const std::string& RecordWriter::GetMessageType(const std::string& channel_name) const {
  ...
  if (search != channel_message_type_map_.end()) { return search->second; }
  return kEmptyString;
}
...
const std::string& RecordWriter::GetProtoDesc(const std::string& channel_name) const {
  ...
  if (search != channel_proto_desc_map_.end()) { return search->second; }
  return kEmptyString;
}
