...
struct Chunk {
  ...
  inline void add(const proto::SingleMessage& message) {
    ...
    *p_message                            = message;
    if (header_.begin_time() == 0) { header_.set_begin_time(message.time()); }
    if (header_.begin_time() > message.time()) { header_.set_begin_time(message.time()); }
    if (header_.end_time() < message.time()) { header_.set_end_time(message.time()); }
...
bool RecordFileWriter::WriteSection(const T& message) {
  ...
  if (count != sizeof(section)) {
    AERROR << "Write fd failed, fd: " << fd_ << ", expect count: " << sizeof(section)
           << ", actual count: " << count;
    return false;
  }
  {
