...
class ReentrantRWLock {
 ...
 public:
  ...
  ReentrantRWLock() {}
  explicit ReentrantRWLock(bool write_first)
...
inline void ReentrantRWLock::ReadLock() {
  if (write_thread_id_ == std::this_thread::get_id()) { return; }
...
inline void ReentrantRWLock::WriteUnlock() {
  if (lock_num_.fetch_add(1) == WRITE_EXCLUSIVE) { write_thread_id_ = NULL_THREAD_ID; }
}
