...
template <typename T>
CCObjectPool<T>::CCObjectPool(uint32_t size)
    : capacity_(size) {
...
bool CCObjectPool<T>::FindFreeHead(Head* head) {
  ...
  do {
    if (cyber_unlikely(old_head.node == nullptr)) { return false; }
    new_head.node  = old_head.node->next;
    new_head.count = old_head.count + 1;
  } while (!free_head_.compare_exchange_weak(old_head, new_head, std::memory_order_acq_rel,
...
std::shared_ptr<T> CCObjectPool<T>::GetObject() {
  Head free_head;
  if (cyber_unlikely(!FindFreeHead(&free_head))) { return nullptr; }
