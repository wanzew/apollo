...
bool AttributesFiller::FillInPubAttr(const std::string&                       channel_name,
  ...
  if (pub_attr->topic.historyQos.depth < 0) { return false; }
  ...
  pub_attr->historyMemoryPolicy                 = eprosima::fastrtps::DYNAMIC_RESERVE_MEMORY_MODE;
  pub_attr->topic.resourceLimitsQos.max_samples = 10000;

  return true;
}

bool AttributesFiller::FillInSubAttr(const std::string&                        channel_name,
                                     const QosProfile&                         qos,
  ...
  // ensure the history depth is at least the requested queue size
  if (sub_attr->topic.historyQos.depth < 0) { return false; }

  sub_attr->historyMemoryPolicy                 = eprosima::fastrtps::DYNAMIC_RESERVE_MEMORY_MODE;
