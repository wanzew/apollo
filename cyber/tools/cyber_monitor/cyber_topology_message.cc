...
CyberTopologyMessage::~CyberTopologyMessage(void) {
  for (auto item : all_channels_map_) {
    if (!GeneralChannelMessage::IsErrorCode(item.second)) { delete item.second; }
  }
}
...
void CyberTopologyMessage::TopologyChanged(const apollo::cyber::proto::ChangeMsg& changeMsg) {
  if (::apollo::cyber::proto::OperateType::OPT_JOIN == changeMsg.operate_type()) {
    bool isWriter = true;
    if (::apollo::cyber::proto::RoleType::ROLE_READER == changeMsg.role_type()) isWriter = false;
...
void CyberTopologyMessage::AddReaderWriter(const apollo::cyber::proto::RoleAttributes& role,
  ...
  if (!specified_channel_.empty() && specified_channel_ != channel_name) { return; }
  ...
  if (IsFromHere(node_name)) { return; }
  ...
  if (!GeneralChannelMessage::IsErrorCode(channel_msg)) {
    if (isWriter) {
      if (msgTypeName !=
          apollo::cyber::message::MessageType<apollo::cyber::message::RawMessage>()) {
        channel_msg->set_message_type(msgTypeName);
      }
...
void CyberTopologyMessage::ChangeState(const Screen* s, int key) {
  switch (key) {
    case 'f':
    case 'F': second_column_ = SecondColumnType::MessageFrameRatio; break;

    case 't':
    case 'T': second_column_ = SecondColumnType::MessageType; break;
