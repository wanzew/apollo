  ...
  for (std::size_t i = 0; i < fields.size(); ++i) {
    ...
    if (field->is_repeated()) {
      if (*jump_lines) {
        ...
        if (item) { baseMsg->InsertRepeatedMessage(*line_no, item); }
...
#define OUTPUT_FIELD(CPPTYPE, METHOD, PRECISION)                                                   \
  case google::protobuf::FieldDescriptor::CPPTYPE_##CPPTYPE:                                       \
    if (*jump_lines) {                                                                             \
      ...
      const std::string& fieldName = field->name();                                                \
      out_str << fieldName << ": ";                                                                \
      if (field->is_repeated()) { out_str << "[" << index << "] "; }                               \
      old_flags = out_str.flags();                                                                 \
      out_str << std::fixed << std::setprecision(PRECISION)                                        \
              << (field->is_repeated() ? ref->GetRepeated##METHOD(msg, field, index) :             \
...
#undef OUTPUT_FIELD
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_STRING: {
      std::string        scratch;
      const std::string& str = field->is_repeated() ?
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_ENUM: {
      if (*jump_lines) {
        ...
        out_str << fieldName << ": ";
        if (field->is_repeated()) { out_str << "[" << index << "] "; }
        int enum_value = field->is_repeated() ? ref->GetRepeatedEnumValue(msg, field, index) :
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE:
      ...
      GeneralMessageBase::PrintMessage(baseMsg,
                                       field->is_repeated() ?
...
RenderableMessage* GeneralMessageBase::Child(int line_no) const {
  if (line_no < 0) { return nullptr; }
  auto iter = children_map_.find(line_no);
  if (iter == children_map_.cend()) { return nullptr; }
