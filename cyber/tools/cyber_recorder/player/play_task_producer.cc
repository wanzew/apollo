...
PlayTaskProducer::PlayTaskProducer(const TaskBufferPtr& task_buffer, const PlayParam& play_param)
...
bool PlayTaskProducer::ReadRecordInfo() {
  ...
  for (auto& file : play_param_.files_to_play) {
    ...
    if (!record_reader->IsValid()) { continue; }
    ...
    if (play_param_.is_play_all_channels) { total_msg_num_ += header.message_number(); }

    if (header.begin_time() < earliest_begin_time_) { earliest_begin_time_ = header.begin_time(); }
    if (header.end_time() > latest_end_time_) { latest_end_time_ = header.end_time(); }
  ...
  std::cout << "earliest_begin_time: " << earliest_begin_time_
            << ", latest_end_time: " << latest_end_time_ << ", total_msg_num: " << total_msg_num_
            << std::endl;
...
bool PlayTaskProducer::UpdatePlayParam() {
  ...
  if (play_param_.start_time_s > 0) {
    play_param_.begin_time_ns +=
        static_cast<uint64_t>(static_cast<double>(play_param_.start_time_s) * 1e9);
  }
  if (play_param_.end_time_ns > latest_end_time_) { play_param_.end_time_ns = latest_end_time_; }
...
void PlayTaskProducer::ThreadFunc() {
  ...
  if (total_msg_num_ > 0) { avg_interval_time_ns = loop_time_ns / total_msg_num_; }

  double avg_freq_hz =
      static_cast<double>(total_msg_num_) / (static_cast<double>(loop_time_ns) * 1e-9);
  uint32_t preload_size = (uint32_t)avg_freq_hz * play_param_.preload_time_s;
  AINFO << "preload_size: " << preload_size;
  if (preload_size < kMinTaskBufferSize) { preload_size = kMinTaskBufferSize; }

  auto record_viewer =
      std::make_shared<RecordViewer>(record_readers_, play_param_.begin_time_ns,
  ...
  while (!is_stopped_.load()) {
    ...
    while (itr != itr_end && !is_stopped_.load()) {
      ...
      for (; itr != itr_end && !is_stopped_.load(); ++itr) {
        if (task_buffer_->Size() > preload_size) { break; }

        auto search = writers_.find(itr->channel_name);
        if (search == writers_.end()) { continue; }
